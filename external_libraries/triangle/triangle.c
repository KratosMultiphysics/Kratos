...
std::size_t randomseed;                     /* Current random number seed. */
...
/* Other variables. */
  ...
  std::ptrdiff_t  edges;                                     /* Number of output edges. */
  int mesh_dim;                                /* Dimension (ought to be 2). */
  int nextras;                           /* Number of attributes per vertex. */
  int eextras;                         /* Number of attributes per triangle. */
  std::ptrdiff_t  hullsize;                          /* Number of edges in convex hull. */
  ...
  std::ptrdiff_t  samples;              /* Number of random samples for point location. */

  std::ptrdiff_t  incirclecount;                 /* Number of incircle tests performed. */
  std::ptrdiff_t  counterclockcount;     /* Number of counterclockwise tests performed. */
  std::ptrdiff_t  orient3dcount;           /* Number of 3D orientation tests performed. */
  std::ptrdiff_t  hyperbolacount;      /* Number of right-of-hyperbola tests performed. */
  std::ptrdiff_t  circumcentercount;  /* Number of circumcenter calculations performed. */
  std::ptrdiff_t  circletopcount;       /* Number of circle top calculations performed. */
...
/*  triangles are four-byte-aligned and that the `std::size_t' type is     */
/*  std::ptrdiff_t  enough to hold a pointer are two of the few kludges in this program.*/
/*                                                                           */
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((std::size_t) (ptr) & (std::size_t) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((std::size_t) (ptr) ^ (std::size_t) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((std::size_t) (otri).tri | (std::size_t) (otri).orient)
...
#define infect(otri)                                                          \
  (otri).tri[6] = (triangle)                                                  \
                    ((std::size_t) (otri).tri[6] | (std::size_t) 2l)
...
#define infected(otri)                                                        \
  (((std::size_t) (otri).tri[6] & (std::size_t) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((std::size_t) (sptr) & (std::size_t) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((std::size_t) (sptr) & ~ (std::size_t) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((std::size_t) (osub).ss | (std::size_t) (osub).ssorient)
...
"    Triangle calculates the intersection points for you and adds them to\n");
  printf(
"    the triangulation--as std::ptrdiff_t  as your machine's floating-point precision\n");
  printf(
...
"  row of evenly spaced, segment-connected vertices?  Have you simply\n");
  printf(
"  defined one std::ptrdiff_t  segment connecting the leftmost vertex to the rightmost\n"
);
  printf(
...

{
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (std::size_t) printvertex,
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (std::size_t) printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
    ...
    printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
           t->orient + 3, (std::size_t) printvertex,
           printvertex[0], printvertex[1]);

  if (b->usesegments) {
    sdecode(t->tri[6], printsh);
    if (printsh.ss != m->dummysub) {
      printf("    [6] = x%lx  %d\n", (std::size_t) printsh.ss,
             printsh.ssorient);
    }
...

{
  ...
  printf("subsegment x%lx with orientation %d and mark %d:\n",
         (std::size_t) s->ss, s->ssorient, mark(*s));
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           2 + s->ssorient, (std::size_t) printvertex,
           printvertex[0], printvertex[1]);
  sdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Dest  [%d] = NULL\n", 3 - s->ssorient);
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           3 - s->ssorient, (std::size_t) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
           4 + s->ssorient, (std::size_t) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
           5 - s->ssorient, (std::size_t) printvertex,
           printvertex[0], printvertex[1]);
}
...

{
  ...
  alignptr = (std::size_t) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
    (alignptr + (std::size_t) pool->alignbytes -
     (alignptr % (std::size_t) pool->alignbytes));
...

{
  ...
  alignptr = (std::size_t) (pool->pathblock + 1);
...

{
  ...
  alignptr = (std::size_t) m->dummytribase;
  m->dummytri = (triangle *)
    (alignptr + (std::size_t) m->triangles.alignbytes -
     (alignptr % (std::size_t) m->triangles.alignbytes));
  /* Initialize the three adjoining triangles to be "outer space."  These  */
  /*   will eventually be changed by various bonding operations, but their */
  /*   values don't really matter, as std::ptrdiff_t  as they can legally be          */
  /*   dereferenced.                                                       */
  ...
  if (b->usesegments) {
    ...
    alignptr = (std::size_t) m->dummysubbase;
    m->dummysub = (subseg *)
      (alignptr + (std::size_t) m->subsegs.alignbytes -
       (alignptr % (std::size_t) m->subsegs.alignbytes));
    /* Initialize the two adjoining subsegments to be the omnipresent      */
    /*   subsegment.  These will eventually be changed by various bonding  */
    /*   operations, but their values don't really matter, as std::ptrdiff_t  as they */
...

{
  ...
  alignptr = (std::size_t) (getblock + 1);
  foundvertex = (char *) (alignptr + (std::size_t) m->vertices.alignbytes -
                          (alignptr % (std::size_t) m->vertices.alignbytes));
...
#ifdef ANSI_DECLARATORS
std::size_t randomnation(unsigned int choices)
#else /* not ANSI_DECLARATORS */
std::size_t randomnation(choices)
...
/*  std::ptrdiff_t  as the search is begun from the triangle in question.               */
/*                                                                           */
/*****************************************************************************/
...

{
  ...
  REAL ahead;
  std::ptrdiff_t  samplesperblock, totalsamplesleft, samplesleft;
  std::ptrdiff_t  population, totalpopulation;
  triangle ptr;                         /* Temporary variable used by sym(). */
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (std::size_t) (sampleblock + 1);
    firsttri = (char *) (alignptr +
                         (std::size_t) m->triangles.alignbytes -
                         (alignptr %
                          (std::size_t) m->triangles.alignbytes));
        ...
        if (doflip) {
          ...
          if (b->vararea) {
            if ((areabound(top) <= 0.0) || (areabound(horiz) <= 0.0)) {
              ...
              /*   long, std::ptrdiff_t  way from their original location due to flips. */
...
/*  std::ptrdiff_t  as the fan remains connected to them.                               */
/*                                                                           */
...
/*  there's a lot of std::ptrdiff_t  segments that each cut many triangles.  I ought to */
...

{
  ...
  std::ptrdiff_t  hullsize;
  int notfound;
  std::ptrdiff_t  elementnumber, segmentnumber;
  int i, j;
  triangle ptr;                         /* Temporary variable used by sym(). */
...
#else /* not TRILIBRARY */
  FILE *outfile;
  std::ptrdiff_t  holenumber, regionnumber;
#endif /* not TRILIBRARY */
  struct osub subsegloop;
  vertex endpoint1, endpoint2;
  std::ptrdiff_t  subsegnumber;
...

{
  ...
  REAL circumcenter[2];
  REAL xi, eta;
  std::ptrdiff_t  vnodenumber, vedgenumber;
  int p1, p2;
  int i;
  triangle ptr;                         /* Temporary variable used by sym(). */
