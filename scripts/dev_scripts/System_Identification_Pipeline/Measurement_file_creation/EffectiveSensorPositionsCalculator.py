...
class EffectiveSensorPositionsCalculator():
    ...
    def run_primal_analysis(self, primal_settings_file_name: str, model_file_name: str) -> Kratos.Model:
    ...
    def calculate_displacement_to_young_modulus_sensitivity_matrix(self, model_part: Kratos.ModelPart, model_file_name: str, primal_settings_file_name: str, adjoint_settings_file_name: str, potential_positions_model_part: str, sensor_direction: "Kratos.Vector") -> "(np.ndarray, list(int))":
        ...
        primal_model = self.run_primal_analysis(primal_settings_file_name=primal_settings_file_name, model_file_name=model_file_name)
        ...
        displacement_expression = Kratos.Expression.NodalExpression(primal_model.GetModelPart("Structure"))
        Kratos.Expression.VariableExpressionIO.Read(displacement_expression,Kratos.DISPLACEMENT,True)
        displacement_vector = displacement_expression.Evaluate()
        displacement_vector = displacement_vector @ np.array([sensor_direction[0],sensor_direction[1],sensor_direction[2]])
        displacement_vector /= np.max(np.abs(displacement_vector))
        ...
        adjoint_parameters["solver_settings"]["response_function_settings"]["direction"].SetVector(sensor_direction)
        ...
        for i, node_id in enumerate(node_ids_of_potential_sensor_positions):
            ...
            # self.excluded_element_ids = [6, 7, 9, 8, 68, 5, 28, 29, 30, 84, 1, 2]
            # self._SetSensitivitiesToZero(adjoint_young_modulus_sensitivity, self.excluded_element_ids)

            sensitivities = adjoint_young_modulus_sensitivity.Evaluate()
            sensitivities *= displacement_vector[node_id-1]
    ...
    def output_vector_as_vtu(self, file_name: str, vector: np.ndarray, model_part: Kratos.ModelPart) -> None:
        ...
        if len(model_part.GetElements()) == vector.size:
        ...
        elif len(model_part.GetNodes()) == vector.size:
            nodal_expression = Kratos.Expression.NodalExpression(model_part)
            Kratos.Expression.CArrayExpressionIO.Read(nodal_expression, vector)

            expression = Kratos.Expression.ElementExpression(model_part)

            KratosOA.ExpressionUtils.MapNodalVariableToContainerVariable(expression, nodal_expression)

            # KratosOA.PropertiesVariableExpressionIO.Write(expression, Kratos.DISPLACEMENT_X)

            # vtu_output_process = Kratos.VtuOutput(model_part, True, Kratos.VtuOutput.BINARY, 9)
            # vtu_output_process.AddNonHistoricalVariable(Kratos.PRESSURE, Kratos.VtuOutput.NODES)
            ...
            raise ValueError(f"Vector to be outputted has length {vector.size()} that nether matches the number of elements nor nodes in the provided model part ({model_part.Name})")
    ...
    def get_number_of_observable_elements(self, gradient_matrix_in_info_space) -> int:
        is_seen_by_n_sensors = np.sum(gradient_matrix_in_info_space, axis=1)
        def cap_to_one(x): return 1.0 if x >= 1.0 else 0.0
        vectorized_mapping = np.vectorize(cap_to_one)
        is_seen_by_n_sensors = vectorized_mapping(is_seen_by_n_sensors)
        return int(np.sum(is_seen_by_n_sensors))

    def calculate_sensor_positions(self, model_file_name: str, primal_settings_file_name: str, adjoint_settings_file_name: str, potential_positions_model_part: str, num_of_sensors_to_select: int, sensor_direction: "list", redundancy_factor: float = 0.0) -> "list(list(float))":
        ...
        sensitivities_per_node, node_ids_of_potential_sensor_positions = self.calculate_displacement_to_young_modulus_sensitivity_matrix(
            ...
            sensor_direction=Kratos.Vector(sensor_direction))

        information_threshold = 1e-14
        # Map sensitivities to the adjacent information space
        def mapping_to_information_space_function(x): return 1.0 if x > information_threshold else -1.0
        ...
        def make_zero_or_one(x): return 1.0 if x > information_threshold else 0.0
        ...
        n_observed_elements = self.get_number_of_observable_elements(sensitivities_per_node)
        input(f"elements: {len(model_part.GetElements())}, n_observed: {n_observed_elements}")


        redundancy_weight = redundancy_factor  # 0 only new. elements count <-> 1 equally weight of new. and red. elements count <-> inf only red. elements count
        best_picks_batch = 1  # Batch size that is selected based on the score ranking
        ...
        for n in range(num_of_sensors_to_select):
            ...
            n_new_opt = (n_observed_elements / num_of_sensors_to_select)
            # total_score = 1/np.abs((n_new_visible_elements_score/n_red_visible_elements_score)-factor)
            # total_score = (n_new_visible_elements_score+n_red_visible_elements_score)/np.abs((n_new_visible_elements_score/n_red_visible_elements_score)-factor)
            # total_score = 1/(np.abs(n_new_visible_elements_score-n_red_visible_elements_score * factor)+1)
            # total_score = abs/(n_new_visible_elements_score-n_red_visible_elements_score) * (-1)

            total_score = -((n_new_opt - n_new_visible_elements_score)**2 + redundancy_weight*np.abs(n_new_visible_elements_score - n_red_visible_elements_score))

            if n == 0:
                # total_score = n_new_visible_elements_score/np.max(n_new_visible_elements_score)
                total_score = -((n_new_opt - n_new_visible_elements_score)**2)

            total_score /= np.max(np.abs(total_score))
            # input(total_score)
            # fig = px.imshow([total_score])
            # fig.show()

            self.output_vector_as_vtu(file_name=f"score_map_sens{num_of_sensors_to_select}_rf{redundancy_factor}_{n}", vector=total_score, model_part=model_part)

            sorted_indices = np.argsort(total_score)
            best_picks = sorted_indices[-(best_picks_batch):]
            ...
            for pick in best_picks:
                if max_visible_elements < n_new_visible_elements_score[pick] and not (pick in selected_sensor_node_ids):
            ...
            # input(f"bests: {best_picks}, selected:{selected_sensor_node_ids}")
