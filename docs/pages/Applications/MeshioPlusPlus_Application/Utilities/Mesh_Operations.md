...
| `curvature` | Per-vertex mean and Gaussian curvature of a **surface** (angle defect for `K`, cotangent Laplace-Beltrami for `H`). The report's `total_angle_defect` is the Gauss-Bonnet invariant (`2·π·χ` for a closed surface, `4·π` for anything sphere-like), a checkable oracle whatever the tessellation. `output` renames `curvature:mean` — see the write-back note below. | `curvature_mean`, `curvature_gaussian`, `dual_area`, `include_boundary`, `record_area`, `record_principal`, `region`, `output` |
| `repair` | Surface topology fix: rewind triangles so neighbours agree (`fix_orientation`), flip closed components outward (`orient_outward`), fan-fill boundary loops up to `max_hole_edges` (`fill_holes`), duplicate bowtie vertices (`split_non_manifold`). Reports both the input's and the output's `SurfaceQuality` defect counts, so what was fixed and what remains are both visible. | `fix_orientation`, `orient_outward`, `fill_holes`, `split_non_manifold`, `repair_record_provenance`, `max_hole_edges`, `weld_tolerance` |
| `sobolev_deform` | Sobolev-filters a raw nodal displacement field (`array_name`, a `point_data` array) before moving the mesh's points by the filtered result. `length_scale` `0.0` (the default) applies it directly with no solve at all — an exact, zero-iteration result. | `array_name`, `length_scale`, `frozen`, `fixed_points_array`, `sobolev_fix_boundary`, `record_filtered`, `sobolev_max_iterations`, `sobolev_tolerance`, `output` |
...
| `Shrinkwrap(source, target, settings, destination)` | Projects `source`'s points onto `target`'s surface, optionally offset along the hit feature's pseudonormal (`offset`) — one projection, not an iteration, so a **fit** rather than a smoothing. Two independent meshes, like `Interpolate`. Settings: `offset`, `max_distance`, `weights`, `target_region`, `sdf_weight` (reused — the pseudonormal weighting), `record_distance`, `record_closest_cell`, `grid_cell_size`, plus `"output"` (renames `shrinkwrap:distance`). |
