...
IndexType num_rows_for_current_rank = 10; // Example: this rank will own 10 rows.
...
Kratos::DistributedSparseGraph<IndexType> dist_graph(num_rows_for_current_rank, kratos_comm);
```
Example from `test_distributed_sparse_graph.cpp`:
```cpp
...
```cpp
Kratos::DistributedSparseGraph<IndexType> graph_copy(dist_graph); // Error: Copy constructor deleted
Kratos::DistributedSparseGraph<IndexType> another_graph(10, kratos_comm);
...
IndexType global_row_idx = 5;  // Example global row index
IndexType global_col_idx = 10; // Example global column index
...
```cpp
IndexType source_row = 7;
std::vector<IndexType> target_cols = {12, 15, 20};
...
```cpp
std::vector<IndexType> source_rows = {0, 4};
std::vector<IndexType> target_cols_for_multiple = {1, 3};
...
```cpp
const Kratos::SparseContiguousRowGraph<IndexType>& local_graph = dist_graph.GetLocalGraph();

// To iterate, using local row indices:
for(IndexType local_row_idx = 0; local_row_idx < local_graph.Size(); ++local_row_idx) {
    // To get global row index:
    // IndexType global_row_idx = dist_graph.GetRowNumbering().GlobalId(local_row_idx);
...
```cpp
IndexType target_rank = 1; // Example rank
const Kratos::SparseGraph<IndexType>& non_local_buffer_for_rank_1 = dist_graph.GetNonLocalGraph(target_rank);
...
```cpp
const Kratos::DistributedNumbering<IndexType>& numbering = dist_graph.GetRowNumbering();
// IndexType owner_rank = numbering.OwnerRank(some_global_row_index);
...
```cpp
IndexType global_number_of_rows = dist_graph.Size();
...
```cpp
IndexType num_locally_owned_rows_with_entries = dist_graph.LocalSize();
```
...
```cpp
IndexType global_row_idx = 5;
IndexType global_col_idx = 10;
...
IndexType local_row_to_access = 0; // Example: Access first local row
if (local_row_to_access < dist_graph.GetLocalGraph().Size()) {
    ...
    for (IndexType global_col_idx : neighbors) {
...
```cpp
IndexType local_min_col, local_max_col;
...
```cpp
IndexType max_global_j = dist_graph.ComputeMaxGlobalColumnIndex();
