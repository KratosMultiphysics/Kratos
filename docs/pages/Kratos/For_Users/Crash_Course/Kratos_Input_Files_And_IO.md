...
In the first tutorial you have successfully installed Kratos and confirmed it works, but in order to run a real simulation you will need some files that define your problem. There are three different types of files that compose a Kratos case:
...
This is a first step in order to customize the input files for Kratos to special use cases as it will be done in the following tutorials and the more flexible usage of Kratos.
