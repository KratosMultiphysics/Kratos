...
namespace MPMSearchElementUtility
{
    ...
    void KRATOS_API(PARTICLE_MECHANICS_APPLICATION) Check(IntegrationPointsArrayType& rIntergrationSubPoints, const double Tolerance, const Matrix& rN, const DenseVector<Matrix>& rDN_De);


    const bool KRATOS_API(PARTICLE_MECHANICS_APPLICATION) CheckAllPointsAreInGeom(
        const std::vector<array_1d<double, 3>>& rPoints,
        const GeometryType& rReferenceGeom,
        const double Tolerance);

    void KRATOS_API(PARTICLE_MECHANICS_APPLICATION) Check3DBackGroundMeshIsCubicAxisAligned(const std::vector<typename GeometryType::Pointer> rIntersectedGeometries);

    const bool KRATOS_API(PARTICLE_MECHANICS_APPLICATION) CheckNoPointsAreInGeom(
        const std::vector<array_1d<double, 3>>& rPoints,
        const GeometryType& rReferenceGeom,
        const double Tolerance);

    void KRATOS_API(PARTICLE_MECHANICS_APPLICATION) Create2DPolygonBoundingSquareFromPoints(const std::vector<array_1d<double, 3>>& rPoints,
    ...
    void KRATOS_API(PARTICLE_MECHANICS_APPLICATION) Create2DPolygonFromGeometry(const GeometryType& rGeom,
        std::vector<Boost2DPointType>& rPolygonPoints,
        Boost2DPolygonType& rPolygon,
        const bool XActive = true, const bool YActive = true, const bool ZActive = false);
    ...
    typename Geometry<Node<3>>::Pointer KRATOS_API(PARTICLE_MECHANICS_APPLICATION) CreateCustomQuadraturePoint(
    ...
    typename Geometry<Node<3>>::Pointer KRATOS_API(PARTICLE_MECHANICS_APPLICATION) PartitionMasterMaterialPointsIntoSubPoints(const ModelPart& rBackgroundGridModelPart,
