...
namespace Kratos
{
            ...
            for (IndexType i = 0; i < number_of_nodes; i++)
            {
                if (dimension == 2 && StrainSize == 3)
                {
                    ...
                    nodal_force_internal_normal[0] = rMPVolume * weight *
                        (rMPStress[0] * DN_DX_vec[int_p](i, 0) +
                            rMPStress[2] * DN_DX_vec[int_p](i, 1));
                    //f_y = V*(s_yy*dNdY + s_yx*dNdX)
                    nodal_force_internal_normal[1] = rMPVolume * weight *
                        (rMPStress[1] * DN_DX_vec[int_p](i, 1) +
                            rMPStress[2] * DN_DX_vec[int_p](i, 0));
                }
                else if (is_axisym)
                {
                    ...
                    nodal_force_internal_normal[0] = rMPVolume * weight *
                        (rMPStress[0] * DN_DX_vec[int_p](i, 0) +
                            ...
                            rMPStress[3] * DN_DX_vec[int_p](i, 1));

                    nodal_force_internal_normal[1] = rMPVolume * weight *
                        (rMPStress[1] * DN_DX_vec[int_p](i, 1) +
                            rMPStress[3] * DN_DX_vec[int_p](i, 0));
                }
                else if (dimension == 3 && StrainSize == 6)
                {
                    ...
                    nodal_force_internal_normal[0] = rMPVolume * weight *
                        (rMPStress[0] * DN_DX_vec[int_p](i, 0) +
                            rMPStress[3] * DN_DX_vec[int_p](i, 1) +
                            rMPStress[5] * DN_DX_vec[int_p](i, 2));
                    //f_y = V*(s_yy*dNdY + s_yx*dNdX + s_yz*dNdZ)
                    nodal_force_internal_normal[1] = rMPVolume * weight *
                        (rMPStress[1] * DN_DX_vec[int_p](i, 1) +
                            rMPStress[3] * DN_DX_vec[int_p](i, 0) +
                            rMPStress[4] * DN_DX_vec[int_p](i, 2));
                    //f_z = V*(s_zz*dNdZ + s_zx*dNdX + s_zy*dNdY)
                    nodal_force_internal_normal[2] = rMPVolume * weight *
                        (rMPStress[2] * DN_DX_vec[int_p](i, 2) +
                            rMPStress[5] * DN_DX_vec[int_p](i, 0) +
                            rMPStress[4] * DN_DX_vec[int_p](i, 1));
            ...
            for (IndexType i = 0; i < number_of_nodes; i++)
            {
                ...
                if (nodal_mass > std::numeric_limits<double>::epsilon())
                {
                    ...
                    for (IndexType j = 0; j < dimension; j++)
                    {
                        ...
                        MP_Acceleration[j] += r_N(int_p, i) * r_current_residual[j] / nodal_mass;

                        if (isCentralDifference)
                        {
                            delta_xg[j] += rDeltaTime * r_N(int_p, i) * r_middle_velocity[j];
                        }
                        else if (!isUpdateMPPositionFromUpdatedMPVelocity)
                        {
                            delta_xg[j] += rDeltaTime * r_N(int_p, i) * r_nodal_momenta[j] / nodal_mass;
                        }
                    }
                }
            }
        ...
        for (IndexType int_p = 0; int_p < rGeom.IntegrationPointsNumber(); ++int_p) {
            ...
            for (IndexType nodeIndex = 0; nodeIndex < number_of_nodes; nodeIndex++)
            {
                ...
                for (IndexType i = 0; i < dimension; i++)
                {
                    for (IndexType j = 0; j < dimension; j++)
                    {
                        velocityGradient(i, j) += nodal_velocity[i] * DN_DX_vec[0](nodeIndex, j) * weight;
                    }
                }
    ...
    void MPMExplicitUtilities::GetCartesianDerivatives(std::vector<Matrix>& rDN_DXVec, const GeometryType& rGeom)
    {
        KRATOS_TRY
