...
public:
    ...
    /**
     * @brief Fills the given model part with the entities where the @p rClusterMask has minimum value of RequiredMinimumRedundancy.
     * @details This method will fill the given model part with the elements where the @p rClusterMask has the value for
     *          corresponding entity greater than or equal to the RequiredMinimumRedundancy.
     *
     *          In the case of @p rClusterMask contains ModelPart::ElementsContainerType or ModelPart::ConditionsContainerType,
     *          then corresponding nodes of those valid elements or conditions will also be added to the model part.
     *
     * @throws std::runtime_error if @p rModelPart is not empty.
     *
     * @param rModelPart                    Model part to be filled with the cluster entities.
     * @param rClusterMask                  Cluster mask.
     * @param RequiredMinimumRedundancy     Required minimum redundancy to consider the entity should be put in the @p rModelPart .
     */
    ...
    /**
     * @brief Smoothen the masks.
     * @details This smoothens all the masks provided by @p rMaskList using @ref Kratos::SmoothClamper.
     *          The @p Radius is used to identify the smoothing area. @p MaxLeafSize is used in the
     *          KD tree search.
     *
     * @throws std::runtime_error if all the masks does not have the same container.
     * @throws std::runtime_error if all the masks are not scalar masks.
     *
     * @param rMaskList                                     List of masks.
     * @param Radius                                        Radius to be used to smooth out the masks.
     * @param MaxLeafSize                                   Max leaf size used in the KD tree.
     * @return std::vector<TensorAdaptor<double>::Pointer>  Smoothened masks.
     */
    static std::vector<TensorAdaptor<double>::Pointer> SmoothenMasks(
