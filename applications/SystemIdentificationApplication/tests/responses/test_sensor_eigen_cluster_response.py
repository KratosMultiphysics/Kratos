...
from KratosMultiphysics.SystemIdentificationApplication.responses.sensor_eigen_cluster_response import SensorEigenClusterResponse

class TestSensorEigenClusterResponse(UnitTest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        elem_ta.data[:] = np.array([0, 0.5, 1, 0, 1, 1], dtype=np.float64)
        cls.sensors[4].AddTensorAdaptor("mask_exp", elem_ta.Clone())

        elem_ta.data[:] = np.array([0, 0, 0.1, 0, 0.1, 1], dtype=np.float64)
        cls.sensors[5].AddTensorAdaptor("mask_exp", elem_ta.Clone())
    ...
    def test_CalculateValue1(self):
        self.sensor_model_part.GetNode(1).SetValue(KratosSI.SENSOR_STATUS, 0.5)
        self.sensor_model_part.GetNode(2).SetValue(KratosSI.SENSOR_STATUS, 0.4)
        self.sensor_model_part.GetNode(3).SetValue(KratosSI.SENSOR_STATUS, 0.8)
        self.sensor_model_part.GetNode(4).SetValue(KratosSI.SENSOR_STATUS, 0.6)
        self.sensor_model_part.GetNode(5).SetValue(KratosSI.SENSOR_STATUS, 0.6)
        self.sensor_model_part.GetNode(6).SetValue(KratosSI.SENSOR_STATUS, 0.2)
        ...
        params = Kratos.Parameters("""{
            ...
            "number_of_eigen_values": 2,
            "echo_level"            : 0
        }""")
        response = SensorEigenClusterResponse("test1", self.model, params, self.optimization_problem)
        response.Initialize()
        self.assertAlmostEqual(response.CalculateValue(), 0.7299429898078622)

    def test_CalculateGradient(self):
        params = Kratos.Parameters("""{
            "sensor_group_name"     : "sensors",
            "sensor_mask_name"      : "mask_exp",
            "number_of_eigen_values": 1,
            "echo_level"            : 0
        }""")
        response = SensorEigenClusterResponse("test4", self.model, params, self.optimization_problem)
