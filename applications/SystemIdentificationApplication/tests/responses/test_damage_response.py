...
class TestDamageDetectionResponseBase(kratos_unittest.TestCase, ABC):
    ...
    def setUpClass(cls):
        ...
        cls.response_function = DamageDetectionResponse("mass", cls.model, cls.GetParameters(), cls.optimization_problem)

        cls.response_function.Initialize()
        cls.response_function.Check()
        cls.ref_value = cls.response_function.CalculateValue()

    def _CheckSensitivity(self, response_function: DamageDetectionResponse, entities, sensitivity_method, update_method, container_expression_data, delta, precision):
...
class TestDamageDetectionResponseSolids(TestDamageDetectionResponseBase):
    @classmethod
    def GetParameters(cls):
        return Kratos.Parameters("""{
            "evaluated_model_part_names": ["test"]
    ...
    def CreateElements(cls):
        ...
        properties = cls.model_part.CreateNewProperties(1)
        properties[Kratos.YOUNG_MODULUS] = 2.0
        cls.model_part.CreateNewElement("Element3D4N", 1, [1, 2, 3, 4], properties)

        properties = cls.model_part.CreateNewProperties(2)
        properties[Kratos.YOUNG_MODULUS] = 4.0
        cls.model_part.CreateNewElement("Element3D4N", 2, [5, 1, 2, 3], properties)

    def test_CalculateValue(self):
        v = 0.0
        for element in self.model_part.Elements:
            v += element.GetGeometry().DomainSize() * element.Properties[Kratos.YOUNG_MODULUS]
    ...
    def test_CalculateYoungModulusSensitivity(self):
        ...
        # calculate element YOUNG_MODULUS sensitivity
        self._CheckSensitivity(
            self.response_function,
            self.model_part.Elements,
