...
void DisplacementSensor::GetStateVariables(
...
void DisplacementSensor::ComputeDerivative(
    Vector& rOutput,
    const Element& rElement,
            ...
            if (rElement.Id() == mElementId) {
                ...
                for (std::size_t i_variable=0ul; i_variable<Variables.size(); ++i_variable) {
                    ...
                    if (r_variable.SourceKey() == DISPLACEMENT.SourceKey())
                        rOutput[i_variable] = mNs[i_node] * mDirection[r_variable.GetComponentIndex()];
                    else
                        KRATOS_ERROR << "unsupported variable " << r_variable.Name();
                } // for variable
            }
        KRATOS_CATCH("")
}
