...
Sensor::Pointer VelocitySensor::Create(
    ...
    Parameters SensorParameters)
{
    ...
    SensorParameters.ValidateAndAssignDefaults(VelocitySensor::GetDefaultParameters());
    ...
    auto p_sensor = Kratos::make_shared<VelocitySensor>(
...
Parameters VelocitySensor::GetSensorParameters() const
{
    auto parameters = BaseType::GetSensorParameters();
    parameters.AddString("type", "velocity_sensor");
...
double VelocitySensor::CalculateValue(ModelPart& rModelPart)
{
    ...
    if (rModelPart.HasElement(mElementId)) {
        ...
        directional_velocity = inner_prod(velocity, mDirection);
    }
