...
double EigenvalueSensor::CalculateValue(ModelPart &rModelPart)
{
    ...
    double eigen_value = rModelPart.GetProcessInfo()[EIGENVALUE_VECTOR][index_number - 1];
    ...
    const ProcessInfo &rProcessInfo)
{
    ...
    // Matrix LHS;
    // Matrix mass_matrix;
    // rAdjointElement.CalculateLeftHandSide(LHS, rProcessInfo);
    // rAdjointElement.CalculateMassMatrix(mass_matrix, rProcessInfo);
    ...
    // CalculateLeftHandSideDerivative(rAdjointElement, LHS, mdelta, derived_LHS, rProcessInfo);
    // CalculateMassMatrixDerivative(rAdjointElement, mass_matrix, mdelta, derived_mass_matrix, rProcessInfo);
...
// --------------------------------------------------------------------------
void EigenvalueSensor::DetermineEigenvectorOfElement(ModelPart::ElementType& rElement, const int eigenfrequency_id, 
...
void EigenvalueSensor::CalculateElementContributionToPartialSensitivity(Element& rAdjointElement,
                                    ...
                                    const ProcessInfo& rProcessInfo)
{
    ...
    if (KratosComponents<Variable<double>>::Has(design_variable_name))
    {
        ...
        noalias(extra) = LHS_design_variable_derivative - eigen_value * Massmatrix_design_variable_derivative;
