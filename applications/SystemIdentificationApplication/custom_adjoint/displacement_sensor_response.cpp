...
double DisplacementSensorResponse::ComputeValue(
    const ModelPart& rProcessInfo,
    int iBuffer) const {
        KRATOS_ERROR_IF_NOT(iBuffer == 0)
            << "requested buffer " << iBuffer << ", which is not supported";

        KRATOS_TRY
            if (auto p_maybe_element = this->GetContainingElement()) {
                const Element& r_element = **p_maybe_element;
...
void DisplacementSensorResponse::ComputeDerivative(
    ...
    int iBuffer) const {
        ...
        KRATOS_TRY
            ...
            if (auto p_maybe_containing_element = this->GetContainingElement()) {
                const Element& r_containing_element = **p_maybe_containing_element;
                if (rElement.Id() == r_containing_element.Id()) {
                ...
                } // if rElement.Id() == containing_element.Id()
            } // if maybe_containing_element
        KRATOS_CATCH("")
}
