...
class SystemIdentificationSpectralAnalysis(AnalysisStage):
    ...
    def _PassIfftResult(self, ifft_result):
        self.ifft_result = ifft_result
    ...
    def InitializeSolutionStep(self):
        ...
        step = self.nsteps - adjoint_step
        sensor_index = 0
        for sensor in self.list_of_sensors:
            sensor.SetValue(KratosSI.SENSOR_ERROR, self.ifft_result[sensor_index][step] * self.nsteps)
            sensor_index += 1
