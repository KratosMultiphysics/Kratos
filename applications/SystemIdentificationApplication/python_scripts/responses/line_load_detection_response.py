...
def Factory(model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem) -> ResponseFunction:
    if not parameters.Has("name"):
        raise RuntimeError(f"LineLoadDetectionResponse instantiation requires a \"name\" in parameters [ parameters = {parameters}].")
    if not parameters.Has("settings"):
        raise RuntimeError(f"LineLoadDetectionResponse instantiation requires a \"settings\" in parameters [ parameters = {parameters}].")
    return LineLoadDetectionResponse(parameters["name"].GetString(), model, parameters["settings"], optimization_problem)


class LineLoadDetectionResponse(ResponseFunction):
    def __init__(self, name: str, model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem):
        ...
        if len(evaluated_model_part_names) == 0:
            raise RuntimeError(f"No model parts were provided for LineLoadDetectionResponse. [ response name = \"{self.GetName()}\"]")
    ...
    def GetInfluencingModelPart(self) -> Kratos.ModelPart:
        if self.analysis_model_part is None:
            raise RuntimeError("Please call LineLoadDetectionResponse::Initialize first.")
    ...
    def CalculateGradient(self, physical_variable_collective_expressions: 'dict[SupportedSensitivityFieldVariableTypes, KratosOA.CollectiveExpression]') -> None:
        ...
        for exec_policy, sensor_measurement_data_file_name, test_case_weight in self.list_of_test_analysis_data:
            ...
            for physical_variable, collective_expression in physical_variable_collective_expressions.items():
                ...
                sensitivity_variable = Kratos.KratosGlobals.GetVariable("LINE_LOAD_SENSITIVITY_X")
                for container_expression in collective_expression.GetContainerExpressions():
