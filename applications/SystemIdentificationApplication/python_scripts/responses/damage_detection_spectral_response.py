...
from KratosMultiphysics.SystemIdentificationApplication.sensor_sensitivity_solvers.system_identification_spectral_analysis import SystemIdentificationSpectralAnalysis
from KratosMultiphysics.OptimizationApplication.utilities.component_data_view import ComponentDataView
from KratosMultiphysics.HDF5Application.core.file_io import OpenHDF5File

def Factory(model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem) -> ResponseFunction:
    if not parameters.Has("name"):
        raise RuntimeError(f"DamageDetectionSpectralResponse instantiation requires a \"name\" in parameters [ parameters = {parameters}].")
    if not parameters.Has("settings"):
        raise RuntimeError(f"DamageDetectionSpectralResponse instantiation requires a \"settings\" in parameters [ parameters = {parameters}].")
    return DamageDetectionSpectralResponse(parameters["name"].GetString(), model, parameters["settings"], optimization_problem)


class DamageDetectionSpectralResponse(ResponseFunction):
    def __init__(self, name: str, model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem):
        ...
        if len(evaluated_model_part_names) == 0:
            raise RuntimeError(f"No model parts were provided for DamageDetectionSpectralResponse. [ response name = \"{self.GetName()}\"]")
        ...
        self.adjoint_analysis = SystemIdentificationSpectralAnalysis(self.model, parameters["adjoint_parameters"])
    ...
    def GetInfluencingModelPart(self) -> Kratos.ModelPart:
        if self.analysis_model_part is None:
            raise RuntimeError("Please call DamageDetectionSpectralResponse::Initialize first.")
    ...
    def CalculateValue(self) -> float:
        ...
        for exec_policy, sensor_measurement_data_file_name, primal_hdf5_data_file, test_case_weight in self.list_of_test_analysis_data:
            ...
            if time_step_settings.Has("time_step_table"):
                raise Exception("DamageDetectionSpectralResponse:: there is currently no variable time stepping possible!")
            delta_time = time_step_settings["time_step"].GetDouble()
            ...
            self.number_of_sensors = len(self.list_of_sensors)

            # get number of time steps
            self.number_of_time_steps = int((end_time - start_time)/delta_time) + 1

            # initialize time data array
            measured_time_data_array = np.zeros((self.number_of_sensors, self.number_of_time_steps))
            simulated_time_data_array = np.zeros((self.number_of_sensors, self.number_of_time_steps))

            # initialize time data array
            self.measured_frequ_data_array = np.zeros((self.number_of_sensors, self.number_of_time_steps), dtype=np.complex128)
            self.simulated_frequ_data_array = np.zeros((self.number_of_sensors, self.number_of_time_steps), dtype=np.complex128)
            ...
            while time < end_time:
                ...
                with open(sensor_measurement_data_file_name.replace("<step>", f"{time_step:06d}"), "r") as csv_measurement_file:
                    csv_measurement_stream = csv.reader(csv_measurement_file, delimiter=",")
                    # get value index
                    headers = [s.strip() for s in next(csv_measurement_stream)]
                    measured_value_index = headers.index("value")

                    # fill time_data_array
                    sensor_index = 0
                    for measured_row in csv_measurement_stream:
                        measured_value = float(measured_row[measured_value_index])
                        measured_time_data_array[sensor_index][time_step-1] = measured_value
                        sensor_index += 1
                ...
                for sensor in self.list_of_sensors:
                    simulated_value = sensor.CalculateValue(exec_policy.GetAnalysisModelPart())
                    simulated_time_data_array[sensor_index][time_step-1] = simulated_value
                    sensor_index += 1
                
            # Fast Fourier Transformation and value calculation
            value = 0.0
            for sensor_index in range(self.number_of_sensors):
                self.measured_frequ_data_array[sensor_index] = np.fft.fft(measured_time_data_array[sensor_index])
                self.simulated_frequ_data_array[sensor_index] = np.fft.fft(simulated_time_data_array[sensor_index])
                measured_sensor_magnitude = np.abs(self.measured_frequ_data_array[sensor_index])
                simulated_sensor_magnitude = np.abs(self.simulated_frequ_data_array[sensor_index])
                sensor_magnitude_square_difference = simulated_sensor_magnitude**2 - measured_sensor_magnitude**2
                value += np.sum(sensor_magnitude_square_difference**2)
            value *= 0.25
    ...
    def CalculateGradient(self, physical_variable_collective_expressions: 'dict[SupportedSensitivityFieldVariableTypes, KratosOA.CollectiveExpression]') -> None:
        ...
        for exec_policy, sensor_measurement_data_file_name, _, test_case_weight in self.list_of_test_analysis_data:
            ...
            # Inverse Fast Fourier Transformation of sensor magnitute square difference times simulated sensor state in frequency domain
            ifft_result = np.zeros((self.number_of_sensors, self.number_of_time_steps))
            for sensor_index in range(self.number_of_sensors):
                measured_sensor_magnitude = np.abs(self.measured_frequ_data_array[sensor_index])
                simulated_sensor_magnitude = np.abs(self.simulated_frequ_data_array[sensor_index])
                sensor_magnitude_square_difference = simulated_sensor_magnitude**2 - measured_sensor_magnitude**2
                ifft_result[sensor_index] = scipy.fft.ifft(sensor_magnitude_square_difference*self.simulated_frequ_data_array[sensor_index])

            self.adjoint_analysis._PassIfftResult(ifft_result)
