...
def Factory(model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem) -> Control:
    if not parameters.Has("name"):
        raise RuntimeError(f"LineLoadControl instantiation requires a \"name\" in parameters [ parameters = {parameters}].")
    if not parameters.Has("settings"):
        raise RuntimeError(f"LineLoadControl instantiation requires a \"settings\" in parameters [ parameters = {parameters}].")

    return LineLoadControl(parameters["name"].GetString(), model, parameters["settings"], optimization_problem)

class LineLoadControl(Control):
    """line load control

    This is a generic line load control which creates a control
    ...
    def __init__(self, name: str, model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem):
        ...
        print("controlled phy var is : ", self.controlled_physical_variable)
        ...
        if len(controlled_model_part_names) == 0:
            raise RuntimeError(f"No model parts were provided for LineLoadControl. [ control name = \"{self.GetName()}\"]")
        ...
        self.filter = FilterFactory(self.model, self.primal_model_part_operation.GetModelPartFullName(), self.controlled_physical_variable, Kratos.Globals.DataLocation.Condition, self.parameters["filter_settings"])
        ...
        self.clamper = KratosSI.ConditionSmoothClamper(control_variable_bounds[0], control_variable_bounds[1])
    ...
    def Initialize(self) -> None:
        ...
        props = self.primal_model_part.GetProperties()[1]
        #cond = self.adjoint_model_part.CreateNewCondition("AdjointSemiAnalytic" +  "LineLoadCondition3D2N", 1, [1,2], props)
        #cond.SetValue(Kratos.LOCAL_AXIS_2, [-1.0, 1.0, 0.0])
        
        load_on_cond = Kratos.Vector(3)
        load_on_cond[0] = 1000.0
        load_on_cond[1] = 0.0
        load_on_cond[2] = 0.0

        for condition in self.primal_model_part.Conditions:
            condition: Kratos.Condition
            #cond = self.adjoint_model_part.GetCondition()
            condition.SetValue(KratosSM.LINE_LOAD,load_on_cond)
            print("condition set to ", KratosSM.LINE_LOAD_X)
            print(condition.GetValue(KratosSM.LINE_LOAD_X))

        
        if not KratosOA.OptAppModelPartUtils.CheckModelPartStatus(self.primal_model_part, "condition_specific_properties_created"):
            KratosOA.OptimizationUtils.CreateEntitySpecificPropertiesForContainer(self.primal_model_part, self.primal_model_part.Conditions)
            KratosOA.OptAppModelPartUtils.LogModelPartStatus(self.primal_model_part, "condition_specific_properties_created")

            if self.primal_model_part != self.adjoint_model_part:
                if KratosOA.OptAppModelPartUtils.CheckModelPartStatus(self.adjoint_model_part, "condition_specific_properties_created"):
                    raise RuntimeError(f"Trying to create condition specific properties for {self.adjoint_model_part.FullName()} which already has element specific properties.")

                # now assign the properties of primal to the adjoint model parts
                KratosSI.ControlUtils.AssignEquivalentProperties(self.primal_model_part.Conditions, self.adjoint_model_part.Conditions)
                KratosOA.OptAppModelPartUtils.LogModelPartStatus(self.adjoint_model_part, "condition_specific_properties_created")
        ...
        # print("Initial modulus found is ", amb_temp)
        #Kratos.Expression.LiteralExpressionIO.SetData(physical_field, 1000)
        
        print("Initialize: Physical field: ", physical_field.Evaluate())

        # get the phi field which is in [0, 1] range
        self.physical_phi_field = self.clamper.ProjectBackward(physical_field)
        print("Initialize: Physical phi field: ", self.physical_phi_field.Evaluate())

        # compute the control phi field
        self.control_phi_field = self.filter.UnfilterField(self.physical_phi_field)
        print("Initialize: Control phi field: ", self.control_phi_field.Evaluate())
    ...
    def GetEmptyField(self) -> ContainerExpressionTypes:
        field = Kratos.Expression.ConditionExpression(self.primal_model_part)
    ...
    def GetPhysicalField(self) -> ContainerExpressionTypes:
        physical_thickness_field = Kratos.Expression.ConditionExpression(self.primal_model_part)
    ...
    def _UpdateAndOutputFields(self, update: ContainerExpressionTypes) -> None:
        ...
        print("Update: filtered_phi_field_update: ", filtered_phi_field_update.Evaluate())
        self.physical_phi_field = Kratos.Expression.Utils.Collapse(self.physical_phi_field + filtered_phi_field_update)

        print("Update: Physical phi field: ", self.physical_phi_field.Evaluate())
        ...
        print("Update: Physical field: ", physical_field.Evaluate())

        # now update physical field
        KratosOA.PropertiesVariableExpressionIO.Write(physical_field, self.controlled_physical_variable)

        print("Updated : Physical field: ", physical_field.Evaluate())
        print("Updated :  controlled_physical_variable ", self.controlled_physical_variable)
