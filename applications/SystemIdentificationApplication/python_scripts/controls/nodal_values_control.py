...
def Factory(model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem) -> Control:
    if not parameters.Has("name"):
        raise RuntimeError(f"NodalValuesControl instantiation requires a \"name\" in parameters [ parameters = {parameters}].")
    if not parameters.Has("settings"):
        raise RuntimeError(f"NodalValuesControl instantiation requires a \"settings\" in parameters [ parameters = {parameters}].")

    return NodalValuesControl(parameters["name"].GetString(), model, parameters["settings"], optimization_problem)

class NodalValuesControl(Control):
    """Nodal values control

    This is a generic nodal values control which creates a control
    ...
    def __init__(self, name: str, model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem):
        ...
        self.is_historical = parameters["is_historical"].GetBool()
        ...
        if len(controlled_model_part_names) == 0:
            raise RuntimeError(f"No model parts were provided for NodalValuesControl. [ control name = \"{self.GetName()}\"]")
        ...
        self.filter = FilterFactory(self.model, self.primal_model_part_operation.GetModelPartFullName(), self.controlled_physical_variable, Kratos.Globals.DataLocation.NodeNonHistorical, self.parameters["filter_settings"])
    ...
    def Initialize(self) -> None:
        ...
        if not KratosOA.OptAppModelPartUtils.CheckModelPartStatus(self.primal_model_part, "primal_adjoint_element_properties_linked"):
            KratosSI.ControlUtils.AssignEquivalentProperties(self.primal_model_part.Elements, self.adjoint_model_part.Elements)
            KratosOA.OptAppModelPartUtils.LogModelPartStatus(self.adjoint_model_part, "primal_adjoint_element_properties_linked")
    ...
    def GetPhysicalField(self) -> ContainerExpressionTypes:
        ...
        Kratos.Expression.VariableExpressionIO.Read(physical_thickness_field, self.controlled_physical_variable, self.is_historical)
        return physical_thickness_field

    def MapGradient(self, physical_gradient_variable_container_expression_map: 'dict[SupportedSensitivityFieldVariableTypes, ContainerExpressionTypes]') -> ContainerExpressionTypes:
        with TimeLogger("NodalValuesControl::MapGradient", None, "Finished",False):
    ...
    def _UpdateAndOutputFields(self, update: ContainerExpressionTypes) -> None:
        ...
        Kratos.Expression.VariableExpressionIO.Write(physical_field, self.controlled_physical_variable, self.is_historical)
