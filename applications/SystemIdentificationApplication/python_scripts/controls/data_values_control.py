...
class DataValuesControl(Control):
    ...
    Generic control for container data values (nodal historical/non-historical, element, condition).
    Supports optional filtering and bound handling via ExpressionBoundingManager and smooth clampers.
    """
    ...
    def __init__(self, name: str, model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem):
        ...
        # store names; create operations lazily in Initialize to allow analyses to create model parts first
        self._primal_model_part_names = [param["primal_model_part_name"].GetString() for param in controlled_model_part_names]
        self._adjoint_model_part_names = [param["adjoint_model_part_name"].GetString() for param in controlled_model_part_names]

        self.primal_model_part_operation = None
        self.adjoint_model_part_operation = None
        self.filter = None
        ...
        # manage bounds via [lo, hi] -> phi in [0,1]
    ...
    def Initialize(self) -> None:
        # lazily create model part operations now (roots should exist after analyses init)
        if self.primal_model_part_operation is None:
            self.primal_model_part_operation = ModelPartOperation(
                                                    self.model,
                                                    ModelPartOperation.OperationType.UNION,
                                                    f"control_primal_{self.GetName()}",
                                                    self._primal_model_part_names,
                                                    False)
        if self.adjoint_model_part_operation is None:
            self.adjoint_model_part_operation = ModelPartOperation(
                                                    self.model,
                                                    ModelPartOperation.OperationType.UNION,
                                                    f"control_adjoint_{self.GetName()}",
                                                    self._adjoint_model_part_names,
                                                    False)

        # filter uses the primal model part
    ...
    def GetPhysicalField(self) -> ContainerExpressionTypes:
        physical_field = self.container_type_helper.expression_type(self.primal_model_part)
        self.container_type_helper.ReadExpression(physical_field, self.controlled_physical_variable)
    ...
    def MapGradient(self, physical_gradient_variable_container_expression_map: 'dict[SupportedSensitivityFieldVariableTypes, ContainerExpressionTypes]') -> ContainerExpressionTypes:
        with TimeLogger("DataValuesControl::MapGradient", None, "Finished", False):
            ...
            if not IsSameContainerExpression(physical_gradient, self.GetEmptyField()):
                raise RuntimeError(f"Gradients for the required container not found for control \"{self.GetName()}\". [ required model part name: {self.primal_model_part.FullName()}, given model part name: {physical_gradient.GetModelPart().FullName()} ]")

            # dj/dx -> physical_gradient
            # dj/dphi = dj/dx * dx/dphi
    ...
    def Update(self, new_control_field: ContainerExpressionTypes) -> bool:
        if not IsSameContainerExpression(new_control_field, self.GetEmptyField()):
            raise RuntimeError(f"Updates for the required container not found for control \"{self.GetName()}\". [ required model part name: {self.primal_model_part.FullName()}, given model part name: {new_control_field.GetModelPart().FullName()} ]")
    ...
    def _UpdateAndOutputFields(self, update: ContainerExpressionTypes) -> None:
        ...
        # project forward to physical field within bounds
        ...
        # compute and store projection derivatives
        self.physical_phi_derivative_field = self.clamper.CalculateForwardProjectionGradient(self.physical_phi_field) * self.interval_bounder.GetBoundGap()
