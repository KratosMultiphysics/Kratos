...
def Factory(model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem) -> Control:
    if not parameters.Has("name"):
        raise RuntimeError(f"NodalPropertiesControl instantiation requires a \"name\" in parameters [ parameters = {parameters}].")
    if not parameters.Has("settings"):
        raise RuntimeError(f"NodalPropertiesControl instantiation requires a \"settings\" in parameters [ parameters = {parameters}].")

    return NodalPropertiesControl(parameters["name"].GetString(), model, parameters["settings"], optimization_problem)

class NodalPropertiesControl(Control):
    """Nodal properties control

    This is a generic nodal properties control which creates a control
    ...
    def __init__(self, name: str, model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem):
        ...
        if len(controlled_model_part_names) == 0:
            raise RuntimeError(f"No model parts were provided for NodalPropertiesControl. [ control name = \"{self.GetName()}\"]")
        ...
        self.filter = FilterFactory(self.model, self.primal_model_part_operation.GetModelPartFullName(), 
                                    self.controlled_physical_variable, 
                                    Kratos.Globals.DataLocation.NodeNonHistorical, 
                                    self.parameters["filter_settings"])
        ...
        self.clamper = KratosSI.NodeSmoothClamper(control_variable_bounds[0], control_variable_bounds[1])
    ...
    def Initialize(self) -> None:
        ...
        ###DO WE NEED THIS FOR NODAL DATA?
        ...
        #     node: Kratos.Node
        #     node.SetValue(self.controlled_physical_variable, 18.0)
        for prop in self.primal_model_part.GetProperties():
            prop: Kratos.Properties
            if prop.Has(Kratos.AMBIENT_TEMPERATURE):
                prop: Kratos.Properties
                amb_temp = prop.GetValue(Kratos.AMBIENT_TEMPERATURE)
        print("amb temp found is ", amb_temp)
        Kratos.Expression.LiteralExpressionIO.SetData(physical_field, amb_temp)
        # print(self.controlled_physical_variable)
        # physical_field = self.GetPhysicalField()
    ...
    def GetEmptyField(self) -> ContainerExpressionTypes:
        field = Kratos.Expression.NodalExpression(self.primal_model_part)
    ...
    def GetPhysicalField(self) -> ContainerExpressionTypes:
        physical_thickness_field = Kratos.Expression.NodalExpression(self.primal_model_part)

        Kratos.Expression.VariableExpressionIO.Read(physical_thickness_field, self.controlled_physical_variable, False)

        print(physical_thickness_field.Evaluate())
        print("size ", len(physical_thickness_field.Evaluate()))
        return physical_thickness_field

    def MapGradient(self, physical_gradient_variable_container_expression_map: 'dict[SupportedSensitivityFieldVariableTypes, ContainerExpressionTypes]') -> ContainerExpressionTypes:
        with TimeLogger("NodalPropertiesControl::MapGradient", None, "Finished",False):
            ...
            if not IsSameContainerExpression(physical_gradient, self.GetEmptyField()):
                raise RuntimeError(f"Gradients for the required nodal container not found for control \"{self.GetName()}\". [ required model part name: {self.primal_model_part.FullName()}, given model part name: {physical_gradient.GetModelPart().FullName()} ]")
    ...
    def Update(self, new_control_field: ContainerExpressionTypes) -> bool:
        if not IsSameContainerExpression(new_control_field, self.GetEmptyField()):
            raise RuntimeError(f"Updates for the required nodal container not found for control \"{self.GetName()}\". [ required model part name: {self.primal_model_part.FullName()}, given model part name: {control_field.GetModelPart().FullName()} ]")

        update = new_control_field - self.control_phi_field
        print("     difference in control field is ", update.Evaluate())
        ...
        if not math.isclose(Kratos.Expression.Utils.NormL2(update), 0.0, abs_tol=1e-16):
            with TimeLogger(self.__class__.__name__, f"Updating {self.GetName()}...", f"Finished updating of {self.GetName()}.",False):
                # update the control temperature field
    ...
    def _UpdateAndOutputFields(self, update: ContainerExpressionTypes) -> None:
        ...
        Kratos.Expression.VariableExpressionIO.Write(physical_field, self.controlled_physical_variable, 1)
