...
namespace Kratos
{
    ...
    void IgaFEMMappingGeometriesModeler::SetupGeometryModel()
    {
        ...
        // These variables are used to decide whether the origin and destination domains are IGA or FEM meshes 
        int origin_model_part_working_dim=(mpModels[0]->GetModelPart(origin_interface_sub_model_part_name)).GetRootModelPart().ConditionsBegin()->GetGeometry().WorkingSpaceDimension();
        int destination_model_part_working_dim=(mpModels.back()->GetModelPart(destination_interface_sub_model_part_name)).GetRootModelPart().ConditionsBegin()->GetGeometry().WorkingSpaceDimension();
        ...
        // create coupling conditions on origin interface depending on the dimension
        if (origin_model_part_working_dim == 3)
        {
            CreateInterfaceLineBrepCurveOnSurface(mpModels[0]->GetModelPart(origin_interface_sub_model_part_name)); // Used for IGA patch
        }
        else
        {
            CreateInterfaceLineBrepCurve(mpModels[0]->GetModelPart(origin_interface_sub_model_part_name));
        }

        // create coupling conditions on destination interface depending on the dimension
        if (destination_model_part_working_dim == 2)
        {
            CreateInterfaceLineBrepCurve(mpModels.back()->GetModelPart(destination_interface_sub_model_part_name)); // Used for FEM domain 
        }
        else
        {
        ...
        {
            CopySubModelPartFEM(coupling_interface_origin,
            mpModels[0]->GetModelPart(origin_interface_sub_model_part_name));
        }
        ...
        if (destination_model_part_working_dim == 2)
        {
            CopySubModelPartFEM(coupling_interface_destination,
    ...
    void IgaFEMMappingGeometriesModeler::CreateInterfaceLineBrepCurveOnSurface(ModelPart& rInterfaceModelPart)
    {
    ...
    void IgaFEMMappingGeometriesModeler::CreateInterfaceLineBrepCurve(ModelPart& rInterfaceModelPart)
    {
        rInterfaceModelPart.CreateSubModelPart("coupling_conditions");
        ModelPart& coupling_conditions = rInterfaceModelPart.GetSubModelPart("coupling_conditions");
        const ModelPart& root_mp = rInterfaceModelPart.GetRootModelPart();
    ...
    void IgaFEMMappingGeometriesModeler::CreateInterfaceLineBrepCurveOnSurfaceStrongSupport(ModelPart& rInterfaceModelPart)
    {
        rInterfaceModelPart.CreateSubModelPart("coupling_conditions");
        ModelPart& coupling_conditions = rInterfaceModelPart.GetSubModelPart("coupling_conditions");
