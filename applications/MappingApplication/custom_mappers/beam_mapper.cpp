...
void BeamMapperInterfaceInfo::ProcessSearchResult(const InterfaceObject& rInterfaceObject)
{
    ...
    if (neighbor_distance < mBeamMapperDistance) {
        mBeamMapperDistance = neighbor_distance;
        mBeamMapperId.resize(1);
        mBeamMapperId[0] = rInterfaceObject.pGetBaseNode()->GetValue(INTERFACE_EQUATION_ID);
    } else if (neighbor_distance == mBeamMapperDistance) {
        mBeamMapperId.push_back(rInterfaceObject.pGetBaseNode()->GetValue(INTERFACE_EQUATION_ID));
    }
}
                    ...
                    MapperLocalSystem::PairingStatus& rPairingStatus) const
{
    if (mInterfaceInfos.size() > 0) {
        ...
        for (std::size_t i=1; i<mInterfaceInfos.size(); ++i) {
            ...
            // bcs this does not exist for BeamMapper
            double distance;
