...
KRATOS_TEST_CASE_IN_SUITE(InterfacePreprocessorNodeBasedConditions, KratosMappingApplicationSerialTestSuite)
{
    ...
    // std::unique_ptr<MapperLocalSystem> ref_local_system = std::make_unique<NearestElementLocalSystem>();

    // InterfacePreprocessor interface_preprocess(model_part, p_mapper_local_systems);

    // interface_preprocess.GenerateInterfaceModelPart(std::move(ref_local_system));
