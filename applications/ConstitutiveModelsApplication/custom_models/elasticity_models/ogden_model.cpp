...
namespace Kratos
{
  ...
  void OgdenModel::CalculateAndAddStressTensor(HyperElasticDataType& rVariables, MatrixType& rStressMatrix)
  {
	...
	KRATOS_ERROR << "calling Ogden based method with PK2 stress. not implemented" << std::endl;
      ...
      double& rBulkModulus = rMaterial.GetBulkModulus();
      std::vector<double>& rModelParameters = rMaterial.GetModelParameters(); //nu values, lambda values

      array1d<double,3> MainStresses;
      array1d<double,3> EigenVector;
	  ...
	  {
	      double& alpha_p = rMaterialModels[p+size-1];
	      MainStresses[i] = rMaterialModels[p] * std::pow(rVariables.Strain.Invariants.J,(-alpha_p*athird)) * ( std::pow(rVariables.Strain.Eigen.Values[i],alpha_p) - athird * ( std::pow(rVariables.Strain.Eigen.Values[0],alpha_p) + std::pow(rVariables.Strain.Eigen.Values[1],alpha_p) + std::pow(rVariables.Strain.Eigen.Values[2],alpha_p) ) ) + rBulkModulus * std::log(rVariables.Strain.Invariants.J);
	  }

	  noalias(EigenVector) = matrix_column<const MatrixType>(rVariables.Strain.Eigen.Vectors, i);
  ...
  void OgdenModel::CalculateAndAddConstitutiveTensor(HyperElasticDataType& rVariables, Matrix& rConstitutiveMatrix)
  {
    ...
    array1d<double,3> StrainEigenValues;
    for (unsigned int i = 0; i < 3; i++)
	StrainEigenValues = rVariables.Strain.Eigen.Values[i] * rVariables.Strain.Eigen.Values[i];
    
    MatrixType EigenValues;
    MatrixType EigenVectors;
    MathUtils<double>::EigenSystem<3> ( rModelData.StressMatrix, EigenVectors, EigenValuesMatrix);

    array1d<double,3> StressEigenValues;
    for (unsigned int i = 0; i < 3; i++)
	StressEigenValues[i] = EigenValues(i,i);
    
    //Store stress eigenvalues in strain invariants variables in a vector
    array1d<double,6> ScalingFactors;
	...
	if( ConstitutiveModelUtilities::IsEqual(StrainEigenValues[1],StrainEigenValues[2]) ){
		...
		{
		    rConstitutiveMatrix(i,j) = this->AddConstitutiveComponent(rVariables,StressDerivatives,rConstitutiveMatrix(i,j),
	    ...
	    array1d<unsigned int,3> Permutation;
	    Permutation[0] = 2;
	    Permutation[1] = 0;
	    Permutation[2] = 1;
	...
	if( ConstitutiveModelUtilities::IsEqual(StrainEigenValues[0],StrainEigenValues[2]) ){
		...
		for(SizeType j=0; j<rVoigtSize; j++)
		{
		    rConstitutiveMatrix(i,j) = this->AddConstitutiveComponent(rVariables,ScalingFactors,StressDerivatives,rConstitutiveMatrix(i,j),
									      rIndexVoigtTensor[i][0],rIndexVoigtTensor[i][1],
									      rIndexVoigtTensor[j][0],rIndexVoigtTensor[j][1]);
		}
	...
	for(SizeType i=0; i<rVoigtSize; i++)
	{
	    for(SizeType j=0; j<rVoigtSize; j++)
	    {
		rConstitutiveMatrix(i,j) = this->AddConstitutiveComponent(rVariables,ScalingFactors,rConstitutiveMatrix(i,j),
									  rIndexVoigtTensor[i][0],rIndexVoigtTensor[i][1],
									  rIndexVoigtTensor[j][0],rIndexVoigtTensor[j][1]);
	    }
  ...
  void OgdenModel::CalculateScalingFactors(array1d<double,6>& rScalingFactors, MatrixType& rStressDerivatives, array1d<double,3>& rStressEigenValues, array1d<double,3>& rStrainEigenValues, array1d<unsigned int,3>& rPermutation)
  {
    KRATOS_TRY
    ...
    rScalingFactors[0] = (t1-t3)/((b1-b3)*(b1-b3)) + (rStressDerivatives(rPermutation[2],rPermutation[1]) - rStressDerivatives(rPermutation[2],rPermutation[1]))/(b1-b3);
    rScalingFactors[1] =  2*b3*(t1-t3)/((b1-b3)*(b1-b3)) + (rStressDerivatives(rPermutation[2],rPermutation[1]) - rStressDerivatives(rPermutation[2],rPermutation[1]))*(b1+b3)/(b1-b3);
    ...
    rScalingFactors[3] = 2*b3*(t1-t3)/((b1-b3)*(b1-b3)*(b1-b3)) + (rStressDerivatives(rPermutation[0],rPermutation[2]) - rStressDerivatives(rPermutation[2],rPermutation[1]))/(b1-b3) + (rStressDerivatives(rPermutation[0],rPermutation[2]) + rStressDerivatives(rPermutation[2],rPermutation[0]) - rStressDerivatives(rPermutation[0],rPermutation[0]) - rStressDerivatives(rPermutation[2],rPermutation[2]))*b3/((b1-b3)*(b1-b3));
    rScalingFactors[4] = 2*b3*(t1-t3)/((b1-b3)*(b1-b3)*(b1-b3)) + (rStressDerivatives(rPermutation[2],rPermutation[0]) - rStressDerivatives(rPermutation[2],rPermutation[1]))/(b1-b3) + (rStressDerivatives(rPermutation[0],rPermutation[2]) + rStressDerivatives(rPermutation[2],rPermutation[0]) - rStressDerivatives(rPermutation[0],rPermutation[0]) - rStressDerivatives(rPermutation[2],rPermutation[2]))*b3/((b1-b3)*(b1-b3)); 
    rScalingFactors[5] = 2*b3*b3*(t1-t3)/((b1-b3)*(b1-b3)*(b1-b3)) + (rStressDerivatives(rPermutation[0],rPermutation[2]) - rStressDerivatives(rPermutation[2],rPermutation[0]))*b1*b3/((b1-b3)*(b1-b3)) - (rStressDerivatives(rPermutation[0],rPermutation[0]) + rStressDerivatives(rPermutation[2],rPermutation[2]))*b3*b3/((b1-b3)*(b1-b3)) - (b1+b3)*rStressDerivatives(rPermutation[2],rPermutation[1])/(b1-b3);
  ...
  double& OgdenModel::AddConstitutiveComponent(HyperElasticDataType& rVariables, array1d<double>& rScalingFactors,
					       ...
					       const unsigned int& c, const unsigned int& d)
  {
	...
	array1d<double,3> EigenVectorA;
	array1d<double,3> EigenVectorB;    

	array1d<unsigned int,5> Permutation;
	Permutation[0] = 0;
	Permutation[1] = 1;
	Permutation[2] = 2;
	Permutation[1] = 0;
	...
	for(unsigned int e=0; e<3; e++)
	{
	    ...
	    for(unsigned int i=0; i<3; i++)
	    {
		double alpha = rScalingFactors[Permuation[i]]/((rScalingFactors[Permutation[i]+3]-rScalingFactors[Permutation[i+1]+3])*(rScalingFactors[Permuation[i]+3]-rScalingFactors[Permutation[i+2]+3]));
	  
		Dabce = ConstitutiveModelUtilities::CalculateSquareTensorDerivative(rVariables.Strain.Matrix,this->msIdentityMatrix,Dabce,a,b,c,d);
		Cabce += Dabce;

		Dabce = ConstitutiveModelUtilities::CalculateFourthOrderUnitTensor(Dabce,this->msIdentityMatrix,a,b,c,d);
		Cabce -= (rScalingFactors[Permuation[i+1]+3]+rScalingFactors[Permutation[i+2]+3]) * Dabce;

		noalias(EigenVectorA) = matrix_column<const MatrixType>(rVariables.Strain.Eigen.Vectors,Permutation[i]);  
		Dabce = ConstitutiveModelUtilities::CalculateFourthOrderTensorProduct(EigenVectorA,EigenVectorA,Dabce,a,b,c,d);
		Cabce -= ((rScalingFactors[Permuation[i]+3]-rScalingFactors[Permutation[i+1]+3])+(rScalingFactors[Permuation[i]+3]-rScalingFactors[Permutation[i+2]+3])) * Dabce;
	  
		noalias(EigenVectorA) = matrix_column<const MatrixType>(rVariables.Strain.Eigen.Vectors,Permutation[i+1]);
		Dabce = ConstitutiveModelUtilities::CalculateFourthOrderTensorProduct(EigenVectorA,EigenVectorA,Dabce,a,b,c,d);
		Cabce -= (rScalingFactors[Permuation[i+1]+3]-rScalingFactors[Permutation[i+2]+3]) * Dabce;
		...
		{
		    noalias(EigenVectorA) = matrix_column<const MatrixType>(rVariables.Strain.Eigen.Vectors,i);
		    noalias(EigenVectorB) = matrix_column<const MatrixType>(rVariables.Strain.Eigen.Vectors,j);	      
		    Dabce = ConstitutiveModelUtilities::CalculateFourthOrderTensorProduct(EigenVectorA,EigenVectorB,Dabce,a,b,c,d);
  ...
  double& OgdenModel::AddConstitutiveComponent(HyperElasticDataType& rVariables,
					       ...
					       const unsigned int& c, const unsigned int& d)
  {
	...
	{     	    	    
	    ...
	    Dabce = ConstitutiveModelUtilities::CalculateFourthOrderUnitTensor(Dabce,this->msIdentityMatrix,a,b,c,e);
  ...
  double& OgdenModel::AddConstitutiveComponent(HyperElasticDataType& rVariables,
					       aMatrixType& rStressDerivatives, double &rCabcd,
					       const unsigned int& a, const unsigned int& b,
					       const unsigned int& c, const unsigned int& d)
  {
    KRATOS_TRY
