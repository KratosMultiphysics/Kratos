    ...
    def CalculateMaterialResponse(self):
        ...
        if ( True):
            ...
            for t in range(0, nSize):
                self.strain_vector[0] = final * float(t) / float(nSize)
                self.strain_vector[1] = -self.strain_vector[0]/2.0
                self.strain_vector[2] = -self.strain_vector[0]/2.0
                self.parameters.SetStrainVector( self.strain_vector )
                ...
                pStress[t] = stress[0];
                pStrain2[t] = self.strain_vector[2];
                pStress2[t] = stress[2];
