...
#if !defined(KRATOS_PORO_NEWMARK_QUASISTATIC_U_PL_PG_SCHEME )
#define  KRATOS_PORO_NEWMARK_QUASISTATIC_U_PL_PG_SCHEME
...
class PoroNewmarkQuasistaticUPlPgScheme : public Scheme<TSparseSpace,TDenseSpace>
{

public:

    KRATOS_CLASS_POINTER_DEFINITION( PoroNewmarkQuasistaticUPlPgScheme );
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    ///Constructor
    PoroNewmarkQuasistaticUPlPgScheme(double theta_u,
                                      double theta_pl,
                                      double theta_pg,
                                      double beta,
                                      double gamma) :
    ...
    ///Destructor
    ~PoroNewmarkQuasistaticUPlPgScheme() override {}
    ...
    int Check(ModelPart& r_model_part) override
    {
        ...
        for(ModelPart::NodesContainerType::iterator it=r_model_part.NodesBegin(); it!=r_model_part.NodesEnd(); it++)
        {
            ...
            if(it->SolutionStepsDataHas(DT_GAS_PRESSURE) == false)
                KRATOS_THROW_ERROR( std::logic_error, "DT_GAS_PRESSURE variable is not allocated for node ", it->Id() )
        ...
        // Check theta
        if(mTheta_u <= 0.0 || mTheta_pl <= 0.0 || mTheta_pg <= 0.0)
            KRATOS_THROW_ERROR( std::invalid_argument,"Some of the scheme variables: theta_u, theta_pl or theta_pg has an invalid value ", "" )
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    void Initialize(ModelPart& r_model_part) override
    {
        ...
        r_model_part.GetProcessInfo().SetValue(DT_LIQUID_PRESSURE_COEFFICIENT,1.0/(mTheta_pl*mDeltaTime));
        r_model_part.GetProcessInfo().SetValue(DT_GAS_PRESSURE_COEFFICIENT,1.0/(mTheta_pg*mDeltaTime));
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        TSystemVectorType& b) override
    {
        ...
        this->UpdateVariables(r_model_part);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    virtual inline void UpdateVariables(ModelPart& r_model_part)
    {
        KRATOS_TRY
        ...
        for(int i = 0; i < NNodes; i++)
        {
            ...
            double& CurrentCapillaryPressure = itNode->FastGetSolutionStepValue(CAPILLARY_PRESSURE);
            CurrentCapillaryPressure = itNode->FastGetSolutionStepValue(GAS_PRESSURE) - itNode->FastGetSolutionStepValue(LIQUID_PRESSURE);
        }
...
}; // Class PoroNewmarkQuasistaticUPlPgScheme
}  // namespace Kratos

#endif // KRATOS_PORO_NEWMARK_QUASISTATIC_U_PL_PG_SCHEME defined
