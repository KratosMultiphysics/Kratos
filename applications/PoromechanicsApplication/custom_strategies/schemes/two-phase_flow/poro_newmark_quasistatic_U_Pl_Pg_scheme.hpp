...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    ///Constructor
    PoroNewmarkQuasistaticUPlPgScheme(double beta, double gamma, double theta) : Scheme<TSparseSpace,TDenseSpace>()
    {
        mBeta = beta;
        mGamma = gamma;
        mTheta = theta;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    inline void UpdateVariablesDerivatives(ModelPart& r_model_part)
    {
        ...
        for(int i = 0; i < NNodes; i++)
        {
            ...
            CurrentDtGasPressure = 1.0/(mTheta*mDeltaTime)*(DeltaGasPressure - (1.0-mTheta)*mDeltaTime*PreviousDtGasPressure);
        }
