                                                                                ...
                                                                                std::vector<array_1d<double,3>>& rOutput, const ProcessInfo& rCurrentProcessInfo )
{
    ...
    if(rVariable == FLUID_FLUX_VECTOR)
    {
        ...
        bounded_matrix<double,TNumNodes, TDim> GradNpT; 
        double JointWidth;
        bounded_matrix<double,TDim, TDim> LocalPermeabilityMatrix = ZeroMatrix(TDim,TDim);
        ...
        for ( unsigned int GPoint = 0; GPoint < NumGPoints; GPoint++ )
        {
            ...
            this->template CalculateShapeFunctionsGradients< bounded_matrix<double,TNumNodes,TDim> >(GradNpT,SFGradAuxVars,JContainer[GPoint],RotationMatrix,
                                                                                    ...
                                                                                    const ProcessInfo& rCurrentProcessInfo )
{
    ...
    if(rVariable == PERMEABILITY_MATRIX)
    {
        ...
        for ( unsigned int GPoint = 0; GPoint < mConstitutiveLawVector.size(); GPoint++ )
        {
            ...
            noalias(PermeabilityMatrix) = prod(trans(RotationMatrix),bounded_matrix<double,TDim, TDim>(prod(LocalPermeabilityMatrix,RotationMatrix)));
