...
def CreateSolver(model, custom_settings):
    return UPlPgSolver(model, custom_settings)

class UPlPgSolver(PythonSolver):
    '''Solver for the solution of displacement-pore pressure coupled problems
    for two-phase flow in porous media.'''

    def __init__(self, model, custom_settings):

        super(UPlPgSolver,self).__init__(model, custom_settings)
        ...
        KratosMultiphysics.Logger.PrintInfo("UPlPgSolver", "Construction of UPlPgSolver finished.")

    @classmethod
    def GetDefaultParameters(cls):
        this_defaults = KratosMultiphysics.Parameters("""{
            "solver_type": "poromechanics_U_Pl_Pg_solver",
            ...
            "newmark_gamma": 0.5,
            "newmark_theta_u": 0.5,
            "newmark_theta_pl": 0.5,
            "newmark_theta_pg": 0.5,
        ...
        this_defaults.AddMissingParameters(super(UPlPgSolver, cls).GetDefaultParameters())
    ...
    def AddVariables(self):
        ...
        self.main_model_part.AddNodalSolutionStepVariable(KratosPoro.GAS_DISCHARGE)
        ## Other variables
        self.main_model_part.AddNodalSolutionStepVariable(KratosMultiphysics.VOLUME_ACCELERATION)
        self.main_model_part.AddNodalSolutionStepVariable(KratosMultiphysics.PERIODIC_PAIR_INDEX)
        self.main_model_part.AddNodalSolutionStepVariable(KratosPoro.CAPILLARY_PRESSURE)
        ...
        KratosMultiphysics.Logger.PrintInfo("UPlPgSolver", "Variables added correctly.")
    ...
    def AddDofs(self):
        ...
        KratosMultiphysics.Logger.PrintInfo("UPlPgSolver", "DOFs added correctly.")
    ...
    def Initialize(self):
        ...
        KratosMultiphysics.Logger.PrintInfo("UPlPgSolver", "Solver initialization finished.")
    ...
    def Solve(self):
        message = "".join([
            "Calling UPlPgSolver.Solve() method, which is deprecated\n",
        ...
        KratosMultiphysics.Logger.PrintWarning("UPlPgSolver",message)
    ...
    def _ExecuteCheckAndPrepare(self):
        ...
        if materials_imported:
            KratosMultiphysics.Logger.PrintInfo("UPlPgSolver", "Constitutive law was successfully imported via json.")
        else:
            KratosMultiphysics.Logger.PrintInfo("UPlPgSolver", "Constitutive law was not successfully imported.")
    ...
    def _ConstructScheme(self, scheme_type, solution_type):
        ...
        if(scheme_type == "Newmark"):
            ...
            theta_pl = self.settings["newmark_theta_pl"].GetDouble()
            theta_pg = self.settings["newmark_theta_pg"].GetDouble()
            ...
            if self.settings["calculate_alpha_beta"].GetBool():
                ...
                KratosMultiphysics.Logger.PrintInfo("::[UPlPgSolver]:: Scheme Information")
                KratosMultiphysics.Logger.PrintInfo("::[UPlPgSolver]:: omega_1: ",omega_1)
                KratosMultiphysics.Logger.PrintInfo("::[UPlPgSolver]:: omega_n: ",omega_n)
                KratosMultiphysics.Logger.PrintInfo("::[UPlPgSolver]:: xi_1: ",xi_1)
                KratosMultiphysics.Logger.PrintInfo("::[UPlPgSolver]:: xi_n: ",xi_n)
                KratosMultiphysics.Logger.PrintInfo("::[UPlPgSolver]:: Alpha and Beta output")
                KratosMultiphysics.Logger.PrintInfo("::[UPlPgSolver]:: rayleigh_alpha: ",rayleigh_alpha)
                KratosMultiphysics.Logger.PrintInfo("::[UPlPgSolver]:: rayleigh_beta: ",rayleigh_beta)
            ...
            if(solution_type == "implicit_quasi_static"):
                if(rayleigh_alpha<1.0e-20 and rayleigh_beta<1.0e-20):
                    scheme = KratosPoro.PoroNewmarkQuasistaticUPlPgScheme(theta_u,theta_pl,theta_pg,beta,gamma)
                else:
                    scheme = KratosPoro.PoroNewmarkQuasistaticDampedUPlPgScheme(theta_u,theta_pl,theta_pg,beta,gamma)
            else:
                scheme = KratosPoro.PoroNewmarkDynamicUPlPgScheme(theta_u,theta_pl,theta_pg,beta,gamma)
        else:
