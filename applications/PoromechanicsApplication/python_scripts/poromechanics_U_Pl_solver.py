        ...
        if(scheme_type == "Newmark"):
            ...
            if(solution_type == "implicit_quasi_static"):
                if(rayleigh_alpha<1.0e-20 and rayleigh_beta<1.0e-20):
                    scheme = KratosPoro.PoroNewmarkQuasistaticUPlScheme(beta,gamma,theta)
                else:
                    scheme = KratosPoro.PoroNewmarkQuasistaticDampedUPlScheme(beta,gamma,theta)
            else:
                scheme = KratosPoro.PoroNewmarkDynamicUPlScheme(beta,gamma,theta)
        else:
