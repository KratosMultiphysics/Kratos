...
#include "custom_conditions/two-phase_flow/U_Pl_Pg_face_load_interface_condition.hpp"

namespace Kratos
{

template< unsigned int TDim, unsigned int TNumNodes >
Condition::Pointer UPlPgFaceLoadInterfaceCondition<TDim,TNumNodes>::Create(IndexType NewId,NodesArrayType const& ThisNodes,PropertiesType::Pointer pProperties) const
{
    return Condition::Pointer(new UPlPgFaceLoadInterfaceCondition(NewId, this->GetGeometry().Create(ThisNodes), pProperties));
}

//----------------------------------------------------------------------------------------

template< unsigned int TDim, unsigned int TNumNodes >
void UPlPgFaceLoadInterfaceCondition<TDim,TNumNodes>::Initialize(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
template< >
void UPlPgFaceLoadInterfaceCondition<2,2>::CalculateInitialGap(const GeometryType& Geom)
{
...
template< >
void UPlPgFaceLoadInterfaceCondition<3,4>::CalculateInitialGap(const GeometryType& Geom)
{
    const double& InitialJointWidth = this->GetProperties()[INITIAL_JOINT_WIDTH];
    const double Tolerance = std::numeric_limits<double>::epsilon();
...
template< unsigned int TDim, unsigned int TNumNodes >
void UPlPgFaceLoadInterfaceCondition<TDim,TNumNodes>::CalculateRHS( VectorType& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo )
{        
...
template< >
void UPlPgFaceLoadInterfaceCondition<2,2>::CheckJointWidth(double& rJointWidth, bool& rComputeJointWidth, BoundedMatrix<double,2,2>& rRotationMatrix,
...
template< >
void UPlPgFaceLoadInterfaceCondition<3,4>::CheckJointWidth(double& rJointWidth, bool& rComputeJointWidth, BoundedMatrix<double,3,3>& rRotationMatrix,
                                                            const double& InitialJointWidth, const Element::GeometryType& Geom)
{
...
template< >
void UPlPgFaceLoadInterfaceCondition<2,2>::CalculateJointWidth( double& rJointWidth, const BoundedMatrix<double,2,4>& Nu,
...
template< >
void UPlPgFaceLoadInterfaceCondition<3,4>::CalculateJointWidth( double& rJointWidth, const BoundedMatrix<double,3,12>& Nu,
...
template< >
void UPlPgFaceLoadInterfaceCondition<2,2>::CalculateIntegrationCoefficient(double& rIntegrationCoefficient, const Matrix& Jacobian, const double& Weight, const double& JointWidth)
{
...
template< >
void UPlPgFaceLoadInterfaceCondition<3,4>::CalculateIntegrationCoefficient(double& rIntegrationCoefficient, const Matrix& Jacobian, const double& Weight, const double& JointWidth)
{
    KRATOS_TRY
...
template class UPlPgFaceLoadInterfaceCondition<2,2>;
template class UPlPgFaceLoadInterfaceCondition<3,4>;
