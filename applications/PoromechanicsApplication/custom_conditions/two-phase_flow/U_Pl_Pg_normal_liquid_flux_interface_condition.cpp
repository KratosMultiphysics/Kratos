...
#include "custom_conditions/two-phase_flow/multiphase_flow/U_Pl_Pg_normal_liquid_flux_interface_condition.hpp"

namespace Kratos
{

template< unsigned int TDim, unsigned int TNumNodes >
Condition::Pointer UPlPgNormalLiquidFluxInterfaceCondition<TDim,TNumNodes>::Create(IndexType NewId,NodesArrayType const& ThisNodes,PropertiesType::Pointer pProperties) const
{
    return Condition::Pointer(new UPlPgNormalLiquidFluxInterfaceCondition(NewId, this->GetGeometry().Create(ThisNodes), pProperties));
}

//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

template< unsigned int TDim, unsigned int TNumNodes >
void UPlPgNormalLiquidFluxInterfaceCondition<TDim,TNumNodes>::CalculateRHS( VectorType& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo )
{        
    ...
    for(unsigned int GPoint = 0; GPoint < NumGPoints; GPoint++)
    {
        ...
        PoroElementUtilities::AssemblePlBlockVector< array_1d<double,TNumNodes> >(rRightHandSideVector,PVector,TDim,TNumNodes);
    }
}

//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

template class UPlPgNormalLiquidFluxInterfaceCondition<2,2>;
template class UPlPgNormalLiquidFluxInterfaceCondition<3,4>;
