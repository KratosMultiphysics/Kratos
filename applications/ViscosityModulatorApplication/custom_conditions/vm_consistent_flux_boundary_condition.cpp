...
#include "vm_consistent_flux_boundary_condition.h"
...
VmConsistentFluxBoundaryCondition::VmConsistentFluxBoundaryCondition(
    IndexType NewId,
...
VmConsistentFluxBoundaryCondition::~VmConsistentFluxBoundaryCondition() = default;

Condition::Pointer VmConsistentFluxBoundaryCondition::Create(
    IndexType NewId,
    NodesArrayType const& ThisNodes,
    Properties::Pointer pProperties) const
{
    return Kratos::make_intrusive<VmConsistentFluxBoundaryCondition>(NewId, GetGeometry().Create(ThisNodes), pProperties);
}
...
Condition::Pointer VmConsistentFluxBoundaryCondition::Create(
    ...
    Properties::Pointer pProperties) const
{
    return Kratos::make_intrusive<VmConsistentFluxBoundaryCondition>(NewId, pGeom, pProperties);
}

void VmConsistentFluxBoundaryCondition::CalculateLocalSystem(
...
void VmConsistentFluxBoundaryCondition::CalculateLeftHandSide(
    MatrixType& rLeftHandSideMatrix,
    const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
...
void VmConsistentFluxBoundaryCondition::CalculateRightHandSide(
    VectorType& rRightHandSideVector,
    const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
...
void VmConsistentFluxBoundaryCondition::EquationIdVector(
...
void VmConsistentFluxBoundaryCondition::GetDofList(
...
GeometryData::IntegrationMethod VmConsistentFluxBoundaryCondition::GetIntegrationMethod() const
{
    return IntegrationUtilities::GetIntegrationMethodForExactMassMatrixEvaluation(this->GetGeometry());
}

int VmConsistentFluxBoundaryCondition::Check(const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY;
...
std::string VmConsistentFluxBoundaryCondition::Info() const
{
    std::stringstream buffer;
    buffer << "VmConsistentFluxBoundaryCondition #" << Id();
    return buffer.str();
}

void VmConsistentFluxBoundaryCondition::PrintInfo(std::ostream& rOStream) const
{
    rOStream << "VmConsistentFluxBoundaryCondition #" << Id();
}

void VmConsistentFluxBoundaryCondition::PrintData(std::ostream& rOStream) const
{
    rOStream << "VmConsistentFluxBoundaryCondition #" << Id() << std::endl;
    this->GetGeometry().PrintData(rOStream);
}

Element& VmConsistentFluxBoundaryCondition::GetParentElement()
{
    KRATOS_ERROR_IF_NOT(this->Has(NEIGHBOUR_ELEMENTS))
        << "NEIGHBOUR_ELEMENTS was not assigned to condition " << this->Id() << '.' << std::endl;
...
const Element& VmConsistentFluxBoundaryCondition::GetParentElement() const
{
    KRATOS_ERROR_IF_NOT(this->Has(NEIGHBOUR_ELEMENTS))
        << "NEIGHBOUR_ELEMENTS was not assigned to condition " << this->Id() << '.' << std::endl;
...
void VmConsistentFluxBoundaryCondition::CalculateConditionSystem(
...
void VmConsistentFluxBoundaryCondition::ComputeIntegrationPointData(
...
void VmConsistentFluxBoundaryCondition::CalculateParentGeometryValues(
    const GeometryType& rConditionGeometry,
    const GeometryType& rParentGeometry,
...
Vector VmConsistentFluxBoundaryCondition::GetParentUnknownValues(const Variable<double>& rUnknownVariable) const
{
    const auto& r_parent_geometry = GetParentElement().GetGeometry();
...
void VmConsistentFluxBoundaryCondition::save(Serializer& rSerializer) const
{
    KRATOS_SERIALIZE_SAVE_BASE_CLASS(rSerializer, Condition);
}

void VmConsistentFluxBoundaryCondition::load(Serializer& rSerializer)
{
