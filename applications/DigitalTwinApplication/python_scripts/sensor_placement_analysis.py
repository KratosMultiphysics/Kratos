...
from KratosMultiphysics.DigitalTwinApplication.sensor_specification_solvers.sensor_specification_analysis import SensorSpecificationAnalysis
...
class SensorPlacementAnalysis:
    ...
    def __init__(self, model: Kratos.Model, project_parameters: Kratos.Parameters):
        ...
        self.__list_of_test_analyses: 'list[SensorSpecificationAnalysis]' = self._CreateAnalyses(self.project_parameters["test_analyses"])
        self.__list_of_sensor_analyses: 'list[SensorSpecificationAnalysis]' = self._CreateAnalyses(self.project_parameters["sensor_analyses"])
    ...
    def Run(self):
        ...
        list(map(SensorSpecificationAnalysis.Run, self.__list_of_test_analyses))
        list(map(SensorSpecificationAnalysis.Run, self.__list_of_sensor_analyses))

        specifications_list: 'list[KratosDT.Sensors.SensorSpecification]' = []
        for sensor_analysis in self.__list_of_sensor_analyses:
            specifications_list.extend(sensor_analysis.GetListOfSpecifications())

        self.algorithm.Execute(specifications_list)
    ...
    def _CreateAnalyses(self, parameters: Kratos.Parameters) -> 'list[SensorSpecificationAnalysis]':
        list_of_analyses: 'list[SensorSpecificationAnalysis]' = []
