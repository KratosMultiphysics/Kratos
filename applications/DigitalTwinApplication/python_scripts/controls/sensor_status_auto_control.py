...
class SensorStatusControl(Control):
    def __init__(self, name: str, model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem) -> None:
        ...
        self.beta_min = beta_settings["min_value"].GetDouble()
        ...
        self.beta_last_computed_step = 1
    ...
    def Initialize(self) -> None:
        ...
        component_data = ComponentDataView(self, self.optimization_problem)
        component_data.SetDataBuffer(self.beta_update_period)
        self.un_buffered_data = component_data.GetUnBufferedData()
        self.buffered_data = component_data.GetBufferedData()
        ...
        self.beta = Kratos.Expression.NodalExpression(self.model_part)
        Kratos.Expression.LiteralExpressionIO.SetData(self.beta, self.beta_min)
    ...
    def __UpdateBeta(self, update: ContainerExpressionTypes) -> None:
        if self.beta_adaptive:
            step = self.optimization_problem.GetStep()
            if step != self.beta_last_computed_step:
                self.buffered_data.SetValue("sensor_status_update", update.Clone(), overwrite=True)

                if step >= self.beta_update_period:
                    last_updates = [self.buffered_data.GetValue("sensor_status_update", i) for i in range(self.beta_update_period)]
                    self.beta = KratosOA.ControlUtils.SigmoidalProjectionUtils.ComputeBeta(self.beta, last_updates, self.beta_min, self.beta_max, self.beta_increase_frac)
                    Kratos.Logger.PrintInfo(self.__class__.__name__, "Computed beta.")

                self.beta_last_computed_step = step
    ...
    def _UpdateAndOutputFields(self, physical_phi_update: ContainerExpressionTypes) -> None:
        ...
        if self.output_all_fields:
            ...
            un_buffered_data.SetValue("beta", self.beta.Clone(), overwrite=True)
