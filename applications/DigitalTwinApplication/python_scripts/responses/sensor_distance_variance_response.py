...
def Factory(model: Kratos.Model, parameters: Kratos.Parameters, _) -> ResponseFunction:
    if not parameters.Has("name"):
        raise RuntimeError(f"SensorDistanceVarianceResponse instantiation requires a \"name\" in parameters [ parameters = {parameters}].")
    if not parameters.Has("settings"):
        raise RuntimeError(f"SensorDistanceVarianceResponse instantiation requires a \"settings\" in parameters [ parameters = {parameters}].")
    return SensorDistanceVarianceResponse(parameters["name"].GetString(), model, parameters["settings"])

class SensorDistanceVarianceResponse(ResponseFunction):
    def __init__(self, name: str, model: Kratos.Model, parameters: Kratos.Parameters):
        ...
        if len(evaluated_model_part_names) == 0:
            raise RuntimeError(f"No model parts were provided for SensorDistanceVarianceResponse. [ response name = \"{self.GetName()}\"]")
    ...
    def Initialize(self) -> None:
        ...
        self.utils = KratosDT.SensorDistanceVarianceUtils(self.model_part)
    ...
    def GetEvaluatedModelPart(self) -> Kratos.ModelPart:
        if self.model_part is None:
            raise RuntimeError("Please call SensorDistanceVarianceResponse::Initialize first.")
