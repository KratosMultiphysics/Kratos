...
from KratosMultiphysics.DigitalTwinApplication.responses.combinatorial_response import CombinatorialResponse
...
class TestSensorDistanceBoltzmannOperatorResponse(UnitTest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        for i, sensor in enumerate(cls.sensors):
            ...
            node.SetValue(KratosDT.SENSOR_STATUS, (node.Id % 3) / 4 + 1e-2)

        params = Kratos.Parameters("""{
            "sensor_model_part_name" : "sensors"
        }""")
        cls.response = CombinatorialResponse("test", cls.model, params)
