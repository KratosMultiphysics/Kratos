...
#     return ApplyHydraulicSubCriticalOutletProcess(Model, settings["Parameters"])


class ApplyHydraulicSubCriticalOutletProcess(KratosMultiphysics.Process):
    """
    ...
    def ExecuteInitializeSolutionStep(self):
        ...
        KratosMultiphysics.NormalCalculationUtils().CalculateOnSimplexNonHistorical(self.outlet_model_part,self.domain_size,KratosFluidDynamics.OUTLET_NORMAL)

        # Calculate Outlet water discharge (negative side)
        # outlet_discharge = KratosFluidDynamics.FluidAuxiliaryUtilities.CalculateFlowRateNegativeSkin(self.outlet_model_part)
        tolerance = 0.01
        KratosFluidHydraulics.HydraulicFluidAuxiliaryUtilities.ImposeOutletPressure(self.outlet_model_part, self.water_depth, KratosFluidDynamics.OUTLET_DEPTH)
        #TODO: UXUEEEEEEEEEEEEEEEEEEEEEEEEE. YOU NEED TO IMPLEMENT DOMAIN WATER DEPTH HAY UNA CASUUISTICA QUE NO CUMPLEEEEE
        # if abs(self.inlet_discharge-outlet_discharge)>tolerance:
    ...
    def ExecuteFinalizeSolutionStep(self):
        if self.inflow_detection:
            KratosFluidHydraulics.HydraulicFluidAuxiliaryUtilities.ApplyOutletInflowLimiter(self.outlet_model_part, KratosMultiphysics.VELOCITY,KratosFluidDynamics.OUTLET_NORMAL)
            KratosFluidHydraulics.HydraulicFluidAuxiliaryUtilities.ApplyOutletInflowLimiter(self.outlet_model_part, KratosFluidDynamics.FRACTIONAL_VELOCITY, KratosFluidDynamics.OUTLET_NORMAL)
