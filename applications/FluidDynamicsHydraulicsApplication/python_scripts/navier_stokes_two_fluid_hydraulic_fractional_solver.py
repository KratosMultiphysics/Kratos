...
class NavierStokesTwoFluidsHydraulicFractionalSolver(FluidSolver):
    ...
    def GetDefaultParameters(cls):
        ...
        {
            "solver_type": "two_fluid_hydraulic",
            "model_part_name": "",
            "domain_size": -1,
    ...
    def __init__(self, model, custom_settings):
        ...
        self.outlet_inflow_limiter = self.settings["outlet_inflow_limiter_settings"]["apply_inflow_limiter"].GetBool()
        if self.outlet_inflow_limiter:
            self.inflow_model_part_name = self.settings["outlet_inflow_limiter_settings"]["apply_inflow_limiter"].GetString()
    ...
    def _CreateFractionalNSplittingProcess(self):
        ...
        if domain_size == 2:
            fractional_splitting_process = KratosCFD.NavierStokesVectorialFractionalConvectionProcess2D(computing_model_part,linear_solver, fractional_splitting_settings)
        else:
            fractional_splitting_process = KratosCFD.NavierStokesVectorialFractionalConvectionProcess3D(computing_model_part, linear_solver, fractional_splitting_settings)
