...
class NavierStokesTwoFluidsHydraulicSolver(FluidSolver):
    ...
    def __init__(self, model, custom_settings):
        ...
        # if self.settings["levelset_convection_settings"]["element_type"].GetString == "levelset_convection_bdf":
        #     self.settings["levelset_convection_settings"].AddEmptyValue("<_name").SetString("HEAT_FLUX")
        ...
        #     # self.settings["eulerian_fm_ale_settings"].AddEmptyValue("levelset_volume_source_variable_name").SetString("HEAT_FLUX")
    ...
    def Initialize(self):
        ...
        if self.eulerian_fm_ale:
            ...
            # for node in self.main_model_part.Nodes:
                # # node.SetSolutionStepValue(KratosCFD.FRACTIONAL_ACCELERATION, [0.1,0.1,0.1])
        ...
        self.my_energy_process_01 = KratosCFD.EnergyCheckProcess(
            self.main_model_part, domain_size, "energy_pre_split_1.txt")
        self.my_energy_process_02 = KratosCFD.EnergyCheckProcess(
            self.main_model_part, domain_size, "energy_pre_split_2.txt.txt")
        self.my_energy_process_03 = KratosCFD.EnergyCheckProcess(
            self.main_model_part, domain_size, "energy_post_split_2.txt")
        self.my_energy_process_04 = KratosCFD.EnergyCheckProcess(
            self.main_model_part, domain_size, "energy_post_levelset.txt")
    ...
    def InitializeSolutionStep(self):
        ...
        self.my_energy_process_01.Execute()
        ...
        self.my_energy_process_02.Execute()
    ...
    def FinalizeSolutionStep(self):
        ...
        self.my_energy_process_03.Execute()
        self.__PerformLevelSetConvection()
        ...
        self.my_energy_process_04.Execute()
    ...
    #     for node in self.GetComputingModelPart().Nodes:
    #         g=node.GetSolutionStepValue(KratosMultiphysics.BODY_FORCE)
    #         an = node.GetSolutionStepValue(KratosMultiphysics.ACCELERATION)
    #         node.SetSolutionStepValue(KratosMultiphysics.BODY_FORCE,g-an)
    ...
    def __CorrectVelocityHistory(self, velocity_components, fractional_velocity_componentes, mesh_variable):
        for node in self.GetComputingModelPart().Nodes:
                ...
                if node.Is(KratosMultiphysics.SLIP):
                    ...
                    node.SetSolutionStepValue(
                        fractional_velocity_componentes, velocity_history_corrected)
                    node.SetSolutionStepValue(
                        fractional_velocity_componentes, 1, velocity_history_corrected)
    ...
    def __SlipConditionFixity(self):
        for node in self.GetComputingModelPart().Nodes:
            if node.Is(KratosMultiphysics.SLIP):
                    ...
                    v -= v_prooj * n
                    node.SetSolutionStepValue(KratosMultiphysics.FRACTIONAL_VELOCITY, v)
                    node.SetSolutionStepValue(KratosMultiphysics.FRACTIONAL_VELOCITY, 1, v)
    ...
    def _CreateFmAleConvectionProcess(self):
        ...
        print(linear_solver)
        fm_ale_convection_settings = self.settings["eulerian_fm_ale_settings"]
        if domain_size == 2:
            eulerian_fm_ale_process = KratosCFD.VectorialConvectionProcess2D(
                computing_model_part,
                linear_solver, fm_ale_convection_settings)

        else:
            eulerian_fm_ale_process=KratosCFD.VectorialConvectionProcess3D(
                computing_model_part,
                linear_solver, fm_ale_convection_settings)
