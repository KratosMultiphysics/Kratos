...
class TwoFluidHydraulicFractionalSolverTest(UnitTest.TestCase):
    ...
    def testTwoFluidMassHydraulicFractionalSolverTestArtificialViscosity2D(self):
        ...
        self._AuxiliaryRunTest("ProjectParametersFractional2D.json")

    def testTwoFluidMassHydraulicFractionalSolverTestMassSource2D(self):
        self._artificial_viscosity = False
        self._mass_source = True
        self.problem_data = "mass_conservation"
        self._AuxiliaryRunTest("ProjectParametersFractional2D.json")

    def testTwoFluidMassHydraulicFractionalSolverTestTotal2D(self):
        self._artificial_viscosity = True
        self._mass_source = True
        self.problem_data = "all_activated"
        self._AuxiliaryRunTest("ProjectParametersFractional2D.json")

    def testTwoFluidMassHydraulicFractionalSolverTestArtificialViscosity3D(self):
        self._artificial_viscosity = True
        self._mass_source = False
        self.problem_data ="artificial_viscosity"
        self._AuxiliaryRunTest("ProjectParametersFractional3D.json")

    def testTwoFluidMassHydraulicFractionalSolverTestMassSource3D(self):
        self._artificial_viscosity = False
        self._mass_source = True
        self.problem_data = "mass_conservation"
        self._AuxiliaryRunTest("ProjectParametersFractional3D.json")

    def testTwoFluidMassHydraulicFractionalSolverTestTotal3D(self):
        self._artificial_viscosity = True
        self._mass_source = True
        self.problem_data = "all_activated"
        self._AuxiliaryRunTest("ProjectParametersFractional3D.json")
            ...
            "Parameters"    : {
                ...
                "postprocess_parameters" : {
                    "result_file_configuration" : {
                        ...
                        "skin_output"                 : false,
                        "plane_output"                : [],
                        "nodal_results"               : ["DISTANCE","VELOCITY","PRESSURE","FRACTIONAL_VELOCITY"]
                    },
                    "point_data_configuration"  : []
                }
            }
        }""")
        output_name = "hydraulic_fractional_solver_{0}d_{1}".format(self.parameters["solver_settings"]["domain_size"].GetInt(),self.parameters["problem_data"]["problem_name"].GetString())
    ...
    def _AddReferenceValuesOutput(self):
        ...
        output_file_name = "hydraulic_fractional_solver_{0}d_{1}_results.json".format(dim,name)
    ...
    def _AddReferenceValuesCheck(self):
        ...
        input_file_name = "hydraulic_fractional_solver_{0}d_{1}_results.json".format(dim,name)
