    ...
    def _ConstructScheme(self, scheme_type, solution_type):
        ...
        if(solution_type == "Quasi-Static"):
            if(rayleigh_m<1.0e-20 and rayleigh_k<1.0e-20):
                ...
                scheme =  KratosDam.TrilinosIncrementalUpdateStaticDampedScheme(rayleigh_m,rayleigh_k)
        else:
            if(scheme_type == "Newmark"):
                damp_factor_m = 0.0
            else:
                damp_factor_m = -0.01
            scheme = KratosDam.TrilinosBossakDisplacementScheme(damp_factor_m,rayleigh_m,rayleigh_k)
