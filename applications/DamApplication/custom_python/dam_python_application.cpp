...
PYBIND11_MODULE(KratosDamApplication, m)
{
    ...
    KRATOS_REGISTER_IN_PYTHON_VARIABLE( m, PLACEMENT_TEMPERATURE )

    // Nodal smoothing ownership
    KRATOS_REGISTER_IN_PYTHON_VARIABLE(m, USE_PROCESS_BASED_NODAL_CAUCHY_STRESS_EXTRAPOLATION)

    // LOCAL equivalent-strain ownership
    KRATOS_REGISTER_IN_PYTHON_VARIABLE(m, USE_PROCESS_BASED_LOCAL_EQUIVALENT_STRAIN)
