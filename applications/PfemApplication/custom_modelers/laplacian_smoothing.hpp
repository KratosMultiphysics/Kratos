...
namespace Kratos
{
   ...
   class LaplacianSmoothing
   {
               ...
               for(NodesContainerType::iterator i_node = rModelPart.NodesBegin() ; i_node != rModelPart.NodesEnd() ; i_node++)
               {
                  ...
                  if( UniquePosition[i_node->Id()] && i_node->IsNot(TO_ERASE) ){
                     ...
                     if( i_node->IsNot(BOUNDARY) ){
                        ...
                        const array_1d<double,3>& disp = i_node->FastGetSolutionStepValue(DISPLACEMENT);
                        const array_1d<double,3>& CF = i_node->FastGetSolutionStepValue(CONTACT_FORCE);
                        std::cout << " i_node " << i_node->Id() <<  " CF " << CF << std::endl;
                        bool why_IAM_HERE = false;
                        if ( fabs(CF[0]) + fabs(CF[1]) + fabs(CF[2])  > 1e-9) {
                           why_IAM_HERE = true;
                        }

                        if ( why_IAM_HERE) {
                           for (unsigned int i = 0; i < 100; i++)
                              std::cout << " BIG ERROR, i do not know how to code " << std::endl;
                           continue;
                        }
                        ...
                        if (MoveFixedNodes)
                        {
                           ...
                           if ( i_node->pGetDof(DISPLACEMENT_X)->IsFixed() == true) {
                              any_fixed = true;
                           }
                           if ( i_node->pGetDof(DISPLACEMENT_Y)->IsFixed() == true) {
                              any_fixed = true;
                           }
                           if ( i_node->pGetDof(DISPLACEMENT_Z)->IsFixed() == true) {
                              any_fixed = true;
                           }
      ...
      std::cout << " not doing boundary smoothing " << std::endl;
            return;
