...
public:
    ...
    virtual void Execute()
    {
		...
		std::cout<<" --- critical size for inside element : "<<size_for_inside_elements<<std::endl;
		std::cout<<" --- critical size for boundary element : "<<size_for_boundary_elements<<std::endl;
		...
		if(mrRemesh.Refine->RefiningOptions.Is(ModelerUtilities::REFINE_ELEMENTS) && mrRemesh.Refine->RefiningOptions.Is(ModelerUtilities::REFINE_ADD_NODES) )
		{
			...
			// loop over out mesh elements
			for(int el = 0; el< OutNumberOfElements; el++)
			{
				// check if element is a preserved elements
				if(mrRemesh.PreservedElements[el])
				{
					...
					{  
						...
						// sum up the NODAL_H of the element nodes
						prescribed_h += (nodes_begin + OutElementList[el*nds+pn]-1)->FastGetSolutionStepValue(NODAL_H);

						// count dissipative nodes, boundary nodes, new boundary nodes and boundary contact nodes of the element
					...
					//calculate the prescribed h being the average over the element nodes
					prescribed_h *= 0.3333;
					...
					if( refine_candidate )
					{
						...
						if(mrRemesh.Refine->RefiningOptions.Is(ModelerUtilities::REFINE_ELEMENTS_ON_THRESHOLD) && mrRemesh.Refine->RefiningOptions.Is(ModelerUtilities::REFINE_ELEMENTS_ON_DISTANCE))
						{
							// if( dissipative )
