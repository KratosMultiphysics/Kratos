      ...
      : public Process
   {
      ...
      private:
         ...
         bool RemoveNodesOnError(ModelPart& rModelPart, unsigned int& error_removed_nodes)
         {
            ...
            double size_for_criterion_error   = 1.0 * mrRemesh.Refine->CriticalRadius; //compared with mean node radius //LMV
            ...
            {
               if ( in->GetSolutionStepValue(LMV, 1) ) {
                  LMVSet = true;
                  break;
               }
            }
            ...
            for(ModelPart::NodesContainerType::const_iterator in = rModelPart.NodesBegin(); in != rModelPart.NodesEnd(); in++)
            {
               ...
               if( in->IsNot(BOUNDARY) && erased_nodes < 1 )
               {
                  ...
                  if(NodalError[nodes_ids[in->Id()]] < mrRemesh.Refine->ReferenceError && mean_node_radius < size_for_criterion_error)
                  {
                     ...
                     if ( in->GetSolutionStepValue(LMV) && LMVSet) {
                        //std::cout << " do nothing to erase  " << in->Id() << " X: " << in->X() << " , " << in->Y() << std::endl;
                     } else {
                        in->Set(TO_ERASE);
                        std::cout << " to erase on error " << in->Id() << " X: " << in->X() << " , " << in->Y() << std::endl;
                        any_node_removed = true;
                        error_removed_nodes++;
                     }
                  }
               ...
               if( in->IsNot(NEW_ENTITY) )
               {
                  ...
                  if (n_points_in_radius>1)
                  {
                        ...
                        //boundary nodes will be removed if they get REALLY close to another boundary node (0.2(=extra_factor) * h_factor)
                     {
                        ...
                        unsigned int k = 0;
                        unsigned int counterC2 = 0, counterC3 = 0, counterNew = 0;
                        for(std::vector<Node<3>::Pointer>::iterator nn=neighbours.begin(); nn!=neighbours.begin() + n_points_in_radius ; nn++)
                        {
                           ...
                           if ( (*nn)->Is(BOUNDARY)  && neighbour_distances[k] > 0.0 && (*nn)->IsNot(TO_ERASE) )
                           {
                              ...
                              if ( neighbour_distances[k] < size_for_wall_tip_contact_side ) {
                                 ...
                                 if ( neighbour_distances[k] < size_for_wall_tip_contact_side * SF * SF)
                                 {
                                    counterNew += 1;
                                 }
                              }
                        ...
                        else if ( counterNew > 0 && in->IsNot(NEW_ENTITY) )
                        {
                           in->Set(TO_ERASE);
                           any_node_removed = true;
                           boundary_nodes_removed++;
                           std::cout << "    RemovingCNEW CONDITION: boundary nodes are to nearer " << in->Id() << std::endl;
                           std::cout << "      X: " << in->X() << " Y: " << in->Y() << std::endl;
                        }
                     } 
                  } 
      ...
      if ( mEchoLevel > 0) {
         std::cout << " [ REMOVE NODES ON DISTANCE " << std::endl;
      }
                     ...
                     if ( in->IsNot(BOUNDARY) )
                     {
                        if( mrRemesh.Refine->RemovingOptions.Is(ModelerUtilities::REMOVE_NODES_ON_DISTANCE) ){
                           ...
                           if( erased_nodes < 1 && contact_nodes < 1){ //we release the node if no other nodes neighbours are being erased
                              ...
                              std::cout << " X Y " << in->X() << " , " << in->Y() << std::endl;
                              any_node_removed = true;
                              inside_nodes_removed++;
                              //distance_remove++;
                           }
               ...
               if( in->Is(BOUNDARY) && in->IsNot(BLOCKED) && in->IsNot(NEW_ENTITY) )
               {
                  ...
                  if(node_shared_conditions[nodeId].size()>=2){
                     ...
                     if(node_shared_conditions[nodeId][0]->IsNot(TO_ERASE) && node_shared_conditions[nodeId][1]->IsNot(TO_ERASE)){
                        ...
                        remove_S1 = false; remove_S2 = false;
