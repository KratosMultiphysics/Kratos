...
public:
    ...
    virtual void Execute()
    {
		...
		// if the REMOVE_NODES or REMOVE_BOUNDARY_NODES switch is activated, we check if the nodes got too close
	    ...
	    {
			// remove nodes on error: REMOVE_NODES_ON_ERROR || REMOVE_BOUNDARY_NODES
			...
			if (mrRemesh.Refine->RemovingOptions.Is(ModelerUtilities::REMOVE_NODES_ON_ERROR) && mrRemesh.Refine->RemovingOptions.Is(ModelerUtilities::REMOVE_NODES) ){
				...
				std::cout<<"   [ sum of removed body nodes (on error) : "<<mrRemesh.Refine->Info.BodyNodesRemoved.on_error<<" ]"<<std::endl;
			}

			// remove non convex boundaries: REMOVE_BOUNDARY_NODES
			...
			// remove nodes on distance: REMOVE_NODES_ON_DISTANCE || REMOVE_BOUNDARY_NODES_ON_DISTANCE
			...
			if (mrRemesh.Refine->RemovingOptions.Is(ModelerUtilities::REMOVE_NODES_ON_DISTANCE) || mrRemesh.Refine->RemovingOptions.Is(ModelerUtilities::REMOVE_BOUNDARY_NODES_ON_DISTANCE)){
				...
				std::cout<<"   [ sum of removed body nodes (on distance) : "<<mrRemesh.Refine->Info.BodyNodesRemoved.on_distance<<" ]"<<std::endl;
				std::cout<<"   [ sum of removed boundary nodes (on distance) : "<<mrRemesh.Refine->Info.BoundaryNodesRemoved.on_distance<<" ]"<<std::endl;
			}

			// remove contact nodes (and boundary near the contact) on distance: REMOVE_BOUNDARY_NODES_ON_DISTANCE
			if ( mrRemesh.Refine->RemovingOptions.Is(ModelerUtilities::REMOVE_BOUNDARY_NODES_ON_DISTANCE) ){
				...
				std::cout<<"   [ sum of removed (contact) body nodes (on distance) : "<<mrRemesh.Refine->Info.BodyNodesRemoved.on_distance<<" ]"<<std::endl;
				std::cout<<"   [ sum of removed (contact) boundary nodes (on distance) : "<<mrRemesh.Refine->Info.BoundaryNodesRemoved.on_distance<<" ]"<<std::endl;
...
private:
    	...
    	for(ModelPart::NodesContainerType::iterator i_node = temporal_nodes.begin() ; i_node != temporal_nodes.end() ; i_node++)
	    {
	      if( i_node->IsNot(TO_ERASE) ){
	        ...
	        if( i_node->Is(BOUNDARY) )
				...
				std::cout<<"   NODE RELEASED "<<i_node->Id()<<std::endl;
	      }
	    }
	...
	bool RemoveNodesOnContact(ModelPart& rModelPart, unsigned int& inside_nodes_removed, unsigned int& boundary_nodes_removed, bool& any_condition_removed) 
	{
		...
		//create the list of the nodes to be check during the search containing all nodes of the model part
		...
		// loop over nodes of the model part
		for(ModelPart::NodesContainerType::const_iterator in = mrModelPart.NodesBegin(); in != mrModelPart.NodesEnd(); in++)
		{
			...
			// check for nodal contact force greater 0 or TO_SPLIT or CONTACT and set on_contact_tip
			...
			// check for nodal contact force greater 0 and set on_contact_tip_strict
			if (norm_2(ContactForceNormal) > 0){
				on_contact_tip_strict = true;
			}
			
			//only old nodes are removed -> check for NEW_ENTITY
			if( in->IsNot(NEW_ENTITY) )
			{
				radius = size_for_distance_inside; // 1.0*CriticalRadius

				// define working point based on coordinates of node
				work_point[0]=in->X();
				work_point[1]=in->Y();
				work_point[2]=in->Z();

				// search point in radius of working point using nodes_tree.SearchInRadius
				n_points_in_radius = nodes_tree.SearchInRadius(work_point, radius, neighbours.begin(),neighbour_distances.begin(), num_neighbours);

				// if points within the radius are found
				if (n_points_in_radius>1)
				{
					...
					if ( in->IsNot(BOUNDARY) )
					{
						...
						if( mrRemesh.Refine->RemovingOptions.Is(ModelerUtilities::REMOVE_NODES_ON_DISTANCE) ){
							...
							if ( erased_nodes < 1 && contact_nodes < 2 && near_to_contact_nodes == 1) // we release node if it is very very near a contact
							{
								...
								std::cout <<"   RemovingC0, an interior node very very near to a (possibly) contacting node: ID "<<in->Id()<<" ["<<in->X()<<", "<<in->Y()<<", "<<in->Z()<<"]"<< std::endl;
					...
					//boundary nodes will be removed if they get REALLY close to another boundary node (0.2(=extra_factor) * h_factor)
					{
						...
						if ( counterC2 > 1 && in->IsNot(NEW_ENTITY) && on_contact_tip_strict) {
							...
							any_node_removed = true;
							boundary_nodes_removed++;
							std::cout << "    RemovingC3: a non_contacting_node was too close to a contacting. removing the non_contacting " << in->Id() << std::endl;
							std::cout << "      X: " << in->X() << " Y: " << in->Y() << std::endl;
						}
					} 
				} 
		...
		// new loop over nodes if REMOVE_BOUNDARY_NODES & REMOVE_BOUNDARY_NODES_ON_DISTANCE
    ...
    bool RemoveNodesOnDistance(ModelPart& rModelPart, unsigned int& inside_nodes_removed, unsigned int& boundary_nodes_removed, bool& any_condition_removed)
    {
		...
		for(ModelPart::NodesContainerType::const_iterator in = rModelPart.NodesBegin(); in != rModelPart.NodesEnd(); in++)
		{
			...
			// check for nodal contact force greater 0 and set contact_active
				 ...
				 //if( in->IsNot(STRUCTURE) ) {//MEANS DOFS FIXED
					...
					if ( in->IsNot(BOUNDARY) )
					{
						if( mrRemesh.Refine->RemovingOptions.Is(ModelerUtilities::REMOVE_NODES_ON_DISTANCE) )
						{
							...
							for(std::vector<Node<3>::Pointer>::iterator nn=neighbours.begin(); nn!=neighbours.begin() + n_points_in_radius ; nn++)
							{
								// count neighbouring nodes that are boundary & contact nodes
								if( (*nn)->Is(BOUNDARY) && (*nn)->Is(CONTACT) ){
									contact_nodes += 1;
								}

								// count erased neighbouring nodes
								if( (*nn)->Is(TO_ERASE) ){
									erased_nodes += 1;
								}
							}
					...
					// if node is on boundary and REMOVE_BOUNDARY_NODES & REMOVE_BOUNDARY_NODES_ON_DISTANCE & node in NOT TO_ERASE
					else if ( (mrRemesh.Refine->RemovingOptions.Is(ModelerUtilities::REMOVE_BOUNDARY_NODES) && mrRemesh.Refine->RemovingOptions.Is(ModelerUtilities::REMOVE_BOUNDARY_NODES_ON_DISTANCE)) && (in)->IsNot(TO_ERASE))
					{
						// boundary nodes will be removed if they get REALLY close to another boundary node (0.2(=extra_factor) * h_factor)
						...
						// here we loop over the neighbouring nodes and if there are nodes
						// with BOUNDARY flag and closer than 0.2*nodal_h from our node, we remove the node we are considering
						unsigned int k = 0; // visited neighbouring nodes
						unsigned int counter = 0; // neighbouring nodes that are on boundary, not set to erase, not a contact node and within the critcal radius*1.5
						
						// loop over neighbouring nodes nn
						for(std::vector<Node<3>::Pointer>::iterator nn=neighbours.begin(); nn!=neighbours.begin() + n_points_in_radius ; nn++)
						{
							bool nn_on_contact_tip = false;
							bool contact_active = false;
							...
							// set nn_on_contact_tip if nn is TO_SPLIT or CONTACT or contact_active
							...
							// if nn is BOUNDARY & nn is not on contact tip & neughbour distance < 1.5*CriticalRadius & neughbour distance > 0
						...
						// delete boundary node if not a new entity and k > 1
						if(counter > 1 && in->IsNot(NEW_ENTITY) && !on_contact_tip )
