      ...
      : public Process
   {
            ...
            mrRemesh(rRemeshingParameters)
      {
         ...
         virtual void Execute()
         {
            ...
            {
               array_1d<double, 3> ContactForce;
               bool set_contact_force = false;

               if ( ( nodes_begin + i)->SolutionStepsDataHas( CONTACT_FORCE) ) {
                   ContactForce = (nodes_begin + i)->FastGetSolutionStepValue(CONTACT_FORCE);
                   set_contact_force = true;
               }
               if ( ( nodes_begin + i)->SolutionStepsDataHas( CONTACT_NORMAL) ) {
                   ContactForce = (nodes_begin + i)->FastGetSolutionStepValue(CONTACT_NORMAL);
                   set_contact_force = true;
               }
               if ( set_contact_force ) {
                  if ( norm_2(ContactForce) > 0) {
                     //if  (fabs(ContactForce[0]) > fabs(ContactForce[1]) ) {
                     if (  ( fabs(ContactForce[0]) ) > 1e-2* fabs(ContactForce[1]) ) {
               ...
               bool &  boolHere= (nodes_begin + i)->GetSolutionStepValue( LMV);
               boolHere = false;
            }
            double Radi = xConeMax - xConeMin; 
            double xCone = xConeMin + 0.5* Radi ;

            const double FirstDistance = mRadius/2.0 * Radi;
            const double SecondDistance = mRadius * Radi;

            std::cout << " position of the cone " << yCone << std::endl;
            std::cout << " radii " << Radi << std::endl;
            std::cout << " pos " << xConeMax  << " , " << xConeMin << std::endl;



            // 2. Try to set the size deppending on the relative position with the cone
            int id = 0; 
                  ...
                  if(mrRemesh.PreservedElements[el])
                  {
                     ...
                     if ( distance < SecondDistance ) {
                        for(unsigned int pn=0; pn<nds; pn++)
                        {
                           ...
                           (nodes_begin+OutElementList[el*nds+pn]-1)->GetSolutionStepValue( LMV) = true;
                        }
                     }
