...
#if !defined(KRATOS_SET_ACTIVE_ENTITIES_MESHER_PROCESS_H_INCLUDED )
#define  KRATOS_SET_ACTIVE_ENTITIES_MESHER_PROCESS_H_INCLUDED
 ...
 */
class SetActiveEntitiesMesherProcess
    : public SetActiveFlagProcess
{
 public:
  ...
  /// Pointer definition of SetActiveEntitiesMesherProcess
  KRATOS_CLASS_POINTER_DEFINITION( SetActiveEntitiesMesherProcess );
  ...
  /// Default constructor.
  SetActiveEntitiesMesherProcess(ModelPart& rModelPart,
  ...
  /// Destructor.
  virtual ~SetActiveEntitiesMesherProcess()
  {
  }
...
#pragma omp parallel
{
  ...
  const unsigned int dimension = this->mrModelPart.ElementsBegin()->GetGeometry().WorkingSpaceDimension();

  // const unsigned int dimension = (itElem)->GetGeometry().WorkingSpaceDimension();
  ModelPart::ElementIterator ElemBegin;
  ModelPart::ElementIterator ElemEnd;
  OpenMPUtils::PartitionedIterators(this->mrModelPart.Elements(),ElemBegin,ElemEnd);
  double ModelPartVolume=0;
  if(mUnactiveSliverElements == true){
    MesherUtilities MesherUtils;
    ModelPartVolume=MesherUtils.ComputeModelPartVolume(this->mrModelPart);
  }
  for ( ModelPart::ElementIterator itElem = ElemBegin; itElem != ElemEnd; ++itElem )
  {
    ...
    if(mUnactiveSliverElements == true){
      ...
      double CriticalVolume=0.01*ModelPartVolume/double(this->mrModelPart.Elements().size());
  ...
  {
    return "SetActiveEntitiesMesherProcess";
  }
 ...
 private:
  ...
  /// Assignment operator.
  SetActiveEntitiesMesherProcess& operator=(SetActiveEntitiesMesherProcess const& rOther);

  /// Copy constructor.
  //SetActiveEntitiesMesherProcess(SetActiveEntitiesMesherProcess const& rOther);
...
inline std::istream& operator >> (std::istream& rIStream,
                                  SetActiveEntitiesMesherProcess& rThis);

/// output stream function
inline std::ostream& operator << (std::ostream& rOStream,
                                  const SetActiveEntitiesMesherProcess& rThis)
{
