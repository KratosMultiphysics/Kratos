...
namespace Kratos
{
      ...
      : public Process
   {
	...
	virtual void Execute()
    {
            ...
            {
				// set all elements from OutMesh as preserved elements (1)
				// go straigth to line 423
            ...
            else // SELECT_TESSELLATION_ELEMENTS
            {
				if( mEchoLevel > 0 )
					std::cout<<"   Start Element Selection "<<OutNumberOfElements<<std::endl;

				// set number of nodes and dimensions
				unsigned int nds = 3;
				...
				for(el=0; el<OutNumberOfElements; el++)
				{
					...
					for(unsigned int pn=0; pn<nds; pn++)
					{
						...
						//check flags on nodes and count free surface and boundary nodes
					...
					if(accepted)
					{
						...
						if(nds==4)
						{
							...
							if( sliver )
							{
								std::cout << " HELLO I AM A SLIVER " << std::endl;
								if(mrRemesh.Options.Is(ModelerUtilities::CONTACT_SEARCH))
									accepted = true;
								else
									accepted = false;
					...
					if(accepted)
					{
						...
						bool at_least_one_fix = false;

						for (unsigned int n = 0; n < nds; n++) 
						{
							if ( vertices[n].pGetDof(DISPLACEMENT_X)->IsFixed() == true)
								at_least_one_fix = true;
							if ( vertices[n].pGetDof(DISPLACEMENT_Y)->IsFixed() == true)
								at_least_one_fix = true;
							if ( vertices[n].pGetDof(DISPLACEMENT_Z)->IsFixed() == true)
								at_least_one_fix = true;
						}

						if ( at_least_one_fix == true) 
						{
							number+=1;
							mrRemesh.PreservedElements[el] = number;
							accepted = true;
							...
							std::cout <<" hello, finally i am accepting, since it might create a dirichlet boundary condition problem " << std::endl;
						}
						else
						{
							std::cout<<" Element DID NOT pass INNER/OUTER check "<<std::endl;
				...
				for(ModelPart::NodesContainerType::iterator i_node = rNodes.begin() ; i_node != rNodes.end() ; i_node++)
				{
					if( i_node->IsNot(BLOCKED)  )
					{
						if(!(i_node->Is(FREE_SURFACE) || i_node->Is(RIGID)))
						{
							i_node->Set(TO_ERASE);
							if( mEchoLevel > 0 )
								std::cout<<" NODE "<<i_node->Id()<<" ["<<i_node->X()<<", "<<i_node->Y()<<", "<<i_node->Z()<<"] RELEASE "<<std::endl;
							if( i_node->Is(BOUNDARY) )
								std::cout<<" ERROR: node "<<i_node->Id()<<" ["<<i_node->X()<<", "<<i_node->Y()<<", "<<i_node->Z()<<"] IS BOUNDARY RELEASE "<<std::endl;
							count_release++;
						}
					}
