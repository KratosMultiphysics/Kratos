...
bool malla::edt(double delta)
{
    ...
    count=0;
    nexti=0;
    while (nexti<e.len)  //  cada elemento
    {
        i=nexti;
        nexti=i+1;
        if (INFO_CL&&(i>>10)<<10==i)
        {
            cout << "\rEDT (union): " << i << "\t" << count;
            cout.flush();
        }
        do
        {
            ...
            for (j=0; j<nc; j++) // cada cara
            {
                ...
                if (iej<i)
                {
                    nexti=i;    // debe volar el de mayor indice
                    Swap(iej,i);
                }
        ...
        while (nexti<e.len)  //  cada elemento
        {
            ...
            do
            {
                ...
                for (j=0; j<nc; j++) // cada cara
                {
                    ...
                    juntae(i,iej); // lo agrega
                    re[i]=ri;
                    ce[i]=ci; // sin modificar centro y radio
                    added=true;
                    count++;
                    break; // retestea solo porque cambio la estructura de caras
                }
            }
            while(added);  //si junta vuelve a revisar
        }
