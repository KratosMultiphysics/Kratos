...
bool listnorep::operator +=(int element)  //(si puso=>t)
{
    ...
    if (element!=vertex[max])
    {
        pline::insert(element,max);
        return true;
    }
    pline::remove(1,max);
    return false;
...
{
    for (int i=0; i<intern.len; i++) *this+=intern[i];
    return *this;
}
...
{
    for (int i=0; i<intern.len; i++) remove1(intern[i]);
    return *this;
}
pline& pline::removeall(const pline& intern)
{
    for (int i=0; i<intern.len; i++) removeall(intern[i]);
    return *this;
}
