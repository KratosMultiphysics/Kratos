...
public:
    ...
    int& last() const
    {
        return vertex[len-1];   // el ultimo (ojo si len==0)
    }
    bool have(int elm) const
    {
        return index(elm)<len;   // esta en la pline ?
    }
...
public:
    ...
    const
    {
        return vertex[ciclo(n,len)];   // indice circular (malditos negativos!)
    }
...
public:
    ...
    ordlist& operator-=(int elm)
    {
        pline::remove1(elm);
        return *this;
    }
    ordlist& remove(int elm)
    {
        pline::remove1(elm);
        return *this;
    }
    ordlist& remove(const pline &intern)
    {
        pline::remove1(intern);
        return *this;
    }
};
