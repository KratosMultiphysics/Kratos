...
GeoTrussElementBase<TDim,TNumNodes>::
...
//----------------------------------------------------------------------------------------
template< unsigned int TDim, unsigned int TNumNodes >
Element::Pointer GeoTrussElementBase<TDim,TNumNodes>::
    Create(IndexType NewId,
           NodesArrayType const& rThisNodes,
           PropertiesType::Pointer pProperties) const
{
    const GeometryType& rGeom = GetGeometry();
    return Kratos::make_intrusive<GeoTrussElementBase>(NewId,
                                                       rGeom.Create(rThisNodes),
                                                       pProperties);
}
...
template< unsigned int TDim, unsigned int TNumNodes >
void GeoTrussElementBase<TDim,TNumNodes>::
    CreateElementStiffnessMatrix(MatrixType& rLocalStiffnessMatrix,
                                 const ProcessInfo& rCurrentProcessInfo)
{

    KRATOS_TRY

    CalculateElasticStiffnessMatrix(rLocalStiffnessMatrix,
                                    rCurrentProcessInfo);
...
void GeoTrussElementBase<TDim,TNumNodes>::
    CalculateDampingMatrix(MatrixType& rDampingMatrix,
                           const ProcessInfo& rCurrentProcessInfo)
{
    GeoStructuralMechanicsElementUtilities::
        CalculateRayleighDampingMatrix(*this,
                                       rDampingMatrix,
                                       rCurrentProcessInfo,
                                       TDim*TNumNodes);
}
                        ...
                        const ProcessInfo& rCurrentProcessInfo)
{
    ...
    if (GeoStructuralMechanicsElementUtilities::
        ComputeLumpedMassMatrix(GetProperties(), rCurrentProcessInfo)) {
...
template< unsigned int TDim, unsigned int TNumNodes >
void GeoTrussElementBase<TDim,TNumNodes>::
    Calculate(const Variable<Matrix>& rVariable,
              Matrix& rOutput,
              const ProcessInfo& rCurrentProcessInfo)
{
...
template< unsigned int TDim, unsigned int TNumNodes >
void GeoTrussElementBase<TDim,TNumNodes>::
    Calculate(const Variable<double>& rVariable,
              double& rOutput,
              const ProcessInfo& rCurrentProcessInfo)
{
                                 ...
                                 const ProcessInfo& rCurrentProcessInfo)
{
    ...
    if ((rVariable == CAUCHY_STRESS_VECTOR) || (rVariable == PK2_STRESS_VECTOR)) {
        ...
        const double prestress = (GetProperties().Has(TRUSS_PRESTRESS_PK2) ? GetProperties()[TRUSS_PRESTRESS_PK2] : 0.0);
...
//----------------------------------------------------------------------------------------
template< unsigned int TDim, unsigned int TNumNodes >
int GeoTrussElementBase<TDim,TNumNodes>::
    ...
    if ( TDim == 2 ) {
        ...
        for (unsigned int i=0; i<TNumNodes; ++i) {
            if (GetGeometry()[i].Z() != 0.0)
                KRATOS_ERROR << " Node with non-zero Z coordinate found. Id: " << GetGeometry()[i].Id() << std::endl;
        }
    }
    ...
    CreateTransformationMatrix(FullDofMatrixType& rRotationMatrix)
{
    ...
    BoundedMatrix<double, DIM, DIM> current_coordinate_system =
        ZeroMatrix(DIM, DIM);
    for (unsigned int i = 0; i < DIM; ++i) {
