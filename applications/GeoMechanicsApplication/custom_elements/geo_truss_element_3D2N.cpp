...
namespace Kratos {
GeoTrussElement3D2N::GeoTrussElement3D2N(IndexType NewId,
                         ...
                         PropertiesType::Pointer pProperties) const
{
    ...
    return Kratos::make_intrusive<GeoTrussElement3D2N>(NewId, rGeom.Create(rThisNodes),
            pProperties);
}
...
GeoTrussElement3D2N::Create(IndexType NewId, GeometryType::Pointer pGeom,
                         PropertiesType::Pointer pProperties) const
{
    return Kratos::make_intrusive<GeoTrussElement3D2N>(NewId, pGeom,
            pProperties);
}
...
void GeoTrussElement3D2N::GetDofList(DofsVectorType& rElementalDofList,
                                     const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY
...
void GeoTrussElement3D2N::Initialize()
{
    KRATOS_TRY
...
GeoTrussElement3D2N::CreateElementStiffnessMatrix(
...
GeoTrussElement3D2N::CalculateBodyForces()
{
...
void GeoTrussElement3D2N::CalculateOnIntegrationPoints(
...
void GeoTrussElement3D2N::GetValueOnIntegrationPoints(
...
int GeoTrussElement3D2N::Check(const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY
    ...
    KRATOS_ERROR_IF(GeoStructuralMechanicsElementUtilities::CalculateReferenceLength3D2N(*this)
...
double GeoTrussElement3D2N::CalculateGreenLagrangeStrain() const
{
...
void GeoTrussElement3D2N::CreateTransformationMatrix(
    BoundedMatrix<double, GeoTrussElement3D2N::msLocalSize,
    GeoTrussElement3D2N::msLocalSize>& rRotationMatrix)
{
...
void GeoTrussElement3D2N::WriteTransformationCoordinates(
    BoundedVector<double, GeoTrussElement3D2N::msLocalSize>
...
void GeoTrussElement3D2N::CalculateGeometricStiffnessMatrix(
    BoundedMatrix<double, GeoTrussElement3D2N::msLocalSize,
    GeoTrussElement3D2N::msLocalSize>& rGeometricStiffnessMatrix,
    const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
...
void GeoTrussElement3D2N::CalculateElasticStiffnessMatrix(
    BoundedMatrix<double, GeoTrussElement3D2N::msLocalSize,
    GeoTrussElement3D2N::msLocalSize>& rElasticStiffnessMatrix,
    const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
...
void GeoTrussElement3D2N::InitializeNonLinearIteration(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
...
void GeoTrussElement3D2N::FinalizeNonLinearIteration(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
    const Vector &temp_shape_function = ZeroVector(3);
    const Properties &MaterialProperties = GetProperties();
    const GeometryType &ElementGeometry = GetGeometry();
    mpConstitutiveLaw->FinalizeNonLinearIteration(MaterialProperties,
            ElementGeometry,temp_shape_function,rCurrentProcessInfo);
    KRATOS_CATCH("");
}


BoundedVector<double,GeoTrussElement3D2N::msLocalSize>
GeoTrussElement3D2N::GetConstitutiveLawTrialResponse(
    const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
    ...
    const double normal_force = ((stress_vector[0] + prestress) * l * A) / L0;
...
bool GeoTrussElement3D2N::HasSelfWeight() const
{
...
void GeoTrussElement3D2N::CalculateLumpedMassVector(VectorType& rMassVector)
{
    KRATOS_TRY
...
double GeoTrussElement3D2N::ReturnTangentModulus1D(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
