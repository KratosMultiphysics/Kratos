...
class KRATOS_API(GEO_MECHANICS_APPLICATION) TransientThermalWellElement : public Element
{
private:
...
public:
    KRATOS_CLASS_INTRUSIVE_POINTER_DEFINITION(TransientThermalWellElement);

    explicit TransientThermalWellElement(IndexType NewId = 0) : Element(NewId) {}

    TransientThermalWellElement(IndexType NewId, GeometryType::Pointer pGeometry)
        : Element(NewId, pGeometry)
    {
    }

    TransientThermalWellElement(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties)
    ...
    {
        return make_intrusive<TransientThermalWellElement>(NewId, GetGeometry().Create(rThisNodes), pProperties);
    }
