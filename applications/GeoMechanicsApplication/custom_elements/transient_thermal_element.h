...
public:
    ...
    {
        return make_intrusive<TransientThermalElement>(
                              ...
                              const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        const auto integration_coefficients =
            CalculateIntegrationCoefficients(det_J_container);
        const auto conductivity_matrix = CalculateConductivityMatrix(
...
private:
    void CheckDomainSize() const
    {
        ...
        KRATOS_ERROR_IF(GetGeometry().DomainSize() < min_domain_size)
            << "DomainSize smaller than " << min_domain_size << " for element "
            << Id() << std::endl;
    }
    ...
    void CheckProperty(const Kratos::Variable<double>& rVariable) const
    {
        ...
        KRATOS_ERROR_IF(GetProperties()[rVariable] < 0.0)
            << rVariable.Name() << " has an invalid value at element " << Id()
            << std::endl;
    }

    void CheckProperty(const Kratos::Variable<std::string>& rVariable,
    ...
    void CheckForNonZeroZCoordinateIn2D() const
    {
        if constexpr (TDim == 2) {
            ...
            KRATOS_ERROR_IF_NOT(pos == r_geometry.end())
                << " Node with non-zero Z coordinate found. Id: " << pos->Id()
                << std::endl;
        }
    }

    static void AddContributionsToLhsMatrix(
    ...
    void AddContributionsToRhsVector(
        VectorType& rRightHandSideVector,
        const BoundedMatrix<double, TNumNodes, TNumNodes>& rConductivityMatrix,
        ...
        const ProcessInfo& rCurrentProcessInfo) const
    {
        ...
        auto result =
            BoundedMatrix<double, TNumNodes, TNumNodes>{ZeroMatrix{TNumNodes, TNumNodes}};
        for (unsigned int integration_point_index = 0;
             integration_point_index <
