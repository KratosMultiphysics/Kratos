...
namespace Kratos
{
GeoCrBeamElement3D2N::GeoCrBeamElement3D2N(IndexType NewId,
...
Element::Pointer
GeoCrBeamElement3D2N::Create(IndexType NewId, NodesArrayType const& rThisNodes,
                          PropertiesType::Pointer pProperties) const
{
    const GeometryType& rGeom = GetGeometry();
    return Kratos::make_intrusive<GeoCrBeamElement3D2N>(NewId, rGeom.Create(rThisNodes),
            pProperties);
}

Element::Pointer
GeoCrBeamElement3D2N::Create(IndexType NewId, GeometryType::Pointer pGeom,
                          PropertiesType::Pointer pProperties) const
{
    return Kratos::make_intrusive<GeoCrBeamElement3D2N>(NewId, pGeom,
            pProperties);
}
                     ...
                     const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    for (int i = 0; i < msNumberOfNodes; ++i) {
        ...
        rResult[index + 3] = rGeom[i].GetDof(ROTATION_X).EquationId();
        rResult[index + 4] = rGeom[i].GetDof(ROTATION_Y).EquationId();
        rResult[index + 5] = rGeom[i].GetDof(ROTATION_Z).EquationId();
    }
}
...
void GeoCrBeamElement3D2N::GetSecondDerivativesVector(Vector& rValues, int Step) const
{
    ...
    for (int i = 0; i < msNumberOfNodes; ++i) {
        ...
        const auto& ang_acc = GetGeometry()[i].FastGetSolutionStepValue(ANGULAR_ACCELERATION, Step);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void GeoCrBeamElement3D2N::InitializeNonLinearIteration(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
...
void GeoCrBeamElement3D2N::GetFirstDerivativesVector(Vector& rValues, int Step) const
{
    ...
    for (int i = 0; i < msNumberOfNodes; ++i) {
        ...
        const auto& ang_vel = GetGeometry()[i].FastGetSolutionStepValue(ANGULAR_VELOCITY, Step);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BoundedVector<double, GeoCrBeamElement3D2N::msElementSize>
GeoCrBeamElement3D2N::CalculateBodyForces() const
{
    KRATOS_TRY
    ...
    for (int i = 0; i < msNumberOfNodes; ++i) {
        ...
        for (int j = 0; j < msDimension; ++j) {
            body_forces_global[j + index] =  equivalent_line_load[j] * Ncontainer(0, i) * l;
        }
    }

    // adding the nodal moments
    CalculateAndAddWorkEquivalentNodalForcesLineLoad(equivalent_line_load, body_forces_global, l);
...
void GeoCrBeamElement3D2N::
    CalculateAndAddWorkEquivalentNodalForcesLineLoad(
         const BoundedVector<double, GeoCrBeamElement3D2N::msDimension> ForceInput,
         BoundedVector<double, GeoCrBeamElement3D2N::msElementSize> & rRightHandSideVector,
         const double GeometryLength ) const
{
    ...
    geometric_orientation[0] = GetGeometry()[1].X() - GetGeometry()[0].X();
    geometric_orientation[1] = GetGeometry()[1].Y() - GetGeometry()[0].Y();
    if (msDimension == 3) {
        geometric_orientation[2] = GetGeometry()[1].Z() - GetGeometry()[0].Z();
    }
    ...
    const double custom_moment = norm_force_vector_orthogonal * GeometryLength * GeometryLength / 12.00;

    Vector moment_a = ZeroVector(msDimension);
    moment_a = MathUtils<double>::CrossProduct(geometric_orientation, load_orthogonal_direction);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vector GeoCrBeamElement3D2N::CalculateLocalNodalForces() const
{
    // deformation modes
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BoundedMatrix<double, GeoCrBeamElement3D2N::msElementSize, GeoCrBeamElement3D2N::msElementSize>
GeoCrBeamElement3D2N::CreateElementStiffnessMatrix_Material() const
{
    ...
    BoundedMatrix<double, msElementSize, msElementSize> local_stiffness_matrix = ZeroMatrix(msElementSize, msElementSize);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BoundedMatrix<double, GeoCrBeamElement3D2N::msElementSize,GeoCrBeamElement3D2N::msElementSize>
GeoCrBeamElement3D2N::CreateElementStiffnessMatrix_Geometry() const
{
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BoundedMatrix<double, GeoCrBeamElement3D2N::msLocalSize, GeoCrBeamElement3D2N::msLocalSize>
GeoCrBeamElement3D2N::CalculateDeformationStiffness() const
{

    KRATOS_TRY
    BoundedMatrix<double, msLocalSize, msLocalSize> Kd = ZeroMatrix(msLocalSize, msLocalSize);
    const double E = GetProperties()[YOUNG_MODULUS];
    const double G = CalculateShearModulus();
    const double A = GetProperties()[CROSS_AREA];
    const double L = GeoStructuralMechanicsElementUtilities::CalculateReferenceLength3D2N(*this);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BoundedMatrix<double, GeoCrBeamElement3D2N::msElementSize, GeoCrBeamElement3D2N::msElementSize>
GeoCrBeamElement3D2N::CalculateInitialLocalCS() const
{
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vector GeoCrBeamElement3D2N::GetIncrementDeformation() const
{
    KRATOS_TRY;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void GeoCrBeamElement3D2N::
    UpdateQuaternionParameters( double& rScalNodeA,
                                double& rScalNodeB,
                                Vector& rVecNodeA,
                                Vector& rVecNodeB ) const
{
    KRATOS_TRY;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void GeoCrBeamElement3D2N::SaveQuaternionParameters()
{
    KRATOS_TRY;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BoundedMatrix<double, GeoCrBeamElement3D2N::msDimension, GeoCrBeamElement3D2N::msDimension>
GeoCrBeamElement3D2N::
    UpdateRotationMatrixLocal(Vector& Bisectrix, Vector& VectorDifference) const
{
    ...
    mean_rotation_scalar = (quaternion_sca_a + quaternion_sca_b) * 0.50;
    mean_rotation_scalar = mean_rotation_scalar / scalar_diff;

    BoundedVector<double, msDimension> mean_rotation_vector = ZeroVector(msDimension);
    mean_rotation_vector = (quaternion_vec_a + quaternion_vec_b) * 0.50;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vector GeoCrBeamElement3D2N::CalculateSymmetricDeformationMode() const
{
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vector GeoCrBeamElement3D2N::CalculateAntiSymmetricDeformationMode() const
{
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BoundedMatrix<double, GeoCrBeamElement3D2N::msElementSize, GeoCrBeamElement3D2N::msLocalSize>
GeoCrBeamElement3D2N::CalculateTransformationS() const
{
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void GeoCrBeamElement3D2N::
    ConstCalculateLocalSystem( MatrixType& rLeftHandSideMatrix,
                               VectorType& rRightHandSideVector,
                               const ProcessInfo& rCurrentProcessInfo ) const
{
    KRATOS_TRY;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void GeoCrBeamElement3D2N::
    ConstCalculateLeftHandSide( MatrixType& rLeftHandSideMatrix,
                                const ProcessInfo& rCurrentProcessInfo) const
{
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Vector GeoCrBeamElement3D2N::CalculateGlobalNodalForces() const
{
    Vector nodal_forces_local_qe = CalculateLocalNodalForces();
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BoundedVector<double, GeoCrBeamElement3D2N::msLocalSize>
GeoCrBeamElement3D2N::CalculateElementForces() const
{
    KRATOS_TRY;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
double GeoCrBeamElement3D2N::CalculatePsi(const double I, const double A_eff) const
{
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BoundedVector<double, GeoCrBeamElement3D2N::msLocalSize>
GeoCrBeamElement3D2N::GetCurrentNodalPosition() const
{
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
BoundedMatrix<double, GeoCrBeamElement3D2N::msElementSize, GeoCrBeamElement3D2N::msElementSize>
  GeoCrBeamElement3D2N::GetTransformationMatrixGlobal() const
{
    KRATOS_TRY;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void GeoCrBeamElement3D2N::
    BuildSingleMassMatrix( MatrixType& rMassMatrix,
                           double Phi,
                           double CT,
                           double CR,
                           double L,
                           double dir ) const
{
    ...
    BoundedMatrix<double, MatSize, MatSize> temp_mass_matrix = ZeroMatrix(MatSize, MatSize);
    const double Phi2 = Phi * Phi;
    const double L2 = L * L;

    temp_mass_matrix(0, 0) = (13.00 / 35.00) + (7.00 / 10.00) * Phi + (1.00 / 3.00) * Phi2;
    temp_mass_matrix(0, 1) = dir * ((11.00 / 210.00) + (11.00 / 210.00) * Phi + (1.00 / 24.00) * Phi2) * L;
    temp_mass_matrix(0, 2) = (9.00 / 70.00) + (3.00 / 10.00) * Phi + (1.00 / 6.00) * Phi2;
    temp_mass_matrix(0, 3) =-((13.00 / 420.00) + (3.00 / 40.00) * Phi + (1.00 / 24.00) * Phi2) * L * dir;
    temp_mass_matrix(1, 0) = temp_mass_matrix(0, 1);
    temp_mass_matrix(1, 1) = ((1.00 / 105.00) + (1.00 / 60.00) * Phi + (1.00 / 120.00) * Phi2) * L2;
    temp_mass_matrix(1, 2) = dir * ((13.00 / 420.00) + (3.00 / 40.00) * Phi + (1.00 / 24.00) * Phi2) * L;
    temp_mass_matrix(1, 3) =-((1.00 / 140.00) + (1.00 / 60.00) * Phi + (1.00 / 120.00) * Phi2) * L2;
    temp_mass_matrix(2, 0) = temp_mass_matrix(0, 2);
    temp_mass_matrix(2, 1) = temp_mass_matrix(1, 2);
    temp_mass_matrix(2, 2) = (13.00 / 35.00) + (7.00 / 10.00) * Phi + (1.00 / 3.00) * Phi2;
    temp_mass_matrix(2, 3) =-((11.00 / 210.00) + (11.00 / 210.00) * Phi + (1.00 / 24.00) * Phi2) * L * dir;
    ...
    temp_mass_matrix(3, 3) = ((1.00 / 105.00) + (1.00 / 60.00) * Phi + (1.00 / 120.00) * Phi2) * L2;
    ...
    temp_mass_matrix(1, 0) = temp_mass_matrix(0, 1);
    temp_mass_matrix(1, 1) = ((2.00 / 15.00) + (1.00 / 6.00) * Phi + (1.00 / 3.00) * Phi2) * L2;
    temp_mass_matrix(1, 2) = dir * ((-1.00 / 10.00) + (1.00 / 2.00) * Phi) * L;
    temp_mass_matrix(1, 3) =-((1.00 / 30.00) + (1.00 / 6.00) * Phi - (1.00 / 6.00) * Phi2) * L2;
    temp_mass_matrix(2, 0) = temp_mass_matrix(0, 2);
    temp_mass_matrix(2, 1) = temp_mass_matrix(1, 2);
    ...
    temp_mass_matrix(3, 1) = temp_mass_matrix(1, 3);
    temp_mass_matrix(3, 2) = temp_mass_matrix(2, 3);
    temp_mass_matrix(3, 3) = ((2.00 / 15.00) + (1.00 / 6.00) * Phi + (1.00 / 3.00) * Phi2) * L2;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void GeoCrBeamElement3D2N::
    CalculateLumpedMassMatrix( MatrixType& rMassMatrix,
                               const ProcessInfo& rCurrentProcessInfo ) const
{
    KRATOS_TRY;
    if (rMassMatrix.size1() != msElementSize) {
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
GeoCrBeamElement3D2N::IntegrationMethod
GeoCrBeamElement3D2N::GetIntegrationMethod() const
{
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
double GeoCrBeamElement3D2N::CalculateShearModulus() const
{
    KRATOS_TRY;
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
int GeoCrBeamElement3D2N::Check(const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY
    const double numerical_limit = std::numeric_limits<double>::epsilon();
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void GeoCrBeamElement3D2N::FinalizeNonLinearIteration(const ProcessInfo& rCurrentProcessInfo)
{
