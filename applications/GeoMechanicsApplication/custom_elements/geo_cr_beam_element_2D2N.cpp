                          ...
                          PropertiesType::Pointer pProperties) const
{
    ...
    return Kratos::make_intrusive<GeoCrBeamElement2D2N>(NewId, rGeom.Create(rThisNodes),
            pProperties);
}
                     ...
                     const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    for (int i = 0; i < msNumberOfNodes; ++i) {
        ...
        rResult[index + 2] = rGeom[i].GetDof(ROTATION_Z).EquationId();
    }
}

void GeoCrBeamElement2D2N::GetDofList(DofsVectorType& rElementalDofList,
                                      const ProcessInfo& rCurrentProcessInfo) const
{
...
void GeoCrBeamElement2D2N::Initialize(const ProcessInfo& rCurrentProcessInfo)
{
    ...
    KRATOS_TRY;
    // KRATOS_INFO("InitializeSolutionStep: Initialize") << std::endl;
...
BoundedVector<double, GeoCrBeamElement2D2N::msElementSize>
GeoCrBeamElement2D2N::CalculateBodyForces() const
{
    KRATOS_TRY
...
void GeoCrBeamElement2D2N::CalculateAndAddWorkEquivalentNodalForcesLineLoad(
...
double GeoCrBeamElement2D2N::CalculateShearModulus() const
{
    KRATOS_TRY;
...
double GeoCrBeamElement2D2N::CalculatePsi(const double I, const double A_eff) const
{
...
double GeoCrBeamElement2D2N::CalculateInitialElementAngle() const
{
    KRATOS_TRY;
    const double numerical_limit = std::numeric_limits<double>::epsilon();
...
double GeoCrBeamElement2D2N::CalculateDeformedElementAngle()
{
    KRATOS_TRY;
    const double numerical_limit = std::numeric_limits<double>::epsilon();
...
double GeoCrBeamElement2D2N::CalculateLength() const
{
    KRATOS_TRY;
    return GeoStructuralMechanicsElementUtilities::CalculateCurrentLength2D2N(*this);
    KRATOS_CATCH("")
}

BoundedMatrix<double, GeoCrBeamElement2D2N::msElementSize,
GeoCrBeamElement2D2N::msLocalSize>
GeoCrBeamElement2D2N::CalculateTransformationS() const
{
    KRATOS_TRY;
    const double L = CalculateLength();
...
BoundedMatrix<double, GeoCrBeamElement2D2N::msLocalSize,
GeoCrBeamElement2D2N::msLocalSize>
GeoCrBeamElement2D2N::CreateElementStiffnessMatrix_Kd_mat() const
{
    KRATOS_TRY
    // element properties
    const double E = GetProperties()[YOUNG_MODULUS];
...
BoundedMatrix<double, GeoCrBeamElement2D2N::msLocalSize,
GeoCrBeamElement2D2N::msLocalSize>
GeoCrBeamElement2D2N::CreateElementStiffnessMatrix_Kd_geo() const
{
    KRATOS_TRY
    // element properties
    const double L = CalculateLength();
...
GeoCrBeamElement2D2N::CreateElementStiffnessMatrix_Kr() const
{
    KRATOS_TRY
    // element properties
    const double L = CalculateLength();
...
BoundedMatrix<double, GeoCrBeamElement2D2N::msElementSize,
GeoCrBeamElement2D2N::msElementSize>
GeoCrBeamElement2D2N::CreateElementStiffnessMatrix_Total() const
{
    KRATOS_TRY
    // co-rotating K
    BoundedMatrix<double, msElementSize, msElementSize> K_r = CreateElementStiffnessMatrix_Kr();

    // element K (mat+geo)
    BoundedMatrix<double, msLocalSize, msLocalSize> K_d_mat = CreateElementStiffnessMatrix_Kd_mat();
    BoundedMatrix<double, msLocalSize, msLocalSize> K_d_geo = CreateElementStiffnessMatrix_Kd_geo();
    BoundedMatrix<double, msLocalSize, msLocalSize> K_d = K_d_mat + K_d_geo;

    BoundedMatrix<double, msElementSize, msLocalSize> S = CalculateTransformationS();
    BoundedMatrix<double, msElementSize, msElementSize> K_d_element = prod(K_d, Matrix(trans(S)));
    K_d_element = prod(S, K_d_element);

    // total K
    BoundedMatrix<double, msElementSize, msElementSize> K_total = ZeroMatrix(msElementSize, msElementSize);
    K_total += K_r;
    K_total += K_d_element;
...
BoundedVector<double, GeoCrBeamElement2D2N::msLocalSize>
GeoCrBeamElement2D2N::CalculateDeformationParameters()
{
    KRATOS_TRY;
    // calculate v
    ...
    BoundedVector<double, msLocalSize> deformation_parameters = ZeroVector(msLocalSize);
    deformation_parameters[0] =
        CalculateLength() - GeoStructuralMechanicsElementUtilities::CalculateReferenceLength2D2N(*this);
    ...
    deformation_parameters[2] = Modulus2Pi(deformation_parameters[2] + Globals::Pi) - Globals::Pi;
...
BoundedVector<double, GeoCrBeamElement2D2N::msLocalSize>
GeoCrBeamElement2D2N::CalculateInternalStresses_DeformationModes()
{
    KRATOS_TRY;
    // calculate t

    BoundedVector<double, msLocalSize> deformation_stresses = ZeroVector(msLocalSize);

    BoundedVector<double, msLocalSize> deformation_modes = CalculateDeformationParameters();
...
BoundedMatrix<double, GeoCrBeamElement2D2N::msElementSize,
GeoCrBeamElement2D2N::msElementSize>
GeoCrBeamElement2D2N::CreateRotationMatrix()
{
    KRATOS_TRY;
    ...
    BoundedMatrix<double, msElementSize, msElementSize> rotation_matrix = ZeroMatrix(msElementSize, msElementSize);
...
void GeoCrBeamElement2D2N::GlobalizeMatrix(Matrix& A)
{
    KRATOS_TRY;
    BoundedMatrix<double, msElementSize, msElementSize> R = CreateRotationMatrix();
...
void GeoCrBeamElement2D2N::GlobalizeVector(Vector& A)
{
    KRATOS_TRY;
    BoundedMatrix<double, msElementSize, msElementSize> R = CreateRotationMatrix();
    A = prod(R, A);
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    const unsigned int& write_points_number = GetGeometry().IntegrationPointsNumber(Kratos::GeometryData::GI_GAUSS_3);
...
GeoCrBeamElement2D2N::IntegrationMethod
GeoCrBeamElement2D2N::GetIntegrationMethod() const
{
...
BoundedVector<double, GeoCrBeamElement2D2N::msElementSize>
GeoCrBeamElement2D2N::ReturnElementForces_Local()
{
    KRATOS_TRY;
    // calculate qe

    BoundedMatrix<double, msElementSize, msLocalSize> S = CalculateTransformationS();
    BoundedVector<double, msLocalSize> t = CalculateInternalStresses_DeformationModes();
...
double GeoCrBeamElement2D2N::Modulus2Pi(double A) const
{
    KRATOS_TRY;
...
int GeoCrBeamElement2D2N::Check(const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY
    const double numerical_limit = std::numeric_limits<double>::epsilon();
    ...
    KRATOS_ERROR_IF(GeoStructuralMechanicsElementUtilities::CalculateReferenceLength2D2N(*this)
