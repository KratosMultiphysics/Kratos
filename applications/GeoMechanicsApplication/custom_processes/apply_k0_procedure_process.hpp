...
public:
    ...
    std::string Info() const override { return "ApplyK0ProcedureProcess"; }
...
private:
    ...
    void CalculateK0Stresses(Element& rElement)
    {
        ...
        if (rProp.Has(K0_NC)) {
            ...
            if (phi < 0. || phi > 90.) {
                KRATOS_ERROR << "friction angle Phi out of range in ApplyK0ProcedureProcess: " << phi
                             << std::endl;
            }
        ...
        const auto PoissonUR =
            rProp.Has(POISSON_UNLOADING_RELOADING) ? rProp[POISSON_UNLOADING_RELOADING] : 0.;
            ...
            for (int i_dir = 0; i_dir <= 2; ++i_dir) {
                if (i_dir != k0_main_direction) {
                    rStressVectors[g_point][i_dir] =
                        k0_vector[i_dir] * rStressVectors[g_point][k0_main_direction];
                }
            }
