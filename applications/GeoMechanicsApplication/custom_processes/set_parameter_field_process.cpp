...
void SetParameterFieldProcess::SetParameterFieldUsingInputFunction(const Variable<double>& rVar)
{
    ...
    for (Element& r_element : mrModelPart.Elements()) {
        ...
        const double val = parameter_function.CallFunction(
            r_geom.Center().X(), r_geom.Center().Y(), r_geom.Center().Z(), current_time, 0, 0, 0);
                                                                     ...
                                                                     const Parameters& rParameters)
{
    ...
    KRATOS_ERROR_IF_NOT(r_data_vector.size() == mrModelPart.Elements().size())
        << "The parameter field "
           "does not have the same size as the amount of elements within the model part!"
        << std::endl;
                                                                     ...
                                                                     const Parameters& rParameters)
{
    ...
    KRATOS_ERROR_IF_NOT(r_data_matrix.size1() == mrModelPart.Elements().size())
        << "The parameter field "
           "does not have the same size as the amount of elements within the model part!"
        << std::endl;
