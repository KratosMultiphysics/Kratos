    ...
    void ExecuteFinalizeSolutionStep() override
    {
        ...
        if (mrModelPart.NumberOfConditions() > 0) {
            ...
            block_for_each(mrModelPart.Conditions(), [&StressTensorSize](Condition& rCondition) {
                ...
                noalias(NodalStressMatrix) =
                    0.5 * (rGeom[0].FastGetSolutionStepValue(NODAL_CAUCHY_STRESS_TENSOR) +
                           rGeom[1].FastGetSolutionStepValue(NODAL_CAUCHY_STRESS_TENSOR));
                Vector PrincipalStresses(StressTensorSize);
                noalias(PrincipalStresses) =
                    SolidMechanicsMathUtilities<double>::EigenValuesDirectMethod(NodalStressMatrix);
...
#pragma omp critical
    ...
    std::string Info() const override { return "PeriodicInterfaceProcess"; }
...
inline std::istream&
operator>>(std::istream& rIStream, PeriodicInterfaceProcess& rThis);

/// output stream function
inline std::ostream& operator<<(std::ostream& rOStream, const PeriodicInterfaceProcess& rThis)
{
