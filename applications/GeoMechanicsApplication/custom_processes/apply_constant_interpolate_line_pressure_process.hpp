...
private:
                                                     ...
                                                     double& coordinate)
    {
        ...
        if (std::abs(CoordinatesRight[mHorizontalDirection] - CoordinatesLeft[mHorizontalDirection]) > TINY) {
            ...
            pressure = slopeP * (rNode.Coordinates()[mHorizontalDirection] - CoordinatesLeft[mHorizontalDirection]) +
                       pressureLeft;

            const double slopeY =
                (CoordinatesRight[mGravityDirection] - CoordinatesLeft[mGravityDirection]) /
                (CoordinatesRight[mHorizontalDirection] - CoordinatesLeft[mHorizontalDirection]);
            coordinate = slopeY * (rNode.Coordinates()[mHorizontalDirection] - CoordinatesLeft[mHorizontalDirection]) +
    ...
    void InterpolateBoundaryPressure(
        const Node& rNode, const Node* LeftNode, const Node* RightNode, double& pressure, double& coordinate)
    {
        const Variable<double>& var = KratosComponents<Variable<double>>::Get(mVariableName);
    ...
    Node* FindClosestNodeOnBoundaryNodes(const Node& rNode, const std::vector<Node*>& BoundaryNodes, const bool isBottom)
    {
        ...
        for (unsigned int i = 0; i < BoundaryNodes.size(); ++i) {
            if (std::abs(BoundaryNodes[i]->Coordinates()[mHorizontalDirection] - HorizontalCoordinate) <=
                horizontalDistance) {
                horizontalDistance =
                    std::abs(BoundaryNodes[i]->Coordinates()[mHorizontalDirection] - HorizontalCoordinate);
    ...
    void FindLeftBoundaryNodes(const Node& rNode, const std::vector<Node*>& BoundaryNodes, std::vector<Node*>& LeftBoundaryNodes)
    {
        for (unsigned int i = 0; i < BoundaryNodes.size(); ++i) {
                ...
                for (unsigned int i = 0; i < ElementIDs[iPoint].size(); ++i) {
                    ...
                    if (found) {
                        std::vector<int>::iterator it =
                            std::find(SharedElementIDs.begin(), SharedElementIDs.end(), iElementID);
