...
3. A settlement stage (100 days or 8640000 seconds), the abc is applied, but the keyword `GEO_DRAINAGE_TYPE` is set to `FULLY_COUPLED` such that settlement may influence water pressures. The keyword `reset_displacement` is set to `true` here, such that the total displacements start counting from the start of this stage.
