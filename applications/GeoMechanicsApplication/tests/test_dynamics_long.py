...
class KratosGeoMechanicsDynamicsLongTests(KratosUnittest.TestCase):
    ...
    def test_constant_point_load_2d_axi(self):
        ...
        test_name = 'test_constant_point_load_2d_axi'
        ...
        # get expected results
        with open(os.path.join(file_path, "expected_json_output.json")) as fp:  #TODO
            expected_result = json.load(fp)
        ...
        if CHECK_RESULTS:
            ...
            for node in nodes:
                ...
                time_normalized = np.asarray(time) * cs / x_coord
                node_id = node.Id
                vert_displ = calculated_result[f"NODE_{node_id}"]["DISPLACEMENT_Y"]
                vert_displ = - np.asarray(vert_displ) * shear_modulus * x_coord / load_value
                calculated_vert_displacement.append(vert_displ)
                
                for time_step in time:
                    vert_displ_analytic = analytical_solution.calculate_vertical_displacement(x_coord, time_step, load_value)
