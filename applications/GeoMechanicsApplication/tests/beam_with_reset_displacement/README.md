# Staged analysis on a beam with reset totals option

This test asserts the 'reset_totals' option of the LinearTimoshenkoBeamElement2D2N element works as expected. It exists of 4 stages, with different settings for 'reset_totals' and applied load.
...
2.  Essentially the same as stage 1: due to the 'reset_totals' option being true, the stage displacements as well as the total displacements are expected to be 0.0 after this stage.
3.  The 'reset_totals' option is set to false again. The load is still the same. We expect that nothing changes.
4.  The load is removed and 'reset_totals' is kept on false. The beam is expected to bend upwards again to the original position.
