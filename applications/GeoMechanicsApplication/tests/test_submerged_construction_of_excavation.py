...
class KratosGeoMechanicsSubmergedConstructionOfExcavation(KratosUnittest.TestCase):
    ...
    def run_simulation(self, sub_directory_name):
        ...
        if test_helper.want_test_plots():
            output_data_wall = output_reader.read_output_from(os.path.join(project_path, "3_Wall_installation_wall.post.res"))
            ...
            coordinates = test_helper.read_coordinates_from_post_msh_file(
                pathlib.Path(project_path) / "3_Wall_installation_wall.post.msh", node_ids=node_ids
            )
            y_coords = [coord[1] for coord in coordinates]
            axial_forces = GiDOutputFileReader.nodal_values_at_time("AXIAL_FORCE", time, output_data_wall, node_ids=node_ids)
