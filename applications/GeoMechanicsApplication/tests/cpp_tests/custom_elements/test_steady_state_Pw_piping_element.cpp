...
KRATOS_TEST_CASE_IN_SUITE(CalculateEquilibriumPipeHeight, KratosGeoMechanicsFastSuiteWithoutKernel)
{
    ...
    double expected_eq_height = p_element->CalculateEquilibriumPipeHeight(
        *p_elem_prop, Geom, p_elem_prop->GetValue(PIPE_ELEMENT_LENGTH));

    KRATOS_EXPECT_NEAR(expected_eq_height, 0.000489, 1.0e-6);
}
...
KRATOS_TEST_CASE_IN_SUITE(CalculateHeadGradient, KratosGeoMechanicsFastSuiteWithoutKernel)
{
    ...
    double expected_gradient =
        p_element->CalculateHeadGradient(*p_elem_prop, Geom, p_elem_prop->GetValue(PIPE_ELEMENT_LENGTH));

    // assert gradient
    // expected gradient should be 2. Test is failing on purpose to check CI
    KRATOS_EXPECT_NEAR(expected_gradient, 2, 1.0e-10);
}
} // namespace Kratos::Testing
