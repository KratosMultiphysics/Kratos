...
public:
    ...
    {
        rValues.GetStressVector() = 2 * rValues.GetStrainVector();
    };
...
//     };
//     std::vector<ConstitutiveLaw::Pointer> mock_laws{std::make_shared<MockLaw>(),
//     std::make_shared<MockLaw>()}; Geo::ConstitutiveLawsGetter constitutive_laws_getter =
//         [mock_laws]() -> const std::vector<ConstitutiveLaw::Pointer>& { return mock_laws; };
...
//
//     const auto expected_stifness_force = BoundedVector<double, 4>(4);
//
//     KRATOS_EXPECT_VECTOR_NEAR(actual_stiffness_force, expected_stifness_force, 1e-4);
// }
} // namespace Kratos::Testing