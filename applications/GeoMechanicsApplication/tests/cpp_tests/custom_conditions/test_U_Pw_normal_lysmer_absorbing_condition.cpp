...
////PointsArrayType
//Element::GeometryType::Pointer CreateMockGeometry2D3N(Kratos::PointerVector<Node>& rConditionNodes, ModelPart& rModelPart)
//{
//
...
//
//	auto p_geometry = Kratos::make_shared<Kratos::Triangle2D3<Element::NodeType>>(
//        node_1,
//        node_2,
//        node_3);
//	return p_geometry;
...
Element::GeometryType::Pointer CreateMockGeometry2D4N(Kratos::PointerVector<Node>& rConditionNodes, ModelPart& rModelPart)
{
    ...
    auto p_geometry = Kratos::make_shared<Kratos::Quadrilateral2D4<Element::NodeType>>(
        node_1,
        node_2,
        node_3,
        node_4);
...
Element::GeometryType::Pointer CreateMockGeometry2D6N(Kratos::PointerVector<Node>& rConditionNodes, ModelPart& rModelPart)
{
...
Element::GeometryType::Pointer CreateMockGeometry3D4N(Kratos::PointerVector<Node>& rConditionNodes, ModelPart& rModelPart)
{
    ...
    auto p_geometry = Kratos::make_shared<Kratos::Tetrahedra3D4<Element::NodeType>>(
        node_1,
        node_2,
        node_3,
        node_4);
...
Element::GeometryType::Pointer CreateMockGeometry3D8N(Kratos::PointerVector<Node>& rConditionNodes, ModelPart& rModelPart)
{
    ...
    auto node_5 = rModelPart.CreateNewNode(5, 1.0, 1.0, 0.0);
    auto node_6 = rModelPart.CreateNewNode(6, 1.0, 1.0, 0.0);
    auto node_7 = rModelPart.CreateNewNode(7, 1.0, 1.0, 0.0);
    auto node_8 = rModelPart.CreateNewNode(8, 1.0, 1.0, 0.0);

    auto p_geometry = Kratos::make_shared<Kratos::Hexahedra3D8<Element::NodeType>>(
        node_1,
        node_2,
        node_3,
        node_4,
...
Element::GeometryType::Pointer CreateMockGeometry3D10N(Kratos::PointerVector<Node>& rConditionNodes, ModelPart& rModelPart)
{
...
Element::GeometryType::Pointer CreateMockGeometry3D20N(Kratos::PointerVector<Node>& rConditionNodes, ModelPart& rModelPart)
{
...
void SetPropertiesAndInitialize(ModelPart::ConditionType::Pointer pCondition, Element::GeometryType::Pointer  pNeighbourGeometry, ModelPart& rModelPart)
{
    ...
    const auto p_neighbour_element = Kratos::make_intrusive<MockElement>(1, pNeighbourGeometry, p_neighbour_prop);
...
ModelPart::ConditionType::Pointer SetUpUPwLysmerAbsorbingCondition2D2NCondition(ModelPart& rModelPart)
{
	...
	Element::GeometryType::Pointer p_neighbour_geometry = CreateMockGeometry2D4N(p_cond->pGetGeometry()->Points(), rModelPart);
...
ModelPart::ConditionType::Pointer SetUpUPwLysmerAbsorbingCondition2D3NCondition(ModelPart& rModelPart)
{
    ...
    Element::GeometryType::Pointer p_neighbour_geometry = CreateMockGeometry2D6N(p_cond->pGetGeometry()->Points(), rModelPart);
...
ModelPart::ConditionType::Pointer SetUpUPwLysmerAbsorbingCondition3D3NCondition(ModelPart& rModelPart)
{
    // Set the element properties
    auto cond_prop = rModelPart.CreateNewProperties(0);
    ...
    std::vector<ModelPart::IndexType> cond_nodes{ 1, 2, 3 };
    ModelPart::ConditionType::Pointer p_cond =
        rModelPart.CreateNewCondition("UPwLysmerAbsorbingCondition3D3N", 1, cond_nodes, cond_prop);

    Element::GeometryType::Pointer p_neighbour_geometry = CreateMockGeometry3D4N(p_cond->pGetGeometry()->Points(), rModelPart);
...
ModelPart::ConditionType::Pointer SetUpUPwLysmerAbsorbingCondition3D4NCondition(ModelPart& rModelPart)
{
	// Set the element properties
	auto cond_prop = rModelPart.CreateNewProperties(0);
	...
	auto node_4 = rModelPart.CreateNewNode(4, 0.0, 0.1, 0.0);
	// Create the test Lysmer condition
	std::vector<ModelPart::IndexType> cond_nodes{ 1, 2, 3, 4 };
	ModelPart::ConditionType::Pointer p_cond =
		rModelPart.CreateNewCondition("UPwLysmerAbsorbingCondition3D4N", 1, cond_nodes, cond_prop);
	Element::GeometryType::Pointer p_neighbour_geometry = CreateMockGeometry3D8N(p_cond->pGetGeometry()->Points(), rModelPart);
	SetPropertiesAndInitialize(p_cond, p_neighbour_geometry, rModelPart);
	return p_cond;
}
...
ModelPart::ConditionType::Pointer SetUpUPwLysmerAbsorbingCondition3D6NCondition(ModelPart& rModelPart)
{
    ...
    Element::GeometryType::Pointer p_neighbour_geometry = CreateMockGeometry3D10N(p_cond->pGetGeometry()->Points(), rModelPart);
    SetPropertiesAndInitialize(p_cond, p_neighbour_geometry, rModelPart);
    return p_cond;
}
...
ModelPart::ConditionType::Pointer SetUpUPwLysmerAbsorbingCondition3D8NCondition(ModelPart& rModelPart)
{
    ...
    Element::GeometryType::Pointer p_neighbour_geometry = CreateMockGeometry3D20N(p_cond->pGetGeometry()->Points(), rModelPart);
    SetPropertiesAndInitialize(p_cond, p_neighbour_geometry, rModelPart);
    return p_cond;
}

void write_matrix_to_csv(const boost::numeric::ublas::matrix<double>& mat, const std::string& filename) {
    ...
    for (std::size_t i = 0; i < mat.size1(); ++i) {
        for (std::size_t j = 0; j < mat.size2(); ++j) {
            file << mat(i, j);
            if (j < mat.size2() - 1) file << ", ";  // Add comma if not last column
        }
...
KRATOS_TEST_CASE_IN_SUITE(CalculateLocalSystemUPwNormalLysmerAbsorbingCondition2D3N, KratosGeoMechanicsFastSuite)
{
    ...
    //write_matrix_to_csv(rLeftHandSideMatrix, "C:/software_development/Kratos2/applications/GeoMechanicsApplication/left_hand_side_matrix.csv"); 
    ...
    Vector expected_rhs = ZeroVector(condition_size);
	expected_rhs[0] = -(expected_matrix(0, 0) * 1.0 + expected_matrix(0, 2) * 3.0 + expected_matrix(0, 4) * 2.0);
	expected_rhs[1] = -(expected_matrix(1, 1) * 2.0 + expected_matrix(1, 3) * 4.0 + expected_matrix(1, 5) * 3.0);
	expected_rhs[2] = -(expected_matrix(2, 0) * 1.0 + expected_matrix(2, 2) * 3.0 + expected_matrix(2, 4) * 2.0);
	expected_rhs[3] = -(expected_matrix(3, 1) * 2.0 + expected_matrix(3, 3) * 4.0 + expected_matrix(3, 5) * 3.0);
	expected_rhs[4] = -(expected_matrix(4, 0) * 1.0 + expected_matrix(4, 2) * 3.0 + expected_matrix(4, 4) * 2.0);
	expected_rhs[5] = -(expected_matrix(5, 1) * 2.0 + expected_matrix(5, 3) * 4.0 + expected_matrix(5, 5) * 3.0);
...
KRATOS_TEST_CASE_IN_SUITE(CalculateDampingMatrixUPwNormalLysmerAbsorbingCondition2D3N, KratosGeoMechanicsFastSuite)
{
    ...
    //write_matrix_to_csv(rLeftHandSideMatrix, "C:/software_development/Kratos2/applications/GeoMechanicsApplication/damping_matrix2.csv"); 
...
KRATOS_TEST_CASE_IN_SUITE(CalculateLocalSystemUPwNormalLysmerAbsorbingCondition3D6N, KratosGeoMechanicsFastSuite)
{
    ...
    Vector expected_rhs = ZeroVector(condition_size);
	expected_rhs[0] = -(expected_matrix(0, 0) * 1.0 + expected_matrix(0, 3) * 2.0 + expected_matrix(0, 6) * 3.0 + expected_matrix(0, 9) * 1.5 + expected_matrix(0, 12) * 2.5 + expected_matrix(0, 15) * 2.0);
	expected_rhs[1] = -(expected_matrix(1, 1) * 2.0 + expected_matrix(1, 4) * 3.0 + expected_matrix(1, 7) * 4.0 + expected_matrix(1, 10) * 2.5 + expected_matrix(1, 13) * 3.5 + expected_matrix(1, 16) * 3.0);
	expected_rhs[2] = -(expected_matrix(2, 2) * 3.0 + expected_matrix(2, 5) * 4.0 + expected_matrix(2, 8) * 5.0 + expected_matrix(2, 11) * 3.5 + expected_matrix(2, 14) * 4.5 + expected_matrix(2, 17) * 4.0);

	expected_rhs[3] = -(expected_matrix(3, 0) * 1.0 + expected_matrix(3, 3) * 2.0 + expected_matrix(3, 6) * 3.0 + expected_matrix(3, 9) * 1.5 + expected_matrix(3, 12) * 2.5 + expected_matrix(3, 15) * 2.0);
	expected_rhs[4] = -(expected_matrix(4, 1) * 2.0 + expected_matrix(4, 4) * 3.0 + expected_matrix(4, 7) * 4.0 + expected_matrix(4, 10) * 2.5 + expected_matrix(4, 13) * 3.5 + expected_matrix(4, 16) * 3.0);
	expected_rhs[5] = -(expected_matrix(5, 2) * 3.0 + expected_matrix(5, 5) * 4.0 + expected_matrix(5, 8) * 5.0 + expected_matrix(5, 11) * 3.5 + expected_matrix(5, 14) * 4.5 + expected_matrix(5, 17) * 4.0);

	expected_rhs[6] = -(expected_matrix(6, 0) * 1.0 + expected_matrix(6, 3) * 2.0 + expected_matrix(6, 6) * 3.0 + expected_matrix(6, 9) * 1.5 + expected_matrix(6, 12) * 2.5 + expected_matrix(6, 15) * 2.0);
	expected_rhs[7] = -(expected_matrix(7, 1) * 2.0 + expected_matrix(7, 4) * 3.0 + expected_matrix(7, 7) * 4.0 + expected_matrix(7, 10) * 2.5 + expected_matrix(7, 13) * 3.5 + expected_matrix(7, 16) * 3.0);
	expected_rhs[8] = -(expected_matrix(8, 2) * 3.0 + expected_matrix(8, 5) * 4.0 + expected_matrix(8, 8) * 5.0 + expected_matrix(8, 11) * 3.5 + expected_matrix(8, 14) * 4.5 + expected_matrix(8, 17) * 4.0);

	expected_rhs[9] = -(expected_matrix(9, 0) * 1.0 + expected_matrix(9, 3) * 2.0 + expected_matrix(9, 6) * 3.0 + expected_matrix(9, 9) * 1.5 + expected_matrix(9, 12) * 2.5 + expected_matrix(9, 15) * 2.0);
	expected_rhs[10] = -(expected_matrix(10, 1) * 2.0 + expected_matrix(10, 4) * 3.0 + expected_matrix(10, 7) * 4.0 + expected_matrix(10, 10) * 2.5 + expected_matrix(10, 13) * 3.5 + expected_matrix(10, 16) * 3.0);
	expected_rhs[11] = -(expected_matrix(11, 2) * 3.0 + expected_matrix(11, 5) * 4.0 + expected_matrix(11, 8) * 5.0 + expected_matrix(11, 11) * 3.5 + expected_matrix(11, 14) * 4.5 + expected_matrix(11, 17) * 4.0);

	expected_rhs[12] = -(expected_matrix(12, 0) * 1.0 + expected_matrix(12, 3) * 2.0 + expected_matrix(12, 6) * 3.0 + expected_matrix(12, 9) * 1.5 + expected_matrix(12, 12) * 2.5 + expected_matrix(12, 15) * 2.0);
	expected_rhs[13] = -(expected_matrix(13, 1) * 2.0 + expected_matrix(13, 4) * 3.0 + expected_matrix(13, 7) * 4.0 + expected_matrix(13, 10) * 2.5 + expected_matrix(13, 13) * 3.5 + expected_matrix(13, 16) * 3.0);
	expected_rhs[14] = -(expected_matrix(14, 2) * 3.0 + expected_matrix(14, 5) * 4.0 + expected_matrix(14, 8) * 5.0 + expected_matrix(14, 11) * 3.5 + expected_matrix(14, 14) * 4.5 + expected_matrix(14, 17) * 4.0);

	expected_rhs[15] = -(expected_matrix(15, 0) * 1.0 + expected_matrix(15, 3) * 2.0 + expected_matrix(15, 6) * 3.0 + expected_matrix(15, 9) * 1.5 + expected_matrix(15, 12) * 2.5 + expected_matrix(15, 15) * 2.0);
	expected_rhs[16] = -(expected_matrix(16, 1) * 2.0 + expected_matrix(16, 4) * 3.0 + expected_matrix(16, 7) * 4.0 + expected_matrix(16, 10) * 2.5 + expected_matrix(16, 13) * 3.5 + expected_matrix(16, 16) * 3.0);
	expected_rhs[17] = -(expected_matrix(17, 2) * 3.0 + expected_matrix(17, 5) * 4.0 + expected_matrix(17, 8) * 5.0 + expected_matrix(17, 11) * 3.5 + expected_matrix(17, 14) * 4.5 + expected_matrix(17, 17) * 4.0);
...
KRATOS_TEST_CASE_IN_SUITE(CalculateLocalSystemUPwNormalLysmerAbsorbingCondition3D8N, KratosGeoMechanicsFastSuite)
{
   ...
   // write_matrix_to_csv(rLeftHandSideMatrix, "C:/software_development/Kratos2/applications/GeoMechanicsApplication/left_hand_side_matrix.csv"); 
    ...
    Vector expected_rhs = ZeroVector(condition_size);
	expected_rhs[0] = -(expected_matrix(0, 0) * 1.0 + expected_matrix(0, 3) * 2.0 + expected_matrix(0, 6) * 3.0 + expected_matrix(0, 9) * 4.0 + expected_matrix(0, 12) * 1.5 + expected_matrix(0, 15) * 2.5 + expected_matrix(0, 18) * 3.5 + expected_matrix(0, 21) * 2.5);
	expected_rhs[1] = -(expected_matrix(1, 1) * 2.0 + expected_matrix(1, 4) * 3.0 + expected_matrix(1, 7) * 4.0 + expected_matrix(1, 10) * 5.0 + expected_matrix(1, 13) * 2.5 + expected_matrix(1, 16) * 3.5 + expected_matrix(1, 19) * 4.5 + expected_matrix(1, 22) * 3.5);
	expected_rhs[2] = -(expected_matrix(2, 2) * 3.0 + expected_matrix(2, 5) * 4.0 + expected_matrix(2, 8) * 5.0 + expected_matrix(2, 11) * 6.0 + expected_matrix(2, 14) * 3.5 + expected_matrix(2, 17) * 4.5 + expected_matrix(2, 20) * 5.5 + expected_matrix(2, 23) * 4.5);   
    expected_rhs[3] = -(expected_matrix(3, 0) * 1.0 + expected_matrix(3, 3) * 2.0 + expected_matrix(3, 6) * 3.0 + expected_matrix(3, 9) * 4.0 + expected_matrix(3, 12) * 1.5 + expected_matrix(3, 15) * 2.5 + expected_matrix(3, 18) * 3.5 + expected_matrix(3, 21) * 2.5);
	expected_rhs[4] = -(expected_matrix(4, 1) * 2.0 + expected_matrix(4, 4) * 3.0 + expected_matrix(4, 7) * 4.0 + expected_matrix(4, 10) * 5.0 + expected_matrix(4, 13) * 2.5 + expected_matrix(4, 16) * 3.5 + expected_matrix(4, 19) * 4.5 + expected_matrix(4, 22) * 3.5);
	expected_rhs[5] = -(expected_matrix(5, 2) * 3.0 + expected_matrix(5, 5) * 4.0 + expected_matrix(5, 8) * 5.0 + expected_matrix(5, 11) * 6.0 + expected_matrix(5, 14) * 3.5 + expected_matrix(5, 17) * 4.5 + expected_matrix(5, 20) * 5.5 + expected_matrix(5, 23) * 4.5);
	expected_rhs[6] = -(expected_matrix(6, 0) * 1.0 + expected_matrix(6, 3) * 2.0 + expected_matrix(6, 6) * 3.0 + expected_matrix(6, 9) * 4.0 + expected_matrix(6, 12) * 1.5 + expected_matrix(6, 15) * 2.5 + expected_matrix(6, 18) * 3.5 + expected_matrix(6, 21) * 2.5);
	expected_rhs[7] = -(expected_matrix(7, 1) * 2.0 + expected_matrix(7, 4) * 3.0 + expected_matrix(7, 7) * 4.0 + expected_matrix(7, 10) * 5.0 + expected_matrix(7, 13) * 2.5 + expected_matrix(7, 16) * 3.5 + expected_matrix(7, 19) * 4.5 + expected_matrix(7, 22) * 3.5);
	expected_rhs[8] = -(expected_matrix(8, 2) * 3.0 + expected_matrix(8, 5) * 4.0 + expected_matrix(8, 8) * 5.0 + expected_matrix(8, 11) * 6.0 + expected_matrix(8, 14) * 3.5 + expected_matrix(8, 17) * 4.5 + expected_matrix(8, 20) * 5.5 + expected_matrix(8, 23) * 4.5);
	expected_rhs[9] = -(expected_matrix(9, 0) * 1.0 + expected_matrix(9, 3) * 2.0 + expected_matrix(9, 6) * 3.0 + expected_matrix(9, 9) * 4.0 + expected_matrix(9, 12) * 1.5 + expected_matrix(9, 15) * 2.5 + expected_matrix(9, 18) * 3.5 + expected_matrix(9, 21) * 2.5);
	expected_rhs[10] = -(expected_matrix(10, 1) * 2.0 + expected_matrix(10, 4) * 3.0 + expected_matrix(10, 7) * 4.0 + expected_matrix(10, 10) * 5.0 + expected_matrix(10, 13) * 2.5 + expected_matrix(10, 16) * 3.5 + expected_matrix(10, 19) * 4.5 + expected_matrix(10, 22) * 3.5);
	expected_rhs[11] = -(expected_matrix(11, 2) * 3.0 + expected_matrix(11, 5) * 4.0 + expected_matrix(11, 8) * 5.0 + expected_matrix(11, 11) * 6.0 + expected_matrix(11, 14) * 3.5 + expected_matrix(11, 17) * 4.5 + expected_matrix(11, 20) * 5.5 + expected_matrix(11, 23) * 4.5);
	expected_rhs[12] = -(expected_matrix(12, 0) * 1.0 + expected_matrix(12, 3) * 2.0 + expected_matrix(12, 6) * 3.0 + expected_matrix(12, 9) * 4.0 + expected_matrix(12, 12) * 1.5 + expected_matrix(12, 15) * 2.5 + expected_matrix(12, 18) * 3.5 + expected_matrix(12, 21) * 2.5);
	expected_rhs[13] = -(expected_matrix(13, 1) * 2.0 + expected_matrix(13, 4) * 3.0 + expected_matrix(13, 7) * 4.0 + expected_matrix(13, 10) * 5.0 + expected_matrix(13, 13) * 2.5 + expected_matrix(13, 16) * 3.5 + expected_matrix(13, 19) * 4.5 + expected_matrix(13, 22) * 3.5);
	expected_rhs[14] = -(expected_matrix(14, 2) * 3.0 + expected_matrix(14, 5) * 4.0 + expected_matrix(14, 8) * 5.0 + expected_matrix(14, 11) * 6.0 + expected_matrix(14, 14) * 3.5 + expected_matrix(14, 17) * 4.5 + expected_matrix(14, 20) * 5.5 + expected_matrix(14, 23) * 4.5);
	expected_rhs[15] = -(expected_matrix(15, 0) * 1.0 + expected_matrix(15, 3) * 2.0 + expected_matrix(15, 6) * 3.0 + expected_matrix(15, 9) * 4.0 + expected_matrix(15, 12) * 1.5 + expected_matrix(15, 15) * 2.5 + expected_matrix(15, 18) * 3.5 + expected_matrix(15, 21) * 2.5);
	expected_rhs[16] = -(expected_matrix(16, 1) * 2.0 + expected_matrix(16, 4) * 3.0 + expected_matrix(16, 7) * 4.0 + expected_matrix(16, 10) * 5.0 + expected_matrix(16, 13) * 2.5 + expected_matrix(16, 16) * 3.5 + expected_matrix(16, 19) * 4.5 + expected_matrix(16, 22) * 3.5);
	expected_rhs[17] = -(expected_matrix(17, 2) * 3.0 + expected_matrix(17, 5) * 4.0 + expected_matrix(17, 8) * 5.0 + expected_matrix(17, 11) * 6.0 + expected_matrix(17, 14) * 3.5 + expected_matrix(17, 17) * 4.5 + expected_matrix(17, 20) * 5.5 + expected_matrix(17, 23) * 4.5);
	expected_rhs[18] = -(expected_matrix(18, 0) * 1.0 + expected_matrix(18, 3) * 2.0 + expected_matrix(18, 6) * 3.0 + expected_matrix(18, 9) * 4.0 + expected_matrix(18, 12) * 1.5 + expected_matrix(18, 15) * 2.5 + expected_matrix(18, 18) * 3.5 + expected_matrix(18, 21) * 2.5);
	expected_rhs[19] = -(expected_matrix(19, 1) * 2.0 + expected_matrix(19, 4) * 3.0 + expected_matrix(19, 7) * 4.0 + expected_matrix(19, 10) * 5.0 + expected_matrix(19, 13) * 2.5 + expected_matrix(19, 16) * 3.5 + expected_matrix(19, 19) * 4.5 + expected_matrix(19, 22) * 3.5);
	expected_rhs[20] = -(expected_matrix(20, 2) * 3.0 + expected_matrix(20, 5) * 4.0 + expected_matrix(20, 8) * 5.0 + expected_matrix(20, 11) * 6.0 + expected_matrix(20, 14) * 3.5 + expected_matrix(20, 17) * 4.5 + expected_matrix(20, 20) * 5.5 + expected_matrix(20, 23) * 4.5);
	expected_rhs[21] = -(expected_matrix(21, 0) * 1.0 + expected_matrix(21, 3) * 2.0 + expected_matrix(21, 6) * 3.0 + expected_matrix(21, 9) * 4.0 + expected_matrix(21, 12) * 1.5 + expected_matrix(21, 15) * 2.5 + expected_matrix(21, 18) * 3.5 + expected_matrix(21, 21) * 2.5);
	expected_rhs[22] = -(expected_matrix(22, 1) * 2.0 + expected_matrix(22, 4) * 3.0 + expected_matrix(22, 7) * 4.0 + expected_matrix(22, 10) * 5.0 + expected_matrix(22, 13) * 2.5 + expected_matrix(22, 16) * 3.5 + expected_matrix(22, 19) * 4.5 + expected_matrix(22, 22) * 3.5);
	expected_rhs[23] = -(expected_matrix(23, 2) * 3.0 + expected_matrix(23, 5) * 4.0 + expected_matrix(23, 8) * 5.0 + expected_matrix(23, 11) * 6.0 + expected_matrix(23, 14) * 3.5 + expected_matrix(23, 17) * 4.5 + expected_matrix(23, 20) * 5.5 + expected_matrix(23, 23) * 4.5);
