...
KRATOS_TEST_CASE_IN_SUITE(GetProcessList_ReturnsExpectedProcesses_ForAllListTypes, WorkInProgress)
{
    ...
    // When
    ProcessInfoParser parser{process_list};
    const auto actual_result = parser.GetProcessList();
...
KRATOS_TEST_CASE_IN_SUITE(GetProcessList_GivesDuplicates_ForProcessesWithIdenticalNames, WorkInProgress)
{
    ...
    // When
    ProcessInfoParser parser{process_list_with_duplicate_names};
    const auto actual_result = parser.GetProcessList();
...
KRATOS_TEST_CASE_IN_SUITE(GetProcessList_Throws_WhenProcessDoesNotHaveParameters, WorkInProgress)
{
    ...
    // When
    ProcessInfoParser parser{process_without_parameters};
    KRATOS_EXPECT_EXCEPTION_IS_THROWN(parser.GetProcessList(), "Getting a value that does not exist. entry string : Parameters")
}

KRATOS_TEST_CASE_IN_SUITE(GetProcessList_ReturnsEmptyString_WhenNoProcessesAreDefined, WorkInProgress)
{
    ProcessInfoParser parser{Parameters{}};
    KRATOS_EXPECT_EQ(parser.GetProcessList().size(), 0);
}
