...
TEST_F(KratosGeoMechanicsFastSuiteWithoutKernel, TestCoulombYieldSurface)
{
    ...
    EXPECT_NEAR(coulomb_yield_surface.YieldFunctionValue(sigma_tau), 1.0, Defaults::absolute_tolerance);
    ...
    EXPECT_NEAR(coulomb_yield_surface.YieldFunctionValue(sigma_tau), 0.0, Defaults::absolute_tolerance);
...
TEST_F(KratosGeoMechanicsFastSuiteWithoutKernel, CoulombYieldSurface_CanBeSavedAndLoadedThroughInterface)
{
    // Arrange
    ...
    EXPECT_NEAR(p_loaded_coulomb_yield_surface->YieldFunctionValue(sigma_tau),
...
TEST_F(KratosGeoMechanicsFastSuiteWithoutKernel, TestTensionCutoff)
{
    ...
    EXPECT_NEAR(tensionCutoff.YieldFunctionValue(sigma_tau), 1.0, Defaults::absolute_tolerance);

    principal_stress <<= 2.0, 1.5, 1.0;
    sigma_tau = StressStrainUtilities::TransformPrincipalStressesToSigmaTau(principal_stress);
    EXPECT_NEAR(tensionCutoff.YieldFunctionValue(sigma_tau), 0.0, Defaults::absolute_tolerance);
...
TEST_F(KratosGeoMechanicsFastSuiteWithoutKernel, TensionCutOff_CanBeSavedAndLoadedThroughInterface)
{
    // Arrange
    const auto scoped_registration =
    ...
    const auto sigma_tau = StressStrainUtilities::TransformPrincipalStressesToSigmaTau(principal_stresses);
    EXPECT_NEAR(p_loaded_tension_cut_off->YieldFunctionValue(sigma_tau), 0.0, Defaults::absolute_tolerance);
    auto expected_derivative = Vector(2);
