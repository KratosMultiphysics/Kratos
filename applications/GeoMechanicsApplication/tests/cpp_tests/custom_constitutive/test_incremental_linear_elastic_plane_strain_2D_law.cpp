                          ...
                          KratosGeoMechanicsFastSuiteWithoutKernel)
{
    ...
    KRATOS_EXPECT_EXCEPTION_IS_THROWN(
        ...
        "YOUNG_MODULUS does not exist in the parameters of material 3.")
    properties.SetValue(YOUNG_MODULUS, -1.0e7);
    KRATOS_EXPECT_EXCEPTION_IS_THROWN(law.Check(properties, element_geometry, process_info),
                                      "YOUNG_MODULUS in the parameters of material 3 has an "
                                      "invalid value: -1e+07 out of the range (0; -).")
    properties.SetValue(YOUNG_MODULUS, 1.0e7);
    KRATOS_EXPECT_EXCEPTION_IS_THROWN(
        law.Check(properties, element_geometry, process_info),
        "POISSON_RATIO does not exist in the parameters of material 3.")
    properties.SetValue(POISSON_RATIO, 0.7);
    KRATOS_EXPECT_EXCEPTION_IS_THROWN(law.Check(properties, element_geometry, process_info),
                                      "POISSON_RATIO in the parameters of material 3 has an "
                                      "invalid value: 0.7 out of the range [-1; 0.500000).")
