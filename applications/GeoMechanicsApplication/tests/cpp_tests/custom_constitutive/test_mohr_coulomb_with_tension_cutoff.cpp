...
KRATOS_TEST_CASE_IN_SUITE(MohrCoulombWithTensionCutOff_Check, KratosGeoMechanicsFastSuiteWithoutKernel)
{
    ...
    KRATOS_EXPECT_EXCEPTION_IS_THROWN(
        [[maybe_unused]] const auto unused = law.Check(properties, element_geometry, process_info),
        "GEO_FRICTION_ANGLE in the property 3 has an invalid value: -30 out of the range [0; -].")
    properties.SetValue(GEO_FRICTION_ANGLE, 30.0);
    ...
    KRATOS_EXPECT_EXCEPTION_IS_THROWN(
        [[maybe_unused]] const auto unused = law.Check(properties, element_geometry, process_info),
        "YOUNG_MODULUS does not exist in the property 3.")
    properties.SetValue(YOUNG_MODULUS, -1.0);
    KRATOS_EXPECT_EXCEPTION_IS_THROWN(
        [[maybe_unused]] const auto unused = law.Check(properties, element_geometry, process_info),
        "YOUNG_MODULUS in the property 3 has an invalid value: -1 out of the range [0; -].")
    properties.SetValue(YOUNG_MODULUS, 1.0);

    KRATOS_EXPECT_EXCEPTION_IS_THROWN(
        [[maybe_unused]] const auto unused = law.Check(properties, element_geometry, process_info),
        "POISSON_RATIO does not exist in the property 3.")
    properties.SetValue(POISSON_RATIO, -0.5);
    KRATOS_EXPECT_EXCEPTION_IS_THROWN(
        [[maybe_unused]] const auto unused = law.Check(properties, element_geometry, process_info), "POISSON_RATIO in the property 3 has an invalid value: -0.5 out of the range [0; 0.500000].")
    properties.SetValue(POISSON_RATIO, 1.0);
    KRATOS_EXPECT_EXCEPTION_IS_THROWN(
        [[maybe_unused]] const auto unused = law.Check(properties, element_geometry, process_info),
        " POISSON_RATIO in the property 3 has an invalid value: 1 out of the range [0; 0.500000].")
