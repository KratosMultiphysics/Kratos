...
KRATOS_TEST_CASE_IN_SUITE(AccumulateWaterPressureEntriesPicksOnlyWaterPressureDofs, KratosGeoMechanicsFastSuiteWithoutKernel)
{
    ...
    auto dofs = std::vector{r_model_part.pGetNode(1)->pGetDof(DISPLACEMENT_X),
                          ...
                          KratosGeoMechanicsFastSuiteWithoutKernel)
{
    ...
    auto dofs = std::vector{r_model_part.pGetNode(1)->pGetDof(WATER_PRESSURE),
                            r_model_part.pGetNode(2)->pGetDof(WATER_PRESSURE)};

    const auto first_contribution  = UblasUtilities::CreateVector({1.5, 2.5});
    const auto second_contribution = UblasUtilities::CreateVector({0.5, -2.5});
