...
KRATOS_TEST_CASE_IN_SUITE(ElementUtilities_ReturnsCorrectListOfShapeFunctionsValues, KratosGeoMechanicsFastSuiteWithoutKernel)
{
    ...
    auto geometry = LineInterfaceGeometry<Line2D2<Node>>{1, nodes};
    ...
    const Geo::IntegrationPointVectorType integration_points{
        IntegrationPoint<3>(-1.0, 0.0, 0.0, 1.0), IntegrationPoint<3>(1.0, 0.0, 0.0, 1.0)};

    auto shape_function_values = GeoElementUtilities::CalculateShapeFunctionValues(integration_points, geometry);
