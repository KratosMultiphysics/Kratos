...
TEST_F(KratosGeoMechanicsFastSuiteWithoutKernel, CalculatePermeabilityMatrix2D3NGivesCorrectResults)
{
    ...
    const auto MaterialPermeabilityMatrix = UblasUtilities::CreateMatrix({{1.0, 2.0}, {3.0, 4.0}});
    // clang-format on
...
TEST_F(KratosGeoMechanicsFastSuiteWithoutKernel, CalculatePermeabilityMatrix3D4NGivesCorrectResults)
{
    const auto GradNpT =
    ...
    const double                DynamicViscosityInverse = 1;
    const auto                  MaterialPermeabilityMatrix =
    ...
    const auto PMatrix = UblasUtilities::CreateMatrix({{-45.6, -88.8, -132.0, -175.2},
