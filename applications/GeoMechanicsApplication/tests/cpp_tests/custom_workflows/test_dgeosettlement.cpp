...
{
    const auto input_utility_from_settlement =
        dynamic_cast<const StubInputUtility*>(rSettlement.GetInterfaceInputUtility());
...
KRATOS_TEST_CASE_IN_SUITE(RunStageMakesRelevantCallsOnce, KratosGeoMechanicsFastSuite)
{
    ...
    // At the moment the parameter_json_settings do not contain enough info to instantiate the
    // solving strategy. That is why the exit_status is now 1, as with other tests in this suite.
...
KRATOS_TEST_CASE_IN_SUITE(RunStageDoesNotPerformMaterialCallWhenNotSpecified, KratosGeoMechanicsFastSuite)
{
    ...
    KratosGeoSettlement settlement(
        std::make_unique<StubInputUtility>(parameter_json_string_without_material_import_settings),
        std::make_unique<StubProcessInfoParser>(), std::make_unique<StubTimeLoopExecutor>());
...
KRATOS_TEST_CASE_IN_SUITE(RunStage_PassesTheCorrectProcessReferences, KratosGeoMechanicsFastSuite)
{
    ...
    KratosGeoSettlement settlement(
        std::make_unique<StubInputUtility>(process_parameters_in_json_settings),
        std::make_unique<StubProcessInfoParser>(),
