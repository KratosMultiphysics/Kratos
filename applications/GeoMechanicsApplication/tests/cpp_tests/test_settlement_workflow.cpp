...
[[maybe_unused]] void TestSettlement()
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        const int  status       = settlement->RunStage(working_directory, project_file,
        ...
        KRATOS_EXPECT_TRUE(TestUtilities::CompareFiles(working_directory / original_file,
                                                       working_directory / result_file))
    }
}
