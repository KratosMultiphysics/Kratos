...
{
    auto workingDirectory = std::filesystem::path(std::filesystem::current_path()) / ".." 
                          / "applications"
    ...
    const int status = execute.ExecuteFlowAnalysis(workingDirectory.u8string(), projectFile, critical_head_info, "", call_back_functions);
