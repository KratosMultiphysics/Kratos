...
class KratosGeoMechanicsPiecewiseLinearMomentCapacityTests(KratosUnittest.TestCase):
    ...
    def _run_piecewise_case(self, project_subpath, times, expected_moments, expected_displacements, unreload_modulus=None, tol=1e-4):
        ...
        for t, exp_m, exp_d in zip(times, expected_moments, expected_displacements):
            ...
            self.assertLessEqual(abs(moment - exp_m), abs_tol_m, 
                                     f"Moment mismatch at t={t}: {moment} != {exp_m}")
            self.assertLessEqual(abs(disp_y - exp_d), abs_tol_d,
    ...
    def test_piecewise_linear_moment_capacity(self):
        ...
        self._run_piecewise_case('.', times, expected_moments_y, expected_displacements_y, tol=1e-4)
    ...
    def test_piecewise_linear_moment_capacity_with_unreload(self):
        ...
        expected_displacements_y = [0.006, 0.004, 0.0075, -0.005, -0.0125, -0.0075, -0.02, -0.0575]
        self._run_piecewise_case('.', times, expected_moments_y, expected_displacements_y, unreload_modulus=20.0, tol=1e-4)
