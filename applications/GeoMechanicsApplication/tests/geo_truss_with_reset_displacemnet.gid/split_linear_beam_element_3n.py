...
def write_mdpa_file(filename, lines, new_beam_elements, sub_model_parts, altered_sub_model_parts):
    ...
    i = 0
    while i < len(lines):
        if "GeoCrBeamElement2D3N" in lines[i]:
...
def main(filename):
    ...
    beam_elements = [element for element in elements if element["type"] == "GeoCrBeamElement2D3N"]
