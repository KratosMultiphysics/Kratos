...
{
    // Note that the file extension of the model part file must be excluded, since that is
    // automatically appended by the constructor of class ModelPartIO
    ModelPartIO reader{rModelPartFilePath.generic_string()};
    reader.ReadModelPart(rModelPart);
    KRATOS_INFO("FileInputUtility::ReadModelFromFile")
        << "Read the mesh data from " << rModelPartFilePath << std::endl;
}

void FileInputUtility::AddMaterialsFromFile(const std::filesystem::path& rMaterialFilePath, Model& rModel) const
{
    const std::string parameters =
        R"({ "Parameters" : { "materials_filename" :")" + rMaterialFilePath.generic_string() + R"("}})";
