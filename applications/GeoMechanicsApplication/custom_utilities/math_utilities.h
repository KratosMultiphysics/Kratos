...
public:
    ...
    static inline bool CardanoFormula(double a, double b, double c, double d, Vector& solution)
    {
        ...
        solution(0) = -sqrt(-4.0 / 3.0 * p) *
                      ...
                      b / (3 * a);
        solution(1) = sqrt(-4.0 / 3.0 * p) * cos(1.0 / 3.0 * acos(-q / 2.0 * sqrt(-27.0 / (p * p * p)))) -
                      b / (3 * a);
    ...
    static inline Vector EigenValues(const Matrix& A, double tolerance, double zero)
    {
        ...
        while (is_converged == false && iter < max_iters) {
            ...
            if (delta < zero || (delta / abs) < tolerance) is_converged = true;
        ...
        for (int i = 0; i < dim; i++) {
            ...
            if (fabs(Result(i)) < zero) Result(i) = 0.0;
        }
    ...
    static inline void QRFactorization(const MatrixType& A, MatrixType& Q, MatrixType& R)
    {
        ...
        for (int iteration = 0; iteration < dim - 1; iteration++) {
            ...
            if (y[iteration] != 0) k = -y(iteration) / fabs(y(iteration)) * normy;
            else k = -normy;
                                    ...
                                    int               max_iterations = 10)
    {
        ...
        for (int iterations = 0; iterations < max_iterations; iterations++) {
            ...
            if (fabs(gamma) > zero_tolerance && fabs(gamma) < (1 / zero_tolerance)) {
                ...
                if (fabs(gamma) >= (1.0 / zero_tolerance)) u = 0.5 / gamma;
            }
    ...
    /**
     * Performs clipping on the two polygons clipping_points and subjected_points (the technique
     * used is Sutherland-Hodgman clipping) and returns the overlapping polygon result_points. The
     * method works in 3D. Both polygons have to be convex, but they can be slightly perturbated in
     * 3D space, this allows for performing clipping on two interpolated interfaces
     ...
     */
    static bool Clipping(std::vector<Point*>& clipping_points,
                         ...
                         double               tolerance)
    {
        ...
        for (unsigned int clipp_edge = 0; clipp_edge < clipping_points.size(); clipp_edge++) {
            ...
            if (clipp_edge < (clipping_points.size() - 1)) index_clipp_2 = clipp_edge + 1;
            ...
            if (clipp_edge < (clipping_points.size() - 2))
                actual_normal = *(clipping_points[clipp_edge + 2]) -
                                (*(clipping_points[clipp_edge]) +
                                 actual_edge * inner_prod((*(clipping_points[clipp_edge + 2]) -
                                                           *(clipping_points[clipp_edge])),
                                                          actual_edge));
                ...
                actual_normal =
                    *(clipping_points[0]) -
                    (*(clipping_points[clipp_edge]) +
                     actual_edge * inner_prod((*(clipping_points[0]) - *(clipping_points[clipp_edge])), actual_edge));
            else
                actual_normal =
                    *(clipping_points[1]) -
                    (*(clipping_points[clipp_edge]) +
                     actual_edge * inner_prod((*(clipping_points[1]) - *(clipping_points[clipp_edge])), actual_edge));
            ...
            for (unsigned int subj_edge = 0; subj_edge < result_points.size(); subj_edge++) {
                ...
                if (subj_edge < (result_points.size() - 1)) index_subj_2 = subj_edge + 1;

                // Test whether the points of the actual subj_edge lay on clipp_edge
                if (fabs(inner_prod((*(result_points[subj_edge]) - (*(clipping_points[clipp_edge]))),
                                    actual_normal)) <= tolerance) {
                    temp_results.push_back(result_points[subj_edge]);
                    if (inner_prod((*(result_points[index_subj_2]) - *(clipping_points[clipp_edge])),
                                   actual_normal) > tolerance)
                        is_visible = true;
                ...
                Vector b(2);
                b(0) = -inner_prod((*(result_points[index_subj_2]) - *(result_points[subj_edge])),
                                   (*(result_points[subj_edge]) - *(clipping_points[clipp_edge])));
                b(1) = inner_prod((*(clipping_points[index_clipp_2]) - *(clipping_points[clipp_edge])),
                                  (*(result_points[subj_edge]) - *(clipping_points[clipp_edge])));
                Matrix A(2, 2);
                A(0, 0) = inner_prod((*(result_points[index_subj_2]) - *(result_points[subj_edge])),
                                     (*(result_points[index_subj_2]) - *(result_points[subj_edge])));
                A(0, 1) = -inner_prod((*(result_points[index_subj_2]) - *(result_points[subj_edge])),
                                      (*(clipping_points[index_clipp_2]) - *(clipping_points[clipp_edge])));
                A(1, 0) = A(0, 1);
                A(1, 1) = inner_prod(*(clipping_points[index_clipp_2]) - *(clipping_points[clipp_edge]),
                                     *(clipping_points[index_clipp_2]) - *(clipping_points[clipp_edge]));
                Vector coeff(2);
                coeff(0) = 1.0 / A(0, 0) *
                           (b(0) - A(0, 1) / (A(1, 1) - A(0, 1) * A(1, 0) / A(0, 0)) *
                                       (b(1) - b(0) * A(1, 0) / A(0, 0)));
                ...
                Vector dist_vec(3);
                noalias(dist_vec) =
                    *(result_points[subj_edge]) +
                    coeff(0) * (*(result_points[index_subj_2]) - *(result_points[subj_edge])) -
                    (*(clipping_points[clipp_edge]) +
                     coeff(1) * (*(clipping_points[index_clipp_2]) - *(clipping_points[clipp_edge])));

                if (coeff(0) > tolerance && coeff(0) < (1 - tolerance) &&
                    (sqrt(inner_prod(dist_vec, dist_vec)) < tolerance)) {
                    if (is_visible) temp_results.push_back(result_points[subj_edge]);

                    temp_results.push_back(new Point(
                        (*(clipping_points[clipp_edge]) +
                         coeff(1) * (*(clipping_points[index_clipp_2]) - *(clipping_points[clipp_edge])))));
