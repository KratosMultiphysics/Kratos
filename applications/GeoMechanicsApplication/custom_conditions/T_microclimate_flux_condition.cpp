...
{
    return make_intrusive<GeoTMicroClimateFluxCondition>(NewId, this->GetGeometry().Create(rNodes), pProperties);
}
...
template <unsigned int TDim, unsigned int TNumNodes>
void GeoTMicroClimateFluxCondition<TDim, TNumNodes>::CalculateLocalSystem(Matrix& rLeftHandSideMatrix,
                                                                          ...
                                                                          const ProcessInfo& rCurrentProcessInfo)
{
    ...
    mWaterStorage = CalculateCurrentWaterStorage(time_step_size, previous_storage, previous_radiation);
...
template <unsigned int TDim, unsigned int TNumNodes>
void GeoTMicroClimateFluxCondition<TDim, TNumNodes>::CalculateAndAddLHS(Matrix& rLeftHandSideMatrix,
...
{
    auto temporary_matrix =
        BoundedMatrix<double, TNumNodes, TNumNodes>{outer_prod(rN, rN) * IntegrationCoefficient};
    ...
    double TimeStepSize, double PreviousStorage, double PreviousRadiation) const
{
    ...
    for (unsigned int i = 0; i < TNumNodes; ++i) {
        ...
        const auto surface_heat_storage =
            CalculateSurfaceHeatStorage(TimeStepSize, PreviousRadiation, net_radiation);
        const WaterFluxes water_fluxes =
            CalculateWaterFluxes(i, TimeStepSize, PreviousStorage, net_radiation, surface_heat_storage);

        result[i] = CalculateRightHandSideFlux(net_radiation, surface_heat_storage, water_fluxes.evaporation);
    }
...
template <unsigned int TDim, unsigned int TNumNodes>
double GeoTMicroClimateFluxCondition<TDim, TNumNodes>::CalculateRightHandSideFlux(double NetRadiation,
                                                                                  ...
                                                                                  double ActualEvaporation) const
{
    ...
    // Eq 5.31
    const auto subsurface_heat_flux = NetRadiation - sensible_heat_flux_right - latent_heat_flux +
                                      mBuildEnvironmentRadiation - SurfaceHeatStorage;
    return subsurface_heat_flux;
}

template <unsigned int TDim, unsigned int TNumNodes>
double GeoTMicroClimateFluxCondition<TDim, TNumNodes>::CalculateCurrentWaterStorage(double TimeStepSize,
                                                                                    ...
                                                                                    double PreviousRadiation) const
{
    ...
    for (unsigned int i = 0; i < TNumNodes; ++i) {
        ...
        const auto actual_storage =
            PreviousStorage + TimeStepSize * (water_fluxes.precipitation - water_fluxes.evaporation);
...
double GeoTMicroClimateFluxCondition<TDim, TNumNodes>::CalculateNetRadiation(unsigned int NodeIndex) const
{
    ...
    // Eq 5.16
    const auto incoming_radiation   = r_geom[NodeIndex].FastGetSolutionStepValue(SOLAR_RADIATION);
    const auto short_wave_radiation = (1.0 - mAlbedoCoefficient) * incoming_radiation;

    // Eq 5.17
    const auto atmospheric_temperature = r_geom[NodeIndex].FastGetSolutionStepValue(AIR_TEMPERATURE);
    ...
    // Eq 5.18
    const auto initial_soil_temperature = r_geom[NodeIndex].FastGetSolutionStepValue(TEMPERATURE, 1);
...
template <unsigned int TDim, unsigned int TNumNodes>
double GeoTMicroClimateFluxCondition<TDim, TNumNodes>::CalculateSurfaceHeatStorage(double TimeStepSize,
    ...
    unsigned int NodeIndex, double TimeStepSize, double PreviousStorage, double NetRadiation, double SurfaceHeatStorage) const
{
    ...
    const auto potential_storage = PreviousStorage + TimeStepSize * (precipitation - potential_evaporation);
    ...
    if (potential_storage > mMaximalStorage) {
        actual_precipitation = (mMaximalStorage - PreviousStorage) / TimeStepSize + actual_evaporation;
    ...
    unsigned int NodeIndex, double NetRadiation, double SurfaceHeatStorage) const
{
    ...
    const auto vapor_pressure_increment =
        4098.0 * saturated_vapor_pressure / (std::pow((atmospheric_temperature + 237.3), 2.0));
    ...
    auto latent_heat_flux =
        (vapor_pressure_increment * (NetRadiation + mBuildEnvironmentRadiation - SurfaceHeatStorage) +
         AirHeatCapacity * AirDensity * (saturated_vapor_pressure - actual_vapor_pressure) / atmospheric_resistance) /
        (vapor_pressure_increment + PsychometricConstant * (1.0 + SurfaceResistance / atmospheric_resistance));
...
void GeoTMicroClimateFluxCondition<TDim, TNumNodes>::CalculateRoughness(const ProcessInfo& rCurrentProcessInfo)
{
    ...
    const auto current_wind_speed = std::max(lower_bound, r_geom[0].FastGetSolutionStepValue(WIND_SPEED));
    ...
    for (unsigned int i = 0; i < TNumNodes; ++i) {
        ...
        const auto surface_roughness_factor =
            CalculateSurfaceRoughnessFactor(current_air_temperature, previous_roughness_temperature,
                                            richardson_bulk_modulus, friction_drag_coefficient);

        // Eq 5.30
        const auto denominator = RoughnessLayerResistance * RoughnessLayerHeight + time_step_size +
        ...
        const auto current_roughness_temperature =
            (RoughnessLayerResistance * RoughnessLayerHeight * previous_roughness_temperature +
             ...
             time_step_size * current_wind_speed * RoughnessLayerResistance * surface_roughness_factor *
                 friction_drag_coefficient * friction_drag_coefficient * current_air_temperature) /
            denominator;
...
double GeoTMicroClimateFluxCondition<TDim, TNumNodes>::CalculateSurfaceRoughnessFactor(
    double CurrentAirTemperature, double PreviousRoughnessTemperature, double RichardsonBulkModulus, double FrictionDragCoefficient) const
{
    if (PreviousRoughnessTemperature >= CurrentAirTemperature) {
        ...
        const auto coefficient =
            ...
            (1.0 + 75.0 * FrictionDragCoefficient * FrictionDragCoefficient *
                       std::sqrt(MeasurementHeight / RoughnessHeight * std::abs(RichardsonBulkModulus)));
