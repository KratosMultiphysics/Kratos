...
{
    return Condition::Pointer(new UPwFaceLoadInterfaceCondition(
                                                                  ...
                                                                  const ProcessInfo& CurrentProcessInfo)
{
    ...
    for (unsigned int GPoint = 0; GPoint < NumGPoints; GPoint++) {
        ...
        ConditionUtilities::InterpolateVariableWithComponents<TDim, TNumNodes>(
            TractionVector, NContainer, FaceLoadVector, GPoint);

        // Compute Nu Matrix
        InterfaceElementUtilities::CalculateNuMatrix(Nu, NContainer, GPoint);

        if (ComputeJointWidth == true)
            this->CalculateJointWidth(JointWidth, Nu, DisplacementVector, RelDispVector,
                                      RotationMatrix, LocalRelDispVector, MinimumJointWidth, GPoint);

        // Compute weighting coefficient for integration
        double integration_coefficient = this->CalculateIntegrationCoefficient(
            JContainer[GPoint], IntegrationPoints[GPoint].Weight(), JointWidth);
...
void UPwFaceLoadInterfaceCondition<2, 2>::CalculateJointWidth(double& rJointWidth,
                                                              ...
                                                              const array_1d<double, 4>& DisplacementVector,
                                                              array_1d<double, 2>& rRelDispVector,
                                                              const BoundedMatrix<double, 2, 2>& RotationMatrix,
                                                              array_1d<double, 2>& rLocalRelDispVector,
                                                              const double&       MinimumJointWidth,
...
void UPwFaceLoadInterfaceCondition<3, 4>::CalculateJointWidth(double& rJointWidth,
                                                              const BoundedMatrix<double, 3, 12>& Nu,
                                                              const array_1d<double, 12>& DisplacementVector,
                                                              array_1d<double, 3>& rRelDispVector,
                                                              const BoundedMatrix<double, 3, 3>& RotationMatrix,
                                                              array_1d<double, 3>& rLocalRelDispVector,
                                                              const double&       MinimumJointWidth,
                                                              const unsigned int& GPoint)
{
    // Quadrilateral_interface_3d_4
