...
CalculateAbsorbingMatrix(MatrixType& rAbsorbingMatrix,
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    //Resizing absorbing matrix
    if (rAbsorbingMatrix.size1() != TDim)
        rAbsorbingMatrix.resize(TDim, TDim, false);
    noalias(rAbsorbingMatrix) = ZeroMatrix(TDim, TDim);
  ...
  /*  this->InitializeElementVariables(Variables,
    ...
    BoundedMatrix<double, TDim + 1, TNumNodes* (TDim + 1)> AuxIdentityMatrix = ZeroMatrix(TDim + 1, TNumNodes * (TDim + 1));
    BoundedMatrix<double, TDim + 1, TDim + 1>             IdentityMatrix = ZeroMatrix(TDim + 1, TDim + 1);
    ...
    for (unsigned int GPoint = 0; GPoint < NumGPoints; ++GPoint) {
        ...
        //this->CalculateDerivativesOnInitialConfiguration(detJContainer,
        //    J0,
        //    InvJ0,
        //    DNu_DX0,
        //    GPoint);
        ...
        GeoElementUtilities::
            AssembleDensityMatrix< TDim >(IdentityMatrix, 1);

        noalias(AuxIdentityMatrix) = prod(IdentityMatrix, Nut);

        //Adding contribution to Mass matrix
        noalias(rAbsorbingMatrix) += prod(trans(Nut), AuxIdentityMatrix)
...
{        
    ...
    NormalLysmerAbsorbingVariables Variables;
    //Variables.NContainer = NContainer;
    
    MatrixType rAbsorbingMatrix;

    this->CalculateAbsorbingMatrix(rAbsorbingMatrix,
        CurrentProcessInfo);
    ...
    for(unsigned int GPoint = 0; GPoint < NumGPoints; ++GPoint) {
        ...
        Variables.localRelVelVector = localRelVelVector;
        // normal velocity????
        //Compute normal flux 
        ...
        NormalLysmerAbsorbingVariables& rVariables )
{
    ...
    double G = E / (2 * (1 + nu));
    double vp = sqrt(Ec / rho);
    double vs = sqrt(G / rho);

    array_1d<double, TDim> localRelVelVector = rVariables.localRelVelVector;
    array_1d<double, TDim> absorbingCoefficientVector;

    array_1d<double, TNumNodes* TDim> VelocityVector;
    const GeometryType& Geom = this->GetGeometry();
    GeoElementUtilities::GetNodalVariableVector<TDim, TNumNodes>(VelocityVector, Geom, VELOCITY);

    //absorbingCoefficientVector[0] = vs * rho * localRelVelVector[0];
    //for (unsigned int i = 1; i < TDim; i++) {
    //    absorbingCoefficientVector[i] = vp * rho *localRelVelVector[i];
    //};

    absorbingCoefficientVector[0] = vs * rho ;
    for (unsigned int i = 1; i < TDim; i++) {
        absorbingCoefficientVector[i] = vp * rho ;
    };


    //prod(trans(Nu), TractionVector)* IntegrationCoefficient;
    noalias(rVariables.UVector) = -prod(trans(rVariables.Nu), absorbingCoefficientVector)* rVariables.IntegrationCoefficient;
    //noalias(rVariables.PVector) = rVariables.NormalFlux * rVariables.Np * rVariables.IntegrationCoefficient;

   /* rVariables.PVector(0) = vs*localRelVelVector[0]* rVariables.Np * rVariables.IntegrationCoefficient;
    for (unsigned int i = 1; i < TDim; i++) {
        rVariables.PVector(i) = vp * localRelVelVector[i] * rVariables.Np * rVariables.IntegrationCoefficient;
    }*/
