...
template <class TSparseSpace,
          ...
          >
class GeoResidualBasedBlockBuilderAndSolver : public BuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>
{
public:
    ///@name Type Definitions
    ///@{
    ...
    KRATOS_CLASS_POINTER_DEFINITION(GeoResidualBasedBlockBuilderAndSolver);

    /// Definition of the base class
    typedef BuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver> BaseType;

    /// The definition of the current class
    typedef GeoResidualBasedBlockBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver> ClassType;
     ...
     */
    explicit GeoResidualBasedBlockBuilderAndSolver() : BaseType() {}

    /**
     * @brief Default constructor. (with parameters)
     */
    explicit GeoResidualBasedBlockBuilderAndSolver(typename TLinearSolver::Pointer pNewLinearSystemSolver,
    ...
    /** Destructor.
     */
    ~GeoResidualBasedBlockBuilderAndSolver() override {}
     ...
     */
    typename BaseType::Pointer Create(typename TLinearSolver::Pointer pNewLinearSystemSolver,
            ...
            for (int k = 0; k < nelements; k++) {
                ...
                if (it_elem->IsActive()) {
                    ...
                    pScheme->CalculateSystemContributions(
                        *it_elem, LHS_Contribution, RHS_Contribution, EquationId, CurrentProcessInfo);
...
#pragma omp for schedule(guided, 512)
            for (int k = 0; k < nconditions; k++) {
                ...
                if (it_cond->IsActive()) {
                    // Calculate elemental contribution
                    pScheme->CalculateSystemContributions(
                        *it_cond, LHS_Contribution, RHS_Contribution, EquationId, CurrentProcessInfo);
        ...
        KRATOS_INFO_IF("ResidualBasedBlockBuilderAndSolver", this->GetEchoLevel() >= 1)
            << "Build time: " << timer << std::endl;

        KRATOS_INFO_IF("ResidualBasedBlockBuilderAndSolver", this->GetEchoLevel() > 2)
...
#pragma omp for schedule(guided, 512)
    ...
    /**
     * @brief Build a rectangular matrix of size n*N where "n" is the number of unrestrained degrees
     * of freedom and "N" is the total number of degrees of freedom involved.
     * @details This matrix is obtained by building the total matrix without the lines corresponding
     * to the fixed degrees of freedom (but keeping the columns!!)
     * @param pScheme The integration scheme considered
     * @param rModelPart The model part of the problem to solve
     * @param A The LHS matrix
     */
    ...
    void InternalSystemSolveWithPhysics(TSystemMatrixType& rA, TSystemVectorType& rDx, TSystemVectorType& rb, ModelPart& rModelPart)
    {
        ...
        if (norm_b != 0.00) {
            ...
            KRATOS_WARNING_IF("ResidualBasedBlockBuilderAndSolver", mOptions.IsNot(SILENT_WARNINGS))
                       ...
                       TSystemVectorType&            b) override
    {
        ...
        Timer::Stop("Solve");
        KRATOS_INFO_IF("ResidualBasedBlockBuilderAndSolver", this->GetEchoLevel() >= 1)
            << "System solve time: " << timer << std::endl;
     ...
     */
    void BuildAndSolveLinearizedOnPreviousIteration(typename TSchemeType::Pointer pScheme,
                                                    ModelPart&                    rModelPart,
                                                    ...
                                                    const bool                    MoveMesh) override
    {
        ...
        KRATOS_ERROR_IF(rModelPart.GetBufferSize() == 1)
            ...
            << "is :" << rModelPart.GetBufferSize() << " Please set IN THE STRATEGY SETTINGS "
        ...
        //     KRATOS_INFO("BuildAndSolveLinearizedOnPreviousIteration") << "sol step val 0 = " << r_dof.GetSolutionStepValue(0) << " | sol step val 1 = " << r_dof.GetSolutionStepValue(1) << std::endl;
        // }

        // block_for_each(BaseType::mDofSet, [&](Dof<double>& rDof) {
        //     // NOTE: this is initialized to - the value of dx prediction
        //     const auto& incremental_displacement_vector = rDof.GetSolutionStepValue(INCREMENTAL_DISPLACEMENT);
        //     // dx_prediction[rDof.EquationId()] = rNode.FastGetSolutionStepValue(INCREMENTAL_DISPLACEMENT);
        //     dx_prediction[rDof.EquationId()] = rDof.GetSolutionStepValue(0) - rDof.GetSolutionStepValue(1);
        // });

        for (const auto& rNode : rModelPart.Nodes()) {
            for (const auto& rDof : rNode.GetDofs()) {
                if (rDof->GetVariable() == DISPLACEMENT_X) {
                    dx_prediction[rDof->EquationId()] =
                        rNode.FastGetSolutionStepValue(INCREMENTAL_DISPLACEMENT_X);
                } else if (rDof->GetVariable() == DISPLACEMENT_Y) {
                    dx_prediction[rDof->EquationId()] =
                        rNode.FastGetSolutionStepValue(INCREMENTAL_DISPLACEMENT_Y);
                } else if (rDof->GetVariable() == DISPLACEMENT_Z) {
                    dx_prediction[rDof->EquationId()] =
                        rNode.FastGetSolutionStepValue(INCREMENTAL_DISPLACEMENT_Z);
                }
            }
        }
        ...
        // KRATOS_INFO("BuildAndSolveLinearizedOnPreviousIteration") << "[1] norm2(dx_prediction) = " << TSparseSpace::TwoNorm(dx_prediction) << "\n";
        // KRATOS_INFO("BuildAndSolveLinearizedOnPreviousIteration") << "[1] norm2(rDx) = " << TSparseSpace::TwoNorm(rDx) << "\n";
        // this->ApplyDirichletConditions(pScheme, rModelPart, rA, rDx, rb);
        // KRATOS_INFO("BuildAndSolveLinearizedOnPreviousIteration") << "norm2(rb) = " << TSparseSpace::TwoNorm(rb) << "\n";
        // KRATOS_INFO("BuildAndSolveLinearizedOnPreviousIteration") << "[2] norm2(rDx) = " << TSparseSpace::TwoNorm(rDx) << "\n";
    ...
    void SetUpDofSet(typename TSchemeType::Pointer pScheme, ModelPart& rModelPart) override
    {
        ...
        BlockBuildDofArrayUtility::SetUpDofArray(rModelPart, BaseType::mDofSet, this->GetEchoLevel(),
                                                 BaseType::GetCalculateReactionsFlag());
                                    ...
                                    ModelPart&                    rModelPart) override
    {
        ...
        if (A.size1() == 0 || BaseType::GetReshapeMatrixFlag() == true) // if the matrix is not initialized
        {
            ...
            if (A.size1() != BaseType::mEquationSystemSize || A.size2() != BaseType::mEquationSystemSize) {
                KRATOS_ERROR << "The equation system size has changed during the simulation. This "
                                "is not permitted."
                             << std::endl;
                                  ...
                                  TSystemVectorType&            rb) override
    {
        ...
        mScaleFactor = TSparseSpace::CheckAndCorrectZeroDiagonalValues(rModelPart.GetProcessInfo(),
                                                                       rA, rb, mScalingDiagonal);
    ...
    void ApplyRHSConstraints(typename TSchemeType::Pointer pScheme, ModelPart& rModelPart, TSystemVectorType& rb) override
    {
        ...
        if (rModelPart.MasterSlaveConstraints().size() != 0) {
            ...
            SparseMatrixMultiplicationUtility::TransposeMatrix<TSystemMatrixType, TSystemMatrixType>(
                T_transpose_matrix, mT, 1.0);
     ...
     * @param rb The RHS vector
     */
    void ApplyConstraints(typename TSchemeType::Pointer pScheme,
                          ModelPart&                    rModelPart,
                          TSystemMatrixType&            rA,
                          TSystemVectorType&            rb) override
    {
        ...
        if (rModelPart.MasterSlaveConstraints().size() != 0) {
            ...
            SparseMatrixMultiplicationUtility::MatrixMultiplication(
                T_transpose_matrix, rA, auxiliar_A_matrix); // auxiliar = T_transpose * rA
     ...
     */
    static std::string Name() { return "block_builder_and_solver"; }
     ...
     */
    typename TSparseSpace::MatrixType& GetConstraintRelationMatrix() override { return mT; }
     ...
     */
    void SetScaleFactor(const double ScaleFactor) { mScaleFactor = ScaleFactor; }
    ...
    std::string Info() const override { return "ResidualBasedBlockBuilderAndSolver"; }

    /// Print information about this object.
    void PrintInfo(std::ostream& rOStream) const override { rOStream << Info(); }

    /// Print object's data.
    void PrintData(std::ostream& rOStream) const override { rOStream << Info(); }
...
protected:
    ...
    SCALING_DIAGONAL mScalingDiagonal =
        SCALING_DIAGONAL::CONSIDER_MAX_DIAGONAL; /// We identify the scaling considered for the dirichlet dofs
...
#pragma omp for schedule(guided, 512) nowait
                for (int i_const = 0;
                     i_const < static_cast<int>(rModelPart.MasterSlaveConstraints().size()); ++i_const) {
                ...
                for (auto& pair_temp_indices : temp_indices) {
                    ...
                    indices[pair_temp_indices.first].insert(pair_temp_indices.second.begin(),
                                                            pair_temp_indices.second.end());
            ...
            // Count the row sizes
            const std::size_t nnz = block_for_each<SumReduction<std::size_t>>(
                indices, [](auto& rIndices) { return rIndices.size(); });
...
#pragma omp for schedule(guided, 512)
            for (int i_const = 0; i_const < number_of_constraints; ++i_const) {
                ...
                if (it_const->IsActive()) {
                    ...
                    auxiliar_inactive_slave_dofs.insert(slave_equation_ids.begin(),
                                                        slave_equation_ids.end());
                }
            }
...
#pragma omp critical
            {
                mInactiveSlaveDofs.insert(auxiliar_inactive_slave_dofs.begin(),
                                          auxiliar_inactive_slave_dofs.end());
            }
        }
    ...
    virtual void ConstructMatrixStructure(typename TSchemeType::Pointer pScheme, TSystemMatrixType& A, ModelPart& rModelPart)
    {
        ...
        block_for_each(indices,
                       [](std::unordered_set<std::size_t>& rIndices) { rIndices.reserve(40); });
        ...
        if (rModelPart.MasterSlaveConstraints().size() != 0) {
            ...
            block_for_each(rModelPart.MasterSlaveConstraints(), tls,
                           [&](MasterSlaveConstraint& rConst, TLS& rTls) {
    ...
    void AssembleLHS(TSystemMatrixType&             rA,
    ...
    inline void AssembleRowContribution(TSystemMatrixType&             A,
                                        const Matrix&                  Alocal,
    ...
    void AssignSettings(const Parameters ThisParameters) override
    {
        ...
        // Setting flags<
        const std::string& r_diagonal_values_for_dirichlet_dofs =
            ThisParameters["diagonal_values_for_dirichlet_dofs"].GetString();

        std::set<std::string> available_options_for_diagonal = {
            "no_scaling", "use_max_diagonal", "use_diagonal_norm", "defined_in_process_info"};

        if (available_options_for_diagonal.find(r_diagonal_values_for_dirichlet_dofs) ==
            available_options_for_diagonal.end()) {
            std::stringstream msg;
            msg << "Currently prescribed diagonal values for dirichlet dofs : " << r_diagonal_values_for_dirichlet_dofs
                << "\n";
            msg << "Admissible values for the diagonal scaling are : no_scaling, use_max_diagonal, "
                   "use_diagonal_norm, or defined_in_process_info"
                << "\n";
        ...
        if (r_diagonal_values_for_dirichlet_dofs == "no_scaling") {
            ...
            mScalingDiagonal = SCALING_DIAGONAL::CONSIDER_MAX_DIAGONAL;
                   ...
                   "use_diagonal_norm") { // On this case the norm of the diagonal will be considered
...
private:
    ...
    inline unsigned int ForwardFind(const unsigned int id_to_find, const unsigned int start, const size_t* index_vector)
    {
    ...
    inline unsigned int BackwardFind(const unsigned int id_to_find, const unsigned int start, const size_t* index_vector)
    {
        unsigned int pos = start;
        while (id_to_find != index_vector[pos])
            pos--;
...
const Kratos::Flags GeoResidualBasedBlockBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>::SILENT_WARNINGS(
