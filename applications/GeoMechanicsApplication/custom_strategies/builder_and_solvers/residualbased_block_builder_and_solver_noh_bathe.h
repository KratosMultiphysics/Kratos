...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(ResidualBasedBlockBuilderAndSolverNohBathe);
    ...
    /** Destructor.
     */
    ~ResidualBasedBlockBuilderAndSolverNohBathe() override = default;
    ...
    void InitializeSolutionStep(ModelPart& rModelPart, TSystemMatrixType& rA, TSystemVectorType& rDx, TSystemVectorType& rb) override
    {
		...
		double t_step = rModelPart.GetProcessInfo()[DELTA_TIME];

        mq1 = (1. - 2. * mp) / (2 * mp * (1 - mp));
        mq2 = 1. / 2. - mp * mq1;
        mq0 = -mq1 - mq2 + 1.0 / 2.0;
        ma0 = mp * t_step;
        ma1 = 1.0 / 2.0 * pow((mp * t_step), 2.0);
        ma2 = ma0 / 2;
        ma3 = (1.0 - mp) * t_step;
        ma4 = 1.0 / 2.0 * pow(((1 - mp) * t_step), 2.0);
        ma5 = mq0 * ma3;
        ma6 = (1.0 / 2.0 + mq1) * ma3;
        ma7 = mq2 * ma3;
    }
    ...
    void BuildLHS(typename TSchemeType::Pointer pScheme, ModelPart& rModelPart, TSystemMatrixType& rA) override
    {
        ...
        KRATOS_INFO_IF("ResidualBasedBlockBuilderAndSolverNohBathe",
                          ...
                          TSystemVectorType&            rb) override
    {
        ...
        KRATOS_INFO_IF("ResidualBasedBlockBuilderAndSolverNohBathe", BaseType::GetEchoLevel() >= 3)
            << "Before the solution of the system"
            << "\nSystem Matrix = " << rA << "\nUnknowns vector = " << rDx
            << "\nRHS vector = " << rb << std::endl;
    ...
    [[nodiscard]] Parameters GetDefaultParameters() const override
    {
        ...
        {
            "name"                                : "block_builder_and_solver_noh_bathe"
        })");
     ...
     */
    static std::string Name() { return "block_builder_and_solver_noh_bathe"; }
    ...
    {
        return "ResidualBasedBlockBuilderAndSolverNohBathe";
    }

private:
    TSystemMatrixType mMassMatrix;
    TSystemVectorType mInvLumpedMassMatrix;
    ...
    void LumpMassMatrix(const TSystemMatrixType& M, TSystemVectorType& LumpedM)
    {
        ...
        // Iterate over non-zero elements row-wise
        for (compressed_matrix<double>::const_iterator1 row_it = M.begin1(); row_it != M.end1(); ++row_it) {
            ...
            for (compressed_matrix<double>::const_iterator2 col_it = row_it.begin(); col_it != row_it.end(); ++col_it) {
                LumpedM(row) += *col_it;  // sum all entries in the row
            }
        }
...
template <class TSparseSpace, class TDenseSpace, class TLinearSolver>
const Kratos::Flags ResidualBasedBlockBuilderAndSolverNohBathe<TSparseSpace, TDenseSpace, TLinearSolver>::SILENT_WARNINGS(
