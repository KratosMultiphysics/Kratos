...
/**
 * @class IncrementalNewmarkBlockBuilderAndSolverWithMassAndDamping
...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(IncrementalNewmarkBlockBuilderAndSolverWithMassAndDamping);
     ...
     */
    IncrementalNewmarkBlockBuilderAndSolverWithMassAndDamping() = default;
     ...
     * @brief Constructor.
     */
    explicit IncrementalNewmarkBlockBuilderAndSolverWithMassAndDamping(typename TLinearSolver::Pointer pNewLinearSystemSolver)
        : BaseType(pNewLinearSystemSolver)
    {
    }
    ...
    /** Destructor.
     */
    ~IncrementalNewmarkBlockBuilderAndSolverWithMassAndDamping() override = default;
        ...
        TSystemVectorType& rb) override
    {
        ...
        TSparseSpace::SetToZero(mCurrentRhs);
        ...
        if (mPrevExternalForce.size() == 0)
        {
            mPrevExternalForce.resize(BaseType::mEquationSystemSize, false);
            TSparseSpace::SetToZero(mPrevExternalForce);
        ...
        #pragma omp parallel firstprivate(nelements,nconditions, lhs_contribution, mass_contribution,damping_contribution, rhs_contribution, equation_ids )
        {
            ...
            for (int k = 0; k < nelements; k++) {
                ...
                if (it_elem->IsActive()) {
                    ...
                    BaseType::Assemble(rA, mCurrentRhs, lhs_contribution, rhs_contribution, equation_ids);
                }
            }
    ...
    Parameters GetDefaultParameters() const override
    {
        ...
        {
            "name"                                 : "incremental_newmark_block_builder_and_solver_with_mass_and_damping",
            "block_builder"                        : true,
        ...
        TSystemVectorType& rb)
    {
        ...
        for (int i = 0; i<nconditions; i++) {
            ...
            if(it->IsActive()) {
                ...
                //assemble the elemental contribution
                BaseType::AssembleRHS(mCurrentRhs, rhs_contribution, equation_ids);
...
private:
    ...
    TSystemVectorType mPrevExternalForce;
    TSystemVectorType mPrevIterationForce;
    TSystemVectorType mCurrentRhs;
     ...
     * @param rb The RHS vector
     */
    void AddMassAndDampingToRhs(ModelPart& rModelPart, TSystemVectorType& rdelta_b)
    {
...
const Kratos::Flags IncrementalNewmarkBlockBuilderAndSolverWithMassAndDamping<TSparseSpace, TDenseSpace, TLinearSolver>::SILENT_WARNINGS(Kratos::Flags::Create(0));
