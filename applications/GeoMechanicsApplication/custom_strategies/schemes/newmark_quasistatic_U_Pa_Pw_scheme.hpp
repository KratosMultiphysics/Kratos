...
#if !defined(KRATOS_NEWMARK_QUASISTATIC_U_PA_PW_SCHEME )
#define  KRATOS_NEWMARK_QUASISTATIC_U_PA_PW_SCHEME
...
class NewmarkQuasistaticUPaPwScheme : public Scheme<TSparseSpace,TDenseSpace>
{

public:

    KRATOS_CLASS_POINTER_DEFINITION( NewmarkQuasistaticUPaPwScheme );
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    ///Constructor
    NewmarkQuasistaticUPaPwScheme(double beta, double gamma, double theta) : Scheme<TSparseSpace,TDenseSpace>()
    {
        mBeta = beta;
        mGamma = gamma;
        mTheta = theta;
    ...
    ///Destructor
    ~NewmarkQuasistaticUPaPwScheme() override {}
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    int Check(const ModelPart& rModelPart) const override
    {
        ...
        for (const auto& rNode : rModelPart.Nodes())
        {
            ...
            if(rNode.SolutionStepsDataHas(AIR_PRESSURE) == false)
                KRATOS_ERROR << "AIR_PRESSURE variable is not allocated for node "
                             << rNode.Id()
                             << std::endl;

            if(rNode.SolutionStepsDataHas(DT_AIR_PRESSURE) == false)
                KRATOS_ERROR << "DT_AIR_PRESSURE variable is not allocated for node "
                             << rNode.Id()
                             << std::endl;
            ...
            if(rNode.HasDofFor(AIR_PRESSURE) == false)
                KRATOS_ERROR << "missing AIR_PRESSURE dof on node "
                             << rNode.Id()
                             << std::endl;
        }
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    NewmarkQuasistaticUPaPwScheme() : Scheme<TSparseSpace,TDenseSpace>()
    {
        mBeta = 0.25;
        mGamma = 0.5;
        mTheta = 0.5;
    ...
    virtual inline void UpdateVariablesDerivatives(ModelPart& rModelPart)
    {
        ...
        block_for_each(rModelPart.Nodes(), [&](Node<3>& rNode) {
            ...
            const double DeltaWaterPressure =  rNode.FastGetSolutionStepValue(WATER_PRESSURE)
                                             - rNode.FastGetSolutionStepValue(WATER_PRESSURE, 1);

            rNode.FastGetSolutionStepValue(DT_WATER_PRESSURE) = ( DeltaWaterPressure
                                                                 - (1.0-mTheta) * mDeltaTime
                                                                  * rNode.FastGetSolutionStepValue(DT_WATER_PRESSURE, 1))
                                                                / (mTheta*mDeltaTime);

            const double DeltaAirPressure =  rNode.FastGetSolutionStepValue(AIR_PRESSURE)
                                             - rNode.FastGetSolutionStepValue(AIR_PRESSURE, 1);

            rNode.FastGetSolutionStepValue(DT_AIR_PRESSURE) = ( DeltaAirPressure
                                                                 - (1.0-mTheta) * mDeltaTime
                                                                  * rNode.FastGetSolutionStepValue(DT_AIR_PRESSURE, 1))
                                                                / (mTheta*mDeltaTime);
        });
...
}; // Class NewmarkQuasistaticUPaPwScheme
}  // namespace Kratos

#endif // KRATOS_NEWMARK_QUASISTATIC_U_PA_PW_SCHEME defined
