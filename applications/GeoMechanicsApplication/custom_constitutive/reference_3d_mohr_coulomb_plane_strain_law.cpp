...
ConstitutiveLaw::Pointer Reference3DMohrCoulombPlaneStrainLaw::Clone() const
{
    ...
    p_result->mTraceInitialState              = mTraceInitialState;
    return p_result;
}
    ...
    const Vector& rShapeFunctionsValues)
{
    ...
    SynchronizeReferenceInitialState(rElementGeometry);
    ...
    Parameters& rValues)
{
    ...
    // if (mTraceInitialState) {
    //     KRATOS_WATCH("Element 4: response calculated by the reference 3D law")
    //     KRATOS_WATCH(rValues.GetProcessInfo()[STEP])
    //     KRATOS_WATCH(rValues.GetProcessInfo()[NL_ITERATION_NUMBER])
    //     KRATOS_WATCH(reference_properties[COHESION])
    //     KRATOS_WATCH(reference_properties[FRICTION_ANGLE])
    //     if (HasInitialState()) {
    //         KRATOS_WATCH(GetInitialState().GetInitialStressVector())
    //     }
    //     KRATOS_WATCH(strain_vector_3d)
    //     KRATOS_WATCH(stress_vector_3d)
    // }
...
void Reference3DMohrCoulombPlaneStrainLaw::SynchronizeReferenceInitialState(
    const Geometry<Node>& rElementGeometry)
{
    if (!HasInitialState()) {
        return;
    ...
    const auto  initial_strain_3d =
        MapPlaneStrainVectorTo3D(r_initial_state.GetInitialStrainVector());
    const auto initial_stress_3d =
        MapPlaneStrainVectorTo3D(r_initial_state.GetInitialStressVector());

    if (mTraceInitialState) {
        KRATOS_WATCH("Element 4: InitialState mapped from plane strain to 3D")
        KRATOS_WATCH(r_initial_state.GetInitialStressVector())
        KRATOS_WATCH(initial_stress_3d)
    }

    mpReferenceLaw->SetInitialState(
        Kratos::make_intrusive<InitialState>(initial_strain_3d, initial_stress_3d));
}

void Reference3DMohrCoulombPlaneStrainLaw::SetInitialStateTracing(bool EnableTracing)
{
    mTraceInitialState = EnableTracing;
}
