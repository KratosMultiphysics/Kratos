...
namespace Kratos
{
	//************************************************************************************
	//************************************************************************************
	void MeshlessShellKLThickElement::EquationIdVector(
	...
	//************************************************************************************
	//************************************************************************************
	void MeshlessShellKLThickElement::GetDofList(
	...
	//***********************************************************************************
	//***********************************************************************************
	void MeshlessShellKLThickElement::FinalizeSolutionStep(
		ProcessInfo& rCurrentProcessInfo)
	{
		for (int i = 0; i < 3; i++)
		{
	...
	//************************************************************************************
	//************************************************************************************
	void MeshlessShellKLThickElement::InitializeMaterial()
	{
		KRATOS_TRY
			...
			if (GetProperties()[CONSTITUTIVE_LAW] != NULL)
			{
				...
				if (mConstitutiveLawVector.size() != 3)
				{
					mConstitutiveLawVector.resize(3);
				}
				...
				for (int i = 0; i < 3; i++)
				{
					...
					mConstitutiveLawVector[i]->SetValue(INTEGRATION_WEIGHT, integration_weight, emptyProcessInfo);
					mConstitutiveLawVector[i]->SetValue(SHAPE_FUNCTION_LOCAL_DERIVATIVES, DN_De, emptyProcessInfo);
	...
	//************************************************************************************
	//************************************************************************************
	void MeshlessShellKLThickElement::CalculateAll(
	...
	)
	{
		...
		CalculateStrain(strain_vector, actual_metric.gab, mInitialMetric.gab);

		constitutive_variables.StrainVector = prod(mInitialMetric.Q, strain_vector);

		//KRATOS_WATCH(actual_metric.gab)
		//	KRATOS_WATCH(mInitialMetric.curvature)
		CalculateCurvature(curvature_vector, actual_metric.curvature, mInitialMetric.curvature);
		curvature_vector *= thickness / 2;


		Vector thickness_gps_location = ZeroVector(3);
		thickness_gps_location[0] = thickness / 2;
		thickness_gps_location[1] = 0;
		thickness_gps_location[2] = - thickness / 2;

		Vector thickness_gps_length = ZeroVector(3);
		thickness_gps_length[0] = thickness / 3;
		thickness_gps_length[1] = thickness / 3;
		thickness_gps_location[2] = thickness / 3;
		...
		for (int i = 0; i < 3; i++)
		{
			...
			CrossProduct2(a, curvature_vector, mInitialMetric.g3);
			constitutive_variables.StrainVector += a * thickness_gps_location[i] / 2;
			std::cout << "curvature_vector " << i << " is: " << curvature_vector << std::endl;
			std::cout << "constitutive_variables.StrainVector " << i << " is: " << constitutive_variables.StrainVector << std::endl;
			//Constitive Matrices DMembrane and DCurvature
			Values.SetStrainVector(constitutive_variables.StrainVector);    //this is the input parameter
			Values.SetStressVector(constitutive_variables.StressVector);    //this is an ouput parameter
			Values.SetConstitutiveMatrix(constitutive_variables.DMembrane); //this is an ouput parameter

			mConstitutiveLawVector[i]->CalculateMaterialResponse(Values, ConstitutiveLaw::StressMeasure_PK2);

			constitutive_variables.DCurvature = constitutive_variables.DMembrane*pow(thickness_gps_location[i], 2) / 2;

			//Local Cartesian Forces and Moments
			constitutive_variables.StressVector = prod(
				trans(constitutive_variables.DMembrane), constitutive_variables.StrainVector);

			integration_weight = this->GetValue(INTEGRATION_WEIGHT) * mInitialMetric.detJ * thickness_gps_length[i];
	...
	//************************************************************************************
	//************************************************************************************
	void MeshlessShellKLThickElement::CalculateOnIntegrationPoints(
	...
	)
	{
		...
		{
			rOutput[0] = 0.0;
			mConstitutiveLawVector[1]->GetValue(DAMAGE_T, rOutput[0]);
		}
		else if (rVariable == DAMAGE_T_INSIDE)
		{
			//rOutput[0] = 0.0;
			mConstitutiveLawVector[0]->GetValue(DAMAGE_T, rOutput[0]);
		}
		else if (rVariable == DAMAGE_T_OUTSIDE)
		{
			//rOutput[0] = 0.0;
			mConstitutiveLawVector[2]->GetValue(DAMAGE_T, rOutput[0]);
		}
		else if (rVariable == DAMAGE_C)
		{
			//rOutput[0] = 0.0;
			mConstitutiveLawVector[1]->GetValue(DAMAGE_C, rOutput[0]);
		}
		else if (rVariable == DAMAGE_C_INSIDE)
		{
			//rOutput[0] = 0.0;
			mConstitutiveLawVector[0]->GetValue(DAMAGE_C, rOutput[0]);
		}
		else if (rVariable == DAMAGE_C_OUTSIDE)
		{
			//rOutput[0] = 0.0;
			mConstitutiveLawVector[2]->GetValue(DAMAGE_C, rOutput[0]);
		}
		else
		{
	...
	//************************************************************************************
	//************************************************************************************
	void MeshlessShellKLThickElement::CalculateMetric(
	...
	//************************************************************************************
	//************************************************************************************
	void MeshlessShellKLThickElement::CalculateConstitutiveVariables(
