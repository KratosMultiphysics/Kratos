...
Element::Pointer MeshlessMembraneElement::Create(
	IndexType NewId, 
...
void MeshlessMembraneElement::EquationIdVector(
...
void MeshlessMembraneElement::GetDofList(
...
void MeshlessMembraneElement::Initialize()
...
void MeshlessMembraneElement::CalculateRightHandSide(
...
void MeshlessMembraneElement::CalculateLocalSystem(
...
void MeshlessMembraneElement::CalculateOnIntegrationPoints(
...
void MeshlessMembraneElement::FinalizeSolutionStep(
	ProcessInfo& rCurrentProcessInfo)
{
		//
		//            ConstitutiveLaw::Parameters Values (GetGeometry(),GetProperties(),rCurrentProcessInfo);
...
void MeshlessMembraneElement::GetValuesVector(
	Vector& values,
	int Step)
...
void MeshlessMembraneElement::GetFirstDerivativesVector(
	Vector& values,
	int Step)
...
void MeshlessMembraneElement::GetSecondDerivativesVector(
	Vector& values,
	int Step)
...
void MeshlessMembraneElement::CalculateAndAddKm(
	Matrix& K,
	Matrix& B,
	Matrix& D,
	double weight)
...
void MeshlessMembraneElement::CalculateAndSubKp(
	Matrix& K,
	array_1d<double, 3>& ge,
	array_1d<double, 3>& gn,
	const Matrix& DN_De,
...
//***********************************************************************************
//***********************************************************************************
void MeshlessMembraneElement::ClearNodalForces()
{
	KRATOS_TRY
...
void MeshlessMembraneElement::AddExplicitContribution(const VectorType& rRHSVector,
...
void MeshlessMembraneElement::CrossProduct(
...
void  MeshlessMembraneElement::ExpandReducedMatrix(
	Matrix& Destination,
...
void MeshlessMembraneElement::CalculateAndAdd_BodyForce(
	const Vector& N,
	const ProcessInfo& rCurrentProcessInfo,
...
void MeshlessMembraneElement::CalculateAndAdd_PressureForce(
...
void MeshlessMembraneElement::CalculateAll(
	MatrixType& rLeftHandSideMatrix,
	VectorType& rRightHandSideVector,
	const ProcessInfo& rCurrentProcessInfo,
...

{
	...
	CalculateQ(Q, mG_Vector[0]);
	CalculateB(B, Q, DN_De, ge, gn);
...
void MeshlessMembraneElement::Calculate_GlobalStressVector(
...
inline array_1d<double, 6> MeshlessMembraneElement::VoigtTensorComponents(
	array_1d<double, 3>& a,
	array_1d<double, 3>& b)
...
//************************************************************************************
//************************************************************************************
void MeshlessMembraneElement::GetValueOnIntegrationPoints(const Variable<Matrix>& rVariable,
...
int  MeshlessMembraneElement::Check(const ProcessInfo& rCurrentProcessInfo)
{
	KRATOS_TRY
