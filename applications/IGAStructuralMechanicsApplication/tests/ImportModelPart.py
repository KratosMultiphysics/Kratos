...
class ModelPartIOIGA(KratosMultiphysics.IO):
	...
	def ReadModelPart(self, model_part):
		...
		#--- CONDITIONS ---#
		#name = self.settings["own_name"]
		#with open(name) as JsonFile:
		load_list = self.projectparameters["loads_process_list"]
		for i in range(0,load_list.size()):
			loadID = load_list[i]["parameters"]["brep_id"].GetInt()
			for element in dim2_elements_list:
				if (loadID == element[0]):
					self.ReadConditions(model_part, element, load_list[i], IGAModeler, ElementNodeIds)
		...
		for brep_element in brep_elements_list:
			...
			#parameter_file = open(name,'r')
			#ProjectParameters = KratosMultiphysics.Parameters( parameter_file.read())
			
			Condition = 0
		
			#with open(name) as JsonFile:
			support_list = self.projectparameters["constraints_process_list"]
			#with open(name) as JsonFile:
			coupling_list = self.projectparameters["coupling_condition_list"]

			for i in range(0,support_list.size()):
				#if ProjectParameters["loads_process_list"]["Parameters"]["mesh_id"] in property:
				if support_list[i]["parameters"]["brep_id"].GetInt() == brepID:
					Condition = "support"
					brepProperty = support_list[i]

			for i in range(0,coupling_list.size()):
				#if ProjectParameters["loads_process_list"]["Parameters"]["mesh_id"] in property:
				if coupling_list[i]["parameters"]["brep_id"].GetInt() == brepID:
					Condition = "coupling"
					brepProperty = coupling_list[i]
			...
			for brep in brep_element[1]:
				...
				for quadrature_point in brep[1]:
					...
					if Condition == "coupling":
						...
						if (brepProperty["parameters"]["condition_name"] == "ContinuityConditionPenalty"):
						...
						elif (brepProperty["parameters"]["condition_name"].GetString() == "MeshlessPenaltyCouplingRotationCondition"):
							print("applied BC")
							nodeIds = []
							...
							#PENALTY
							penalty = brepProperty["parameters"]["penalty_factor"].GetDouble() # math.pow(10,7)
							Master.SetValue(KratosMultiphysics.IGAStructuralMechanicsApplication.PENALTY_FACTOR, penalty)
							...
							if (brepProperty["parameters"]["is_fixed_rot"].GetBool()):
								DisplacementRotationFix += 1000
							if (brepProperty["parameters"]["is_fixed_x"].GetBool()):
								DisplacementRotationFix += 100
							if (brepProperty["parameters"]["is_fixed_y"].GetBool()):
								DisplacementRotationFix += 10
							if (brepProperty["parameters"]["is_fixed_z"].GetBool()):
						...
						elif (brepProperty["parameters"]["condition_name"].GetString() == "MeshlessLagrangeCouplingCondition"):
							print("applied BC")
							nodeIds = []
							LagrangeVector = KratosMultiphysics.Vector(2*(n+m)) 
							for index in range (0, n):
					...
					if Condition == "support":
						...
						nodeIds = []
						Penalty = math.sqrt(brepProperty["parameters"]["penalty_factor"].GetDouble())
						for index in range (0, n):
							nodeIds.append(nodeIdsMaster[index])
						sub_model_part = model_part.GetSubModelPart(brepProperty["parameters"]["model_part_name"].GetString())
	...
	def ReadConditions(self, model_part, condition, property, IGAModeler, ElementNodeIds):
		...
		for element in condition[1]:
			if (property["description"].GetString() == "Surface"):
				qpID = 5
				elementID = int(element[0])
			if (property["description"].GetString() == "Edge"):
				qpID = 1
			...
			for quadrature_point_system in element[qpID]:
				...
				for index in range (0, n):
					nodeIds.append(nodeIdsMaster[index])
					ShapeFunctionVector[index] = ShapeFunctionValuesMaster[index]
				sub_model_part = model_part.GetSubModelPart(property["parameters"]["model_part_name"].GetString())

				for nodeID in nodeIds:
					sub_model_part.AddNode(model_part.GetNode(nodeID),0)

				QuadratureWeighting = quadrature_point[1]
				Master = sub_model_part.CreateNewCondition(property["condition_name"].GetString(), cond_id, nodeIds, model_part.Properties[0])
				Master.SetValue(KratosMultiphysics.IGAStructuralMechanicsApplication.SHAPE_FUNCTION_VALUES, ShapeFunctionVector)
				Master.SetValue(KratosMultiphysics.IGAStructuralMechanicsApplication.SHAPE_FUNCTION_LOCAL_DERIVATIVES, ShapeFunctionLocalDerivativesMaster)
				Master.SetValue(KratosMultiphysics.IGAStructuralMechanicsApplication.INTEGRATION_WEIGHT, QuadratureWeighting)
