...
class AssignVectorVariableAndConstraintsToConditionProcess(KratosMultiphysics.Process):
    def __init__(self, Model, settings ):
        ...
        if(not settings["value"][0].IsNull()):
            ...
            flag_name = "KratosMultiphysics.IGAStructuralMechanicsApplication.IGAFlags.FIX_" + settings["variable_name"].GetString() + "_X"
        ...
        if(not settings["value"][1].IsNull()):
            ...
            flag_name = "KratosMultiphysics.IGAStructuralMechanicsApplication.IGAFlags.FIX_" + settings["variable_name"].GetString() + "_Y"
            KratosMultiphysics.VariableUtils().SetFlag(eval(flag_name), True, self.model_part.Conditions)
        else:
            flag_name = "KratosMultiphysics.IGAStructuralMechanicsApplication.IGAFlags.FIX_" + settings["variable_name"].GetString() + "_Y"
            KratosMultiphysics.VariableUtils().SetFlag(eval(flag_name), False, self.model_part.Conditions)
        ...
        if(not settings["value"][2].IsNull()):
            ...
            flag_name = "KratosMultiphysics.IGAStructuralMechanicsApplication.IGAFlags.FIX_" + settings["variable_name"].GetString() + "_Z"
            KratosMultiphysics.VariableUtils().SetFlag(eval(flag_name), True, self.model_part.Conditions)
        else:
            flag_name = "KratosMultiphysics.IGAStructuralMechanicsApplication.IGAFlags.FIX_" + settings["variable_name"].GetString() + "_Z"
            KratosMultiphysics.VariableUtils().SetFlag(eval(flag_name), False, self.model_part.Conditions)
