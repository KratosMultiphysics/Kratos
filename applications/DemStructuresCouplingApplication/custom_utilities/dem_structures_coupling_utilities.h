...
std::string CheckProvidedProperties(Properties::Pointer props) {
    std::vector<const Variable<double>* > list_of_variables_double_to_check = {&STATIC_FRICTION, &KINEMATIC_FRICTION, &FRICTION_DECAY, &WALL_COHESION, &SEVERITY_OF_WEAR, &IMPACT_WEAR_SEVERITY, &BRINELL_HARDNESS, &YOUNG_MODULUS, &POISSON_RATIO};
