...
# ==============================================================================
class ThicknessResponseFunction(ResponseFunctionInterface):
    """
    ...
    def __init__(self, identifier, response_settings, model):
        ...
        if input_type == "mdpa":
            ...
            if domain_size not in [3]:
                raise Exception("ThicknessResponseFunction: Invalid 'domain_size': {}".format(domain_size))
        ...
        self.model.GetModelPart(self._model_part_name.split(".")[0]).AddNodalSolutionStepVariable(KSO.NODAL_THICKNESS)
    ...
    def Initialize(self):
        ...
        if only != "":
            ...
            if only_part.NumberOfConditions() == 0:
                ...
                Logger.PrintWarning("ThicknessResponse", "Automatically added {} conditions to model_part '{}'.".format(only_part.NumberOfConditions(), only_part.Name))
        else:
        ...
        self.response_function_utility = KSO.ThicknessResponseFunctionUtility(only_part, self.response_settings)
    ...
    def CalculateValue(self):
        Logger.PrintInfo("ThicknessResponse", "Starting calculation of response value:", self.identifier)

        startTime = timer.time()
        self.value = self.response_function_utility.CalculateValue()
        Logger.PrintInfo("ThicknessResponse", "Time needed for calculating the response value = ",round(timer.time() - startTime,2),"s")

    def CalculateGradient(self):
        Logger.PrintInfo("ThicknessResponse", "Starting gradient calculation for response", self.identifier)
        ...
        Logger.PrintInfo("ThicknessResponse", "Time needed for calculating gradients",round(timer.time() - startTime,2),"s")
