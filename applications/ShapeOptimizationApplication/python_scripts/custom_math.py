...
def QuadProg(A, b, max_itr, tolerance):
    ...
    while error>tolerance and k <= max_itr:
        ...
        if Norm2(deltaXYLAff)>1e20:
            KM.Logger.PrintWarning("custom_math::quadprodg: deltaXYLAff is NAN. The reason is, that feasible domain might be empty. This happens e.g. when the dJdX is parallel to dCdX (like at convergence with a single constraint)")
            exit_code = 2
            return
...
def PerformBisectioning(func, a, b, target, tolerance, max_itr):
    ...
    if abs(fa_value_0-target) < tolerance:
        ...
        KM.Logger.PrintWarning("Bisectioning intervall yiels to identical function values!")
        res_function_argument = a
        error = abs(fb_value_0-target)

    elif (fa_value_0-target)*(fb_value_0-target)>0:
        KM.Logger.PrintWarning("Bisectioning on function, that has no root in specified intervall!! Returning the argument which yiels a closer value to the target.")
    ...
    # Perform bisectioning if no special case applies
        ...
        for itr in range(1,max_itr+1):
            ...
            if itr == max_itr:
                ...
                KM.Logger.PrintWarning("Bisectioning did not converge in the specified maximum number of iterations!")
                break
...
def PerformGramSchmidtOrthogonalization(vector_space):
    ...
    for v in V[1:]:
        ...
        if norm2_v>1e-10:
            ...
            KM.Logger.PrintWarning("Zero basis vector after Gram-Schmidt orthogonalization!")
            B.append(v)
