...
#if !defined(KRATOS_INPUT_PRIMAL_STRUCTURAL_SOLUTION_PROCESS_H_INCLUDED )
#define KRATOS_INPUT_PRIMAL_STRUCTURAL_SOLUTION_PROCESS_H_INCLUDED
 ...
 */
class InputPrimalStructuralSolutionProcess : public Process
{
public:
    ///@name Type Definitions
    ///@{

    KRATOS_CLASS_POINTER_DEFINITION(InputPrimalStructuralSolutionProcess);
    ...
    /// Constructor.
    InputPrimalStructuralSolutionProcess(ModelPart& rModelPart, Parameters& rParameters)
    ...
    /// Destructor.
    ~InputPrimalStructuralSolutionProcess() override {}
    ...
    void ExecuteInitialize() override
    {
        ...
        for (unsigned int i = 0; i < mVariables.size(); i++)
        {
            ...
            else if (KratosComponents< Variable<Vector > >::Has(mVariables[i])) // A.Kayis
            {
                if (mrModelPart.GetNodalSolutionStepVariablesList().Has(KratosComponents< Variable<Vector > >::Get(mVariables[i])) == false)
                {
                    KRATOS_THROW_ERROR(std::runtime_error,"variable is not found in nodal solution steps variable list: ", mVariables[i])
                }
            }
    ...
    void ExecuteInitializeSolutionStep() override
    {
        ...
        for (unsigned int i = 0; i < mVariables.size(); i++)
        {
            ...
            else if (KratosComponents< Variable<Vector > >::Has(mVariables[i])) // A.Kayis // variable needs to be initialized before process.ExecuteInitializeSolutionStep
            {
                const Variable< Vector >& rVariable = KratosComponents< Variable<Vector > >::Get(mVariables[i]);
                unsigned int VectorSize = mrModelPart.Nodes().front().FastGetSolutionStepValue(rVariable,0).size();
                std::vector<double> VariableDataBuffer(VectorSize * mrModelPart.Nodes().size());
                VariableDataset.read(VariableDataBuffer.data(), H5::PredType::NATIVE_DOUBLE);
                unsigned int BlockBegin = 0;
                for (auto it = std::begin(mrModelPart.Nodes()); it != std::end(mrModelPart.Nodes()); it++)
                {
                    Vector& rData = it->FastGetSolutionStepValue(KratosComponents< Variable<Vector > >::Get(mVariables[i]));
                    for (unsigned int k = 0; k < VectorSize; k++)
                        rData[k] = VariableDataBuffer[BlockBegin + k];
                    BlockBegin += VectorSize;
                }
            }
        }
    ...
    {
        return "InputPrimalStructuralSolutionProcess";
    }
...
private:
    ...
    ///@}
}; /* Class InputPrimalStructuralSolutionProcess */
...
inline std::istream& operator >> (std::istream& rIStream,
                                  InputPrimalStructuralSolutionProcess& rThis)
{
    return rIStream;
}
...
inline std::ostream& operator << (std::ostream& rOStream,
                                  const InputPrimalStructuralSolutionProcess& rThis)
{
...
#endif /* KRATOS_INPUT_PRIMAL_STRUCTURAL_SOLUTION_PROCESS_H_INCLUDED defined */
