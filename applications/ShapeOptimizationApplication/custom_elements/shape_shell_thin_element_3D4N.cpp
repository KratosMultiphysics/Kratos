...
#include "shape_shell_thin_element_3D4N.hpp"
...
/*
Element overview:---------------------------------------------------------------
This element represents a 4-node ShapeShell element.
...
ShapeShell formulation references:---------------------------------------------------
ANDES formulation:
Bjorn Haugen. "Buckling and Stability Problems for Thin ShapeShell Structures
...
// =========================================================================
//
// Class ShapeShellThinElement3D4N
//
// =========================================================================

template <ShapeShellKinematics TKinematics>
ShapeShellThinElement3D4N<TKinematics>::ShapeShellThinElement3D4N(IndexType NewId,
...
template <ShapeShellKinematics TKinematics>
Element::Pointer ShapeShellThinElement3D4N<TKinematics>::Create(IndexType NewId,
        NodesArrayType const& ThisNodes,
        PropertiesType::Pointer pProperties) const
{
    GeometryType::Pointer newGeom(GetGeometry().Create(ThisNodes));
    return Kratos::make_intrusive< ShapeShellThinElement3D4N >(NewId, newGeom, pProperties);
}
...
Element::Pointer ShapeShellThinElement3D4N<TKinematics>::Create(IndexType NewId,
        ...
        PropertiesType::Pointer pProperties) const
{
    return Kratos::make_intrusive< ShapeShellThinElement3D4N >(NewId, pGeom, pProperties);
}

template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::AddBodyForces(CalculationData& data,
...
// =========================================================================
//
// Class ShapeShellThinElement3D4N - Results on Gauss Points
//
// =========================================================================

template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CalculateOnIntegrationPoints(
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    int caseId = -1;
    if (rVariable == SHAPE_TSAI_WU_RESERVE_FACTOR) {
        caseId = 10;
    } else if (rVariable == SHAPE_SHEAR_ANGLE) {
        caseId = 40;
        ...
        caseId = 20;
    } else if (rVariable == SHAPE_SHELL_ELEMENT_MEMBRANE_ENERGY ||
               SHAPE_SHELL_ELEMENT_MEMBRANE_ENERGY_FRACTION ||
               SHAPE_SHELL_ELEMENT_BENDING_ENERGY ||
               SHAPE_SHELL_ELEMENT_BENDING_ENERGY_FRACTION ||
               SHAPE_SHELL_ELEMENT_SHEAR_ENERGY ||
               SHAPE_SHELL_ELEMENT_SHEAR_ENERGY_FRACTION) {
        caseId = 30;
    }
    ...
    if (caseId > 19) {
        ...
        for (unsigned int gauss_point = 0; gauss_point < size; ++gauss_point) {
            ...
            ShapeShellCrossSection::Pointer& section = this->mSections[gauss_point];
            ...
            if (caseId == 30) {
                ...
                CalculateShapeShellElementEnergy(data, rVariable, resultDouble);
            } else if (caseId == 20) {
                //Von mises calcs
                ...
                KRATOS_ERROR <<
                             "Error: ELEMENT ShapeShellThinElement3D4N, METHOD CalculateOnIntegrationPoints(double)"
                             << std::endl;
            }
...
template <ShapeShellKinematics TKinematics>
int ShapeShellThinElement3D4N<TKinematics>::Check(const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY;
    ...
    KRATOS_ERROR_IF_NOT(points_number == 4) <<"ShapeShellThinElement3D4N - Wrong number of nodes" << points_number << std::endl;
...
// =========================================================================
//
// Class ShapeShellThinElement3D4N - Private methods
//
// =========================================================================

template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CalculateStressesFromForceResultants
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CalculateLaminaStrains(CalculationData& data)
{
    ShapeShellCrossSection::Pointer& section = this->mSections[data.gpIndex];
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CalculateLaminaStresses(CalculationData& data)
{
    ShapeShellCrossSection::Pointer& section = this->mSections[data.gpIndex];
...
template <ShapeShellKinematics TKinematics>
double ShapeShellThinElement3D4N<TKinematics>::CalculateTsaiWuPlaneStress(const CalculationData& data, const Matrix& rLamina_Strengths, const unsigned int& rPly)
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CalculateVonMisesStress(const CalculationData& data, const Variable<double>& rVariable, double& rVon_Mises_Result)
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CalculateShapeShellElementEnergy(const CalculationData& data, const Variable<double>& rVariable, double& rEnergy_Result)
{
    // Energy calcs - these haven't been verified or tested yet.
    ...
    if (rVariable == SHAPE_SHELL_ELEMENT_MEMBRANE_ENERGY_FRACTION ||
            rVariable == SHAPE_SHELL_ELEMENT_BENDING_ENERGY_FRACTION ||
            rVariable == SHAPE_SHELL_ELEMENT_SHEAR_ENERGY_FRACTION) {
    ...
    if (rVariable == SHAPE_SHELL_ELEMENT_MEMBRANE_ENERGY || rVariable == SHAPE_SHELL_ELEMENT_MEMBRANE_ENERGY_FRACTION) {
        for (SizeType i = 0; i < 3; i++) {
    ...
    } else if (rVariable == SHAPE_SHELL_ELEMENT_BENDING_ENERGY || rVariable == SHAPE_SHELL_ELEMENT_BENDING_ENERGY_FRACTION) {
    ...
    } else if (rVariable == SHAPE_SHELL_ELEMENT_SHEAR_ENERGY || rVariable == SHAPE_SHELL_ELEMENT_SHEAR_ENERGY_FRACTION) {
        rEnergy_Result = 0.0;
    }
}

template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CheckGeneralizedStressOrStrainOutput(const Variable<Matrix>& rVariable, int& ijob, bool& bGlobal)
{
    if (rVariable == SHAPE_SHELL_STRAIN) {
        ...
        ijob = 9;
        bGlobal = true;
    } else if (rVariable == SHAPE_SHELL_ORTHOTROPIC_4PLY_THROUGH_THICKNESS) {
        // TESTING VARIABLE
        ijob = 99;
    }
}
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::InitializeCalculationData(CalculationData& data)
{
    KRATOS_TRY
    //-------------------------------------
    ...
    if (data.basicQuad == false) {
        ...
        ShapeShellUtilities::ShapeFunc(1, 0, data.N);
        for (int i = 0; i < 4; i++) {
        ...
        data.N.clear();
        ShapeShellUtilities::ShapeFunc(0, 1, data.N);
        for (int i = 0; i < 4; i++) {
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CalculateBMatrix(CalculationData& data)
{
    //---------------------------------------------
    // geom data
    const GeometryType& geom = GetGeometry();
    ...
    for (int node = 0; node < 4; node++) {
        ...
        dpsiX_dxi[3 * node] = 1.5 *
                              (ar*ShapeShellUtilities::dN_seren_dxi(r, xi, eta) -
                               as*ShapeShellUtilities::dN_seren_dxi(s, xi, eta));

        dpsiX_dxi[3 * node + 1] = br*ShapeShellUtilities::dN_seren_dxi(r, xi, eta) +
                                  bs*ShapeShellUtilities::dN_seren_dxi(s, xi, eta);

        dpsiX_dxi[3 * node + 2] =
            ShapeShellUtilities::dN_seren_dxi(node + 1, xi, eta) -
            cr*ShapeShellUtilities::dN_seren_dxi(r, xi, eta) -
            cs*ShapeShellUtilities::dN_seren_dxi(s, xi, eta);

        // Compute vector of dPsi_y/dxi
        dpsiY_dxi[3 * node] = 1.5 *
                              (dr*ShapeShellUtilities::dN_seren_dxi(r, xi, eta) -
                               ds*ShapeShellUtilities::dN_seren_dxi(s, xi, eta));

        dpsiY_dxi[3 * node + 1] = -1.0 *
                                  ShapeShellUtilities::dN_seren_dxi(node + 1, xi, eta) +
                                  er*ShapeShellUtilities::dN_seren_dxi(r, xi, eta) +
                                  es*ShapeShellUtilities::dN_seren_dxi(s, xi, eta);

        dpsiY_dxi[3 * node + 2] = -1.0 * br*
                                  ShapeShellUtilities::dN_seren_dxi(r, xi, eta) -
                                  bs*ShapeShellUtilities::dN_seren_dxi(s, xi, eta);

        // d( ) / deta
        // Compute vector of dPsi_x/deta
        dpsiX_deta[3 * node] = 1.5 *
                               (ar*ShapeShellUtilities::dN_seren_deta(r, xi, eta) -
                                as*ShapeShellUtilities::dN_seren_deta(s, xi, eta));

        dpsiX_deta[3 * node + 1] = br*
                                   ShapeShellUtilities::dN_seren_deta(r, xi, eta) +
                                   bs*ShapeShellUtilities::dN_seren_deta(s, xi, eta);

        dpsiX_deta[3 * node + 2] =
            ShapeShellUtilities::dN_seren_deta(node + 1, xi, eta) -
            cr*ShapeShellUtilities::dN_seren_deta(r, xi, eta) -
            cs*ShapeShellUtilities::dN_seren_deta(s, xi, eta);

        // Compute vector of dPsi_y/deta
        dpsiY_deta[3 * node] = 1.5 *
                               (dr*ShapeShellUtilities::dN_seren_deta(r, xi, eta) -
                                ds*ShapeShellUtilities::dN_seren_deta(s, xi, eta));

        dpsiY_deta[3 * node + 1] = -1.0 *
                                   ShapeShellUtilities::dN_seren_deta(node + 1, xi, eta) +
                                   er*ShapeShellUtilities::dN_seren_deta(r, xi, eta) +
                                   es*ShapeShellUtilities::dN_seren_deta(s, xi, eta);
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CalculateSectionResponse(CalculationData& data)
{
    const GeometryType& geom = GetGeometry();
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CalculateGaussPointContribution
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::CalculateAll(MatrixType& rLeftHandSideMatrix,
                                        VectorType& rRightHandSideVector,
                                        const ProcessInfo& rCurrentProcessInfo,
...
template <ShapeShellKinematics TKinematics>
bool ShapeShellThinElement3D4N<TKinematics>::
 ...
 const ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int i = 0; i < size; i++) {
        ...
        if (ijob == 1) { // strains
            ...
            iValue(0, 2) = iValue(2, 0) = 0.0;
            iValue(1, 2) = iValue(2, 1) = 0.0;
        } else if (ijob == 99) { // SHAPE_SHELL_ORTHOTROPIC_4PLY_THROUGH_THICKNESS
...
template <ShapeShellKinematics TKinematics>
ShapeShellThinElement3D4N<TKinematics>::CalculationData::CalculationData
(const ShapeShellQ4_LocalCoordinateSystem& localcoordsys,
 const ShapeShellQ4_LocalCoordinateSystem& refcoordsys,
 const ProcessInfo& rCurrentProcessInfo)
    : LCS(localcoordsys)
    , LCS0(refcoordsys)
...
template <ShapeShellKinematics TKinematics>
ShapeShellCrossSection::SectionBehaviorType ShapeShellThinElement3D4N<TKinematics>::GetSectionBehavior() const
{
    return ShapeShellCrossSection::Thin;
}

// =========================================================================
//
// Class ShapeShellThinElement3D4N - Serialization
//
// =========================================================================

template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::save(Serializer& rSerializer) const
{
    KRATOS_SERIALIZE_SAVE_BASE_CLASS(rSerializer, BaseType);
}

template <ShapeShellKinematics TKinematics>
void ShapeShellThinElement3D4N<TKinematics>::load(Serializer& rSerializer)
{
    KRATOS_SERIALIZE_LOAD_BASE_CLASS(rSerializer, BaseType);
}

template class ShapeShellThinElement3D4N<ShapeShellKinematics::NONLINEAR_COROTATIONAL>;
