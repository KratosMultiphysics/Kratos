 ...
 */
KRATOS_CREATE_LOCAL_FLAG( SmallDisplacementBeamElement3D2N4SensitivityAnalysis, COMPUTE_RHS_VECTOR,                 0 );
KRATOS_CREATE_LOCAL_FLAG( SmallDisplacementBeamElement3D2N4SensitivityAnalysis, COMPUTE_LHS_MATRIX,                 1 );
KRATOS_CREATE_LOCAL_FLAG( SmallDisplacementBeamElement3D2N4SensitivityAnalysis, COMPUTE_RHS_VECTOR_WITH_COMPONENTS, 2 );
KRATOS_CREATE_LOCAL_FLAG( SmallDisplacementBeamElement3D2N4SensitivityAnalysis, COMPUTE_LHS_MATRIX_WITH_COMPONENTS, 3 );


//******************************CONSTRUCTOR*******************************************
//************************************************************************************

SmallDisplacementBeamElement3D2N4SensitivityAnalysis::SmallDisplacementBeamElement3D2N4SensitivityAnalysis(IndexType NewId,GeometryType::Pointer pGeometry)
...
SmallDisplacementBeamElement3D2N4SensitivityAnalysis::SmallDisplacementBeamElement3D2N4SensitivityAnalysis(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties)
    : Element(NewId, pGeometry, pProperties)
{
    ...
    // TODO: Define ConstitutiveLaws for beams!!!
...
SmallDisplacementBeamElement3D2N4SensitivityAnalysis::SmallDisplacementBeamElement3D2N4SensitivityAnalysis( SmallDisplacementBeamElement3D2N4SensitivityAnalysis const& rOther)
    :Element(rOther)
...
Element::Pointer SmallDisplacementBeamElement3D2N4SensitivityAnalysis::Create(IndexType NewId, NodesArrayType const& ThisNodes, PropertiesType::Pointer pProperties) const
{
    //return boost::make_shared< SmallDisplacementBeamElement3D2N4SensitivityAnalysis >(NewId, GetGeometry().Create(ThisNodes), pProperties);
     return Element::Pointer(new SmallDisplacementBeamElement3D2N4SensitivityAnalysis(NewId, GetGeometry().Create(ThisNodes), pProperties));
}

//*******************************DESTRUCTOR*******************************************
//************************************************************************************

SmallDisplacementBeamElement3D2N4SensitivityAnalysis::~SmallDisplacementBeamElement3D2N4SensitivityAnalysis()
{
}
...
SmallDisplacementBeamElement3D2N4SensitivityAnalysis::IntegrationMethod  SmallDisplacementBeamElement3D2N4SensitivityAnalysis::GetIntegrationMethod() const
{
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::GetDofList(
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::EquationIdVector(EquationIdVectorType& rResult,
        ProcessInfo& CurrentProcessInfo)
{

    const unsigned int NumberOfNodes = GetGeometry().size();
    const unsigned int Dimension     = GetGeometry().WorkingSpaceDimension();
    const unsigned int ElementSize   = NumberOfNodes * ( Dimension * 2 );
    ...
    {
        const unsigned int index = i * ( Dimension * 2 );
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::GetValuesVector(Vector& rValues, int Step)
{
    ...
    {
        const unsigned int index = i * ( Dimension * 2 );
        const array_1d<double, 3> Displacement = GetGeometry()[i].GetSolutionStepValue( DISPLACEMENT, Step );
        rValues[index]     = Displacement[0];
        rValues[index + 1] = Displacement[1];
        rValues[index + 2] = Displacement[2];

        const array_1d<double, 3> Rotations = GetGeometry()[i].GetSolutionStepValue( ROTATION, Step );
        rValues[index + 3] = Rotations[0];
        rValues[index + 4] = Rotations[1];
        rValues[index + 5] = Rotations[2];
    }
...
//************************************************************************************
//************************************************************************************
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::GetFirstDerivativesVector(Vector& rValues, int Step)
{
    KRATOS_TRY
    ...
    {
        const unsigned int index = i * ( Dimension * 2 );
        const array_1d<double, 3> Velocities = GetGeometry()[i].GetSolutionStepValue( VELOCITY, Step );
        rValues[index]     = Velocities[0];
        rValues[index + 1] = Velocities[1];
        rValues[index + 2] = Velocities[2];

        const array_1d<double, 3> AngularVelocities = GetGeometry()[i].GetSolutionStepValue( ANGULAR_VELOCITY, Step );
        rValues[index + 3] = AngularVelocities[0];
        rValues[index + 4] = AngularVelocities[1];
        rValues[index + 5] = AngularVelocities[2];
    }
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::GetSecondDerivativesVector(Vector& rValues, int Step)
{
    KRATOS_TRY
    ...
    {
        const unsigned int index = i * ( Dimension * 2 );
        const array_1d<double, 3> Accelerations = GetGeometry()[i].GetSolutionStepValue( ACCELERATION, Step );
        rValues[index]     = Accelerations[0];
        rValues[index + 1] = Accelerations[1];
        rValues[index + 2] = Accelerations[2];

        const array_1d<double, 3> AngularAcceleration = GetGeometry()[i].GetSolutionStepValue( ANGULAR_ACCELERATION, Step );
        rValues[index + 3] = AngularAcceleration[0];
        rValues[index + 4] = AngularAcceleration[1];
        rValues[index + 5] = AngularAcceleration[2];
    }
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::GetValueOnIntegrationPoints( const Variable<array_1d<double, 3 > >& rVariable,
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::Initialize()
{
    KRATOS_TRY
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::InitializeSolutionStep(ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::FinalizeSolutionStep(ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::InitializeSystemMatrices(MatrixType& rLeftHandSideMatrix,
...

{
    ...
    unsigned int MatSize = NumberOfNodes * ( Dimension * 2 );

    if ( rCalculationFlags.Is(SmallDisplacementBeamElement3D2N4SensitivityAnalysis::COMPUTE_LHS_MATRIX) ) //calculation of the matrix is required
    {
    ...
    if ( rCalculationFlags.Is(SmallDisplacementBeamElement3D2N4SensitivityAnalysis::COMPUTE_RHS_VECTOR) ) //calculation of the matrix is required
    {
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateSectionProperties()
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateElementalSystem( 
    ...
    )
{
    ...
    //size needed
    const unsigned int NumberOfNodes = GetGeometry().size();
    const unsigned int MatSize = NumberOfNodes * ( Dimension * 2 );

    //initialize local transformation/rotation matrix
    rLocalSystem.RotationMatrix     = ZeroMatrix(MatSize);

    if ( rLocalSystem.CalculationFlags.Is(SmallDisplacementBeamElement3D2N4SensitivityAnalysis::COMPUTE_LHS_MATRIX) || rLocalSystem.CalculationFlags.Is(SmallDisplacementBeamElement3D2N4SensitivityAnalysis::COMPUTE_RHS_VECTOR) )
    {
    ...
    for ( std::size_t PointNumber = 0; PointNumber < IntegrationPoints.size(); PointNumber++ )
    {
        const Vector N = row( Ncontainer, PointNumber);

        if ( rLocalSystem.CalculationFlags.Is(SmallDisplacementBeamElement3D2N4SensitivityAnalysis::COMPUTE_LHS_MATRIX) ) //calculation of the matrix is required
        {
            this->CalculateAndAddLHS( rLocalSystem );
        }

        if ( rLocalSystem.CalculationFlags.Is(SmallDisplacementBeamElement3D2N4SensitivityAnalysis::COMPUTE_RHS_VECTOR) ) //calculation of the vector is required
        {
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateAndAddLHS(LocalSystemComponents& rLocalSystem)
{
    KRATOS_TRY
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateAndAddRHS(LocalSystemComponents& rLocalSystem, Vector& VolumeForce)
{
    KRATOS_TRY
...
void  SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateRightHandSide(VectorType& rRightHandSideVector,
        ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    //calculation flags
    LocalSystem.CalculationFlags.Set(SmallDisplacementBeamElement3D2N4SensitivityAnalysis::COMPUTE_RHS_VECTOR);
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateLeftHandSide(MatrixType& rLeftHandSideMatrix, ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    //calculation flags
    LocalSystem.CalculationFlags.Set(SmallDisplacementBeamElement3D2N4SensitivityAnalysis::COMPUTE_LHS_MATRIX);
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateLocalStiffnessMatrix(Matrix& LocalStiffnessMatrix)
{
    KRATOS_TRY

    // TODO: Add Timoshenko theory
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateTransformationMatrix(Matrix& rRotationMatrix)

{
    ...
    const unsigned int NumberOfNodes = GetGeometry().size();
    const unsigned int Dimension       = GetGeometry().WorkingSpaceDimension();
    unsigned int       size            = NumberOfNodes * Dimension;
...
Vector&  SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateVolumeForce( Vector& rVolumeForce, const Vector &rN)
{
    KRATOS_TRY
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateLocalBodyForce( Vector& rLocalForceVector, Vector& rVolumeForce )
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateGlobalBodyForce( Vector& rGlobalForceVector, Vector& rVolumeForce )
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateMassMatrix(MatrixType& rMassMatrix, ProcessInfo& rCurrentProcessInfo)
{
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateDistributedBodyForce(const int Direction, Vector& Load, Vector& rVolumeForce)
{
    KRATOS_TRY
    ...
    if(Direction==1) // 0->x, 1->y, 2->z
    {
        ...
        {
            alpha = inner_prod(NormalPlaneProjection,LocalBeamAxis)/(norm_2(LocalBeamAxis)*norm_2(NormalPlaneProjection));
            alpha = sign*std::acos(alpha);
        }

        sinus = std::sin(alpha);
        cosinus = std::cos(alpha);

        if(std::abs(sinus) < 1E-7) sinus = 0.00;
        if(std::abs(cosinus) < 1E-7) cosinus = 0.00;
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateOnIntegrationPoints( 
    ...
    )
{
    ...
    const unsigned int& IntegrationPointsNumber = GetGeometry().IntegrationPointsNumber( mThisIntegrationMethod );
    const unsigned int Dimension                  = GetGeometry().WorkingSpaceDimension();
    ...
    //(in fact is the two nodes beam element, only one integration point)
    for ( std::size_t PointNumber = 0; PointNumber < IntegrationPointsNumber; PointNumber++ )
    {
        const Vector N = row( Ncontainer, PointNumber);
        ...
        {
            if( std::abs(Stress[i])< 1E-6) Stress[i] = 0.00;
        }
...
double SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateInternalMoment(const double& M1, const double& M2, const double& ShearLoad, const double& X)
{
...
double SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateInternalShear(const double& Q, const double& ShearLoad, const double& X)
{
...
double SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateInternalAxil(const double& N, const double& AxialLoad, const double& X)
{
...
void SmallDisplacementBeamElement3D2N4SensitivityAnalysis::CalculateLocalNodalStress(Vector& Stress, Vector& rVolumeForce)
{
    KRATOS_TRY
...
int  SmallDisplacementBeamElement3D2N4SensitivityAnalysis::Check(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY

    if (GetGeometry().WorkingSpaceDimension() != 3 || GetGeometry().size()!=2 )
    {
        KRATOS_ERROR <<  "This element works only in 3D and with 2 noded linear elements" << std::endl;
    }
    ...
    if (this->GetProperties().Has(LOCAL_INERTIA_TENSOR)==false)
    {
        ...
        {
            KRATOS_ERROR <<"INERTIA not provided for this element " << this->Id() << std::endl;
        }
    }
