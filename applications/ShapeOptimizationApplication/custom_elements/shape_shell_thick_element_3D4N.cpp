...
#include "shape_shell_thick_element_3D4N.hpp"
...
template <ShapeShellKinematics TKinematics>
ShapeShellThickElement3D4N<TKinematics>::MITC4Params::MITC4Params(const ShapeShellQ4_LocalCoordinateSystem& LCS)
    : Transformation(2, 2)
...
template <ShapeShellKinematics TKinematics>
ShapeShellThickElement3D4N<TKinematics>::EASOperatorStorage::EASOperatorStorage()
    : mInitialized(false)
{
}

template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::EASOperatorStorage::Initialize(const GeometryType& geom)
{
    if (!mInitialized) {
        ...
        for (SizeType i = 0; i < 4; i++) {
            SizeType ii = i * 6;
            const array_1d<double, 3>& initialDispl = geom[i].FastGetSolutionStepValue(HELMHOLTZ_VARS);
            const array_1d<double, 3>& initialRot = geom[i].FastGetSolutionStepValue(HELMHOLTZ_ROTS);
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::EASOperatorStorage::InitializeSolutionStep()
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::EASOperatorStorage::FinalizeSolutionStep()
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::EASOperatorStorage::FinalizeNonLinearIteration(const Vector& displacementVector)
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::EASOperatorStorage::save(Serializer& rSerializer) const
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::EASOperatorStorage::load(Serializer& rSerializer)
{
...
template <ShapeShellKinematics TKinematics>
ShapeShellThickElement3D4N<TKinematics>::EASOperator::EASOperator(const ShapeShellQ4_LocalCoordinateSystem& LCS, EASOperatorStorage& storage)
    : mF0inv(3, 3)
    , mEnhancedStrains(3)
    , mG(3, 5)
{
    ...
    Matrix dN(4, 2);
    ShapeShellUtilities::ShapeFunc_NaturalDerivatives(xi, eta, dN);
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::EASOperator::GaussPointComputation_Step1(double xi, double eta, const ShapeShellUtilities::JacobianOperator& jac,
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::EASOperator::GaussPointComputation_Step2(const Matrix& D,
        const Matrix& B,
        const Vector& S,
        EASOperatorStorage& storage)
{
    ...
    int num_stress = D.size2(); // it can be 6 for thin shape_shells or 8 for thick  shape_shells
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::EASOperator::ComputeModfiedTangentAndResidual(Matrix& rLeftHandSideMatrix,
...
// =====================================================================================
//
// Class ShapeShellThickElement3D4N
//
// =====================================================================================

template <ShapeShellKinematics TKinematics>
ShapeShellThickElement3D4N<TKinematics>::ShapeShellThickElement3D4N(IndexType NewId,
...
// TODO are the GetIntegrationMethod methods needed (implemented in the other 3 shape_shells)

template <ShapeShellKinematics TKinematics>
Element::Pointer ShapeShellThickElement3D4N<TKinematics>::Create(IndexType NewId, NodesArrayType const& ThisNodes, PropertiesType::Pointer pProperties) const
{
    GeometryType::Pointer newGeom(GetGeometry().Create(ThisNodes));
    return Kratos::make_intrusive< ShapeShellThickElement3D4N >(NewId, newGeom, pProperties);
}

template <ShapeShellKinematics TKinematics>
Element::Pointer ShapeShellThickElement3D4N<TKinematics>::Create(IndexType NewId, GeometryType::Pointer pGeom, PropertiesType::Pointer pProperties) const
{
    return Kratos::make_intrusive< ShapeShellThickElement3D4N >(NewId, pGeom, pProperties);
}

template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::Initialize(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::FinalizeNonLinearIteration(const ProcessInfo& rCurrentProcessInfo)
{
    BaseType::FinalizeNonLinearIteration(rCurrentProcessInfo);

    ShapeShellQ4_LocalCoordinateSystem LCS(this->mpCoordinateTransformation->CreateLocalCoordinateSystem());
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::InitializeSolutionStep(const ProcessInfo& rCurrentProcessInfo)
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::FinalizeSolutionStep(const ProcessInfo& rCurrentProcessInfo)
{
...
// =====================================================================================
//
// Class ShapeShellThickElement3D4N - Results on Gauss Points
//
// =====================================================================================

template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::CalculateOnIntegrationPoints(const Variable<double>& rVariable,
        ...
        const ProcessInfo& rCurrentProcessInfo)
{
    ...
    if (!rCurrentProcessInfo.Has(NL_ITERATION_NUMBER)) {
        KRATOS_WARNING_ONCE("ShapeShellThickElement3D4N") << "Warning: Gauss point results have "
    ...
    if (rVariable == SHAPE_VON_MISES_STRESS ||
            rVariable == SHAPE_VON_MISES_STRESS_TOP_SURFACE ||
            rVariable == SHAPE_VON_MISES_STRESS_MIDDLE_SURFACE ||
            rVariable == SHAPE_VON_MISES_STRESS_BOTTOM_SURFACE) {
        // Von mises calcs
        ...
        ShapeShellQ4_LocalCoordinateSystem localCoordinateSystem(
            this->mpCoordinateTransformation->CreateLocalCoordinateSystem());

        ShapeShellQ4_LocalCoordinateSystem referenceCoordinateSystem(
        ...
        // coordinate system
        ShapeShellUtilities::JacobianOperator jacOp;
        ...
        ShapeShellCrossSection::SectionParameters parameters(geom, props, rCurrentProcessInfo);
        ...
        for (unsigned int i = 0; i < size; i++) {
            ...
            ShapeShellCrossSection::Pointer& section = this->mSections[i];
    ...
    } else if (rVariable == SHAPE_TSAI_WU_RESERVE_FACTOR) {
        // resize output
        SizeType size = 4;
        if (rValues.size() != size) {
            rValues.resize(size);
        ...
        ShapeShellCrossSection::Pointer& section = this->mSections[0];
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::CalculateOnIntegrationPoints(const Variable<Matrix>& rVariable,
...
template <ShapeShellKinematics TKinematics>
int ShapeShellThickElement3D4N<TKinematics>::Check(const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY;
    ...
    KRATOS_ERROR_IF_NOT((r_geom.IntegrationPoints(this->GetIntegrationMethod())).size() == 4) << "ShapeShellThickElement3D4N - needs a full integration scheme" << std::endl;

    const int points_number = r_geom.PointsNumber();
    KRATOS_ERROR_IF_NOT(points_number == 4) << "ShapeShellThickElement3D4N - Wrong number of nodes" << points_number << std::endl;
...
// =====================================================================================
//
// Class ShapeShellThickElement3D4N - Private methods
//
// =====================================================================================

template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::CalculateStressesFromForceResultants(VectorType& rstresses, const double& rthickness)
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::CalculateLaminaStrains(ShapeShellCrossSection::Pointer& section, const Vector& generalizedStrains, std::vector<VectorType>& rlaminateStrains)
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::CalculateLaminaStresses(ShapeShellCrossSection::Pointer& section, ShapeShellCrossSection::SectionParameters parameters, const std::vector<VectorType>& rlaminateStrains, std::vector<VectorType>& rlaminateStresses)
{
...
template <ShapeShellKinematics TKinematics>
double ShapeShellThickElement3D4N<TKinematics>::CalculateTsaiWuPlaneStress(const std::vector<VectorType>& rlaminateStresses, const Matrix& rLamina_Strengths, const unsigned int& rPly)
{
    // Incoming lamina strengths are organized as follows:
    // Refer to 'shape_shell_cross_section.cpp' for details.
    //
    //	|	T1,		C1,		T2	|
    //	|	C2,		S12,	S13	|
    //	|   S23		0		0	|
    ...
    // All F_i3 components ignored - thin shape_shell theory.
    //
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::CalculateVonMisesStress(const Vector& generalizedStresses, const Variable<double>& rVariable, double& rVon_Mises_Result)
{
    ...
    if (rVariable == SHAPE_VON_MISES_STRESS_TOP_SURFACE) {
        rVon_Mises_Result = std::sqrt(von_mises_top);
    } else if (rVariable == SHAPE_VON_MISES_STRESS_MIDDLE_SURFACE) {
        rVon_Mises_Result = std::sqrt(von_mises_mid);
    } else if (rVariable == SHAPE_VON_MISES_STRESS_BOTTOM_SURFACE) {
        rVon_Mises_Result = std::sqrt(von_mises_bottom);
    } else if (rVariable == SHAPE_VON_MISES_STRESS) {
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::CheckGeneralizedStressOrStrainOutput(const Variable<Matrix>& rVariable, int& ijob, bool& bGlobal)
{
    if (rVariable == SHAPE_SHELL_STRAIN) {
        ijob = 1;
    } else if (rVariable == SHAPE_SHELL_STRAIN_GLOBAL) {
        ijob = 1;
        bGlobal = true;
    } else if (rVariable == SHAPE_SHELL_CURVATURE) {
        ijob = 2;
    } else if (rVariable == SHAPE_SHELL_CURVATURE_GLOBAL) {
        ijob = 2;
        bGlobal = true;
    } else if (rVariable == SHAPE_SHELL_FORCE) {
        ijob = 3;
    } else if (rVariable == SHAPE_SHELL_FORCE_GLOBAL) {
        ijob = 3;
        bGlobal = true;
    } else if (rVariable == SHAPE_SHELL_MOMENT) {
        ijob = 4;
    } else if (rVariable == SHAPE_SHELL_MOMENT_GLOBAL) {
        ijob = 4;
        bGlobal = true;
    } else if (rVariable == SHAPE_SHELL_STRESS_TOP_SURFACE) {
        ijob = 5;
    } else if (rVariable == SHAPE_SHELL_STRESS_TOP_SURFACE_GLOBAL) {
        ijob = 5;
        bGlobal = true;
    } else if (rVariable == SHAPE_SHELL_STRESS_MIDDLE_SURFACE) {
        ijob = 6;
    } else if (rVariable == SHAPE_SHELL_STRESS_MIDDLE_SURFACE_GLOBAL) {
        ijob = 6;
        bGlobal = true;
    } else if (rVariable == SHAPE_SHELL_STRESS_BOTTOM_SURFACE) {
        ijob = 7;
    } else if (rVariable == SHAPE_SHELL_STRESS_BOTTOM_SURFACE_GLOBAL) {
        ijob = 7;
        bGlobal = true;
    } else if (rVariable == SHAPE_SHELL_ORTHOTROPIC_STRESS_BOTTOM_SURFACE) {
        ijob = 8;
    } else if (rVariable == SHAPE_SHELL_ORTHOTROPIC_STRESS_BOTTOM_SURFACE_GLOBAL) {
        ijob = 8;
        bGlobal = true;
    } else if (rVariable == SHAPE_SHELL_ORTHOTROPIC_STRESS_TOP_SURFACE) {
        ijob = 9;
    } else if (rVariable == SHAPE_SHELL_ORTHOTROPIC_STRESS_TOP_SURFACE_GLOBAL) {
        ijob = 9;
        bGlobal = true;
    }
}
...
template <ShapeShellKinematics TKinematics>
double ShapeShellThickElement3D4N<TKinematics>::CalculateStenbergShearStabilization(const ShapeShellQ4_LocalCoordinateSystem& referenceCoordinateSystem, const double& hMean)
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::CalculateBMatrix(double xi, double eta,
        const ShapeShellUtilities::JacobianOperator& Jac, const MITC4Params& mitc_params,
        const Vector& N,
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::CalculateAll(MatrixType& rLeftHandSideMatrix,
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::AddBodyForces(const array_1d<double,4>& dA, VectorType& rRightHandSideVector)
{
    const GeometryType& geom = GetGeometry();
...
template <ShapeShellKinematics TKinematics>
bool ShapeShellThickElement3D4N<TKinematics>::TryCalculateOnIntegrationPoints_GeneralizedStrainsOrStresses(const Variable<Matrix>& rVariable,
        std::vector<Matrix>& rValues,
        const ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int i = 0; i < size; i++) {
        ...
        if (ijob == 1) { // strains
            ...
            iValue(0, 2) = iValue(2, 0) = 0.0;
            iValue(1, 2) = iValue(2, 1) = 0.0;
        } else if (ijob == 5) { // SHAPE_SHELL_STRESS_TOP_SURFACE
            iValue(0, 0) = generalizedStresses(0) +
            ...
            iValue(0, 2) = iValue(2, 0) = 0.0;
            iValue(1, 2) = iValue(2, 1) = 0.0;
        } else if (ijob == 6) { // SHAPE_SHELL_STRESS_MIDDLE_SURFACE
            iValue(0, 0) = generalizedStresses(0);
            iValue(1, 1) = generalizedStresses(1);
            iValue(2, 2) = 0.0;
            iValue(0, 1) = iValue(1, 0) = generalizedStresses[2];
        ...
        } else if (ijob == 7) { // SHAPE_SHELL_STRESS_BOTTOM_SURFACE
            iValue(0, 0) = generalizedStresses(0) -
                           generalizedStresses(3);
            iValue(1, 1) = generalizedStresses(1) -
                           generalizedStresses(4);
            ...
            iValue(0, 2) = iValue(2, 0) = 0.0;
            iValue(1, 2) = iValue(2, 1) = 0.0;
        } else if (ijob == 8) { // SHAPE_SHELL_ORTHOTROPIC_STRESS_BOTTOM_SURFACE
            iValue(0, 0) =
        ...
        } else if (ijob == 9) { // SHAPE_SHELL_ORTHOTROPIC_STRESS_TOP_SURFACE
...
template <ShapeShellKinematics TKinematics>
ShapeShellCrossSection::SectionBehaviorType ShapeShellThickElement3D4N<TKinematics>::GetSectionBehavior() const
{
    return ShapeShellCrossSection::Thick;
}


// =====================================================================================
//
// Class ShapeShellThickElement3D4N - Serialization
//
// =====================================================================================

template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::save(Serializer& rSerializer) const
{
...
template <ShapeShellKinematics TKinematics>
void ShapeShellThickElement3D4N<TKinematics>::load(Serializer& rSerializer)
{
    KRATOS_SERIALIZE_LOAD_BASE_CLASS(rSerializer, BaseType);
    rSerializer.load("EAS", mEASStorage);
}

template class ShapeShellThickElement3D4N<ShapeShellKinematics::LINEAR>;
template class ShapeShellThickElement3D4N<ShapeShellKinematics::NONLINEAR_COROTATIONAL>;
