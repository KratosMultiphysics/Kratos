...
#include "custom_utilities/shape_structural_mechanics_element_utilities.h"
...
BaseShapeShellElement<TCoordinateTransformation>::BaseShapeShellElement(IndexType NewId,
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::EquationIdVector(EquationIdVectorType& rResult,
                                        const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    for (IndexType i = 0; i < r_geom.size(); ++i) {
        ...
        rResult[index]     = i_node.GetDof(HELMHOLTZ_VARS_X).EquationId();
        rResult[index + 1] = i_node.GetDof(HELMHOLTZ_VARS_Y).EquationId();
        rResult[index + 2] = i_node.GetDof(HELMHOLTZ_VARS_Z).EquationId();

        rResult[index + 3] = i_node.GetDof(HELMHOLTZ_ROTS_X).EquationId();
        rResult[index + 4] = i_node.GetDof(HELMHOLTZ_ROTS_Y).EquationId();
        rResult[index + 5] = i_node.GetDof(HELMHOLTZ_ROTS_Z).EquationId();
    }
}

template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::GetDofList(DofsVectorType& rElementalDofList,
                                  const ProcessInfo& rCurrentProcessInfo) const
{
    const SizeType num_dofs = GetNumberOfDofs();
    ...
    for (IndexType i = 0; i < r_geom.size(); ++i) {
        ...
        rElementalDofList.push_back(i_node.pGetDof(HELMHOLTZ_VARS_X));
        rElementalDofList.push_back(i_node.pGetDof(HELMHOLTZ_VARS_Y));
        rElementalDofList.push_back(i_node.pGetDof(HELMHOLTZ_VARS_Z));

        rElementalDofList.push_back(i_node.pGetDof(HELMHOLTZ_ROTS_X));
        rElementalDofList.push_back(i_node.pGetDof(HELMHOLTZ_ROTS_Y));
        rElementalDofList.push_back(i_node.pGetDof(HELMHOLTZ_ROTS_Z));
    }
}

template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::GetValuesVector(Vector& rValues, int Step) const
{
    const SizeType num_dofs = GetNumberOfDofs();
    ...
    for (IndexType i = 0; i < r_geom.size(); ++i) {
        const NodeType& i_node = r_geom[i];
        const array_1d<double, 3>& disp = i_node.FastGetSolutionStepValue(HELMHOLTZ_VARS, Step);
        const array_1d<double, 3>& rot = i_node.FastGetSolutionStepValue(HELMHOLTZ_ROTS, Step);
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::GetFirstDerivativesVector(Vector& rValues, int Step) const
{
    const SizeType num_dofs = GetNumberOfDofs();
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::GetSecondDerivativesVector(Vector& rValues, int Step) const
{
    const SizeType num_dofs = GetNumberOfDofs();
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::ResetConstitutiveLaw()
{
    KRATOS_TRY
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::Initialize(const ProcessInfo& rCurrentProcessInfo)
{
    ...
    if (!rCurrentProcessInfo[IS_RESTARTED]) {
        ...
        if (mSections.size() != num_gps) {
            ...
            ShapeShellCrossSection::Pointer p_ref_section;

            if (ShapeShellUtilities::IsOrthotropic(r_props)) {
                // make new instance of shape_shell cross section
                p_ref_section = Kratos::make_shared<ShapeShellCrossSection>();
            ...
            for (SizeType i = 0; i < num_gps; ++i) {
                ShapeShellCrossSection::Pointer p_section_clone = p_ref_section->Clone();
        ...
        if (Has(SHAPE_LOCAL_MATERIAL_AXIS_1)) {
            ...
            const array_1d<double, 3> prescribed_direcition = GetValue(SHAPE_LOCAL_MATERIAL_AXIS_1);
            ...
            SetValue(SHAPE_MATERIAL_ORIENTATION_ANGLE, mat_orientation_angle);
        }
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::InitializeNonLinearIteration(const ProcessInfo& rCurrentProcessInfo)
{
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::FinalizeNonLinearIteration(const ProcessInfo& rCurrentProcessInfo)
{
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::InitializeSolutionStep(const ProcessInfo& rCurrentProcessInfo)
{
    const auto& r_props = GetProperties();
    const auto& r_geom = GetGeometry();
    const Matrix& r_shape_fct_values = r_geom.ShapeFunctionsValues(GetIntegrationMethod());
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::FinalizeSolutionStep(const ProcessInfo& rCurrentProcessInfo)
{
    const auto& r_props = GetProperties();
    const auto& r_geom = GetGeometry();
    const Matrix& r_shape_fct_values = r_geom.ShapeFunctionsValues(GetIntegrationMethod());
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix,
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::CalculateLeftHandSide(MatrixType& rLeftHandSideMatrix,
        const ProcessInfo& rCurrentProcessInfo)
{
    // Calculation flags
    const bool calculate_stiffness_matrix_flag = true;
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::CalculateRightHandSide(VectorType& rRightHandSideVector,
        const ProcessInfo& rCurrentProcessInfo)
{
    // Calculation flags
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::CalculateMassMatrix(MatrixType& rMassMatrix, const ProcessInfo& rCurrentProcessInfo)
{
    // const bool compute_lumped_mass_matrix = ShapeStructuralMechanicsElementUtilities::ComputeLumpedMassMatrix(GetProperties(), rCurrentProcessInfo);
    const bool compute_lumped_mass_matrix = false;
    ...
    if (compute_lumped_mass_matrix) {
        ...
        if (num_nodes == 3) { // triangular element
            ...
            ShapeShellUtilities::JacobianOperator jac_operator;
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::CalculateDampingMatrix(
...
)
{
    ...
    ShapeStructuralMechanicsElementUtilities::CalculateRayleighDampingMatrix(
        *this,
        rDampingMatrix,
        rCurrentProcessInfo,
        matrix_size);
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::CalculateOnIntegrationPoints(
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    } else if (rVariable == SHAPE_LOCAL_MATERIAL_AXIS_1 ||
               rVariable == SHAPE_LOCAL_MATERIAL_AXIS_2 ||
               rVariable == SHAPE_LOCAL_MATERIAL_AXIS_3) {
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::Calculate(
    const Variable<Matrix>& rVariable, Matrix& Output,
    const ProcessInfo& rCurrentProcessInfo)
{
    if (rVariable == SHAPE_LOCAL_ELEMENT_ORIENTATION) {
    ...
    if (rVariable == HELMHOLTZ_MASS_MATRIX){
        CalculateMassMatrix(Output,rCurrentProcessInfo);
    }
}

template <class TCoordinateTransformation>
int BaseShapeShellElement<TCoordinateTransformation>::Check(const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY;
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::SetCrossSectionsOnIntegrationPoints(std::vector< ShapeShellCrossSection::Pointer >& crossSections)
{
    KRATOS_TRY
...
template <class TCoordinateTransformation>
const Parameters BaseShapeShellElement<TCoordinateTransformation>::GetSpecifications() const
{
    const Parameters specifications = Parameters(R"({
        ...
        "output"                     : {
            "gauss_point"            : [],
            "nodal_historical"       : ["HELMHOLTZ_VARS","HELMHOLTZ_ROTS","VELOCITY","ACCELERATION"],
            "nodal_non_historical"   : [],
            "entity"                 : []
        },
        "required_variables"         : ["HELMHOLTZ_VARS","HELMHOLTZ_ROTS"],
        "required_dofs"              : ["HELMHOLTZ_VARS_X","HELMHOLTZ_VARS_Y","HELMHOLTZ_VARS_Z","HELMHOLTZ_ROTS_X","HELMHOLTZ_ROTS_Y","HELMHOLTZ_ROTS_Z"],
        "flags_used"                 : [],
        ...
        "documentation"   : "Base element for thin/thick shape_shell formulations."
    })");
...
template <class TCoordinateTransformation>
std::string BaseShapeShellElement<TCoordinateTransformation>::Info() const
{
    std::stringstream buffer;
    buffer << "BaseShapeShellElement #" << Id();
    return buffer.str();
}

template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::PrintInfo(std::ostream& rOStream) const
{
    rOStream << "BaseShapeShellElement #" << Id();
}

template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::PrintData(std::ostream& rOStream) const
{
    pGetGeometry()->PrintData(rOStream);
}

template <class TCoordinateTransformation>
SizeType BaseShapeShellElement<TCoordinateTransformation>::GetNumberOfDofs() const
{
    return (6 * GetGeometry().PointsNumber());   // 6 dofs per node
}

template <class TCoordinateTransformation>
SizeType BaseShapeShellElement<TCoordinateTransformation>::GetNumberOfGPs() const
{
    return GetGeometry().IntegrationPoints(mIntegrationMethod).size();
}

template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::CalculateAll(
...
{
    KRATOS_ERROR << "You have called to the CalculateAll from the base class for shape_shell elements" << std::endl;
}

template <class TCoordinateTransformation>
ShapeShellCrossSection::SectionBehaviorType BaseShapeShellElement<TCoordinateTransformation>::GetSectionBehavior() const
{
    KRATOS_ERROR << "You have called to the GetSectionBehavior from the base class for shape_shell elements" << std::endl;
}

template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::SetupOrientationAngles()
{
    if (Has(SHAPE_MATERIAL_ORIENTATION_ANGLE)) {
        for (auto it = mSections.begin(); it != mSections.end(); ++it) {
            (*it)->SetOrientationAngle(GetValue(SHAPE_MATERIAL_ORIENTATION_ANGLE));
        }
    } else {
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::ComputeLocalAxis(
    const Variable<array_1d<double, 3> >& rVariable,
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::ComputeLocalMaterialAxis(
    const Variable<array_1d<double, 3> >& rVariable,
    std::vector<array_1d<double, 3> >& rOutput) const
{
    const double mat_angle = GetValue(SHAPE_MATERIAL_ORIENTATION_ANGLE);
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::CheckDofs() const
{
    // verify that the dofs exist
    for (const auto& r_node : GetGeometry().Points()) {
        KRATOS_CHECK_VARIABLE_IN_NODAL_DATA(HELMHOLTZ_VARS, r_node);
        KRATOS_CHECK_VARIABLE_IN_NODAL_DATA(HELMHOLTZ_ROTS, r_node);

        KRATOS_CHECK_DOF_IN_NODE(HELMHOLTZ_ROTS_X, r_node);
        KRATOS_CHECK_DOF_IN_NODE(HELMHOLTZ_ROTS_Y, r_node);
        KRATOS_CHECK_DOF_IN_NODE(HELMHOLTZ_ROTS_Z, r_node);
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::CheckProperties(const ProcessInfo& rCurrentProcessInfo) const
{
    // check properties
    ...
    if (r_props.Has(SHAPE_SHELL_ORTHOTROPIC_LAYERS)) {
        CheckSpecificProperties();

        const auto& r_props = GetProperties();

        KRATOS_ERROR_IF(r_props.Has(THICKNESS)) << "Specifying THICKNESS conflicts with the "
                                                << "definition of SHAPE_SHELL_ORTHOTROPIC_LAYERS (where the thickness is also specified)"
                                                << std::endl;

        KRATOS_ERROR_IF(r_props.Has(DENSITY)) << "Specifying DENSITY conflicts with the "
                                              << "definition of SHAPE_SHELL_ORTHOTROPIC_LAYERS (where the density is also specified)"
                                              << std::endl;

        KRATOS_ERROR_IF(r_props.Has(YOUNG_MODULUS)) << "Specifying YOUNG_MODULUS conflicts with the "
                << "definition of SHAPE_SHELL_ORTHOTROPIC_LAYERS (where the youngs-modulus is also specified)"
                << std::endl;

        KRATOS_ERROR_IF(r_props.Has(POISSON_RATIO)) << "Specifying POISSON_RATIO conflicts with the "
                << "definition of SHAPE_SHELL_ORTHOTROPIC_LAYERS (where the poisson-ratio is also specified)"
                << std::endl;

        // perform detailed orthotropic check later in shape_shell_cross_section
        ...
        ShapeShellCrossSection::Pointer dummySection = ShapeShellCrossSection::Pointer(new ShapeShellCrossSection());
        dummySection->BeginStack();
        dummySection->AddPly(0, 5, GetProperties());
        dummySection->EndStack();
        dummySection->SetSectionBehavior(ShapeShellCrossSection::Thick);
        dummySection->Check(r_props, r_geom, rCurrentProcessInfo);
    }
}

template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::CheckSpecificProperties() const
{
    const auto& r_props = GetProperties();
    ...
    //                 !Has(SHAPE_MATERIAL_ORIENTATION_ANGLE))
    //     << "Using an Anisotropic Constitutive law requires the specification of "
    //     << "\"SHAPE_MATERIAL_ORIENTATION_ANGLE\" for shape_shell element with Id " << Id() << std::endl;

    if (GetSectionBehavior() == ShapeShellCrossSection::Thick) {
        // Check constitutive law has been verified with Stenberg stabilization
        // applicable for 5-parameter shape_shells only.
        bool stenberg_stabilization_suitable = false;
        claw->GetValue(SHAPE_STENBERG_SHEAR_STABILIZATION_SUITABLE, stenberg_stabilization_suitable);
        KRATOS_WARNING_IF("BaseShapeShellElement", !stenberg_stabilization_suitable)
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::DecimalCorrection(Vector& a)
{
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::save(Serializer& rSerializer) const
{
...
template <class TCoordinateTransformation>
void BaseShapeShellElement<TCoordinateTransformation>::load(Serializer& rSerializer)
{
...
template class BaseShapeShellElement< ShapeShellT3_CoordinateTransformation >;
template class BaseShapeShellElement< ShapeShellT3_CorotationalCoordinateTransformation  >;
template class BaseShapeShellElement< ShapeShellQ4_CoordinateTransformation >;
template class BaseShapeShellElement< ShapeShellQ4_CorotationalCoordinateTransformation  >;
