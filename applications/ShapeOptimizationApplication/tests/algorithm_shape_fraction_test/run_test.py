...
with open(os.path.join(output_directory, optimization_log_filename), 'r') as csvfile:
    ...
    penalty_factor = float(last_line[5].strip())
    f_p = float(last_line[6].strip())
    shape_frac = float(last_line[8].strip())
    c1 = float(last_line[9].strip())
    c2 = float(last_line[11].strip())
    c3 = float(last_line[13].strip())

    # Check against specifications
    TestCase().assertEqual(n_iter, 12)
    TestCase().assertAlmostEqual(f, 2.96994E+02, 4)
    TestCase().assertAlmostEqual(penalty_factor, 4.58211E+02, 4)
    TestCase().assertAlmostEqual(f_p, 1.79846E+03, 4)
    TestCase().assertAlmostEqual(shape_frac, 6.09756E-01, 4)
    TestCase().assertAlmostEqual(c1, -1.00002E+00, 4)
    TestCase().assertAlmostEqual(c2, 1.00100E+00, 4)
    TestCase().assertAlmostEqual(c3, 1.00000E+00, 4)
