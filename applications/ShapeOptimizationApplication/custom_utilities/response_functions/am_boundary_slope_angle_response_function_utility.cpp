...
AMBoundarySlopeAngleResponseFunctionUtility::AMBoundarySlopeAngleResponseFunctionUtility(ModelPart& rModelPart, Parameters ResponseSettings)
	: mrModelPart(rModelPart)
{
	const std::size_t domain_size = mrModelPart.GetProcessInfo()[DOMAIN_SIZE];
	KRATOS_ERROR_IF(domain_size != 3) << "AMBoundarySlopeAngleResponseFunctionUtility can only be used on 3D geometries!" << std::endl;

	mMainDirection = ResponseSettings["main_direction"].GetVector();
	const double direction_norm = norm_2(mMainDirection);
	KRATOS_ERROR_IF(direction_norm < std::numeric_limits<double>::epsilon()) << "AMBoundarySlopeAngleResponseFunctionUtility: 'main_direction' vector norm is 0!" << std::endl;
	mMainDirection /= direction_norm;

	const double min_angle = ResponseSettings["min_angle"].GetDouble();
	mCosMinAngle = std::cos(min_angle * Globals::Pi / 180);
...
double AMBoundarySlopeAngleResponseFunctionUtility::CalculateValue()
{
	KRATOS_TRY;
...
void AMBoundarySlopeAngleResponseFunctionUtility::CalculateGradient()
{
	KRATOS_TRY;
	...
	for (auto& cond_i : mrModelPart.Conditions()){
		...
		for (auto& node_i : cond_i.GetGeometry()){
			...
			node_i.FastGetSolutionStepValue(NODAL_MIN_ANGLE) = g_i;
		}
	}
...
double AMBoundarySlopeAngleResponseFunctionUtility::CalculateConditionValue(const Condition& rFace)
{
	// face normal
	const array_3d local_coords = ZeroVector(3);
	const array_3d face_normal = rFace.GetGeometry().UnitNormal(local_coords);

	double delta_angle = -inner_prod(mMainDirection, face_normal) - mCosMinAngle;
