...
void MapperCenterline::Initialize()
{
    BuiltinTimer timer;
    KRATOS_INFO("ShapeOpt") << "Starting initialization of centerline mapper..." << std::endl;
...
void MapperCenterline::Map( const Variable<array_3d> &rOriginVariable, const Variable<array_3d> &rDestinationVariable )
{
...
void MapperCenterline::Map( const Variable<double> &rOriginVariable, const Variable<double> &rDestinationVariable )
{
    if (mIsMappingInitialized == false)
        Initialize();
...
void MapperCenterline::InverseMap( const Variable<array_3d> &rDestinationVariable, const Variable<array_3d> &rOriginVariable )
{
    if (mIsMappingInitialized == false)
        Initialize();
    ...
    std::cout << "rOriginModelPart:\n" << mrOriginModelPart.Nodes() << std::endl;
...
void MapperCenterline::InverseMap( const Variable<double> &rDestinationVariable, const Variable<double> &rOriginVariable )
{
    if (mIsMappingInitialized == false)
        Initialize();
...
void MapperCenterline::Update(LinearSolver<SparseSpaceType, LocalSpaceType>& rSolver, ModelPart& rOriginModelPart)
{
    if (mIsMappingInitialized == false)
    ...
    mpHeatMethod -> ComputeHeatField(rSolver, rOriginModelPart);
    //new function: create centerline
...
void MapperCenterline::CreateListOfNodesInOriginModelPart()
{
...
void MapperCenterline::CreateFilterFunction()
{
...
void MapperCenterline::InitializeMappingVariables()
{
...
void MapperCenterline::AssignMappingIds()
{
    unsigned int i = 0;
...
void MapperCenterline::CreateSearchTreeWithAllNodesInOriginModelPart()
{
    BuiltinTimer timer;
...
void MapperCenterline::ThrowWarningIfNumberOfNeighborsExceedsLimit(ModelPart::NodeType& given_node, unsigned int number_of_neighbors)
{
    if(number_of_neighbors >= mMaxNumberOfNeighbors)
        KRATOS_WARNING("ShapeOpt::MapperCenterline") << "For node " << given_node.Id() << " and specified filter radius, maximum number of neighbor nodes (=" << mMaxNumberOfNeighbors << " nodes) reached!" << std::endl;
}

void MapperCenterline::ComputeWeightForAllNeighbors(  const ModelPart::NodeType& design_node,
