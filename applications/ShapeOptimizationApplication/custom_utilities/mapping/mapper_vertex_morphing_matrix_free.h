...
public:
    ...
    {
        BuiltinTimer timer;
        KRATOS_INFO("ShapeOptimization") << "Starting initialization of matrix-free mapper..." << std::endl;
        ...
        KRATOS_INFO("ShapeOptimization") << "Finished initialization of matrix-free mapper in " << timer.ElapsedSeconds() << " s." << std::endl;
    }
    ...
    void Map( const Variable<array_3d> &rOriginVariable, const Variable<array_3d> &rDestinationVariable ) override
    {
        ...
        KRATOS_INFO("ShapeOptimization") << "Finished mapping in " << mapping_time.ElapsedSeconds() << " s." << std::endl;
    }
...
private:
    ...
    {
        BuiltinTimer timer;
        KRATOS_INFO("ShapeOptimization") << "Creating search tree to perform mapping..." << std::endl;
    ...
    void ThrowWarningIfNumberOfNeighborsExceedsLimit(ModelPart::NodeType& given_node, unsigned int number_of_neighbors)
    {
        if(number_of_neighbors >= mMaxNumberOfNeighbors)
            KRATOS_WARNING("ShapeOptimization") << "For node " << given_node.Id() << " and specified filter radius, maximum number of neighbor nodes (=" << mMaxNumberOfNeighbors << " nodes) reached!" << std::endl;
    }
