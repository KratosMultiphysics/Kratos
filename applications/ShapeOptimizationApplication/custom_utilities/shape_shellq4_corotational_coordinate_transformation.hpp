...
#if !defined(KRATOS_SHAPE_SHELLQ4_COROTATIONAL_COORDINATE_TRANSFORMATION_H_INCLUDED )
#define  KRATOS_SHAPE_SHELLQ4_COROTATIONAL_COORDINATE_TRANSFORMATION_H_INCLUDED

#include "shape_EICR.hpp"
#include "shape_shellq4_coordinate_transformation.hpp"

namespace Kratos
{
/** \brief SHAPE_EICR ShapeShellQ4_CorotationalCoordinateTransformation
*
...
* - C.A.Felippa, AFEM.Ch.38, "Quadrilateral ShapeShell Elements",
*   Chapter 5 of B.Haugen's Thesis.
*   link: http://www.colorado.edu/engineering/CAS/courses.d/AFEM.d/
*/
class ShapeShellQ4_CorotationalCoordinateTransformation : public ShapeShellQ4_CoordinateTransformation
{

public:

    KRATOS_CLASS_POINTER_DEFINITION(ShapeShellQ4_CorotationalCoordinateTransformation);
...
public:
    ...
    ShapeShellQ4_CorotationalCoordinateTransformation(const GeometryType::Pointer& pGeometry)
        : ShapeShellQ4_CoordinateTransformation(pGeometry)
        , mInitialized(false)
    {
    }

    ~ShapeShellQ4_CorotationalCoordinateTransformation() override
    {
    }

public:

    ShapeShellQ4_CoordinateTransformation::Pointer Create(GeometryType::Pointer pGeometry)const override
    {
        return ShapeShellQ4_CorotationalCoordinateTransformation::Pointer(new ShapeShellQ4_CorotationalCoordinateTransformation(pGeometry));
    }
    ...
    void Initialize() override
    {
        ...
        if (!mInitialized) {
            ...
            ShapeShellQ4_LocalCoordinateSystem LCS(CreateReferenceCoordinateSystem());
    ...
    ShapeShellQ4_LocalCoordinateSystem CreateLocalCoordinateSystem()const override
    {
        const GeometryType& geom = GetGeometry();

        //return ShapeShellQ4_LocalCoordinateSystem(geom[0], geom[1], geom[2], geom[3]);

        // reference coordinate system
        ShapeShellQ4_LocalCoordinateSystem a(CreateReferenceCoordinateSystem());

        // current coordinate system using the 1-2 side alignement
        ShapeShellQ4_LocalCoordinateSystem b(geom[0], geom[1], geom[2], geom[3]);
        ...
        return ShapeShellQ4_LocalCoordinateSystem(geom[0], geom[1], geom[2], geom[3], alpha);
    }

    VectorType CalculateLocalDisplacements(const ShapeShellQ4_LocalCoordinateSystem& LCS,
    ...
    void FinalizeCalculations(const ShapeShellQ4_LocalCoordinateSystem& LCS,
                              ...
                              const bool LHSrequired) override
    {
        ...
        MatrixType P(SHAPE_EICR::Compute_Pt(4));
        ...
        SHAPE_EICR::Spin_AtRow(projectedLocalForces, Fnm, 18);
        ...
        SHAPE_EICR::Spin_AtRow(projectedLocalForces, Fnm, 21);
    ...
    MatrixType GetNodalDeformationalRotationTensor(const ShapeShellQ4_LocalCoordinateSystem& LCS,
...
private:
    ...
    */
    inline MatrixType RotationGradient()
    {
        ...
        ShapeShellQ4_LocalCoordinateSystem a(CreateReferenceCoordinateSystem());   // current coordinate system

        ShapeShellQ4_LocalCoordinateSystem::Vector3ContainerType nodes = a.Nodes();
        ...
        for (int i = 0; i < 4; i++) { // for each node
            ...
            for (int j = 0; j < 3; j++) { // for each component in [x, y, z]
                ...
                ShapeShellQ4_LocalCoordinateSystem b(nodes[0], nodes[1], nodes[2], nodes[3]);   // perturbed coordinate system (1-2 side alignement)
                ...
                ShapeShellQ4_LocalCoordinateSystem c(nodes[0], nodes[1], nodes[2], nodes[3], alpha);   // perturbed coordinate system (polar decomposition)
...
private:
    ...
    {
        KRATOS_SERIALIZE_SAVE_BASE_CLASS(rSerializer,  ShapeShellQ4_CoordinateTransformation);
    ...
    {
        KRATOS_SERIALIZE_LOAD_BASE_CLASS(rSerializer,  ShapeShellQ4_CoordinateTransformation);
...
#endif // KRATOS_SHAPE_SHELLQ4_COROTATIONAL_COORDINATE_TRANSFORMATION_H_INCLUDED
