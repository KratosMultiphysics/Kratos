...
#include "shape_shell_cross_section.hpp"
#include "shape_optimization_application_variables.h"

namespace Kratos {

ShapeShellCrossSection::ShapeShellCrossSection()
    : mStack()
    , mEditingStack(false)
...
ShapeShellCrossSection::ShapeShellCrossSection(const ShapeShellCrossSection& other)
{
    PrivateCopy(other);
}
...
ShapeShellCrossSection& ShapeShellCrossSection::operator = (const ShapeShellCrossSection& other)
{
    PrivateCopy(other);
    return *this;
}

void ShapeShellCrossSection::BeginStack()
{
    if (!mEditingStack) {
        mEditingStack = true;
        mStack.clear();
...
void ShapeShellCrossSection::AddPly(const IndexType PlyIndex, int numPoints, const Properties& rProps)
{
    if (mEditingStack) {
...
void ShapeShellCrossSection::EndStack()
{
    if (mEditingStack) {
        mEditingStack = false;
    }
...
std::string ShapeShellCrossSection::GetInfo(const Properties& rProps)
{
    std::stringstream ss;
    ss << std::fixed;
...
ShapeShellCrossSection::Pointer ShapeShellCrossSection::Clone()const
{
    ShapeShellCrossSection::Pointer theClone(new ShapeShellCrossSection(*this));
    theClone->EndStack();
    return theClone;
}

bool ShapeShellCrossSection::Has(const Variable<double>& rThisVariable)
{
    return false;
}

bool ShapeShellCrossSection::Has(const Variable<Vector>& rThisVariable)
{
    return false;
}

bool ShapeShellCrossSection::Has(const Variable<Matrix>& rThisVariable)
{
    return false;
}

bool ShapeShellCrossSection::Has(const Variable<array_1d<double, 3 > >& rThisVariable)
{
    return false;
}

bool ShapeShellCrossSection::Has(const Variable<array_1d<double, 6 > >& rThisVariable)
{
    return false;
}

double& ShapeShellCrossSection::GetValue(const Variable<double>& rThisVariable, const Properties& rProps, double& rValue)
{
...
Vector& ShapeShellCrossSection::GetValue(const Variable<Vector>& rThisVariable, Vector& rValue)
{
    return rValue;
}

Matrix& ShapeShellCrossSection::GetValue(const Variable<Matrix>& rThisVariable, Matrix& rValue)
{
    return rValue;
}

array_1d<double, 3 >& ShapeShellCrossSection::GetValue(const Variable<array_1d<double, 3 > >& rVariable,
        array_1d<double, 3 >& rValue)
{
    return rValue;
}

array_1d<double, 6 >& ShapeShellCrossSection::GetValue(const Variable<array_1d<double, 6 > >& rVariable,
        array_1d<double, 6 >& rValue)
{
    return rValue;
}

void ShapeShellCrossSection::SetValue(const Variable<double>& rVariable,
                                 const double& rValue,
                                 const ProcessInfo& rCurrentProcessInfo)
{
}

void ShapeShellCrossSection::SetValue(const Variable<Vector >& rVariable,
                                 const Vector& rValue,
                                 const ProcessInfo& rCurrentProcessInfo)
{
}

void ShapeShellCrossSection::SetValue(const Variable<Matrix >& rVariable,
                                 const Matrix& rValue,
                                 const ProcessInfo& rCurrentProcessInfo)
{
}

void ShapeShellCrossSection::SetValue(const Variable<array_1d<double, 3 > >& rVariable,
                                 const array_1d<double, 3 >& rValue,
                                 const ProcessInfo& rCurrentProcessInfo)
{
}

void ShapeShellCrossSection::SetValue(const Variable<array_1d<double, 6 > >& rVariable,
                                 const array_1d<double, 6 >& rValue,
                                 const ProcessInfo& rCurrentProcessInfo)
{
}

bool ShapeShellCrossSection::ValidateInput(const Properties& rMaterialProperties)
{
    return true;
}

void ShapeShellCrossSection::InitializeCrossSection(const Properties& rMaterialProperties,
...
void ShapeShellCrossSection::InitializeSolutionStep(const Properties& rMaterialProperties,
        const GeometryType& rElementGeometry,
        const Vector& rShapeFunctionsValues,
        const ProcessInfo& rCurrentProcessInfo)
{
...
void ShapeShellCrossSection::FinalizeSolutionStep(const Properties& rMaterialProperties,
        const GeometryType& rElementGeometry,
        const Vector& rShapeFunctionsValues,
        const ProcessInfo& rCurrentProcessInfo)
{
...
void ShapeShellCrossSection::InitializeNonLinearIteration(const Properties& rMaterialProperties,
        const GeometryType& rElementGeometry,
        const Vector& rShapeFunctionsValues,
        const ProcessInfo& rCurrentProcessInfo)
{
...
void ShapeShellCrossSection::FinalizeNonLinearIteration(const Properties& rMaterialProperties,
        const GeometryType& rElementGeometry,
        const Vector& rShapeFunctionsValues,
        const ProcessInfo& rCurrentProcessInfo)
{
...
void ShapeShellCrossSection::CalculateSectionResponse(SectionParameters& rValues, const ConstitutiveLaw::StressMeasure& rStressMeasure)
{
...
//         Hinv = ZeroMatrix(condensed_strain_size, condensed_strain_size);
    ...
    while (true) {
        ...
        for (auto& r_ply : mStack) {
            ...
            if (ShapeShellUtilities::IsOrthotropic(r_props)) {
...
void ShapeShellCrossSection::FinalizeSectionResponse(SectionParameters& rValues, const ConstitutiveLaw::StressMeasure& rStressMeasure)
{
    ConstitutiveLaw::Parameters materialValues;
    GeneralVariables variables;
    InitializeParameters(rValues, materialValues, variables);
...
void ShapeShellCrossSection::ResetCrossSection(const Properties& rMaterialProperties,
        const GeometryType& rElementGeometry,
        const Vector& rShapeFunctionsValues)
{
    mInitialized = false;
...
int ShapeShellCrossSection::Check(const Properties& rMaterialProperties,
                             const GeometryType& rElementGeometry,
                             const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    KRATOS_ERROR_IF(this->mEditingStack)
            << "The Ply Stack of a ShapeShellCrossSection is in Editing mode" << std::endl;

    KRATOS_ERROR_IF(this->mStack.size() < 1)
            << "The Ply Stack of a ShapeShellCrossSection cannot be empty" << std::endl;

    KRATOS_ERROR_IF(this->GetThickness(rMaterialProperties) <= 0.0)
            << "The Thickness of a ShapeShellCrossSection should be a "
    ...
    for (auto& r_ply : mStack) {
        ...
        for (const auto& r_int_point : r_ply.GetIntegrationPoints(rMaterialProperties)) {
            KRATOS_ERROR_IF(r_int_point.GetWeight() <= 0.0)
                    << "The Weight of a ShapeShellCrossSection.IntegrationPoint "
            ...
            KRATOS_ERROR_IF(iPointLaw == nullptr)
                    << "The Constitutive law of a ShapeShellCrossSection.IntegrationPoint "
                    << "is nullptr" << std::endl;
            ...
            KRATOS_ERROR_IF(strain_size != 3 && strain_size != 6)
                    << "The Constitutive law of a ShapeShellCrossSection.IntegrationPoint needs a "
            ...
            //		"The Constitutive law of a ShapeShellCrossSection.IntegrationPoint is incompatible with the strain measure required by this cross section ",
...
void ShapeShellCrossSection::ParseOrthotropicPropertyMatrix(const Properties& rProps)
{
    // ascertain how many plies there are and begin stacking them
    const SizeType num_plies = rProps.GetValue(SHAPE_SHELL_ORTHOTROPIC_LAYERS).size1();
    this->BeginStack();
...
void ShapeShellCrossSection::GetLaminaeOrientation(const Properties& rProps, Vector& rOrientation_Vector)
{
...
void ShapeShellCrossSection::GetLaminaeStrengths(std::vector<Matrix>& rLaminae_Strengths, const Properties& rProps)
{
    // ascertain how many plies there are
    SizeType plies = (rProps)[SHAPE_SHELL_ORTHOTROPIC_LAYERS].size1();

    // figure out the format of material properties based on it's width
    int my_format = (rProps)[SHAPE_SHELL_ORTHOTROPIC_LAYERS].size2();
    int offset = 0;
    if (my_format == 16) {
        offset = 9;
    } else {
    ...
    for (unsigned int currentPly = 0; currentPly < plies; currentPly++) {
        ...
        rLaminae_Strengths[currentPly](0, 0) =
            (rProps)[SHAPE_SHELL_ORTHOTROPIC_LAYERS](currentPly, offset);

        // C1
        rLaminae_Strengths[currentPly](0, 1) =
            (rProps)[SHAPE_SHELL_ORTHOTROPIC_LAYERS](currentPly, offset + 1);

        // T2
        rLaminae_Strengths[currentPly](0, 2) =
            (rProps)[SHAPE_SHELL_ORTHOTROPIC_LAYERS](currentPly, offset + 2);

        // C2
        rLaminae_Strengths[currentPly](1, 0) =
            (rProps)[SHAPE_SHELL_ORTHOTROPIC_LAYERS](currentPly, offset + 3);

        // S12
        rLaminae_Strengths[currentPly](1, 1) =
            (rProps)[SHAPE_SHELL_ORTHOTROPIC_LAYERS](currentPly, offset + 4);

        // S13
        rLaminae_Strengths[currentPly](1, 2) =
            (rProps)[SHAPE_SHELL_ORTHOTROPIC_LAYERS](currentPly, offset + 5);

        // S23
        rLaminae_Strengths[currentPly](2, 0) =
            (rProps)[SHAPE_SHELL_ORTHOTROPIC_LAYERS](currentPly, offset + 6);
...
void ShapeShellCrossSection::InitializeParameters(SectionParameters& rValues, ConstitutiveLaw::Parameters& rMaterialValues, GeneralVariables& rVariables)
{
...
void ShapeShellCrossSection::UpdateIntegrationPointParameters(const IntegrationPoint& rPoint, ConstitutiveLaw::Parameters& rMaterialValues, GeneralVariables& rVariables)
{
    if (rPoint.GetConstitutiveLaw()->GetStrainSize() == 3) {
        ...
        if (mBehavior == Thick) {
            ...
            if (props.Has(SHAPE_SHELL_ORTHOTROPIC_LAYERS)) {
                rVariables.GXZ = props[SHAPE_SHELL_ORTHOTROPIC_LAYERS](0,5);
                rVariables.GYZ = props[SHAPE_SHELL_ORTHOTROPIC_LAYERS](0,6);
...
void ShapeShellCrossSection::CalculateIntegrationPointResponse(const IntegrationPoint& rPoint,
...
void ShapeShellCrossSection::PrivateCopy(const ShapeShellCrossSection& other)
{
    if (this != &other) {
        mStack = other.mStack;
...
void ShapeShellCrossSection::Ply::RecoverOrthotropicProperties(const IndexType IdxCurrentPly, Properties& laminaProps)
{
    ...
    for (IndexType i=0; i<7; ++i) {
        current_ply_properties(0,i) = laminaProps[SHAPE_SHELL_ORTHOTROPIC_LAYERS](IdxCurrentPly, i+2);
    }

    laminaProps[SHAPE_SHELL_ORTHOTROPIC_LAYERS] = current_ply_properties;
}


std::vector<ConstitutiveLaw::Pointer> ShapeShellCrossSection::GetConstitutiveLawsVector(const Properties& rProps)
{
