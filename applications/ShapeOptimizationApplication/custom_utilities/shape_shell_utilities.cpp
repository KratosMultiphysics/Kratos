...
void JacobianOperator::Calculate(const ShapeShellQ4_LocalCoordinateSystem& CS, const Matrix& dN)
{
...
double dN_seren_dxi(const int nNode, const double Xi, const double Eta)
{
    ...
    default:
        KRATOS_ERROR <<
                     "Error: ELEMENT ShapeShellThinElement3D4N, METHOD dN_seren_dxi"
                     << std::endl;
    }
}
...
{
    return rProps.Has(SHAPE_SHELL_ORTHOTROPIC_LAYERS);
}
...
double GetThickness(const Properties& rProps)
{
    if (IsOrthotropic(rProps)) {
        ...
        const auto& orthotropic_layers = rProps.GetValue(SHAPE_SHELL_ORTHOTROPIC_LAYERS);
...
{
    if (rProps.Has(SHAPE_SHELL_OFFSET)) {
        return rProps.GetValue(SHAPE_SHELL_OFFSET);
    } else {
        return 0.0;
    }
}
