...
public:
    ...
    void ComputeSearchDirectionSteepestDescent()
    {
        ...
        KRATOS_INFO("\nShapeOptimization") << "No constraints given or active. The negative objective gradient is chosen as search direction..." << std::endl;
    ...
    void ComputeProjectedSearchDirection()
    {
        ...
        // Some output for information
        KRATOS_INFO("\nShapeOptimization") << "Constraint is active. Modified search direction on the constraint hyperplane is computed..." << std::endl;
    ...
    double ComputeCorrectionFactor()
    {
        ...
        if(mOptimizationSettings["optimization_algorithm"]["use_adaptive_correction"].GetBool())
        {
            ...
            if(mConstraintValue*mPreviousConstraintValue<0.0)
            {
                ...
                KRATOS_INFO("ShapeOptimization") << "Correction scaling needs to decrease...." << std::endl;
            }
            ...
            {
                KRATOS_INFO("ShapeOptimization") << "Correction scaling needs to increase...." << std::endl;
