...
public:
    ...
    {
        BuiltinTimer timer;
        KRATOS_INFO("\nShapeOpt") << "Creating search tree to perform damping..." << std::endl;
    ...
    {
        KRATOS_INFO("\nShapeOpt") << "Starting to prepare damping..." << std::endl;
    ...
    void ThrowWarningIfNodeNeighborsExceedLimit( ModelPart::NodeType& given_node, unsigned int number_of_neighbors )
    {
        if(number_of_neighbors >= mMaxNeighborNodes)
            KRATOS_WARNING("ShapeOpt") << "For node " << given_node.Id() << " and specified damping radius, maximum number of neighbor nodes (=" << mMaxNeighborNodes << " nodes) reached!" << std::endl;
    }
