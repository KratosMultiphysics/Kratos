...
#include "shape_structural_mechanics_element_utilities.h"
#include "shape_optimization_application_variables.h"
#include "utilities/math_utils.h"

namespace Kratos {
namespace ShapeStructuralMechanicsElementUtilities {
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    // The default for all elements in ShapeStructuralMechanics is
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    if (rProperties.Has(SHAPE_RAYLEIGH_ALPHA)) {
        return rProperties[SHAPE_RAYLEIGH_ALPHA];
    } else if (rCurrentProcessInfo.Has(SHAPE_RAYLEIGH_ALPHA)) {
        return rCurrentProcessInfo[SHAPE_RAYLEIGH_ALPHA];
    }
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    // Giving the locally defined setting (through Properties) priority
    // over the globally defined one (through ProcessInfo)
    if (rProperties.Has(SHAPE_RAYLEIGH_BETA)) {
        return rProperties[SHAPE_RAYLEIGH_BETA];
    } else if (rCurrentProcessInfo.Has(SHAPE_RAYLEIGH_BETA)) {
        return rCurrentProcessInfo[SHAPE_RAYLEIGH_BETA];
    }
...
double GetDensityForMassMatrixComputation(const Element& rElement)
{
    ...
    if (rElement.Has(SHAPE_MASS_FACTOR)) {
        return rElement.GetValue(SHAPE_MASS_FACTOR) * density;
    } else if (r_prop.Has(SHAPE_MASS_FACTOR)) {
        return r_prop.GetValue(SHAPE_MASS_FACTOR) * density;
    } else {
        return density;
    }
}
