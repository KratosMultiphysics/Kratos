...
def ApplyLoad(initial_value, model_part, time):
    for node in model_part.Nodes:
        ...
        node.SetSolutionStepValue(POINT_MOMENT_Z,0,value)

def ApplyDisp(initial_value, model_part, time):
    for node in model_part.Nodes:
        factor = 1
        value = initial_value * factor * time
        node.SetSolutionStepValue(DISPLACEMENT_Y,0,value)

def ApplyCentrifugalBCs( model_part ):
    ...
    for node in model_part.Nodes:
        ...
        radius_vector[2] = 0.0
        radius = radius_vector[0]*radius_vector[0] + radius_vector[1]*radius_vector[1]
...
while(time <= end_time):
    ...
    ApplyCentrifugalBCs( main_model_part )
