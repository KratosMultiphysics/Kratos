...
class AdjointVMSMonolithicSolver(AdjointFluidSolver):
    ...
    def Initialize(self):
        ...
        if self.settings["response_function_settings"]["response_type"].GetString() == "drag":
            if (domain_size == 2):
                self.response_function = KratosCFD.DragResponseFunction2D(self.settings["response_function_settings"], self.computing_model_part)
            elif (domain_size == 3):
                self.response_function = KratosCFD.DragResponseFunction3D(self.settings["response_function_settings"], self.computing_model_part)
            else:
