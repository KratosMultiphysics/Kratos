...
def Factory(settings, model):
    ...
    return ComputeBaseMomentProcess(model, settings["Parameters"])


class ComputeBaseMomentProcess(KratosMultiphysics.Process):
    """
        ...
        default_settings = KratosMultiphysics.Parameters("""
            {
                "model_part_name"           : "",
                "interval"                  : [0.0, 1e30],
                "print_moment_to_screen"      : false,
                "print_format"              : ".8f",
                "write_moment_output_file"    : true,
    ...
    def ExecuteInitialize(self):
        ...
        self.print_moment_to_screen = self.params["print_moment_to_screen"].GetBool()
        self.write_moment_output_file = self.params["write_moment_output_file"].GetBool()
        ...
        if (self.model_part.GetCommunicator().MyPID() == 0):
            if (self.write_moment_output_file):

                output_file_name = self.params["model_part_name"].GetString() + "_moment.dat"
                ...
                if file_handler_params.Has("file_name"):
                    ...
                    KratosMultiphysics.Logger.PrintWarning("ComputeBaseMomentProcess", warn_msg)
                else:
    ...
    def ExecuteFinalizeSolutionStep(self):
        ...
        if(((current_time >= self.interval[0]) and  (current_time < self.interval[1])) and (current_step + 1 >= self.model_part.GetBufferSize())):
            # Compute base moment
            base_moment = self._GetCorrespondingBaseMoment()

            # Write the moment values
            if (self.model_part.GetCommunicator().MyPID() == 0):
                if (self.print_moment_to_screen):
                    result_msg = str(current_time) + " Mx: " + format(base_moment[0],self.format) + " My: " + format(base_moment[1],self.format) + " Mz: " + format(base_moment[2],self.format)
                ...
                if (self.write_moment_output_file):
                    self.output_file.write(str(current_time)+" " + format(base_moment[0],self.format)+" "+format(base_moment[1],self.format)+" "+format(base_moment[2],self.format)+"\n")
    ...
    def _GetFileHeader(self):
        err_msg  = 'ComputeBaseMomentProcess: _GetFileHeader called in base class\n'
        err_msg += 'this needs to be implemented and called from derived class'
        raise Exception(err_msg)

    def _PrintToScreen(self,result_msg):
        err_msg  = 'ComputeBaseMomentProcess: _PrinToScreen called in base class\n'
        err_msg += 'this needs to be implemented and called from derived class'
        raise Exception(err_msg)

    def _GetCorrespondingBaseMoment(self):
        err_msg  = 'ComputeBaseMomentProcess: _GetCorrespondingBaseAndMoment called in base class\n'
        err_msg += 'this needs to be implemented and called from derived class'
        raise Exception(err_msg)