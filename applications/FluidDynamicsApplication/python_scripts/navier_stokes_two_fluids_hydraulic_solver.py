...
class NavierStokesTwoFluidsHydraulicSolver(FluidSolver):
    ...
    def FinalizeSolutionStep(self):
        ...
        # Artificial viscosity according to Ramon's shock capturing. IT IS SOMETHING THAT PROVISIONAL

        self.shock_capturing= False
        if self.shock_capturing:
            domain_size = self.main_model_part.ProcessInfo[KratosMultiphysics.DOMAIN_SIZE]
            if domain_size == 2:
                KratosCFD.ShockCapturingUtilities2D3N.ExecuteRMethodType(self.GetComputingModelPart())
            else:
                KratosCFD.ShockCapturingUtilities3D4N.ExecuteRMethodType(self.GetComputingModelPart())
