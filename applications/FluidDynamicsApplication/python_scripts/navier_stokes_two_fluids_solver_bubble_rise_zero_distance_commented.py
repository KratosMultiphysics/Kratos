...
class NavierStokesTwoFluidsSolver(FluidSolver):
    ...
    def Initialize(self):
        ...
        with open("MaxVelocity.log", "w") as distLogFile:
            distLogFile.write( "time_step" + "\t" + "VMax" + "\n" )
    ...
    def InitializeSolutionStep(self):
        ...
        if self._TimeBufferIsInitialized():
            ...
            # If variables are needed for recovery purposes
            #for node in self.main_model_part.Nodes:
            #    velocity = node.GetSolutionStepValue(KratosMultiphysics.VELOCITY)
            ...
            DistPlus = 1.0e5
            DistMinus = -1.0e5
            YZero = (YPlus + YMinus) / 2.0

            for node in self.main_model_part.Nodes:
                NodeX = node.X
                NodeZ = node.Z
                if (abs(NodeX - 0.005) < 1.0e-6 and abs(NodeZ - 0.005) < 1.0e-6):
                    NodeY = node.Y
                    if (NodeY > 0.005):
