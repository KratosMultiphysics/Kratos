...
const double crLHS29 = DN(0,0)*v_ns(0,1);
const double crLHS30 = DN(1,0)*v_ns(1,1);
const double crLHS31 = DN(2,0)*v_ns(2,1);
const double crLHS32 = crLHS29 + crLHS30 + crLHS31;
const double crLHS33 = DN(0,1)*v_ns(0,0);
const double crLHS34 = DN(1,1)*v_ns(1,0);
const double crLHS35 = DN(2,1)*v_ns(2,0);
...
const double crLHS30 = DN(0,1)*v_ns(0,0) + DN(1,1)*v_ns(1,0) + DN(2,1)*v_ns(2,0) + DN(3,1)*v_ns(3,0);
const double crLHS31 = DN(0,0)*v_ns(0,1) + DN(1,0)*v_ns(1,1) + DN(2,0)*v_ns(2,1) + DN(3,0)*v_ns(3,1);
const double crLHS32 = crLHS19*crLHS31;
const double crLHS33 = crLHS30*crLHS32;
const double crLHS34 = DN(0,2)*v_ns(0,0) + DN(1,2)*v_ns(1,0) + DN(2,2)*v_ns(2,0) + DN(3,2)*v_ns(3,0);
const double crLHS35 = DN(0,0)*v_ns(0,2) + DN(1,0)*v_ns(1,2) + DN(2,0)*v_ns(2,2) + DN(3,0)*v_ns(3,2);
...
const double crLHS55 = DN(0,2)*v_ns(0,1) + DN(1,2)*v_ns(1,1) + DN(2,2)*v_ns(2,1) + DN(3,2)*v_ns(3,1);
...
const double crLHS67 = DN(0,1)*v_ns(0,2) + DN(1,1)*v_ns(1,2) + DN(2,1)*v_ns(2,2) + DN(3,1)*v_ns(3,2);
...
const double crRHS32 = pow(crRHS16, 2) + pow(crRHS5, 2);
const double crRHS33 = pow(crRHS32, 1.0/4.0)*stab_c3;
const double crRHS34 = sqrt(crRHS32)*rho*stab_c2;
const double crRHS35 = crRHS30*(h*(crRHS31*h + crRHS33*h + crRHS34)/stab_c1 + mu);
...
const double crRHS40 = 1.0/(crRHS31 + crRHS33 + crRHS34/h + mu*stab_c1/pow(h, 2));
...
const double crRHS46 = DN(0,1)*v_ns(0,1) + DN(1,1)*v_ns(1,1) + DN(2,1)*v_ns(2,1);
const double crRHS47 = crRHS24*crRHS46;
...
const double crRHS51 = DN(0,1)*p_adj[0] + DN(1,1)*p_adj[1] + DN(2,1)*p_adj[2] - crRHS29*crRHS37 - crRHS36*(DN(0,0)*v_adj(0,1) + DN(1,0)*v_adj(1,1) + DN(2,0)*v_adj(2,1)) - crRHS43 + crRHS44 + crRHS45*rho + crRHS47*rho + crRHS48 + crRHS49 + crRHS50;
const double crRHS52 = crRHS11*crRHS51 + crRHS18*crRHS39;
const double crRHS53 = crRHS27*crRHS40;
const double crRHS54 = 1.0*crRHS46;
const double crRHS55 = crRHS45 + crRHS47;
const double crRHS56 = crRHS40*crRHS51;
const double crRHS57 = crRHS20*crRHS39 + crRHS46*crRHS51;
const double crRHS58 = rho*(DN(1,0)*crRHS5 + DN(1,1)*crRHS16);
const double crRHS59 = N[1]*rho;
const double crRHS60 = N[1]*crRHS2;
const double crRHS61 = crRHS40*crRHS59;
const double crRHS62 = rho*(DN(2,0)*crRHS5 + DN(2,1)*crRHS16);
const double crRHS63 = N[2]*rho;
const double crRHS64 = N[2]*crRHS2;
const double crRHS65 = crRHS40*crRHS63;
rRHS[0]+=-gauss_weight*(-DN(0,0)*crRHS0 + DN(0,0)*crRHS35 + DN(0,0)*stress_adj[0] - DN(0,1)*crRHS12 + DN(0,1)*stress_adj[2] - N[0]*crRHS1 + N[0]*crRHS4 + N[0]*crRHS7 + crRHS15*functional_weights[6] + crRHS15 + crRHS17*crRHS3 - crRHS17*crRHS41 + crRHS22*(DN(0,0)*crRHS19 + DN(0,1)*crRHS21) + crRHS26*crRHS27 - crRHS41*crRHS42 - crRHS52*crRHS53);
rRHS[1]+=-gauss_weight*(DN(0,0)*crRHS12 + DN(0,0)*stress_adj[2] - DN(0,1)*crRHS0 + DN(0,1)*crRHS35 + DN(0,1)*stress_adj[1] - N[0]*crRHS43 + N[0]*crRHS44 + N[0]*crRHS48 + N[0]*crRHS49 + N[0]*crRHS50 + crRHS17*crRHS24 - crRHS17*crRHS56 + crRHS22*(DN(0,0)*crRHS21 + DN(0,1)*crRHS54) + crRHS27*crRHS55 - crRHS42*crRHS56 - crRHS53*crRHS57);
rRHS[2]+=-gauss_weight*(DN(0,0)*crRHS41 + DN(0,1)*crRHS56 + N[0]*crRHS30);
rRHS[3]+=-gauss_weight*(-DN(1,0)*crRHS0 + DN(1,0)*crRHS35 + DN(1,0)*stress_adj[0] - DN(1,1)*crRHS12 + DN(1,1)*stress_adj[2] - N[1]*crRHS1 + N[1]*crRHS14 + N[1]*crRHS38 + N[1]*crRHS4 + N[1]*crRHS7 + crRHS22*(DN(1,0)*crRHS19 + DN(1,1)*crRHS21) + crRHS26*crRHS59 + crRHS3*crRHS58 - crRHS41*crRHS58 - crRHS41*crRHS60 - crRHS52*crRHS61);
rRHS[4]+=-gauss_weight*(DN(1,0)*crRHS12 + DN(1,0)*stress_adj[2] - DN(1,1)*crRHS0 + DN(1,1)*crRHS35 + DN(1,1)*stress_adj[1] - N[1]*crRHS43 + N[1]*crRHS44 + N[1]*crRHS48 + N[1]*crRHS49 + N[1]*crRHS50 + crRHS22*(DN(1,0)*crRHS21 + DN(1,1)*crRHS54) + crRHS24*crRHS58 + crRHS55*crRHS59 - crRHS56*crRHS58 - crRHS56*crRHS60 - crRHS57*crRHS61);
rRHS[5]+=-gauss_weight*(DN(1,0)*crRHS41 + DN(1,1)*crRHS56 + N[1]*crRHS30);
rRHS[6]+=-gauss_weight*(-DN(2,0)*crRHS0 + DN(2,0)*crRHS35 + DN(2,0)*stress_adj[0] - DN(2,1)*crRHS12 + DN(2,1)*stress_adj[2] - N[2]*crRHS1 + N[2]*crRHS14 + N[2]*crRHS38 + N[2]*crRHS4 + N[2]*crRHS7 + crRHS22*(DN(2,0)*crRHS19 + DN(2,1)*crRHS21) + crRHS26*crRHS63 + crRHS3*crRHS62 - crRHS41*crRHS62 - crRHS41*crRHS64 - crRHS52*crRHS65);
rRHS[7]+=-gauss_weight*(DN(2,0)*crRHS12 + DN(2,0)*stress_adj[2] - DN(2,1)*crRHS0 + DN(2,1)*crRHS35 + DN(2,1)*stress_adj[1] - N[2]*crRHS43 + N[2]*crRHS44 + N[2]*crRHS48 + N[2]*crRHS49 + N[2]*crRHS50 + crRHS22*(DN(2,0)*crRHS21 + DN(2,1)*crRHS54) + crRHS24*crRHS62 + crRHS55*crRHS63 - crRHS56*crRHS62 - crRHS56*crRHS64 - crRHS57*crRHS65);
rRHS[8]+=-gauss_weight*(DN(2,0)*crRHS41 + DN(2,1)*crRHS56 + N[2]*crRHS30);
...
const double crRHS47 = pow(crRHS11, 2) + pow(crRHS12, 2) + pow(crRHS5, 2);
const double crRHS48 = pow(crRHS47, 1.0/4.0)*stab_c3;
const double crRHS49 = sqrt(crRHS47)*rho*stab_c2;
const double crRHS50 = crRHS45*(h*(crRHS46*h + crRHS48*h + crRHS49)/stab_c1 + mu);
const double crRHS51 = crRHS5*rho;
const double crRHS52 = crRHS11*rho;
const double crRHS53 = crRHS12*rho;
const double crRHS54 = crRHS9*functional_weights[6];
const double crRHS55 = DN(0,0)*p_adj[0] + DN(1,0)*p_adj[1] + DN(2,0)*p_adj[2] + DN(3,0)*p_adj[3] - crRHS1 + crRHS35*rho + crRHS37*rho + crRHS39*rho + crRHS4 - crRHS42*crRHS51 - crRHS52*(DN(0,1)*v_adj(0,0) + DN(1,1)*v_adj(1,0) + DN(2,1)*v_adj(2,0) + DN(3,1)*v_adj(3,0)) - crRHS53*(DN(0,2)*v_adj(0,0) + DN(1,2)*v_adj(1,0) + DN(2,2)*v_adj(2,0) + DN(3,2)*v_adj(3,0)) + crRHS54 + crRHS7 + crRHS9;
const double crRHS56 = 1.0/(crRHS46 + crRHS48 + crRHS49/h + mu*stab_c1/pow(h, 2));
const double crRHS57 = crRHS55*crRHS56;
const double crRHS58 = N[0]*crRHS2;
const double crRHS59 = rho*(N[0]*f_adj(0,1) + N[1]*f_adj(1,1) + N[2]*f_adj(2,1) + N[3]*f_adj(3,1));
const double crRHS60 = crRHS2*crRHS36;
const double crRHS61 = crRHS29*crRHS3;
const double crRHS62 = DN(0,1)*v_ns(0,1) + DN(1,1)*v_ns(1,1) + DN(2,1)*v_ns(2,1) + DN(3,1)*v_ns(3,1);
const double crRHS63 = crRHS36*crRHS62;
const double crRHS64 = DN(0,1)*v_ns(0,2) + DN(1,1)*v_ns(1,2) + DN(2,1)*v_ns(2,2) + DN(3,1)*v_ns(3,2);
const double crRHS65 = crRHS38*crRHS64;
const double crRHS66 = crRHS11*crRHS6;
const double crRHS67 = crRHS8*(DN(0,1)*t[0] + DN(1,1)*t[1] + DN(2,1)*t[2] + DN(3,1)*t[3]);
const double crRHS68 = crRHS67*functional_weights[6];
const double crRHS69 = DN(0,1)*p_adj[0] + DN(1,1)*p_adj[1] + DN(2,1)*p_adj[2] + DN(3,1)*p_adj[3] - crRHS43*crRHS52 - crRHS51*(DN(0,0)*v_adj(0,1) + DN(1,0)*v_adj(1,1) + DN(2,0)*v_adj(2,1) + DN(3,0)*v_adj(3,1)) - crRHS53*(DN(0,2)*v_adj(0,1) + DN(1,2)*v_adj(1,1) + DN(2,2)*v_adj(2,1) + DN(3,2)*v_adj(3,1)) - crRHS59 + crRHS60 + crRHS61*rho + crRHS63*rho + crRHS65*rho + crRHS66 + crRHS67 + crRHS68;
const double crRHS70 = rho*(N[0]*f_adj(0,2) + N[1]*f_adj(1,2) + N[2]*f_adj(2,2) + N[3]*f_adj(3,2));
const double crRHS71 = crRHS2*crRHS38;
const double crRHS72 = crRHS3*crRHS31;
const double crRHS73 = DN(0,2)*v_ns(0,1);
const double crRHS74 = DN(1,2)*v_ns(1,1);
const double crRHS75 = DN(2,2)*v_ns(2,1);
const double crRHS76 = DN(3,2)*v_ns(3,1);
const double crRHS77 = crRHS73 + crRHS74 + crRHS75 + crRHS76;
const double crRHS78 = crRHS36*crRHS77;
const double crRHS79 = DN(0,2)*v_ns(0,2) + DN(1,2)*v_ns(1,2) + DN(2,2)*v_ns(2,2) + DN(3,2)*v_ns(3,2);
const double crRHS80 = crRHS38*crRHS79;
const double crRHS81 = crRHS12*crRHS6;
const double crRHS82 = crRHS8*(DN(0,2)*t[0] + DN(1,2)*t[1] + DN(2,2)*t[2] + DN(3,2)*t[3]);
const double crRHS83 = crRHS82*functional_weights[6];
const double crRHS84 = DN(0,2)*p_adj[0] + DN(1,2)*p_adj[1] + DN(2,2)*p_adj[2] + DN(3,2)*p_adj[3] - crRHS44*crRHS53 - crRHS51*(DN(0,0)*v_adj(0,2) + DN(1,0)*v_adj(1,2) + DN(2,0)*v_adj(2,2) + DN(3,0)*v_adj(3,2)) - crRHS52*(DN(0,1)*v_adj(0,2) + DN(1,1)*v_adj(1,2) + DN(2,1)*v_adj(2,2) + DN(3,1)*v_adj(3,2)) - crRHS70 + crRHS71 + crRHS72*rho + crRHS78*rho + crRHS80*rho + crRHS81 + crRHS82 + crRHS83;
const double crRHS85 = crRHS18*crRHS69 + crRHS24*crRHS84 + crRHS27*crRHS55;
const double crRHS86 = crRHS41*crRHS56;
const double crRHS87 = crRHS64 - crRHS73 - crRHS74 - crRHS75 - crRHS76;
const double crRHS88 = 1.0*crRHS62;
const double crRHS89 = crRHS64 + crRHS77;
const double crRHS90 = crRHS61 + crRHS63 + crRHS65;
const double crRHS91 = crRHS56*crRHS69;
const double crRHS92 = crRHS29*crRHS55 + crRHS62*crRHS69 + crRHS64*crRHS84;
const double crRHS93 = 1.0*crRHS79;
const double crRHS94 = 0.5*crRHS32;
const double crRHS95 = 0.5*crRHS89;
const double crRHS96 = crRHS72 + crRHS78 + crRHS80;
const double crRHS97 = crRHS56*crRHS84;
const double crRHS98 = crRHS31*crRHS55 + crRHS69*crRHS77 + crRHS79*crRHS84;
const double crRHS99 = rho*(DN(1,0)*crRHS5 + DN(1,1)*crRHS11 + DN(1,2)*crRHS12);
const double crRHS100 = N[1]*rho;
const double crRHS101 = N[1]*crRHS2;
const double crRHS102 = crRHS100*crRHS56;
const double crRHS103 = rho*(DN(2,0)*crRHS5 + DN(2,1)*crRHS11 + DN(2,2)*crRHS12);
const double crRHS104 = N[2]*rho;
const double crRHS105 = N[2]*crRHS2;
const double crRHS106 = crRHS104*crRHS56;
const double crRHS107 = rho*(DN(3,0)*crRHS5 + DN(3,1)*crRHS11 + DN(3,2)*crRHS12);
const double crRHS108 = N[3]*rho;
const double crRHS109 = N[3]*crRHS2;
const double crRHS110 = crRHS108*crRHS56;
rRHS[0]+=-gauss_weight*(-DN(0,0)*crRHS0 + DN(0,0)*crRHS50 + DN(0,0)*stress_adj[0] + DN(0,1)*stress_adj[3] + DN(0,2)*stress_adj[5] - N[0]*crRHS1 + N[0]*crRHS4 + N[0]*crRHS7 + crRHS10*functional_weights[6] + crRHS10 + crRHS13*crRHS3 - crRHS13*crRHS57 - crRHS26*(DN(0,1)*crRHS19 + DN(0,2)*crRHS25) + crRHS34*(DN(0,0)*crRHS28 + DN(0,1)*crRHS30 + crRHS32*crRHS33) + crRHS40*crRHS41 - crRHS57*crRHS58 - crRHS85*crRHS86);
rRHS[1]+=-gauss_weight*(DN(0,0)*stress_adj[3] - DN(0,1)*crRHS0 + DN(0,1)*crRHS50 + DN(0,1)*stress_adj[1] + DN(0,2)*stress_adj[4] - N[0]*crRHS59 + N[0]*crRHS60 + N[0]*crRHS66 + N[0]*crRHS67 + N[0]*crRHS68 + crRHS13*crRHS36 - crRHS13*crRHS91 + crRHS26*(DN(0,0)*crRHS19 - DN(0,2)*crRHS87) + crRHS34*(DN(0,0)*crRHS30 + DN(0,1)*crRHS88 + crRHS33*crRHS89) + crRHS41*crRHS90 - crRHS58*crRHS91 - crRHS86*crRHS92);
rRHS[2]+=-gauss_weight*(DN(0,0)*stress_adj[5] + DN(0,1)*stress_adj[4] - DN(0,2)*crRHS0 + DN(0,2)*crRHS50 + DN(0,2)*stress_adj[2] - N[0]*crRHS70 + N[0]*crRHS71 + N[0]*crRHS81 + N[0]*crRHS82 + N[0]*crRHS83 + crRHS13*crRHS38 - crRHS13*crRHS97 + crRHS26*(DN(0,0)*crRHS25 + DN(0,1)*crRHS87) + crRHS34*(DN(0,0)*crRHS94 + DN(0,1)*crRHS95 + DN(0,2)*crRHS93) + crRHS41*crRHS96 - crRHS58*crRHS97 - crRHS86*crRHS98);
rRHS[3]+=-gauss_weight*(DN(0,0)*crRHS57 + DN(0,1)*crRHS91 + DN(0,2)*crRHS97 + N[0]*crRHS45);
rRHS[4]+=-gauss_weight*(-DN(1,0)*crRHS0 + DN(1,0)*crRHS50 + DN(1,0)*stress_adj[0] + DN(1,1)*stress_adj[3] + DN(1,2)*stress_adj[5] - N[1]*crRHS1 + N[1]*crRHS4 + N[1]*crRHS54 + N[1]*crRHS7 + N[1]*crRHS9 + crRHS100*crRHS40 - crRHS101*crRHS57 - crRHS102*crRHS85 - crRHS26*(DN(1,1)*crRHS19 + DN(1,2)*crRHS25) + crRHS3*crRHS99 + crRHS34*(DN(1,0)*crRHS28 + DN(1,1)*crRHS30 + DN(1,2)*crRHS94) - crRHS57*crRHS99);
rRHS[5]+=-gauss_weight*(DN(1,0)*stress_adj[3] - DN(1,1)*crRHS0 + DN(1,1)*crRHS50 + DN(1,1)*stress_adj[1] + DN(1,2)*stress_adj[4] - N[1]*crRHS59 + N[1]*crRHS60 + N[1]*crRHS66 + N[1]*crRHS67 + N[1]*crRHS68 + crRHS100*crRHS90 - crRHS101*crRHS91 - crRHS102*crRHS92 + crRHS26*(DN(1,0)*crRHS19 - DN(1,2)*crRHS87) + crRHS34*(DN(1,0)*crRHS30 + DN(1,1)*crRHS88 + DN(1,2)*crRHS95) + crRHS36*crRHS99 - crRHS91*crRHS99);
rRHS[6]+=-gauss_weight*(DN(1,0)*stress_adj[5] + DN(1,1)*stress_adj[4] - DN(1,2)*crRHS0 + DN(1,2)*crRHS50 + DN(1,2)*stress_adj[2] - N[1]*crRHS70 + N[1]*crRHS71 + N[1]*crRHS81 + N[1]*crRHS82 + N[1]*crRHS83 + crRHS100*crRHS96 - crRHS101*crRHS97 - crRHS102*crRHS98 + crRHS26*(DN(1,0)*crRHS25 + DN(1,1)*crRHS87) + crRHS34*(DN(1,0)*crRHS94 + DN(1,1)*crRHS95 + DN(1,2)*crRHS93) + crRHS38*crRHS99 - crRHS97*crRHS99);
rRHS[7]+=-gauss_weight*(DN(1,0)*crRHS57 + DN(1,1)*crRHS91 + DN(1,2)*crRHS97 + N[1]*crRHS45);
rRHS[8]+=-gauss_weight*(-DN(2,0)*crRHS0 + DN(2,0)*crRHS50 + DN(2,0)*stress_adj[0] + DN(2,1)*stress_adj[3] + DN(2,2)*stress_adj[5] - N[2]*crRHS1 + N[2]*crRHS4 + N[2]*crRHS54 + N[2]*crRHS7 + N[2]*crRHS9 + crRHS103*crRHS3 - crRHS103*crRHS57 + crRHS104*crRHS40 - crRHS105*crRHS57 - crRHS106*crRHS85 - crRHS26*(DN(2,1)*crRHS19 + DN(2,2)*crRHS25) + crRHS34*(DN(2,0)*crRHS28 + DN(2,1)*crRHS30 + DN(2,2)*crRHS94));
rRHS[9]+=-gauss_weight*(DN(2,0)*stress_adj[3] - DN(2,1)*crRHS0 + DN(2,1)*crRHS50 + DN(2,1)*stress_adj[1] + DN(2,2)*stress_adj[4] - N[2]*crRHS59 + N[2]*crRHS60 + N[2]*crRHS66 + N[2]*crRHS67 + N[2]*crRHS68 + crRHS103*crRHS36 - crRHS103*crRHS91 + crRHS104*crRHS90 - crRHS105*crRHS91 - crRHS106*crRHS92 + crRHS26*(DN(2,0)*crRHS19 - DN(2,2)*crRHS87) + crRHS34*(DN(2,0)*crRHS30 + DN(2,1)*crRHS88 + DN(2,2)*crRHS95));
rRHS[10]+=-gauss_weight*(DN(2,0)*stress_adj[5] + DN(2,1)*stress_adj[4] - DN(2,2)*crRHS0 + DN(2,2)*crRHS50 + DN(2,2)*stress_adj[2] - N[2]*crRHS70 + N[2]*crRHS71 + N[2]*crRHS81 + N[2]*crRHS82 + N[2]*crRHS83 + crRHS103*crRHS38 - crRHS103*crRHS97 + crRHS104*crRHS96 - crRHS105*crRHS97 - crRHS106*crRHS98 + crRHS26*(DN(2,0)*crRHS25 + DN(2,1)*crRHS87) + crRHS34*(DN(2,0)*crRHS94 + DN(2,1)*crRHS95 + DN(2,2)*crRHS93));
rRHS[11]+=-gauss_weight*(DN(2,0)*crRHS57 + DN(2,1)*crRHS91 + DN(2,2)*crRHS97 + N[2]*crRHS45);
rRHS[12]+=-gauss_weight*(-DN(3,0)*crRHS0 + DN(3,0)*crRHS50 + DN(3,0)*stress_adj[0] + DN(3,1)*stress_adj[3] + DN(3,2)*stress_adj[5] - N[3]*crRHS1 + N[3]*crRHS4 + N[3]*crRHS54 + N[3]*crRHS7 + N[3]*crRHS9 + crRHS107*crRHS3 - crRHS107*crRHS57 + crRHS108*crRHS40 - crRHS109*crRHS57 - crRHS110*crRHS85 - crRHS26*(DN(3,1)*crRHS19 + DN(3,2)*crRHS25) + crRHS34*(DN(3,0)*crRHS28 + DN(3,1)*crRHS30 + DN(3,2)*crRHS94));
rRHS[13]+=-gauss_weight*(DN(3,0)*stress_adj[3] - DN(3,1)*crRHS0 + DN(3,1)*crRHS50 + DN(3,1)*stress_adj[1] + DN(3,2)*stress_adj[4] - N[3]*crRHS59 + N[3]*crRHS60 + N[3]*crRHS66 + N[3]*crRHS67 + N[3]*crRHS68 + crRHS107*crRHS36 - crRHS107*crRHS91 + crRHS108*crRHS90 - crRHS109*crRHS91 - crRHS110*crRHS92 + crRHS26*(DN(3,0)*crRHS19 - DN(3,2)*crRHS87) + crRHS34*(DN(3,0)*crRHS30 + DN(3,1)*crRHS88 + DN(3,2)*crRHS95));
rRHS[14]+=-gauss_weight*(DN(3,0)*stress_adj[5] + DN(3,1)*stress_adj[4] - DN(3,2)*crRHS0 + DN(3,2)*crRHS50 + DN(3,2)*stress_adj[2] - N[3]*crRHS70 + N[3]*crRHS71 + N[3]*crRHS81 + N[3]*crRHS82 + N[3]*crRHS83 + crRHS107*crRHS38 - crRHS107*crRHS97 + crRHS108*crRHS96 - crRHS109*crRHS97 - crRHS110*crRHS98 + crRHS26*(DN(3,0)*crRHS25 + DN(3,1)*crRHS87) + crRHS34*(DN(3,0)*crRHS94 + DN(3,1)*crRHS95 + DN(3,2)*crRHS93));
rRHS[15]+=-gauss_weight*(DN(3,0)*crRHS57 + DN(3,1)*crRHS91 + DN(3,2)*crRHS97 + N[3]*crRHS45);
