...
for dim, nnodes in zip(dim_vector, nnodes_vector):
    ...
    accel_gauss = (bdf0*v + bdf1*vn + bdf2*vnn).transpose()*N # Time derivative term: acceleration at gauss points
    ...
    vn_adj  = DefineMatrix('vn_adj',nnodes,dim)   # Previous step velocity
    vnn_adj = DefineMatrix('vnn_adj',nnodes,dim)  # 2 previous step velocity
    ...
    if (stabilization):
        ...
        tau1_denominator_adj += rho*dyn_tau/dt # time contribution to tau_1
