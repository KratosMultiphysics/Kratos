    ...
    def __init__(self, Model, settings):
        ...
        if ( self.write_to_log ):
            with open("ApplyMassConservationCheckProcess.log", "w") as logFile:
                logFile.write( "positiveVolume" + "\t" + "negativeVolume" + "\t" + "interfaceArea" + "\t" + "inFlowWater" + "\t" + "outFlowWater" + "\t" + "netFlowWater" + "\t" + "DIVwater" + "\t" + "DIVair" + "\t" + "DIVcut" + "\t" + "DIVcut_on_side_of_water" + "\n" )
    ...
    def ExecuteFinalizeSolutionStep(self):
        ...
        if ( self.counter > 2 ):
            ...
            if ( updated ):
                ...
                divergenceInCut = self.MassConservationCheckProcess.GetDivergenceCut()

                divergenceOnTheSideOfWater = self.MassConservationCheckProcess.GetDivergenceCutWaterDistribution()
                ...
                KratosMultiphysics.Logger.PrintInfo("ApplyMassConservationCheckProcess", "Divergence in Cut    = " + str( divergenceInCut ) )
                KratosMultiphysics.Logger.PrintInfo("ApplyMassConservationCheckProcess", "Divergence in Cut (water)   = " + str( divergenceOnTheSideOfWater ) )
                KratosMultiphysics.Logger.Flush()

                # adds additional lines to the log file
                if ( self.write_to_log ):
                    with open("ApplyMassConservationCheckProcess.log", "a+") as logFile:
                        logFile.write( str(posVol) + "\t" + str(negVol) + "\t" + str(interfaceArea) + "\t" + str(inflow) + "\t" + str(-outflow) + "\t" + str(inflow+outflow) + "\t" + str(divergenceInWater) + "\t" + str(divergenceInAir) + "\t" + str(divergenceInCut) + "\t" + str(divergenceOnTheSideOfWater)+ "\n" )
