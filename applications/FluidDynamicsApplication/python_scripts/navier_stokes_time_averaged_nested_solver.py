...
class NavierStokesTimeAveragedMonolithicSolver(FluidSolver):
    ...
    def InitializeSolutionFromMonolithicSolver(self):
        ## Initialize the first steps with monolithic solver
        self._initialize_monolithic_solver()
        index = 1
        for node in self.main_model_part.Nodes:
            initial_pressure   = self.main_model_part.GetNode(index).GetSolutionStepValue(KratosMultiphysics.PRESSURE)
            initial_velocity_x = self.main_model_part.GetNode(index).GetSolutionStepValue(KratosMultiphysics.VELOCITY_X)
            initial_velocity_y = self.main_model_part.GetNode(index).GetSolutionStepValue(KratosMultiphysics.VELOCITY_Y)
            initial_velocity_z = self.main_model_part.GetNode(index).GetSolutionStepValue(KratosMultiphysics.VELOCITY_Z)
            #print("initial_velocity_x " + str(initial_velocity_x))
            node.SetSolutionStepValue(KratosCFD.TIME_AVERAGED_PRESSURE,   initial_pressure)
            node.SetSolutionStepValue(KratosCFD.TIME_AVERAGED_VELOCITY_X, initial_velocity_x)
            node.SetSolutionStepValue(KratosCFD.TIME_AVERAGED_VELOCITY_Y, initial_velocity_y)
            node.SetSolutionStepValue(KratosCFD.TIME_AVERAGED_VELOCITY_Z, initial_velocity_z)
            index +=1
         
        for i in range (0,self.GetMinimumBufferSize() + 1):
            self.main_model_part.CloneSolutionStep()
            #self.main_model_part.ProcessInfo[KratosMultiphysics.STEP] += 1
        
        print("_TimeBufferIsInitialized: " + str(self._TimeBufferIsInitialized()))
    ...
    def _initialize_monolithic_solver(self):
        import navier_stokes_solver_vmsmonolithic
        import json
        monolithic_solver_settings =  """{
            "solver_type"                 : "Monolithic",
            "model_part_name" : "MainModelPart",
            "domain_size"     : 2,
            "model_import_settings"       : {
                "input_type"     : "mdpa",
                "input_filename" : "cylinder_2d_water"
            },
            "echo_level"                  : 0,
            "compute_reactions"           : false,
            "dynamic_tau"                 : 1.0,
            ...
            "skin_parts"                  : ["AutomaticInlet2D_inlet","Outlet2D_outlet","NoSlip2D_wall","NoSlip2D_cylinder"],
            "no_skin_parts"               : [],
            "time_stepping"               : {
                "automatic_time_step"   : true,
                "CFL_number"            : 1.0,
                "minimum_delta_time"    : 0.1,
                "maximum_delta_time"    : 5.0
            },
            "compute_time_averaged_results": true
        }"""

        monolithic_solver_settings = KratosMultiphysics.Parameters(monolithic_solver_settings)

        self.monolithic_solver  = navier_stokes_solver_vmsmonolithic.CreateSolver(self.model, monolithic_solver_settings)
        ...
        step = 0
        while(step < 10):
            print("Inner loop STEP: " + str(step))
        ...
        # self.monolithic_model = self.model.GetModelPart("Parts_fluid")
        print("NavierStokesTimeAveragedMonolithicSolver: " + "MONOLITHIC SOLVER INITIALIZED")