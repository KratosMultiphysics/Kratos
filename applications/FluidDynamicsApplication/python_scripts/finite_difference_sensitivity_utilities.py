...
def ComputeAdjointSensitivity(node_ids, kratos_parameters):
    test = AdjointFluidAnalysis(Kratos.Model(), kratos_parameters)
    test.Run()

    sensitivities = Kratos.Matrix(len(node_ids), 2)
    for i, node_id in enumerate(node_ids):
        node_sensitivity = test._GetSolver().main_model_part.GetNode(node_id).GetSolutionStepValue(Kratos.SHAPE_SENSITIVITY)
...
def ComputeFiniteDifferenceSensitivity(node_ids, step_size,
    ...
    for i, node_id in enumerate(node_ids):
        ...
        for k in range(2):
            ...
            sensitivity[i, k] = objective_value_evaluation_method(kratos_parameters.Clone())
            coord[k] -= step_size
...
class FiniteDifferenceVelocityPressureNormSquareShapeSensitivityAnalysis:
    ...
    def ComputeSensitivity(node_ids, step_size,
        ...
        def compute_drag(kratos_parameters):
            primal_problem_solving_method(kratos_parameters)
            return FiniteDifferenceVelocityPressureNormSquareShapeSensitivityAnalysis._ObjectiveValueEvaluation()
