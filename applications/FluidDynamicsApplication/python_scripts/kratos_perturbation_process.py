...
from KratosMultiphysics.FluidDynamicsApplication.GenerateCN import GenerateCN
from KratosMultiphysics.FluidDynamicsApplication.projection_utilities import _DotProduct, RegularGrid1D, DomainPanel2D, DomainPanel3D
...
class ImposePerturbedInitialConditionAnalysisStage(ConvectionDiffusionAnalysis):
    ...
    def __init__(self,input_model,input_parameters):
        ...
        self._GetSolver().main_model_part.AddNodalSolutionStepVariable(KratosMultiphysics.FluidDynamicsApplication.VELOCITY_NOISE) # for storing P(u_{cn})
    ...
    def ApplyBoundaryConditions(self):
        ...
        for node in structure_model_part.Nodes:
            perturbed_velocity = self.project_parameters["problem_data"]["penalty_coefficient"].GetDouble() * node.GetSolutionStepValue(KratosMultiphysics.FluidDynamicsApplication.VELOCITY_NOISE) # c*P(u_{cn})
    ...
    def RestoreBoundaryValues(self):
        for submdpa in self.project_parameters["solver_settings"]["processes_sub_model_part_list"].GetStringArray():
            for node in self._GetSolver().main_model_part.GetSubModelPart(submdpa).Nodes:
                node.SetValue(KratosMultiphysics.VELOCITY_LAPLACIAN, -1*node.GetSolutionStepValue(KratosMultiphysics.FluidDynamicsApplication.VELOCITY_NOISE)) # recall VELOCITY_NOISE is P(u_{cn}) (without penalty coefficient)
...
class ImposePerturbedInitialConditionProcess(KratosMultiphysics.Process):
    ...
    def SolveAssociatedPoissonProblem(self):
        ...
        mapper.Map(KratosMultiphysics.VELOCITY,KratosMultiphysics.FluidDynamicsApplication.VELOCITY_NOISE) # map P(u_{cn}) into Poisson problem variable VELOCITY_NOISE
