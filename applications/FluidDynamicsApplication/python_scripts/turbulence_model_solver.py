...
def CreateTurbulenceModel(model, model_part, settings):
    ...
    return Factory(model, model_part, settings)

class TurbulenceModelSolver(PythonSolver):
    def __init__(self, model, model_part, settings):
        super(TurbulenceModelSolver, self).__init__(model, settings)
