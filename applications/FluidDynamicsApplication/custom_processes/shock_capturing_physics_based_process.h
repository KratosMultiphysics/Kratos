...
private:
        ...
        TTLSContainerType &rShockCapturingTLS)
    {
        ...
        if (mShockSensor) {
            ...
            constexpr double k_beta = 4.5;  // Diferent from original paper. Experimentally we've seen that this value works better for us.
        ...
        if (mThermalSensor || mShearSensor) {
            ...
            if (mThermalSensor) {
                ...
                const double s_kappa_hat = s_kappa; // Diferent from original paper. Experimentally we've seen that this expression works better for us.
                rElement.GetValue(THERMAL_SENSOR) = s_kappa_hat;

                // Calculate elemental artificial conductivity (thermal sensor)
                constexpr double k_kappa = 3.0; // Diferent from original paper. Experimentally we've seen that this value works better for us.
