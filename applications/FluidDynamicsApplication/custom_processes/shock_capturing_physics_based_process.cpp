...
namespace Kratos
{
    ShockCapturingPhysicsBasedProcess::ShockCapturingPhysicsBasedProcess(
        ...
        : Process(), mrModelPart(rModelPart)
    {
        ...
        mK_beta_constant = rParameters["k_beta_constant"].GetDouble();
        mPr_beta_constant = rParameters["Pr_beta_constant"].GetDouble();
        mThermalSensor = rParameters["thermal_sensor"].GetBool();
        mK_kappa_constant = rParameters["k_kappa_constant"].GetDouble();
        mShearSensor = rParameters["shear_sensor"].GetBool();
        mK_mu_constant = rParameters["k_mu_constant"].GetDouble();
    ...
    const Parameters ShockCapturingPhysicsBasedProcess::GetDefaultParameters() const
    {
        const Parameters default_parameters = Parameters(R"({
            ...
            "shock_sensor"    : true,
            "k_beta_constant" : 1.5,
            "Pr_beta_constant": 0.71,
            "thermal_sensor"  : true,
            "k_kappa_constant": 0.0,
            "shear_sensor"    : true,
            "k_mu_constant"   : 0.0,
    ...
    void ShockCapturingPhysicsBasedProcess::CalculatePhysicsBasedShockCapturing()
    {
        ...
        if (geometry_type == GeometryData::KratosGeometryType::Kratos_Triangle2D3)
        {
            ...
            auto aux_function_2D3N = [&, this](Element &rElement, ShockCapturingTLSContainer2D3N &rShockCapturingTLS)
            { this->CalculatePhysicsBasedShockCapturingElementContribution<2, 3>(rElement, rShockCapturingTLS); };
        ...
        else if (geometry_type == GeometryData::KratosGeometryType::Kratos_Tetrahedra3D4)
        {
            ...
            auto aux_function_3D4N = [&, this](Element &rElement, ShockCapturingTLSContainer3D4N &rShockCapturingTLS)
            { this->CalculatePhysicsBasedShockCapturingElementContribution<3, 4>(rElement, rShockCapturingTLS); };
            // Perform the elemental loop
    ...
    double ShockCapturingPhysicsBasedProcess::SmoothedLimitingFunction(
        ...
        const double s_max)
    {
        const double c1 = 100.0;
        const double g = s_min + (s - s_min) * ((1.0 / Globals::Pi) * std::atan(c1 * (s - s_min)) + 0.5) - (1.0 / Globals::Pi) * std::atan(c1) + 0.5;
        const double smooted_value = g - (g - s_max) * ((1.0 / Globals::Pi) * std::atan(c1 * (g - s_max)) + 0.5) - (1.0 / Globals::Pi) * std::atan(c1) + 0.5;
        return smooted_value;
    }
