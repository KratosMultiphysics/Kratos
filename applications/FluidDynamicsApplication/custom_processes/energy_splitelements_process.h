...
public:
    ...
    void Execute() override
    {
        ...
        double air_zg = 0.0;
        double water_zg = 0.0;
        // double Total_artificial_term=0.0;
        double integral_velocity_dissipative_term=0;
        if(mDomainSize == 2)
        // Total Energy check for 2D cases[Fluid1_energy and FLuid2_energy]: A loop in each element of the mesh considering splitted elements (air/water elements)
        {
            ...
            for (auto it_elem = mrModelPart.ElementsBegin(); it_elem != mrModelPart.ElementsEnd(); ++it_elem){
                ...
                if (this->IsCut(elem_dist)){
                    ...
                    const auto &r_neg_subdivisions = p_divide_util->GetNegativeSubdivisions();
                    const unsigned int n_neg_sbdiv = r_neg_subdivisions.size();
                    ...
                    for (unsigned int i_gauss = 0; i_gauss < n_neg_gauss_pt; ++i_gauss){
                        ...
                        // energy artificial viscosity prod(data.DN_DX, trans(data.DN_DX));
                         
                        Matrix velocity_gradient_vector= CalculateGaussInterpolatedGradientVelocity(r_geom,DN_DX_neg_side,w_gauss);
                        
                        // array_1d<double,3> velocity_gradient_transpose=transpose(velocity_gradient_vector);
                        // const double velocity_gradient_symetric_term= prod(velocity_gradient_vector,trans(velocity_gradient_vector));
                        Matrix velocity_gradient_symetric_term=velocity_gradient_vector+ trans(velocity_gradient_vector);
                        
                        const double velocity_dissipative_term= GetDissipativeTerm(velocity_gradient_symetric_term,velocity_gradient_symetric_term);
                        
                        integral_velocity_dissipative_term+=velocity_dissipative_term*w_gauss;
                    ...
                    const auto &r_pos_subdivisions = p_divide_util->GetPositiveSubdivisions();
                    const unsigned int n_pos_sbdiv = r_pos_subdivisions.size();
                ...
                // No split element case
                    ...
                    if (this->IsPositive(elem_dist)){
                        ...
                        BoundedMatrix<double,3,2> my_dn_dx;
                        array_1d<double,3> my_N;
                        double Weight;

                        GeometryUtils::CalculateGeometryData(r_geom, my_dn_dx, my_N, Weight);
                        ...
                        for (unsigned int i_gauss = 0; i_gauss < n_gauss; ++i_gauss){
                            ...
                            // energy artificial viscosity 
                            
                            
                            Matrix velocity_gradient_vector= CalculateGaussInterpolatedGradientVelocity(r_geom,my_dn_dx,Weight);
                            
                            // array_1d<double,3> velocity_gradient_transpose=transpose(velocity_gradient_vector)
                        
                            Matrix velocity_gradient_symetric_term= velocity_gradient_vector+trans(velocity_gradient_vector);
                        
                            // const double velocity_gradient_symetric_term= prod(velocity_gradient_vector,velocity_gradient_transpose);
                            const double velocity_dissipative_term= GetDissipativeTerm(velocity_gradient_symetric_term,velocity_gradient_symetric_term);
                        
                            integral_velocity_dissipative_term+=velocity_dissipative_term*Weight;
                        }
                     }
                     }
...
//  NO ESTA BIEN 
        ...
        // if (mPreviousStepWaterEnergy > tol) {
        //     mArtificialViscosity = GetArtificialViscosity(mPreviousStepWaterEnergy,Total_energy_water_system,integral_velocity_dissipative_term);
        //     // mArtificialViscosity = 0.0;
        // } else {
        //     mArtificialViscosity = 0.0;
        // }
        const double delta_energy=Total_energy_water_system-mPreviousStepWaterEnergy;
       
        // KRATOS_WATCH(mArtificialViscosity)
        KRATOS_WATCH(mPreviousStepWaterEnergy)
        KRATOS_WATCH(Total_energy_water_system)
        ...
        // WritingFileviscosiy(mArtificialViscosity, Total_energy_water_system, integral_velocity_dissipative_term, delta_energy);
        mPreviousStepWaterEnergy = Total_energy_water_system;
        KRATOS_CATCH("");
                        }
            ...
            for (auto it_elem = mrModelPart.ElementsBegin(); it_elem != mrModelPart.ElementsEnd(); ++it_elem)
            {
                ...
                else
                {
                    ...
                    if (this->IsPositive(elem_dist))
                    {
                        ...
                        BoundedMatrix<double,4,3> my_dn_dx;
                        array_1d<double,4> my_N;
                        double Weight;
...
// double GetArtificialViscosity(double old_water_energy, double current_water_energy, double dissipative_velocity_term){
//     KRATOS_WATCH(old_water_energy)
//     const double delta_water_energy=current_water_energy-old_water_energy;
//     KRATOS_WATCH(delta_water_energy)
//     const double delta_time = mrModelPart.GetProcessInfo()[DELTA_TIME];
//     const double tolerance=1e-06;
//     if  (abs(dissipative_velocity_term)<tolerance){
//         dissipative_velocity_term=1;
//         KRATOS_WATCH("NOT TAKE INTO ACCOUNT THIS VALUE 0 DIVISION")
//     }
//     const double artificial_viscosity = delta_water_energy / (dissipative_velocity_term * delta_time);
    

//     // Save artificial viscosity in ProcessInfo
//     auto& r_process_info = mrModelPart.GetProcessInfo();
//     r_process_info[ARTIFICIAL_DYNAMIC_VISCOSITY] = artificial_viscosity;
...
// // void WritingFileviscosiy(const double Artificial_viscosity,const double Total_energy_water_system,const double integral_velocity_dissipative_term, const double energy_increment){
//         KRATOS_WATCH("I am writting into file energy check")
//         const double current_time = mrModelPart.GetProcessInfo()[TIME];
//         std::ofstream MyFile("artificial_viscosity.txt", std::ios::app);
//         if (MyFile.is_open())
//         {
//             std::cout<< "success" << std::endl;
//         }
...
//         std::string output_line = std::to_string(current_time) + "\t";
//         output_line += std::to_string(Total_energy_water_system) + "\t";
//         output_line += std::to_string(integral_velocity_dissipative_term)+ "\t";
//         output_line += std::to_string(energy_increment)+ "\t";
//         output_line += std::to_string(Artificial_viscosity)+"\n";
...
protected:
    ...
    // array_1d<double,3>  transpose(array_1d<double,3>  A)
    // {z
    //     int i, j;
    //     for (i = 0; i < N; i++){
    //         for (j = 0; j < N; j++){
    ...
    double GetDissipativeTerm(Matrix A, Matrix B)
    {     double result_gradient=0;
        for (unsigned int i=0;i<3; i++){
            for (unsigned int j=0;j<3;j++){
            result_gradient += A(i,j)*B(i,j);
    ...
    // not cut elements
   Matrix  CalculateGaussInterpolatedGradientVelocity(
        const Geometry<Node<3>>& rGeometry,
         BoundedMatrix<double,4,3> DN,
         const double Weight
    )
    {
        // Gauss pt. interpolation of velocity gradient
        
        const unsigned int n_nodes = rGeometry.PointsNumber();
        // Velocity gradient
        Matrix v_gauss_gradient_interpolated = ZeroMatrix(3,3);
        for (unsigned int i_node = 0; i_node< n_nodes; i_node++)
        ...
        {  
            ...
            array_1d<double,3> velocity= rGeometry[i_node].FastGetSolutionStepValue(VELOCITY);
            for (unsigned int i = 0; i <  3; i++){
                for (unsigned int j = 0; j < 3; j++){
                    
                    v_gauss_gradient_interpolated(i ,j) += DN(i_node,j)*velocity[i];
                }
            }
        }
        ...
        return v_gauss_gradient_interpolated;
    }
    ...
    Matrix  CalculateGaussInterpolatedGradientVelocity(
        const Geometry<Node<3>>& rGeometry,
        Geometry<Node<3>>::ShapeFunctionsGradientsType DN,
         const double Weight
    )
    {
        // Gauss pt. interpolation of velocity gradient
        ...
        {  
            ...
            for (unsigned int i = 0; i < 3; i++){
                for (unsigned int j = 0; j < 3; j++){
                     ...
                     v_gauss_gradient_interpolated(i,j) += DN[i_node](i_node,j)*velocity[i];
                }
            }
        }
        return v_gauss_gradient_interpolated;
...
private:
    ...
    double mPreviousStepWaterEnergy = 0.0;
