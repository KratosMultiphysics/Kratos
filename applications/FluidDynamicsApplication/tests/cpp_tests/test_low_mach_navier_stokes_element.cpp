...
KRATOS_TEST_CASE_IN_SUITE(LowMachNavierStokes2D3N, FluidDynamicsApplicationFastSuite)
{
    ...
    // Check values
    const std::vector<double> rhs_ref = {-18.706895248, 6.29942291278, 1.24608488868, 2184.12230066, -17.2919092301, -6.05733490486, 8.31670542341, 1856.70578947, -10.010253629, 1.63204159152, -5.3423003252, 3967.02995819};        // LowMachNavierStokes2D3N
    const std::vector<double> lhs_0_ref = {0.277673832983, -41.4537929812, -1.33922918645, -0.176601650961, -0.312383062105, -23.5493053155, -4.33364988921, -0.0689813510326, 0.0347092291228, -24.6437146269, 3.33550965495, -0.0626415370005}; // LowMachNavierStokes2D3N
    KRATOS_EXPECT_VECTOR_NEAR(RHS, rhs_ref, 1.0e-8)
    KRATOS_EXPECT_VECTOR_NEAR(row(LHS,0), lhs_0_ref, 1.0e-8)
}

KRATOS_TEST_CASE_IN_SUITE(LowMachNavierStokes3D4N, FluidDynamicsApplicationFastSuite)
{
    Model model;
    unsigned int buffer_size = 3;
    auto& r_model_part = model.CreateModelPart("TestModelPart", buffer_size);
    ...
    p_properties->SetValue(DYNAMIC_VISCOSITY, 1.0);
    p_properties->SetValue(HEAT_CAPACITY_RATIO, gamma);
    auto p_cons_law = Kratos::make_shared<Newtonian3DLaw>();
    p_properties->SetValue(CONSTITUTIVE_LAW, p_cons_law);
    ...
    r_model_part.CreateNewNode(2, 1.0, 0.1, 0.0);
    r_model_part.CreateNewNode(3, 1.0, 0.9, 0.0);
    r_model_part.CreateNewNode(4, 1.0, 0.9, 1.0);
    ...
    auto p_elem = r_model_part.CreateNewElement("LowMachNavierStokes3D4N", 1, element_nodes, p_properties);

    // Set the elemental values
    p_elem->SetValue(RESISTANCE, 1.0);

    // Define and set the nodal values
    Matrix reference_velocity(4,3);
    reference_velocity(0,0) = 0.0; reference_velocity(0,1) = 0.1; reference_velocity(0,2) = 0.2;
    reference_velocity(1,0) = 0.1; reference_velocity(1,1) = 0.2; reference_velocity(1,2) = 0.3;
    reference_velocity(2,0) = 0.2; reference_velocity(2,1) = 0.3; reference_velocity(2,2) = 0.4;
    reference_velocity(3,0) = 0.3; reference_velocity(3,1) = 0.4; reference_velocity(3,2) = 0.5;
    ...
    // Check values
    const std::vector<double> rhs_ref = {-3.73806116195,2.6709476794,0.273337342089,0.4070621432,473.517713081,-3.47831754198,-2.721118126,3.42763363534,0.369045519895,468.003870545,-3.03841986038,0.472889286405,-2.89300961216,2.94843136606,553.008533792,-1.88154500844,0.26076858932,0.478792595532,-1.83451853668,1001.17893047};        // LowMachNavierStokes3D4N
    const std::vector<double> lhs_0_ref = {0.0555683626097,-5.07148877663,-0.177219727252,-0.141775781802,-0.0233696343629,-0.0625144079359,-2.82903346431,-0.832272683783,-0.0567508479442,-0.00935453537542,0.00694604532622,-3.17445019983,0.696156982101,-0.661209012484,-0.0085947879233,1.92791486281e-18,-3.15471058906,-0.0616764628713,0.559726128785,-0.00813315993907}; // LowMachNavierStokes3D4N
    KRATOS_EXPECT_VECTOR_NEAR(RHS, rhs_ref, 1.0e-8)
    KRATOS_EXPECT_VECTOR_NEAR(row(LHS,0), lhs_0_ref, 1.0e-8)
}
