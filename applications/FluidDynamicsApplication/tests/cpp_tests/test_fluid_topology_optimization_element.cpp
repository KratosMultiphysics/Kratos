...
KRATOS_TEST_CASE_IN_SUITE(FluidTopologyOptimizationElement2D3N, FluidDynamicsApplicationFastSuite)
{
    ...
    if (test_adjoint)
    {
        ...
        // Define and set the nodal values
        // ADJ NS
        Matrix reference_velocity_adj(4,2);
        ...
        for (std::size_t i=0; i<3; i++){
            r_geometry[i].SetValue(SOUND_VELOCITY, 1.0);
            r_geometry[i].FastGetSolutionStepValue(DENSITY) = 1.0;
            r_geometry[i].SetValue(RESISTANCE, 0.0); // RESISTANCE IS A NODAL VARIABLE
            ...
            for (std::size_t k=0; k<2; k++){
                r_geometry[i].FastGetSolutionStepValue(VELOCITY_ADJ)[k] = reference_velocity_adj(i,k);
                r_geometry[i].FastGetSolutionStepValue(VELOCITY_ADJ, 1)[k] = 0.9*reference_velocity_adj(i,k);
                r_geometry[i].FastGetSolutionStepValue(VELOCITY_ADJ, 2)[k] = 0.7*reference_velocity_adj(i,k);
            }
        }

        // Set the elemental values
        // p_elem->SetValue(RESISTANCE, 1.0e3); IT IS A NODAL VALUE
        ...
        // Check values
        const std::vector<double> rhs_ref_adj = {34.2921172632, -19.1612962435, -0.0212935839184, -58.7701017724, 31.2334272413, -0.0186565395118, -8.37866202262, -77.7742128742, -0.0450498765698}; // FluidTopologyOptimization2D3N-ADJ_NS_RHS
        const std::vector<double> lhs_0_ref_adj = {255.745969647, 0, 0.155863699867, -197.183916289, 250.033529091, 0.105861405575, 52.887840474, -250.033529091, 0.13527448821};                     // FluidTopologyOptimization2D3N-ADJ_NS_LHS
        KRATOS_EXPECT_VECTOR_NEAR(RHS_ADJ, rhs_ref_adj, 1.0e-8)
        KRATOS_EXPECT_VECTOR_NEAR(row(LHS_ADJ,0), lhs_0_ref_adj, 1.0e-8)
    }
}
