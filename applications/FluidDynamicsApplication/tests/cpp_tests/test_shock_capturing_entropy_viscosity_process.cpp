...
namespace {
    ...
    {        // Set the test model part
        ...
        printf("%.10f\n", r_model_part.GetNode(5100).GetValue(ARTIFICIAL_MASS_DIFFUSIVITY));
        printf("%.10f\n", r_model_part.GetNode(5100).GetValue(ARTIFICIAL_CONDUCTIVITY));
        printf("%.10f\n", r_model_part.GetNode(5100).GetValue(ARTIFICIAL_BULK_VISCOSITY));
        printf("%.10f\n", r_model_part.GetNode(5100).GetValue(ARTIFICIAL_DYNAMIC_VISCOSITY));

        KRATOS_CHECK_NEAR(r_model_part.GetNode(5100).GetValue(ARTIFICIAL_MASS_DIFFUSIVITY), 0.0, tolerance);
        KRATOS_CHECK_NEAR(r_model_part.GetNode(5100).GetValue(ARTIFICIAL_CONDUCTIVITY), 0.0, tolerance);
        KRATOS_CHECK_NEAR(r_model_part.GetNode(5100).GetValue(ARTIFICIAL_BULK_VISCOSITY), 0.0, tolerance);
     ...
     */
    KRATOS_TEST_CASE_IN_SUITE(ShockCapturingEntropyViscosityAbgrallFunction, FluidDynamicsApplicationFastSuite)
    {
        // Set the test model part
        Model model;
        auto& r_model_part = model.CreateModelPart("MainModelPart");
        ...
        printf("%.10f\n", r_model_part.GetNode(4223).GetValue(ARTIFICIAL_CONDUCTIVITY));
        printf("%.10f\n", r_model_part.GetNode(4364).GetValue(ARTIFICIAL_CONDUCTIVITY));
        printf("%.10f\n", r_model_part.GetNode(7131).GetValue(ARTIFICIAL_BULK_VISCOSITY));
        printf("%.10f\n", r_model_part.GetNode(5309).GetValue(ARTIFICIAL_BULK_VISCOSITY));
        printf("%.10f\n", r_model_part.GetNode(4810).GetValue(ARTIFICIAL_DYNAMIC_VISCOSITY));
        printf("%.10f\n", r_model_part.GetNode(6000).GetValue(ARTIFICIAL_DYNAMIC_VISCOSITY));
