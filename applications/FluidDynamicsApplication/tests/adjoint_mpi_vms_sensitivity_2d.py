...
class AdjointMPIVMSSensitivity(KratosUnittest.TestCase):
    ...
    def testSlipCylinder(self):
        with ControlledExecutionScope(os.path.dirname(os.path.realpath(__file__))):
            # solve fluid
            model = Kratos.Model()
            with open('AdjointVMSSensitivity2DTest/mpi_cylinder_slip_test_parameters.json', 'r') as parameter_file:
                project_parameters = Kratos.Parameters(parameter_file.read())
                parameter_file.close()
            primal_simulation = FluidDynamicsAnalysis(model,project_parameters)
            primal_simulation.Run()
            ...
            # solve adjoint
            with open('AdjointVMSSensitivity2DTest/mpi_cylinder_slip_test_adjoint_parameters.json', 'r') as parameter_file:
                project_parameters = Kratos.Parameters(parameter_file.read())
                parameter_file.close()
