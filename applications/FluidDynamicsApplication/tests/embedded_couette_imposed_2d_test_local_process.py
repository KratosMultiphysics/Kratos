...
def Factory(settings, Model):
    ...
    return EmbeddedCouetteImposed2DTestLocalProcess(Model, settings["Parameters"])

class EmbeddedCouetteImposed2DTestLocalProcess(KratosMultiphysics.Process, KratosUnittest.TestCase):
    ...
    def ExecuteInitialize(self):
        ...
        for node in self.fluid_model_part.Nodes:
            node_distance = self.distance-node.Y
        ...
        for elem in self.fluid_model_part.Elements:
            ...
            elif (interior<3) and (interior>0):
        ...
        for node in self.fluid_model_part.GetSubModelPart("Inlet2D").Nodes:
            if node.GetSolutionStepValue(KratosMultiphysics.DISTANCE) > 0.0:
                ...
                velocity[0] = (self.level_set_velocity/self.distance)*node.Y
                velocity[1] = 0.0
                velocity[2] = 0.0
    ...
    def ExecuteFinalize(self):
        ...
        for node in self.fluid_model_part.Nodes:
            if node.GetSolutionStepValue(KratosMultiphysics.DISTANCE) > 0.0:
                ...
                expected_solution = (self.level_set_velocity/self.distance)*node.Y
