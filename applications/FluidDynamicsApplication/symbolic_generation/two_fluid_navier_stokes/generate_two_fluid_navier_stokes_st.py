...
templatefile = open("two_fluid_navier_stokes_template_st.cpp")
...
for dim in dim_vector:
    ...
    penr2 = DefineVector('penr2',nnodes)	        # Enriched Pressure
    ...
    fst = DefineVector('fst',dim)               # Forcing term according to surface tension
    ...
    sigma = Symbol('sigma', positive = True) #surface tension coefficient
    ...
    #tau1 = 1.0/((rho*dyn_tau)/dt + (stab_c2*rho*vconv_gauss_norm)/h + (stab_c1*mu)/(h*h) + K_darcy + 2*sigma*dt)   # Stabilization parameter 1
    ...
    grad_penr2 = DNenr.transpose()*penr2
    ...
    #vel_residual = fst + rho*f_gauss - rho*accel_gauss - rho*convective_term.transpose() - grad_p - K_darcy*v_gauss - grad_penr
    vel_residual = rho*f_gauss - rho*accel_gauss - rho*convective_term.transpose() - grad_p - K_darcy*v_gauss - grad_penr2
    ...
    #vel_residual_enr = fst + rho*f_gauss - rho*(accel_gauss + convective_term.transpose()) - grad_p - K_darcy*v_gauss - grad_penr
    vel_residual_enr = rho*f_gauss - rho*(accel_gauss + convective_term.transpose()) - grad_p - K_darcy*v_gauss - grad_penr2
