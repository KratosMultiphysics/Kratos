...
templatefile = open("two_fluid_navier_stokes_alpha_method_template.cpp")
...
for dim in dim_vector:
    ...
    pn = DefineVector('pn',nnodes)              # Pressure
    ...
    #accelation from previous step saved
    ...
    tau2 = stab_c1*mu/stab_c2 + rho*vconv_gauss_norm*h                                    # Stabilization parameter 2
    ...
    # Galerkin functional at n+alpha
    ...
    if (divide_by_rho):
        ...
        mas_residual_stabilization=-div_v_stabilization[0,0]+volume_error_time_ratio
    else:
        mas_residual = -rho*div_v[0,0] + rho*volume_error_time_ratio
        mas_residual_stabilization=-div_v_stabilization[0,0]+volume_error_time_ratio

    vel_subscale = tau1*vel_residual
    mas_subscale = tau2*mas_residual_stabilization
...
out = open("two_fluid_navier_stokes_alpha_method.cpp",'w')
out.write(outstring)
out.close()
