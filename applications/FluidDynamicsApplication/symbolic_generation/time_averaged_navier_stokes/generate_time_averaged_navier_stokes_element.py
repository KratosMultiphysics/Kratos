...
templatefile = open("time_averaged_navier_stokes_cpp_template.cpp")
...
for dim in dim_vector:
    ...
    ## Unknown time averaged fields definition
    v_ave =    DefineMatrix('v_ave',nnodes,dim)         # Averaged current step velocity (v_ave(i,j) refers to velocity of node i component j)
    vn_ave =   DefineMatrix('vn_ave',nnodes,dim)        # Averaged previous step velocity
    vnn_ave =  DefineMatrix('vnn_ave',nnodes,dim)       # Averaged 2 previous step velocity
    vnnn_ave = DefineMatrix('vnnn_ave',nnodes,dim)      # Averaged 3 previous step velocity
    p_ave =    DefineVector('p_ave',nnodes)             # Averaged pressure
    pn_ave =   DefineVector('pn_ave',nnodes)            # Averaged pevious step pressure
