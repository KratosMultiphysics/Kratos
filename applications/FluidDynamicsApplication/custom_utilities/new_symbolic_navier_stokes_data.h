...
#if !defined(KRATOS_NEW_SYMBOLIC_NAVIER_STOKES_DATA_H)
#define KRATOS_NEW_SYMBOLIC_NAVIER_STOKES_DATA_H
...
class NewSymbolicNavierStokesData : public FluidElementData<TDim,TNumNodes, true>
{
public:
...
void Initialize(const Element& rElement, const ProcessInfo& rProcessInfo) override
{
    ...
    this->FillFromNodalData(SoundVelocity,SOUND_VELOCITY,r_geometry);
...
static int Check(const Element& rElement, const ProcessInfo& rProcessInfo)
{
    ...
    for (unsigned int i = 0; i < TNumNodes; i++)
    {
        ...
        KRATOS_CHECK_VARIABLE_IN_NODAL_DATA(SOUND_VELOCITY, r_geometry[i]);
    }
