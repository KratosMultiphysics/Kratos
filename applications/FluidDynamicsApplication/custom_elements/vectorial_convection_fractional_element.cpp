...
const double crhs1 = N[0]*(vn(0,0) - vnn(0,0));
const double crhs2 = N[1]*(vn(1,0) - vnn(1,0));
const double crhs3 = N[2]*(vn(2,0) - vnn(2,0));
const double crhs4 = crhs1 + crhs2 + crhs3;
const double crhs5 = N[0]*(bdf0*vfrac(0,0) + bdf1*vn(0,0) + bdf2*vnn(0,0)) + N[1]*(bdf0*vfrac(1,0) + bdf1*vn(1,0) + bdf2*vnn(1,0)) + N[2]*(bdf0*vfrac(2,0) + bdf1*vn(2,0) + bdf2*vnn(2,0));
const double crhs6 = N[0]*vconv(0,0) + N[1]*vconv(1,0) + N[2]*vconv(2,0);
const double crhs7 = N[0]*vconv(0,1) + N[1]*vconv(1,1) + N[2]*vconv(2,1);
const double crhs8 = crhs6*(DN(0,0)*vfrac(0,0) + DN(1,0)*vfrac(1,0) + DN(2,0)*vfrac(2,0)) + crhs7*(DN(0,1)*vfrac(0,0) + DN(1,1)*vfrac(1,0) + DN(2,1)*vfrac(2,0));
const double crhs9 = N[0]*vn(0,0) + N[1]*vn(1,0) + N[2]*vn(2,0);
const double crhs10 = crhs9*(DN(0,0)*vn(0,0) + DN(1,0)*vn(1,0) + DN(2,0)*vn(2,0));
const double crhs11 = N[0]*vn(0,1) + N[1]*vn(1,1) + N[2]*vn(2,1);
const double crhs12 = crhs11*(DN(0,1)*vn(0,0) + DN(1,1)*vn(1,0) + DN(2,1)*vn(2,0));
const double crhs13 = crhs10 + crhs12;
const double crhs14 = -crhs0*crhs1 - crhs0*crhs2 - crhs0*crhs3 - crhs10 - crhs12 + crhs5 + crhs8;
const double crhs15 = 1.0*1.0/(crhs0*dyn_tau + stab_c2*1.0/h*sqrt(crhs6*crhs6 + crhs7*crhs7));
const double crhs16 = crhs15*(DN(0,0)*vconv(0,0) + DN(0,1)*vconv(0,1) + DN(1,0)*vconv(1,0) + DN(1,1)*vconv(1,1) + DN(2,0)*vconv(2,0) + DN(2,1)*vconv(2,1));
const double crhs17 = N[0]*crhs16;
const double crhs18 = crhs15*(DN(0,0)*crhs6 + DN(0,1)*crhs7);
const double crhs19 = N[0]*(vn(0,1) - vnn(0,1));
const double crhs20 = N[1]*(vn(1,1) - vnn(1,1));
const double crhs21 = N[2]*(vn(2,1) - vnn(2,1));
const double crhs22 = crhs19 + crhs20 + crhs21;
const double crhs23 = N[0]*(bdf0*vfrac(0,1) + bdf1*vn(0,1) + bdf2*vnn(0,1)) + N[1]*(bdf0*vfrac(1,1) + bdf1*vn(1,1) + bdf2*vnn(1,1)) + N[2]*(bdf0*vfrac(2,1) + bdf1*vn(2,1) + bdf2*vnn(2,1));
const double crhs24 = crhs6*(DN(0,0)*vfrac(0,1) + DN(1,0)*vfrac(1,1) + DN(2,0)*vfrac(2,1)) + crhs7*(DN(0,1)*vfrac(0,1) + DN(1,1)*vfrac(1,1) + DN(2,1)*vfrac(2,1));
const double crhs25 = crhs9*(DN(0,0)*vn(0,1) + DN(1,0)*vn(1,1) + DN(2,0)*vn(2,1));
const double crhs26 = crhs11*(DN(0,1)*vn(0,1) + DN(1,1)*vn(1,1) + DN(2,1)*vn(2,1));
const double crhs27 = crhs25 + crhs26;
const double crhs28 = -crhs0*crhs19 - crhs0*crhs20 - crhs0*crhs21 + crhs23 + crhs24 - crhs25 - crhs26;
const double crhs29 = N[1]*crhs16;
const double crhs30 = crhs15*(DN(1,0)*crhs6 + DN(1,1)*crhs7);
const double crhs31 = N[2]*crhs16;
const double crhs32 = crhs15*(DN(2,0)*crhs6 + DN(2,1)*crhs7);
rhs[0]=N[0]*crhs0*crhs4 + N[0]*crhs13 - N[0]*crhs5 - N[0]*crhs8 - crhs14*crhs17 - crhs14*crhs18;
rhs[1]=N[0]*crhs0*crhs22 - N[0]*crhs23 - N[0]*crhs24 + N[0]*crhs27 - crhs17*crhs28 - crhs18*crhs28;
rhs[2]=N[1]*crhs0*crhs4 + N[1]*crhs13 - N[1]*crhs5 - N[1]*crhs8 - crhs14*crhs29 - crhs14*crhs30;
rhs[3]=N[1]*crhs0*crhs22 - N[1]*crhs23 - N[1]*crhs24 + N[1]*crhs27 - crhs28*crhs29 - crhs28*crhs30;
rhs[4]=N[2]*crhs0*crhs4 + N[2]*crhs13 - N[2]*crhs5 - N[2]*crhs8 - crhs14*crhs31 - crhs14*crhs32;
rhs[5]=N[2]*crhs0*crhs22 - N[2]*crhs23 - N[2]*crhs24 + N[2]*crhs27 - crhs28*crhs31 - crhs28*crhs32;
...
const double crhs2 = N[1]*(vn(1,0) - vnn(1,0));
const double crhs3 = N[2]*(vn(2,0) - vnn(2,0));
const double crhs4 = N[3]*(vn(3,0) - vnn(3,0));
const double crhs5 = crhs1 + crhs2 + crhs3 + crhs4;
const double crhs6 = N[0]*(bdf0*vfrac(0,0) + bdf1*vn(0,0) + bdf2*vnn(0,0)) + N[1]*(bdf0*vfrac(1,0) + bdf1*vn(1,0) + bdf2*vnn(1,0)) + N[2]*(bdf0*vfrac(2,0) + bdf1*vn(2,0) + bdf2*vnn(2,0)) + N[3]*(bdf0*vfrac(3,0) + bdf1*vn(3,0) + bdf2*vnn(3,0));
...
const double crhs10 = crhs7*(DN(0,0)*vfrac(0,0) + DN(1,0)*vfrac(1,0) + DN(2,0)*vfrac(2,0) + DN(3,0)*vfrac(3,0)) + crhs8*(DN(0,1)*vfrac(0,0) + DN(1,1)*vfrac(1,0) + DN(2,1)*vfrac(2,0) + DN(3,1)*vfrac(3,0)) + crhs9*(DN(0,2)*vfrac(0,0) + DN(1,2)*vfrac(1,0) + DN(2,2)*vfrac(2,0) + DN(3,2)*vfrac(3,0));
const double crhs11 = N[0]*vn(0,0) + N[1]*vn(1,0) + N[2]*vn(2,0) + N[3]*vn(3,0);
const double crhs12 = crhs11*(DN(0,0)*vn(0,0) + DN(1,0)*vn(1,0) + DN(2,0)*vn(2,0) + DN(3,0)*vn(3,0));
const double crhs13 = N[0]*vn(0,1) + N[1]*vn(1,1) + N[2]*vn(2,1) + N[3]*vn(3,1);
const double crhs14 = crhs13*(DN(0,1)*vn(0,0) + DN(1,1)*vn(1,0) + DN(2,1)*vn(2,0) + DN(3,1)*vn(3,0));
const double crhs15 = N[0]*vn(0,2) + N[1]*vn(1,2) + N[2]*vn(2,2) + N[3]*vn(3,2);
const double crhs16 = crhs15*(DN(0,2)*vn(0,0) + DN(1,2)*vn(1,0) + DN(2,2)*vn(2,0) + DN(3,2)*vn(3,0));
const double crhs17 = crhs12 + crhs14 + crhs16;
const double crhs18 = -crhs0*crhs1 - crhs0*crhs2 - crhs0*crhs3 - crhs0*crhs4 + crhs10 - crhs12 - crhs14 - crhs16 + crhs6;
const double crhs19 = 1.0*1.0/(crhs0*dyn_tau + stab_c2*1.0/h*sqrt(crhs7*crhs7 + crhs8*crhs8 + crhs9*crhs9));
const double crhs20 = crhs19*(DN(0,0)*vconv(0,0) + DN(0,1)*vconv(0,1) + DN(0,2)*vconv(0,2) + DN(1,0)*vconv(1,0) + DN(1,1)*vconv(1,1) + DN(1,2)*vconv(1,2) + DN(2,0)*vconv(2,0) + DN(2,1)*vconv(2,1) + DN(2,2)*vconv(2,2) + DN(3,0)*vconv(3,0) + DN(3,1)*vconv(3,1) + DN(3,2)*vconv(3,2));
const double crhs21 = N[0]*crhs20;
const double crhs22 = crhs19*(DN(0,0)*crhs7 + DN(0,1)*crhs8 + DN(0,2)*crhs9);
const double crhs23 = N[0]*(vn(0,1) - vnn(0,1));
const double crhs24 = N[1]*(vn(1,1) - vnn(1,1));
const double crhs25 = N[2]*(vn(2,1) - vnn(2,1));
const double crhs26 = N[3]*(vn(3,1) - vnn(3,1));
const double crhs27 = crhs23 + crhs24 + crhs25 + crhs26;
const double crhs28 = N[0]*(bdf0*vfrac(0,1) + bdf1*vn(0,1) + bdf2*vnn(0,1)) + N[1]*(bdf0*vfrac(1,1) + bdf1*vn(1,1) + bdf2*vnn(1,1)) + N[2]*(bdf0*vfrac(2,1) + bdf1*vn(2,1) + bdf2*vnn(2,1)) + N[3]*(bdf0*vfrac(3,1) + bdf1*vn(3,1) + bdf2*vnn(3,1));
const double crhs29 = crhs7*(DN(0,0)*vfrac(0,1) + DN(1,0)*vfrac(1,1) + DN(2,0)*vfrac(2,1) + DN(3,0)*vfrac(3,1)) + crhs8*(DN(0,1)*vfrac(0,1) + DN(1,1)*vfrac(1,1) + DN(2,1)*vfrac(2,1) + DN(3,1)*vfrac(3,1)) + crhs9*(DN(0,2)*vfrac(0,1) + DN(1,2)*vfrac(1,1) + DN(2,2)*vfrac(2,1) + DN(3,2)*vfrac(3,1));
const double crhs30 = crhs11*(DN(0,0)*vn(0,1) + DN(1,0)*vn(1,1) + DN(2,0)*vn(2,1) + DN(3,0)*vn(3,1));
const double crhs31 = crhs13*(DN(0,1)*vn(0,1) + DN(1,1)*vn(1,1) + DN(2,1)*vn(2,1) + DN(3,1)*vn(3,1));
const double crhs32 = crhs15*(DN(0,2)*vn(0,1) + DN(1,2)*vn(1,1) + DN(2,2)*vn(2,1) + DN(3,2)*vn(3,1));
const double crhs33 = crhs30 + crhs31 + crhs32;
const double crhs34 = -crhs0*crhs23 - crhs0*crhs24 - crhs0*crhs25 - crhs0*crhs26 + crhs28 + crhs29 - crhs30 - crhs31 - crhs32;
const double crhs35 = N[0]*(vn(0,2) - vnn(0,2));
const double crhs36 = N[1]*(vn(1,2) - vnn(1,2));
const double crhs37 = N[2]*(vn(2,2) - vnn(2,2));
const double crhs38 = N[3]*(vn(3,2) - vnn(3,2));
const double crhs39 = crhs35 + crhs36 + crhs37 + crhs38;
const double crhs40 = N[0]*(bdf0*vfrac(0,2) + bdf1*vn(0,2) + bdf2*vnn(0,2)) + N[1]*(bdf0*vfrac(1,2) + bdf1*vn(1,2) + bdf2*vnn(1,2)) + N[2]*(bdf0*vfrac(2,2) + bdf1*vn(2,2) + bdf2*vnn(2,2)) + N[3]*(bdf0*vfrac(3,2) + bdf1*vn(3,2) + bdf2*vnn(3,2));
const double crhs41 = crhs7*(DN(0,0)*vfrac(0,2) + DN(1,0)*vfrac(1,2) + DN(2,0)*vfrac(2,2) + DN(3,0)*vfrac(3,2)) + crhs8*(DN(0,1)*vfrac(0,2) + DN(1,1)*vfrac(1,2) + DN(2,1)*vfrac(2,2) + DN(3,1)*vfrac(3,2)) + crhs9*(DN(0,2)*vfrac(0,2) + DN(1,2)*vfrac(1,2) + DN(2,2)*vfrac(2,2) + DN(3,2)*vfrac(3,2));
const double crhs42 = crhs11*(DN(0,0)*vn(0,2) + DN(1,0)*vn(1,2) + DN(2,0)*vn(2,2) + DN(3,0)*vn(3,2));
const double crhs43 = crhs13*(DN(0,1)*vn(0,2) + DN(1,1)*vn(1,2) + DN(2,1)*vn(2,2) + DN(3,1)*vn(3,2));
const double crhs44 = crhs15*(DN(0,2)*vn(0,2) + DN(1,2)*vn(1,2) + DN(2,2)*vn(2,2) + DN(3,2)*vn(3,2));
const double crhs45 = crhs42 + crhs43 + crhs44;
const double crhs46 = -crhs0*crhs35 - crhs0*crhs36 - crhs0*crhs37 - crhs0*crhs38 + crhs40 + crhs41 - crhs42 - crhs43 - crhs44;
const double crhs47 = N[1]*crhs20;
const double crhs48 = crhs19*(DN(1,0)*crhs7 + DN(1,1)*crhs8 + DN(1,2)*crhs9);
const double crhs49 = N[2]*crhs20;
const double crhs50 = crhs19*(DN(2,0)*crhs7 + DN(2,1)*crhs8 + DN(2,2)*crhs9);
const double crhs51 = N[3]*crhs20;
const double crhs52 = crhs19*(DN(3,0)*crhs7 + DN(3,1)*crhs8 + DN(3,2)*crhs9);
rhs[0]=N[0]*crhs0*crhs5 - N[0]*crhs10 + N[0]*crhs17 - N[0]*crhs6 - crhs18*crhs21 - crhs18*crhs22;
rhs[1]=N[0]*crhs0*crhs27 - N[0]*crhs28 - N[0]*crhs29 + N[0]*crhs33 - crhs21*crhs34 - crhs22*crhs34;
rhs[2]=N[0]*crhs0*crhs39 - N[0]*crhs40 - N[0]*crhs41 + N[0]*crhs45 - crhs21*crhs46 - crhs22*crhs46;
rhs[3]=N[1]*crhs0*crhs5 - N[1]*crhs10 + N[1]*crhs17 - N[1]*crhs6 - crhs18*crhs47 - crhs18*crhs48;
rhs[4]=N[1]*crhs0*crhs27 - N[1]*crhs28 - N[1]*crhs29 + N[1]*crhs33 - crhs34*crhs47 - crhs34*crhs48;
rhs[5]=N[1]*crhs0*crhs39 - N[1]*crhs40 - N[1]*crhs41 + N[1]*crhs45 - crhs46*crhs47 - crhs46*crhs48;
rhs[6]=N[2]*crhs0*crhs5 - N[2]*crhs10 + N[2]*crhs17 - N[2]*crhs6 - crhs18*crhs49 - crhs18*crhs50;
rhs[7]=N[2]*crhs0*crhs27 - N[2]*crhs28 - N[2]*crhs29 + N[2]*crhs33 - crhs34*crhs49 - crhs34*crhs50;
rhs[8]=N[2]*crhs0*crhs39 - N[2]*crhs40 - N[2]*crhs41 + N[2]*crhs45 - crhs46*crhs49 - crhs46*crhs50;
rhs[9]=N[3]*crhs0*crhs5 - N[3]*crhs10 + N[3]*crhs17 - N[3]*crhs6 - crhs18*crhs51 - crhs18*crhs52;
rhs[10]=N[3]*crhs0*crhs27 - N[3]*crhs28 - N[3]*crhs29 + N[3]*crhs33 - crhs34*crhs51 - crhs34*crhs52;
rhs[11]=N[3]*crhs0*crhs39 - N[3]*crhs40 - N[3]*crhs41 + N[3]*crhs45 - crhs46*crhs51 - crhs46*crhs52;
