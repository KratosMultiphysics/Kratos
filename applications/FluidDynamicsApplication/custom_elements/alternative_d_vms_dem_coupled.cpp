    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int g = 0; g < number_of_integration_points; g++ ) {
        ...
        if (rVariable == FLUID_FRACTION_GRADIENT){
            mExactPorosityGradient[g] = rOutput[g];
        }
    }
}
    ...
    array_1d<double,3>& rResidual) const
{
    ...
    for (unsigned int d = 0; d < Dim; d++) {
        fluid_fraction_gradient[d] += mExactPorosityGradient[rData.IntegrationPointIndex][d];
    }
    ...
    double &TauTwo) const
{
    ...
    //const double epsilon = 0.0 * 1e-3 * tau_one/std::pow(rData.ElementSize,2);
    TauOne = tau_one * I;
    //TauTwo = std::pow(std::pow(std::pow(h/p,2.0) / (c1 * fluid_fraction * tau_one_NS),-1.0) + epsilon,-1.0);
