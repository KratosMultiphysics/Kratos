...
const double crLHS8 = rho*stab_c2*sqrt(pow(crLHS5, 2) + pow(crLHS6, 2) + pow(crLHS7, 2));
...
const double crLHS20 = 1.0/(crLHS8/h + dyn_tau*rho/rData.DeltaTime + mu*stab_c1/pow(h, 2));
...
const double crLHS25 = pow(N_v[0], 2)*crLHS16 + N_v[0]*crLHS15;
...
const double crLHS687 = pow(N_v[1], 2)*crLHS16 + N_v[1]*crLHS53;
...
const double crLHS799 = pow(N_v[2], 2)*crLHS16 + N_v[2]*crLHS85;
...
const double crLHS901 = pow(N_v[3], 2)*crLHS16 + N_v[3]*crLHS117;
...
const double crLHS993 = pow(N_v[4], 2)*crLHS16 + N_v[4]*crLHS149;
...
const double crLHS1075 = pow(N_v[5], 2)*crLHS16 + N_v[5]*crLHS181;
...
const double crLHS1147 = pow(N_v[6], 2)*crLHS16 + N_v[6]*crLHS213;
...
const double crLHS1209 = pow(N_v[7], 2)*crLHS16 + N_v[7]*crLHS245;
...
const double crLHS1261 = pow(N_v[8], 2)*crLHS16 + N_v[8]*crLHS277;
...
const double crLHS1303 = pow(N_v[9], 2)*crLHS16 + N_v[9]*crLHS309;
...
rLHS(0,0)+=gauss_weight*(pow(DN_v(0,0), 2)*crLHS9 + DN_v(0,0)*crLHS0 + DN_v(0,1)*crLHS2 + DN_v(0,2)*crLHS4 - crLHS19*crLHS21 - crLHS19*crLHS24 + crLHS25);
...
rLHS(1,1)+=gauss_weight*(DN_v(0,0)*crLHS28 + pow(DN_v(0,1), 2)*crLHS9 + DN_v(0,1)*crLHS343 + DN_v(0,2)*crLHS345 - crLHS21*crLHS350 - crLHS24*crLHS350 + crLHS25);
...
rLHS(2,2)+=gauss_weight*(DN_v(0,0)*crLHS40 + DN_v(0,1)*crLHS353 + pow(DN_v(0,2), 2)*crLHS9 + DN_v(0,2)*crLHS543 - crLHS21*crLHS547 - crLHS24*crLHS547 + crLHS25);
...
rLHS(3,3)+=gauss_weight*(pow(DN_v(1,0), 2)*crLHS9 + DN_v(1,0)*crLHS43 + DN_v(1,1)*crLHS45 + DN_v(1,2)*crLHS47 - crLHS56*crLHS683 - crLHS56*crLHS685 + crLHS687);
...
rLHS(4,4)+=gauss_weight*(DN_v(1,0)*crLHS61 + pow(DN_v(1,1), 2)*crLHS9 + DN_v(1,1)*crLHS363 + DN_v(1,2)*crLHS365 - crLHS370*crLHS683 - crLHS370*crLHS685 + crLHS687);
...
rLHS(5,5)+=gauss_weight*(DN_v(1,0)*crLHS72 + DN_v(1,1)*crLHS373 + pow(DN_v(1,2), 2)*crLHS9 + DN_v(1,2)*crLHS558 - crLHS562*crLHS683 - crLHS562*crLHS685 + crLHS687);
...
rLHS(6,6)+=gauss_weight*(pow(DN_v(2,0), 2)*crLHS9 + DN_v(2,0)*crLHS75 + DN_v(2,1)*crLHS77 + DN_v(2,2)*crLHS79 - crLHS794*crLHS88 - crLHS796*crLHS88 + crLHS799);
...
rLHS(7,7)+=gauss_weight*(DN_v(2,0)*crLHS93 + pow(DN_v(2,1), 2)*crLHS9 + DN_v(2,1)*crLHS382 + DN_v(2,2)*crLHS384 - crLHS389*crLHS794 - crLHS389*crLHS796 + crLHS799);
...
rLHS(8,8)+=gauss_weight*(DN_v(2,0)*crLHS104 + DN_v(2,1)*crLHS392 + pow(DN_v(2,2), 2)*crLHS9 + DN_v(2,2)*crLHS572 - crLHS576*crLHS794 - crLHS576*crLHS796 + crLHS799);
...
rLHS(9,9)+=gauss_weight*(pow(DN_v(3,0), 2)*crLHS9 + DN_v(3,0)*crLHS107 + DN_v(3,1)*crLHS109 + DN_v(3,2)*crLHS111 - crLHS120*crLHS895 - crLHS120*crLHS897 + crLHS901);
...
rLHS(10,10)+=gauss_weight*(DN_v(3,0)*crLHS125 + pow(DN_v(3,1), 2)*crLHS9 + DN_v(3,1)*crLHS401 + DN_v(3,2)*crLHS403 - crLHS408*crLHS895 - crLHS408*crLHS897 + crLHS901);
...
rLHS(11,11)+=gauss_weight*(DN_v(3,0)*crLHS136 + DN_v(3,1)*crLHS411 + pow(DN_v(3,2), 2)*crLHS9 + DN_v(3,2)*crLHS586 - crLHS590*crLHS895 - crLHS590*crLHS897 + crLHS901);
...
rLHS(12,12)+=gauss_weight*(pow(DN_v(4,0), 2)*crLHS9 + DN_v(4,0)*crLHS139 + DN_v(4,1)*crLHS141 + DN_v(4,2)*crLHS143 - crLHS152*crLHS986 - crLHS152*crLHS988 + crLHS993);
...
rLHS(13,13)+=gauss_weight*(DN_v(4,0)*crLHS157 + pow(DN_v(4,1), 2)*crLHS9 + DN_v(4,1)*crLHS420 + DN_v(4,2)*crLHS422 - crLHS427*crLHS986 - crLHS427*crLHS988 + crLHS993);
...
rLHS(14,14)+=gauss_weight*(DN_v(4,0)*crLHS168 + DN_v(4,1)*crLHS430 + pow(DN_v(4,2), 2)*crLHS9 + DN_v(4,2)*crLHS600 - crLHS604*crLHS986 - crLHS604*crLHS988 + crLHS993);
...
rLHS(15,15)+=gauss_weight*(pow(DN_v(5,0), 2)*crLHS9 + DN_v(5,0)*crLHS171 + DN_v(5,1)*crLHS173 + DN_v(5,2)*crLHS175 - crLHS1067*crLHS184 - crLHS1069*crLHS184 + crLHS1075);
...
rLHS(16,16)+=gauss_weight*(DN_v(5,0)*crLHS189 + pow(DN_v(5,1), 2)*crLHS9 + DN_v(5,1)*crLHS439 + DN_v(5,2)*crLHS441 - crLHS1067*crLHS446 - crLHS1069*crLHS446 + crLHS1075);
...
rLHS(17,17)+=gauss_weight*(DN_v(5,0)*crLHS200 + DN_v(5,1)*crLHS449 + pow(DN_v(5,2), 2)*crLHS9 + DN_v(5,2)*crLHS614 - crLHS1067*crLHS618 - crLHS1069*crLHS618 + crLHS1075);
...
rLHS(18,18)+=gauss_weight*(pow(DN_v(6,0), 2)*crLHS9 + DN_v(6,0)*crLHS203 + DN_v(6,1)*crLHS205 + DN_v(6,2)*crLHS207 - crLHS1138*crLHS216 - crLHS1140*crLHS216 + crLHS1147);
...
rLHS(19,19)+=gauss_weight*(DN_v(6,0)*crLHS221 + pow(DN_v(6,1), 2)*crLHS9 + DN_v(6,1)*crLHS458 + DN_v(6,2)*crLHS460 - crLHS1138*crLHS465 - crLHS1140*crLHS465 + crLHS1147);
...
rLHS(20,20)+=gauss_weight*(DN_v(6,0)*crLHS232 + DN_v(6,1)*crLHS468 + pow(DN_v(6,2), 2)*crLHS9 + DN_v(6,2)*crLHS628 - crLHS1138*crLHS632 - crLHS1140*crLHS632 + crLHS1147);
...
rLHS(21,21)+=gauss_weight*(pow(DN_v(7,0), 2)*crLHS9 + DN_v(7,0)*crLHS235 + DN_v(7,1)*crLHS237 + DN_v(7,2)*crLHS239 - crLHS1199*crLHS248 - crLHS1201*crLHS248 + crLHS1209);
...
rLHS(22,22)+=gauss_weight*(DN_v(7,0)*crLHS253 + pow(DN_v(7,1), 2)*crLHS9 + DN_v(7,1)*crLHS477 + DN_v(7,2)*crLHS479 - crLHS1199*crLHS484 - crLHS1201*crLHS484 + crLHS1209);
...
rLHS(23,23)+=gauss_weight*(DN_v(7,0)*crLHS264 + DN_v(7,1)*crLHS487 + pow(DN_v(7,2), 2)*crLHS9 + DN_v(7,2)*crLHS642 - crLHS1199*crLHS646 - crLHS1201*crLHS646 + crLHS1209);
...
rLHS(24,24)+=gauss_weight*(pow(DN_v(8,0), 2)*crLHS9 + DN_v(8,0)*crLHS267 + DN_v(8,1)*crLHS269 + DN_v(8,2)*crLHS271 - crLHS1250*crLHS280 - crLHS1252*crLHS280 + crLHS1261);
...
rLHS(25,25)+=gauss_weight*(DN_v(8,0)*crLHS285 + pow(DN_v(8,1), 2)*crLHS9 + DN_v(8,1)*crLHS496 + DN_v(8,2)*crLHS498 - crLHS1250*crLHS503 - crLHS1252*crLHS503 + crLHS1261);
...
rLHS(26,26)+=gauss_weight*(DN_v(8,0)*crLHS296 + DN_v(8,1)*crLHS506 + pow(DN_v(8,2), 2)*crLHS9 + DN_v(8,2)*crLHS656 - crLHS1250*crLHS660 - crLHS1252*crLHS660 + crLHS1261);
...
rLHS(27,27)+=gauss_weight*(pow(DN_v(9,0), 2)*crLHS9 + DN_v(9,0)*crLHS299 + DN_v(9,1)*crLHS301 + DN_v(9,2)*crLHS303 - crLHS1291*crLHS311 - crLHS1293*crLHS311 + crLHS1303);
...
rLHS(28,28)+=gauss_weight*(DN_v(9,0)*crLHS316 + pow(DN_v(9,1), 2)*crLHS9 + DN_v(9,1)*crLHS515 + DN_v(9,2)*crLHS517 - crLHS1291*crLHS522 - crLHS1293*crLHS522 + crLHS1303);
...
rLHS(29,29)+=gauss_weight*(DN_v(9,0)*crLHS327 + DN_v(9,1)*crLHS525 + pow(DN_v(9,2), 2)*crLHS9 + DN_v(9,2)*crLHS670 - crLHS1291*crLHS674 - crLHS1293*crLHS674 + crLHS1303);
...
rLHS(30,30)+=crLHS1320*(pow(DN_p(0,0), 2) + pow(DN_p(0,1), 2) + pow(DN_p(0,2), 2));
...
rLHS(31,31)+=crLHS1320*(pow(DN_p(1,0), 2) + pow(DN_p(1,1), 2) + pow(DN_p(1,2), 2));
...
rLHS(32,32)+=crLHS1320*(pow(DN_p(2,0), 2) + pow(DN_p(2,1), 2) + pow(DN_p(2,2), 2));
...
rLHS(33,33)+=crLHS1320*(pow(DN_p(3,0), 2) + pow(DN_p(3,1), 2) + pow(DN_p(3,2), 2));
...
const double crRHS11 = rho*stab_c2*sqrt(pow(crRHS4, 2) + pow(crRHS5, 2) + pow(crRHS6, 2));
...
const double crRHS88 = 1.0/(crRHS11/h + dyn_tau*rho/rData.DeltaTime + mu*stab_c1/pow(h, 2));
