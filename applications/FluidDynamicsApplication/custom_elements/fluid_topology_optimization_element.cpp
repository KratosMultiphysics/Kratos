...
const double crRHS25 = crRHS20*crRHS24;
...
const double crRHS45 = crRHS11*crRHS3;
...
const double crRHS52 = crRHS18*crRHS39 + crRHS20*crRHS51;
...
const double crRHS57 = crRHS11*crRHS39 + crRHS46*crRHS51;
...
const double crRHS61 = crRHS18*crRHS3;
const double crRHS62 = DN(0,1)*v_ns(0,1) + DN(1,1)*v_ns(1,1) + DN(2,1)*v_ns(2,1) + DN(3,1)*v_ns(3,1);
const double crRHS63 = crRHS36*crRHS62;
const double crRHS64 = DN(0,2)*v_ns(0,1);
const double crRHS65 = DN(1,2)*v_ns(1,1);
const double crRHS66 = DN(2,2)*v_ns(2,1);
const double crRHS67 = DN(3,2)*v_ns(3,1);
const double crRHS68 = crRHS64 + crRHS65 + crRHS66 + crRHS67;
const double crRHS69 = crRHS38*crRHS68;
const double crRHS70 = crRHS11*crRHS6;
const double crRHS71 = crRHS8*(DN(0,1)*t[0] + DN(1,1)*t[1] + DN(2,1)*t[2] + DN(3,1)*t[3]);
const double crRHS72 = crRHS71*functional_weights[6];
const double crRHS73 = DN(0,1)*p_adj[0] + DN(1,1)*p_adj[1] + DN(2,1)*p_adj[2] + DN(3,1)*p_adj[3] - crRHS43*crRHS52 - crRHS51*(DN(0,0)*v_adj(0,1) + DN(1,0)*v_adj(1,1) + DN(2,0)*v_adj(2,1) + DN(3,0)*v_adj(3,1)) - crRHS53*(DN(0,2)*v_adj(0,1) + DN(1,2)*v_adj(1,1) + DN(2,2)*v_adj(2,1) + DN(3,2)*v_adj(3,1)) - crRHS59 + crRHS60 + crRHS61*rho + crRHS63*rho + crRHS69*rho + crRHS70 + crRHS71 + crRHS72;
const double crRHS74 = rho*(N[0]*f_adj(0,2) + N[1]*f_adj(1,2) + N[2]*f_adj(2,2) + N[3]*f_adj(3,2));
const double crRHS75 = crRHS2*crRHS38;
const double crRHS76 = crRHS24*crRHS3;
const double crRHS77 = DN(0,1)*v_ns(0,2) + DN(1,1)*v_ns(1,2) + DN(2,1)*v_ns(2,2) + DN(3,1)*v_ns(3,2);
...
const double crRHS84 = DN(0,2)*p_adj[0] + DN(1,2)*p_adj[1] + DN(2,2)*p_adj[2] + DN(3,2)*p_adj[3] - crRHS44*crRHS53 - crRHS51*(DN(0,0)*v_adj(0,2) + DN(1,0)*v_adj(1,2) + DN(2,0)*v_adj(2,2) + DN(3,0)*v_adj(3,2)) - crRHS52*(DN(0,1)*v_adj(0,2) + DN(1,1)*v_adj(1,2) + DN(2,1)*v_adj(2,2) + DN(3,1)*v_adj(3,2)) - crRHS74 + crRHS75 + crRHS76*rho + crRHS78*rho + crRHS80*rho + crRHS81 + crRHS82 + crRHS83;
const double crRHS85 = crRHS27*crRHS55 + crRHS29*crRHS73 + crRHS31*crRHS84;
const double crRHS86 = crRHS41*crRHS56;
const double crRHS87 = -crRHS64 - crRHS65 - crRHS66 - crRHS67 + crRHS77;
const double crRHS88 = 1.0*crRHS62;
const double crRHS89 = crRHS68 + crRHS77;
const double crRHS90 = crRHS61 + crRHS63 + crRHS69;
const double crRHS91 = crRHS56*crRHS73;
const double crRHS92 = crRHS18*crRHS55 + crRHS62*crRHS73 + crRHS68*crRHS84;
const double crRHS93 = 1.0*crRHS79;
const double crRHS94 = 0.5*crRHS32;
const double crRHS95 = 0.5*crRHS89;
const double crRHS96 = crRHS76 + crRHS78 + crRHS80;
const double crRHS97 = crRHS56*crRHS84;
const double crRHS98 = crRHS24*crRHS55 + crRHS73*crRHS77 + crRHS79*crRHS84;
...
rRHS[1]+=-gauss_weight*(DN(0,0)*stress_adj[3] - DN(0,1)*crRHS0 + DN(0,1)*crRHS50 + DN(0,1)*stress_adj[1] + DN(0,2)*stress_adj[4] - N[0]*crRHS59 + N[0]*crRHS60 + N[0]*crRHS70 + N[0]*crRHS71 + N[0]*crRHS72 + crRHS13*crRHS36 - crRHS13*crRHS91 + crRHS26*(DN(0,0)*crRHS19 - DN(0,2)*crRHS87) + crRHS34*(DN(0,0)*crRHS30 + DN(0,1)*crRHS88 + crRHS33*crRHS89) + crRHS41*crRHS90 - crRHS58*crRHS91 - crRHS86*crRHS92);
rRHS[2]+=-gauss_weight*(DN(0,0)*stress_adj[5] + DN(0,1)*stress_adj[4] - DN(0,2)*crRHS0 + DN(0,2)*crRHS50 + DN(0,2)*stress_adj[2] - N[0]*crRHS74 + N[0]*crRHS75 + N[0]*crRHS81 + N[0]*crRHS82 + N[0]*crRHS83 + crRHS13*crRHS38 - crRHS13*crRHS97 + crRHS26*(DN(0,0)*crRHS25 + DN(0,1)*crRHS87) + crRHS34*(DN(0,0)*crRHS94 + DN(0,1)*crRHS95 + DN(0,2)*crRHS93) + crRHS41*crRHS96 - crRHS58*crRHS97 - crRHS86*crRHS98);
...
rRHS[5]+=-gauss_weight*(DN(1,0)*stress_adj[3] - DN(1,1)*crRHS0 + DN(1,1)*crRHS50 + DN(1,1)*stress_adj[1] + DN(1,2)*stress_adj[4] - N[1]*crRHS59 + N[1]*crRHS60 + N[1]*crRHS70 + N[1]*crRHS71 + N[1]*crRHS72 + crRHS100*crRHS90 - crRHS101*crRHS91 - crRHS102*crRHS92 + crRHS26*(DN(1,0)*crRHS19 - DN(1,2)*crRHS87) + crRHS34*(DN(1,0)*crRHS30 + DN(1,1)*crRHS88 + DN(1,2)*crRHS95) + crRHS36*crRHS99 - crRHS91*crRHS99);
rRHS[6]+=-gauss_weight*(DN(1,0)*stress_adj[5] + DN(1,1)*stress_adj[4] - DN(1,2)*crRHS0 + DN(1,2)*crRHS50 + DN(1,2)*stress_adj[2] - N[1]*crRHS74 + N[1]*crRHS75 + N[1]*crRHS81 + N[1]*crRHS82 + N[1]*crRHS83 + crRHS100*crRHS96 - crRHS101*crRHS97 - crRHS102*crRHS98 + crRHS26*(DN(1,0)*crRHS25 + DN(1,1)*crRHS87) + crRHS34*(DN(1,0)*crRHS94 + DN(1,1)*crRHS95 + DN(1,2)*crRHS93) + crRHS38*crRHS99 - crRHS97*crRHS99);
...
rRHS[9]+=-gauss_weight*(DN(2,0)*stress_adj[3] - DN(2,1)*crRHS0 + DN(2,1)*crRHS50 + DN(2,1)*stress_adj[1] + DN(2,2)*stress_adj[4] - N[2]*crRHS59 + N[2]*crRHS60 + N[2]*crRHS70 + N[2]*crRHS71 + N[2]*crRHS72 + crRHS103*crRHS36 - crRHS103*crRHS91 + crRHS104*crRHS90 - crRHS105*crRHS91 - crRHS106*crRHS92 + crRHS26*(DN(2,0)*crRHS19 - DN(2,2)*crRHS87) + crRHS34*(DN(2,0)*crRHS30 + DN(2,1)*crRHS88 + DN(2,2)*crRHS95));
rRHS[10]+=-gauss_weight*(DN(2,0)*stress_adj[5] + DN(2,1)*stress_adj[4] - DN(2,2)*crRHS0 + DN(2,2)*crRHS50 + DN(2,2)*stress_adj[2] - N[2]*crRHS74 + N[2]*crRHS75 + N[2]*crRHS81 + N[2]*crRHS82 + N[2]*crRHS83 + crRHS103*crRHS38 - crRHS103*crRHS97 + crRHS104*crRHS96 - crRHS105*crRHS97 - crRHS106*crRHS98 + crRHS26*(DN(2,0)*crRHS25 + DN(2,1)*crRHS87) + crRHS34*(DN(2,0)*crRHS94 + DN(2,1)*crRHS95 + DN(2,2)*crRHS93));
...
rRHS[13]+=-gauss_weight*(DN(3,0)*stress_adj[3] - DN(3,1)*crRHS0 + DN(3,1)*crRHS50 + DN(3,1)*stress_adj[1] + DN(3,2)*stress_adj[4] - N[3]*crRHS59 + N[3]*crRHS60 + N[3]*crRHS70 + N[3]*crRHS71 + N[3]*crRHS72 + crRHS107*crRHS36 - crRHS107*crRHS91 + crRHS108*crRHS90 - crRHS109*crRHS91 - crRHS110*crRHS92 + crRHS26*(DN(3,0)*crRHS19 - DN(3,2)*crRHS87) + crRHS34*(DN(3,0)*crRHS30 + DN(3,1)*crRHS88 + DN(3,2)*crRHS95));
rRHS[14]+=-gauss_weight*(DN(3,0)*stress_adj[5] + DN(3,1)*stress_adj[4] - DN(3,2)*crRHS0 + DN(3,2)*crRHS50 + DN(3,2)*stress_adj[2] - N[3]*crRHS74 + N[3]*crRHS75 + N[3]*crRHS81 + N[3]*crRHS82 + N[3]*crRHS83 + crRHS107*crRHS38 - crRHS107*crRHS97 + crRHS108*crRHS96 - crRHS109*crRHS97 - crRHS110*crRHS98 + crRHS26*(DN(3,0)*crRHS25 + DN(3,1)*crRHS87) + crRHS34*(DN(3,0)*crRHS94 + DN(3,1)*crRHS95 + DN(3,2)*crRHS93));
