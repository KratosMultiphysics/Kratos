...
const double crRHS2 = N[0]*t_adj[0] + N[1]*t_adj[1] + N[2]*t_adj[2];
const double crRHS3 = crRHS2*(DN(0,0)*t[0] + DN(1,0)*t[1] + DN(2,0)*t[2]);
const double crRHS4 = N[0]*alpha[0] + N[1]*alpha[1] + N[2]*alpha[2];
const double crRHS5 = N[0]*v_adj(0,0) + N[1]*v_adj(1,0) + N[2]*v_adj(2,0);
const double crRHS6 = crRHS4*crRHS5;
const double crRHS7 = N[0]*v_ns(0,0) + N[1]*v_ns(1,0) + N[2]*v_ns(2,0);
const double crRHS8 = 2.0*crRHS4*functional_weights[0];
const double crRHS9 = crRHS7*crRHS8;
const double crRHS10 = DN(0,1)*v_ns(0,0);
const double crRHS11 = DN(1,1)*v_ns(1,0);
const double crRHS12 = DN(2,1)*v_ns(2,0);
const double crRHS13 = DN(0,0)*v_ns(0,1) + DN(1,0)*v_ns(1,1) + DN(2,0)*v_ns(2,1);
const double crRHS14 = 2.0*functional_weights[2]*mu*(-crRHS10 - crRHS11 - crRHS12 + crRHS13);
const double crRHS15 = N[0]*v_ns(0,1) + N[1]*v_ns(1,1) + N[2]*v_ns(2,1);
const double crRHS16 = rho*(DN(0,0)*crRHS7 + DN(0,1)*crRHS15);
const double crRHS17 = DN(0,0)*v_ns(0,0) + DN(1,0)*v_ns(1,0) + DN(2,0)*v_ns(2,0);
const double crRHS18 = 1.0*crRHS17;
const double crRHS19 = crRHS10 + crRHS11 + crRHS12;
const double crRHS20 = 0.5*crRHS13 + 0.5*crRHS19;
const double crRHS21 = 4.0*functional_weights[1]*mu;
const double crRHS22 = crRHS17*crRHS5;
const double crRHS23 = N[0]*v_adj(0,1) + N[1]*v_adj(1,1) + N[2]*v_adj(2,1);
const double crRHS24 = crRHS19*crRHS23;
const double crRHS25 = crRHS22 + crRHS24;
const double crRHS26 = N[0]*rho;
const double crRHS27 = DN(0,0)*v_adj(0,0) + DN(1,0)*v_adj(1,0) + DN(2,0)*v_adj(2,0);
const double crRHS28 = DN(0,1)*v_adj(0,1) + DN(1,1)*v_adj(1,1) + DN(2,1)*v_adj(2,1);
const double crRHS29 = crRHS27 + crRHS28;
const double crRHS30 = crRHS4*stab_c3;
const double crRHS31 = pow(crRHS15, 2) + pow(crRHS7, 2);
const double crRHS32 = pow(crRHS31, 1.0/4.0)*stab_c3;
const double crRHS33 = sqrt(crRHS31)*rho*stab_c2;
const double crRHS34 = crRHS29*(h*(crRHS30*h + crRHS32*h + crRHS33)/stab_c1 + mu);
const double crRHS35 = crRHS7*rho;
const double crRHS36 = crRHS15*rho;
const double crRHS37 = DN(0,0)*p_adj[0] + DN(1,0)*p_adj[1] + DN(2,0)*p_adj[2] - crRHS1 + crRHS22*rho + crRHS24*rho - crRHS27*crRHS35 + crRHS3 - crRHS36*(DN(0,1)*v_adj(0,0) + DN(1,1)*v_adj(1,0) + DN(2,1)*v_adj(2,0)) + crRHS6 + crRHS9;
const double crRHS38 = 1.0/(crRHS30 + crRHS32 + crRHS33/h + mu*stab_c1/pow(h, 2));
const double crRHS39 = crRHS37*crRHS38;
const double crRHS40 = N[0]*crRHS4;
const double crRHS41 = rho*(N[0]*f_adj(0,1) + N[1]*f_adj(1,1) + N[2]*f_adj(2,1));
const double crRHS42 = crRHS2*(DN(0,1)*t[0] + DN(1,1)*t[1] + DN(2,1)*t[2]);
const double crRHS43 = crRHS23*crRHS4;
const double crRHS44 = crRHS13*crRHS5;
const double crRHS45 = DN(0,1)*v_ns(0,1) + DN(1,1)*v_ns(1,1) + DN(2,1)*v_ns(2,1);
const double crRHS46 = crRHS23*crRHS45;
const double crRHS47 = crRHS15*crRHS8;
const double crRHS48 = DN(0,1)*p_adj[0] + DN(1,1)*p_adj[1] + DN(2,1)*p_adj[2] - crRHS28*crRHS36 - crRHS35*(DN(0,0)*v_adj(0,1) + DN(1,0)*v_adj(1,1) + DN(2,0)*v_adj(2,1)) - crRHS41 + crRHS42 + crRHS43 + crRHS44*rho + crRHS46*rho + crRHS47;
const double crRHS49 = crRHS17*crRHS37 + crRHS19*crRHS48;
const double crRHS50 = crRHS26*crRHS38;
const double crRHS51 = 1.0*crRHS45;
const double crRHS52 = crRHS44 + crRHS46;
const double crRHS53 = crRHS38*crRHS48;
const double crRHS54 = crRHS13*crRHS37 + crRHS45*crRHS48;
const double crRHS55 = rho*(DN(1,0)*crRHS7 + DN(1,1)*crRHS15);
const double crRHS56 = N[1]*rho;
const double crRHS57 = N[1]*crRHS4;
const double crRHS58 = crRHS38*crRHS56;
const double crRHS59 = rho*(DN(2,0)*crRHS7 + DN(2,1)*crRHS15);
const double crRHS60 = N[2]*rho;
const double crRHS61 = N[2]*crRHS4;
const double crRHS62 = crRHS38*crRHS60;
rRHS[0]+=-gauss_weight*(-DN(0,0)*crRHS0 + DN(0,0)*crRHS34 + DN(0,0)*stress_adj[0] - DN(0,1)*crRHS14 + DN(0,1)*stress_adj[2] - N[0]*crRHS1 + N[0]*crRHS3 + N[0]*crRHS6 + N[0]*crRHS9 - crRHS16*crRHS39 + crRHS16*crRHS5 + crRHS21*(DN(0,0)*crRHS18 + DN(0,1)*crRHS20) + crRHS25*crRHS26 - crRHS39*crRHS40 - crRHS49*crRHS50);
rRHS[1]+=-gauss_weight*(DN(0,0)*crRHS14 + DN(0,0)*stress_adj[2] - DN(0,1)*crRHS0 + DN(0,1)*crRHS34 + DN(0,1)*stress_adj[1] - N[0]*crRHS41 + N[0]*crRHS42 + N[0]*crRHS43 + N[0]*crRHS47 + crRHS16*crRHS23 - crRHS16*crRHS53 + crRHS21*(DN(0,0)*crRHS20 + DN(0,1)*crRHS51) + crRHS26*crRHS52 - crRHS40*crRHS53 - crRHS50*crRHS54);
rRHS[2]+=-gauss_weight*(DN(0,0)*crRHS39 + DN(0,1)*crRHS53 + N[0]*crRHS29);
rRHS[3]+=-gauss_weight*(-DN(1,0)*crRHS0 + DN(1,0)*crRHS34 + DN(1,0)*stress_adj[0] - DN(1,1)*crRHS14 + DN(1,1)*stress_adj[2] - N[1]*crRHS1 + N[1]*crRHS3 + N[1]*crRHS6 + N[1]*crRHS9 + crRHS21*(DN(1,0)*crRHS18 + DN(1,1)*crRHS20) + crRHS25*crRHS56 - crRHS39*crRHS55 - crRHS39*crRHS57 - crRHS49*crRHS58 + crRHS5*crRHS55);
rRHS[4]+=-gauss_weight*(DN(1,0)*crRHS14 + DN(1,0)*stress_adj[2] - DN(1,1)*crRHS0 + DN(1,1)*crRHS34 + DN(1,1)*stress_adj[1] - N[1]*crRHS41 + N[1]*crRHS42 + N[1]*crRHS43 + N[1]*crRHS47 + crRHS21*(DN(1,0)*crRHS20 + DN(1,1)*crRHS51) + crRHS23*crRHS55 + crRHS52*crRHS56 - crRHS53*crRHS55 - crRHS53*crRHS57 - crRHS54*crRHS58);
rRHS[5]+=-gauss_weight*(DN(1,0)*crRHS39 + DN(1,1)*crRHS53 + N[1]*crRHS29);
rRHS[6]+=-gauss_weight*(-DN(2,0)*crRHS0 + DN(2,0)*crRHS34 + DN(2,0)*stress_adj[0] - DN(2,1)*crRHS14 + DN(2,1)*stress_adj[2] - N[2]*crRHS1 + N[2]*crRHS3 + N[2]*crRHS6 + N[2]*crRHS9 + crRHS21*(DN(2,0)*crRHS18 + DN(2,1)*crRHS20) + crRHS25*crRHS60 - crRHS39*crRHS59 - crRHS39*crRHS61 - crRHS49*crRHS62 + crRHS5*crRHS59);
rRHS[7]+=-gauss_weight*(DN(2,0)*crRHS14 + DN(2,0)*stress_adj[2] - DN(2,1)*crRHS0 + DN(2,1)*crRHS34 + DN(2,1)*stress_adj[1] - N[2]*crRHS41 + N[2]*crRHS42 + N[2]*crRHS43 + N[2]*crRHS47 + crRHS21*(DN(2,0)*crRHS20 + DN(2,1)*crRHS51) + crRHS23*crRHS59 + crRHS52*crRHS60 - crRHS53*crRHS59 - crRHS53*crRHS61 - crRHS54*crRHS62);
rRHS[8]+=-gauss_weight*(DN(2,0)*crRHS39 + DN(2,1)*crRHS53 + N[2]*crRHS29);
...
const double crRHS2 = N[0]*t_adj[0] + N[1]*t_adj[1] + N[2]*t_adj[2] + N[3]*t_adj[3];
const double crRHS3 = crRHS2*(DN(0,0)*t[0] + DN(1,0)*t[1] + DN(2,0)*t[2] + DN(3,0)*t[3]);
const double crRHS4 = N[0]*alpha[0] + N[1]*alpha[1] + N[2]*alpha[2] + N[3]*alpha[3];
const double crRHS5 = N[0]*v_adj(0,0) + N[1]*v_adj(1,0) + N[2]*v_adj(2,0) + N[3]*v_adj(3,0);
const double crRHS6 = crRHS4*crRHS5;
const double crRHS7 = N[0]*v_ns(0,0) + N[1]*v_ns(1,0) + N[2]*v_ns(2,0) + N[3]*v_ns(3,0);
const double crRHS8 = 2.0*crRHS4*functional_weights[0];
const double crRHS9 = crRHS7*crRHS8;
const double crRHS10 = N[0]*v_ns(0,1) + N[1]*v_ns(1,1) + N[2]*v_ns(2,1) + N[3]*v_ns(3,1);
const double crRHS11 = N[0]*v_ns(0,2) + N[1]*v_ns(1,2) + N[2]*v_ns(2,2) + N[3]*v_ns(3,2);
const double crRHS12 = rho*(DN(0,0)*crRHS7 + DN(0,1)*crRHS10 + DN(0,2)*crRHS11);
const double crRHS13 = DN(0,1)*v_ns(0,0);
const double crRHS14 = DN(1,1)*v_ns(1,0);
const double crRHS15 = DN(2,1)*v_ns(2,0);
const double crRHS16 = DN(3,1)*v_ns(3,0);
const double crRHS17 = DN(0,0)*v_ns(0,1) + DN(1,0)*v_ns(1,1) + DN(2,0)*v_ns(2,1) + DN(3,0)*v_ns(3,1);
const double crRHS18 = -crRHS13 - crRHS14 - crRHS15 - crRHS16 + crRHS17;
const double crRHS19 = DN(0,2)*v_ns(0,0);
const double crRHS20 = DN(1,2)*v_ns(1,0);
const double crRHS21 = DN(2,2)*v_ns(2,0);
const double crRHS22 = DN(3,2)*v_ns(3,0);
const double crRHS23 = DN(0,0)*v_ns(0,2) + DN(1,0)*v_ns(1,2) + DN(2,0)*v_ns(2,2) + DN(3,0)*v_ns(3,2);
const double crRHS24 = -crRHS19 - crRHS20 - crRHS21 - crRHS22 + crRHS23;
const double crRHS25 = 2.0*functional_weights[2]*mu;
const double crRHS26 = DN(0,0)*v_ns(0,0) + DN(1,0)*v_ns(1,0) + DN(2,0)*v_ns(2,0) + DN(3,0)*v_ns(3,0);
const double crRHS27 = 1.0*crRHS26;
const double crRHS28 = crRHS13 + crRHS14 + crRHS15 + crRHS16;
const double crRHS29 = 0.5*crRHS17 + 0.5*crRHS28;
const double crRHS30 = crRHS19 + crRHS20 + crRHS21 + crRHS22;
const double crRHS31 = crRHS23 + crRHS30;
const double crRHS32 = 0.5*DN(0,2);
const double crRHS33 = 4.0*functional_weights[1]*mu;
const double crRHS34 = crRHS26*crRHS5;
const double crRHS35 = N[0]*v_adj(0,1) + N[1]*v_adj(1,1) + N[2]*v_adj(2,1) + N[3]*v_adj(3,1);
const double crRHS36 = crRHS28*crRHS35;
const double crRHS37 = N[0]*v_adj(0,2) + N[1]*v_adj(1,2) + N[2]*v_adj(2,2) + N[3]*v_adj(3,2);
const double crRHS38 = crRHS30*crRHS37;
const double crRHS39 = crRHS34 + crRHS36 + crRHS38;
const double crRHS40 = N[0]*rho;
const double crRHS41 = DN(0,0)*v_adj(0,0) + DN(1,0)*v_adj(1,0) + DN(2,0)*v_adj(2,0) + DN(3,0)*v_adj(3,0);
const double crRHS42 = DN(0,1)*v_adj(0,1) + DN(1,1)*v_adj(1,1) + DN(2,1)*v_adj(2,1) + DN(3,1)*v_adj(3,1);
const double crRHS43 = DN(0,2)*v_adj(0,2) + DN(1,2)*v_adj(1,2) + DN(2,2)*v_adj(2,2) + DN(3,2)*v_adj(3,2);
const double crRHS44 = crRHS41 + crRHS42 + crRHS43;
const double crRHS45 = crRHS4*stab_c3;
const double crRHS46 = pow(crRHS10, 2) + pow(crRHS11, 2) + pow(crRHS7, 2);
const double crRHS47 = pow(crRHS46, 1.0/4.0)*stab_c3;
const double crRHS48 = sqrt(crRHS46)*rho*stab_c2;
const double crRHS49 = crRHS44*(h*(crRHS45*h + crRHS47*h + crRHS48)/stab_c1 + mu);
const double crRHS50 = crRHS7*rho;
const double crRHS51 = crRHS10*rho;
const double crRHS52 = crRHS11*rho;
const double crRHS53 = DN(0,0)*p_adj[0] + DN(1,0)*p_adj[1] + DN(2,0)*p_adj[2] + DN(3,0)*p_adj[3] - crRHS1 + crRHS3 + crRHS34*rho + crRHS36*rho + crRHS38*rho - crRHS41*crRHS50 - crRHS51*(DN(0,1)*v_adj(0,0) + DN(1,1)*v_adj(1,0) + DN(2,1)*v_adj(2,0) + DN(3,1)*v_adj(3,0)) - crRHS52*(DN(0,2)*v_adj(0,0) + DN(1,2)*v_adj(1,0) + DN(2,2)*v_adj(2,0) + DN(3,2)*v_adj(3,0)) + crRHS6 + crRHS9;
const double crRHS54 = 1.0/(crRHS45 + crRHS47 + crRHS48/h + mu*stab_c1/pow(h, 2));
const double crRHS55 = crRHS53*crRHS54;
const double crRHS56 = N[0]*crRHS4;
const double crRHS57 = rho*(N[0]*f_adj(0,1) + N[1]*f_adj(1,1) + N[2]*f_adj(2,1) + N[3]*f_adj(3,1));
const double crRHS58 = crRHS2*(DN(0,1)*t[0] + DN(1,1)*t[1] + DN(2,1)*t[2] + DN(3,1)*t[3]);
const double crRHS59 = crRHS35*crRHS4;
const double crRHS60 = crRHS17*crRHS5;
const double crRHS61 = DN(0,1)*v_ns(0,1) + DN(1,1)*v_ns(1,1) + DN(2,1)*v_ns(2,1) + DN(3,1)*v_ns(3,1);
const double crRHS62 = crRHS35*crRHS61;
const double crRHS63 = DN(0,2)*v_ns(0,1);
const double crRHS64 = DN(1,2)*v_ns(1,1);
const double crRHS65 = DN(2,2)*v_ns(2,1);
const double crRHS66 = DN(3,2)*v_ns(3,1);
const double crRHS67 = crRHS63 + crRHS64 + crRHS65 + crRHS66;
const double crRHS68 = crRHS37*crRHS67;
const double crRHS69 = crRHS10*crRHS8;
const double crRHS70 = DN(0,1)*p_adj[0] + DN(1,1)*p_adj[1] + DN(2,1)*p_adj[2] + DN(3,1)*p_adj[3] - crRHS42*crRHS51 - crRHS50*(DN(0,0)*v_adj(0,1) + DN(1,0)*v_adj(1,1) + DN(2,0)*v_adj(2,1) + DN(3,0)*v_adj(3,1)) - crRHS52*(DN(0,2)*v_adj(0,1) + DN(1,2)*v_adj(1,1) + DN(2,2)*v_adj(2,1) + DN(3,2)*v_adj(3,1)) - crRHS57 + crRHS58 + crRHS59 + crRHS60*rho + crRHS62*rho + crRHS68*rho + crRHS69;
const double crRHS71 = rho*(N[0]*f_adj(0,2) + N[1]*f_adj(1,2) + N[2]*f_adj(2,2) + N[3]*f_adj(3,2));
const double crRHS72 = crRHS2*(DN(0,2)*t[0] + DN(1,2)*t[1] + DN(2,2)*t[2] + DN(3,2)*t[3]);
const double crRHS73 = crRHS37*crRHS4;
const double crRHS74 = crRHS23*crRHS5;
const double crRHS75 = DN(0,1)*v_ns(0,2) + DN(1,1)*v_ns(1,2) + DN(2,1)*v_ns(2,2) + DN(3,1)*v_ns(3,2);
const double crRHS76 = crRHS35*crRHS75;
const double crRHS77 = DN(0,2)*v_ns(0,2) + DN(1,2)*v_ns(1,2) + DN(2,2)*v_ns(2,2) + DN(3,2)*v_ns(3,2);
const double crRHS78 = crRHS37*crRHS77;
const double crRHS79 = crRHS11*crRHS8;
const double crRHS80 = DN(0,2)*p_adj[0] + DN(1,2)*p_adj[1] + DN(2,2)*p_adj[2] + DN(3,2)*p_adj[3] - crRHS43*crRHS52 - crRHS50*(DN(0,0)*v_adj(0,2) + DN(1,0)*v_adj(1,2) + DN(2,0)*v_adj(2,2) + DN(3,0)*v_adj(3,2)) - crRHS51*(DN(0,1)*v_adj(0,2) + DN(1,1)*v_adj(1,2) + DN(2,1)*v_adj(2,2) + DN(3,1)*v_adj(3,2)) - crRHS71 + crRHS72 + crRHS73 + crRHS74*rho + crRHS76*rho + crRHS78*rho + crRHS79;
const double crRHS81 = crRHS26*crRHS53 + crRHS28*crRHS70 + crRHS30*crRHS80;
const double crRHS82 = crRHS40*crRHS54;
const double crRHS83 = -crRHS63 - crRHS64 - crRHS65 - crRHS66 + crRHS75;
const double crRHS84 = 1.0*crRHS61;
const double crRHS85 = crRHS67 + crRHS75;
const double crRHS86 = crRHS60 + crRHS62 + crRHS68;
const double crRHS87 = crRHS54*crRHS70;
const double crRHS88 = crRHS17*crRHS53 + crRHS61*crRHS70 + crRHS67*crRHS80;
const double crRHS89 = 1.0*crRHS77;
const double crRHS90 = 0.5*crRHS31;
const double crRHS91 = 0.5*crRHS85;
const double crRHS92 = crRHS74 + crRHS76 + crRHS78;
const double crRHS93 = crRHS54*crRHS80;
const double crRHS94 = crRHS23*crRHS53 + crRHS70*crRHS75 + crRHS77*crRHS80;
const double crRHS95 = rho*(DN(1,0)*crRHS7 + DN(1,1)*crRHS10 + DN(1,2)*crRHS11);
const double crRHS96 = N[1]*rho;
const double crRHS97 = N[1]*crRHS4;
const double crRHS98 = crRHS54*crRHS96;
const double crRHS99 = rho*(DN(2,0)*crRHS7 + DN(2,1)*crRHS10 + DN(2,2)*crRHS11);
const double crRHS100 = N[2]*rho;
const double crRHS101 = N[2]*crRHS4;
const double crRHS102 = crRHS100*crRHS54;
const double crRHS103 = rho*(DN(3,0)*crRHS7 + DN(3,1)*crRHS10 + DN(3,2)*crRHS11);
const double crRHS104 = N[3]*rho;
const double crRHS105 = N[3]*crRHS4;
const double crRHS106 = crRHS104*crRHS54;
rRHS[0]+=-gauss_weight*(-DN(0,0)*crRHS0 + DN(0,0)*crRHS49 + DN(0,0)*stress_adj[0] + DN(0,1)*stress_adj[3] + DN(0,2)*stress_adj[5] - N[0]*crRHS1 + N[0]*crRHS3 + N[0]*crRHS6 + N[0]*crRHS9 + crRHS12*crRHS5 - crRHS12*crRHS55 - crRHS25*(DN(0,1)*crRHS18 + DN(0,2)*crRHS24) + crRHS33*(DN(0,0)*crRHS27 + DN(0,1)*crRHS29 + crRHS31*crRHS32) + crRHS39*crRHS40 - crRHS55*crRHS56 - crRHS81*crRHS82);
rRHS[1]+=-gauss_weight*(DN(0,0)*stress_adj[3] - DN(0,1)*crRHS0 + DN(0,1)*crRHS49 + DN(0,1)*stress_adj[1] + DN(0,2)*stress_adj[4] - N[0]*crRHS57 + N[0]*crRHS58 + N[0]*crRHS59 + N[0]*crRHS69 + crRHS12*crRHS35 - crRHS12*crRHS87 + crRHS25*(DN(0,0)*crRHS18 - DN(0,2)*crRHS83) + crRHS33*(DN(0,0)*crRHS29 + DN(0,1)*crRHS84 + crRHS32*crRHS85) + crRHS40*crRHS86 - crRHS56*crRHS87 - crRHS82*crRHS88);
rRHS[2]+=-gauss_weight*(DN(0,0)*stress_adj[5] + DN(0,1)*stress_adj[4] - DN(0,2)*crRHS0 + DN(0,2)*crRHS49 + DN(0,2)*stress_adj[2] - N[0]*crRHS71 + N[0]*crRHS72 + N[0]*crRHS73 + N[0]*crRHS79 + crRHS12*crRHS37 - crRHS12*crRHS93 + crRHS25*(DN(0,0)*crRHS24 + DN(0,1)*crRHS83) + crRHS33*(DN(0,0)*crRHS90 + DN(0,1)*crRHS91 + DN(0,2)*crRHS89) + crRHS40*crRHS92 - crRHS56*crRHS93 - crRHS82*crRHS94);
rRHS[3]+=-gauss_weight*(DN(0,0)*crRHS55 + DN(0,1)*crRHS87 + DN(0,2)*crRHS93 + N[0]*crRHS44);
rRHS[4]+=-gauss_weight*(-DN(1,0)*crRHS0 + DN(1,0)*crRHS49 + DN(1,0)*stress_adj[0] + DN(1,1)*stress_adj[3] + DN(1,2)*stress_adj[5] - N[1]*crRHS1 + N[1]*crRHS3 + N[1]*crRHS6 + N[1]*crRHS9 - crRHS25*(DN(1,1)*crRHS18 + DN(1,2)*crRHS24) + crRHS33*(DN(1,0)*crRHS27 + DN(1,1)*crRHS29 + DN(1,2)*crRHS90) + crRHS39*crRHS96 + crRHS5*crRHS95 - crRHS55*crRHS95 - crRHS55*crRHS97 - crRHS81*crRHS98);
rRHS[5]+=-gauss_weight*(DN(1,0)*stress_adj[3] - DN(1,1)*crRHS0 + DN(1,1)*crRHS49 + DN(1,1)*stress_adj[1] + DN(1,2)*stress_adj[4] - N[1]*crRHS57 + N[1]*crRHS58 + N[1]*crRHS59 + N[1]*crRHS69 + crRHS25*(DN(1,0)*crRHS18 - DN(1,2)*crRHS83) + crRHS33*(DN(1,0)*crRHS29 + DN(1,1)*crRHS84 + DN(1,2)*crRHS91) + crRHS35*crRHS95 + crRHS86*crRHS96 - crRHS87*crRHS95 - crRHS87*crRHS97 - crRHS88*crRHS98);
rRHS[6]+=-gauss_weight*(DN(1,0)*stress_adj[5] + DN(1,1)*stress_adj[4] - DN(1,2)*crRHS0 + DN(1,2)*crRHS49 + DN(1,2)*stress_adj[2] - N[1]*crRHS71 + N[1]*crRHS72 + N[1]*crRHS73 + N[1]*crRHS79 + crRHS25*(DN(1,0)*crRHS24 + DN(1,1)*crRHS83) + crRHS33*(DN(1,0)*crRHS90 + DN(1,1)*crRHS91 + DN(1,2)*crRHS89) + crRHS37*crRHS95 + crRHS92*crRHS96 - crRHS93*crRHS95 - crRHS93*crRHS97 - crRHS94*crRHS98);
rRHS[7]+=-gauss_weight*(DN(1,0)*crRHS55 + DN(1,1)*crRHS87 + DN(1,2)*crRHS93 + N[1]*crRHS44);
rRHS[8]+=-gauss_weight*(-DN(2,0)*crRHS0 + DN(2,0)*crRHS49 + DN(2,0)*stress_adj[0] + DN(2,1)*stress_adj[3] + DN(2,2)*stress_adj[5] - N[2]*crRHS1 + N[2]*crRHS3 + N[2]*crRHS6 + N[2]*crRHS9 + crRHS100*crRHS39 - crRHS101*crRHS55 - crRHS102*crRHS81 - crRHS25*(DN(2,1)*crRHS18 + DN(2,2)*crRHS24) + crRHS33*(DN(2,0)*crRHS27 + DN(2,1)*crRHS29 + DN(2,2)*crRHS90) + crRHS5*crRHS99 - crRHS55*crRHS99);
rRHS[9]+=-gauss_weight*(DN(2,0)*stress_adj[3] - DN(2,1)*crRHS0 + DN(2,1)*crRHS49 + DN(2,1)*stress_adj[1] + DN(2,2)*stress_adj[4] - N[2]*crRHS57 + N[2]*crRHS58 + N[2]*crRHS59 + N[2]*crRHS69 + crRHS100*crRHS86 - crRHS101*crRHS87 - crRHS102*crRHS88 + crRHS25*(DN(2,0)*crRHS18 - DN(2,2)*crRHS83) + crRHS33*(DN(2,0)*crRHS29 + DN(2,1)*crRHS84 + DN(2,2)*crRHS91) + crRHS35*crRHS99 - crRHS87*crRHS99);
rRHS[10]+=-gauss_weight*(DN(2,0)*stress_adj[5] + DN(2,1)*stress_adj[4] - DN(2,2)*crRHS0 + DN(2,2)*crRHS49 + DN(2,2)*stress_adj[2] - N[2]*crRHS71 + N[2]*crRHS72 + N[2]*crRHS73 + N[2]*crRHS79 + crRHS100*crRHS92 - crRHS101*crRHS93 - crRHS102*crRHS94 + crRHS25*(DN(2,0)*crRHS24 + DN(2,1)*crRHS83) + crRHS33*(DN(2,0)*crRHS90 + DN(2,1)*crRHS91 + DN(2,2)*crRHS89) + crRHS37*crRHS99 - crRHS93*crRHS99);
rRHS[11]+=-gauss_weight*(DN(2,0)*crRHS55 + DN(2,1)*crRHS87 + DN(2,2)*crRHS93 + N[2]*crRHS44);
rRHS[12]+=-gauss_weight*(-DN(3,0)*crRHS0 + DN(3,0)*crRHS49 + DN(3,0)*stress_adj[0] + DN(3,1)*stress_adj[3] + DN(3,2)*stress_adj[5] - N[3]*crRHS1 + N[3]*crRHS3 + N[3]*crRHS6 + N[3]*crRHS9 + crRHS103*crRHS5 - crRHS103*crRHS55 + crRHS104*crRHS39 - crRHS105*crRHS55 - crRHS106*crRHS81 - crRHS25*(DN(3,1)*crRHS18 + DN(3,2)*crRHS24) + crRHS33*(DN(3,0)*crRHS27 + DN(3,1)*crRHS29 + DN(3,2)*crRHS90));
rRHS[13]+=-gauss_weight*(DN(3,0)*stress_adj[3] - DN(3,1)*crRHS0 + DN(3,1)*crRHS49 + DN(3,1)*stress_adj[1] + DN(3,2)*stress_adj[4] - N[3]*crRHS57 + N[3]*crRHS58 + N[3]*crRHS59 + N[3]*crRHS69 + crRHS103*crRHS35 - crRHS103*crRHS87 + crRHS104*crRHS86 - crRHS105*crRHS87 - crRHS106*crRHS88 + crRHS25*(DN(3,0)*crRHS18 - DN(3,2)*crRHS83) + crRHS33*(DN(3,0)*crRHS29 + DN(3,1)*crRHS84 + DN(3,2)*crRHS91));
rRHS[14]+=-gauss_weight*(DN(3,0)*stress_adj[5] + DN(3,1)*stress_adj[4] - DN(3,2)*crRHS0 + DN(3,2)*crRHS49 + DN(3,2)*stress_adj[2] - N[3]*crRHS71 + N[3]*crRHS72 + N[3]*crRHS73 + N[3]*crRHS79 + crRHS103*crRHS37 - crRHS103*crRHS93 + crRHS104*crRHS92 - crRHS105*crRHS93 - crRHS106*crRHS94 + crRHS25*(DN(3,0)*crRHS24 + DN(3,1)*crRHS83) + crRHS33*(DN(3,0)*crRHS90 + DN(3,1)*crRHS91 + DN(3,2)*crRHS89));
rRHS[15]+=-gauss_weight*(DN(3,0)*crRHS55 + DN(3,1)*crRHS87 + DN(3,2)*crRHS93 + N[3]*crRHS44);
