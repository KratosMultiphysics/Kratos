...
const double crhs12 = N[0]*vfrac(0,0) + N[1]*vfrac(1,0) + N[2]*vfrac(2,0);
const double crhs13 = N[0]*vfrac(0,1) + N[1]*vfrac(1,1) + N[2]*vfrac(2,1);
...
const double crhs19 = N[0]*vn(0,0) + N[1]*vn(1,0) + N[2]*vn(2,0);
const double crhs20 = N[0]*vn(0,1) + N[1]*vn(1,1) + N[2]*vn(2,1);
...
const double crhs34 = rho*(DN(0,0)*crhs16 + DN(0,1)*crhs17);
const double crhs35 = rho*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1));
const double crhs36 = K_darcy*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1));
const double crhs37 = N[0]*(v(0,1) - vfrac(0,1)) + N[1]*(v(1,1) - vfrac(1,1)) + N[2]*(v(2,1) - vfrac(2,1));
const double crhs38 = N[0]*(vn(0,1) - vnn(0,1));
const double crhs39 = N[1]*(vn(1,1) - vnn(1,1));
const double crhs40 = N[2]*(vn(2,1) - vnn(2,1));
const double crhs41 = crhs38 + crhs39 + crhs40;
const double crhs42 = rho*(crhs12*(DN(0,0)*vfrac(0,1) + DN(1,0)*vfrac(1,1) + DN(2,0)*vfrac(2,1)) + crhs13*(DN(0,1)*vfrac(0,1) + DN(1,1)*vfrac(1,1) + DN(2,1)*vfrac(2,1)));
const double crhs43 = rho*(crhs16*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1)) + crhs17*crhs22);
const double crhs44 = crhs19*(DN(0,0)*vn(0,1) + DN(1,0)*vn(1,1) + DN(2,0)*vn(2,1)) + crhs20*(DN(0,1)*vn(0,1) + DN(1,1)*vn(1,1) + DN(2,1)*vn(2,1));
const double crhs45 = crhs26*crhs37;
const double crhs46 = crhs29*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] - crhs35 + crhs36 - crhs42 + crhs43 + crhs45 + rho*(crhs10*crhs38 + crhs10*crhs39 + crhs10*crhs40 + crhs44));
const double crhs47 = N[1]*crhs28;
const double crhs48 = N[1]*rho;
const double crhs49 = K_darcy*N[1];
const double crhs50 = crhs32*crhs48;
const double crhs51 = rho*(DN(1,0)*crhs16 + DN(1,1)*crhs17);
const double crhs52 = N[2]*crhs28;
const double crhs53 = N[2]*rho;
const double crhs54 = K_darcy*N[2];
const double crhs55 = crhs32*crhs53;
const double crhs56 = rho*(DN(2,0)*crhs16 + DN(2,1)*crhs17);
rhs[0]=DN(0,0)*crhs0 - DN(0,0)*crhs25 - DN(0,0)*stress[0] - DN(0,1)*stress[2] + N[0]*crhs1 + N[0]*crhs14 - N[0]*crhs18 - N[0]*crhs2 - crhs11*crhs9 - crhs21*crhs4 - crhs3*crhs5 + crhs30*crhs31 - crhs30*crhs33 - crhs30*crhs34;
rhs[1]=-DN(0,0)*stress[2] + DN(0,1)*crhs0 - DN(0,1)*crhs25 - DN(0,1)*stress[1] + N[0]*crhs35 - N[0]*crhs36 + N[0]*crhs42 - N[0]*crhs43 - crhs11*crhs41 + crhs31*crhs46 - crhs33*crhs46 - crhs34*crhs46 - crhs37*crhs5 - crhs4*crhs44;
rhs[2]=-DN(0,0)*crhs30 - DN(0,1)*crhs46 - N[0]*crhs23;
rhs[3]=DN(1,0)*crhs0 - DN(1,0)*crhs25 - DN(1,0)*stress[0] - DN(1,1)*stress[2] + N[1]*crhs1 + N[1]*crhs14 - N[1]*crhs18 - N[1]*crhs2 - N[1]*crhs27 - crhs21*crhs48 + crhs30*crhs49 - crhs30*crhs50 - crhs30*crhs51 - crhs47*crhs9;
rhs[4]=-DN(1,0)*stress[2] + DN(1,1)*crhs0 - DN(1,1)*crhs25 - DN(1,1)*stress[1] + N[1]*crhs35 - N[1]*crhs36 + N[1]*crhs42 - N[1]*crhs43 - N[1]*crhs45 - crhs41*crhs47 - crhs44*crhs48 + crhs46*crhs49 - crhs46*crhs50 - crhs46*crhs51;
rhs[5]=-DN(1,0)*crhs30 - DN(1,1)*crhs46 - N[1]*crhs23;
rhs[6]=DN(2,0)*crhs0 - DN(2,0)*crhs25 - DN(2,0)*stress[0] - DN(2,1)*stress[2] + N[2]*crhs1 + N[2]*crhs14 - N[2]*crhs18 - N[2]*crhs2 - N[2]*crhs27 - crhs21*crhs53 + crhs30*crhs54 - crhs30*crhs55 - crhs30*crhs56 - crhs52*crhs9;
rhs[7]=-DN(2,0)*stress[2] + DN(2,1)*crhs0 - DN(2,1)*crhs25 - DN(2,1)*stress[1] + N[2]*crhs35 - N[2]*crhs36 + N[2]*crhs42 - N[2]*crhs43 - N[2]*crhs45 - crhs41*crhs52 - crhs44*crhs53 + crhs46*crhs54 - crhs46*crhs55 - crhs46*crhs56;
rhs[8]=-DN(2,0)*crhs30 - DN(2,1)*crhs46 - N[2]*crhs23;
...
const double crhs13 = N[0]*vfrac(0,0) + N[1]*vfrac(1,0) + N[2]*vfrac(2,0) + N[3]*vfrac(3,0);
const double crhs14 = N[0]*vfrac(0,1) + N[1]*vfrac(1,1) + N[2]*vfrac(2,1) + N[3]*vfrac(3,1);
const double crhs15 = N[0]*vfrac(0,2) + N[1]*vfrac(1,2) + N[2]*vfrac(2,2) + N[3]*vfrac(3,2);
...
const double crhs22 = N[0]*vn(0,0) + N[1]*vn(1,0) + N[2]*vn(2,0) + N[3]*vn(3,0);
const double crhs23 = N[0]*vn(0,1) + N[1]*vn(1,1) + N[2]*vn(2,1) + N[3]*vn(3,1);
const double crhs24 = N[0]*vn(0,2) + N[1]*vn(1,2) + N[2]*vn(2,2) + N[3]*vn(3,2);
const double crhs25 = crhs22*(DN(0,0)*vn(0,0) + DN(1,0)*vn(1,0) + DN(2,0)*vn(2,0) + DN(3,0)*vn(3,0)) + crhs23*(DN(0,1)*vn(0,0) + DN(1,1)*vn(1,0) + DN(2,1)*vn(2,0) + DN(3,1)*vn(3,0)) + crhs24*(DN(0,2)*vn(0,0) + DN(1,2)*vn(1,0) + DN(2,2)*vn(2,0) + DN(3,2)*vn(3,0));
...
const double crhs38 = crhs37*crhs4;
const double crhs39 = rho*(DN(0,0)*crhs18 + DN(0,1)*crhs19 + DN(0,2)*crhs20);
const double crhs40 = rho*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1) + N[3]*f(3,1));
const double crhs41 = K_darcy*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1) + N[3]*v(3,1));
const double crhs42 = N[0]*(v(0,1) - vfrac(0,1)) + N[1]*(v(1,1) - vfrac(1,1)) + N[2]*(v(2,1) - vfrac(2,1)) + N[3]*(v(3,1) - vfrac(3,1));
const double crhs43 = N[0]*(vn(0,1) - vnn(0,1));
const double crhs44 = N[1]*(vn(1,1) - vnn(1,1));
const double crhs45 = N[2]*(vn(2,1) - vnn(2,1));
const double crhs46 = N[3]*(vn(3,1) - vnn(3,1));
const double crhs47 = crhs43 + crhs44 + crhs45 + crhs46;
const double crhs48 = rho*(crhs13*(DN(0,0)*vfrac(0,1) + DN(1,0)*vfrac(1,1) + DN(2,0)*vfrac(2,1) + DN(3,0)*vfrac(3,1)) + crhs14*(DN(0,1)*vfrac(0,1) + DN(1,1)*vfrac(1,1) + DN(2,1)*vfrac(2,1) + DN(3,1)*vfrac(3,1)) + crhs15*(DN(0,2)*vfrac(0,1) + DN(1,2)*vfrac(1,1) + DN(2,2)*vfrac(2,1) + DN(3,2)*vfrac(3,1)));
const double crhs49 = rho*(crhs18*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1) + DN(3,0)*v(3,1)) + crhs19*crhs26 + crhs20*(DN(0,2)*v(0,1) + DN(1,2)*v(1,1) + DN(2,2)*v(2,1) + DN(3,2)*v(3,1)));
const double crhs50 = crhs22*(DN(0,0)*vn(0,1) + DN(1,0)*vn(1,1) + DN(2,0)*vn(2,1) + DN(3,0)*vn(3,1)) + crhs23*(DN(0,1)*vn(0,1) + DN(1,1)*vn(1,1) + DN(2,1)*vn(2,1) + DN(3,1)*vn(3,1)) + crhs24*(DN(0,2)*vn(0,1) + DN(1,2)*vn(1,1) + DN(2,2)*vn(2,1) + DN(3,2)*vn(3,1));
const double crhs51 = crhs31*crhs42;
const double crhs52 = crhs34*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] + DN(3,1)*p[3] - crhs40 + crhs41 - crhs48 + crhs49 + crhs51 + rho*(crhs11*crhs43 + crhs11*crhs44 + crhs11*crhs45 + crhs11*crhs46 + crhs50));
...
const double crhs55 = N[0]*(v(0,2) - vfrac(0,2)) + N[1]*(v(1,2) - vfrac(1,2)) + N[2]*(v(2,2) - vfrac(2,2)) + N[3]*(v(3,2) - vfrac(3,2));
const double crhs56 = N[0]*(vn(0,2) - vnn(0,2));
const double crhs57 = N[1]*(vn(1,2) - vnn(1,2));
const double crhs58 = N[2]*(vn(2,2) - vnn(2,2));
const double crhs59 = N[3]*(vn(3,2) - vnn(3,2));
const double crhs60 = crhs56 + crhs57 + crhs58 + crhs59;
const double crhs61 = rho*(crhs13*(DN(0,0)*vfrac(0,2) + DN(1,0)*vfrac(1,2) + DN(2,0)*vfrac(2,2) + DN(3,0)*vfrac(3,2)) + crhs14*(DN(0,1)*vfrac(0,2) + DN(1,1)*vfrac(1,2) + DN(2,1)*vfrac(2,2) + DN(3,1)*vfrac(3,2)) + crhs15*(DN(0,2)*vfrac(0,2) + DN(1,2)*vfrac(1,2) + DN(2,2)*vfrac(2,2) + DN(3,2)*vfrac(3,2)));
const double crhs62 = rho*(crhs18*(DN(0,0)*v(0,2) + DN(1,0)*v(1,2) + DN(2,0)*v(2,2) + DN(3,0)*v(3,2)) + crhs19*(DN(0,1)*v(0,2) + DN(1,1)*v(1,2) + DN(2,1)*v(2,2) + DN(3,1)*v(3,2)) + crhs20*crhs27);
const double crhs63 = crhs22*(DN(0,0)*vn(0,2) + DN(1,0)*vn(1,2) + DN(2,0)*vn(2,2) + DN(3,0)*vn(3,2)) + crhs23*(DN(0,1)*vn(0,2) + DN(1,1)*vn(1,2) + DN(2,1)*vn(2,2) + DN(3,1)*vn(3,2)) + crhs24*(DN(0,2)*vn(0,2) + DN(1,2)*vn(1,2) + DN(2,2)*vn(2,2) + DN(3,2)*vn(3,2));
const double crhs64 = crhs31*crhs55;
const double crhs65 = crhs34*(DN(0,2)*p[0] + DN(1,2)*p[1] + DN(2,2)*p[2] + DN(3,2)*p[3] - crhs53 + crhs54 - crhs61 + crhs62 + crhs64 + rho*(crhs11*crhs56 + crhs11*crhs57 + crhs11*crhs58 + crhs11*crhs59 + crhs63));
const double crhs66 = N[1]*crhs33;
const double crhs67 = N[1]*rho;
const double crhs68 = K_darcy*N[1];
const double crhs69 = crhs37*crhs67;
const double crhs70 = rho*(DN(1,0)*crhs18 + DN(1,1)*crhs19 + DN(1,2)*crhs20);
const double crhs71 = N[2]*crhs33;
const double crhs72 = N[2]*rho;
const double crhs73 = K_darcy*N[2];
const double crhs74 = crhs37*crhs72;
const double crhs75 = rho*(DN(2,0)*crhs18 + DN(2,1)*crhs19 + DN(2,2)*crhs20);
const double crhs76 = N[3]*crhs33;
const double crhs77 = N[3]*rho;
const double crhs78 = K_darcy*N[3];
const double crhs79 = crhs37*crhs77;
const double crhs80 = rho*(DN(3,0)*crhs18 + DN(3,1)*crhs19 + DN(3,2)*crhs20);
rhs[0]=DN(0,0)*crhs0 - DN(0,0)*crhs30 - DN(0,0)*stress[0] - DN(0,1)*stress[3] - DN(0,2)*stress[5] + N[0]*crhs1 + N[0]*crhs16 - N[0]*crhs2 - N[0]*crhs21 - crhs10*crhs12 - crhs25*crhs4 - crhs3*crhs5 + crhs35*crhs36 - crhs35*crhs38 - crhs35*crhs39;
rhs[1]=-DN(0,0)*stress[3] + DN(0,1)*crhs0 - DN(0,1)*crhs30 - DN(0,1)*stress[1] - DN(0,2)*stress[4] + N[0]*crhs40 - N[0]*crhs41 + N[0]*crhs48 - N[0]*crhs49 - crhs12*crhs47 + crhs36*crhs52 - crhs38*crhs52 - crhs39*crhs52 - crhs4*crhs50 - crhs42*crhs5;
rhs[2]=-DN(0,0)*stress[5] - DN(0,1)*stress[4] + DN(0,2)*crhs0 - DN(0,2)*crhs30 - DN(0,2)*stress[2] + N[0]*crhs53 - N[0]*crhs54 + N[0]*crhs61 - N[0]*crhs62 - crhs12*crhs60 + crhs36*crhs65 - crhs38*crhs65 - crhs39*crhs65 - crhs4*crhs63 - crhs5*crhs55;
rhs[3]=-DN(0,0)*crhs35 - DN(0,1)*crhs52 - DN(0,2)*crhs65 - N[0]*crhs28;
rhs[4]=DN(1,0)*crhs0 - DN(1,0)*crhs30 - DN(1,0)*stress[0] - DN(1,1)*stress[3] - DN(1,2)*stress[5] + N[1]*crhs1 + N[1]*crhs16 - N[1]*crhs2 - N[1]*crhs21 - N[1]*crhs32 - crhs10*crhs66 - crhs25*crhs67 + crhs35*crhs68 - crhs35*crhs69 - crhs35*crhs70;
rhs[5]=-DN(1,0)*stress[3] + DN(1,1)*crhs0 - DN(1,1)*crhs30 - DN(1,1)*stress[1] - DN(1,2)*stress[4] + N[1]*crhs40 - N[1]*crhs41 + N[1]*crhs48 - N[1]*crhs49 - N[1]*crhs51 - crhs47*crhs66 - crhs50*crhs67 + crhs52*crhs68 - crhs52*crhs69 - crhs52*crhs70;
rhs[6]=-DN(1,0)*stress[5] - DN(1,1)*stress[4] + DN(1,2)*crhs0 - DN(1,2)*crhs30 - DN(1,2)*stress[2] + N[1]*crhs53 - N[1]*crhs54 + N[1]*crhs61 - N[1]*crhs62 - N[1]*crhs64 - crhs60*crhs66 - crhs63*crhs67 + crhs65*crhs68 - crhs65*crhs69 - crhs65*crhs70;
rhs[7]=-DN(1,0)*crhs35 - DN(1,1)*crhs52 - DN(1,2)*crhs65 - N[1]*crhs28;
rhs[8]=DN(2,0)*crhs0 - DN(2,0)*crhs30 - DN(2,0)*stress[0] - DN(2,1)*stress[3] - DN(2,2)*stress[5] + N[2]*crhs1 + N[2]*crhs16 - N[2]*crhs2 - N[2]*crhs21 - N[2]*crhs32 - crhs10*crhs71 - crhs25*crhs72 + crhs35*crhs73 - crhs35*crhs74 - crhs35*crhs75;
rhs[9]=-DN(2,0)*stress[3] + DN(2,1)*crhs0 - DN(2,1)*crhs30 - DN(2,1)*stress[1] - DN(2,2)*stress[4] + N[2]*crhs40 - N[2]*crhs41 + N[2]*crhs48 - N[2]*crhs49 - N[2]*crhs51 - crhs47*crhs71 - crhs50*crhs72 + crhs52*crhs73 - crhs52*crhs74 - crhs52*crhs75;
rhs[10]=-DN(2,0)*stress[5] - DN(2,1)*stress[4] + DN(2,2)*crhs0 - DN(2,2)*crhs30 - DN(2,2)*stress[2] + N[2]*crhs53 - N[2]*crhs54 + N[2]*crhs61 - N[2]*crhs62 - N[2]*crhs64 - crhs60*crhs71 - crhs63*crhs72 + crhs65*crhs73 - crhs65*crhs74 - crhs65*crhs75;
rhs[11]=-DN(2,0)*crhs35 - DN(2,1)*crhs52 - DN(2,2)*crhs65 - N[2]*crhs28;
rhs[12]=DN(3,0)*crhs0 - DN(3,0)*crhs30 - DN(3,0)*stress[0] - DN(3,1)*stress[3] - DN(3,2)*stress[5] + N[3]*crhs1 + N[3]*crhs16 - N[3]*crhs2 - N[3]*crhs21 - N[3]*crhs32 - crhs10*crhs76 - crhs25*crhs77 + crhs35*crhs78 - crhs35*crhs79 - crhs35*crhs80;
rhs[13]=-DN(3,0)*stress[3] + DN(3,1)*crhs0 - DN(3,1)*crhs30 - DN(3,1)*stress[1] - DN(3,2)*stress[4] + N[3]*crhs40 - N[3]*crhs41 + N[3]*crhs48 - N[3]*crhs49 - N[3]*crhs51 - crhs47*crhs76 - crhs50*crhs77 + crhs52*crhs78 - crhs52*crhs79 - crhs52*crhs80;
rhs[14]=-DN(3,0)*stress[5] - DN(3,1)*stress[4] + DN(3,2)*crhs0 - DN(3,2)*crhs30 - DN(3,2)*stress[2] + N[3]*crhs53 - N[3]*crhs54 + N[3]*crhs61 - N[3]*crhs62 - N[3]*crhs64 - crhs60*crhs76 - crhs63*crhs77 + crhs65*crhs78 - crhs65*crhs79 - crhs65*crhs80;
rhs[15]=-DN(3,0)*crhs35 - DN(3,1)*crhs52 - DN(3,2)*crhs65 - N[3]*crhs28;
