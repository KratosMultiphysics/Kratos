...
//                     KRATOS_WATCH(enriched_grad);
                    ...
                    for (unsigned int inode = 0; inode < TNumNodes; inode++)
                    {
                        ...
                        {
                            double velocity_stab = wGauss * TauOne * enriched_grad(enriched_id,k)* ( Density * AGradN[inode] - DarcyTerm * Nenriched(igauss, enriched_id) );

                            //                      enrichment_terms_vertical[base_index + k] += velocity_stab + wGauss*N[inode]*enriched_grad(0, k);
                            enrichment_terms_vertical(base_index + k,enriched_id) += velocity_stab - wGauss * DN_DX(inode, k) * Nenriched(igauss, enriched_id);
                            enrichment_terms_horizontal(enriched_id,base_index + k) += velocity_stab + wGauss * DN_DX(inode, k) * Nenriched(igauss, enriched_id);
