...
const double crLHS32 = bdf0/(crLHS5*pow(N[0]*c[0] + N[1]*c[1] + N[2]*c[2], 2));
...
const double crLHS73 = pow(DN(0,1), 2);
...
const double crLHS96 = pow(DN(1,0), 2);
const double crLHS97 = pow(N[1], 2);
...
const double crLHS109 = pow(DN(1,1), 2);
...
const double crLHS122 = pow(DN(2,0), 2);
const double crLHS123 = pow(N[2], 2);
...
const double crLHS127 = pow(DN(2,1), 2);
...
const double crLHS44 = bdf0/(crLHS7*pow(N[0]*c[0] + N[1]*c[1] + N[2]*c[2] + N[3]*c[3], 2));
...
const double crLHS138 = pow(DN(0,1), 2);
...
const double crLHS189 = pow(DN(0,2), 2);
...
const double crLHS226 = pow(DN(1,0), 2);
const double crLHS227 = pow(N[1], 2);
...
const double crLHS249 = pow(DN(1,1), 2);
...
const double crLHS285 = pow(DN(2,0), 2);
const double crLHS286 = pow(N[2], 2);
...
const double crLHS304 = pow(DN(2,1), 2);
...
const double crLHS339 = pow(DN(3,1), 2);
const double crLHS340 = DN(3,1)*DN(3,2)*crLHS12;
const double crLHS341 = pow(DN(3,2), 2);
...
const double crRHS1 = sigma*(N[0]*v(0,0) + N[1]*v(1,0) + N[2]*v(2,0));
const double crRHS2 = N[0]*rho[0] + N[1]*rho[1] + N[2]*rho[2];
const double crRHS3 = crRHS2*(N[0]*f(0,0) + N[1]*f(1,0) + N[2]*f(2,0));
const double crRHS4 = crRHS2*(N[0]*(bdf0*v(0,0) + bdf1*vn(0,0) + bdf2*vnn(0,0)) + N[1]*(bdf0*v(1,0) + bdf1*vn(1,0) + bdf2*vnn(1,0)) + N[2]*(bdf0*v(2,0) + bdf1*vn(2,0) + bdf2*vnn(2,0)));
...
const double crRHS8 = crRHS2*(crRHS5*crRHS6 + crRHS7*(DN(0,1)*v(0,0) + DN(1,1)*v(1,0) + DN(2,1)*v(2,0)));
const double crRHS9 = sigma*stab_c3;
const double crRHS10 = crRHS2*stab_c2*sqrt(pow(crRHS6, 2) + pow(crRHS7, 2));
const double crRHS11 = 1.0/crRHS2;
const double crRHS12 = crRHS11*(crRHS6*(DN(0,0)*rho[0] + DN(1,0)*rho[1] + DN(2,0)*rho[2]) + crRHS7*(DN(0,1)*rho[0] + DN(1,1)*rho[1] + DN(2,1)*rho[2]));
...
const double crRHS14 = DN(0,1)*v(0,1) + DN(1,1)*v(1,1) + DN(2,1)*v(2,1);
const double crRHS15 = crRHS14 + crRHS5;
const double crRHS16 = (mu + (crRHS10*h + crRHS9)/stab_c1)*(crRHS12 + crRHS13 + crRHS15);
const double crRHS17 = pow(h, -2);
const double crRHS18 = 1.0/(crRHS10/h + crRHS17*crRHS9 + crRHS17*mu*stab_c1 + crRHS2*dyn_tau/dt);
const double crRHS19 = crRHS18*(DN(0,0)*p[0] + DN(1,0)*p[1] + DN(2,0)*p[2] + crRHS1 - crRHS3 + crRHS4 + crRHS8);
const double crRHS20 = N[0]*sigma;
const double crRHS21 = crRHS2*(DN(0,0)*vconv(0,0) + DN(0,1)*vconv(0,1) + DN(1,0)*vconv(1,0) + DN(1,1)*vconv(1,1) + DN(2,0)*vconv(2,0) + DN(2,1)*vconv(2,1));
const double crRHS22 = N[0]*crRHS21;
const double crRHS23 = crRHS2*(DN(0,0)*crRHS6 + DN(0,1)*crRHS7);
const double crRHS24 = sigma*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1));
const double crRHS25 = crRHS2*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1));
const double crRHS26 = crRHS2*(N[0]*(bdf0*v(0,1) + bdf1*vn(0,1) + bdf2*vnn(0,1)) + N[1]*(bdf0*v(1,1) + bdf1*vn(1,1) + bdf2*vnn(1,1)) + N[2]*(bdf0*v(2,1) + bdf1*vn(2,1) + bdf2*vnn(2,1)));
const double crRHS27 = crRHS2*(crRHS14*crRHS7 + crRHS6*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1)));
const double crRHS28 = crRHS18*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] + crRHS24 - crRHS25 + crRHS26 + crRHS27);
const double crRHS29 = N[1]*sigma;
const double crRHS30 = N[1]*crRHS21;
const double crRHS31 = crRHS2*(DN(1,0)*crRHS6 + DN(1,1)*crRHS7);
const double crRHS32 = N[2]*sigma;
const double crRHS33 = N[2]*crRHS21;
const double crRHS34 = crRHS2*(DN(2,0)*crRHS6 + DN(2,1)*crRHS7);
rRHS[0]+=-gauss_weight*(-DN(0,0)*crRHS0 + DN(0,0)*crRHS16 + DN(0,0)*stress[0] + DN(0,1)*stress[2] + N[0]*crRHS1 - N[0]*crRHS3 + N[0]*crRHS4 + N[0]*crRHS8 - crRHS19*crRHS20 + crRHS19*crRHS22 + crRHS19*crRHS23);
rRHS[1]+=-gauss_weight*(DN(0,0)*stress[2] - DN(0,1)*crRHS0 + DN(0,1)*crRHS16 + DN(0,1)*stress[1] + N[0]*crRHS24 - N[0]*crRHS25 + N[0]*crRHS26 + N[0]*crRHS27 - crRHS20*crRHS28 + crRHS22*crRHS28 + crRHS23*crRHS28);
rRHS[2]+=-gauss_weight*(DN(0,0)*crRHS19 + DN(0,1)*crRHS28 + N[0]*crRHS12 + N[0]*crRHS13 + N[0]*crRHS15);
rRHS[3]+=-gauss_weight*(-DN(1,0)*crRHS0 + DN(1,0)*crRHS16 + DN(1,0)*stress[0] + DN(1,1)*stress[2] + N[1]*crRHS1 - N[1]*crRHS3 + N[1]*crRHS4 + N[1]*crRHS8 - crRHS19*crRHS29 + crRHS19*crRHS30 + crRHS19*crRHS31);
rRHS[4]+=-gauss_weight*(DN(1,0)*stress[2] - DN(1,1)*crRHS0 + DN(1,1)*crRHS16 + DN(1,1)*stress[1] + N[1]*crRHS24 - N[1]*crRHS25 + N[1]*crRHS26 + N[1]*crRHS27 - crRHS28*crRHS29 + crRHS28*crRHS30 + crRHS28*crRHS31);
rRHS[5]+=-gauss_weight*(DN(1,0)*crRHS19 + DN(1,1)*crRHS28 + N[1]*crRHS12 + N[1]*crRHS13 + N[1]*crRHS15);
rRHS[6]+=-gauss_weight*(-DN(2,0)*crRHS0 + DN(2,0)*crRHS16 + DN(2,0)*stress[0] + DN(2,1)*stress[2] + N[2]*crRHS1 - N[2]*crRHS3 + N[2]*crRHS4 + N[2]*crRHS8 - crRHS19*crRHS32 + crRHS19*crRHS33 + crRHS19*crRHS34);
rRHS[7]+=-gauss_weight*(DN(2,0)*stress[2] - DN(2,1)*crRHS0 + DN(2,1)*crRHS16 + DN(2,1)*stress[1] + N[2]*crRHS24 - N[2]*crRHS25 + N[2]*crRHS26 + N[2]*crRHS27 - crRHS28*crRHS32 + crRHS28*crRHS33 + crRHS28*crRHS34);
rRHS[8]+=-gauss_weight*(DN(2,0)*crRHS19 + DN(2,1)*crRHS28 + N[2]*crRHS12 + N[2]*crRHS13 + N[2]*crRHS15);
...
const double crRHS1 = sigma*(N[0]*v(0,0) + N[1]*v(1,0) + N[2]*v(2,0) + N[3]*v(3,0));
const double crRHS2 = N[0]*rho[0] + N[1]*rho[1] + N[2]*rho[2] + N[3]*rho[3];
const double crRHS3 = crRHS2*(N[0]*f(0,0) + N[1]*f(1,0) + N[2]*f(2,0) + N[3]*f(3,0));
const double crRHS4 = crRHS2*(N[0]*(bdf0*v(0,0) + bdf1*vn(0,0) + bdf2*vnn(0,0)) + N[1]*(bdf0*v(1,0) + bdf1*vn(1,0) + bdf2*vnn(1,0)) + N[2]*(bdf0*v(2,0) + bdf1*vn(2,0) + bdf2*vnn(2,0)) + N[3]*(bdf0*v(3,0) + bdf1*vn(3,0) + bdf2*vnn(3,0)));
...
const double crRHS8 = N[0]*vconv(0,2) + N[1]*vconv(1,2) + N[2]*vconv(2,2) + N[3]*vconv(3,2);
const double crRHS9 = crRHS2*(crRHS5*crRHS6 + crRHS7*(DN(0,1)*v(0,0) + DN(1,1)*v(1,0) + DN(2,1)*v(2,0) + DN(3,1)*v(3,0)) + crRHS8*(DN(0,2)*v(0,0) + DN(1,2)*v(1,0) + DN(2,2)*v(2,0) + DN(3,2)*v(3,0)));
const double crRHS10 = sigma*stab_c3;
const double crRHS11 = crRHS2*stab_c2*sqrt(pow(crRHS6, 2) + pow(crRHS7, 2) + pow(crRHS8, 2));
const double crRHS12 = 1.0/crRHS2;
const double crRHS13 = crRHS12*(crRHS6*(DN(0,0)*rho[0] + DN(1,0)*rho[1] + DN(2,0)*rho[2] + DN(3,0)*rho[3]) + crRHS7*(DN(0,1)*rho[0] + DN(1,1)*rho[1] + DN(2,1)*rho[2] + DN(3,1)*rho[3]) + crRHS8*(DN(0,2)*rho[0] + DN(1,2)*rho[1] + DN(2,2)*rho[2] + DN(3,2)*rho[3]));
const double crRHS14 = crRHS12*(N[0]*(bdf0*p[0] + bdf1*pn[0] + bdf2*pnn[0]) + N[1]*(bdf0*p[1] + bdf1*pn[1] + bdf2*pnn[1]) + N[2]*(bdf0*p[2] + bdf1*pn[2] + bdf2*pnn[2]) + N[3]*(bdf0*p[3] + bdf1*pn[3] + bdf2*pnn[3]))/pow(N[0]*c[0] + N[1]*c[1] + N[2]*c[2] + N[3]*c[3], 2);
...
const double crRHS17 = crRHS15 + crRHS16 + crRHS5;
const double crRHS18 = (mu + (crRHS10 + crRHS11*h)/stab_c1)*(crRHS13 + crRHS14 + crRHS17);
const double crRHS19 = pow(h, -2);
const double crRHS20 = 1.0/(crRHS10*crRHS19 + crRHS11/h + crRHS19*mu*stab_c1 + crRHS2*dyn_tau/dt);
const double crRHS21 = crRHS20*(DN(0,0)*p[0] + DN(1,0)*p[1] + DN(2,0)*p[2] + DN(3,0)*p[3] + crRHS1 - crRHS3 + crRHS4 + crRHS9);
const double crRHS22 = N[0]*sigma;
const double crRHS23 = crRHS2*(DN(0,0)*vconv(0,0) + DN(0,1)*vconv(0,1) + DN(0,2)*vconv(0,2) + DN(1,0)*vconv(1,0) + DN(1,1)*vconv(1,1) + DN(1,2)*vconv(1,2) + DN(2,0)*vconv(2,0) + DN(2,1)*vconv(2,1) + DN(2,2)*vconv(2,2) + DN(3,0)*vconv(3,0) + DN(3,1)*vconv(3,1) + DN(3,2)*vconv(3,2));
const double crRHS24 = N[0]*crRHS23;
const double crRHS25 = crRHS2*(DN(0,0)*crRHS6 + DN(0,1)*crRHS7 + DN(0,2)*crRHS8);
const double crRHS26 = sigma*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1) + N[3]*v(3,1));
const double crRHS27 = crRHS2*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1) + N[3]*f(3,1));
const double crRHS28 = crRHS2*(N[0]*(bdf0*v(0,1) + bdf1*vn(0,1) + bdf2*vnn(0,1)) + N[1]*(bdf0*v(1,1) + bdf1*vn(1,1) + bdf2*vnn(1,1)) + N[2]*(bdf0*v(2,1) + bdf1*vn(2,1) + bdf2*vnn(2,1)) + N[3]*(bdf0*v(3,1) + bdf1*vn(3,1) + bdf2*vnn(3,1)));
const double crRHS29 = crRHS2*(crRHS15*crRHS7 + crRHS6*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1) + DN(3,0)*v(3,1)) + crRHS8*(DN(0,2)*v(0,1) + DN(1,2)*v(1,1) + DN(2,2)*v(2,1) + DN(3,2)*v(3,1)));
const double crRHS30 = crRHS20*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] + DN(3,1)*p[3] + crRHS26 - crRHS27 + crRHS28 + crRHS29);
const double crRHS31 = sigma*(N[0]*v(0,2) + N[1]*v(1,2) + N[2]*v(2,2) + N[3]*v(3,2));
const double crRHS32 = crRHS2*(N[0]*f(0,2) + N[1]*f(1,2) + N[2]*f(2,2) + N[3]*f(3,2));
const double crRHS33 = crRHS2*(N[0]*(bdf0*v(0,2) + bdf1*vn(0,2) + bdf2*vnn(0,2)) + N[1]*(bdf0*v(1,2) + bdf1*vn(1,2) + bdf2*vnn(1,2)) + N[2]*(bdf0*v(2,2) + bdf1*vn(2,2) + bdf2*vnn(2,2)) + N[3]*(bdf0*v(3,2) + bdf1*vn(3,2) + bdf2*vnn(3,2)));
const double crRHS34 = crRHS2*(crRHS16*crRHS8 + crRHS6*(DN(0,0)*v(0,2) + DN(1,0)*v(1,2) + DN(2,0)*v(2,2) + DN(3,0)*v(3,2)) + crRHS7*(DN(0,1)*v(0,2) + DN(1,1)*v(1,2) + DN(2,1)*v(2,2) + DN(3,1)*v(3,2)));
const double crRHS35 = crRHS20*(DN(0,2)*p[0] + DN(1,2)*p[1] + DN(2,2)*p[2] + DN(3,2)*p[3] + crRHS31 - crRHS32 + crRHS33 + crRHS34);
const double crRHS36 = N[1]*sigma;
const double crRHS37 = N[1]*crRHS23;
const double crRHS38 = crRHS2*(DN(1,0)*crRHS6 + DN(1,1)*crRHS7 + DN(1,2)*crRHS8);
const double crRHS39 = N[2]*sigma;
const double crRHS40 = N[2]*crRHS23;
const double crRHS41 = crRHS2*(DN(2,0)*crRHS6 + DN(2,1)*crRHS7 + DN(2,2)*crRHS8);
const double crRHS42 = N[3]*sigma;
const double crRHS43 = N[3]*crRHS23;
const double crRHS44 = crRHS2*(DN(3,0)*crRHS6 + DN(3,1)*crRHS7 + DN(3,2)*crRHS8);
rRHS[0]+=-gauss_weight*(-DN(0,0)*crRHS0 + DN(0,0)*crRHS18 + DN(0,0)*stress[0] + DN(0,1)*stress[3] + DN(0,2)*stress[5] + N[0]*crRHS1 - N[0]*crRHS3 + N[0]*crRHS4 + N[0]*crRHS9 - crRHS21*crRHS22 + crRHS21*crRHS24 + crRHS21*crRHS25);
rRHS[1]+=-gauss_weight*(DN(0,0)*stress[3] - DN(0,1)*crRHS0 + DN(0,1)*crRHS18 + DN(0,1)*stress[1] + DN(0,2)*stress[4] + N[0]*crRHS26 - N[0]*crRHS27 + N[0]*crRHS28 + N[0]*crRHS29 - crRHS22*crRHS30 + crRHS24*crRHS30 + crRHS25*crRHS30);
rRHS[2]+=-gauss_weight*(DN(0,0)*stress[5] + DN(0,1)*stress[4] - DN(0,2)*crRHS0 + DN(0,2)*crRHS18 + DN(0,2)*stress[2] + N[0]*crRHS31 - N[0]*crRHS32 + N[0]*crRHS33 + N[0]*crRHS34 - crRHS22*crRHS35 + crRHS24*crRHS35 + crRHS25*crRHS35);
rRHS[3]+=-gauss_weight*(DN(0,0)*crRHS21 + DN(0,1)*crRHS30 + DN(0,2)*crRHS35 + N[0]*crRHS13 + N[0]*crRHS14 + N[0]*crRHS17);
rRHS[4]+=-gauss_weight*(-DN(1,0)*crRHS0 + DN(1,0)*crRHS18 + DN(1,0)*stress[0] + DN(1,1)*stress[3] + DN(1,2)*stress[5] + N[1]*crRHS1 - N[1]*crRHS3 + N[1]*crRHS4 + N[1]*crRHS9 - crRHS21*crRHS36 + crRHS21*crRHS37 + crRHS21*crRHS38);
rRHS[5]+=-gauss_weight*(DN(1,0)*stress[3] - DN(1,1)*crRHS0 + DN(1,1)*crRHS18 + DN(1,1)*stress[1] + DN(1,2)*stress[4] + N[1]*crRHS26 - N[1]*crRHS27 + N[1]*crRHS28 + N[1]*crRHS29 - crRHS30*crRHS36 + crRHS30*crRHS37 + crRHS30*crRHS38);
rRHS[6]+=-gauss_weight*(DN(1,0)*stress[5] + DN(1,1)*stress[4] - DN(1,2)*crRHS0 + DN(1,2)*crRHS18 + DN(1,2)*stress[2] + N[1]*crRHS31 - N[1]*crRHS32 + N[1]*crRHS33 + N[1]*crRHS34 - crRHS35*crRHS36 + crRHS35*crRHS37 + crRHS35*crRHS38);
rRHS[7]+=-gauss_weight*(DN(1,0)*crRHS21 + DN(1,1)*crRHS30 + DN(1,2)*crRHS35 + N[1]*crRHS13 + N[1]*crRHS14 + N[1]*crRHS17);
rRHS[8]+=-gauss_weight*(-DN(2,0)*crRHS0 + DN(2,0)*crRHS18 + DN(2,0)*stress[0] + DN(2,1)*stress[3] + DN(2,2)*stress[5] + N[2]*crRHS1 - N[2]*crRHS3 + N[2]*crRHS4 + N[2]*crRHS9 - crRHS21*crRHS39 + crRHS21*crRHS40 + crRHS21*crRHS41);
rRHS[9]+=-gauss_weight*(DN(2,0)*stress[3] - DN(2,1)*crRHS0 + DN(2,1)*crRHS18 + DN(2,1)*stress[1] + DN(2,2)*stress[4] + N[2]*crRHS26 - N[2]*crRHS27 + N[2]*crRHS28 + N[2]*crRHS29 - crRHS30*crRHS39 + crRHS30*crRHS40 + crRHS30*crRHS41);
rRHS[10]+=-gauss_weight*(DN(2,0)*stress[5] + DN(2,1)*stress[4] - DN(2,2)*crRHS0 + DN(2,2)*crRHS18 + DN(2,2)*stress[2] + N[2]*crRHS31 - N[2]*crRHS32 + N[2]*crRHS33 + N[2]*crRHS34 - crRHS35*crRHS39 + crRHS35*crRHS40 + crRHS35*crRHS41);
rRHS[11]+=-gauss_weight*(DN(2,0)*crRHS21 + DN(2,1)*crRHS30 + DN(2,2)*crRHS35 + N[2]*crRHS13 + N[2]*crRHS14 + N[2]*crRHS17);
rRHS[12]+=-gauss_weight*(-DN(3,0)*crRHS0 + DN(3,0)*crRHS18 + DN(3,0)*stress[0] + DN(3,1)*stress[3] + DN(3,2)*stress[5] + N[3]*crRHS1 - N[3]*crRHS3 + N[3]*crRHS4 + N[3]*crRHS9 - crRHS21*crRHS42 + crRHS21*crRHS43 + crRHS21*crRHS44);
rRHS[13]+=-gauss_weight*(DN(3,0)*stress[3] - DN(3,1)*crRHS0 + DN(3,1)*crRHS18 + DN(3,1)*stress[1] + DN(3,2)*stress[4] + N[3]*crRHS26 - N[3]*crRHS27 + N[3]*crRHS28 + N[3]*crRHS29 - crRHS30*crRHS42 + crRHS30*crRHS43 + crRHS30*crRHS44);
rRHS[14]+=-gauss_weight*(DN(3,0)*stress[5] + DN(3,1)*stress[4] - DN(3,2)*crRHS0 + DN(3,2)*crRHS18 + DN(3,2)*stress[2] + N[3]*crRHS31 - N[3]*crRHS32 + N[3]*crRHS33 + N[3]*crRHS34 - crRHS35*crRHS42 + crRHS35*crRHS43 + crRHS35*crRHS44);
rRHS[15]+=-gauss_weight*(DN(3,0)*crRHS21 + DN(3,1)*crRHS30 + DN(3,2)*crRHS35 + N[3]*crRHS13 + N[3]*crRHS14 + N[3]*crRHS17);
