...
template< class TElementData >
void QSVMSDEMCoupled<TElementData>::GetShapeSecondDerivatives(
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (IndexType g = 0; g < number_of_integration_points; ++g ) {
        ...
        if (rVariable == PERMEABILITY){
            mViscousResistanceTensor[g] = rOutput[g];
        }
    }
}
...
template< class TElementData >
void QSVMSDEMCoupled<TElementData>::SetValuesOnIntegrationPoints(
    const Variable<array_1d<double,3>>& rVariable,
    ...
    array_1d<double,3>& rResidual) const
{
    ...
    for (unsigned int i = 0; i < NumNodes; i++) {
        ...
        for (unsigned int d = 0; d < Dim; d++) {
            ...
            rResidual[d] += density * ( rData.N[i]*( - r_acceleration[d]) - convection[i]*r_velocities(i,d)) + 2.0 * viscosity * div_sym_grad_u[d] - 2.0/3.0 * viscosity * grad_div_u[d] - rData.DN_DX(i,d) * r_pressures[i] - sigma_U[d];
        }
    }
    for (unsigned int d = 0; d < Dim; d++)
    ...
    VectorType& rLocalRHS)
{
    ...
    const double density = this->GetAtCoordinate(rData.Density, rData.N);
    const array_1d<double,3> body_force = density * mExactBodyForce[rData.IntegrationPointIndex]; // Force per unit of volume
    ...
    for (unsigned int i = 0; i < NumNodes; i++)
    {
        ...
        for (unsigned int j = 0; j < NumNodes; j++)
        {
            ...
            for (unsigned int d = 0; d < Dim; d++)
            {
                ...
                LHS(row+Dim,col+d) += rData.Weight * (GAlphaA + U + QD - GAlphaL + GAlphaC - EQD - QU);
                LHS(row+d,col+Dim) += rData.Weight * (AG - P + LG - CG + DP + EGP);
        ...
        rLocalRHS[row+Dim] += rData.Weight * (QAlphaF + Q - QPhi);
    }
    ...
    double &rMassRHS) const
{
        ...
        const double fluid_fraction = mExactPorosity[rData.IntegrationPointIndex];
        const double epsilon = 0.0 * 1e-3 * tau_one(0,0)/std::pow(rData.ElementSize,2);
        const auto fluid_fraction_gradient = mExactPorosityGradient[rData.IntegrationPointIndex];
    ...
    double &TauTwo) const
{
    ...
    const double c_alpha = 1.0 + h / std::sqrt(c1) * fluid_fraction_gradient_norm;

    const double inv_tau_NS = c1 * viscosity / std::pow(h,2) + density * (c2 * velocity_norm / h );
    const double tau_one_NS = 1.0 / inv_tau_NS;

    const double inv_tau = inv_tau_NS * c_alpha;
    const double tau_one = 1 / (inv_tau + sigma(0,0));
