...
#if !defined(KRATOS_COMPRESSIBLE_NS_BIPHASE_EXPLICIT_H_INCLUDED)
#define  KRATOS_COMPRESSIBLE_NS_BIPHASE_EXPLICIT_H_INCLUDED
...
/**
 * @brief Compressible Biphase Navier-Stokes explicit element
 * This element implements a biphase compressible Navier-Stokes in thermo-mechanical 
 * equilibrium, with an explicit formulation.
 * The formulation is written in conservative form so the element unknowns are
 * the DENSITY, DENITY_SOLID, MOMENTUM and TOTAL_ENERGY variables.
...
class CompressibleNSBiphaseExplicit : public Element
{
public:
    ///@name Type Definitions
    ///@{
    ...
    /// Block size
    constexpr static unsigned int BlockSize = TDim + 3;

    /// Counted pointer of
    KRATOS_CLASS_INTRUSIVE_POINTER_DEFINITION(CompressibleNSBiphaseExplicit);
    ...
    /// Destructor.
    ~CompressibleNSBiphaseExplicit() override = default;
    ...
    {
        KRATOS_TRY
        return Kratos::make_intrusive< CompressibleNSBiphaseExplicit < TDim, TNumNodes > >(NewId, this->GetGeometry().Create(rThisNodes), pProperties);
        KRATOS_CATCH("");
    }
    ...
    {
        KRATOS_TRY
        return Kratos::make_intrusive< CompressibleNSBiphaseExplicit < TDim, TNumNodes > >(NewId, pGeom, pProperties);
        KRATOS_CATCH("");
    }
    ...
    {
        return "CompressibleNSBiphaseExplicit #";
    }
...
protected:
    ...
    CompressibleNSBiphaseExplicit() = default;
