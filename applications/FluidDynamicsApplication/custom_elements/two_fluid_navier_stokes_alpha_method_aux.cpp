...
{
    if (rVariable == VELOCITY_RESIDUAL_NORM) {
        Vector temp_RHS;
        this->CalculateRightHandSide(temp_RHS, rCurrentProcessInfo);
        rOutput = 0.0;
        for (IndexType i = 0; i < NumNodes; ++i) {
            for (IndexType d = 0; d < Dim; ++d) {
                rOutput = std::pow(temp_RHS[i*BlockSize + d],2);
            }
        }
        rOutput = sqrt(rOutput);
    }
    else
    {
    ...
    const ProcessInfo &rCurrentProcessInfo)
{
    ...
    {
        const double aux_val = this->GetValue(ARTIFICIAL_DYNAMIC_VISCOSITY);


        for (double& r_val : rOutput) {
            r_val = aux_val;
   ...
   else if (rVariable == ARTIFICIAL_MASS_DIFFUSIVITY ){
        const double aux_val = this->GetValue(ARTIFICIAL_MASS_DIFFUSIVITY);
        for (double& r_val : rOutput) {
            r_val = aux_val;
        ...
        const double aux_val = this->GetValue(ARTIFICIAL_CONDUCTIVITY);
        for (double& r_val : rOutput) {
            r_val = aux_val;
