...
const double crhs12 = DN(0,0)*v(0,0) + DN(1,0)*v(1,0) + DN(2,0)*v(2,0);
const double crhs13 = N[0]*vconv(0,0) + N[1]*vconv(1,0) + N[2]*vconv(2,0);
const double crhs14 = N[0]*vconv(0,1) + N[1]*vconv(1,1) + N[2]*vconv(2,1);
const double crhs15 = rho*(crhs12*crhs13 + crhs14*(DN(0,1)*v(0,0) + DN(1,1)*v(1,0) + DN(2,1)*v(2,0)));
const double crhs16 = N[0]*vn(0,0) + N[1]*vn(1,0) + N[2]*vn(2,0);
const double crhs17 = N[0]*vn(0,1) + N[1]*vn(1,1) + N[2]*vn(2,1);
const double crhs18 = crhs16*(DN(0,0)*vn(0,0) + DN(1,0)*vn(1,0) + DN(2,0)*vn(2,0)) + crhs17*(DN(0,1)*vn(0,0) + DN(1,1)*vn(1,0) + DN(2,1)*vn(2,0));
const double crhs19 = DN(0,1)*v(0,1) + DN(1,1)*v(1,1) + DN(2,1)*v(2,1);
const double crhs20 = crhs12 + crhs19 - volume_error_ratio;
const double crhs21 = rho*stab_c2*sqrt(pow(crhs13, 2) + pow(crhs14, 2));
const double crhs22 = crhs20*(crhs21*h/stab_c1 + mu);
const double crhs23 = bdf0*rho;
const double crhs24 = crhs23*crhs3;
const double crhs25 = crhs10*rho;
const double crhs26 = 1.0/(K_darcy + crhs21/h + crhs25*dyn_tau + mu*stab_c1/pow(h, 2));
const double crhs27 = crhs26*(DN(0,0)*p[0] + DN(1,0)*p[1] + DN(2,0)*p[2] - crhs1 + crhs15 + crhs2 + crhs24 + rho*(crhs10*crhs6 + crhs10*crhs7 + crhs10*crhs8 + crhs18));
const double crhs28 = K_darcy*N[0];
const double crhs29 = DN(0,0)*vconv(0,0) + DN(0,1)*vconv(0,1) + DN(1,0)*vconv(1,0) + DN(1,1)*vconv(1,1) + DN(2,0)*vconv(2,0) + DN(2,1)*vconv(2,1);
const double crhs30 = crhs29*crhs4;
const double crhs31 = rho*(DN(0,0)*crhs13 + DN(0,1)*crhs14);
const double crhs32 = rho*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1));
const double crhs33 = K_darcy*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1));
...
const double crhs39 = rho*(crhs13*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1)) + crhs14*crhs19);
const double crhs40 = crhs16*(DN(0,0)*vn(0,1) + DN(1,0)*vn(1,1) + DN(2,0)*vn(2,1)) + crhs17*(DN(0,1)*vn(0,1) + DN(1,1)*vn(1,1) + DN(2,1)*vn(2,1));
const double crhs41 = crhs23*crhs34;
const double crhs42 = crhs26*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] - crhs32 + crhs33 + crhs39 + crhs41 + rho*(crhs10*crhs35 + crhs10*crhs36 + crhs10*crhs37 + crhs40));
const double crhs43 = N[1]*crhs25;
const double crhs44 = N[1]*rho;
const double crhs45 = K_darcy*N[1];
const double crhs46 = crhs29*crhs44;
const double crhs47 = rho*(DN(1,0)*crhs13 + DN(1,1)*crhs14);
const double crhs48 = N[2]*crhs25;
const double crhs49 = N[2]*rho;
const double crhs50 = K_darcy*N[2];
const double crhs51 = crhs29*crhs49;
const double crhs52 = rho*(DN(2,0)*crhs13 + DN(2,1)*crhs14);
rhs[0]=DN(0,0)*crhs0 - DN(0,0)*crhs22 - DN(0,0)*stress[0] - DN(0,1)*stress[2] + N[0]*crhs1 - N[0]*crhs15 - N[0]*crhs2 - crhs11*crhs9 - crhs18*crhs4 + crhs27*crhs28 - crhs27*crhs30 - crhs27*crhs31 - crhs3*crhs5;
rhs[1]=-DN(0,0)*stress[2] + DN(0,1)*crhs0 - DN(0,1)*crhs22 - DN(0,1)*stress[1] + N[0]*crhs32 - N[0]*crhs33 - N[0]*crhs39 - crhs11*crhs38 + crhs28*crhs42 - crhs30*crhs42 - crhs31*crhs42 - crhs34*crhs5 - crhs4*crhs40;
rhs[2]=-DN(0,0)*crhs27 - DN(0,1)*crhs42 - N[0]*crhs20;
rhs[3]=DN(1,0)*crhs0 - DN(1,0)*crhs22 - DN(1,0)*stress[0] - DN(1,1)*stress[2] + N[1]*crhs1 - N[1]*crhs15 - N[1]*crhs2 - N[1]*crhs24 - crhs18*crhs44 + crhs27*crhs45 - crhs27*crhs46 - crhs27*crhs47 - crhs43*crhs9;
rhs[4]=-DN(1,0)*stress[2] + DN(1,1)*crhs0 - DN(1,1)*crhs22 - DN(1,1)*stress[1] + N[1]*crhs32 - N[1]*crhs33 - N[1]*crhs39 - N[1]*crhs41 - crhs38*crhs43 - crhs40*crhs44 + crhs42*crhs45 - crhs42*crhs46 - crhs42*crhs47;
rhs[5]=-DN(1,0)*crhs27 - DN(1,1)*crhs42 - N[1]*crhs20;
rhs[6]=DN(2,0)*crhs0 - DN(2,0)*crhs22 - DN(2,0)*stress[0] - DN(2,1)*stress[2] + N[2]*crhs1 - N[2]*crhs15 - N[2]*crhs2 - N[2]*crhs24 - crhs18*crhs49 + crhs27*crhs50 - crhs27*crhs51 - crhs27*crhs52 - crhs48*crhs9;
rhs[7]=-DN(2,0)*stress[2] + DN(2,1)*crhs0 - DN(2,1)*crhs22 - DN(2,1)*stress[1] + N[2]*crhs32 - N[2]*crhs33 - N[2]*crhs39 - N[2]*crhs41 - crhs38*crhs48 - crhs40*crhs49 + crhs42*crhs50 - crhs42*crhs51 - crhs42*crhs52;
rhs[8]=-DN(2,0)*crhs27 - DN(2,1)*crhs42 - N[2]*crhs20;
...
const double crhs13 = DN(0,0)*v(0,0) + DN(1,0)*v(1,0) + DN(2,0)*v(2,0) + DN(3,0)*v(3,0);
...
const double crhs17 = rho*(crhs13*crhs14 + crhs15*(DN(0,1)*v(0,0) + DN(1,1)*v(1,0) + DN(2,1)*v(2,0) + DN(3,1)*v(3,0)) + crhs16*(DN(0,2)*v(0,0) + DN(1,2)*v(1,0) + DN(2,2)*v(2,0) + DN(3,2)*v(3,0)));
...
const double crhs24 = crhs13 + crhs22 + crhs23 - volume_error_ratio;
...
const double crhs29 = crhs11*rho;
const double crhs30 = 1.0/(K_darcy + crhs25/h + crhs29*dyn_tau + mu*stab_c1/pow(h, 2));
const double crhs31 = crhs30*(DN(0,0)*p[0] + DN(1,0)*p[1] + DN(2,0)*p[2] + DN(3,0)*p[3] - crhs1 + crhs17 + crhs2 + crhs28 + rho*(crhs11*crhs6 + crhs11*crhs7 + crhs11*crhs8 + crhs11*crhs9 + crhs21));
const double crhs32 = K_darcy*N[0];
...
const double crhs34 = crhs33*crhs4;
const double crhs35 = rho*(DN(0,0)*crhs14 + DN(0,1)*crhs15 + DN(0,2)*crhs16);
const double crhs36 = rho*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1) + N[3]*f(3,1));
const double crhs37 = K_darcy*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1) + N[3]*v(3,1));
const double crhs38 = N[0]*(v(0,1) - vfrac(0,1)) + N[1]*(v(1,1) - vfrac(1,1)) + N[2]*(v(2,1) - vfrac(2,1)) + N[3]*(v(3,1) - vfrac(3,1));
const double crhs39 = N[0]*(vn(0,1) - vnn(0,1));
const double crhs40 = N[1]*(vn(1,1) - vnn(1,1));
const double crhs41 = N[2]*(vn(2,1) - vnn(2,1));
const double crhs42 = N[3]*(vn(3,1) - vnn(3,1));
const double crhs43 = crhs39 + crhs40 + crhs41 + crhs42;
const double crhs44 = rho*(crhs14*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1) + DN(3,0)*v(3,1)) + crhs15*crhs22 + crhs16*(DN(0,2)*v(0,1) + DN(1,2)*v(1,1) + DN(2,2)*v(2,1) + DN(3,2)*v(3,1)));
const double crhs45 = crhs18*(DN(0,0)*vn(0,1) + DN(1,0)*vn(1,1) + DN(2,0)*vn(2,1) + DN(3,0)*vn(3,1)) + crhs19*(DN(0,1)*vn(0,1) + DN(1,1)*vn(1,1) + DN(2,1)*vn(2,1) + DN(3,1)*vn(3,1)) + crhs20*(DN(0,2)*vn(0,1) + DN(1,2)*vn(1,1) + DN(2,2)*vn(2,1) + DN(3,2)*vn(3,1));
const double crhs46 = crhs27*crhs38;
const double crhs47 = crhs30*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] + DN(3,1)*p[3] - crhs36 + crhs37 + crhs44 + crhs46 + rho*(crhs11*crhs39 + crhs11*crhs40 + crhs11*crhs41 + crhs11*crhs42 + crhs45));
const double crhs48 = rho*(N[0]*f(0,2) + N[1]*f(1,2) + N[2]*f(2,2) + N[3]*f(3,2));
const double crhs49 = K_darcy*(N[0]*v(0,2) + N[1]*v(1,2) + N[2]*v(2,2) + N[3]*v(3,2));
const double crhs50 = N[0]*(v(0,2) - vfrac(0,2)) + N[1]*(v(1,2) - vfrac(1,2)) + N[2]*(v(2,2) - vfrac(2,2)) + N[3]*(v(3,2) - vfrac(3,2));
const double crhs51 = N[0]*(vn(0,2) - vnn(0,2));
const double crhs52 = N[1]*(vn(1,2) - vnn(1,2));
const double crhs53 = N[2]*(vn(2,2) - vnn(2,2));
const double crhs54 = N[3]*(vn(3,2) - vnn(3,2));
const double crhs55 = crhs51 + crhs52 + crhs53 + crhs54;
const double crhs56 = rho*(crhs14*(DN(0,0)*v(0,2) + DN(1,0)*v(1,2) + DN(2,0)*v(2,2) + DN(3,0)*v(3,2)) + crhs15*(DN(0,1)*v(0,2) + DN(1,1)*v(1,2) + DN(2,1)*v(2,2) + DN(3,1)*v(3,2)) + crhs16*crhs23);
const double crhs57 = crhs18*(DN(0,0)*vn(0,2) + DN(1,0)*vn(1,2) + DN(2,0)*vn(2,2) + DN(3,0)*vn(3,2)) + crhs19*(DN(0,1)*vn(0,2) + DN(1,1)*vn(1,2) + DN(2,1)*vn(2,2) + DN(3,1)*vn(3,2)) + crhs20*(DN(0,2)*vn(0,2) + DN(1,2)*vn(1,2) + DN(2,2)*vn(2,2) + DN(3,2)*vn(3,2));
const double crhs58 = crhs27*crhs50;
const double crhs59 = crhs30*(DN(0,2)*p[0] + DN(1,2)*p[1] + DN(2,2)*p[2] + DN(3,2)*p[3] - crhs48 + crhs49 + crhs56 + crhs58 + rho*(crhs11*crhs51 + crhs11*crhs52 + crhs11*crhs53 + crhs11*crhs54 + crhs57));
const double crhs60 = N[1]*crhs29;
const double crhs61 = N[1]*rho;
const double crhs62 = K_darcy*N[1];
const double crhs63 = crhs33*crhs61;
const double crhs64 = rho*(DN(1,0)*crhs14 + DN(1,1)*crhs15 + DN(1,2)*crhs16);
const double crhs65 = N[2]*crhs29;
const double crhs66 = N[2]*rho;
const double crhs67 = K_darcy*N[2];
const double crhs68 = crhs33*crhs66;
const double crhs69 = rho*(DN(2,0)*crhs14 + DN(2,1)*crhs15 + DN(2,2)*crhs16);
const double crhs70 = N[3]*crhs29;
const double crhs71 = N[3]*rho;
const double crhs72 = K_darcy*N[3];
const double crhs73 = crhs33*crhs71;
const double crhs74 = rho*(DN(3,0)*crhs14 + DN(3,1)*crhs15 + DN(3,2)*crhs16);
rhs[0]=DN(0,0)*crhs0 - DN(0,0)*crhs26 - DN(0,0)*stress[0] - DN(0,1)*stress[3] - DN(0,2)*stress[5] + N[0]*crhs1 - N[0]*crhs17 - N[0]*crhs2 - crhs10*crhs12 - crhs21*crhs4 - crhs3*crhs5 + crhs31*crhs32 - crhs31*crhs34 - crhs31*crhs35;
rhs[1]=-DN(0,0)*stress[3] + DN(0,1)*crhs0 - DN(0,1)*crhs26 - DN(0,1)*stress[1] - DN(0,2)*stress[4] + N[0]*crhs36 - N[0]*crhs37 - N[0]*crhs44 - crhs12*crhs43 + crhs32*crhs47 - crhs34*crhs47 - crhs35*crhs47 - crhs38*crhs5 - crhs4*crhs45;
rhs[2]=-DN(0,0)*stress[5] - DN(0,1)*stress[4] + DN(0,2)*crhs0 - DN(0,2)*crhs26 - DN(0,2)*stress[2] + N[0]*crhs48 - N[0]*crhs49 - N[0]*crhs56 - crhs12*crhs55 + crhs32*crhs59 - crhs34*crhs59 - crhs35*crhs59 - crhs4*crhs57 - crhs5*crhs50;
rhs[3]=-DN(0,0)*crhs31 - DN(0,1)*crhs47 - DN(0,2)*crhs59 - N[0]*crhs24;
rhs[4]=DN(1,0)*crhs0 - DN(1,0)*crhs26 - DN(1,0)*stress[0] - DN(1,1)*stress[3] - DN(1,2)*stress[5] + N[1]*crhs1 - N[1]*crhs17 - N[1]*crhs2 - N[1]*crhs28 - crhs10*crhs60 - crhs21*crhs61 + crhs31*crhs62 - crhs31*crhs63 - crhs31*crhs64;
rhs[5]=-DN(1,0)*stress[3] + DN(1,1)*crhs0 - DN(1,1)*crhs26 - DN(1,1)*stress[1] - DN(1,2)*stress[4] + N[1]*crhs36 - N[1]*crhs37 - N[1]*crhs44 - N[1]*crhs46 - crhs43*crhs60 - crhs45*crhs61 + crhs47*crhs62 - crhs47*crhs63 - crhs47*crhs64;
rhs[6]=-DN(1,0)*stress[5] - DN(1,1)*stress[4] + DN(1,2)*crhs0 - DN(1,2)*crhs26 - DN(1,2)*stress[2] + N[1]*crhs48 - N[1]*crhs49 - N[1]*crhs56 - N[1]*crhs58 - crhs55*crhs60 - crhs57*crhs61 + crhs59*crhs62 - crhs59*crhs63 - crhs59*crhs64;
rhs[7]=-DN(1,0)*crhs31 - DN(1,1)*crhs47 - DN(1,2)*crhs59 - N[1]*crhs24;
rhs[8]=DN(2,0)*crhs0 - DN(2,0)*crhs26 - DN(2,0)*stress[0] - DN(2,1)*stress[3] - DN(2,2)*stress[5] + N[2]*crhs1 - N[2]*crhs17 - N[2]*crhs2 - N[2]*crhs28 - crhs10*crhs65 - crhs21*crhs66 + crhs31*crhs67 - crhs31*crhs68 - crhs31*crhs69;
rhs[9]=-DN(2,0)*stress[3] + DN(2,1)*crhs0 - DN(2,1)*crhs26 - DN(2,1)*stress[1] - DN(2,2)*stress[4] + N[2]*crhs36 - N[2]*crhs37 - N[2]*crhs44 - N[2]*crhs46 - crhs43*crhs65 - crhs45*crhs66 + crhs47*crhs67 - crhs47*crhs68 - crhs47*crhs69;
rhs[10]=-DN(2,0)*stress[5] - DN(2,1)*stress[4] + DN(2,2)*crhs0 - DN(2,2)*crhs26 - DN(2,2)*stress[2] + N[2]*crhs48 - N[2]*crhs49 - N[2]*crhs56 - N[2]*crhs58 - crhs55*crhs65 - crhs57*crhs66 + crhs59*crhs67 - crhs59*crhs68 - crhs59*crhs69;
rhs[11]=-DN(2,0)*crhs31 - DN(2,1)*crhs47 - DN(2,2)*crhs59 - N[2]*crhs24;
rhs[12]=DN(3,0)*crhs0 - DN(3,0)*crhs26 - DN(3,0)*stress[0] - DN(3,1)*stress[3] - DN(3,2)*stress[5] + N[3]*crhs1 - N[3]*crhs17 - N[3]*crhs2 - N[3]*crhs28 - crhs10*crhs70 - crhs21*crhs71 + crhs31*crhs72 - crhs31*crhs73 - crhs31*crhs74;
rhs[13]=-DN(3,0)*stress[3] + DN(3,1)*crhs0 - DN(3,1)*crhs26 - DN(3,1)*stress[1] - DN(3,2)*stress[4] + N[3]*crhs36 - N[3]*crhs37 - N[3]*crhs44 - N[3]*crhs46 - crhs43*crhs70 - crhs45*crhs71 + crhs47*crhs72 - crhs47*crhs73 - crhs47*crhs74;
rhs[14]=-DN(3,0)*stress[5] - DN(3,1)*stress[4] + DN(3,2)*crhs0 - DN(3,2)*crhs26 - DN(3,2)*stress[2] + N[3]*crhs48 - N[3]*crhs49 - N[3]*crhs56 - N[3]*crhs58 - crhs55*crhs70 - crhs57*crhs71 + crhs59*crhs72 - crhs59*crhs73 - crhs59*crhs74;
rhs[15]=-DN(3,0)*crhs31 - DN(3,1)*crhs47 - DN(3,2)*crhs59 - N[3]*crhs24;
