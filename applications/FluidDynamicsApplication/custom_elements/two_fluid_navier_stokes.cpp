...
// template <class TElementData>
// double TwoFluidNavierStokes<TElementData>::ComputeMassSourceTerm(TElementData &rData)
// {
//     //FIXME: Should we create a new data or use the one passed by reference?
//     TElementData data;
//     Vector int_gauss_pts_weights;
...
//         ModifiedShapeFunctions::Pointer p_modified_sh_func = pGetModifiedShapeFunctionsUtility(p_geom,rData.Distance);
//         Matrix int_shape_function, int_shape_function_enr_neg, int_shape_function_enr_pos;
//         GeometryType::ShapeFunctionsGradientsType int_shape_derivatives;
//         Vector int_gauss_pts_weights;
//         std::vector<array_1d<double, 3>> int_normals_neg;
...
//         const auto &r_geom = this->GetGeometry();
//         double epsilon_int = 0.0;
//         //TODO: We calculate the mass source term as the average of the ones in the interface Gauss pts.
//         //TODO: Enhance the ComputeSplitInterface API to pass a custom quadrature
//         double element_volume=0.0;
//         double element_interface=0.0;
//         for (unsigned int intgp = 0; intgp < int_gauss_pts_weights.size(); ++intgp)
//         {
...
//             }
//             element_interface+=int_gauss_pts_weights[intgp];
//             u_prime /= rData.DeltaTime;
//             const array_1d<double, 3> &r_n = int_normals_neg[intgp];
//             double u_inter = u_prime;
//             u_inter -= inner_prod(u_ns, r_n);
//             KRATOS_WATCH("BEFORE DIVIDING BY RHO1-RHO2")
//             KRATOS_WATCH(u_inter)
//             u_interface += (-negative_density+positive_density)*u_inter;


//         }
//         KRATOS_WATCH("AFTERDIVIDING AND OUT OF THE LOOP ")
//         KRATOS_WATCH(u_interface)

//         element_volume=r_geom.DomainSize();
//         u_interface /= int_gauss_pts_weights.size();
//         // u_interface/=element_volume;
//         // u_interface*=element_interface;
//         u_interface*=rData.ElementSize;
//         KRATOS_WATCH(rData.ElementSize)
//         KRATOS_WATCH(element_interface/element_volume)
//         KRATOS_WATCH("CORRECT UNITS")
//         KRATOS_WATCH(u_interface)
...
double TwoFluidNavierStokes<TElementData>::ComputeMassSourceTerm(TElementData &rData)
{
    ...
    {   KRATOS_WATCH(" ELEMENTO CORTADO")
        ...
        for (unsigned int intgp = 0; intgp < int_gauss_pts_weights.size(); ++intgp)
        {
            ...
            u_prime /= rData.DeltaTime;
            double u_inter = u_prime;
            u_field+=inner_prod(u_ns, r_n);
            u_inter -= inner_prod(u_ns, r_n);
            element_interface+=int_gauss_pts_weights[intgp];

            u_interface +=u_inter;
            // KRATOS_WATCH("punto de gauss de la interfaz")
            // KRATOS_WATCH(intgp)
            // KRATOS_WATCH(u_prime)
            // KRATOS_WATCH(u_inter)
            // KRATOS_WATCH(inner_prod(u_ns, r_n))
        ...
        "documentation"   :
            "This element implements Navier-Stokes biphasic fluid-air formulation with a levelset-based interface representation with Variational MultiScales (VMS) stabilization. Note that any viscous behavior can be used for the fluid phase through a constitutive law. The air phase is assumed to be Newtonian. Surface tension contribution can be accounted for by setting the SURFACE_TENSION variable to true in the ProcessInfo container.
                    ...
                    VectorType & rRHS)
                {
                    ...
                    bool momentum_mass =true;
                    if (rData.IsCut())
                    {
                        if (momentum_mass)
                        {
                            double new_error=ComputeMassSourceTerm(rData);
                            volume_error_ratio = -new_error;
                    ...
                    const double crhs15 = rho * (DN(0, 0) * vconv(0, 0) + DN(0, 1) * vconv(0, 1) + DN(1, 0) * vconv(1, 0) + DN(1, 1) * vconv(1, 1) + DN(2, 0) * vconv(2, 0) + DN(2, 1) * vconv(2, 1));
                    const double crhs16 = N[0] * crhs15;
                    const double crhs17 = rho * (DN(0, 0) * crhs5 + DN(0, 1) * crhs6);
                    const double crhs18 = rho * (N[0] * f(0, 1) + N[1] * f(1, 1) + N[2] * f(2, 1));
                    const double crhs19 = K_darcy * (N[0] * v(0, 1) + N[1] * v(1, 1) + N[2] * v(2, 1));
                    const double crhs20 = rho * (N[0] * (bdf0 * v(0, 1) + bdf1 * vn(0, 1) + bdf2 * vnn(0, 1)) + N[1] * (bdf0 * v(1, 1) + bdf1 * vn(1, 1) + bdf2 * vnn(1, 1)) + N[2] * (bdf0 * v(2, 1) + bdf1 * vn(2, 1) + bdf2 * vnn(2, 1)));
                    const double crhs21 = rho * (crhs5 * (DN(0, 0) * v(0, 1) + DN(1, 0) * v(1, 1) + DN(2, 0) * v(2, 1)) + crhs6 * crhs8);
                    const double crhs22 = crhs12 * (DN(0, 1) * p[0] + DN(1, 1) * p[1] + DN(2, 1) * p[2] - crhs18 + crhs19 + crhs20 + crhs21);
                    const double crhs23 = K_darcy * N[1];
                    const double crhs24 = N[1] * crhs15;
                    const double crhs25 = rho * (DN(1, 0) * crhs5 + DN(1, 1) * crhs6);
                    const double crhs26 = K_darcy * N[2];
                    const double crhs27 = N[2] * crhs15;
                    const double crhs28 = rho * (DN(2, 0) * crhs5 + DN(2, 1) * crhs6);
                    rhs[0] = DN(0, 0) * crhs0 - DN(0, 0) * crhs11 - DN(0, 0) * stress[0] - DN(0, 1) * stress[2] + N[0] * crhs1 - N[0] * crhs2 - N[0] * crhs3 - N[0] * crhs7 + crhs13 * crhs14 - crhs13 * crhs16 - crhs13 * crhs17;
                    rhs[1] = -DN(0, 0) * stress[2] + DN(0, 1) * crhs0 - DN(0, 1) * crhs11 - DN(0, 1) * stress[1] + N[0] * crhs18 - N[0] * crhs19 - N[0] * crhs20 - N[0] * crhs21 + crhs14 * crhs22 - crhs16 * crhs22 - crhs17 * crhs22;
                    rhs[2] = -DN(0, 0) * crhs13 - DN(0, 1) * crhs22 - N[0] * crhs9;
                    rhs[3] = DN(1, 0) * crhs0 - DN(1, 0) * crhs11 - DN(1, 0) * stress[0] - DN(1, 1) * stress[2] + N[1] * crhs1 - N[1] * crhs2 - N[1] * crhs3 - N[1] * crhs7 + crhs13 * crhs23 - crhs13 * crhs24 - crhs13 * crhs25;
                    rhs[4] = -DN(1, 0) * stress[2] + DN(1, 1) * crhs0 - DN(1, 1) * crhs11 - DN(1, 1) * stress[1] + N[1] * crhs18 - N[1] * crhs19 - N[1] * crhs20 - N[1] * crhs21 + crhs22 * crhs23 - crhs22 * crhs24 - crhs22 * crhs25;
                    rhs[5] = -DN(1, 0) * crhs13 - DN(1, 1) * crhs22 - N[1] * crhs9;
                    rhs[6] = DN(2, 0) * crhs0 - DN(2, 0) * crhs11 - DN(2, 0) * stress[0] - DN(2, 1) * stress[2] + N[2] * crhs1 - N[2] * crhs2 - N[2] * crhs3 - N[2] * crhs7 + crhs13 * crhs26 - crhs13 * crhs27 - crhs13 * crhs28;
                    rhs[7] = -DN(2, 0) * stress[2] + DN(2, 1) * crhs0 - DN(2, 1) * crhs11 - DN(2, 1) * stress[1] + N[2] * crhs18 - N[2] * crhs19 - N[2] * crhs20 - N[2] * crhs21 + crhs22 * crhs26 - crhs22 * crhs27 - crhs22 * crhs28;
                    rhs[8] = -DN(2, 0) * crhs13 - DN(2, 1) * crhs22 - N[2] * crhs9;
                    ...
                    VectorType & rRHS)
                {
                    ...
                    const double crhs17 = rho * (DN(0, 0) * vconv(0, 0) + DN(0, 1) * vconv(0, 1) + DN(0, 2) * vconv(0, 2) + DN(1, 0) * vconv(1, 0) + DN(1, 1) * vconv(1, 1) + DN(1, 2) * vconv(1, 2) + DN(2, 0) * vconv(2, 0) + DN(2, 1) * vconv(2, 1) + DN(2, 2) * vconv(2, 2) + DN(3, 0) * vconv(3, 0) + DN(3, 1) * vconv(3, 1) + DN(3, 2) * vconv(3, 2));
                    const double crhs18 = N[0] * crhs17;
                    const double crhs19 = rho * (DN(0, 0) * crhs5 + DN(0, 1) * crhs6 + DN(0, 2) * crhs7);
                    const double crhs20 = rho * (N[0] * f(0, 1) + N[1] * f(1, 1) + N[2] * f(2, 1) + N[3] * f(3, 1));
                    const double crhs21 = K_darcy * (N[0] * v(0, 1) + N[1] * v(1, 1) + N[2] * v(2, 1) + N[3] * v(3, 1));
                    ...
                    const double crhs23 = rho * (crhs5 * (DN(0, 0) * v(0, 1) + DN(1, 0) * v(1, 1) + DN(2, 0) * v(2, 1) + DN(3, 0) * v(3, 1)) + crhs6 * crhs9 + crhs7 * (DN(0, 2) * v(0, 1) + DN(1, 2) * v(1, 1) + DN(2, 2) * v(2, 1) + DN(3, 2) * v(3, 1)));
                    const double crhs24 = crhs14 * (DN(0, 1) * p[0] + DN(1, 1) * p[1] + DN(2, 1) * p[2] + DN(3, 1) * p[3] - crhs20 + crhs21 + crhs22 + crhs23);
                    const double crhs25 = rho * (N[0] * f(0, 2) + N[1] * f(1, 2) + N[2] * f(2, 2) + N[3] * f(3, 2));
                    const double crhs26 = K_darcy * (N[0] * v(0, 2) + N[1] * v(1, 2) + N[2] * v(2, 2) + N[3] * v(3, 2));
                    ...
                    const double crhs28 = rho * (crhs10 * crhs7 + crhs5 * (DN(0, 0) * v(0, 2) + DN(1, 0) * v(1, 2) + DN(2, 0) * v(2, 2) + DN(3, 0) * v(3, 2)) + crhs6 * (DN(0, 1) * v(0, 2) + DN(1, 1) * v(1, 2) + DN(2, 1) * v(2, 2) + DN(3, 1) * v(3, 2)));
                    const double crhs29 = crhs14 * (DN(0, 2) * p[0] + DN(1, 2) * p[1] + DN(2, 2) * p[2] + DN(3, 2) * p[3] - crhs25 + crhs26 + crhs27 + crhs28);
                    const double crhs30 = K_darcy * N[1];
                    const double crhs31 = N[1] * crhs17;
                    const double crhs32 = rho * (DN(1, 0) * crhs5 + DN(1, 1) * crhs6 + DN(1, 2) * crhs7);
                    const double crhs33 = K_darcy * N[2];
                    const double crhs34 = N[2] * crhs17;
                    const double crhs35 = rho * (DN(2, 0) * crhs5 + DN(2, 1) * crhs6 + DN(2, 2) * crhs7);
                    const double crhs36 = K_darcy * N[3];
                    const double crhs37 = N[3] * crhs17;
                    const double crhs38 = rho * (DN(3, 0) * crhs5 + DN(3, 1) * crhs6 + DN(3, 2) * crhs7);
                    rhs[0] = DN(0, 0) * crhs0 - DN(0, 0) * crhs13 - DN(0, 0) * stress[0] - DN(0, 1) * stress[3] - DN(0, 2) * stress[5] + N[0] * crhs1 - N[0] * crhs2 - N[0] * crhs3 - N[0] * crhs8 + crhs15 * crhs16 - crhs15 * crhs18 - crhs15 * crhs19;
                    rhs[1] = -DN(0, 0) * stress[3] + DN(0, 1) * crhs0 - DN(0, 1) * crhs13 - DN(0, 1) * stress[1] - DN(0, 2) * stress[4] + N[0] * crhs20 - N[0] * crhs21 - N[0] * crhs22 - N[0] * crhs23 + crhs16 * crhs24 - crhs18 * crhs24 - crhs19 * crhs24;
                    rhs[2] = -DN(0, 0) * stress[5] - DN(0, 1) * stress[4] + DN(0, 2) * crhs0 - DN(0, 2) * crhs13 - DN(0, 2) * stress[2] + N[0] * crhs25 - N[0] * crhs26 - N[0] * crhs27 - N[0] * crhs28 + crhs16 * crhs29 - crhs18 * crhs29 - crhs19 * crhs29;
                    rhs[3] = -DN(0, 0) * crhs15 - DN(0, 1) * crhs24 - DN(0, 2) * crhs29 - N[0] * crhs11;
                    rhs[4] = DN(1, 0) * crhs0 - DN(1, 0) * crhs13 - DN(1, 0) * stress[0] - DN(1, 1) * stress[3] - DN(1, 2) * stress[5] + N[1] * crhs1 - N[1] * crhs2 - N[1] * crhs3 - N[1] * crhs8 + crhs15 * crhs30 - crhs15 * crhs31 - crhs15 * crhs32;
                    rhs[5] = -DN(1, 0) * stress[3] + DN(1, 1) * crhs0 - DN(1, 1) * crhs13 - DN(1, 1) * stress[1] - DN(1, 2) * stress[4] + N[1] * crhs20 - N[1] * crhs21 - N[1] * crhs22 - N[1] * crhs23 + crhs24 * crhs30 - crhs24 * crhs31 - crhs24 * crhs32;
                    rhs[6] = -DN(1, 0) * stress[5] - DN(1, 1) * stress[4] + DN(1, 2) * crhs0 - DN(1, 2) * crhs13 - DN(1, 2) * stress[2] + N[1] * crhs25 - N[1] * crhs26 - N[1] * crhs27 - N[1] * crhs28 + crhs29 * crhs30 - crhs29 * crhs31 - crhs29 * crhs32;
                    rhs[7] = -DN(1, 0) * crhs15 - DN(1, 1) * crhs24 - DN(1, 2) * crhs29 - N[1] * crhs11;
                    rhs[8] = DN(2, 0) * crhs0 - DN(2, 0) * crhs13 - DN(2, 0) * stress[0] - DN(2, 1) * stress[3] - DN(2, 2) * stress[5] + N[2] * crhs1 - N[2] * crhs2 - N[2] * crhs3 - N[2] * crhs8 + crhs15 * crhs33 - crhs15 * crhs34 - crhs15 * crhs35;
                    rhs[9] = -DN(2, 0) * stress[3] + DN(2, 1) * crhs0 - DN(2, 1) * crhs13 - DN(2, 1) * stress[1] - DN(2, 2) * stress[4] + N[2] * crhs20 - N[2] * crhs21 - N[2] * crhs22 - N[2] * crhs23 + crhs24 * crhs33 - crhs24 * crhs34 - crhs24 * crhs35;
                    rhs[10] = -DN(2, 0) * stress[5] - DN(2, 1) * stress[4] + DN(2, 2) * crhs0 - DN(2, 2) * crhs13 - DN(2, 2) * stress[2] + N[2] * crhs25 - N[2] * crhs26 - N[2] * crhs27 - N[2] * crhs28 + crhs29 * crhs33 - crhs29 * crhs34 - crhs29 * crhs35;
                    rhs[11] = -DN(2, 0) * crhs15 - DN(2, 1) * crhs24 - DN(2, 2) * crhs29 - N[2] * crhs11;
                    rhs[12] = DN(3, 0) * crhs0 - DN(3, 0) * crhs13 - DN(3, 0) * stress[0] - DN(3, 1) * stress[3] - DN(3, 2) * stress[5] + N[3] * crhs1 - N[3] * crhs2 - N[3] * crhs3 - N[3] * crhs8 + crhs15 * crhs36 - crhs15 * crhs37 - crhs15 * crhs38;
                    rhs[13] = -DN(3, 0) * stress[3] + DN(3, 1) * crhs0 - DN(3, 1) * crhs13 - DN(3, 1) * stress[1] - DN(3, 2) * stress[4] + N[3] * crhs20 - N[3] * crhs21 - N[3] * crhs22 - N[3] * crhs23 + crhs24 * crhs36 - crhs24 * crhs37 - crhs24 * crhs38;
                    rhs[14] = -DN(3, 0) * stress[5] - DN(3, 1) * stress[4] + DN(3, 2) * crhs0 - DN(3, 2) * crhs13 - DN(3, 2) * stress[2] + N[3] * crhs25 - N[3] * crhs26 - N[3] * crhs27 - N[3] * crhs28 + crhs29 * crhs36 - crhs29 * crhs37 - crhs29 * crhs38;
                    rhs[15] = -DN(3, 0) * crhs15 - DN(3, 1) * crhs24 - DN(3, 2) * crhs29 - N[3] * crhs11;
                    ...
                    VectorType &rRHS)
                {
                    for (unsigned int intgp = 0; intgp < rInterfaceWeights.size(); ++intgp)
                    {
                        ...
                        for (unsigned int j = 0; j < NumNodes; ++j)
                        {
                            for (unsigned int dim = 0; dim < NumNodes - 1; ++dim)
                            {
                                rRHS[j * (NumNodes) + dim] -= SurfaceTensionCoefficient * intgp_normal[dim] * intgp_curv * intgp_w * rInterfaceShapeFunctions(intgp, j);
                            }
                        }
                    }
                }
                    ...
                    VectorType &rRHSeeTot)
                {
                    ...
                    for (unsigned int gp = 0; gp < rInterfaceWeights.size(); ++gp)
                    {
                        ...
                        for (unsigned int j = 0; j < NumNodes; ++j)
                        {
                            for (unsigned int dim = 0; dim < Dim; ++dim)
                            {
                                vconv[dim] += (rEnrInterfaceShapeFunctionNeg(gp, j) + rEnrInterfaceShapeFunctionPos(gp, j)) * v_convection(j, dim);
                            }
                        ...
                        const double penalty_coefficient = cut_stabilization_coefficient *
                                                           density * 1.0 / (dyn_tau * density / dt + stab_c1 * viscosity / h_elem / h_elem + stab_c2 * density * v_conv_norm / h_elem) * element_volume / cut_area;
