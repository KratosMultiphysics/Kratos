...
const double crhs6 = N[0]*(vn(0,0) - vnn(0,0));
const double crhs7 = N[1]*(vn(1,0) - vnn(1,0));
const double crhs8 = N[2]*(vn(2,0) - vnn(2,0));
const double crhs9 = crhs6 + crhs7 + crhs8;
const double crhs10 = 1.0/dt;
const double crhs11 = crhs10*crhs4;
const double crhs12 = N[0]*vn(0,0) + N[1]*vn(1,0) + N[2]*vn(2,0);
const double crhs13 = N[0]*vn(0,1) + N[1]*vn(1,1) + N[2]*vn(2,1);
const double crhs14 = rho*(crhs12*(DN(0,0)*vfrac(0,0) + DN(1,0)*vfrac(1,0) + DN(2,0)*vfrac(2,0)) + crhs13*(DN(0,1)*vfrac(0,0) + DN(1,1)*vfrac(1,0) + DN(2,1)*vfrac(2,0)));
...
const double crhs18 = rho*(crhs15*crhs16 + crhs17*(DN(0,1)*v(0,0) + DN(1,1)*v(1,0) + DN(2,1)*v(2,0)));
const double crhs19 = crhs12*(DN(0,0)*vn(0,0) + DN(1,0)*vn(1,0) + DN(2,0)*vn(2,0)) + crhs13*(DN(0,1)*vn(0,0) + DN(1,1)*vn(1,0) + DN(2,1)*vn(2,0));
const double crhs20 = DN(0,1)*v(0,1) + DN(1,1)*v(1,1) + DN(2,1)*v(2,1);
const double crhs21 = crhs15 + crhs20 - volume_error_ratio;
const double crhs22 = rho*stab_c2*sqrt(pow(crhs16, 2) + pow(crhs17, 2));
const double crhs23 = crhs21*(crhs22*h/stab_c1 + mu);
const double crhs24 = bdf0*rho;
const double crhs25 = crhs24*crhs3;
const double crhs26 = crhs10*rho;
const double crhs27 = 1.0/(K_darcy + crhs22/h + crhs26*dyn_tau + mu*stab_c1/pow(h, 2));
const double crhs28 = crhs27*(DN(0,0)*p[0] + DN(1,0)*p[1] + DN(2,0)*p[2] - crhs1 - crhs14 + crhs18 + crhs2 + crhs25 + rho*(crhs10*crhs6 + crhs10*crhs7 + crhs10*crhs8 + crhs19));
const double crhs29 = K_darcy*N[0];
...
const double crhs31 = crhs30*crhs4;
const double crhs32 = rho*(DN(0,0)*crhs16 + DN(0,1)*crhs17);
const double crhs33 = rho*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1));
const double crhs34 = K_darcy*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1));
const double crhs35 = N[0]*(v(0,1) - vfrac(0,1)) + N[1]*(v(1,1) - vfrac(1,1)) + N[2]*(v(2,1) - vfrac(2,1));
const double crhs36 = N[0]*(vn(0,1) - vnn(0,1));
const double crhs37 = N[1]*(vn(1,1) - vnn(1,1));
const double crhs38 = N[2]*(vn(2,1) - vnn(2,1));
const double crhs39 = crhs36 + crhs37 + crhs38;
const double crhs40 = rho*(crhs12*(DN(0,0)*vfrac(0,1) + DN(1,0)*vfrac(1,1) + DN(2,0)*vfrac(2,1)) + crhs13*(DN(0,1)*vfrac(0,1) + DN(1,1)*vfrac(1,1) + DN(2,1)*vfrac(2,1)));
const double crhs41 = rho*(crhs16*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1)) + crhs17*crhs20);
const double crhs42 = crhs12*(DN(0,0)*vn(0,1) + DN(1,0)*vn(1,1) + DN(2,0)*vn(2,1)) + crhs13*(DN(0,1)*vn(0,1) + DN(1,1)*vn(1,1) + DN(2,1)*vn(2,1));
const double crhs43 = crhs24*crhs35;
const double crhs44 = crhs27*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] - crhs33 + crhs34 - crhs40 + crhs41 + crhs43 + rho*(crhs10*crhs36 + crhs10*crhs37 + crhs10*crhs38 + crhs42));
const double crhs45 = N[1]*crhs26;
const double crhs46 = N[1]*rho;
const double crhs47 = K_darcy*N[1];
const double crhs48 = crhs30*crhs46;
const double crhs49 = rho*(DN(1,0)*crhs16 + DN(1,1)*crhs17);
const double crhs50 = N[2]*crhs26;
const double crhs51 = N[2]*rho;
const double crhs52 = K_darcy*N[2];
const double crhs53 = crhs30*crhs51;
const double crhs54 = rho*(DN(2,0)*crhs16 + DN(2,1)*crhs17);
rhs[0]=DN(0,0)*crhs0 - DN(0,0)*crhs23 - DN(0,0)*stress[0] - DN(0,1)*stress[2] + N[0]*crhs1 + N[0]*crhs14 - N[0]*crhs18 - N[0]*crhs2 - crhs11*crhs9 - crhs19*crhs4 + crhs28*crhs29 - crhs28*crhs31 - crhs28*crhs32 - crhs3*crhs5;
rhs[1]=-DN(0,0)*stress[2] + DN(0,1)*crhs0 - DN(0,1)*crhs23 - DN(0,1)*stress[1] + N[0]*crhs33 - N[0]*crhs34 + N[0]*crhs40 - N[0]*crhs41 - crhs11*crhs39 + crhs29*crhs44 - crhs31*crhs44 - crhs32*crhs44 - crhs35*crhs5 - crhs4*crhs42;
rhs[2]=-DN(0,0)*crhs28 - DN(0,1)*crhs44 - N[0]*crhs21;
rhs[3]=DN(1,0)*crhs0 - DN(1,0)*crhs23 - DN(1,0)*stress[0] - DN(1,1)*stress[2] + N[1]*crhs1 + N[1]*crhs14 - N[1]*crhs18 - N[1]*crhs2 - N[1]*crhs25 - crhs19*crhs46 + crhs28*crhs47 - crhs28*crhs48 - crhs28*crhs49 - crhs45*crhs9;
rhs[4]=-DN(1,0)*stress[2] + DN(1,1)*crhs0 - DN(1,1)*crhs23 - DN(1,1)*stress[1] + N[1]*crhs33 - N[1]*crhs34 + N[1]*crhs40 - N[1]*crhs41 - N[1]*crhs43 - crhs39*crhs45 - crhs42*crhs46 + crhs44*crhs47 - crhs44*crhs48 - crhs44*crhs49;
rhs[5]=-DN(1,0)*crhs28 - DN(1,1)*crhs44 - N[1]*crhs21;
rhs[6]=DN(2,0)*crhs0 - DN(2,0)*crhs23 - DN(2,0)*stress[0] - DN(2,1)*stress[2] + N[2]*crhs1 + N[2]*crhs14 - N[2]*crhs18 - N[2]*crhs2 - N[2]*crhs25 - crhs19*crhs51 + crhs28*crhs52 - crhs28*crhs53 - crhs28*crhs54 - crhs50*crhs9;
rhs[7]=-DN(2,0)*stress[2] + DN(2,1)*crhs0 - DN(2,1)*crhs23 - DN(2,1)*stress[1] + N[2]*crhs33 - N[2]*crhs34 + N[2]*crhs40 - N[2]*crhs41 - N[2]*crhs43 - crhs39*crhs50 - crhs42*crhs51 + crhs44*crhs52 - crhs44*crhs53 - crhs44*crhs54;
rhs[8]=-DN(2,0)*crhs28 - DN(2,1)*crhs44 - N[2]*crhs21;
...
const double crhs7 = N[1]*(vn(1,0) - vnn(1,0));
const double crhs8 = N[2]*(vn(2,0) - vnn(2,0));
const double crhs9 = N[3]*(vn(3,0) - vnn(3,0));
const double crhs10 = crhs6 + crhs7 + crhs8 + crhs9;
const double crhs11 = 1.0/dt;
const double crhs12 = crhs11*crhs4;
const double crhs13 = N[0]*vn(0,0) + N[1]*vn(1,0) + N[2]*vn(2,0) + N[3]*vn(3,0);
const double crhs14 = N[0]*vn(0,1) + N[1]*vn(1,1) + N[2]*vn(2,1) + N[3]*vn(3,1);
const double crhs15 = N[0]*vn(0,2) + N[1]*vn(1,2) + N[2]*vn(2,2) + N[3]*vn(3,2);
const double crhs16 = rho*(crhs13*(DN(0,0)*vfrac(0,0) + DN(1,0)*vfrac(1,0) + DN(2,0)*vfrac(2,0) + DN(3,0)*vfrac(3,0)) + crhs14*(DN(0,1)*vfrac(0,0) + DN(1,1)*vfrac(1,0) + DN(2,1)*vfrac(2,0) + DN(3,1)*vfrac(3,0)) + crhs15*(DN(0,2)*vfrac(0,0) + DN(1,2)*vfrac(1,0) + DN(2,2)*vfrac(2,0) + DN(3,2)*vfrac(3,0)));
const double crhs17 = DN(0,0)*v(0,0) + DN(1,0)*v(1,0) + DN(2,0)*v(2,0) + DN(3,0)*v(3,0);
...
const double crhs21 = rho*(crhs17*crhs18 + crhs19*(DN(0,1)*v(0,0) + DN(1,1)*v(1,0) + DN(2,1)*v(2,0) + DN(3,1)*v(3,0)) + crhs20*(DN(0,2)*v(0,0) + DN(1,2)*v(1,0) + DN(2,2)*v(2,0) + DN(3,2)*v(3,0)));
const double crhs22 = crhs13*(DN(0,0)*vn(0,0) + DN(1,0)*vn(1,0) + DN(2,0)*vn(2,0) + DN(3,0)*vn(3,0)) + crhs14*(DN(0,1)*vn(0,0) + DN(1,1)*vn(1,0) + DN(2,1)*vn(2,0) + DN(3,1)*vn(3,0)) + crhs15*(DN(0,2)*vn(0,0) + DN(1,2)*vn(1,0) + DN(2,2)*vn(2,0) + DN(3,2)*vn(3,0));
const double crhs23 = DN(0,1)*v(0,1) + DN(1,1)*v(1,1) + DN(2,1)*v(2,1) + DN(3,1)*v(3,1);
const double crhs24 = DN(0,2)*v(0,2) + DN(1,2)*v(1,2) + DN(2,2)*v(2,2) + DN(3,2)*v(3,2);
const double crhs25 = crhs17 + crhs23 + crhs24 - volume_error_ratio;
const double crhs26 = rho*stab_c2*sqrt(pow(crhs18, 2) + pow(crhs19, 2) + pow(crhs20, 2));
const double crhs27 = crhs25*(crhs26*h/stab_c1 + mu);
const double crhs28 = bdf0*rho;
const double crhs29 = crhs28*crhs3;
const double crhs30 = crhs11*rho;
const double crhs31 = 1.0/(K_darcy + crhs26/h + crhs30*dyn_tau + mu*stab_c1/pow(h, 2));
const double crhs32 = crhs31*(DN(0,0)*p[0] + DN(1,0)*p[1] + DN(2,0)*p[2] + DN(3,0)*p[3] - crhs1 - crhs16 + crhs2 + crhs21 + crhs29 + rho*(crhs11*crhs6 + crhs11*crhs7 + crhs11*crhs8 + crhs11*crhs9 + crhs22));
const double crhs33 = K_darcy*N[0];
const double crhs34 = DN(0,0)*vconv(0,0) + DN(0,1)*vconv(0,1) + DN(0,2)*vconv(0,2) + DN(1,0)*vconv(1,0) + DN(1,1)*vconv(1,1) + DN(1,2)*vconv(1,2) + DN(2,0)*vconv(2,0) + DN(2,1)*vconv(2,1) + DN(2,2)*vconv(2,2) + DN(3,0)*vconv(3,0) + DN(3,1)*vconv(3,1) + DN(3,2)*vconv(3,2);
const double crhs35 = crhs34*crhs4;
const double crhs36 = rho*(DN(0,0)*crhs18 + DN(0,1)*crhs19 + DN(0,2)*crhs20);
const double crhs37 = rho*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1) + N[3]*f(3,1));
const double crhs38 = K_darcy*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1) + N[3]*v(3,1));
...
const double crhs45 = rho*(crhs13*(DN(0,0)*vfrac(0,1) + DN(1,0)*vfrac(1,1) + DN(2,0)*vfrac(2,1) + DN(3,0)*vfrac(3,1)) + crhs14*(DN(0,1)*vfrac(0,1) + DN(1,1)*vfrac(1,1) + DN(2,1)*vfrac(2,1) + DN(3,1)*vfrac(3,1)) + crhs15*(DN(0,2)*vfrac(0,1) + DN(1,2)*vfrac(1,1) + DN(2,2)*vfrac(2,1) + DN(3,2)*vfrac(3,1)));
const double crhs46 = rho*(crhs18*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1) + DN(3,0)*v(3,1)) + crhs19*crhs23 + crhs20*(DN(0,2)*v(0,1) + DN(1,2)*v(1,1) + DN(2,2)*v(2,1) + DN(3,2)*v(3,1)));
const double crhs47 = crhs13*(DN(0,0)*vn(0,1) + DN(1,0)*vn(1,1) + DN(2,0)*vn(2,1) + DN(3,0)*vn(3,1)) + crhs14*(DN(0,1)*vn(0,1) + DN(1,1)*vn(1,1) + DN(2,1)*vn(2,1) + DN(3,1)*vn(3,1)) + crhs15*(DN(0,2)*vn(0,1) + DN(1,2)*vn(1,1) + DN(2,2)*vn(2,1) + DN(3,2)*vn(3,1));
const double crhs48 = crhs28*crhs39;
const double crhs49 = crhs31*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] + DN(3,1)*p[3] - crhs37 + crhs38 - crhs45 + crhs46 + crhs48 + rho*(crhs11*crhs40 + crhs11*crhs41 + crhs11*crhs42 + crhs11*crhs43 + crhs47));
const double crhs50 = rho*(N[0]*f(0,2) + N[1]*f(1,2) + N[2]*f(2,2) + N[3]*f(3,2));
const double crhs51 = K_darcy*(N[0]*v(0,2) + N[1]*v(1,2) + N[2]*v(2,2) + N[3]*v(3,2));
const double crhs52 = N[0]*(v(0,2) - vfrac(0,2)) + N[1]*(v(1,2) - vfrac(1,2)) + N[2]*(v(2,2) - vfrac(2,2)) + N[3]*(v(3,2) - vfrac(3,2));
const double crhs53 = N[0]*(vn(0,2) - vnn(0,2));
const double crhs54 = N[1]*(vn(1,2) - vnn(1,2));
const double crhs55 = N[2]*(vn(2,2) - vnn(2,2));
const double crhs56 = N[3]*(vn(3,2) - vnn(3,2));
const double crhs57 = crhs53 + crhs54 + crhs55 + crhs56;
const double crhs58 = rho*(crhs13*(DN(0,0)*vfrac(0,2) + DN(1,0)*vfrac(1,2) + DN(2,0)*vfrac(2,2) + DN(3,0)*vfrac(3,2)) + crhs14*(DN(0,1)*vfrac(0,2) + DN(1,1)*vfrac(1,2) + DN(2,1)*vfrac(2,2) + DN(3,1)*vfrac(3,2)) + crhs15*(DN(0,2)*vfrac(0,2) + DN(1,2)*vfrac(1,2) + DN(2,2)*vfrac(2,2) + DN(3,2)*vfrac(3,2)));
const double crhs59 = rho*(crhs18*(DN(0,0)*v(0,2) + DN(1,0)*v(1,2) + DN(2,0)*v(2,2) + DN(3,0)*v(3,2)) + crhs19*(DN(0,1)*v(0,2) + DN(1,1)*v(1,2) + DN(2,1)*v(2,2) + DN(3,1)*v(3,2)) + crhs20*crhs24);
const double crhs60 = crhs13*(DN(0,0)*vn(0,2) + DN(1,0)*vn(1,2) + DN(2,0)*vn(2,2) + DN(3,0)*vn(3,2)) + crhs14*(DN(0,1)*vn(0,2) + DN(1,1)*vn(1,2) + DN(2,1)*vn(2,2) + DN(3,1)*vn(3,2)) + crhs15*(DN(0,2)*vn(0,2) + DN(1,2)*vn(1,2) + DN(2,2)*vn(2,2) + DN(3,2)*vn(3,2));
const double crhs61 = crhs28*crhs52;
const double crhs62 = crhs31*(DN(0,2)*p[0] + DN(1,2)*p[1] + DN(2,2)*p[2] + DN(3,2)*p[3] - crhs50 + crhs51 - crhs58 + crhs59 + crhs61 + rho*(crhs11*crhs53 + crhs11*crhs54 + crhs11*crhs55 + crhs11*crhs56 + crhs60));
const double crhs63 = N[1]*crhs30;
const double crhs64 = N[1]*rho;
const double crhs65 = K_darcy*N[1];
const double crhs66 = crhs34*crhs64;
const double crhs67 = rho*(DN(1,0)*crhs18 + DN(1,1)*crhs19 + DN(1,2)*crhs20);
const double crhs68 = N[2]*crhs30;
const double crhs69 = N[2]*rho;
const double crhs70 = K_darcy*N[2];
const double crhs71 = crhs34*crhs69;
const double crhs72 = rho*(DN(2,0)*crhs18 + DN(2,1)*crhs19 + DN(2,2)*crhs20);
const double crhs73 = N[3]*crhs30;
const double crhs74 = N[3]*rho;
const double crhs75 = K_darcy*N[3];
const double crhs76 = crhs34*crhs74;
const double crhs77 = rho*(DN(3,0)*crhs18 + DN(3,1)*crhs19 + DN(3,2)*crhs20);
rhs[0]=DN(0,0)*crhs0 - DN(0,0)*crhs27 - DN(0,0)*stress[0] - DN(0,1)*stress[3] - DN(0,2)*stress[5] + N[0]*crhs1 + N[0]*crhs16 - N[0]*crhs2 - N[0]*crhs21 - crhs10*crhs12 - crhs22*crhs4 - crhs3*crhs5 + crhs32*crhs33 - crhs32*crhs35 - crhs32*crhs36;
rhs[1]=-DN(0,0)*stress[3] + DN(0,1)*crhs0 - DN(0,1)*crhs27 - DN(0,1)*stress[1] - DN(0,2)*stress[4] + N[0]*crhs37 - N[0]*crhs38 + N[0]*crhs45 - N[0]*crhs46 - crhs12*crhs44 + crhs33*crhs49 - crhs35*crhs49 - crhs36*crhs49 - crhs39*crhs5 - crhs4*crhs47;
rhs[2]=-DN(0,0)*stress[5] - DN(0,1)*stress[4] + DN(0,2)*crhs0 - DN(0,2)*crhs27 - DN(0,2)*stress[2] + N[0]*crhs50 - N[0]*crhs51 + N[0]*crhs58 - N[0]*crhs59 - crhs12*crhs57 + crhs33*crhs62 - crhs35*crhs62 - crhs36*crhs62 - crhs4*crhs60 - crhs5*crhs52;
rhs[3]=-DN(0,0)*crhs32 - DN(0,1)*crhs49 - DN(0,2)*crhs62 - N[0]*crhs25;
rhs[4]=DN(1,0)*crhs0 - DN(1,0)*crhs27 - DN(1,0)*stress[0] - DN(1,1)*stress[3] - DN(1,2)*stress[5] + N[1]*crhs1 + N[1]*crhs16 - N[1]*crhs2 - N[1]*crhs21 - N[1]*crhs29 - crhs10*crhs63 - crhs22*crhs64 + crhs32*crhs65 - crhs32*crhs66 - crhs32*crhs67;
rhs[5]=-DN(1,0)*stress[3] + DN(1,1)*crhs0 - DN(1,1)*crhs27 - DN(1,1)*stress[1] - DN(1,2)*stress[4] + N[1]*crhs37 - N[1]*crhs38 + N[1]*crhs45 - N[1]*crhs46 - N[1]*crhs48 - crhs44*crhs63 - crhs47*crhs64 + crhs49*crhs65 - crhs49*crhs66 - crhs49*crhs67;
rhs[6]=-DN(1,0)*stress[5] - DN(1,1)*stress[4] + DN(1,2)*crhs0 - DN(1,2)*crhs27 - DN(1,2)*stress[2] + N[1]*crhs50 - N[1]*crhs51 + N[1]*crhs58 - N[1]*crhs59 - N[1]*crhs61 - crhs57*crhs63 - crhs60*crhs64 + crhs62*crhs65 - crhs62*crhs66 - crhs62*crhs67;
rhs[7]=-DN(1,0)*crhs32 - DN(1,1)*crhs49 - DN(1,2)*crhs62 - N[1]*crhs25;
rhs[8]=DN(2,0)*crhs0 - DN(2,0)*crhs27 - DN(2,0)*stress[0] - DN(2,1)*stress[3] - DN(2,2)*stress[5] + N[2]*crhs1 + N[2]*crhs16 - N[2]*crhs2 - N[2]*crhs21 - N[2]*crhs29 - crhs10*crhs68 - crhs22*crhs69 + crhs32*crhs70 - crhs32*crhs71 - crhs32*crhs72;
rhs[9]=-DN(2,0)*stress[3] + DN(2,1)*crhs0 - DN(2,1)*crhs27 - DN(2,1)*stress[1] - DN(2,2)*stress[4] + N[2]*crhs37 - N[2]*crhs38 + N[2]*crhs45 - N[2]*crhs46 - N[2]*crhs48 - crhs44*crhs68 - crhs47*crhs69 + crhs49*crhs70 - crhs49*crhs71 - crhs49*crhs72;
rhs[10]=-DN(2,0)*stress[5] - DN(2,1)*stress[4] + DN(2,2)*crhs0 - DN(2,2)*crhs27 - DN(2,2)*stress[2] + N[2]*crhs50 - N[2]*crhs51 + N[2]*crhs58 - N[2]*crhs59 - N[2]*crhs61 - crhs57*crhs68 - crhs60*crhs69 + crhs62*crhs70 - crhs62*crhs71 - crhs62*crhs72;
rhs[11]=-DN(2,0)*crhs32 - DN(2,1)*crhs49 - DN(2,2)*crhs62 - N[2]*crhs25;
rhs[12]=DN(3,0)*crhs0 - DN(3,0)*crhs27 - DN(3,0)*stress[0] - DN(3,1)*stress[3] - DN(3,2)*stress[5] + N[3]*crhs1 + N[3]*crhs16 - N[3]*crhs2 - N[3]*crhs21 - N[3]*crhs29 - crhs10*crhs73 - crhs22*crhs74 + crhs32*crhs75 - crhs32*crhs76 - crhs32*crhs77;
rhs[13]=-DN(3,0)*stress[3] + DN(3,1)*crhs0 - DN(3,1)*crhs27 - DN(3,1)*stress[1] - DN(3,2)*stress[4] + N[3]*crhs37 - N[3]*crhs38 + N[3]*crhs45 - N[3]*crhs46 - N[3]*crhs48 - crhs44*crhs73 - crhs47*crhs74 + crhs49*crhs75 - crhs49*crhs76 - crhs49*crhs77;
rhs[14]=-DN(3,0)*stress[5] - DN(3,1)*stress[4] + DN(3,2)*crhs0 - DN(3,2)*crhs27 - DN(3,2)*stress[2] + N[3]*crhs50 - N[3]*crhs51 + N[3]*crhs58 - N[3]*crhs59 - N[3]*crhs61 - crhs57*crhs73 - crhs60*crhs74 + crhs62*crhs75 - crhs62*crhs76 - crhs62*crhs77;
rhs[15]=-DN(3,0)*crhs32 - DN(3,1)*crhs49 - DN(3,2)*crhs62 - N[3]*crhs25;
...
const double crhs_ee9 = crhs_ee8*(DN(0,0)*p[0] + DN(1,0)*p[1] + DN(2,0)*p[2] + DNenr(0,0)*penr[0] + DNenr(1,0)*penr[1] + DNenr(2,0)*penr[2] + K_darcy*(N[0]*v(0,0) + N[1]*v(1,0) + N[2]*v(2,0)) - rho*(N[0]*f(0,0) + N[1]*f(1,0) + N[2]*f(2,0)) + rho*(crhs_ee0*crhs_ee6 + crhs_ee3*(v(0,0) - vfrac(0,0)) + crhs_ee4*(v(1,0) - vfrac(1,0)) + crhs_ee5*(v(2,0) - vfrac(2,0)) + crhs_ee7*(DN(0,1)*v(0,0) + DN(1,1)*v(1,0) + DN(2,1)*v(2,0))));
const double crhs_ee10 = crhs_ee8*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] + DNenr(0,1)*penr[0] + DNenr(1,1)*penr[1] + DNenr(2,1)*penr[2] + K_darcy*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1)) - rho*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1)) + rho*(crhs_ee1*crhs_ee7 + crhs_ee3*(v(0,1) - vfrac(0,1)) + crhs_ee4*(v(1,1) - vfrac(1,1)) + crhs_ee5*(v(2,1) - vfrac(2,1)) + crhs_ee6*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1))));
...
const double crhs_ee12 = crhs_ee11*(DN(0,0)*p[0] + DN(1,0)*p[1] + DN(2,0)*p[2] + DN(3,0)*p[3] + DNenr(0,0)*penr[0] + DNenr(1,0)*penr[1] + DNenr(2,0)*penr[2] + DNenr(3,0)*penr[3] + K_darcy*(N[0]*v(0,0) + N[1]*v(1,0) + N[2]*v(2,0) + N[3]*v(3,0)) - rho*(N[0]*f(0,0) + N[1]*f(1,0) + N[2]*f(2,0) + N[3]*f(3,0)) + rho*(crhs_ee0*crhs_ee8 + crhs_ee10*(DN(0,2)*v(0,0) + DN(1,2)*v(1,0) + DN(2,2)*v(2,0) + DN(3,2)*v(3,0)) + crhs_ee4*(v(0,0) - vfrac(0,0)) + crhs_ee5*(v(1,0) - vfrac(1,0)) + crhs_ee6*(v(2,0) - vfrac(2,0)) + crhs_ee7*(v(3,0) - vfrac(3,0)) + crhs_ee9*(DN(0,1)*v(0,0) + DN(1,1)*v(1,0) + DN(2,1)*v(2,0) + DN(3,1)*v(3,0))));
const double crhs_ee13 = crhs_ee11*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] + DN(3,1)*p[3] + DNenr(0,1)*penr[0] + DNenr(1,1)*penr[1] + DNenr(2,1)*penr[2] + DNenr(3,1)*penr[3] + K_darcy*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1) + N[3]*v(3,1)) - rho*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1) + N[3]*f(3,1)) + rho*(crhs_ee1*crhs_ee9 + crhs_ee10*(DN(0,2)*v(0,1) + DN(1,2)*v(1,1) + DN(2,2)*v(2,1) + DN(3,2)*v(3,1)) + crhs_ee4*(v(0,1) - vfrac(0,1)) + crhs_ee5*(v(1,1) - vfrac(1,1)) + crhs_ee6*(v(2,1) - vfrac(2,1)) + crhs_ee7*(v(3,1) - vfrac(3,1)) + crhs_ee8*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1) + DN(3,0)*v(3,1))));
const double crhs_ee14 = crhs_ee11*(DN(0,2)*p[0] + DN(1,2)*p[1] + DN(2,2)*p[2] + DN(3,2)*p[3] + DNenr(0,2)*penr[0] + DNenr(1,2)*penr[1] + DNenr(2,2)*penr[2] + DNenr(3,2)*penr[3] + K_darcy*(N[0]*v(0,2) + N[1]*v(1,2) + N[2]*v(2,2) + N[3]*v(3,2)) - rho*(N[0]*f(0,2) + N[1]*f(1,2) + N[2]*f(2,2) + N[3]*f(3,2)) + rho*(crhs_ee10*crhs_ee2 + crhs_ee4*(v(0,2) - vfrac(0,2)) + crhs_ee5*(v(1,2) - vfrac(1,2)) + crhs_ee6*(v(2,2) - vfrac(2,2)) + crhs_ee7*(v(3,2) - vfrac(3,2)) + crhs_ee8*(DN(0,0)*v(0,2) + DN(1,0)*v(1,2) + DN(2,0)*v(2,2) + DN(3,0)*v(3,2)) + crhs_ee9*(DN(0,1)*v(0,2) + DN(1,1)*v(1,2) + DN(2,1)*v(2,2) + DN(3,1)*v(3,2))));
