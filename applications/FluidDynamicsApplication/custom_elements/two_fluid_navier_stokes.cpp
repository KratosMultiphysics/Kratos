...
const double crhs3 = bdf0*rho;
const double crhs4 = crhs3*(N[0]*(v(0,0) - vn(0,0)) + N[1]*(v(1,0) - vn(1,0)) + N[2]*(v(2,0) - vn(2,0)));
...
const double crhs10 = crhs5 + crhs9 - volume_error_ratio;
...
const double crhs14 = crhs13*(DN(0,0)*p[0] + DN(1,0)*p[1] + DN(2,0)*p[2] - crhs1 + crhs2 + crhs4 + crhs8);
const double crhs15 = K_darcy*N[0];
const double crhs16 = rho*(DN(0,0)*vconv(0,0) + DN(0,1)*vconv(0,1) + DN(1,0)*vconv(1,0) + DN(1,1)*vconv(1,1) + DN(2,0)*vconv(2,0) + DN(2,1)*vconv(2,1));
const double crhs17 = N[0]*crhs16;
const double crhs18 = rho*(DN(0,0)*crhs6 + DN(0,1)*crhs7);
const double crhs19 = rho*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1));
const double crhs20 = K_darcy*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1));
const double crhs21 = crhs3*(N[0]*(v(0,1) - vn(0,1)) + N[1]*(v(1,1) - vn(1,1)) + N[2]*(v(2,1) - vn(2,1)));
const double crhs22 = rho*(crhs6*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1)) + crhs7*crhs9);
const double crhs23 = crhs13*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] - crhs19 + crhs20 + crhs21 + crhs22);
const double crhs24 = K_darcy*N[1];
const double crhs25 = N[1]*crhs16;
const double crhs26 = rho*(DN(1,0)*crhs6 + DN(1,1)*crhs7);
const double crhs27 = K_darcy*N[2];
const double crhs28 = N[2]*crhs16;
const double crhs29 = rho*(DN(2,0)*crhs6 + DN(2,1)*crhs7);
rhs[0]=DN(0,0)*crhs0 - DN(0,0)*crhs12 - DN(0,0)*stress[0] - DN(0,1)*stress[2] + N[0]*crhs1 - N[0]*crhs2 - N[0]*crhs4 - N[0]*crhs8 + crhs14*crhs15 - crhs14*crhs17 - crhs14*crhs18;
rhs[1]=-DN(0,0)*stress[2] + DN(0,1)*crhs0 - DN(0,1)*crhs12 - DN(0,1)*stress[1] + N[0]*crhs19 - N[0]*crhs20 - N[0]*crhs21 - N[0]*crhs22 + crhs15*crhs23 - crhs17*crhs23 - crhs18*crhs23;
rhs[2]=-DN(0,0)*crhs14 - DN(0,1)*crhs23 - N[0]*crhs10;
rhs[3]=DN(1,0)*crhs0 - DN(1,0)*crhs12 - DN(1,0)*stress[0] - DN(1,1)*stress[2] + N[1]*crhs1 - N[1]*crhs2 - N[1]*crhs4 - N[1]*crhs8 + crhs14*crhs24 - crhs14*crhs25 - crhs14*crhs26;
rhs[4]=-DN(1,0)*stress[2] + DN(1,1)*crhs0 - DN(1,1)*crhs12 - DN(1,1)*stress[1] + N[1]*crhs19 - N[1]*crhs20 - N[1]*crhs21 - N[1]*crhs22 + crhs23*crhs24 - crhs23*crhs25 - crhs23*crhs26;
rhs[5]=-DN(1,0)*crhs14 - DN(1,1)*crhs23 - N[1]*crhs10;
rhs[6]=DN(2,0)*crhs0 - DN(2,0)*crhs12 - DN(2,0)*stress[0] - DN(2,1)*stress[2] + N[2]*crhs1 - N[2]*crhs2 - N[2]*crhs4 - N[2]*crhs8 + crhs14*crhs27 - crhs14*crhs28 - crhs14*crhs29;
rhs[7]=-DN(2,0)*stress[2] + DN(2,1)*crhs0 - DN(2,1)*crhs12 - DN(2,1)*stress[1] + N[2]*crhs19 - N[2]*crhs20 - N[2]*crhs21 - N[2]*crhs22 + crhs23*crhs27 - crhs23*crhs28 - crhs23*crhs29;
rhs[8]=-DN(2,0)*crhs14 - DN(2,1)*crhs23 - N[2]*crhs10;
...
const double crhs3 = bdf0*rho;
const double crhs4 = crhs3*(N[0]*(v(0,0) - vn(0,0)) + N[1]*(v(1,0) - vn(1,0)) + N[2]*(v(2,0) - vn(2,0)) + N[3]*(v(3,0) - vn(3,0)));
...
const double crhs12 = crhs10 + crhs11 + crhs5 - volume_error_ratio;
...
const double crhs16 = crhs15*(DN(0,0)*p[0] + DN(1,0)*p[1] + DN(2,0)*p[2] + DN(3,0)*p[3] - crhs1 + crhs2 + crhs4 + crhs9);
const double crhs17 = K_darcy*N[0];
const double crhs18 = rho*(DN(0,0)*vconv(0,0) + DN(0,1)*vconv(0,1) + DN(0,2)*vconv(0,2) + DN(1,0)*vconv(1,0) + DN(1,1)*vconv(1,1) + DN(1,2)*vconv(1,2) + DN(2,0)*vconv(2,0) + DN(2,1)*vconv(2,1) + DN(2,2)*vconv(2,2) + DN(3,0)*vconv(3,0) + DN(3,1)*vconv(3,1) + DN(3,2)*vconv(3,2));
const double crhs19 = N[0]*crhs18;
const double crhs20 = rho*(DN(0,0)*crhs6 + DN(0,1)*crhs7 + DN(0,2)*crhs8);
const double crhs21 = rho*(N[0]*f(0,1) + N[1]*f(1,1) + N[2]*f(2,1) + N[3]*f(3,1));
const double crhs22 = K_darcy*(N[0]*v(0,1) + N[1]*v(1,1) + N[2]*v(2,1) + N[3]*v(3,1));
const double crhs23 = crhs3*(N[0]*(v(0,1) - vn(0,1)) + N[1]*(v(1,1) - vn(1,1)) + N[2]*(v(2,1) - vn(2,1)) + N[3]*(v(3,1) - vn(3,1)));
const double crhs24 = rho*(crhs10*crhs7 + crhs6*(DN(0,0)*v(0,1) + DN(1,0)*v(1,1) + DN(2,0)*v(2,1) + DN(3,0)*v(3,1)) + crhs8*(DN(0,2)*v(0,1) + DN(1,2)*v(1,1) + DN(2,2)*v(2,1) + DN(3,2)*v(3,1)));
const double crhs25 = crhs15*(DN(0,1)*p[0] + DN(1,1)*p[1] + DN(2,1)*p[2] + DN(3,1)*p[3] - crhs21 + crhs22 + crhs23 + crhs24);
const double crhs26 = rho*(N[0]*f(0,2) + N[1]*f(1,2) + N[2]*f(2,2) + N[3]*f(3,2));
const double crhs27 = K_darcy*(N[0]*v(0,2) + N[1]*v(1,2) + N[2]*v(2,2) + N[3]*v(3,2));
const double crhs28 = crhs3*(N[0]*(v(0,2) - vn(0,2)) + N[1]*(v(1,2) - vn(1,2)) + N[2]*(v(2,2) - vn(2,2)) + N[3]*(v(3,2) - vn(3,2)));
const double crhs29 = rho*(crhs11*crhs8 + crhs6*(DN(0,0)*v(0,2) + DN(1,0)*v(1,2) + DN(2,0)*v(2,2) + DN(3,0)*v(3,2)) + crhs7*(DN(0,1)*v(0,2) + DN(1,1)*v(1,2) + DN(2,1)*v(2,2) + DN(3,1)*v(3,2)));
const double crhs30 = crhs15*(DN(0,2)*p[0] + DN(1,2)*p[1] + DN(2,2)*p[2] + DN(3,2)*p[3] - crhs26 + crhs27 + crhs28 + crhs29);
const double crhs31 = K_darcy*N[1];
const double crhs32 = N[1]*crhs18;
const double crhs33 = rho*(DN(1,0)*crhs6 + DN(1,1)*crhs7 + DN(1,2)*crhs8);
const double crhs34 = K_darcy*N[2];
const double crhs35 = N[2]*crhs18;
const double crhs36 = rho*(DN(2,0)*crhs6 + DN(2,1)*crhs7 + DN(2,2)*crhs8);
const double crhs37 = K_darcy*N[3];
const double crhs38 = N[3]*crhs18;
const double crhs39 = rho*(DN(3,0)*crhs6 + DN(3,1)*crhs7 + DN(3,2)*crhs8);
rhs[0]=DN(0,0)*crhs0 - DN(0,0)*crhs14 - DN(0,0)*stress[0] - DN(0,1)*stress[3] - DN(0,2)*stress[5] + N[0]*crhs1 - N[0]*crhs2 - N[0]*crhs4 - N[0]*crhs9 + crhs16*crhs17 - crhs16*crhs19 - crhs16*crhs20;
rhs[1]=-DN(0,0)*stress[3] + DN(0,1)*crhs0 - DN(0,1)*crhs14 - DN(0,1)*stress[1] - DN(0,2)*stress[4] + N[0]*crhs21 - N[0]*crhs22 - N[0]*crhs23 - N[0]*crhs24 + crhs17*crhs25 - crhs19*crhs25 - crhs20*crhs25;
rhs[2]=-DN(0,0)*stress[5] - DN(0,1)*stress[4] + DN(0,2)*crhs0 - DN(0,2)*crhs14 - DN(0,2)*stress[2] + N[0]*crhs26 - N[0]*crhs27 - N[0]*crhs28 - N[0]*crhs29 + crhs17*crhs30 - crhs19*crhs30 - crhs20*crhs30;
rhs[3]=-DN(0,0)*crhs16 - DN(0,1)*crhs25 - DN(0,2)*crhs30 - N[0]*crhs12;
rhs[4]=DN(1,0)*crhs0 - DN(1,0)*crhs14 - DN(1,0)*stress[0] - DN(1,1)*stress[3] - DN(1,2)*stress[5] + N[1]*crhs1 - N[1]*crhs2 - N[1]*crhs4 - N[1]*crhs9 + crhs16*crhs31 - crhs16*crhs32 - crhs16*crhs33;
rhs[5]=-DN(1,0)*stress[3] + DN(1,1)*crhs0 - DN(1,1)*crhs14 - DN(1,1)*stress[1] - DN(1,2)*stress[4] + N[1]*crhs21 - N[1]*crhs22 - N[1]*crhs23 - N[1]*crhs24 + crhs25*crhs31 - crhs25*crhs32 - crhs25*crhs33;
rhs[6]=-DN(1,0)*stress[5] - DN(1,1)*stress[4] + DN(1,2)*crhs0 - DN(1,2)*crhs14 - DN(1,2)*stress[2] + N[1]*crhs26 - N[1]*crhs27 - N[1]*crhs28 - N[1]*crhs29 + crhs30*crhs31 - crhs30*crhs32 - crhs30*crhs33;
rhs[7]=-DN(1,0)*crhs16 - DN(1,1)*crhs25 - DN(1,2)*crhs30 - N[1]*crhs12;
rhs[8]=DN(2,0)*crhs0 - DN(2,0)*crhs14 - DN(2,0)*stress[0] - DN(2,1)*stress[3] - DN(2,2)*stress[5] + N[2]*crhs1 - N[2]*crhs2 - N[2]*crhs4 - N[2]*crhs9 + crhs16*crhs34 - crhs16*crhs35 - crhs16*crhs36;
rhs[9]=-DN(2,0)*stress[3] + DN(2,1)*crhs0 - DN(2,1)*crhs14 - DN(2,1)*stress[1] - DN(2,2)*stress[4] + N[2]*crhs21 - N[2]*crhs22 - N[2]*crhs23 - N[2]*crhs24 + crhs25*crhs34 - crhs25*crhs35 - crhs25*crhs36;
rhs[10]=-DN(2,0)*stress[5] - DN(2,1)*stress[4] + DN(2,2)*crhs0 - DN(2,2)*crhs14 - DN(2,2)*stress[2] + N[2]*crhs26 - N[2]*crhs27 - N[2]*crhs28 - N[2]*crhs29 + crhs30*crhs34 - crhs30*crhs35 - crhs30*crhs36;
rhs[11]=-DN(2,0)*crhs16 - DN(2,1)*crhs25 - DN(2,2)*crhs30 - N[2]*crhs12;
rhs[12]=DN(3,0)*crhs0 - DN(3,0)*crhs14 - DN(3,0)*stress[0] - DN(3,1)*stress[3] - DN(3,2)*stress[5] + N[3]*crhs1 - N[3]*crhs2 - N[3]*crhs4 - N[3]*crhs9 + crhs16*crhs37 - crhs16*crhs38 - crhs16*crhs39;
rhs[13]=-DN(3,0)*stress[3] + DN(3,1)*crhs0 - DN(3,1)*crhs14 - DN(3,1)*stress[1] - DN(3,2)*stress[4] + N[3]*crhs21 - N[3]*crhs22 - N[3]*crhs23 - N[3]*crhs24 + crhs25*crhs37 - crhs25*crhs38 - crhs25*crhs39;
rhs[14]=-DN(3,0)*stress[5] - DN(3,1)*stress[4] + DN(3,2)*crhs0 - DN(3,2)*crhs14 - DN(3,2)*stress[2] + N[3]*crhs26 - N[3]*crhs27 - N[3]*crhs28 - N[3]*crhs29 + crhs30*crhs37 - crhs30*crhs38 - crhs30*crhs39;
rhs[15]=-DN(3,0)*crhs16 - DN(3,1)*crhs25 - DN(3,2)*crhs30 - N[3]*crhs12;
