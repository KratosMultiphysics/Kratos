...
//            double k_sc = ShockCapturingConductivity(data);
                                ...
                                ProcessInfo& rCurrentProcessInfo) 
    {
        ...
        // Gauss point position
        // BoundedMatrix<double,TNumNodes, TNumNodes> Ncontainer;
        // int IntegrationType = 0;        // 0 for exact integration, 1 for nodal integration
        // GetShapeFunctionsOnGauss(Ncontainer, IntegrationType);
        ...
        {   
            ...
            #pragma omp atomic
            momentum[0] += rRightHandSideVector[BlockSize*i + 2];

            #pragma omp atomic
            momentum[1] += rRightHandSideVector[BlockSize*i + 3];

            if (TDim == 3){
                #pragma omp atomic
                momentum[2] += rRightHandSideVector[BlockSize*i + 4];
            }

            #pragma omp atomic
            r_geom[i].FastGetSolutionStepValue(TOTAL_ENERGY_RHS) += rRightHandSideVector[BlockSize*i + TDim + 2];
...
//        rData.ros = r_properties.GetValue(DENSITY_MATERIAL);
        ...
        for (unsigned int i = 0; i < TNumNodes; i++)
        {
          ...
          //  if (mass <1e-15)    printf("mass = %.3e\n", mass);
            ...
            {
                rData.U(i,k+2)   = moment[k];
                rData.Un(i,k+2)  = moment_n[k];
            ...
            rData.Up(i,1)= (this->GetGeometry()[i].FastGetSolutionStepValue(DENSITY_SOLID_RHS))/mass;

            rData.U(i,TDim+2) = this->GetGeometry()[i].FastGetSolutionStepValue(TOTAL_ENERGY);
            rData.Un(i,TDim+2) = this->GetGeometry()[i].FastGetSolutionStepValue(TOTAL_ENERGY,1);
            rData.Up(i,TDim+2) = (this->GetGeometry()[i].FastGetSolutionStepValue(TOTAL_ENERGY_RHS))/mass;
