...
#include "../applications/SwimmingDEMApplication/swimming_dem_application_variables.h"
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int g = 0; g < number_of_integration_points; g++ ) {
        ...
        if (rVariable == BODY_FORCE){
            ...
            //     mExactBodyForce[g][d] += rValues[g][d]/(gauss_weights[g]*mExactPorosity[g]*density);
...
void DVMSDEMCoupled<TElementData>::Calculate(
    ...
    if (rVariable == ADVPROJ) {
        ...
        if (rVariable == BODY_FORCE) {
            for (unsigned int g = 0; g < number_of_integration_points; g++)
                noalias(mExactBodyForce[g]) = rOutput;
        }
    }
}
...
void DVMSDEMCoupled<TElementData>::FinalizeSolutionStep(const ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int g = 0; g < number_of_integration_points; g++) {
        ...
        mExactVector[g] = this->GetAtCoordinate(data.Velocity, data.N);
    }
}
    ...
    VectorType &rLocalRHS)
{
    ...
    const array_1d<double,3> fluid_fraction_gradient = mExactPorosityGradient[rData.IntegrationPointIndex];
    ...
    for (unsigned int i = 0; i < NumNodes; i++)
    {
        ...
        for (unsigned int j = 0; j < NumNodes; j++)
        {
            ...
            for (unsigned int d = 0; d < Dim; d++)
            {
                ...
                LHS(row+d,col+Dim) += rData.Weight * (AG - VP - P + LG - CG + DP + EGP);
            }
        ...
        double QPhi = tau_two * epsilon * rData.N[i] * (mass_source - fluid_fraction_rate);
...
void DVMSDEMCoupled<TElementData>::CalculateResistanceTensor(
    const TElementData& rData)
{
    BoundedMatrix<double,3,3>& rsigma = mViscousResistanceTensor[rData.IntegrationPointIndex];
    ...
    array_1d<double, 3> nodal_reaction_term = ZeroVector(3);
    for (unsigned int i = 0; i < NumNodes; i++){
        this->GetGeometry()[i].SetLock();
        array_1d<double,3> nodal_reaction = rGeom[i].FastGetSolutionStepValue(HYDRODYNAMIC_REACTION);
        for (size_t j = 0; j < Dim; j++) {
            nodal_reaction_term[j] += rData.N[i] * nodal_reaction[j];
        }
        this->GetGeometry()[i].UnSetLock();
    }

    for (unsigned int d = 0; d < Dim; ++d){
        //mExactBodyForce[rData.IntegrationPointIndex][d] += nodal_reaction_term[d] /** this->GetAtCoordinate(rData.Velocity,rData.N)[d] */* mExactVector[rData.IntegrationPointIndex][d];
        rsigma(d,d) = fluid_density * nodal_reaction_term[d];
        }
}
    ...
    MatrixType &rMassMatrix)
{
    ...
    double fluid_fraction = mExactPorosity[rData.IntegrationPointIndex];
    double viscosity = this->GetAtCoordinate(rData.DynamicViscosity, rData.N);
