...
#ifndef KRATOS_TIME_AVERAGED_NAVIER_STOKES_WALL_CONDITION_H
#define KRATOS_TIME_AVERAGED_NAVIER_STOKES_WALL_CONDITION_H
...
class KRATOS_API(FLUID_DYNAMICS_APPLICATION) TimeAveragedNavierStokesWallCondition : public Condition
{
public:
    ///@name Type Definitions
    ///@{

    /// Pointer definition of TimeAveragedNavierStokesWallCondition
    KRATOS_CLASS_POINTER_DEFINITION(TimeAveragedNavierStokesWallCondition);
    ...
    /** Admits an Id as a parameter.
      ...
      */
    TimeAveragedNavierStokesWallCondition(IndexType NewId = 0):Condition(NewId)
    {
    }
     ...
     */
    TimeAveragedNavierStokesWallCondition(IndexType NewId, const NodesArrayType& ThisNodes):
     ...
     */
    TimeAveragedNavierStokesWallCondition(IndexType NewId, GeometryType::Pointer pGeometry):
     ...
     */
    TimeAveragedNavierStokesWallCondition(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties):
        Condition(NewId,pGeometry,pProperties)
    {
    }

    /// Copy constructor.
    TimeAveragedNavierStokesWallCondition(TimeAveragedNavierStokesWallCondition const& rOther):
        Condition(rOther)
    {
    }

    /// Destructor.
    ~TimeAveragedNavierStokesWallCondition() override {}
    ...
    /// Assignment operator
    TimeAveragedNavierStokesWallCondition & operator=(TimeAveragedNavierStokesWallCondition const& rOther)
    {
    ...
    /// Create a new TimeAveragedNavierStokesWallCondition object.
    /**
      @param NewId Index of the new condition
      @param ThisNodes An array containing the nodes of the new condition
      @param pProperties Pointer to the element's properties
    ...
    {
        return Kratos::make_shared<TimeAveragedNavierStokesWallCondition>(NewId, GetGeometry().Create(ThisNodes), pProperties);
    }
    ...
    {
        return Kratos::make_shared< TimeAveragedNavierStokesWallCondition >(NewId, pGeom, pProperties);
    }
    ...
    int Check(const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        else
        {
            ...
            if(TIME_AVERAGED_VELOCITY.Key() == 0)
                KRATOS_ERROR << "TIME_AVERAGED_VELOCITY Key is 0. Check if the application was correctly registered.";
            ...
            if(TIME_AVERAGED_PRESSURE.Key() == 0)
                KRATOS_ERROR << "TIME_AVERAGED_PRESSURE Key is 0. Check if the application was correctly registered.";
            if(DENSITY.Key() == 0)
            ...
            for(unsigned int i=0; i<this->GetGeometry().size(); ++i)
            {
                if(this->GetGeometry()[i].SolutionStepsDataHas(TIME_AVERAGED_VELOCITY) == false)
                    KRATOS_ERROR << "missing TIME_AVERAGED_VELOCITY variable on solution step data for node " << this->GetGeometry()[i].Id();
                if(this->GetGeometry()[i].SolutionStepsDataHas(TIME_AVERAGED_PRESSURE) == false)
                    KRATOS_ERROR << "missing TIME_AVERAGED_PRESSURE variable on solution step data for node " << this->GetGeometry()[i].Id();
                ...
                if(this->GetGeometry()[i].HasDofFor(TIME_AVERAGED_VELOCITY_X) == false ||
                   ...
                   this->GetGeometry()[i].HasDofFor(TIME_AVERAGED_VELOCITY_Z) == false)
                    KRATOS_ERROR << "missing TIME_AVERAGED_VELOCITY component degree of freedom on node " << this->GetGeometry()[i].Id();
                if(this->GetGeometry()[i].HasDofFor(TIME_AVERAGED_PRESSURE) == false)
                    KRATOS_ERROR << "missing TIME_AVERAGED_PRESSURE component degree of freedom on node " << this->GetGeometry()[i].Id();
            }
    ...
    std::string Info() const override
    {
        ...
        buffer << "TimeAveragedNavierStokesWallCondition" << TDim << "D";
        return buffer.str();
    }
    ...
    {
        rOStream << "TimeAveragedNavierStokesWallCondition";
    }
...
}; // Class TimeAveragedNavierStokesWallCondition
...
inline std::istream& operator >> (std::istream& rIStream, TimeAveragedNavierStokesWallCondition<TDim,TNumNodes>& rThis)
{
    return rIStream;
}
...
template< unsigned int TDim, unsigned int TNumNodes >
inline std::ostream& operator << (std::ostream& rOStream, const TimeAveragedNavierStokesWallCondition<TDim,TNumNodes>& rThis)
{
