...
#include "fluid_topology_optimization_wall_condition.h"
...
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::EquationIdVector(
    ...
    const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    else if (problem_physics == 2)
    {
        ...
        const unsigned int v_pos =  r_geometry[0].GetDofPosition(VELOCITY_ADJ_X);
        const unsigned int p_pos =  r_geometry[0].GetDofPosition(PRESSURE_ADJ);
        for (const auto& r_node : r_geometry) {
            rResult[local_index++] = r_node.GetDof(VELOCITY_ADJ_X, v_pos).EquationId();
            rResult[local_index++] = r_node.GetDof(VELOCITY_ADJ_Y, v_pos + 1).EquationId();
            if constexpr (TDim == 3) {
                rResult[local_index++] = r_node.GetDof(VELOCITY_ADJ_Z, v_pos + 2).EquationId();
            }
            rResult[local_index++] = r_node.GetDof(PRESSURE_ADJ, p_pos).EquationId();
        }
    }
    else
    {
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::GetDofList(
    ...
    const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    else if (problem_physics == 2)
    {
        ...
        const unsigned int p_pos = r_geometry[0].GetDofPosition(PRESSURE_ADJ);
        for (const auto& r_node : r_geometry) {
            rConditionDofList[local_index++] = r_node.pGetDof(VELOCITY_ADJ_X, v_pos);
            rConditionDofList[local_index++] = r_node.pGetDof(VELOCITY_ADJ_Y, v_pos + 1);
            if constexpr (TDim == 3) {
                rConditionDofList[local_index++] = r_node.pGetDof(VELOCITY_ADJ_Z, v_pos + 2);
            }
            rConditionDofList[local_index++] = r_node.pGetDof(PRESSURE_ADJ, p_pos);
        }
    }
    else
    {
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::CalculateLocalSystem(
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    static_assert(n_wall_models < 2, "More than one template wall model argument in 'FluidTopologyOptimizationWallCondition'.");
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::CalculateLeftHandSide(
    MatrixType& rLeftHandSideMatrix,
    const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::CalculateRightHandSide(
    VectorType& rRightHandSideVector,
    const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
 ...
 */
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
int FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::Check(const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY;
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim, TNumNodes,TWallModel...>::Calculate(
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::CalculateNormal(array_1d<double,3>& rAreaNormal)
{
    const auto& r_geom = GetGeometry();
    if constexpr (TDim == 2) {
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::ComputeGaussPointLHSContribution(
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::ComputeGaussPointRHSContribution(
    ...
    const ProcessInfo& rProcessInfo)
{
    ...
    // Now the distinction of what to do for the primal or adjoint problem must be done calling the relative Kratos property
    unsigned int problem_physics = rProcessInfo[FLUID_TOP_OPT_PROBLEM_STAGE];
    // problem_physics = 1: NS equations
    // problem_physics = 2: ADJOINT NS equations
    if (problem_physics == 1)
    {
    ...
    {
        // Gauss pt. Neumann BC contribution
        this->ComputeRHSNeumannContributionAdjoint(rhs_gauss, data);
        // Gauss pt. outlet inflow prevention contribution
        if (rProcessInfo.Has(OUTLET_INFLOW_CONTRIBUTION_SWITCH)) {
            if (this->Is(OUTLET) && rProcessInfo[OUTLET_INFLOW_CONTRIBUTION_SWITCH]){
                this->ComputeRHSOutletInflowContributionAdjoint(rhs_gauss, data, rProcessInfo);
            }
        }
        // Contribution to avoid spurious tangential components in the pure-slip residual
        if (rProcessInfo.Has(SLIP_TANGENTIAL_CORRECTION_SWITCH)) {
            if (this->Is(SLIP) && rProcessInfo[SLIP_TANGENTIAL_CORRECTION_SWITCH]) {
                CalculateGaussPointSlipTangentialCorrectionRHSContributionAdjoint(rhs_gauss, data);
            }
        }
    }
    else
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::ComputeRHSNeumannContribution(
    array_1d<double,LocalSize>& rhs_gauss,
    const ConditionDataStruct& data)
{
    const unsigned int LocalSize = TDim+1;
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::ComputeRHSNeumannContributionAdjoint(
    array_1d<double,LocalSize>& rhs_gauss,
    const ConditionDataStruct& data)
{
    const unsigned int LocalSize = TDim+1;
    ...
    for (unsigned int i=0; i<TNumNodes; ++i)
    {
        const double pext = rGeom[i].FastGetSolutionStepValue(EXTERNAL_PRESSURE_ADJ);
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::ComputeRHSOutletInflowContribution(
    array_1d<double,LocalSize>& rhs_gauss,
    const ConditionDataStruct& data,
    const ProcessInfo& rProcessInfo)
{
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::ComputeRHSOutletInflowContributionAdjoint(
    array_1d<double,LocalSize>& rhs_gauss,
    const ConditionDataStruct& data,
    const ProcessInfo& rProcessInfo)
{
    ...
    for (unsigned int i=0; i<TNumNodes; ++i)
    {
        const array_1d<double, 3>& rVelNode = rGeom[i].FastGetSolutionStepValue(VELOCITY_ADJ);
        vGauss += data.N[i]*rVelNode;
    }
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::CalculateGaussPointSlipTangentialCorrectionLHSContribution(
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::CalculateGaussPointSlipTangentialCorrectionRHSContribution(
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::CalculateGaussPointSlipTangentialCorrectionRHSContributionAdjoint(
    array_1d<double,LocalSize>& rRightHandSideVector,
    const ConditionDataStruct& rDataStruct)
{
    KRATOS_TRY
    ...
    for (std::size_t i_node = 0; i_node < TNumNodes; ++i_node) {
        ...
        for (std::size_t j_node = 0; j_node < TNumNodes; ++j_node) {
            if constexpr (VoigtSize == 3) {
                // Voigt stress
                voigt_stress[0] = r_viscous_stress[0] - r_geom[j_node].FastGetSolutionStepValue(PRESSURE_ADJ);
                voigt_stress[1] = r_viscous_stress[1] - r_geom[j_node].FastGetSolutionStepValue(PRESSURE_ADJ);
                voigt_stress[2] = r_viscous_stress[2]; // no pressure in shear component
                // Projection along the condition normal
                cauchy_traction_vect[0] += r_N[j_node]*(voigt_stress[0]*r_cond_normal[0] + voigt_stress[2]*r_cond_normal[1]);
                cauchy_traction_vect[1] += r_N[j_node]*(voigt_stress[2]*r_cond_normal[0] + voigt_stress[1]*r_cond_normal[1]);
                ...
                voigt_stress[0] = r_viscous_stress[0] - r_geom[j_node].FastGetSolutionStepValue(PRESSURE_ADJ);
                voigt_stress[1] = r_viscous_stress[1] - r_geom[j_node].FastGetSolutionStepValue(PRESSURE_ADJ);
                voigt_stress[2] = r_viscous_stress[2] - r_geom[j_node].FastGetSolutionStepValue(PRESSURE_ADJ);
                voigt_stress[3] = r_viscous_stress[3]; // no pressure in shear component
                voigt_stress[4] = r_viscous_stress[4]; // no pressure in shear component
                voigt_stress[5] = r_viscous_stress[5]; // no pressure in shear component
                // Projection along the condition normal
...
template<unsigned int TDim, unsigned int TNumNodes, class... TWallModel>
void FluidTopologyOptimizationWallCondition<TDim,TNumNodes,TWallModel...>::ProjectViscousStress(
...
template class FluidTopologyOptimizationWallCondition<2,2>;
template class FluidTopologyOptimizationWallCondition<3,3>;
} // namespace Kratos
