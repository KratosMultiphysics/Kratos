...
///PAVEL: THIS IS THE IDEA OF SERGIO OF PUTTING THE PERIODIC HORIZONATL WALL IN THE MIDDLE OF THE RVE (at y=0.5.. this coordinate us called Y_locatiob)
void AddThemWithTangentInversePeriodicityForBoundryElementWithPeriodicityIntheMidH(const double x_low ,const double x_high, const double y_low, const double y_high, const double Y_BC_location  ) 
		{
			KRATOS_TRY
			...
			{
				ModelPart::NodesContainerType::iterator inode = mr_model_part.NodesBegin() + ii;
				// I WANT TO EXCLUDE THE UPPER CORNER NODES 
				if(inode->X()<=x_low && inode->Y()<y_high) //good!, we have found an element that is on the lower boundary of the slice (y=0)
				{
					left_wall_y_coord.push_back(inode->Y());
					left_wall_pointers.push_back(*inode.base());
					//taken_right.push_back(false);
				}
				else if(inode->X()>=x_high && inode->Y()<y_high) //good!, we have found an element that is on the lower boundary of the slice (y=0)
				{
					right_wall_y_coord.push_back(inode->Y());
					right_wall_pointers.push_back(*inode.base());
					taken_vector_right.push_back(false);
				...
				//PAVEL: added upper bound
				else if(inode->Y()>=y_high && inode->Y()<=Y_BC_location) //good!, we have found an element that is on the lower boundary of the slice (y=0)
				{
					up_wall_x_coord.push_back(inode->X());
					up_wall_pointers.push_back(*inode.base());
					taken_vector_up.push_back(false);
...
//PAVEL
//FOR "DOUBLE BOUNDARY ELEMENTS" - to consider only the lower part (the one that does not contain the "solid" boundary).. we shall consider normal RVE 
//everything that is below y_LIMIT
void AddThemWithTangentInversePeriodicityAllWallsBelowGivenY(const double x_low ,const double x_high, const double y_low, const double y_high, const double y_LIMIT  ) 
		{
			KRATOS_TRY
			...
			std::cout << "Finished adding periodic condtions. In total there are: " << condition_number << std::endl;

			
			KRATOS_CATCH("")
		} 
//PAVEL
//y_bottom is the coordinate, above which there will be these wall conditions applied
void AddThemWithTangentInversePeriodicityOnlyAtVericalWallsAboveGivenY(const double x_low ,const double x_high, const double y_LIMIT) 
		{
			KRATOS_TRY
			...
			//first loop, CREATING THE LIST OF THE WALLS WHERE PERIODIC CONDITIONS ARE TO BE APPLIED
			//IN THIS CASE IT WILL BE JUST THE VERTICAL WALLS

			for(unsigned int ii=0; ii<mr_model_part.Nodes().size(); ii++)
			{
				ModelPart::NodesContainerType::iterator inode = mr_model_part.NodesBegin() + ii;
				if(inode->X()<=x_low && inode->Y()>y_LIMIT) //good!, we have found an element that is on the lower boundary of the slice (y=0)
				{
					left_wall_y_coord.push_back(inode->Y());
					left_wall_pointers.push_back(*inode.base());
					//taken_right.push_back(false);
					KRATOS_WATCH("Left node")
					KRATOS_WATCH(inode->X())
					KRATOS_WATCH(inode->Y())
				}

				else if(inode->X()>=x_high && inode->Y()>y_LIMIT) //good!, we have found an element that is on the lower boundary of the slice (y=0)
				{
					right_wall_y_coord.push_back(inode->Y());
					right_wall_pointers.push_back(*inode.base());
					taken_vector_right.push_back(false);
					KRATOS_WATCH("Right node")
					KRATOS_WATCH(inode->X())
					KRATOS_WATCH(inode->Y())
				}
			...
			//after looping all the elements, we should have found all the segments that are needed to create the conditions.
			//they must match EXACTLY! otherwise this will return an error.				
			if (right_wall_y_coord.size()==left_wall_y_coord.size()) //good! it maches!
			{
					...
					for(unsigned int jj=0; jj<right_wall_y_coord.size(); jj++)
					{
						if(taken_vector_right[jj]==false) //ok, this one is still free! it could be our match
						{
							if( fabs(coordinate-right_wall_y_coord[jj])<1.0e-6 )
							{
								...
								//now we check if we can also do it for the tangential component:
								if (coordinate > y_LIMIT) // && coordinate < y_high) //then it means we are not corner nodes.
								{
