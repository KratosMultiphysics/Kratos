...
namespace Kratos
{
	//************************************************************************************
	//************************************************************************************
	InverseNormalVelocityPeriodicCondition2D2N::InverseNormalVelocityPeriodicCondition2D2N(IndexType NewId, GeometryType::Pointer pGeometry)
	...
	//************************************************************************************
	//************************************************************************************
	InverseNormalVelocityPeriodicCondition2D2N::InverseNormalVelocityPeriodicCondition2D2N(IndexType NewId, GeometryType::Pointer pGeometry,  PropertiesType::Pointer pProperties)
		: Condition(NewId, pGeometry, pProperties)
	{
	}
	Condition::Pointer InverseNormalVelocityPeriodicCondition2D2N::Create(IndexType NewId, NodesArrayType const& ThisNodes,  PropertiesType::Pointer pProperties) const
	{
		return Condition::Pointer(new InverseNormalVelocityPeriodicCondition2D2N(NewId, GetGeometry().Create(ThisNodes), pProperties));
	}

	InverseNormalVelocityPeriodicCondition2D2N::~InverseNormalVelocityPeriodicCondition2D2N()
	{
	}

	//************************************************************************************
	//************************************************************************************
	void InverseNormalVelocityPeriodicCondition2D2N::CalculateRightHandSide(VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY
		/*
	...
	//************************************************************************************
	/*
	void InverseNormalVelocityPeriodicCondition2D2N::CalculateLocalVelocityContribution(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	...
	*/
...

	{
		...
		if(fabs(GetGeometry()[0].X()-GetGeometry()[1].X())<0.00001)
			temp_vector[2]=GetGeometry()[0].GetSolutionStepValue(LAGRANGE_MULTIPLIER_NORMAL_VELOCITY_Y,0);
		else
			temp_vector[2]=GetGeometry()[0].GetSolutionStepValue(LAGRANGE_MULTIPLIER_NORMAL_VELOCITY_X,0);
	...
	void InverseNormalVelocityPeriodicCondition2D2N::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	...
	//************************************************************************************
	//************************************************************************************
	void InverseNormalVelocityPeriodicCondition2D2N::EquationIdVector(EquationIdVectorType& rResult, ProcessInfo& rCurrentProcessInfo)
	{
		 if (rResult.size()!=(3))
			    rResult.resize(3);
		 ...
		 { 
			 ...
			 rResult[LocalIndex++] = GetGeometry()[0].GetDof(LAGRANGE_MULTIPLIER_NORMAL_VELOCITY_Y).EquationId();
		 }
		 else if(fabs(GetGeometry()[0].Y()-GetGeometry()[1].Y())<0.00001)// && GetGeometry()[0].GetSolutionStepValue(NODE_PAIR_Y_COMPONENT)!=0 ) //then this is the condition relating left and right nodes that needs that is not in the corner.
		 {
			for (unsigned int iNode = 0; iNode < 2; ++iNode)
			 ...
			 rResult[LocalIndex++] = GetGeometry()[0].GetDof(LAGRANGE_MULTIPLIER_NORMAL_VELOCITY_X).EquationId();
		 }
	}

	//************************************************************************************
	//************************************************************************************
	  void InverseNormalVelocityPeriodicCondition2D2N::GetDofList(DofsVectorType& ConditionalDofList,ProcessInfo& rCurrentProcessInfo)
	{
		 ...
		 { 
			 ...
			 ConditionalDofList[LocalIndex++] = GetGeometry()[0].pGetDof(LAGRANGE_MULTIPLIER_NORMAL_VELOCITY_Y);
		 }
		 else if(fabs(GetGeometry()[0].Y()-GetGeometry()[1].Y())<0.00001 )// && GetGeometry()[0].GetSolutionStepValue(NODE_PAIR_Y_COMPONENT)!=0 ) //then this is the condition relating left and right nodes that needs that is not in the corner.
		 {
			 for (unsigned int iNode = 0; iNode < 2; ++iNode)
			 ...
			 ConditionalDofList[LocalIndex++] = GetGeometry()[0].pGetDof(LAGRANGE_MULTIPLIER_NORMAL_VELOCITY_X);
		 }
	}
} // Namespace Kratos
