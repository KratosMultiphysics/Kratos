				...
				{ //IS_FREE_SURFACE
					...
					if (number_of_points_in_radius > 0)
					{
						...
						else
						{
							...
							if ((node_it)->FastGetSolutionStepValue(TEMPERATURE) < 298.0)
								(node_it)->FastGetSolutionStepValue(FUEL) = 298.0;
							else
...
#pragma omp parallel for firstprivate(results, N)
			for (int i = 0; i < nparticles; i++)
			{
				...
				if (is_found == true)
				{
					...
					int s2 = 0;
					int sum = 0;
					if (geom[0].FastGetSolutionStepValue(IS_INTERFACE) > 0.5)
						s0 = 1; //IS_INTERFACE
					if (geom[1].FastGetSolutionStepValue(IS_INTERFACE) > 0.5)
						s1 = 1;
					if (geom[2].FastGetSolutionStepValue(IS_INTERFACE) > 0.5)
						s2 = 1;
					sum = s0 + s1 + s2;
			...
			for (ModelPart::NodesContainerType::iterator in = rNodes.begin(); in != rNodes.end(); in++)
			{
				if (in->FastGetSolutionStepValue(IS_LAGRANGIAN_INLET) == 1) //if it is not a wall node i can erase
				{
					...
					for (GlobalPointersVector<Node<3>>::iterator i = in->GetValue(NEIGHBOUR_NODES).begin(); i != in->GetValue(NEIGHBOUR_NODES).end(); i++)
					{
						...
						if (nf >= 2)
						{
							in->FastGetSolutionStepValue(IS_WATER) = 1;
...
#pragma omp parallel for firstprivate(results, N)
		...
		double Calculate_Vol(ModelPart &rLagrangianModelPart)
		{
			...
			{
				if (node_it->GetValue(NEIGHBOUR_ELEMENTS).size() != 0)
				...
				if ((node_it)->Y() < -0.15102)
				{
					...
					if (number_of_points_in_radius > 0)
					{
						...
						if (tot_weight > 0.0)
						{
							...
							if (temperature_aux > 1e9)
								(node_it)->FastGetSolutionStepValue(HEAT_FLUX) += 1e9;
							else
								(node_it)->FastGetSolutionStepValue(HEAT_FLUX) += temperature_aux;
						}
					}
				}
			}
...
#pragma omp parallel for firstprivate(results, N)
			...
			for (ModelPart::NodesContainerType::iterator node_it = rEulerianModelPart.NodesBegin(); node_it != rEulerianModelPart.NodesEnd(); node_it++)
			{
				...
				if (NN != 0.0)
				{
					...
					if (heat_value > 1e9 /*0.5e+7*/)
						heat_value = 1e9 /*0.5e+7*/; //0.5e+7;
					heat = heat_value;
					KRATOS_WATCH(heat);
				}
				//}
	...
	private:
		...
		void CalculateInterfaceNormal(BoundedMatrix<double, 3, 2> &rPoints, array_1d<double, 3> &rDistances, array_1d<double, 2> &normal, double &interface_area, array_1d<double, 3> &Ninterface, BoundedMatrix<double, 2, 2> &rInterfacePoints)
		{
			...
			if ((rDistances(0) * rDistances(1)) < 0.0)
				cut_edges[0] = true; //edge 12 is cut

			else
				cut_edges[0] = false;

			if ((rDistances(1) * rDistances(2)) < 0.0)
				cut_edges[1] = true; //edge 23 is cut.

			else
				cut_edges[1] = false;

			if ((rDistances(2) * rDistances(0)) < 0.0)
				cut_edges[2] = true; //edge 13 is cut.
