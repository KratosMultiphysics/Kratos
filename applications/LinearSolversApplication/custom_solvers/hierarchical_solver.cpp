...
using CoarseMask = std::vector<DofCategory>;
                       ...
                       CoarseMask& rCoarseMask)
{
    ...
    for (auto proxy : MakeProxy<TEntityLocation>(rModelPart)) {
        ...
        if (r_entity.IsActive() && r_geometry.PointsNumber()) {
            ...
            for (std::size_t i_node=0ul; i_node<coarse_node_count; ++i_node) {
                ...
                // The DoF is a slave => mark it as such
    ...
    // Loop through the active DoFs and remove any slave or dirichlet DoF
    // from the coarse system, while making sure that master DoFs are included.
    // This is a required step if the mesh contains hanging nodes that are not
    // part of any element but are connected to the rest of the mesh in some
    // other manner (with MPCs for example).
    for (const Dof<double>& r_dof : rDofs) {
        // If the DoF is fixed or a slave, it should not be included in the coarse system.
        if (r_dof.IsFixed() || r_dof.Is(SLAVE)) {
            rCoarseMask[r_dof.EquationId()] = DofCategory::Constrained;
        }

        // If the DoF is a master, the options are more complicated
        else if (r_dof.Is(MASTER)) {
            // A special case must be handled here: disconnected nodes that aren't part of any element
            // A distinction is made by either marking it DofCategory::Coarse or DofCategory::Unknown.
            // DofCategory::Unknown will also make it into the coarse system in this case, but its
            // restriction must be handled separately.
            const auto dof_category = rCoarseMask[r_dof.EquationId()];
            if (dof_category != DofCategory::Fine && dof_category != DofCategory::Unknown) {
                rCoarseMask[r_dof.EquationId()] = DofCategory::Coarse;
            }
        }

        // If the DoF is not a master and is not part of any elements, the case is unhandled
        // and it should not make it into the coarse system (for now).
        else if (rCoarseMask[r_dof.EquationId()] == DofCategory::Unknown) {
            rCoarseMask[r_dof.EquationId()] = DofCategory::Fine;
        }
    }
...
using DofData = std::vector<std::tuple<
    const Element*,                     // <== element that contains the DoF
    std::size_t,                        // <== index of the coarse node within the element that contains the DoF
    std::size_t                         // <== index of the DoF within the coarse node
>>;
...
using SlaveMap = std::unordered_map<
    ...
    std::pair<
        std::vector<std::pair<
            std::size_t,                // <== master DoF equation IDs
            double                      // <== coefficient of the MPC relationship
        >>,
        DofData                         // <== info about the DoF's connectivities
    >
>;
...
void ProcessMasterDof(const std::size_t iCoarseDofInFineSystem,
                      const double Weight,
                      const Detail::SlaveMap::mapped_type& rSlaveInfo,
                      const double ConstraintCoefficient,
                      const Detail::MasterMap& rMasterMap,
                      const Detail::SlaveMap& rSlaveMap,
                      const CoarseMask& rCoarseDofMask,
                      const std::vector<std::size_t>& rCoarseDofMap,
                      Detail::RestrictionMap& rRestrictionMap)
{
    KRATOS_TRY
    const auto it_slave_end = rSlaveMap.end();
    const std::size_t i_coarse_dof = rCoarseDofMap[iCoarseDofInFineSystem];

    for (const auto& [p_element, i_slave_vertex, i_slave_node_dof] : rSlaveInfo.second) {
        const Geometry<Node>& r_fine_geometry = p_element->GetGeometry();
        std::vector<
            std::vector<std::pair<
        ...
        if (i_slave_vertex < restriction_coefficients.size()) {
            ...
            for (const auto& [i_fine_vertex, restriction_coefficient] : r_restriction_terms) {
                ...
                if (is_slave) {
                    //for (const auto& [i_master, master_coefficient] : it_slave->second.first) {
                    //    if (i_master == iCoarseDofInFineSystem) {
                    //        // This is the same master we're already looping over, so no need to apply
                    //        // the MPC's coefficient.
                    //        rRestrictionMap.emplace(std::make_pair(i_coarse_dof, i_master), Weight * restriction_coefficient);
                    //    } else {
                    //        rRestrictionMap.emplace(std::make_pair(i_coarse_dof, i_master), Weight * restriction_coefficient * slave_coefficient / master_coefficient);
                    //        const auto it_master = rMasterMap.find(i_master);
                    //        KRATOS_ERROR_IF(it_master == rMasterMap.end());
                    //        if (rVisitedDofIds.insert(it_master->first).second) {
                    //            ProcessMasterDof(iCoarseDofInFineSystem,
                    //                             Weight * restriction_coefficient * slave_coefficient / master_coefficient,
                    //                             it_master->second,
                    //                             rMasterMap,
                    //                             rSlaveMap,
                    //                             rCoarseDofMask,
                    ...
                    rRestrictionMap.emplace(std::make_pair(i_coarse_dof, i_fine_dof), Weight * restriction_coefficient * ConstraintCoefficient);
                }
...
void MapHigherToLowerOrder(const ModelPart& rModelPart,
                           Detail::MasterMap& rMasterMap,
                           Detail::SlaveMap& rSlaveMap,
                           CoarseMask& rCoarseDofMask,
                           const std::vector<std::size_t>& rCoarseDofMap,
                           Detail::RestrictionMap& rRestrictionMap)
{
    KRATOS_PROFILE_SCOPE(KRATOS_CODE_LOCATION);
    ...
    for (auto proxy : MakeProxy<TEntityLocation>(rModelPart)) {
        ...
        if (r_entity.IsActive() && fine_vertex_count) {
            ...
            for (std::size_t i_coarse_vertex=0ul; i_coarse_vertex<restriction_coefficients.size(); ++i_coarse_vertex) {
                ...
                for (std::size_t i_node_dof=0ul; i_node_dof<dofs_per_node; ++i_node_dof) {
                    ...
                    for (std::size_t i_term=0ul; i_term<i_term_end; ++i_term) {
                        ...
                        if (rCoarseDofMask[i_coarse_dof_in_fine_system] == DofCategory::Coarse) { // <== ignore DoF if not in the coarse set
                    ...
                    const auto it_master_dof = rMasterMap.find(i_coarse_dof_in_fine_system);
                    if (it_master_dof != rMasterMap.end()) {
                        it_master_dof->second.first.emplace_back(&r_entity,
                                                                 i_coarse_vertex,
                                                                 i_node_dof);
                    } // if it_master_dof != rMasterMap.end
            ...
            // Loop over the rest of the fine vertices to fill the master and slave maps
            if (!rSlaveMap.empty()) {
                for (std::size_t i_fine_vertex=restriction_coefficients.size(); i_fine_vertex<r_fine_geometry.size(); ++i_fine_vertex) {
                    ...
                    for (std::size_t i_node_dof=0ul; i_node_dof<dofs_per_node; ++i_node_dof) {
                        ...
                        const auto it_slave_dof = rSlaveMap.find(i_dof);
                        if (it_slave_dof != it_slave_end) {
                            it_slave_dof->second.second.emplace_back(&r_entity,
                                                                     i_fine_vertex,
                                                                     i_node_dof);
            ...
            if (!rMasterMap.empty()) {
                for (std::size_t i_fine_vertex=restriction_coefficients.size(); i_fine_vertex<r_fine_geometry.size(); ++i_fine_vertex) {
                    ...
                    for (std::size_t i_node_dof=0ul; i_node_dof<dofs_per_node; ++i_node_dof) {
                        const std::size_t i_dof = r_fine_node.GetDofs()[i_node_dof]->EquationId();
                        const auto it_master_dof = rMasterMap.find(i_dof);
                        if (it_master_dof != rMasterMap.end()) {
                            it_master_dof->second.first.emplace_back(&r_entity,
                                                                     i_fine_vertex,
                                                                     i_node_dof);
    ...
    for (const auto& [i_coarse_dof_in_fine_system, r_master_info] : rMasterMap) {
        const auto dof_category = rCoarseDofMask[i_coarse_dof_in_fine_system];

        // Skip the DoF if it is not part of the coarse system
        if (dof_category == DofCategory::Fine || dof_category == DofCategory::Constrained) {
            continue;
        }

        // Special case if the DoF is a master and not part of any element
        if (dof_category == DofCategory::Unknown) {
            const std::size_t i_coarse_dof_in_coarse_system = rCoarseDofMap[i_coarse_dof_in_fine_system];
            rRestrictionMap.emplace(std::make_pair(i_coarse_dof_in_coarse_system, i_coarse_dof_in_fine_system), 1.0);
        }

        for (const auto& [it_slave_info, slave_coefficient] : r_master_info.second) {
            Detail::RestrictionMap local_restriction;
            ProcessMasterDof(i_coarse_dof_in_fine_system,
                             1.0,
                             it_slave_info->second,
                             slave_coefficient,
                             rMasterMap,
                             rSlaveMap,
                             rCoarseDofMask,
            ...
            for (const auto& [key, value] : local_restriction) {
                rRestrictionMap.emplace(key, 0.0).first->second += value;
            }
        }
    }
    KRATOS_CATCH("")
                                                                                    ...
                                                                                    ModelPart& rModelPart)
{
    ...
    CoarseMask coarse_mask(system_size, DofCategory::Unknown);
    ...
    if (mpImpl->mCoarseSystem.mpSolver->AdditionalPhysicalDataIsNeeded()) {
        ...
        for (std::size_t i_mask=0; i_mask<system_size; ++i_mask) {
            if (coarse_mask[i_mask] == DofCategory::Coarse || coarse_mask[i_mask] == DofCategory::Unknown) {
...

    {
        ...
        Detail::RestrictionMap restriction_map;
        ...
        for (const auto& r_constraint : rModelPart.MasterSlaveConstraints()) {
            ...
            for (std::size_t i_master=0ul; i_master<r_constraint.GetMasterDofsVector().size(); ++i_master) {
                const Dof<double>* p_master = r_constraint.GetMasterDofsVector()[i_master];
                for (std::size_t i_slave=0ul; i_slave<r_constraint.GetSlaveDofsVector().size(); ++i_slave) {
                    const Dof<double>* p_slave = r_constraint.GetSlaveDofsVector()[i_slave];
                    const auto coefficient = coefficients(i_slave, i_master);
                    slave_map.emplace(p_slave->EquationId(), Detail::SlaveMap::mapped_type {})
                             .first     // <== iterator to the emplaced entry
                             ->second   // <== mapped value
                             .first     // <== array of master IDs and MPC coefficients
                             .emplace_back(p_master->EquationId(), coefficient);
                } // for p_slave in r_constraint.GetSlaveDofsVector
            } // for p_master in r_constraint.GetMasterDofsVector
            ...
            for (std::size_t i_master=0ul; i_master<r_constraint.GetMasterDofsVector().size(); ++i_master) {
                ...
                for (std::size_t i_slave=0ul; i_slave<r_constraint.GetSlaveDofsVector().size(); ++i_slave) {
                    ...
                    if (coefficient) {
                        ...
                        it_master->second.second.emplace_back(it_slave, coefficients(i_slave, i_master));
                    } // if coefficient != 0
