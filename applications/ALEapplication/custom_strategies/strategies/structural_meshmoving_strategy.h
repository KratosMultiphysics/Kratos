...
class StructuralMeshMovingStrategy : public SolvingStrategy<TSparseSpace,
 ...
 public:
  ...
  : SolvingStrategy<TSparseSpace,TDenseSpace,TLinearSolver>(model_part)
  {
    ...
    (*mp_mesh_model_part,pscheme,pNewLinearSolver,mp_bulider_and_solver,m_compute_reactions,m_reform_dof_set_at_each_step,calculate_norm_dx_flag) );
  ...
  double Solve() override
  {
    ...
    for(ModelPart::NodeIterator i = (*mp_mesh_model_part).NodesBegin(); i != (*mp_mesh_model_part).NodesEnd(); ++i){
  ...
  void CalculateMeshVelocities()
  {
    ...
    else if (m_time_order == 2)  //mesh velocity calculated as (3*x(n+1)-4*x(n)+x(n-1))/(2*Dt)
    {
...
private:
  ...
  void GenerateMeshPart()
  {
    ...
    mp_mesh_model_part->GetNodalSolutionStepVariablesList() = BaseType::GetModelPart().GetNodalSolutionStepVariablesList();
