...
class TestCase(KratosUnittest.TestCase):
    ...
    def test_Rectangle_2D3N(self):
        with ControlledExecutionScope(os.path.dirname(os.path.realpath(__file__))):
            ...
            # remove files
            if rank == 0:
                self.removeFile("./test_laplacian_mesh_motion_2d/rectangle_2D3N_test_probe1.dat")
                self.removeFile("./test_laplacian_mesh_motion_2d/rectangle_2D3N_test_probe2.dat")
                self.removeFile("./test_laplacian_mesh_motion_2d/rectangle_2D3N_test.time")
            self.removeFile("./test_laplacian_mesh_motion_2d/rectangle_2D3N_test_" + str(rank) + ".time")
            self.removeFile("./test_laplacian_mesh_motion_2d/rectangle_2D3N_test_" + str(rank) + ".mdpa")
