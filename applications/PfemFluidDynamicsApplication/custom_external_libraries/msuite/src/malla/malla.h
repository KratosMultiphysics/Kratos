...
#include "quantech.h"
    ...
    array1<double> re;
    array1<punto> ce; // centro y radio
    bool hayfe; // hay flags de elementos
...
//  bool suavej(double clim=.25,const punto *ps=0);
    ...
    void rm_esferas()
    {
        ce.ini();    // no borra volumen ni normales
        re.ini();
    }
    ...
    bool inter(const punto &p)
    {
        return (dequien(p)>=0);   // interior de la malla
    }
