...
#if !defined(KRATOS_PFEM_ELEMENT_UTILITIES_H_INCLUDED)
#define KRATOS_PFEM_ELEMENT_UTILITIES_H_INCLUDED
...
class PfemElementUtilities {
   public:
     ...
     */
	static inline void CalculateVelocityGradient(
	    Matrix& rVelocityGradient,
        ...
        if (Alpha != 1.0) {
            if (dimension == 2) {
                for (SizeType i = 0; i < number_of_nodes; i++) {
                    ...
                    rVelocityGradient(0, 0) +=
                        (rCurrentVelocity[0] * Alpha + rPreviousVelocity[0] * (1.0 - Alpha)) * rDN_DX(i, 0);
                    rVelocityGradient(0, 1) +=
                        (rCurrentVelocity[0] * Alpha + rPreviousVelocity[0] * (1.0 - Alpha)) * rDN_DX(i, 1);
                    rVelocityGradient(1, 0) +=
                        (rCurrentVelocity[1] * Alpha + rPreviousVelocity[1] * (1.0 - Alpha)) * rDN_DX(i, 0);
                    rVelocityGradient(1, 1) +=
                        (rCurrentVelocity[1] * Alpha + rPreviousVelocity[1] * (1.0 - Alpha)) * rDN_DX(i, 1);
                }
                if (VoigtSize == 4) {
                    //
                }
                ...
                //KRATOS_WATCH(rDN_DX);
                //KRATOS_WATCH(rDN_DX);
                //KRATOS_WATCH(rVelocityGradient);
            } else if (dimension == 3) {
                for (SizeType i = 0; i < number_of_nodes; i++) {
                    const array_1d<double, 3>& rPreviousVelocity = rGeometry[i].FastGetSolutionStepValue(VELOCITY, 1);
                    const array_1d<double, 3>& rCurrentVelocity = rGeometry[i].FastGetSolutionStepValue(VELOCITY);
                    ...
                    rVelocityGradient(0, 1) +=
                        (rCurrentVelocity[0] * Alpha + rPreviousVelocity[0] * (1.0 - Alpha)) * rDN_DX(i, 1);
                    rVelocityGradient(0, 2) +=
                        (rCurrentVelocity[0] * Alpha + rPreviousVelocity[0] * (1.0 - Alpha)) * rDN_DX(i, 2);
                    rVelocityGradient(1, 0) +=
                        (rCurrentVelocity[1] * Alpha + rPreviousVelocity[1] * (1.0 - Alpha)) * rDN_DX(i, 0);
                    rVelocityGradient(1, 1) +=
                        (rCurrentVelocity[1] * Alpha + rPreviousVelocity[1] * (1.0 - Alpha)) * rDN_DX(i, 1);
                    rVelocityGradient(1, 2) +=
                        (rCurrentVelocity[1] * Alpha + rPreviousVelocity[1] * (1.0 - Alpha)) * rDN_DX(i, 2);
                    rVelocityGradient(2, 0) +=
                        (rCurrentVelocity[2] * Alpha + rPreviousVelocity[2] * (1.0 - Alpha)) * rDN_DX(i, 0);
                    rVelocityGradient(2, 1) +=
                        (rCurrentVelocity[2] * Alpha + rPreviousVelocity[2] * (1.0 - Alpha)) * rDN_DX(i, 1);
                    rVelocityGradient(2, 2) +=
                        (rCurrentVelocity[2] * Alpha + rPreviousVelocity[2] * (1.0 - Alpha)) * rDN_DX(i, 2);
                }
            }
     ...
     * @param Alpha, parameter to change the step calculation [0,1]
     */
	static inline void CalculateVelocityGradientVector(
	    Vector& rVelocityGradient,
	    const GeometryType& rGeometry,
	    const Matrix& rDN_DX,
	    const double Alpha = 1.0) {
		...
		if (rVelocityGradient.size() != dimension * dimension) rVelocityGradient.resize(dimension * dimension);
    ...
    static inline void CalculateSkewSymmetricVelocityGradientVector(const Matrix& rVelocityGradientMatrix,
        ...
        if (rDimension == 2) {
            ...
            rSkewSymmetricVelocityGradientVector[2] =
                0.5 * (rVelocityGradientMatrix(0, 1) - rVelocityGradientMatrix(1, 0));  // xy
            ...
            rSkewSymmetricVelocityGradientVector[3] =
                0.5 * (rVelocityGradientMatrix(0, 1) - rVelocityGradientMatrix(1, 0));  // xy
            rSkewSymmetricVelocityGradientVector[4] =
                0.5 * (rVelocityGradientMatrix(1, 2) - rVelocityGradientMatrix(2, 1));  // yz
            rSkewSymmetricVelocityGradientVector[5] =
                0.5 * (rVelocityGradientMatrix(0, 2) - rVelocityGradientMatrix(2, 0));  // xz
     ...
     * @param rDN_DX, shape functions derivatives
     */
	static inline void CalculateLinearDeformationMatrix(
	    Matrix& rDeformationMatrix,
	    const GeometryType& rGeometry,
	    const Matrix& rDN_DX,
	    const unsigned int VoigtSize,
		...
		const SizeType number_of_nodes = rGeometry.PointsNumber();
		const SizeType dimension = rGeometry.WorkingSpaceDimension();
        //const unsigned int voigt_size = (dimension - 1) * 3;//this is not valid in case of axisymmetric analysis

        if (rDeformationMatrix.size1() != VoigtSize || rDeformationMatrix.size2() != dimension * number_of_nodes)
            rDeformationMatrix.resize(VoigtSize, dimension * number_of_nodes, false);

        //Vector N;
        //double Radius = 0.0;
		//if (VoigtSize == 4) {
		//	//N = row(GetGeometry().ShapeFunctionsValues(), PointNumber);
		//	Radius = PfemElementUtilities::CalculateRadius(N, GetGeometry(), Current);
		//}

		//TODO the two for loops with dimension == 2 could be grouped (not sure which is the best solution)
        if (dimension == 2) {
            if (VoigtSize == 3) {
                for (SizeType i = 0; i < number_of_nodes; i++) {
				...
				for (SizeType i = 0; i < number_of_nodes; i++) {
					...
					rDeformationMatrix(1, index + 0) = 0.0;
					rDeformationMatrix(1, index + 1) = rDN_DX(i, 1);
					rDeformationMatrix(2, index + 0) = rShapeFunctions[i] / rRadius;
    ...
    static inline double CalculateStressNorm(const Vector& rStressVector) {
        ...
        double StressNorm = ((StressTensor(0, 0) * StressTensor(0, 0)) + (StressTensor(1, 1) * StressTensor(1, 1)) +
                             (StressTensor(2, 2) * StressTensor(2, 2)) + (StressTensor(0, 1) * StressTensor(0, 1)) +
                             (StressTensor(0, 2) * StressTensor(0, 2)) + (StressTensor(1, 2) * StressTensor(1, 2)) +
                             (StressTensor(1, 0) * StressTensor(1, 0)) + (StressTensor(2, 0) * StressTensor(2, 0)) +
                             (StressTensor(2, 1) * StressTensor(2, 1)));
    ...
    static inline double CalculateVonMises(const Vector& rStressVector) {
        ...
        double SigmaEquivalent =
            (0.5) * ((StressTensor(0, 0) - StressTensor(1, 1)) * ((StressTensor(0, 0) - StressTensor(1, 1))) +
                     ...
                     6 * (StressTensor(0, 1) * StressTensor(1, 0) + StressTensor(1, 2) * StressTensor(2, 1) +
                          StressTensor(2, 0) * StressTensor(0, 2)));

        if (SigmaEquivalent < 0) SigmaEquivalent = 0;
    ...
    static inline double CalculateVonMisesUsingPrincipalStresses(const Vector& rStressVector) {
        ...
        if (!main_tensor) {
            ...
            noalias(MainStresses) = ZeroVector(3);
            for (unsigned int i = 0; i < StressTensor.size1(); i++) MainStresses[i] = StressTensor(i, i);
        }

        for (unsigned int i = 0; i < MainStresses.size(); i++) PrincipalStress[i] = MainStresses[i];
     ...
     * @return r_element_size
     */
    static inline double CalculateElementSize(const GeometryType& rGeometry) {
        const unsigned int number_of_nodes = rGeometry.PointsNumber();
        const unsigned int dimension = rGeometry.WorkingSpaceDimension();

        double r_element_size = 0.0;
        array_1d<double, 3> edge(3, 0.0);
        noalias(edge) = rGeometry[1].Coordinates() - rGeometry[0].Coordinates();
        double aux = std::pow(edge[0], 2.0);
        for (unsigned int d = 1; d < dimension; d++) {
            aux += std::pow(edge[d], 2.0);
        }
        r_element_size += sqrt(aux);

        double count = 1.0;
        for (unsigned int i = 2; i < number_of_nodes; i++) {
            for (unsigned int j = 0; j < i; j++) {
                noalias(edge) = rGeometry[i].Coordinates() - rGeometry[j].Coordinates();
                aux = std::pow(edge[0], 2.0);
                for (unsigned int d = 1; d < dimension; d++) {
                    aux += std::pow(edge[d], 2.0);
                }
                r_element_size += sqrt(aux);
	...
	static inline double CalculateRadius(
		...
		double radius = 0.0;
        //KRATOS_WATCH(rGeometry.size());
        //KRATOS_WATCH(N);

		for (unsigned int i = 0; i < rGeometry.size(); i++) {
			if (Configuration == Current) {
				const array_1d<double, 3> current_position = rGeometry[i].Coordinates();
				radius += current_position[0] * N[i];
			} else if (Configuration == Initial) {
				const array_1d<double, 3> delta_displacement =
				    rGeometry[i].FastGetSolutionStepValue(DISPLACEMENT, 0) -
				    rGeometry[i].FastGetSolutionStepValue(DISPLACEMENT, 1);
                //KRATOS_WATCH("initial_radius");
				const array_1d<double, 3> current_position = rGeometry[i].Coordinates();
				const array_1d<double, 3> reference_position = current_position - delta_displacement;
				radius += reference_position[0] * N[i];
			}
		}
...
#endif  // KRATOS_PFEM_ELEMENT_UTILITIES_H_INCLUDED
