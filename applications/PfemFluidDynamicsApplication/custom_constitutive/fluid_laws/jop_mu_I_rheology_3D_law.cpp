...
JopMuIRheology3DLaw::JopMuIRheology3DLaw() : PfemFluidConstitutiveLaw() {}

//******************************COPY CONSTRUCTOR******************************
//****************************************************************************

JopMuIRheology3DLaw::JopMuIRheology3DLaw(const JopMuIRheology3DLaw& rOther) : PfemFluidConstitutiveLaw(rOther) {}

//***********************************CLONE************************************
//****************************************************************************

ConstitutiveLaw::Pointer JopMuIRheology3DLaw::Clone() const { return Kratos::make_shared<JopMuIRheology3DLaw>(*this); }

//*********************************DESTRUCTOR*********************************
//****************************************************************************

JopMuIRheology3DLaw::~JopMuIRheology3DLaw() {}
...
void JopMuIRheology3DLaw::CalculateMaterialResponseCauchy(Parameters& rValues) {
    ...
    if (equivalent_strain_rate != 0 && std::fabs(mean_pressure) != 0) {
        const double first_viscous_term = static_friction / equivalent_strain_rate;
...
std::string JopMuIRheology3DLaw::Info() const { return "JopMuIRheology3DLaw"; }
...
int JopMuIRheology3DLaw::Check(const Properties& rMaterialProperties, const GeometryType& rElementGeometry,
    ...
    if (rMaterialProperties[STATIC_FRICTION] <= 0.0) {
        KRATOS_ERROR << "Incorrect or missing STATIC_FRICTION provided in process info for JopMuIRheology3DLaw: "
                     << rMaterialProperties[STATIC_FRICTION] << std::endl;
    }

    if (rMaterialProperties[DYNAMIC_FRICTION] <= 0.0) {
        KRATOS_ERROR << "Incorrect or missing DYNAMIC_FRICTION provided in process info for JopMuIRheology3DLaw: "
                     << rMaterialProperties[DYNAMIC_FRICTION] << std::endl;
    }

    if (rMaterialProperties[INERTIAL_NUMBER_ZERO] <= 0.0) {
        KRATOS_ERROR << "Incorrect or missing INERTIAL_NUMBER_ZERO provided in process info for JopMuIRheology3DLaw: "
                     << rMaterialProperties[INERTIAL_NUMBER_ZERO] << std::endl;
    }

    if (rMaterialProperties[GRAIN_DIAMETER] <= 0.0) {
        KRATOS_ERROR << "Incorrect or missing GRAIN_DIAMETER provided in process info for JopMuIRheology3DLaw: "
                     << rMaterialProperties[GRAIN_DIAMETER] << std::endl;
    }

    if (rMaterialProperties[GRAIN_DENSITY] <= 0.0) {
        KRATOS_ERROR << "Incorrect or missing GRAIN_DENSITY provided in process info for JopMuIRheology3DLaw: "
                     << rMaterialProperties[GRAIN_DENSITY] << std::endl;
    }

    if (rMaterialProperties[BULK_MODULUS] <= 0.0) {
        KRATOS_ERROR << "Incorrect or missing BULK_MODULUS provided in process info for JopMuIRheology3DLaw: "
...
double JopMuIRheology3DLaw::GetEffectiveViscosity(ConstitutiveLaw::Parameters& rParameters) const {
    return rParameters.GetConstitutiveMatrix()(5, 5);
}

double JopMuIRheology3DLaw::GetEffectiveDensity(ConstitutiveLaw::Parameters& rParameters) const {
    return rParameters.GetMaterialProperties()[DENSITY];
}

void JopMuIRheology3DLaw::save(Serializer& rSerializer) const {
    KRATOS_SERIALIZE_SAVE_BASE_CLASS(rSerializer, PfemFluidConstitutiveLaw)
}

void JopMuIRheology3DLaw::load(Serializer& rSerializer) {
