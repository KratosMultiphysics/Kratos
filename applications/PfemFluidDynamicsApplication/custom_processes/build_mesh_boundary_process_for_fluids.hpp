...
#if !defined(KRATOS_BUILD_MESH_BOUNDARY_PROCESS_FOR_FLUIDS_H_INCLUDED )
#define  KRATOS_BUILD_MESH_BOUNDARY_PROCESS_FOR_FLUIDS_H_INCLUDED
...
namespace Kratos
{
   ...
   */
  class BuildMeshBoundaryProcessForFluids
    : public Process
  {
  public:
    ///@name Type Definitions
    ...
    /// Pointer definition of BuildMeshBoundaryProcessForFluids
    KRATOS_CLASS_POINTER_DEFINITION( BuildMeshBoundaryProcessForFluids );
    ...
    /// Destructor.
    virtual ~BuildMeshBoundaryProcessForFluids()
    {
    }
    ...
    {
      return "BuildMeshBoundaryProcessForFluids";
    }

    /// Print information about this object.
    virtual void PrintInfo(std::ostream& rOStream) const
    {
      rOStream << "BuildMeshBoundaryProcessForFluids";
    }
  ...
  private:
    ...
    bool UniqueSkinSearch( int MeshId = 0 )
    {
      ...
      if( mEchoLevel > 0 ){
	std::cout<<" [ UniqueSkinSearch...SET BOUNDARY CONDITIONS : "<<std::endl;
	std::cout<<"   Initial Conditions : "<<mrModelPart.Conditions(MeshId).size()<<" [MESH:"<<MeshId<<"]"<<std::endl;
      }
      ...
      for(ModelPart::NodesContainerType::const_iterator in = mrModelPart.NodesBegin(MeshId); in!=mrModelPart.NodesEnd(MeshId); in++)
	{
	  ...
	  // if(MeshId==4){
	  //   if(in->Is(BOUNDARY)){
	  //     std::cout<<"4 BOUNDARY NODE"<<std::endl;
	  //   }else{
	  //     std::cout<<"4 NON-BOUNDARY NODE"<<std::endl;
	  //   }
	  //   if(in->Is(RIGID)){
	  //     std::cout<<"4 RIGID NODE";
	  //   }
	  // }
	  ...
	  // }else{
	  //   std::cout<<"NON-OUTLET NODE"<<std::endl;
	  // }
	  // if(in->Is(INLET)){
	  //   std::cout<<"INLET NODE"<<std::endl;
	  // }else{
	  //   std::cout<<"NON-INLET NODE"<<std::endl;
	  // }
	  // if(in->Is(INTERFACE)){
	  //   std::cout<<"--> INTERFACE NODE"<<std::endl;
	  // }else{
	  //   std::cout<<"--> NON-INTERFACE NODE"<<std::endl;
	  // }
		...
		if (ne->Id() == ie->Id())
		  {
			...
			if( condition_found ){
			  ...
			  std::cout<<"condition_found"<<std::endl;
    ...
    /// Assignment operator.
    BuildMeshBoundaryProcessForFluids& operator=(BuildMeshBoundaryProcessForFluids const& rOther);

    /// Copy constructor.
    //BuildMeshBoundaryProcessForFluids(BuildMeshBoundaryProcessForFluids const& rOther);
  ...
  inline std::istream& operator >> (std::istream& rIStream,
				    BuildMeshBoundaryProcessForFluids& rThis);

  /// output stream function
  inline std::ostream& operator << (std::ostream& rOStream,
				    const BuildMeshBoundaryProcessForFluids& rThis)
  {
...
#endif // KRATOS_BUILD_MESH_BOUNDARY_PROCESS_FOR_FLUIDS_H_INCLUDED  defined 
