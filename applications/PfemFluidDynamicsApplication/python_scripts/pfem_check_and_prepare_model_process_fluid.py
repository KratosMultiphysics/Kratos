...
class CheckAndPrepareModelProcess(KratosMultiphysics.Process):
    ...
    def Execute(self):
        ...
        if self.bodies_list == True:
            for i in range(self.bodies_parts_list.size()):
                ...
                for part in body_parts_list:
                    ...
                    if (body_model_part_type=="Fluid"):
                        ...
                        transfer_process = KratosPfemFluid.TransferProcess(body_model_part,part,entity_type,void_flags,assign_flags)
                    ...
                    transfer_process = KratosPfemFluid.TransferProcess(body_model_part,part,entity_type)
                    transfer_process.Execute()
            ...
            for fluid_part in fluid_body_model_parts:
                ...
                for rigid_part in rigid_body_model_parts:
                    ...
                    transfer_process = KratosPfemFluid.TransferProcess(fluid_part,rigid_part,entity_type,transfer_flags)
                    transfer_process.Execute()
        ...
        entity_type = "Nodes"
        transfer_process = KratosPfemFluid.TransferProcess(fluid_computing_model_part,self.main_model_part,entity_type)
        transfer_process.Execute()

        for part in domain_parts:
            entity_type = "Elements"
            transfer_process = KratosPfemFluid.TransferProcess(fluid_computing_model_part,part,entity_type)
            transfer_process.Execute()
