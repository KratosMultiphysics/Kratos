...
eulerian_model_part.AddNodalSolutionStepVariable(DISTANCE)
eulerian_model_part.AddNodalSolutionStepVariable(DISPLACEMENT)
...
if(SolverType == "FractionalStep"):
    import SurfaceTension_fractional_step_solver as solver
    solver.AddVariables(eulerian_model_part)
elif(SolverType == "monolithic_solver_eulerian"):
    import SurfaceTension_embedded_monolithic_solver as solver
    solver.AddVariables(eulerian_model_part)
...
import SurfaceTension_embedded_monolithic_solver as solver_lagr
...
contact_angle = 110 	#contact angle [deg]
...
viscosity_water = 0.000015

density_air = 1.2
viscosity_air = 0.00001
...
####inlet velocity
H = upper_edge - lower_edge
v_max = 1.0
u_const = -4.0 * v_max / (H * H) 
...
for node in lagrangian_model_part.Nodes:
  ...
  #if (node.Y < 0.0000001): elaf chanched it to:
  if (node.Y <=lower_edge):
...
for node in eulerian_model_part.Nodes:
    ...
    if (node.X <=left_edge):# and node.Y > 0.00000001 and node.Y<0.029999): #### elaf changed node.X < -0.059 to (node.X <= -0.06)
      v_inlet = u_const * node.Y * (node.Y - H)
      node.SetSolutionStepValue(VELOCITY_X,0, v_inlet);   #elaf air initial veolcity (v_air_inital)
      node.SetSolutionStepValue(VELOCITY_Y,0, 0.0)
      node.Fix(VELOCITY_X)
      node.Fix(VELOCITY_Y)
...
for node in eulerian_model_part.Nodes:
    if (node.X>=right_edge): #### elaf changed node.X > 0.059 to (node.X>=.06))
      node.SetSolutionStepValue(PRESSURE, 0, 0.0) #elaf outlet pressure (p_air_inital)
      node.Fix(PRESSURE)
...
if(SolverType == "FractionalStep"):
    ...
    eul_model_part = 1
    fluid_solver = solver.STMonolithicSolver(eulerian_model_part,domain_size,eul_model_part, gamma, contact_angle)  
    ...
    fluid_solver_red = solver.STMonolithicSolver(reduced_model_part,domain_size,eul_model_part, gamma, contact_angle)
...
for node in eulerian_model_part.Nodes:
  if(node.Y<= lower_edge): #non-slip boundary condition for lower wall #### elaf changed
    node.SetSolutionStepValue(VELOCITY_X,0,0.0)
    node.SetSolutionStepValue(VELOCITY_Y,0,0.0)
    node.Fix(VELOCITY_X)
    node.Fix(VELOCITY_Y)
  if(node.Y>= upper_edge): #non-slip boundary condition for upper wall    #### elaf changed
...
while(time <= final_time):
    ...
    if(step >= 3):
        ...
        for node in eulerian_model_part.Nodes:
            if (node.X <=left_edge and node.Y >=lower_edge):    #### #### elaf changed
                v_inlet = u_const * node.Y * (node.Y - H)
                node.SetSolutionStepValue(VELOCITY_X,0, v_inlet);    
                node.Fix(VELOCITY_X)
       ...
       ####################################################################################################
        for node in eulerian_model_part.Nodes:
            ...
            node.Free(AUX_VEL_Y)
            #ALEX
            if (node.X > left_edge and node.X< right_edge and node.Y> lower_edge and node.Y< upper_edge): #### elaf changed
                node.Free(VELOCITY_X)
                ...
                #if (node.Y >= upper_edge): #### #### elaf changed it was commented because it is reffering to very high infinite upper edge 
                #node.Free(VELOCITY_X)
        for node in reduced_model_part.Nodes:
            ...
            if (node.X >left_edge and node.X< right_edge and node.Y>lower_edge and node.Y<upper_edge): #### elaf changed
                ...
                node.Free(FRACT_VEL_X)
                node.Free(FRACT_VEL_Y)
                #if (node.Y > upper_edge): #### #### elaf changed
                #node.Free(VELOCITY_X)	      
            #else:
                #node.SetSolutionStepValue(VELOCITY_X,0,0.0)
                #node.SetSolutionStepValue(VELOCITY_Y,0,0.0)
	...
	# (we neglect the velocity effect of lagrangian domain onto eulerian domain
        ...
        CoupledEulerianUlfUtils.FindIntersectionOfEdges(lagrangian_model_part, eulerian_model_part, aux_model_part, interface_part)	
		      #(ModelPart& origin_model_part, ModelPart& destination_model_part, ModelPart& AuxModelPart, ModelPart& InterfacePart) \\\ commented by elaf here
	...
	#mapping of the viscous stress: \\\ commented by elaf here
        for node in lagrangian_model_part.Nodes:
            node.SetSolutionStepValue(VISCOUS_STRESSX_X,0,0.0)
            node.SetSolutionStepValue(VISCOUS_STRESSX_Y,0,0.0)
            node.SetSolutionStepValue(VISCOUS_STRESSY_X,0,0.0)
            node.SetSolutionStepValue(VISCOUS_STRESSY_Y,0,0.0)
            
	  

        AAA.DirectVectorialVarInterpolation(reduced_model_part, lagrangian_model_part, VISCOUS_STRESSX, VISCOUS_STRESSY);	
