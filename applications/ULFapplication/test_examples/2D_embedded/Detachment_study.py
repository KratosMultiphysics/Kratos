...
for node in lagrangian_model_part.Nodes:
  ...
  node.SetSolutionStepValue(VISCOSITY,0, 0.000015)
...
for node in eulerian_model_part.Nodes:
    node.SetSolutionStepValue(DENSITY, 0, 1.2)
    node.SetSolutionStepValue(VISCOSITY, 0, 0.00001)
    if (node.X <= -0.06):# and node.Y > 0.00000001 and node.Y<0.029999): #### elaf changed node.X < -0.059 to (node.X <= -0.06)
...
for node in eulerian_model_part.Nodes:
    if (node.X>=0.06): #### elaf changed node.X > 0.059 to (node.X>=.06))
      node.SetSolutionStepValue(PRESSURE, 0, 0.0)
      node.Fix(PRESSURE)
...
for node in eulerian_model_part.Nodes:
  if(node.Y<=0.0): #non-slip boundary condition for lower wall #### elaf changed
    node.SetSolutionStepValue(VELOCITY_X,0,0.0)
    node.SetSolutionStepValue(VELOCITY_Y,0,0.0)
    node.Fix(VELOCITY_X)
    node.Fix(VELOCITY_Y)
  if(node.Y>=0.003): #non-slip boundary condition for upper wall    #### elaf changed
...
while(time <= final_time):
    ...
    if(step >= 3):
	const = 1.0
	vel_inlet = (1.0/const)*time + 1.0
	if (vel_inlet > 7.0):
	  vel_inlet = 7.0
	for node in eulerian_model_part.Nodes:
	    if (node.X <= -0.06 and node.Y >= 0.0):    #### #### elaf changed
	...
	mesh_solver.Solve()
	  ...
	  node.Free(AUX_VEL_Y)
	  #ALEX
	  if (node.X > -0.06 and node.X<0.06 and node.Y>0.0 and node.Y<0.003): #### elaf changed
	    node.Free(VELOCITY_X)
	...
	for node in lagrangian_model_part.Nodes:
	  if (node.GetSolutionStepValue(IS_BOUNDARY)!=0):
        ...
        CoupledEulerianUlfUtils.FindIntersectionOfEdges(lagrangian_model_part, eulerian_model_part, aux_model_part, interface_part)	
		      #(ModelPart& origin_model_part, ModelPart& destination_model_part, ModelPart& AuxModelPart, ModelPart& InterfacePart)
