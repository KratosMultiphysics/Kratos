...
import SurfaceTension_monolithic_solver_ptfe2 as solver_lagr
...
for node in lagrangian_model_part.Nodes:
  ...
  if (node.GetSolutionStepValue(IS_BOUNDARY) == 1.0 and node.GetSolutionStepValue(IS_WALL) == 0.0):
    ...
    if(step >= 3):
        for node in lagrangian_model_part.Nodes:
            ...
            if (node.GetSolutionStepValue(FLAG_VARIABLE) == 1.0):
                node.SetSolutionStepValue(IS_INTERFACE,0, 1.0)
            #const = 0.1
            #wight_force = (3.0/const)*time + 1.0
            #if (wight_force > 9.8):
                #wight_force =9.8
            #for node in lagrangian_model_part.Nodes:
