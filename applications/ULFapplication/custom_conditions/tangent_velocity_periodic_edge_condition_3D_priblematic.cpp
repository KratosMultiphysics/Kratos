...
namespace Kratos
{
...

	{
		...
		if(fabs(GetGeometry()[0].X()-GetGeometry()[1].X())<0.00001 && fabs(GetGeometry()[0].Y()-GetGeometry()[1].Y())<0.00001)
			{
			if (GetGeometry()[0].Y()>0.000001 && GetGeometry()[0].Y()<0.9999999)			//vertical (y-dir) walls of the back-frond faces // here vy is the same
				{		
				//y_component on the 
				temp_vector[0]=velocity0[1];
				temp_vector[1]=velocity1[1];			
			...
			else if (GetGeometry()[0].Z()<0.000001)			//horizontal low wall of the back face.. here ux is the same
				{		
				//x_component on the 
				temp_vector[0]=velocity0[0];
				temp_vector[1]=velocity1[0];			
	...
	void TangentVelocityPeriodicEdgeCondition3D2N::EquationIdVector(EquationIdVectorType& rResult, ProcessInfo& rCurrentProcessInfo)
	{
	    ...
	    if(fabs(GetGeometry()[0].X()-GetGeometry()[1].X())<0.00001 && fabs(GetGeometry()[0].Y()-GetGeometry()[1].Y())<0.00001)
		 ...
		 if (GetGeometry()[0].Y()>0.000001 && GetGeometry()[0].Y()<0.9999999)			
  		...
  		else if (GetGeometry()[0].Z()<0.00001) //this is the low back horizontak edge //the SECOND TANGENT multiplier is set here in the first condition.. here we prescribe equality of ux
			{
			//X
			//first node of the pair
			rResult[0]=(GetGeometry()[0].GetDof(VELOCITY_X).EquationId());
...
//UPPER - LOWER PAIR
	    ...
	    if(fabs(GetGeometry()[0].Y()-GetGeometry()[1].Y())<0.00001 && fabs(GetGeometry()[0].Z()-GetGeometry()[1].Z())<0.00001)
		 ...
		 //second node of teh pair
 		 ...
 		 if (GetGeometry()[0].Y()>0.000001 && GetGeometry()[0].Y()<0.9999999)			
		 	ConditionalDofList[2] = GetGeometry()[0].pGetDof(LAGRANGE_MULTIPLIER_TANGENT_VELOCITY);		//these are edges oriented in y-direction
	    ...
	    if(fabs(GetGeometry()[0].X()-GetGeometry()[1].X())<0.00001 && fabs(GetGeometry()[0].Y()-GetGeometry()[1].Y())<0.00001)
		 {		 
		 if (GetGeometry()[0].Y()>0.000001 && GetGeometry()[0].Y()<0.9999999)	//its correct! these are vertical wallas (y-direction) of the front-back faces
			{
			//first node of the pair
			ConditionalDofList[0] = GetGeometry()[0].pGetDof(VELOCITY_Y);
			//second node of teh pair
	  	        ConditionalDofList[1] = GetGeometry()[1].pGetDof(VELOCITY_Y);
		 	ConditionalDofList[2] = GetGeometry()[0].pGetDof(LAGRANGE_MULTIPLIER_SECOND_TANGENT_VELOCITY);// these are vertical edges (y-direction).. TANGENT multiplier is used in these edges 				in the above case // thus we use here SECOND_TANGENT-- HERe we prescribe equality of vy
			}
		  else if (GetGeometry()[0].Z()<0.00001) //this is the low back horizontak edge //the TANGENT multiplier is set here in the first condition.. here we prescribe equality of ux
			{
			//first node of the pair
			ConditionalDofList[0] = GetGeometry()[0].pGetDof(VELOCITY_X);
			//second node of teh pair
