...
#include "custom_conditions/second_tangent_velocity_periodic_vertex_condition_3D.h"
...
namespace Kratos
{
	//************************************************************************************
	//************************************************************************************
	SecondTangentVelocityPeriodicVertexCondition3D2N::SecondTangentVelocityPeriodicVertexCondition3D2N(IndexType NewId, GeometryType::Pointer pGeometry)
	...
	//************************************************************************************
	//************************************************************************************
	SecondTangentVelocityPeriodicVertexCondition3D2N::SecondTangentVelocityPeriodicVertexCondition3D2N(IndexType NewId, GeometryType::Pointer pGeometry,  PropertiesType::Pointer pProperties)
		: Condition(NewId, pGeometry, pProperties)
	{
	}
	Condition::Pointer SecondTangentVelocityPeriodicVertexCondition3D2N::Create(IndexType NewId, NodesArrayType const& ThisNodes,  PropertiesType::Pointer pProperties) const
	{
		return Condition::Pointer(new SecondTangentVelocityPeriodicVertexCondition3D2N(NewId, GetGeometry().Create(ThisNodes), pProperties));
	}

	SecondTangentVelocityPeriodicVertexCondition3D2N::~SecondTangentVelocityPeriodicVertexCondition3D2N()
	{
	}

	//************************************************************************************
	//************************************************************************************
	void SecondTangentVelocityPeriodicVertexCondition3D2N::CalculateRightHandSide(VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY
	...
	//************************************************************************************
	//************************************************************************************	
	void SecondTangentVelocityPeriodicVertexCondition3D2N::CalculateLocalVelocityContribution(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)

	{
		...
		if(fabs(GetGeometry()[0].X()-GetGeometry()[1].X())<0.00001 && fabs(GetGeometry()[0].Z()-GetGeometry()[1].Z())<0.00001)
			...
			temp_vector[2]=GetGeometry()[0].GetSolutionStepValue(LAGRANGE_MULTIPLIER_TANGENT_VELOCITY,0);	//we store it in the second node
			}
		...
		if(fabs(GetGeometry()[0].Y()-GetGeometry()[1].Y())<0.00001 && fabs(GetGeometry()[0].Z()-GetGeometry()[1].Z())<0.00001)
			{			
			//y_component
			KRATOS_WATCH("Here Z-vel")
			temp_vector[0]=velocity0[2];
			temp_vector[1]=velocity1[2];
			rRightHandSideVector[2] = -jump_vector[2];		//v1z*-v2z*=0	
		...
		if(fabs(GetGeometry()[0].X()-GetGeometry()[1].X())<0.00001 && fabs(GetGeometry()[0].Y()-GetGeometry()[1].Y())<0.00001)
			...
			temp_vector[2]=GetGeometry()[1].GetSolutionStepValue(LAGRANGE_MULTIPLIER_SECOND_TANGENT_VELOCITY,0);					
			}
	...
	void SecondTangentVelocityPeriodicVertexCondition3D2N::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	...
	//************************************************************************************
	//************************************************************************************
	void SecondTangentVelocityPeriodicVertexCondition3D2N::EquationIdVector(EquationIdVectorType& rResult, ProcessInfo& rCurrentProcessInfo)
	{
		 if (rResult.size()!=(3))
			    rResult.resize(3);
//KRATOS_WATCH("Im hereeeeeeeeeee22222222222")	    
		 ...
		 //TO FIND OUT WHICH OF THE TWO PAIRS THESE ARE: VERTICAL OR HORIZONTAL... 		 
		 rResult[2] = GetGeometry()[0].GetDof(LAGRANGE_MULTIPLIER_TANGENT_VELOCITY).EquationId(); //store LAG MULT DOF in in the first node
		 }
	    ...
	    if(fabs(GetGeometry()[0].X()-GetGeometry()[1].X())<0.00001 && fabs(GetGeometry()[0].Y()-GetGeometry()[1].Y())<0.00001)
		 ...
		 rResult[2] = GetGeometry()[1].GetDof(LAGRANGE_MULTIPLIER_SECOND_TANGENT_VELOCITY).EquationId();		//we store DOF in the second node
		 }
	}

	//************************************************************************************
	//************************************************************************************
	  void SecondTangentVelocityPeriodicVertexCondition3D2N::GetDofList(DofsVectorType& ConditionalDofList,ProcessInfo& rCurrentProcessInfo)
	{
...
//UPPER - LOWER PAIR
	 	...
	 	ConditionalDofList[2] = GetGeometry()[0].pGetDof(LAGRANGE_MULTIPLIER_TANGENT_VELOCITY); //we store LAG dof in the first node
		 }
	    ...
	    if(fabs(GetGeometry()[0].X()-GetGeometry()[1].X())<0.00001 && fabs(GetGeometry()[0].Y()-GetGeometry()[1].Y())<0.00001)
		 ...
		 ConditionalDofList[2] = GetGeometry()[1].pGetDof(LAGRANGE_MULTIPLIER_SECOND_TANGENT_VELOCITY);	//we store LAG dof in the second node
