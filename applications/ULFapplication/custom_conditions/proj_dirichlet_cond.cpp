    ...
    : Condition(NewId, pGeometry, pProperties)
{
    ...
    for (unsigned int i=0; i<GetGeometry().size(); i++)
    {
        ...
        {
	    //GetGeometry()[i].FastGetSolutionStepValue(Hox)=250.0;
	    ...
	    {		
		//KRATOS_WATCH("====================================================")
		//KRATOS_WATCH(GetGeometry()[i].FastGetSolutionStepValue(MATERIAL_VARIABLE))
		//KRATOS_WATCH(i)
		//KRATOS_WATCH(bad_vertex_index1)
		//KRATOS_WATCH(bad_vertex_index2)
		...
		//GetGeometry()[i].FastGetSolutionStepValue(Hox)=11.0;
		
            }
            else if (i==bad_vertex_index1 && i!=bad_vertex_index2)
            {
                //KRATOS_WATCH("Case1")
		...
		//GetGeometry()[i].FastGetSolutionStepValue(Hox)=10.0;

                GetGeometry()[i].Fix(AUX_VEL_X);
                GetGeometry()[i].Fix(AUX_VEL_Y);
		// IN THE BAD REAL NODES FIX ALSO THE VELOCITY - NOT TO SOLVE THERE!!!!
		GetGeometry()[i].FastGetSolutionStepValue(VELOCITY_X)=mVel1[0];
                GetGeometry()[i].FastGetSolutionStepValue(VELOCITY_Y)=mVel1[1];
		GetGeometry()[i].Fix(VELOCITY_X);
                GetGeometry()[i].Fix(VELOCITY_Y);
		//GetGeometry()[i].FastGetSolutionStepValue(Hox)=5.0;

            }
            else if (i==bad_vertex_index2 && i!=bad_vertex_index1)
            {
                //KRATOS_WATCH("Case2")
                //GetGeometry()[i].FastGetSolutionStepValue(IS_INTERFACE)=100.0;
		...
		//GetGeometry()[i].FastGetSolutionStepValue(Hox)=10.0;
                ...
                GetGeometry()[i].Fix(AUX_VEL_Y);
		// IN THE BAD REAL NODES FIX ALSO THE VELOCITY - NOT TO SOLVE THERE!!!!
		GetGeometry()[i].FastGetSolutionStepValue(VELOCITY_X)=mVel2[0];
                GetGeometry()[i].FastGetSolutionStepValue(VELOCITY_Y)=mVel2[1];
		GetGeometry()[i].Fix(VELOCITY_X);
                GetGeometry()[i].Fix(VELOCITY_Y);
		//GetGeometry()[i].FastGetSolutionStepValue(Hox)=15.0;

            }
	    //if the two bad intersections are close to one vertex
	    else if (i==bad_vertex_index1 && i==bad_vertex_index2)
		{
		GetGeometry()[i].FastGetSolutionStepValue(AUX_VEL_X)=(mVel1[0]+mVel2[0])/2.0;
                GetGeometry()[i].FastGetSolutionStepValue(AUX_VEL_Y)=(mVel1[1]+mVel2[1])/2.0;
		 GetGeometry()[i].Fix(AUX_VEL_X);
                GetGeometry()[i].Fix(AUX_VEL_Y);
		// IN THE BAD REAL NODES FIX ALSO THE VELOCITY - NOT TO SOLVE THERE!!!!
		GetGeometry()[i].FastGetSolutionStepValue(VELOCITY_X)=(mVel1[0]+mVel2[0])/2.0;
                GetGeometry()[i].FastGetSolutionStepValue(VELOCITY_Y)=(mVel1[1]+mVel2[1])/2.0;
		GetGeometry()[i].Fix(VELOCITY_X);
                GetGeometry()[i].Fix(VELOCITY_Y);
		//GetGeometry()[i].FastGetSolutionStepValue(Hox)=33.0; 
		}
	    else
		{
		KRATOS_WATCH("=====OTRO===============================================")
		//KRATOS_WATCH(GetGeometry()[i].FastGetSolutionStepValue(Hox))
		KRATOS_WATCH(i)
		KRATOS_WATCH(bad_vertex_index1)
		KRATOS_WATCH(bad_vertex_index2)
		KRATOS_THROW_ERROR(std::logic_error,"SHOULD NOT ENTER HERE...","");
		...
		GetGeometry()[i].Fix(AUX_VEL_X);
		GetGeometry()[i].Fix(AUX_VEL_Y);
		//GetGeometry()[i].FastGetSolutionStepValue(Hox)=25.0;		
		}
		...
		GetGeometry()[i].Fix(AUX_VEL_X);
		GetGeometry()[i].Fix(AUX_VEL_Y);
		//GetGeometry()[i].FastGetSolutionStepValue(Hox)=26.0;		
		}

		else if (i==bad_vertex_index1 && i==bad_vertex_index2)
		{
		GetGeometry()[i].FastGetSolutionStepValue(AUX_VEL_X)=0.5*(mVel2[0]+mVel1[0]);
		GetGeometry()[i].FastGetSolutionStepValue(AUX_VEL_Y)=0.5*(mVel2[1]+mVel1[1]);
		...
		// AND IF THE CUT IS FINE - dont do anything
		else 
		{
		...
		//GetGeometry()[i].FastGetSolutionStepValue(Hox)=4.0;	
		/*KRATOS_WATCH("SHLD BE GOOD CUT")
		KRATOS_WATCH(i)
		KRATOS_WATCH(bad_vertex_index1)
		KRATOS_WATCH(bad_vertex_index2)*/							
		}
	...
	//if (i!=bad_vertex_index1 || i!=bad_vertex_index2)
		...
		//GetGeometry()[i].FastGetSolutionStepValue(MATERIAL_VARIABLE)=25.0;		
...
/*		GetGeometry()[i].FastGetSolutionStepValue(AUX_VEL_X)=0.0;
            GetGeometry()[i].FastGetSolutionStepValue(AUX_VEL_Y)=0.0;
            GetGeometry()[i].FastGetSolutionStepValue(FRACT_VEL_X)=0.0;
            GetGeometry()[i].FastGetSolutionStepValue(FRACT_VEL_Y)=0.0;
            GetGeometry()[i].FastGetSolutionStepValue(VELOCITY_X)=0.0;
            GetGeometry()[i].FastGetSolutionStepValue(VELOCITY_Y)=0.0;*/
	  //  GetGeometry()[i].FastGetSolutionStepValue(MATERIAL_VARIABLE)=35.0;
	    //GetGeometry()[i].FastGetSolutionStepValue(MATERIAL_VARIABLE)=5.0;
	//	}
        }
...
/*    if (interf_length<0.0000000001)
    ...
    if (interf_length<0.0)
        KRATOS_THROW_ERROR(std::logic_error,"negative intersection length!!!!","");

	//GRAN CHAPUZA!!!!!!!!!!!!!!!!!!!!
	if (interf_length<0.000000000000001)
		interf_length=0.0001;
