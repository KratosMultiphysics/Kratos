...
// //                 gamma = 0.07275*(1.0-0.00212*(surface_temp-293.15));
                surface_temp = this->GetGeometry()[i].FastGetSolutionStepValue(TEMPERATURE);
                gamma = this->GetGeometry()[i].FastGetSolutionStepValue(SURFACE_TENSION_COEF);
            }
...
//         
	...
	//adding dissipative_forces varialbes
        ...
        double zeta_dissapative_JM_y = rCurrentProcessInfo[DISSIPATIVE_FORCE_COEFF_JM_Y];
        double zeta_dissapative_BM_y = rCurrentProcessInfo[DISSIPATIVE_FORCE_COEFF_BM_Y];
        double zeta_dissapative_SM_y = rCurrentProcessInfo[DISSIPATIVE_FORCE_COEFF_SM_Y];
...
//          
		  ...
		  double v_cly_abs = fabs (v_cly) ;
                  ...
                  rRightHandSideVector[3*ii]	        -= zeta_dissapative_JM_x*gamma*tanh(4.96 * pow(cap_x,0.702)); 
                  rRightHandSideVector[3*ii+1]	        -= zeta_dissapative_JM_y*gamma*tanh(4.96 * pow(cap_y,0.702)); 
//                 
//                   // using Bracke's model : gamma 2.24 ca ^(0.54)
                  rRightHandSideVector[3*ii]	        -= zeta_dissapative_BM_x*gamma* 2.24 * pow(cap_x,0.54); 
                  rRightHandSideVector[3*ii+1]	        -= zeta_dissapative_BM_y*gamma* 2.24 * pow(cap_y,0.54); 
//                 
//                   // using Seeberg's model : gamm 2.24 ca ^(0.54) for Ca > 10^(-3), otherwise, 4.47 Ca^(0.42)
                  double cap = sqrt((cap_x * cap_x) + (cap_y * cap_y));
                  if (cap > 0.01)
                  {
                    rRightHandSideVector[3*ii]	        -= zeta_dissapative_SM_x*gamma* 2.24 * pow(cap_x,0.54); 
                    rRightHandSideVector[3*ii+1]        -= zeta_dissapative_SM_y*gamma* 2.24 * pow(cap_y,0.54);  
                  }
                  else
                  {
                    rRightHandSideVector[3*ii]	        -= zeta_dissapative_SM_x*gamma* 4.47 * pow(cap_x,0.42); 
                    rRightHandSideVector[3*ii+1]        -= zeta_dissapative_SM_y*gamma* 4.47 * pow(cap_y,0.42);  
                  }
...
// 	    
	...
	double theta_s = rCurrentProcessInfo[CONTACT_ANGLE_STATIC];
        ...
        double zeta_dissapative_BM_y = rCurrentProcessInfo[DISSIPATIVE_FORCE_COEFF_BM_Y];
        double zeta_dissapative_SM_y = rCurrentProcessInfo[DISSIPATIVE_FORCE_COEFF_SM_Y];
        double zeta_dissapative_JM_z = rCurrentProcessInfo[DISSIPATIVE_FORCE_COEFF_JM_Z];
...
// 	    this->GetGeometry()[ii].FastGetSolutionStepValue(FORCE) = -coef_i*beta*fsign1*(gamma*m1*nlen1);
            ...
            //elaf replaced in 2342 with 2347-2349
            
            this->GetGeometry()[ii].FastGetSolutionStepValue(FORCE_X) = -coef_i*beta*fsign1*(gamma*m1[0]*nlen1);
	    this->GetGeometry()[ii].FastGetSolutionStepValue(FORCE_Y) = -coef_i*beta*fsign1*(gamma*m1[1]*nlen1);
	    this->GetGeometry()[ii].FastGetSolutionStepValue(FORCE_Z) = -coef_i*beta*fsign1*(gamma*m1[2]*nlen1);
	    ...
	    double v_clx = this->GetGeometry()[ii].FastGetSolutionStepValue(VELOCITY_X);
	    double v_cly = this->GetGeometry()[ii].FastGetSolutionStepValue(VELOCITY_Y);
	    double v_clz = this->GetGeometry()[ii].FastGetSolutionStepValue(VELOCITY_Z);
//          
	    double mu, rho;
	    mu  = this->GetGeometry()[ii].FastGetSolutionStepValue(VISCOSITY);
...
// 	    this->GetGeometry()[jj].FastGetSolutionStepValue(FORCE) = -coef_j*beta*fsign2*(gamma*m2*nlen2);// + this->GetGeometry()[jj].FastGetSolutionStepValue(ADHESION_FORCE_X));
            ...
            //elaf changed line 2422 by 2425-2427
	    this->GetGeometry()[jj].FastGetSolutionStepValue(FORCE_X) = -coef_j*beta*fsign2*(gamma*m2[0]*nlen2);
	    this->GetGeometry()[jj].FastGetSolutionStepValue(FORCE_Y) = -coef_j*beta*fsign2*(gamma*m2[1]*nlen2);
	    this->GetGeometry()[jj].FastGetSolutionStepValue(FORCE_Z) = -coef_j*beta*fsign2*(gamma*m2[2]*nlen2);
            
            
            //start of adding dissipative force: where v_clx here is the x_velocity at the contact line; and v_cly is the y_velocity at the contact line
	    double v_clx = this->GetGeometry()[jj].FastGetSolutionStepValue(VELOCITY_X);
...
// 	  beta = 1.0;
	  ...
	  {    
	    ...
	    //start of adding dissipative force: where v_clx here is the x_velocity at the contact line; and v_cly is the y_velocity at the contact line
	    double v_clx = this->GetGeometry()[kk].FastGetSolutionStepValue(VELOCITY_X);
	    double v_cly = this->GetGeometry()[kk].FastGetSolutionStepValue(VELOCITY_Y);
	    double v_clz = this->GetGeometry()[kk].FastGetSolutionStepValue(VELOCITY_Z);
//          
	    double mu, rho;
...
// 	    beta = 1.0;
	    ...
	    {    
	      ...
	      //start of adding dissipative force: where v_clx here is the x_velocity at the contact line; and v_cly is the y_velocity at the contact line
	      double v_clx = this->GetGeometry()[ll].FastGetSolutionStepValue(VELOCITY_X);
	      double v_cly = this->GetGeometry()[ll].FastGetSolutionStepValue(VELOCITY_Y);
	      double v_clz = this->GetGeometry()[ll].FastGetSolutionStepValue(VELOCITY_Z);
  //          
	      double mu, rho;
