...
{
    KRATOS_TRY
KRATOS_WATCH("Should not enter here... CalcRHS is empty for this element")
KRATOS_CATCH("")
}
...
*/
	KRATOS_WATCH("MassMatrix calculate - empty function here")
    KRATOS_CATCH("")
}
...
void HypoElasticSolid2D::Stage1(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    if (rLeftHandSideMatrix.size1() != 6)
        rLeftHandSideMatrix.resize(6, 6, false);
  ...
  if(rRightHandSideVector.size() != 6)
    ...
    BoundedMatrix<double,6,6> MassMatrix=ZeroMatrix(6, 6);
...
KRATOS_WATCH(density)
    ...
    ms_temp = prod( ms_constitutive_matrix , msB);
    //ms_temp*=dt;
    rLeftHandSideMatrix+= 0.5 * prod( trans(msB) , ms_temp);

    //Now calculate an additional contribution to the residual: r -= rDampingMatrix * (v)
    array_1d< double, 6 > Vel;

    Vel[0] = this->GetGeometry()[0].FastGetSolutionStepValue(VELOCITY_X,1);
    Vel[1] = this->GetGeometry()[0].FastGetSolutionStepValue(VELOCITY_Y,1);
    Vel[2] = this->GetGeometry()[1].FastGetSolutionStepValue(VELOCITY_X,1);
    Vel[3] = this->GetGeometry()[1].FastGetSolutionStepValue(VELOCITY_Y,1);
    Vel[4] = this->GetGeometry()[2].FastGetSolutionStepValue(VELOCITY_X,1);
    Vel[5] = this->GetGeometry()[2].FastGetSolutionStepValue(VELOCITY_Y,1);
/*

    noalias(rRightHandSideVector) -= prod(rLeftHandSideMatrix, Vel);
...
*/
    for(unsigned int i=0; i<3; i++)
    {
        for(unsigned int j=0; j<2; j++)
        {
            ...
            MassMatrix(index,index) =  density * 0.333333333333333333/dt;
        }
    }

    noalias(rLeftHandSideMatrix)+=MassMatrix ;


    noalias(rRightHandSideVector) += prod(MassMatrix, Vel);
...
KRATOS_WATCH(body_force[1])
    ...
    const BoundedMatrix<double,2,2> & CauchyStress=0.5 * this->GetValue(CAUCHY_STRESS_TENSOR);
...
KRATOS_WATCH(rRightHandSideVector)
    ...
    KRATOS_WATCH("End of stage1 for hypoelastic")
    KRATOS_WATCH(rRightHandSideVector)
...
void HypoElasticSolid2D::InitializeSolutionStep(ProcessInfo& rCurrentProcessInfo)
{
    ...
    GetGeometry()[0].FastGetSolutionStepValue(IS_STRUCTURE)=1.0;
    GetGeometry()[1].FastGetSolutionStepValue(IS_STRUCTURE)=1.0;
    GetGeometry()[2].FastGetSolutionStepValue(IS_STRUCTURE)=1.0;

    KRATOS_WATCH("INItialziing HypoeEl structure...")
    KRATOS_CATCH("");
}
...
{
    //KRATOS_WATCH("Setting is_structure and structure in solid elements")
    if(rVariable == IS_STRUCTURE)
    {
    GetGeometry()[0].Set(STRUCTURE, true);
    GetGeometry()[1].Set(STRUCTURE, true);
    GetGeometry()[2].Set(STRUCTURE, true);
