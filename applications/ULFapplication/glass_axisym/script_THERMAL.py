...
for node in fluid_model_part.Nodes:
    ...
    node.SetSolutionStepValue(BODY_FORCE_Y,0, -9.8)   
      ...
      #node.SetSolutionStepValue(TEMPERATURE, 500)
...
#temperature_model_part.Properties[0].SetValue(EMISSIVITY,2.0);
#temperature_model_part.Properties[0].SetValue(AMBIENT_TEMPERATURE,298.0);
#temperature_model_part.Properties[0].SetValue(CONVECTION_COEFFICIENT,1.0);

#temperature_model_part.Properties[1].SetValue(EMISSIVITY,2.0);
#temperature_model_part.Properties[1].SetValue(AMBIENT_TEMPERATURE,298.0);
#temperature_model_part.Properties[1].SetValue(CONVECTION_COEFFICIENT,1.0);
...
for node in temperature_model_part.Nodes:
    ...
    #node.SetSolutionStepValue(DENSITY,0,density) 
    node.SetSolutionStepValue(CONDUCTIVITY, 0, 2.00) 
    node.SetSolutionStepValue(SPECIFIC_HEAT,0, 700.0)
    node.Free(TEMPERATURE)
    if(node.GetSolutionStepValue(IS_STRUCTURE)==1.0):
        node.SetSolutionStepValue(TEMPERATURE,0,750.0)
        node.Fix(TEMPERATURE)
    else:
        node.SetSolutionStepValue(TEMPERATURE,0,950.0)
    #if(node.Y<-0.01):
...
##              node.SetSolutionStepValue(VISCOSITY, 0, 40.0)
	  ...
	  #for node in fluid_model_part.Nodes: 
	    #if (node.Y<0.013):	 	      
	      #node.SetSolutionStepValue(TEMPERATURE,400.0)
	      #node.Fix(TEMPERATURE)
	      #node.SetSolutionStepValue(VISCOSITY, 0, 40.0)
          #    node.SetSolutionStepValue(BODY_FORCE_Y, 0, 9.8)
	...
	if (time>1.4):
	  for node in temperature_model_part.Nodes:
	    if(node.GetSolutionStepValue(IS_STRUCTURE)==1.0):
	      node.SetSolutionStepValue(TEMPERATURE,600.0)
	      node.Fix(TEMPERATURE)
        #    if(node.GetSolutionStepValue(IS_STRUCTURE)==1.0):
        #        node.SetSolutionStepValue(FACE_HEAT_FLUX,0,-100000.0)
