...
public:
    ...
    virtual void SetUpSystem(ModelPart &r_model_part)
    {
        ...
        for (typename DofsArrayType::iterator itDof = BaseType::mDofSet.begin(); itDof != BaseType::mDofSet.end(); ++itDof)
        {
			...
			else if(itDof->GetVariable()==VELOCITY_Z)
			{
				if ( ( itDof->GetSolutionStepValue(NODE_PAIR_Z_COMPONENT) < static_cast<int>(itDof->Id()) && itDof->GetSolutionStepValue(NODE_PAIR_Z_COMPONENT_ANTIPERIODIC)==0 ) || ( itDof->GetSolutionStepValue(NODE_PAIR_Z_COMPONENT_ANTIPERIODIC) < static_cast<int>(itDof->Id()) &&  itDof->GetSolutionStepValue(NODE_PAIR_Z_COMPONENT)==0 ) )
					itDof->SetEquationId(EqId++);
			}
        ...
        {
            // PeriodicCondition always has exactly 2 nodes
            ...
            if (rGeom.PointsNumber() == 2)
            {
                ...
                const int Node0Pair_z = rGeom[0].FastGetSolutionStepValue(NODE_PAIR_Z_COMPONENT);
		...
		const int Node0Pair_z_antiperiodic = rGeom[0].FastGetSolutionStepValue(NODE_PAIR_Z_COMPONENT_ANTIPERIODIC);
                ...
                const int Node1Pair_z = rGeom[1].FastGetSolutionStepValue(NODE_PAIR_Z_COMPONENT);
		...
		const int Node1Pair_x_antiperiodic = rGeom[1].FastGetSolutionStepValue(NODE_PAIR_X_COMPONENT_ANTIPERIODIC);
                ...
                const int Node1Pair_z_antiperiodic = rGeom[1].FastGetSolutionStepValue(NODE_PAIR_Z_COMPONENT_ANTIPERIODIC);
                ...
                //Z periodicity
                if ( ( Node0 == Node1Pair_z ) && ( Node1 == Node0Pair_z ) )
                {
                    if ( Node0 < Node0Pair_z ) // If Node0 is the one with lower Id (the one that does not have an EquationId yet)
                        CopyEquationId_z(rGeom[1],rGeom[0]);
                    else
                        CopyEquationId_z(rGeom[0],rGeom[1]);
                }

                 //Z anti-periodicity
                if ( ( Node0 == Node1Pair_z_antiperiodic ) && ( Node1 == Node0Pair_z_antiperiodic ) )
                {
                    if ( Node0 < Node0Pair_z_antiperiodic ) // If Node0 is the one with lower Id (the one that does not have an EquationId yet)
                        CopyEquationId_z(rGeom[1],rGeom[0]);
                    else
                        CopyEquationId_z(rGeom[0],rGeom[1]);
                }
...
private:
    ...
    void CopyEquationId_z(ModelPart::NodeType& rOrigin,
                        ModelPart::NodeType& rDest)
    {
			//copying velocity dof of correct component
			rDest.pGetDof( VELOCITY_Z )->SetEquationId(  rOrigin.pGetDof(VELOCITY_Z)->EquationId() );
			//if we are not a corner node, we also copy the pressure dof
            //if(rDest.FastGetSolutionStepValue(NODE_PAIR_X_COMPONENT)==0)
			//	rDest.pGetDof( PRESSURE )->SetEquationId( PRESSURE );			
    }
