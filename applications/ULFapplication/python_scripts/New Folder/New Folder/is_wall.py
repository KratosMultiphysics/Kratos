...
#
    ...
    def vel(self):
        for node in self.model_part.Nodes:
            ...
            d_cl[1] = 0.0
            d_cl[2] = 0.0
            d_cl[0] = node.GetSolutionStepValue(VELOCITY_X,0)
            d_cl[1] = node.GetSolutionStepValue(VELOCITY_Y,0)
            d_cl[2] = node.GetSolutionStepValue(VELOCITY_Z,0)
            ...
            if (node.GetSolutionStepValue(TRIPLE_POINT) != 0.0):
              if ((node.GetSolutionStepValue(CONTACT_ANGLE) > theta_adv) or (node.GetSolutionStepValue(CONTACT_ANGLE) < theta_rec)):
                ...
                #dis_vf_cl[1] = 0.0
                #dis_vf_cl[2] = 0.0
                #dis_vf_cl[0] = node.GetSolutionStepValue(VELOCITY_X,0)
                #dis_vf_cl[1] = node.GetSolutionStepValue(VELOCITY_Y,0)
                #dis_vf_cl[2] = node.GetSolutionStepValue(VELOCITY_Z,0)
                ...
                #elif (dis_vf_cl[0] >= 0.0):
                    #zeta_dissapative_JM_x1 =  1.0 
                    
                #if (dis_vf_cl[1] < 0.0):
                    #zeta_dissapative_JM_y1 = - 1.0 
                #elif (dis_vf_cl[1] >= 0.0):
                    #zeta_dissapative_JM_y1 = 1.0
                #mu  = 8.90 * 1e-4
                #gamma =  0.072
                ##capillary
                #v_clx_abs = abs (dis_vf_cl[0])
                #v_cly_abs = abs (dis_vf_cl[1])
                ...
                #dis_vf_cl[0] = gamma * 2.24 * pow(cap_x, 0.54) * zeta_dissapative_JM_x1 * node.GetSolutionStepValue(NODAL_LENGTH,0)/2.0
                #dis_vf_cl[1] = gamma * 2.24 * pow(cap_y, 0.54) * zeta_dissapative_JM_y1 * node.GetSolutionStepValue(NODAL_LENGTH,0)/2.0
                ...
                #sof_cl = Vector(3)
                #sof_cl[0] = f_cl[0] - dis_vf_cl[0]
                #sof_cl[1] = f_cl[1] - dis_vf_cl[1]
                ...
                #v_x =  dis_vf_cl[0] * time / (h/2.0 * 1000.0) 
                #v_y =  dis_vf_cl[1] * time / (h/2.0 * 1000.0)
                ...
                ########here we fix our b.c and edges as needed 
                #######if (node.GetSolutionStepValue(IS_WALL) == 1.0):
                    #######v_cl[0] = 0
                    #######d_cl[0] = 0
                    #######node.SetSolutionStepValue(DISPLACEMENT_X,0, d_cl[0])
            ...
            ######if ((node.GetSolutionStepValue(IS_WALL) == 2.0 or node.GetSolutionStepValue(IS_WALL) == 1.0) and node.GetSolutionStepValue(TRIPLE_POINT) == 1.0):
                ######node.Free(DISPLACEMENT_X)
                ######node.Free(DISPLACEMENT_Y)
                ######node.Free(VELOCITY_X)
                ######node.Free(VELOCITY_Y)
