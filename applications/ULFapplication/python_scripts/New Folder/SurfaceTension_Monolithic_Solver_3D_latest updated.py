...
#
    ...
    def vel(self):
        for node in self.model_part.Nodes:
            ...
            ##if(node.GetSolutionStepValue(IS_WALL) == 1.0):
                    ##d_x = node.GetSolutionStepValue(DISPLACEMENT_X,0)
                    ##d_x = 0.0
                    ##v_x = node.GetSolutionStepValue(VELOCITY_X,0)
                    ##v_x = 0.0
                    ##node.SetSolutionStepValue(DISPLACEMENT_X,0, d_x)
                    ##node.SetSolutionStepValue(VELOCITY_X,0, v_x)
                    ...
                    ##node.SetSolutionStepValue(DISPLACEMENT_X,0, d_x)
                    ##node.SetSolutionStepValue(VELOCITY_X,0, v_x)
                    ##d_y = node.GetSolutionStepValue(DISPLACEMENT_Y,0)
                    ##d_y = 0.0
                    ##v_y = node.GetSolutionStepValue(VELOCITY_Y,0)
                    ##v_y = 0.0
                    ##node.SetSolutionStepValue(DISPLACEMENT_Y,0, d_y)
                    ##node.SetSolutionStepValue(VELOCITY_Y,0, v_y)
                    ##d_z = node.GetSolutionStepValue(DISPLACEMENT_Z,0)
                    ##d_z = 0.0
                    ##v_x = node.GetSolutionStepValue(VELOCITY_Z,0)
                    ##v_z = 0.0
                    ##node.SetSolutionStepValue(DISPLACEMENT_Z,0, d_z)
                    ##node.SetSolutionStepValue(VELOCITY_Z,0, v_z)
                    ##node.Fix(DISPLACEMENT_X)
                    ##node.Fix(VELOCITY_X)
                ...
                ## flag variable = 3.0 for (perpendicular to the y, and on the xz plane)
                ###if(node.GetSolutionStepValue(FLAG_VARIABLE) == 3.0):
            #if(node.GetSolutionStepValue(IS_WALL) == 2.0):
                    #d_y = node.GetSolutionStepValue(DISPLACEMENT_Y,0)
                    #d_y = 0.0
                    #v_y = node.GetSolutionStepValue(VELOCITY_Y,0)
                    #v_y = 0.0
            ...
            #if(node.GetSolutionStepValue(IS_WALL) == 3.0):
                    #d_x = node.GetSolutionStepValue(DISPLACEMENT_X,0)
                    #d_x = 0.0
                    #v_x = node.GetSolutionStepValue(VELOCITY_X,0)
                    #v_x = 0.0
            ...
            if (node.GetSolutionStepValue(TRIPLE_POINT) != 0.0):
              if ((node.GetSolutionStepValue(CONTACT_ANGLE) > theta_adv) or (node.GetSolutionStepValue(CONTACT_ANGLE) < theta_rec)):
                ...
                #cap_y   =  mu *  v_cly_abs / gamma;
                #cap_z   =  mu *  v_clz_abs / gamma;
                #v_cl[0] = gamma * 2.24 * pow(cap_x, 0.54) * zeta_dissapative_JM_x
                #v_cl[1] = gamma * 2.24 * pow(cap_y, 0.54) * zeta_dissapative_JM_y 
                #v_cl[2] = gamma * 2.24 * pow(cap_z, 0.54) * zeta_dissapative_JM_z 
                #v_cl[2] = 0.0
                ##print("in the solve v_clx_2",  v_cl[0])
                ##print("in the solve v_cly_2",  v_cl[1])
                ##print("in the solve v_clz_2",  v_cl[2])
                ##using Jiang's Model : gamma tanh(4.96 Ca^(0.702))
                ...
                ## flag variable = 2.0 for (perpendicular to the x, and on the yz plane)
	...
	################## For sessile drop examples
        ...
        for node in self.model_part.Nodes:
                ...
                if (node.GetSolutionStepValue(TRIPLE_POINT) != 0.0):
                        ...
                        if (node.GetSolutionStepValue(IS_BOUNDARY) != 1.0 and node.GetSolutionStepValue(IS_STRUCTURE)!= 0.0):
                            node.Set(TO_ERASE,True)
                if (node.GetSolutionStepValue(IS_STRUCTURE) != 0.0 and node.GetSolutionStepValue(IS_BOUNDARY) != 0.0 and node.Z > -1e-5):
                    node.Set(TO_ERASE,False)
