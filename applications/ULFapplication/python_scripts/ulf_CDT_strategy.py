...
class ULFFracStrategyPython:
    #######################################################################
    def __init__(self, model_part, disp_time_scheme, convergence_criteria,ReformDofSetAtEachStep,MoveMeshFlag,domain_size, bulk_modulus, density):
        ...
        self.pres_linear_solver =  CGSolver(1e-4, 1000)#SkylineLUFactorizationSolver()
	...
	#self.builder_and_solver.MarkNewGlassContactNode(self.model_part)
	...
	inverted_elements = False
	  ...
	  #node.Free(PRESSURE)
        ...
        #for node in self.model_part.Nodes:
	  #if (node.GetSolutionStepValue(SYMMETRY_CUT)==1.0):
	...
	CalculateReactionsFlag=False
        #finalize the solution step
        self.FinalizeSolutionStep(CalculateReactionsFlag)
