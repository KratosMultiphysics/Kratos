...
class ULF_FSISolver:
   ...
   #  This is done to make switching off/on of remeshing easier
	...
	#self.UlfUtils.MarkNodesCloseToWall(self.fluid_model_part, self.domain_size, 2.5000)
	  ...
	  h_factor=0.25
	  (self.Mesher).ReGenerateMesh("UlfFrac2DSwimming","Condition2D", self.fluid_model_part, self.node_erase_process, True, self.add_nodes, self.alpha_shape, h_factor)
	elif (self.domain_size == 3):
	  (self.Mesher).ReGenerateMesh("UlfFrac3DSwimming","Condition3D", self.fluid_model_part, self.node_erase_process, True, self.add_nodes, self.alpha_shape, h_factor)
