...
#
    ...
    def cont_angle_cond(self):
        ...
        for node in self.model_part.Nodes:
            if (node.GetSolutionStepValue(IS_STRUCTURE) != 0.0 and node.GetSolutionStepValue(TRIPLE_POINT) != 1.0):
            ...
            #if (time >= 100*dt):
            #if (node.GetSolutionStepValue(TRIPLE_POINT) != 0.0 and (node.GetSolutionStepValue(CONTACT_ANGLE) < 75.0 and node.GetSolutionStepValue(CONTACT_ANGLE) > 75.0)):
                #dtp = Vector(2)
                #dtp[1] = node.GetSolutionStepValue(DISPLACEMENT_Y)
                #vtp = Vector(2)
                #vtp[0] = node.GetSolutionStepValue(VELOCITY_X)
                ...
                #node.Fix(VELOCITY_X)
                #node.Fix(VELOCITY_Y)
            #if (node.GetSolutionStepValue(TRIPLE_POINT) != 0.0 and (node.GetSolutionStepValue(CONTACT_ANGLE) > 108.0 or node.GetSolutionStepValue(CONTACT_ANGLE) < 108.0)):
            if (node.GetSolutionStepValue(TRIPLE_POINT) != 0.0 and (node.GetSolutionStepValue(CONTACT_ANGLE) > 74.0 or node.GetSolutionStepValue(CONTACT_ANGLE) < 74.0)):
                ...
                vtp[1]=0.0
                node.SetSolutionStepValue(VELOCITY_Y,0, vtp[1])
                #if(node.GetSolutionStepValue(CONTACT_ANGLE,0) < 64.0 and vtp[0] < 0.0):
                    ##node.Set(TO_ERASE,True)
                    ...
                    #node.SetSolutionStepValue(DISPLACEMENT_Y,0,dtp[1])
            #def b(a):
                #x_min = min(node.X for TRIPLE_POINT in self.model_part.Nodes)
                #a = x_min
            #return a
