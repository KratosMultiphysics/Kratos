...
// 		}
	...
	for(ModelPart::NodesContainerType::iterator im = ThisModelPart.NodesBegin() ; im != ThisModelPart.NodesEnd() ; im++)
	{
		...
		array_1d<double,3> rik = ZeroVector(3);
                ...
                array_1d<double,3> rkj = ZeroVector(3);
                
		array_1d<double,3> cross_prod_ijk = ZeroVector(3);
                
                array_1d<double,3> cross_prod_jik = ZeroVector(3);
                array_1d<double,3> cross_prod_kij = ZeroVector(3);
                array_1d<double,3> cross_prod_jki = ZeroVector(3);
                
                
		array_1d<double,3> normal_geom = ZeroVector(3);
		array_1d<double,3> normal_tp = ZeroVector(3);
                array_1d<double,3> normal_tp_j = ZeroVector(3);
                array_1d<double,3> normal_tp_k = ZeroVector(3);
		normal_geom = im->FastGetSolutionStepValue(NORMAL_GEOMETRIC);
		double dot_prod = 0.0;
                double dot_prod_j = 0.0;
                double dot_prod_k = 0.0;
...
//  			  if (visited == 0)
		  ...
		  CrossProduct3D(rij, rik, cross_prod_ijk);
                  ...
                  CrossProduct3D(rki, rkj, cross_prod_kij);
                  CrossProduct3D(rjk, rji, cross_prod_jki);
                  
		  dot_prod = DotProduct3D(cross_prod_ijk,normal_geom);
                  
                  dot_prod_j  = DotProduct3D(cross_prod_kij,normal_geom);
                  dot_prod_k  = DotProduct3D(cross_prod_jki,normal_geom);
                  
		  if (dot_prod < 0.0)
                  {
		      CrossProduct3D(rik, rji, cross_prod_ijk);
                  
                      CrossProduct3D(rkj, rki, cross_prod_kij);
                      CrossProduct3D(rji, rjk, cross_prod_jki);
                  }
		  ...
		  normal_tp[2] += cross_prod_ijk[2];
                  ...
                  normal_tp_j[0] += cross_prod_kij[0];
		  normal_tp_j[1] += cross_prod_kij[1];
		  normal_tp_j[2] += cross_prod_kij[2];
                  
                  normal_tp_k[0] += cross_prod_jki[0];
		  normal_tp_k[1] += cross_prod_jki[1];
		  normal_tp_k[2] += cross_prod_jki[2];
		  ...
		  NormalizeVec3D(cross_prod_ijk);
                  NormalizeVec3D(cross_prod_kij);
                  NormalizeVec3D(cross_prod_jki);
		  theta = (acos(cross_prod_ijk[2])*180.0/pi + acos(cross_prod_kij[1])*180.0/pi + acos(cross_prod_jki[0])*180.0/pi)/3.0;
		  im->FastGetSolutionStepValue(CONTACT_ANGLE) += theta;
		...
		im->FastGetSolutionStepValue(NORMAL_TRIPLE_POINT_Z) = normal_tp[2];
                ...
                im->FastGetSolutionStepValue(NORMAL_TRIPLE_POINT_X) = normal_tp_j[0];
		im->FastGetSolutionStepValue(NORMAL_TRIPLE_POINT_Y) = normal_tp_j[1];
		im->FastGetSolutionStepValue(NORMAL_TRIPLE_POINT_Z) = normal_tp_j[2];
                
                im->FastGetSolutionStepValue(NORMAL_TRIPLE_POINT_X) = normal_tp_k[0];
		im->FastGetSolutionStepValue(NORMAL_TRIPLE_POINT_Y) = normal_tp_k[1];
		im->FastGetSolutionStepValue(NORMAL_TRIPLE_POINT_Z) = normal_tp_k[2];
		...
		im->FastGetSolutionStepValue(NORMAL_CONTACT_LINE_Y) = cos(asin(temp[2]))*aux[1];
                ...
                //elaf
                im->FastGetSolutionStepValue(NORMAL_CONTACT_LINE_Z) = cos(asin(temp[2]))*aux[2];
		...
		if((im->FastGetSolutionStepValue(CONTACT_ANGLE)) < 90.0)
		{
		    ...
		    im->FastGetSolutionStepValue(NORMAL_CONTACT_LINE_Y) *= -1.0;
                    im->FastGetSolutionStepValue(NORMAL_CONTACT_LINE_Z) *= -1.0;
