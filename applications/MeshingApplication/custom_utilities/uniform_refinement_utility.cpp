...
)
{
    ...
    for (int i = 0; i < static_cast<int>(elements_to_refine.size()); i++)
    {
        ...
        if (geom.GetGeometryType() == GeometryData::KratosGeometryType::Kratos_Triangle2D3)
        {
            ...
            for (auto edge : geom.GenerateEdges())
                middle_nodes[i_node++] = GetNodeInEdge(EdgeType{edge.Points()}, step_divisions_level, rTagNodes, collection_tag);
        ...
        else if (geom.GetGeometryType() == GeometryData::KratosGeometryType::Kratos_Quadrilateral2D4)
        {
            ...
            for (auto edge : geom.GenerateEdges())
                middle_nodes[i_node++] = GetNodeInEdge(EdgeType{edge.Points()}, step_divisions_level, rTagNodes, collection_tag);
            middle_nodes[i_node++] = GetNodeInFace(FaceType{geom.Points()}, step_divisions_level, rTagNodes, collection_tag);
        ...
        else if (geom.GetGeometryType() == GeometryData::KratosGeometryType::Kratos_Hexahedra3D8)
        {
            ...
            for (auto face : geom.GenerateFaces())
                middle_nodes[i_node++] = GetNodeInFace(FaceType{face.Points()}, step_divisions_level, rTagNodes, collection_tag);
            middle_nodes[i_node++] = GetNodeInBody(BodyType{geom.Points()}, step_divisions_level, rTagNodes,collection_tag);
    ...
    for (int i = 0; i < static_cast<int>(conditions_to_refine.size()); i++)
    {
        ...
        {
            NodeType::Pointer middle_node = GetNodeInEdge(EdgeType{geom.Points()}, step_divisions_level, rTagNodes, collection_tag);
