...
def xyz_cut( xyz_in_file, xyz_out_file, bounding_box = (-999999999.0, -999999999.0, 999999999.0, 999999999.0) ):
    ...
    with open( xyz_in_file ) as f:
        ...
        for line in lines:
            items = line.split()
            if ( all( _isfloat(n) for n in items) ):
    ...
    # writing to files
    _write_standard_xyz( point_list, xyz_out_file )
    _write_qgis_xyz( point_list, xyz_out_file )


def xyz_shift( xyz_in_file, xyz_out_file, x_shift, y_shift ):
...
def xyz_valley( xyz_in_file, xyz_out_file, max_height ):
...
def xyz_mountain( xyz_in_file, xyz_out_file, min_height ):
...
def xyz_river( xyz_in_file, xyz_out_file, radius, start_x_y ):
    ...
    while current_length < length:
        ...
        dict_active_points = _find_points_within_radius( dict_all_points, current_center, radius )

        # Check if still new unvisited points are found
        if not _iteration_goes_on( dict_active_points, list_visited_points ):
        ...
        list_visited_points = _add_ids_of_active_points( list_visited_points, dict_active_points )
        print( " - " + str( len(dict_active_points) ) + " active points" )

        # find the active points with the minimal terrain height
        list_lowest_points = _find_lowest_points( dict_active_points, num )

        # moving the circle forward
        previous_center = current_center
        current_center = _compute_new_center( dict_all_points, list_lowest_points, dist, previous_center )
    ...
    point_list = _create_list_of_visited_points( list_visited_points, dict_all_points )
...
def _create_list_of_visited_points( list_visited_points, dict_all_points ):
...
def _iteration_goes_on( dict_active_points, list_visited_points ):
...
def _compute_new_center( dict_all_points, list_lowest_points, dist, previous_center ):
...
def _find_points_within_radius( dict_all_points, current_center, radius ):
...
def _add_ids_of_active_points( list_visited_points, dict_active_points ):
...
def _find_lowest_points( dict_active_points, num ):
...
def _dist( x_start, y_start, x, y ):
    squares = (x - x_start)**2 + (y - y_start)**2
    return math.sqrt( squares )


def _write_standard_xyz( point_list, xyz_out_file ):
...
def _write_qgis_xyz( point_list, xyz_out_file ):
