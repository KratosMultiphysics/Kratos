...
#include "custom_modelers/internals/cartesian_ray.h"
...
namespace Kratos
{
  namespace Internals
  {
    ...
    void InitializeRays(array_1d< std::size_t, 3 > const& MinRayPosition, array_1d< std::size_t, 3 > const& MaxRayPosition, std::string EntititesToColor){
        ...
        template<typename TPointsContainerType>
        void CalculateOuterMinMaxNodePositions(TPointsContainerType const& Points, array_1d< std::size_t, 3 >& MinNodePosition, array_1d< std::size_t, 3 >& MaxNodePosition){
            if(Points.empty())
                return;
