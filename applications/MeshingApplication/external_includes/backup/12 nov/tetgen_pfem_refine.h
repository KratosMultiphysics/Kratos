			...
			int id = (ThisModelPart.Nodes().end() - 1)->Id() + 1;
				...
				/////// 1
				    ...
				    // Node with index kk is the only not TRIPLE_POINT
                                    ...
                                    //if the distance between IS_STRUCTURE node and TRIPLE_POINT node is too long, we add a node in the middle of the face
                                    ...
                                    if ((dist_ik < mesh_size * 0.25 ) || (dist_jk < mesh_size * 0.25 ))
                                    {
                                        if ( ic->GetGeometry()[kk].FastGetSolutionStepValue(TRIPLE_POINT) < 1e-15)
                                        {
                                            ic->GetGeometry()[kk].Set(TO_ERASE,true);
                                            KRATOS_WATCH("we find some points to be deleted ELAF ELAF ELAF ELAF IS_STRUCTURE IS_STRUCTURE 222222 ELAF iiii")
                                        }
                                    }
                                ...
                                if ((n_st) == 3 && (n_tp == 1))
				{
					...
					kk = 1;
                                ...
                                //note that [ii] is the triplet point, so will delete the other two nodes
                                    if ((dist_jk < mesh_size * 0.25) || (dist_ij < mesh_size * 0.25))
                                    {
                                        if ( ic->GetGeometry()[jj].FastGetSolutionStepValue(TRIPLE_POINT) < 1e-15)
                                        {
                                            ic->GetGeometry()[jj].Set(TO_ERASE,true);
                                            KRATOS_WATCH("we find some points to be deleted ELAF ELAF ELAF ELAF IS_FREE_SURFACE IS_FREE_SURFACE 222222 ELAF jjjj")
                                        }

                                    }
                                    if ((dist_ik < mesh_size * 0.25 ))
                                    {
                                        
                                        if ( ic->GetGeometry()[kk].FastGetSolutionStepValue(TRIPLE_POINT) < 1e-15)
                                        {
                                            ic->GetGeometry()[kk].Set(TO_ERASE,true);
                                            KRATOS_WATCH("we find some points to be deleted ELAF ELAF ELAF ELAF IS_FREE_SURFACE IS_FREE_SURFACE 22222 ELAF kkkk")
                                        }
...
//                                         n_struc==ic->GetGeometry().size()		
 				...
 				////
 				// here we iterate through the neighber nodes to reomve the nodes in the is_sturcture that are not attached to the triple point and having long radius 
                                for(ModelPart::NodesContainerType::iterator im = ThisModelPart.NodesBegin() ; im != ThisModelPart.NodesEnd() ; im++)
                                {
                                    ...
                                    if (im->FastGetSolutionStepValue(IS_STRUCTURE) > 0.9999999)
                                    {
                                        if ((dist_ik < (mesh_size * 0.25) ))
                                            {
                                                int neighnum_tp = 0;
                                                WeakPointerVector< Condition >& num_freesurface = im->GetValue(NEIGHBOUR_CONDITIONS);
    //                                             Loop over faces -> find faces that two IS_STRUCTURE nodes
                                                for (unsigned int s = 0; s < num_freesurface.size(); s++)
                                                {
                                                    ...
                                                    for (unsigned int h = 0; h < num_freesurface[s].GetGeometry().size(); h++)
                                                    {
                                                        ...
                                                        else if (neighnum_tp < 0.01 && (dist_ij < mesh_size * 0.25 ))
                                                        {
                                                            if ( ic->GetGeometry()[ii].FastGetSolutionStepValue(TRIPLE_POINT) < 1e-15 && ic->GetGeometry()[ii].FastGetSolutionStepValue(IS_FREE_SURFACE) < 1e-15)
                                                            {
                                ...
                                if ( (n_fs == 2) && (n_tp == 1) )
                                {
                                        ...
                                        // Node with index ii is the one that is on the contact line
                                        double xi=ic->GetGeometry()[ii].X();
                                        double yi=ic->GetGeometry()[ii].Y();
                                        double zi=ic->GetGeometry()[ii].Z();
                                        double xj=ic->GetGeometry()[jj].X();
                                        ...
                                        pnode->Y0() = pnode->Y() - disp[1];
                                        pnode->Z0() = pnode->Z() - disp[2];
                                        KRATOS_WATCH("Added node at the FACE with IS_FREE_SURFACE and tp")
                                        }
                                        
                                        
                                        // we remove the clse nodes that are attached to the tripple point (but not the triple point)
                                        if ((dist_jk < mesh_size * 0.25) || (dist_ij < mesh_size * 0.25))
                                        {
                                            if ( ic->GetGeometry()[jj].FastGetSolutionStepValue(TRIPLE_POINT) < 1e-15)
                                            {
    ...
    // 				      InterpolateOnEdge(geom, ii, kk, step_data_size, pnode);
                                        ...
                                        pnode->Y0() = pnode->Y() - disp[1];
                                        pnode->Z0() = pnode->Z() - disp[2];
                                        KRATOS_WATCH("Added node at the FACE with IS_FREE_SURFACE and 222 tp")
                                        }
                                    ...
                                    if ((dist_ik < mesh_size * 0.25 ) || (dist_jk < mesh_size * 0.25 ))
                                    {
                                        ...
                                        {
                                            ic->GetGeometry()[kk].Set(TO_ERASE,true);
                                            KRATOS_WATCH("we find some points to be deleted ELAF ELAF ELAF ELAF IS_FREE_SURFACE IS_FREE_SURFACE 222222 ELAF kkkkk")
                                        }
                                    }
    ...
    // 				      InterpolateOnEdge(geom, ii, kk, step_data_size, pnode);
                               ...
                               //here we iterate through the neighber nodes to reomve the nodes in the IS_FREE_SURFACE that are not attached to the triple point and having long radius
                                for(ModelPart::NodesContainerType::iterator im = ThisModelPart.NodesBegin() ; im != ThisModelPart.NodesEnd() ; im++)
                                {
                                    ...
                                    if (im->FastGetSolutionStepValue(IS_FREE_SURFACE) > 0.9999999)
                                    {
                                        if ((dist_ik < (mesh_size * 0.25) ))
                                            {
                                                int nei_tp = 0;
                                                WeakPointerVector< Condition >& num_str = im->GetValue(NEIGHBOUR_CONDITIONS);
    //                                             Loop over faces -> find faces that two IS_FREE_SURFACE nodes
                                                for (unsigned int f = 0; f < num_str.size(); f++)
                                                {

                                                    for (unsigned int p = 0; p < num_str[f].GetGeometry().size(); p++)
                                                    {
                                                        nei_tp += num_str[f].GetGeometry()[p].FastGetSolutionStepValue(TRIPLE_POINT);
                                                    
                                                
                                                        if (nei_tp >= 0.9)
                                                        {
                                                            ic->GetGeometry()[p].Set(TO_ERASE,false);
                                                        }
                                                        else if (nei_tp < 0.01 && (dist_ij < mesh_size * 0.25 ))
                                                        {
                                                            if ( ic->GetGeometry()[ii].FastGetSolutionStepValue(TRIPLE_POINT) < 1e-15 && ic->GetGeometry()[ii].FastGetSolutionStepValue(IS_FREE_SURFACE) < 1e-15)
                                                            {
                                                                ic->GetGeometry()[ii].Set(TO_ERASE,true);
	...
	private:
      				...
      				Node<3>::Pointer pnode)
		{
			...
			pnode->FastGetSolutionStepValue(CONTACT_ANGLE) = 0.0;
                        if (pnode->FastGetSolutionStepValue(IS_STRUCTURE)>0.7 && pnode->FastGetSolutionStepValue(TRIPLE_POINT)<0.0001)
                        {
                            pnode->FastGetSolutionStepValue(CONTACT_ANGLE) = 0.0;
                        }
		}
...
// 		    }
			...
			pnode->FastGetSolutionStepValue(IS_STRUCTURE)=0.0;
                    ...
                    if (pnode->FastGetSolutionStepValue(IS_STRUCTURE)>0.7 && pnode->FastGetSolutionStepValue(TRIPLE_POINT)<0.00001)
		    {
                        pnode->FastGetSolutionStepValue(CONTACT_ANGLE) = 0.0;
		    }
...
// 			pnode->FastGetSolutionStepValue(FLAG_VARIABLE) = 0.0;
			...
			pnode->FastGetSolutionStepValue(TRIPLE_POINT) = 0.0;
                    ...
                    if (pnode->FastGetSolutionStepValue(IS_STRUCTURE)>0.7 && pnode->FastGetSolutionStepValue(TRIPLE_POINT)<0.000001)
		    {
                        pnode->FastGetSolutionStepValue(CONTACT_ANGLE) = 0.0;
		    }
