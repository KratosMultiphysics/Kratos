...
TMesh* TVMWLoader::load(const char* aMeshName)
{
    ...
    // Read first Line
    ctrlFlag = fread (line,14,1,fMesh);
    // Read coords

    ctrlFlag = fscanf(fMesh, "%d", &nCoords);
    if (ctrlFlag == 0)
    {
        std::cout << "Invalid read access " << aMeshName;
        return NULL;
    }
    ...
    ctrlFlag =fscanf(fMesh, "%d", &elGroups);
    if (ctrlFlag == 0 )
    {
        std::cout << "Invalid read access " << aMeshName;
        return NULL;
    ...
    ctrlFlag =fscanf(fMesh, "%d%d", &elGroups,&numElements);
    if (ctrlFlag == 0 )
    {
        std::cout << "Invalid read access " << aMeshName;
        return NULL;
    ...
    ctrlFlag =fread (line,11,1,fMesh);
    if (ctrlFlag == 0 )
    {
        std::cout << "Invalid read access " << aMeshName;
        return NULL;
