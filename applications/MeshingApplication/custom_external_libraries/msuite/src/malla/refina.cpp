...
bool malla::mesh1d(bool parcial)
{
    ...
    for (ie0=0; ie0<oldelen; ie0++)
    {
        ...
        hvar=(fabs(dh)>ERRADM);
        l=dn.mod();
        if (hvar) s=l*log(h1/h0)/dh;
        else s=l/h0;
        pasos=(int)ceil(s); // s va de a uno, pero no da entero
        if (pasos<2) continue;
        es[1]=n.len;
        e0[1]=n.len; // el primero sigue siendo ei
        es.f=e0.f;
        if (hvar) fh=exp(s/pasos*dh/l);
        else fh=1;
        h=h0*fh;
        nf.h=h;
        if (hvar) fn=(h-h0)/dh;
    ...
    for (ie0=e.len-1; ie0>=0; ie0--)
    {
        ...
        if (ieant>=0)
        {
            ...
            inant=eant[1-eant.index(in0)];
            dant=n0.distancia(n[inant]);
        }
        else
        {
            inant=-1;
        ...
        if (iepos>=0)
        {
            ...
            inpos=epos[1-epos.index(in1)];
            dpos=n1.distancia(n[inpos]);
        }
        else
        {
            inpos=-1;
    ...
    if (nborrado&&o)
    {
        delete o;    // el octree quedaria mal
        o=0;
    }
    while (nborrado.len)
    {
