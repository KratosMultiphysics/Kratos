...
namespace Kratos
{
    ...
    void FiniteDifferenceUtility::ComputeFiniteDifferences( Vector& rArrayUndisturbed, Vector& rArrayDisturbed, const double& rDelta)
    {
        KRATOS_TRY;

        for(IndexType i = 0; i < rArrayDisturbed.size(); ++i) {
            rArrayDisturbed[i] -= rArrayUndisturbed[i];
            rArrayDisturbed[i] /= rDelta;
        }
