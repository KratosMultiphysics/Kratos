...
// Reset peturbation
            rElement.SetProperties(p_global_properties);
// Output calculation
            noalias(rOutput) = (LHS_peturbed_plus - LHS_peturbed_minus) / (2.0 * rPertubationSize);
        }
        else
...
// Reset peturbation
            rElement.SetProperties(p_global_properties);
// Output calculation
            noalias(rOutput) = (Massmatrix_peturbed_plus - Massmatrix_peturbed_minus) / (2.0 * rPertubationSize);
        }
        else
            KRATOS_ERROR << "Unsupported variable for this element: " << rDesignVariable << std::endl;
