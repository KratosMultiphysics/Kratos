...
protected:
    ...
    /**
     * Calculates the partial derivative of a post-processing result w.r.t a scalar design variable.
     * The differentiation is done by finite differencing.
     */
    template <typename TDataType>
    void CalculatePartialDesignVariableDerivative(const Variable<TDataType>& rQuantityOfInterestVariable,
                                            std::vector< TDataType >& rOutput, const ProcessInfo& rCurrentProcessInfo)
    {
        KRATOS_TRY;

        if(this->Has(DESIGN_VARIABLE_NAME))
        {
            const std::string& design_variable_name = this->GetValue( DESIGN_VARIABLE_NAME );

            KRATOS_ERROR_IF_NOT(KratosComponents<Variable<double>>::Has(design_variable_name))
                                            << "Chosen design variable is not available!" << std::endl;
            ...
            mpPrimalElement->CalculateOnIntegrationPoints(rQuantityOfInterestVariable, undisturbed_values, rCurrentProcessInfo);

            // Get perturbation size
            const double delta = this->GetPerturbationSize(r_variable);

            if( mpPrimalElement->GetProperties().Has(r_variable) )
            {
                ...
                const double current_property_value = mpPrimalElement->GetProperties()[r_variable];
                p_local_property->SetValue(r_variable, (current_property_value + delta));

                // Compute quantity of interest after disturbance of design variable
                mpPrimalElement->CalculateOnIntegrationPoints(rQuantityOfInterestVariable, rOutput, rCurrentProcessInfo);

                // Compute derivative of stress w.r.t. design variable with finite differences
                for(IndexType i = 0; i < rOutput.size(); ++i)
                    this->CoumputeFiniteDifferences(undisturbed_values[i], rOutput[i], delta);
            ...
            KRATOS_ERROR << "Computation of derivative is not possible because no design variable is provided!" << std::endl;


        KRATOS_CATCH("")
    }

    // MFusseder TODO: Move this to a more suitable place
    void CoumputeFiniteDifferences( array_1d<double, 3 >& rArrayUndisturbed, array_1d<double, 3 >& rArrayDisturbed, const double& rDelta)
    {
        KRATOS_TRY;
