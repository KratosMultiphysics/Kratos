...
void AdjointFiniteDifferenceTrussElementLinear::EquationIdVector(EquationIdVectorType &rResult,
...
void AdjointFiniteDifferenceTrussElementLinear::GetDofList(DofsVectorType &rElementalDofList,
                                  ProcessInfo &rCurrentProcessInfo) 
{
    const SizeType number_of_nodes = this->GetGeometry().size();
    const SizeType dimension = this->GetGeometry().WorkingSpaceDimension();
...
void AdjointFiniteDifferenceTrussElementLinear::GetValuesVector(Vector &rValues, int Step) 
{
    KRATOS_TRY
