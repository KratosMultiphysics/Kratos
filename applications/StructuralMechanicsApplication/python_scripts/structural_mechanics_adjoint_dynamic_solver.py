...
class StructuralMechanicsAdjointDynamicSolver(MechanicalSolver):
    ...
    def AddDofs(self):
        ...
        if self.settings["rotation_dofs"].GetBool():
            ...
            KratosMultiphysics.VariableUtils().AddDof(KratosMultiphysics.ANGULAR_ADJOINT_VECTOR_2_X, self.main_model_part)
            KratosMultiphysics.VariableUtils().AddDof(KratosMultiphysics.ANGULAR_ADJOINT_VECTOR_2_Y, self.main_model_part)
            KratosMultiphysics.VariableUtils().AddDof(KratosMultiphysics.ANGULAR_ADJOINT_VECTOR_2_Z, self.main_model_part)
            KratosMultiphysics.VariableUtils().AddDof(KratosMultiphysics.ANGULAR_ADJOINT_VECTOR_3_X, self.main_model_part)
            KratosMultiphysics.VariableUtils().AddDof(KratosMultiphysics.ANGULAR_ADJOINT_VECTOR_3_Y, self.main_model_part)
            KratosMultiphysics.VariableUtils().AddDof(KratosMultiphysics.ANGULAR_ADJOINT_VECTOR_3_Z, self.main_model_part)
            KratosMultiphysics.VariableUtils().AddDof(KratosMultiphysics.ANGULAR_AUX_ADJOINT_VECTOR_1_X, self.main_model_part)
            KratosMultiphysics.VariableUtils().AddDof(KratosMultiphysics.ANGULAR_AUX_ADJOINT_VECTOR_1_Y, self.main_model_part)
            KratosMultiphysics.VariableUtils().AddDof(KratosMultiphysics.ANGULAR_AUX_ADJOINT_VECTOR_1_Z, self.main_model_part)
    ...
    def Initialize(self):
        ...
        if response_type == "adjoint_nodal_square_integral":
            self.response_function = StructuralMechanicsApplication.TransientAdjointNodalSquareIntegralResponseFunction(self.main_model_part, self.settings["response_function_settings"])
        elif response_type == "adjoint_damping_energy_dissipation":
            self.response_function = StructuralMechanicsApplication.TransientAdjointDampingEnergyDissipationResponseFunction(self.main_model_part, self.settings["response_function_settings"])
        else:
