...
def CreateResponseFunction(response_id, response_settings, model):
    ...
    elif response_type == "adjoint_local_beam_normal_stress":
        return structural_response.AdjointBeamNormalStressResponseFunction(response_id, response_settings, model)

    else:
        raise NameError("The type of the following response function is not specified: "+ response_id +
                        ".\nAvailable types are: 'mass', 'strain_energy', 'eigenfrequency', 'adjoint_nodal_displacement', 'adjoint_linear_strain_energy', 'adjoint_local_stress', 'adjoint_max_stress', 'adjoint_nodal_reaction', 'adjoint_local_beam_normal_stress'." )
