...
class ImposeRigidMovementProcess(KratosMultiphysics.Process):
    ...
    def __init__(self, Model, settings ):
        ...
        sub_model_part.AddNode(self.main_model_part.GetNode(settings["master_node_id"].GetInt()), 0)
        initial_id = 2000
        for node in self.model_part.Nodes:
            sub_model_part.AddNode(node, 0)
            sub_model_part.CreateNewCondition("LineCondition3D2N", initial_id, [settings["master_node_id"].GetInt(), node.Id], self.model_part.GetProperties()[0])
            initial_id += 1
