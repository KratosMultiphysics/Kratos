...
class ImplicitMechanicalSolver(solid_mechanics_solver.MechanicalSolver):
    ...
    def _GetSolutionScheme(self, scheme_type, component_wise, compute_contact_forces):
        ...
        if (scheme_type == "Newmark" or scheme_type == "Bossak"):
            ...
            if(component_wise):
                mechanical_scheme = KratosStructural.ComponentWiseBossakScheme(self.settings["damp_factor_m"].GetDouble(), 
    ...
    def _CreateMechanicalSolver(self, mechanical_scheme, mechanical_convergence_criterion, builder_and_solver, max_iters, compute_reactions, reform_step_dofs, move_mesh_flag, component_wise, line_search, implex):
        if(component_wise):
            self.mechanical_solver = KratosStructural.ComponentWiseNewtonRaphsonStrategy(self.computing_model_part, 
                                                                                    mechanical_scheme, 
                                                                                    self.linear_solver, 
                                                                                    mechanical_convergence_criterion, 
                                                                                    builder_and_solver, 
            ...
            if(line_search):
                if(implex):
                    self.mechanical_solver = KratosStructural.ResidualBasedNewtonRaphsonLineSearchImplexStrategy(self.computing_model_part, 
                                                                                                            mechanical_scheme, 
                                                                                                            self.linear_solver, 
                                                                                                            mechanical_convergence_criterion, 
                                                                                                            builder_and_solver, 
                    ...
                    self.mechanical_solver = KratosStructural.ResidualBasedNewtonRaphsonLineSearchStrategy(self.computing_model_part, 
                                                                                                      mechanical_scheme, 
                                                                                                      self.linear_solver, 
                                                                                                      mechanical_convergence_criterion, 
                                                                                                      builder_and_solver, 
