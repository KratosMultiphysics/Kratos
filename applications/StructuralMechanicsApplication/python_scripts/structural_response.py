...
class StrainEnergyResponseFunction(ResponseFunctionBase):
    ...
    def CalculateValue(self):
        Logger.PrintInfo("StrainEnergyResponseFunction", "Starting primal analysis for response", self.identifier)

        startTime = timer.time()
        self.primal_analysis._GetSolver().Predict()
        self.primal_analysis._GetSolver().SolveSolutionStep()
        Logger.PrintInfo("StrainEnergyResponseFunction", "Time needed for solving the primal analysis",round(timer.time() - startTime,2),"s")
        ...
        Logger.PrintInfo("StrainEnergyResponseFunction", "Time needed for calculating the response value",round(timer.time() - startTime,2),"s")

    def CalculateGradient(self):
        Logger.PrintInfo("StrainEnergyResponseFunction", "Starting gradient calculation for response", self.identifier)

        startTime = timer.time()
        self.response_function_utility.CalculateGradient()
        Logger.PrintInfo("StrainEnergyResponseFunction", "Time needed for calculating gradients",round(timer.time() - startTime,2),"s")
...
class EigenFrequencyResponseFunction(StrainEnergyResponseFunction):
    ...
    def __init__(self, identifier, response_settings, model):
        ...
        if max_required_eigenfrequency is not eigen_solver_settings["number_of_eigenvalues"].GetInt():
            Logger.PrintWarning("EigenFrequencyResponseFunction", "Specified number of eigenvalues in the primal analysis and the max required eigenvalue according the response settings do not match!!!")
            Logger.PrintWarning("EigenFrequencyResponseFunction", "Primal parameters were adjusted accordingly!\n")
        ...
        if not eigen_solver_settings.Has("normalize_eigenvectors"):
            ...
            Logger.PrintWarning("EigenFrequencyResponseFunction", "Eigenfrequency response function requires mass normalization of eigenvectors!")
            Logger.PrintWarning("EigenFrequencyResponseFunction", "Primal parameters were adjusted accordingly!\n")
...
class MassResponseFunction(ResponseFunctionBase):
    ...
    def CalculateValue(self):
        Logger.PrintInfo("MassResponseFunction", "Starting primal analysis for response", self.identifier)

        startTime = timer.time()
        value = self.response_function_utility.CalculateValue()
        self.model_part.ProcessInfo[StructuralMechanicsApplication.RESPONSE_VALUE] = value
        Logger.PrintInfo("MassResponseFunction", "Time needed for calculating the response value = ",round(timer.time() - startTime,2),"s")

    def CalculateGradient(self):
        Logger.PrintInfo("MassResponseFunction", "Starting gradient calculation for response", self.identifier)
...
class AdjointResponseFunction(ResponseFunctionBase):
    ...
    def InitializeSolutionStep(self):
        ...
        Logger.PrintInfo("AdjointResponseFunction", "Starting primal analysis for response:", self.identifier)
        startTime = timer.time()
        ...
        Logger.PrintInfo("AdjointResponseFunction", "Time needed for solving the primal analysis = ",round(timer.time() - startTime,2),"s")

    def CalculateValue(self):
        startTime = timer.time()
        value = self._GetResponseFunctionUtility().CalculateValue(self.primal_model_part)
        Logger.PrintInfo("AdjointResponseFunction", "Time needed for calculating the response value = ",round(timer.time() - startTime,2),"s")
    ...
    def CalculateGradient(self):
        ...
        startTime = timer.time()
        Logger.PrintInfo("AdjointResponseFunction", "Starting adjoint analysis for response:", self.identifier)
        if not self.adjoint_analysis.time < self.adjoint_analysis.end_time:
            self.adjoint_analysis.end_time += 1
        self.adjoint_analysis.RunSolutionLoop()
        Logger.PrintInfo("AdjointResponseFunction", "Time needed for solving the adjoint analysis = ",round(timer.time() - startTime,2),"s")
    ...
    def _SynchronizeAdjointFromPrimal(self):
        Logger.PrintInfo("AdjointResponseFunction", "Synchronize primal and adjoint modelpart for response:", self.identifier)
        ...
        if self.response_settings["adjoint_settings"].GetString() == "auto":
            Logger.PrintInfo("AdjointResponseFunction", "Transfer primal state to adjoint model part.")
    ...
    def _GetAdjointParameters(self):
        ...
        if adjoint_settings == "auto":
            Logger.PrintInfo("AdjointResponseFunction", "Automatic set up adjoint parameters for response:", self.identifier)
            ...
            if adjoint_parameters.Has("output_processes"):
                Logger.PrintInfo("AdjointResponseFunction", "Output process is removed for adjoint analysis. To enable it define adjoint_parameters yourself.")
