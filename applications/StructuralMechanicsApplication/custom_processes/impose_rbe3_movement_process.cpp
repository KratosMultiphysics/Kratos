...
namespace Kratos
{
ImposeRBE3RigidMovementProcess::ImposeRBE3RigidMovementProcess(
...
void ImposeRBE3RigidMovementProcess::Execute()
{
    KRATOS_TRY
...
void ImposeRBE3RigidMovementProcess::ExecuteInitialize()
{
    KRATOS_TRY
    ...
    const int slave_node_id = mThisParameters["slave_node_id"].GetInt();
    ...
    #pragma omp parallel
    {
        ...
        NodeType::Pointer p_slave_node = (slave_node_id == 0) ? *(r_rigid_model_part.Nodes().begin()).base() : r_root_model_part.pGetNode(slave_node_id);
        #pragma omp for
        for (int i = 0; i < number_of_nodes; ++i) {
            auto it_node = it_node_begin + i;
            if (it_node->Id() != p_slave_node->Id()) {
                for (IndexType i_var = 0; i_var < number_of_variables; ++i_var) {
                    auto p_constraint = r_clone_constraint.Create(constraint_id + (i * number_of_variables + i_var) + 1, *it_node, *master_list_variables[i_var], *p_slave_node, *slave_list_variables[i_var], relation / number_of_nodes, constant);
...
const Parameters ImposeRBE3RigidMovementProcess::GetDefaultParameters() const
{
...
// class ImposeRBE3RigidMovementProcess
} // namespace Kratos
