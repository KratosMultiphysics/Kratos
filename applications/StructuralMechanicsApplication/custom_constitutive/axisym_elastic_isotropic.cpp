    ...
    )
{
    ...
    const double c0 = (1.0 - NU);
    const double c1 = E / ((1.00 + NU) * (1 - 2 * NU));
    const double c2 = (1.0 - 2.0 * NU) / (2.0);

    rStressVector[0] = c1*(c0 * rStrainVector[0] + NU * rStrainVector[1] + NU * rStrainVector[2]);
    rStressVector[1] = c1*(NU * rStrainVector[0] + c0 * rStrainVector[1] + NU * rStrainVector[2]);
    rStressVector[2] = c1*(NU * rStrainVector[0] + NU * rStrainVector[1] + c0 * rStrainVector[2]);
    rStressVector[3] = c1*(c2 * rStrainVector[3]);
}
