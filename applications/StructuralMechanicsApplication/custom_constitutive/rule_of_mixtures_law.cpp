...
void RuleOfMixturesLaw::InitializeMaterialOnIntegrationPoints(
    const Properties& rMaterialProperties,
    const GeometryType& rElementGeometry,
...
{
    // We compute the maximum and minimal coordinates
    Matrix r_point_local_coordinates;
    r_point_local_coordinates = rElementGeometry.PointsLocalCoordinates(r_point_local_coordinates);
    const SizeType number_of_nodes = rElementGeometry.size();
    std::vector<double> z_coordinates(number_of_nodes);
    for (IndexType i_z = 0; i_z < number_of_nodes; ++i_z) {
        z_coordinates[i_z] = r_point_local_coordinates(i_z, 2);
    }
    const double max_z = *std::max_element(z_coordinates.begin(), z_coordinates.end());
    const double min_z = *std::min_element(z_coordinates.begin(), z_coordinates.end());
    const double delta_z = max_z - min_z;

    // We check first how the GP are distributed along thickness (we will assume that the Z axis of the geometry is the one which defines the distribution)
    const GeometryType::IntegrationPointsArrayType& r_integration_points = rElementGeometry.IntegrationPoints( ThisIntegrationMethod );
    std::set<double> z_components;
    for ( IndexType point_number = 0; point_number < r_integration_points.size(); ++point_number ) {
        const double z = r_integration_points[point_number].Z();
        z_components.insert(z);
    }
    const SizeType number_of_z_layers = z_components.size();
    const double layer_factor = static_cast<double>(number_of_z_layers);
    const double increment_z = delta_z/layer_factor;
    IndexType current_layer = 0;
    const double current_z = r_integration_points[IntegrationPointIndex].Z();
    for (IndexType i = 0; i < number_of_z_layers; ++i) {
        if (current_z > (min_z + (increment_z) * i) &&
            current_z < (min_z + (increment_z) * (i + 1))) {
            current_layer = i;
            break;
        }
    }

    // We know now our layer
    const double min_layer_z = (min_z + (increment_z) * current_layer);
    const double max_layer_z = (min_z + (increment_z) * (current_layer + 1) );

    // We recompute the factors
    IndexType counter = 0;
    std::vector<double> z_levels(mCombinationFactors.size(), min_z);
    for (auto& factors : mCombinationFactors) {
        const double factor = factors.second;
        if (counter > 0) z_levels[counter] = z_levels[counter - 1];
        z_levels[counter] += delta_z * factor;
        ++counter;
    }

    counter = 0;
    std::vector<IndexType> keys_to_remove;
    for (auto& factors : mCombinationFactors) {
        const IndexType key  = factors.first;
        double& factor = factors.second;

        if (z_levels[counter] >= min_layer_z) {
            if (z_levels[counter + 1] <= max_layer_z) { // Completely inside the later
                factor *= layer_factor;
            } else { // Partly
                factor *= layer_factor * (max_layer_z - z_levels[counter])/(z_levels[counter + 1] - z_levels[counter]);
            }
        } else { // Outside the layer
    ...
    // Finally we remove the unused layers on this integration point
    for (auto& key : keys_to_remove) {
        mCombinationFactors.erase(key);
        mMaterialRotationAngles.erase(key);
    }
    ...
    for (auto& factors : mCombinationFactors) {
        ...
        p_inner_law->InitializeMaterial(rMaterialProperties, rElementGeometry, row(r_shape_functions, IntegrationPointIndex));
        mConstitutiveLaws.insert(std::pair<IndexType, ConstitutiveLaw::Pointer>({id, p_inner_law}));
    }
}
