...
#include "custom_utilities/constitutive_law_utilities.cpp"
...
/***********************************************************************************/
/***********************************************************************************/
void DamageDPlusDMinusMasonry2DLaw::CalculateMaterialResponsPK2(ConstitutiveLaw::Parameters& rValues)
{
	this->CalculateMaterialResponseCauchy(rValues);
}
/***********************************************************************************/
/***********************************************************************************/
void DamageDPlusDMinusMasonry2DLaw::CalculateMaterialResponsKirchhoff(ConstitutiveLaw::Parameters& rValues)
{
	this->CalculateMaterialResponseCauchy(rValues);
}
/***********************************************************************************/
/***********************************************************************************/
void DamageDPlusDMinusMasonry2DLaw::CalculateMaterialResponsPK1(ConstitutiveLaw::Parameters& rValues)
{
	this->CalculateMaterialResponseCauchy(rValues);
}
/***********************************************************************************/
/***********************************************************************************/
void DamageDPlusDMinusMasonry2DLaw::CalculateMaterialResponsCauchy(ConstitutiveLaw::Parameters& rValues)
{
	KRATOS_TRY
    ...
    if (r_constitutive_law_options.Is(ConstitutiveLaw::COMPUTE_STRESS)) {
		...
		DamageDPlusDMinusMasonry2DLaw::CalculateEquivalentStressTension(predictive_stress_vector, damage_parameters.UniaxialTensionStress, rValues)	
		DamageDPlusDMinusMasonry2DLaw::CalculateEquivalentStressCompression(predictive_stress_vector, damage_parameters.UniaxialCompressionStress, rValues)
	...
	ConstitutiveLaw::Parameters& rValues)
{
	...
	if (F_tension<=tolerance){//Elastic Case
        ...
        const double characteristic_length = ConstitutiveLawUtilities<3>::CalculateCharacteristicLength(rValues.GetElementGeometry());

        // This routine updates the IntegratedStressVectorTension to verify the yield surf
        DamageDPlusDMinusMasonry2DLaw::IntegrateStressVectorTension(
	...
	ConstitutiveLaw::Parameters& rValues)
{
    ...
    if (F_compression <= tolerance) { // Elastic case
        ...
        TConstLawIntegratorCompressionType::IntegrateStressVectorCompression(
...
/***********************************************************************************/
/***********************************************************************************/
oid DamageDPlusDMinusMasonry2DLaw::CalculateSecantTensor(ConstitutiveLaw::Parameters& rValues, Matrix& rSecantTensor)
{
        ...
        const Vector& rShapeFunctionsValues)
{
	...
	const Properties& r_material_properties = rValues.GetMaterialProperties();
	double initial_threshold_tension = r_material_properties["YIELD_STRESS_TENSION"];
	double initial_threshold_compression = r_material_properties["DAMAGE_ONSET_STRESS_COMPRESSION"];
	...
	)
{
	...
	if (shear_compression_reductor < 0.0){
		KRATOS_WARNING << "The SHEAR_COMPRESSION_REDUCTOR is supposed to be a value between 0 and 1.0" << std::endl;
	} else {
		if (shear_compression_reductor > 1.0){
			KRATOS_WARNING << "The SHEAR_COMPRESSION_REDUCTOR is supposed to be a value between 0 and 1.0" << std::endl;
		}
	}
	...
	const double principal_stress_1 = rPrincipalStressVector[0];
	const double principal_stress_2 = rPrincipalStressVector[1];
	const smax_macaulay = std::max(principal_stress_1, 0.0);
    ...
    const double CharacteristicLength)
{
	...
	DamageDPlusDMinusMasonry2DLaw::CalculateDamageParameterTension(rValues, damage_parameter, CharacteristicLength);
	DamageDPlusDMinusMasonry2DLaw::CalculateExponentialDamageTension(UniaxialStress, rThreshold, damage_parameter, CharacteristicLength, rValues, rDamage);
...
void DamageDPlusDMinusMasonry2DLaw::CalculateExponentialDamageTension(
	...
	double& rDamage)
{
	const Properties& r_material_prop = rValues.GetMaterialProperties();
...
void DamageDPlusDMinusMasonry2DLaw::IntegrateStressVectorCompression(
    ...
    const double CharacteristicLength)
{
	DamageDPlusDMinusMasonry2DLaw::CalculateBezier3DamageCompression(UniaxialStress, rDamage, rThreshold, CharacteristicLength, rValues);
	...
	ConstitutiveLaw::Parameters& rValues)
{
	...
	const double ej = ep + alpha * c2;
	const double ek = ej + alpha * (1.0 - c2);
	const double er = ( (ek - ej) * (sp - sr)/(sp -sk) ) + ej;
	const double eu = er * c3;
	const double gc = Gc / CharacteristicLength;
	
	// Perform the Energy Regularization of the Bezier Determinators
	double gc_bezier; 
	DamageDPlusDMinusMasonry2DLaw::RegulateBezierDeterminators(gc, sp, sk, sr, ep, ej, ek, er, eu);
	
	// Compute rDamage
	double StrainLikeCounterpart = UniaxialStress / E;
	double DamageVariableBezier = UniaxialStress;
	if (StrainLikeCounterpart <= ep){
		DamageVariableBezier = EvaluateBezierCurve(StrainLikeCounterpart, e0, ei, ep, s0, sp, sp);
	} else {if (StrainLikeCounterpart <= ek){
			DamageVariableBezier = EvaluateBezierCurve(StrainLikeCounterpart, ep, ej, ek, sp, sp, sk);
			} else {if (StrainLikeCounterpart <= eu){
				DamageVariableBezier = EvaluateBezierCurve(StrainLikeCounterpart, ek, er, eu, sk, sr, sr);
				} else {
...
{	
	...
	DamageDPlusDMinusMasonry2DLaw::ComputeBezierEnergy(bezier_energy_2, ep, ej, ek, sp, sp, sk);
	DamageDPlusDMinusMasonry2DLaw::ComputeBezierEnergy(bezier_energy_3, ek, er, eu, sk, sr, sr);
