    ...
    const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    KRATOS_ERROR_IF_NOT(mConstitutiveLawVector[0]->GetStrainSize() == 8)
        << "MITCThickShellElement3D4N - Wrong size of the strain vector in constitutive law (got "
        << mConstitutiveLawVector[0]->GetStrainSize() << ", expected 8)." << std::endl
        << "The thick shell quadrilateral element requires a shell constitutive law." << std::endl
        << "If you used LinearElasticPlaneStress2DLaw, swap it for ReissnerMindlinShellElasticConstitutiveLaw." << std::endl
        << "For composite/nonlinear problems (plasticity, damage, etc.), use ConstitutiveLawsApplication laws:" << std::endl
        << "  - custom_constitutive/structural_elements_constitutive_laws/thickness_integrated_composite_constitutive_law" << std::endl
        << "  - custom_constitutive/structural_elements_constitutive_laws/thickness_integrated_isotropic_constitutive_law" << std::endl;
