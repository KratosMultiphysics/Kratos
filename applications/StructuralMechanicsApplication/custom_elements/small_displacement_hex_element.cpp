    ...
    )
{
    ...
    for ( IndexType point_number = 0; point_number < integration_points.size(); ++point_number ) {
        ...
        double damage_value_at_gauss_point = 0.0;
        for (int i = 0; i < N.size(); ++i) {
            damage_value_at_gauss_point += this_kinematic_variables.N[i] * r_geometry[i].FastGetSolutionStepValue(DAMAGE_VARIABLE, 0);
        }

        array_1d<double, 3> damage_derivative_at_gauss_point(0.0);    
        for (int i = 0; i < N.size(); ++i) {
            for (int j = 0; j < 3; ++j) {
                damage_derivative_at_gauss_point[j] += this_kinematic_variables.DN_DX(i, j) * r_geometry[i].FastGetSolutionStepValue(DAMAGE_VARIABLE, 0);
            }
        }            
        ...
        if ( CalculateStiffnessMatrixFlag ) { // Calculation of the matrix is required
            ...
            this->CalculateAndAddKm(k_uu, this_kinematic_variables.B, h_uu, int_to_reference_weight)
            this->CalculateAndAddKm(k_u1, this_kinematic_variables.B, h_uu, int_to_reference_weight)
            this->CalculateAndAddKm(k_1u, this_kinematic_variables.B, h_uu, int_to_reference_weight)
            this->CalculateAndAddKm(k_11, this_kinematic_variables.B, h_uu, int_to_reference_weight)
