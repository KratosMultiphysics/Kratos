    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    if (Is(INTERFACE)) {
        ...
        if (sur_bd_ids_vect.size() != 0) {
            ...
            BoundedMatrix<double,LocalSize,StrainSize> B;
            ...
            BoundedVector<array_1d<double,3>, NumNodes> nodal_unknown;
            for (std::size_t i_node = 0; i_node < NumNodes; ++i_node) {
                nodal_unknown[i_node] = r_geom[i_node].FastGetSolutionStepValue(DISPLACEMENT);
            }
            ...
            for (std::size_t sur_bd_id : sur_bd_ids_vect) {
                ...
                const auto& r_stress = constitutive_variables.GetStressVector();
                const auto& r_C = constitutive_variables.GetConstitutiveMatrix();
                ...
                const double aux_w_k = TDim * dom_size_parent / h_sur_bd;
