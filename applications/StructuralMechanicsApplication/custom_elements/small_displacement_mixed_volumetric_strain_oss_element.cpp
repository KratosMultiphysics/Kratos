...
void SmallDisplacementMixedVolumetricStrainOssElement::CalculateLocalSystemGaussPointContribution(
    ...
    const GaussPointAuxiliaryVariables &rThisGaussPointAuxiliaryVariables) const
{
    ...
    for (IndexType i = 0; i < n_nodes; ++i) {
        ...
        if (IsDynamic()) {
            ...
            //TODO: Check this with the formulation
        ...
        for (IndexType j = 0; j < n_nodes; ++j) {
            ...
            const auto& r_u_s_proj = r_geometry[j].FastGetSolutionStepValue(DISPLACEMENT_PROJECTION);
            eps_proj_gauss += N_j * r_geometry[j].FastGetSolutionStepValue(VOLUMETRIC_STRAIN_PROJECTION);
            for (IndexType d = 0; d < dim; ++d) {
...
void SmallDisplacementMixedVolumetricStrainOssElement::CalculateRightHandSideGaussPointContribution(
    VectorType &rRightHandSideVector,
    const KinematicVariables &rThisKinematicVariables,
    const ConstitutiveVariables &rThisConstitutiveVariables,
    const GaussPointAuxiliaryVariables &rThisGaussPointAuxiliaryVariables) const
