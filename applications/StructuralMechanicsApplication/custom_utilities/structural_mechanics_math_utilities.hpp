...
//        KRATOS_WATCH(aux);
    ...
    static inline void argsort(RAIter iterBegin, RAIter iterEnd, Compare comp,
    ...
    /**
     * Resort the Eigen System
     * @param EigenVectorsMatrix: The matrix with the eigenvectors
     * @param EigenValuesMatrix: The matrix with the eigenvalues
     */

    static inline void SortEigen(
                Matrix& EigenVectorsMatrix,
                Matrix& EigenValuesMatrix
                )
    {
        int dim = EigenVectorsMatrix.size1();
        Matrix aux_Matrix(dim, dim);

        double aux_eigen[dim];
        size_t size = 0;
        for (int i = 0; i < dim ; i++)
        {
            aux_eigen[size++] = EigenValuesMatrix(i, i);
        }

        std::vector<size_t> indexes;
        StructuralMechanicsMathUtilities::argsort(aux_eigen, aux_eigen + sizeof(aux_eigen) / sizeof(aux_eigen[0]), std::less<int>(), indexes) ;

        std::cout << "indexes" << std::endl;
        for (size_t i : indexes) printf("%d\n", int(i)) ;

        noalias(aux_Matrix) =  EigenValuesMatrix;
        for (int i = 0; i < dim; i++)
        {
            int aux_i = indexes[dim - i - 1];
            EigenValuesMatrix(i, i) = aux_Matrix(aux_i, aux_i);
        }

        noalias(aux_Matrix) =  EigenVectorsMatrix;
        for (int i = 0; i < dim; i++)
        {
            int aux_i = indexes[dim - i - 1];
            for (int j = 0; j < dim; j++ )
            {
                EigenVectorsMatrix(j, i) = aux_Matrix(j, aux_i);
            }
        }
    }
