...
template<unsigned int TDim>
UnifiedFatigueLaw<TDim>::UnifiedFatigueLaw(const std::vector<double>& rCombinationFactors) : ConstitutiveLaw()
{
    ...
    KRATOS_ERROR_IF(aux_factor < std::numeric_limits<double>::epsilon()) << "Wrong factors in UnifiedFatigueLaw" << std::endl;
...
template<unsigned int TDim>
ConstitutiveLaw::Pointer UnifiedFatigueLaw<TDim>::Create(Kratos::Parameters NewParameters) const
{
    // We do some checks
    KRATOS_ERROR_IF_NOT(NewParameters.Has("combination_factors")) << "UnifiedFatigueLaw: Please define combination_factors" << std::endl;
    ...
    // We create the law
    return Kratos::make_shared<UnifiedFatigueLaw>(combination_factors);
}
...
template<unsigned int TDim>
std::size_t UnifiedFatigueLaw<TDim>::WorkingSpaceDimension()
{
    IndexType counter = 0;
...
template<unsigned int TDim>
std::size_t UnifiedFatigueLaw<TDim>::GetStrainSize()
{
    IndexType counter = 0;
...
template<unsigned int TDim>
ConstitutiveLaw::StrainMeasure UnifiedFatigueLaw<TDim>::GetStrainMeasure()
{
    // We return the first one
    KRATOS_ERROR_IF(mConstitutiveLaws.size() == 0) << "UnifiedFatigueLaw: No constitutive laws defined" << std::endl;
...
template<unsigned int TDim>
ConstitutiveLaw::StressMeasure UnifiedFatigueLaw<TDim>::GetStressMeasure()
{
    // We return the first one
    KRATOS_ERROR_IF(mConstitutiveLaws.size() == 0) << "UnifiedFatigueLaw: No constitutive laws defined" << std::endl;
...
template<unsigned int TDim>
bool UnifiedFatigueLaw<TDim>::IsIncremental()
{
    // We check it layer by layer
    ...
    )
{
    ...
    KRATOS_DEBUG_ERROR_IF(mConstitutiveLaws.size() == 0) << "UnifiedFatigueLaw: No CL defined" << std::endl;
}
...
template<unsigned int TDim>
void UnifiedFatigueLaw<TDim>::InitializeSolutionStep(
    const Properties& rMaterialProperties,
    const GeometryType& rElementGeometry,
    const Vector& rShapeFunctionsValues,
    const ProcessInfo& rCurrentProcessInfo
...
template<unsigned int TDim>
void UnifiedFatigueLaw<TDim>::FinalizeSolutionStep(
    const Properties& rMaterialProperties,
    const GeometryType& rElementGeometry,
    const Vector& rShapeFunctionsValues,
    const ProcessInfo& rCurrentProcessInfo
...
template<unsigned int TDim>
void UnifiedFatigueLaw<TDim>::InitializeNonLinearIteration(
    const Properties& rMaterialProperties,
    const GeometryType& rElementGeometry,
    const Vector& rShapeFunctionsValues,
    const ProcessInfo& rCurrentProcessInfo
...
template<unsigned int TDim>
void UnifiedFatigueLaw<TDim>::FinalizeNonLinearIteration(
    const Properties& rMaterialProperties,
    const GeometryType& rElementGeometry,
    const Vector& rShapeFunctionsValues,
    const ProcessInfo& rCurrentProcessInfo
...
template<unsigned int TDim>
void UnifiedFatigueLaw<TDim>::ResetMaterial(
    const Properties& rMaterialProperties,
    const GeometryType& rElementGeometry,
    const Vector& rShapeFunctionsValues
    )
...
template<unsigned int TDim>
void UnifiedFatigueLaw<TDim>::GetLawFeatures(Features& rFeatures)
{
    //Set the strain size
...
template<unsigned int TDim>
void UnifiedFatigueLaw<TDim>::CalculateRotationMatrix(
        const Properties& rMaterialProperties,
...
template class UnifiedFatigueLaw<2>;
template class UnifiedFatigueLaw<3>;
