    ...
    )
{
    ...
    if (rVariable == NORMAL) {
        ...
        array_3 tangent_xi, tangent_eta;
        Matrix J(TDim, 1);
    ...
    )
{
    ...
    const auto& rDN_De = r_geometry.ShapeFunctionsLocalGradients(integration_method);
    ...
    array_3 line_load;
    noalias(line_load) = ZeroVector(3);
...
template<>
void LineLoadCondition<2>::GetLocalAxis2(array_3& rLocalAxis) const
{
    rLocalAxis[0] = 0.0;
    rLocalAxis[1] = 0.0;
    rLocalAxis[2] = 1.0;
...
template<>
void LineLoadCondition<3>::GetLocalAxis2(array_3& rLocalAxis) const
{
