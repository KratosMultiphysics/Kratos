...
namespace Testing
{
    ...
    */
    KRATOS_TEST_CASE_IN_SUITE(ConstitutiveLawIntegrateStress, KratosStructuralMechanicsFastSuite)
    {
        ...
        for (int comp = 0; comp < 6; comp++) {
            KRATOS_CHECK_NEAR(MCres[comp], TestMC[comp], 0.0001e+6);
            KRATOS_CHECK_NEAR(VMres[comp], TestVM[comp], 0.0001e+6);
            KRATOS_CHECK_NEAR(DPres[comp], TestDP[comp], 0.0001e+6);
            KRATOS_CHECK_NEAR(Tres[comp],  TestT[comp],  0.0001e+6);
        }
    }
}
}