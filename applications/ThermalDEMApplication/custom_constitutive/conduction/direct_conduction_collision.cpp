...
namespace Kratos {
  ...
  double DirectConductionCollision::ComputeHeatFlux(const ProcessInfo& r_process_info, ThermalSphericParticle* particle) {
    ...
    double col_dur_max = 0.0;
    if (impact_normal_velocity != 0.0)
      col_dur_max = contact_params.impact_duration_max;
    ...
    if (col_dur < col_dur_max) {
      ...
      return C_coeff * Globals::Pi * Rc_max * Rc_max * pow(col_dur_max,-0.5) * temp_grad / (pow(b1,-0.5) + pow(b2,-0.5));
    }
    else {
