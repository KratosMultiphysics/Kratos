...
with open(optimization_log_filename, 'r') as csvfile:
    ...
    resulting_abs_change = float(last_line[2].strip())
    resulting_sina_alpha = float(last_line[12].strip())

    # # Check against specifications
    TestCase().assertEqual(resulting_iteration, 10)
    TestCase().assertAlmostEqual(resulting_abs_change, -27.8997, 4)
    TestCase().assertAlmostEqual(resulting_sina_alpha, 0.295666, 4)
