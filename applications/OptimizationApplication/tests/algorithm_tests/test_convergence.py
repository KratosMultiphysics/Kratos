...
class TestConvergence(kratos_unittest.TestCase):
    ...
    def test_L2(self):
        ...
        convergence_criterium = CreateConvergenceCriteria(param, self.optimization_problem)
        search_direction = KratosOA.ContainerExpression.CollectiveExpression([Kratos.Expression.ElementExpression(self.model_part)])
        KratosOA.ContainerExpression.CollectiveExpressionIO.Read(search_direction, KratosOA.ContainerExpression.CollectiveExpressionIO.PropertiesVariable(Kratos.DENSITY))
