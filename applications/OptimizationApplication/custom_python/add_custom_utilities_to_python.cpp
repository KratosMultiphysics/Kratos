...
void AddOptimizationInfoToPython(pybind11::module m)
{
    ...
    auto optimization_info_class = py::class_<OptimizationInfoType, typename OptimizationInfoType::Pointer>(m, "OptimizationInfo")
        ...
        .def("GetKeys", [](const OptimizationInfoType& rSelf, const IndexType StepIndex, const bool SearchSubItems) { std::vector<std::string> keys; rSelf.GetKeys(keys, StepIndex, SearchSubItems); return keys;}, py::arg("step_index") = 0, py::arg("search_sub_items") = false)
        .def("GetKeysDataMap", [](const OptimizationInfoType& rSelf, const IndexType StepIndex, const bool SearchSubItems) { std::unordered_map<std::string, typename OptimizationInfoType::ValueType> map; rSelf.GetKeysDataMap(map, StepIndex, SearchSubItems); return map;}, py::arg("step_index") = 0, py::arg("search_sub_items") = false)
