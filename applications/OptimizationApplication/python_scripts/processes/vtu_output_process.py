...
def Factory(model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem):
    ...
    return VtuOutputProcess(model, parameters["settings"], optimization_problem)
...
class VtuOutputProcess(Kratos.OutputProcess):
    def GetDefaultParameters(self) -> Kratos.Parameters:
        return Kratos.Parameters("""
        {
            ...
            "nodal_flags"                       : [],
            "element_properties_variables"      : [],
            "element_data_value_variables"      : [],
            "element_flags"                     : [],
            "condition_properties_variables"    : [],
    ...
    def __init__(self, model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem) -> None:
        ...
        self.element_properties_variables = parameters["element_properties_variables"].GetStringArray()
        ...
        self.condition_properties_variables = parameters["condition_properties_variables"].GetStringArray()
    ...
    def PrintOutput(self) -> None:
        current_suffix = self.optimization_problem.GetStep()

        for var, cexp in self.list_of_properties_cexps:
            KratosOA.PropertiesVariableExpressionIO.Read(cexp, var)

        for vtu_output in self.vtu_output_ios:
            vtu_output.PrintOutput(str(self.output_path / vtu_output.GetModelPart().FullName()) + f"_{current_suffix}")
    ...
    def __AddData(self, vtu_output_io: Kratos.VtuOutput) -> None:
        ...
        for properties_var_name in self.element_properties_variables:
            var = Kratos.KratosGlobals.GetVariable(properties_var_name)
            cexp = Kratos.Expression.ElementExpression(vtu_output_io.GetModelPart())
            self.list_of_properties_cexps.append((var, cexp))
            vtu_output_io.AddContainerExpression(properties_var_name, cexp)

        for properties_var_name in self.condition_properties_variables:
            var = Kratos.KratosGlobals.GetVariable(properties_var_name)
            cexp = Kratos.Expression.ConditionExpression(vtu_output_io.GetModelPart())
            self.list_of_properties_cexps.append((var, cexp))
            vtu_output_io.AddContainerExpression(properties_var_name, cexp)
