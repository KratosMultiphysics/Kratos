...
def CreateConvergenceCriteria(parameters: Kratos.Parameters, optimization_problem: OptimizationProblem):
    ...
    elif type == "magnitude_reduction_value":
...
class MagnitudeReductionCriterion:
    ...
    def GetDefaultParameters(cls):
        return Kratos.Parameters("""{
            "type"                            : "magnitude_reduction_value",
            "max_iter"                        : 0,
    ...
    def IsConverged(self) -> bool:
        ...
        if iter == 0:
            ...
            self.__target_value = max(self.value * self.__target_magnitude_reduction, 1e-16)
            print("target value of objective function is ", self.__target_value)
        if not self.conv:
            self.conv = self.value <= self.__target_value
