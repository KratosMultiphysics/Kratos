...
class ConstStep():
    ...
    def ComputeScaleFactor(self) -> float:
        ...
        search_direction = algorithm_buffered_data["search_direction"]
        if self._gradient_scaling == "inf_norm":
            norm = KratosOA.ExpressionUtils.NormInf(search_direction)
        elif self._gradient_scaling == "l2_norm":
            norm = KratosOA.ExpressionUtils.NormL2(search_direction)
