...
class DisplacementResponseFunction(ResponseFunction):
    def __init__(
        ...
        self.primal_analysis_execution_policy_wrapper: ExecutionPolicyWrapper = (
            optimization_info.GetOptimizationRoutine(
                "ExecutionPolicyWrapper", parameters["primal_analysis_name"].GetString()
            )
        )
    ...
    def CalculateValue(self) -> float:
        ...
        return KratosOA.LinearStrainEnergyResponseUtilities.CalculateStrainEnergy(
            self.model_part
        )
    ...
    def CalculateSensitivity(
        ...
        if (
            sensitivity_variable == Kratos.SHAPE_SENSITIVITY
            and sensitivity_container_type == ContainerEnum.NODES
        ):
            KratosOA.LinearStrainEnergyResponseUtilities.CalculateStrainEnergyShapeSensitivity(
                sensitivity_model_part, self.perturbation_size, sensitivity_variable
            )
