...
class ResponseRoutine:
    ...
    def CalculateValue(self, control_field: KratosOA.CollectiveExpression) -> float:
        ...
        Kratos.Logger.PrintInfo(f"Norm Value: {norm}, self.__response_value is None: {self.__response_value is None}, compute_response_value_flag: {compute_response_value_flag}")
        ...
        if compute_response_value_flag:
            ...
            self.value_calculate_count +=1
            Kratos.Logger.PrintInfo(f"Response value is calculated for {self.GetResponseName()}. Count is {self.value_calculate_count}")
        else:
            Kratos.Logger.PrintInfo("The control field is not updated, hence the response value is not computed.")
