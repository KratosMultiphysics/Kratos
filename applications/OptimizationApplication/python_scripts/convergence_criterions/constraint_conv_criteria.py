...
def Factory(_: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem) -> ConvergenceCriteria:
    if not parameters.Has("settings"):
        raise RuntimeError(f"ConstraintConvCriteria instantiation requires a \"settings\" in parameters [ parameters = {parameters}].")
    return ConstraintConvCriteria(parameters["settings"], optimization_problem)

class ConstraintConvCriteria(ConvergenceCriteria):
    @classmethod
