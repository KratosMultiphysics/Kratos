...
def Factory(_: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem) -> ConvergenceCriteria:
    if not parameters.Has("settings"):
        raise RuntimeError(f"TargetValueConvCriteria instantiation requires a \"settings\" in parameters [ parameters = {parameters}].")
    return TargetValueConvCriteria(parameters["settings"], optimization_problem)

class TargetValueConvCriteria(ConvergenceCriteria):
    @classmethod
