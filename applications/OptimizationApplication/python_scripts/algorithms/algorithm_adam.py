...
class AlgorithmAdam(AlgorithmSteepestDescent):
    ...
    def ComputeControlUpdate(self, alpha: Kratos.TensorAdaptors.DoubleCombinedTensorAdaptor):
        ...
        # update the biased first and second raw moment estimates. The moments from the previous
        # call are bookkept in "algorithm"'s own buffered data (step_index 1, i.e. the previous
        # step -- this call writes this step's moments to step_index 0) rather than plain Python
        # attributes, so that a restart checkpoint/resume round-trips them correctly; see
        # optimization_problem_restart_input_process.py.
        ...
        if algorithm_buffered_data.HasValue("moment_1", 1) and algorithm_buffered_data.HasValue("moment_2", 1):
            prev_moment_1: Kratos.TensorAdaptors.DoubleCombinedTensorAdaptor = algorithm_buffered_data.GetValue("moment_1", 1)
            prev_moment_2: Kratos.TensorAdaptors.DoubleCombinedTensorAdaptor = algorithm_buffered_data.GetValue("moment_2", 1)
            moment_1.data[:] = prev_moment_1.data * self.beta_1 + search_direction.data * (1.0 - self.beta_1)
            moment_2.data[:] = prev_moment_2.data * self.beta_2 + search_direction.data ** 2 * (1.0 - self.beta_2)
        else:
            moment_1.data[:] = search_direction.data * (1.0 - self.beta_1)
            moment_2.data[:] = search_direction.data ** 2 * (1.0 - self.beta_2)
        moment_1.StoreData()
        moment_2.StoreData()
        algorithm_buffered_data["moment_1"] = moment_1
        algorithm_buffered_data["moment_2"] = moment_2
