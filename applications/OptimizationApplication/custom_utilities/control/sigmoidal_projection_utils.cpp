...
void SigmoidalProjectionUtils::CheckXYVectors(std::vector<double> xValues, std::vector<double> yValues){
...
double SigmoidalProjectionUtils::ProjectValueForward(
        double xValue,
...
double SigmoidalProjectionUtils::ProjectValueBackward(
        double yValue,
        std::vector<double> xLimits,
        std::vector<double> yLimits,
        double beta,
        int penalFac)
{
    ...
    double log_val = 0.0;
    if (std::abs(yValue-y2)<= std::numeric_limits<double>::min())
        log_val = std::numeric_limits<double>::min();
    else if (std::abs(yValue-y1)<= std::numeric_limits<double>::min())
        log_val = std::numeric_limits<double>::max();
    else
        log_val = ((y2-y1)/(yValue-y1))-1;
    return ((x2+x1)/2.0) + (1.0/(-2.0*beta)) * std::log(log_val);
}
