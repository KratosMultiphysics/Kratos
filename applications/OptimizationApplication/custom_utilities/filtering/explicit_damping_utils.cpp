    ...
    const IndexType DampedComponentIndex)
{
    ...
    for (const auto& p_model_part : rDampedModelParts) {
        const auto& r_entities = block_for_each<AccumReduction<typename EntityPointType<TContainerType>::Pointer>>(ModelPartUtils::GetContainer<TContainerType>(*p_model_part), [&rFilterEntityIdEntityPointMap, &p_model_part, &r_filtering_model_part](const auto& rEntity) -> typename EntityPointType<TContainerType>::Pointer {
