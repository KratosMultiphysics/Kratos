...
void FilterUtils::FillEntityIdEntityPointMap(
...
template<class TContainerType>
void FilterUtils::ComputeDampingCoefficientsBasedOnNearestEntityForComponent(
    ...
    const IndexType DampedComponentIndex)
{
    ...
    damped_entities.reserve(std::accumulate(
        ...
        [](const IndexType Value, const auto& pModelPart) {
            return Value + GetContainer<TContainerType>(*pModelPart).size();
        }));
    for (const auto& p_model_part : rDampedModelParts) {
        const auto& r_entities = block_for_each<AccumReduction<typename EntityPointType<TContainerType>::Pointer>>(GetContainer<TContainerType>(*p_model_part), [&rFilterEntityIdEntityPointMap, &p_model_part, &r_filtering_model_part](const auto& rEntity) -> typename EntityPointType<TContainerType>::Pointer {
    ...
    const auto& r_container = GetContainer<TContainerType>(r_filtering_model_part);
...
template<class TContainerType>
ContainerExpression<TContainerType> FilterUtils::ComputeDampingCoefficientsBasedOnNearestEntity(
    const ContainerExpression<TContainerType>& rDampingRadiusExpression,
...
#define KRATOS_INSTANTIATE_FILTER_UTILS_FOR_CONTAINER_TYPE(ContainerType)                                                                         \
    template KRATOS_API(OPTIMIZATION_APPLICATION) void FilterUtils::FillEntityIdEntityPointMap(                                                   \
        std::unordered_map<IndexType, typename EntityPointType<ContainerType>::Pointer>&,                                                         \
        const ContainerType&);                                                                                                                    \
    template KRATOS_API(OPTIMIZATION_APPLICATION) ContainerExpression<ContainerType> FilterUtils::ComputeDampingCoefficientsBasedOnNearestEntity( \
    ...
    template KRATOS_API(OPTIMIZATION_APPLICATION) void FilterUtils::ComputeDampingCoefficientsBasedOnNearestEntityForComponent(                   \
