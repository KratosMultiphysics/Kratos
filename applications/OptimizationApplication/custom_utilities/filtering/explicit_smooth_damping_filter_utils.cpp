...
// Include base h
#include "explicit_smooth_damping_filter_utils.h"

namespace Kratos {

namespace ExplicitFilterWithIntegratedDampingUtilsHelperUtilities
{
...
template<class TContainerType>
ExplicitSmoothDampingFilterUtils<TContainerType>::ExplicitSmoothDampingFilterUtils(
...
template<class TContainerType>
void ExplicitSmoothDampingFilterUtils<TContainerType>::SetFilterRadius(const ContainerExpression<TContainerType>& rContainerExpression)
{
...
template<class TContainerType>
void ExplicitSmoothDampingFilterUtils<TContainerType>::ExplicitSmoothDampingFilterUtils::Update()
{
    KRATOS_TRY

    BuiltinTimer timer;

    KRATOS_INFO("ExplicitSmoothDampingFilterUtils") << "Creating search tree to perform mapping..." << std::endl;

    const auto& r_container = ExplicitFilterWithIntegratedDampingUtilsHelperUtilities::GetContainer<TContainerType>(mrModelPart);
    ...
    mpSearchTree =  Kratos::make_shared<ExplicitSmoothDampingFilterUtils::KDTree>(mEntityPointVector.begin(), mEntityPointVector.end(), mBucketSize);

    // now make the tree for fixed model part
    if (mpFixedModelPart){
        const auto& r_f_container = ExplicitFilterWithIntegratedDampingUtilsHelperUtilities::GetContainer<TContainerType>(*mpFixedModelPart);
        ...
        mpFixedModelPartSearchTree =  Kratos::make_shared<ExplicitSmoothDampingFilterUtils::KDTree>(mFixedModelPartEntityPointVector.begin(), mFixedModelPartEntityPointVector.end(), mBucketSize);
    }
    ...
    KRATOS_INFO("ExplicitSmoothDampingFilterUtils") << "Search tree created in: " << timer.ElapsedSeconds() << " s" << std::endl;
...
ContainerExpression<TContainerType> ExplicitSmoothDampingFilterUtils<TContainerType>::GenericFilterField(const ContainerExpression<TContainerType>& rContainerExpression) const
{
    KRATOS_TRY
    ...
    IndexPartition<IndexType>(r_container.size()).for_each(TLS(mMaxNumberOfNeighbors), [&](const IndexType Index, TLS& rTLS){
        ...
        ExplicitFilterWithIntegratedDampingUtilsHelperUtilities::ComputeWeightForAllNeighbors<EntityType, TWeightIntegrationType>(
...
template<class TContainerType>
ContainerExpression<TContainerType> ExplicitSmoothDampingFilterUtils<TContainerType>::FilterField(const ContainerExpression<TContainerType>& rContainerExpression) const
{
    return GenericFilterField<ExplicitFilterWithIntegratedDampingUtilsHelperUtilities::IntegratedWeight>(rContainerExpression);
}

template<class TContainerType>
ContainerExpression<TContainerType> ExplicitSmoothDampingFilterUtils<TContainerType>::FilterIntegratedField(const ContainerExpression<TContainerType>& rContainerExpression) const
{
    return GenericFilterField<ExplicitFilterWithIntegratedDampingUtilsHelperUtilities::NonIntegratedWeight>(rContainerExpression);
}

template<class TContainerType>
void ExplicitSmoothDampingFilterUtils<TContainerType>::GetIntegrationWeights(ContainerExpression<TContainerType>& rContainerExpression) const
{
    ...
    IndexPartition<IndexType>(r_container.size()).for_each([&](const IndexType Index){
        ...
        const auto integration_weight = ExplicitFilterWithIntegratedDampingUtilsHelperUtilities::GetDomainSize(entity, this->mpNodalDomainSizeExpression.get());
        const IndexType current_data_begin = Index * stride;
        for (IndexType j = 0; j < stride; ++j) {
            double& current_index_value = *(p_expression->begin() + current_data_begin + j);
...
template<class TContainerType>
std::string ExplicitSmoothDampingFilterUtils<TContainerType>::Info() const
{
    std::stringstream msg;

    msg << "ExplicitSmoothDampingFilterUtils: ";
...
#define KRATOS_INSTANTIATE_EXPLICIT_FILTER_METHODS(CONTAINER_TYPE)                                                                                                                                                      \
    template class ExplicitSmoothDampingFilterUtils<CONTAINER_TYPE>;                                                                                                                                                                        \
    template ContainerExpression<CONTAINER_TYPE> ExplicitSmoothDampingFilterUtils<CONTAINER_TYPE>::GenericFilterField<ExplicitFilterWithIntegratedDampingUtilsHelperUtilities::IntegratedWeight>(const ContainerExpression<CONTAINER_TYPE>&) const;     \
    template ContainerExpression<CONTAINER_TYPE> ExplicitSmoothDampingFilterUtils<CONTAINER_TYPE>::GenericFilterField<ExplicitFilterWithIntegratedDampingUtilsHelperUtilities::NonIntegratedWeight>(const ContainerExpression<CONTAINER_TYPE>&) const;
