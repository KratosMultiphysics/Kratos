...
// Project includes
#include "co_sim_io_cpp.h"
#include "co_sim_file_comm.h"


namespace CoSim {

CoSimIO::CoSimIO(const std::string& rName, const std::string& rSettingsFileName, const bool IsConnectionMaster)
    : CoSimIO::CoSimIO(rName, Internals::ReadSettingsFile(rSettingsFileName), IsConnectionMaster) { } // forwarding constructor call

CoSimIO::CoSimIO(const std::string& rName, SettingsType rSettings, const bool IsConnectionMaster)
{
    Initialize(rName, rSettings, IsConnectionMaster);
}

template<>
bool CoSimIO::Import(DataContainers::Data& rContainer, const std::string& rIdentifier)
{
    return mpComm->Import(rContainer, rIdentifier);
}

template<>
bool CoSimIO::Export(const DataContainers::Data& rContainer, const std::string& rIdentifier)
{
    return mpComm->Export(rContainer, rIdentifier);
}

template<>
bool CoSimIO::Import(DataContainers::Mesh& rContainer, const std::string& rIdentifier)
{
    return mpComm->Import(rContainer, rIdentifier);
}

template<>
bool CoSimIO::Export(const DataContainers::Mesh& rContainer, const std::string& rIdentifier)
{
    return mpComm->Export(rContainer, rIdentifier);
}
...
void CoSimIO::SendControlSignal(const Internals::ControlSignal Signal, const std::string& rIdentifier)
{
    mpComm->SendControlSignal(Signal, rIdentifier);

}
Internals::ControlSignal CoSimIO::RecvControlSignal(std::string& rIdentifier)
{
    return mpComm->RecvControlSignal(rIdentifier);
}


void CoSimIO::RegisterAdvanceInTime(double (*pFuncPtr)(double))
{
    mpAdvInTime = pFuncPtr;
}

void CoSimIO::RegisterInitializeSolutionStep(void (*pFuncPtr)())
{
    mpInitSolStep = pFuncPtr;
}

void CoSimIO::RegisterSolveSolutionStep(void (*pFuncPtr)())
{
    mpSolSolStep = pFuncPtr;
}

void CoSimIO::RegisterFinalizeSolutionStep(void (*pFuncPtr)())
{
    mpFinSolStep = pFuncPtr;
}

void CoSimIO::RegisterImportGeometry(void (*pFuncPtr)(const std::string&))
{
    mpImportGeom = pFuncPtr;
}

void CoSimIO::RegisterExportGeometry(void (*pFuncPtr)(const std::string&))
{
    mpExportGeom = pFuncPtr;
}

void CoSimIO::RegisterImportMesh(void (*pFuncPtr)(const std::string&))
{
    mpImportMesh = pFuncPtr;
}

void CoSimIO::RegisterExportMesh(void (*pFuncPtr)(const std::string&))
{
    mpExportMesh = pFuncPtr;
}

void CoSimIO::RegisterImportData(void (*pFuncPtr)(const std::string&))
{
    mpImportData = pFuncPtr;
}

void CoSimIO::RegisterExportData(void (*pFuncPtr)(const std::string&))
{
...
bool CoSimIO::IsConverged()
{
    std::string dummy("");
    return RecvControlSignal(dummy) == CoSim::Internals::ControlSignal::ConvergenceAchieved;
}

void CoSimIO::Initialize(const std::string& rName, SettingsType& rSettings, const bool IsConnectionMaster)
{
