    ...
    const TObjectType& rObj)
{
    ...
    SerializeToFile(this->GetTempFileName(file_name, mUseAuxFileForFileAvailability), rObj, GetSerializerTraceType());

    info.Set<std::size_t>("memory_usage_ipc", fs::file_size(this->GetTempFileName(file_name, mUseAuxFileForFileAvailability)));
    ...
    const int SizeOfData)
{
    ...
    std::ofstream output_file(this->GetTempFileName(file_name, mUseAuxFileForFileAvailability), std::ios::out|std::ios::binary);
