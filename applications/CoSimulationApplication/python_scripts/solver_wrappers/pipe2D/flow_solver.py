...
class SolverWrapperPipeFlow(CoSimulationComponent):
    ...
    def __init__(self, parameters):
        ...
        axial_offset = self.settings["axial_offset"].GetDouble()  # Start position along axis
        ...
        # Initialization
        self.preference = 0.0  # Reference kinematic pressure
    ...
    def SolveSolutionStep(self, interface_input):
        ...
        # print
        # r = np.sqrt(self.a[1:self.m + 1] / np.pi)
        # self.iter += 1
        # file_name = f"CFD/nodes_update_timestep{self.n}_thread3_Iter{self.iter}.dat"
        # with open(file_name, 'w') as file:
        #     file.write('x-coordinate, y-coordinate\n')
        #     for i in range(len(a)):
        #         file.write(f'{self.z[i]}\t{r[i]}\n')
    ...
    def FinalizeSolutionStep(self):
        ...
        #print
        # a = self.a[1:self.m + 1]
        # file_name = f"CFD2/Area_TS{self.n - 1}"
        # with open(file_name, 'w') as file:
        #     for i in range(len(a)):
        #         file.write(f'{self.z[i]}\t{a[i]}\n')
