    ...
    for i in range(1, it_max):
        ...
        if os.path.exists(file_CF):
            ...
            if b ==1:
                ...
                q = plt.plot(pos[:, 0], pos[:, 1], linestyle='--', label=f"CoCoNuT_x{i}")
        ...
        if os.path.exists(file_CA):
            pos = np.array(pd.read_csv(file_CA, header=None, skiprows=1, sep="\s+"))+pos0_CoCo
            plt.figure(4 * t + 1)
            pos=pos[:,[1,0]]
            sort_id = np.argsort(pos[:, 0])
            pos = pos[sort_id, :]
            if b2 == 1:
                ...
                plt.plot(pos[:, 0], pos[:, 1], linestyle='-', label=f"CoCoNuT_xt{i}",color=q[0].get_color())
        plt.figure(4*t+1)
        leg = plt.legend()
        # leg.draggable()
        # plt.figure(4*t+2)
