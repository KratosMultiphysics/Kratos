    ...
    )
{
    ...
    // Getting variables
    std::vector<Variable<double>*> origin_list_variables, destination_list_variables;
    ...
    for (IndexType i_var = 0; i_var < origin_variables_names.size(); ++i_var) {
        if (KratosComponents<Variable<double>>::Has(origin_variables_names[i_var])) {
            origin_list_variables.push_back(&KratosComponents<Variable<double>>::Get(origin_variables_names[i_var]));
            destination_list_variables.push_back(&KratosComponents<Variable<double>>::Get(destination_variables_names[i_var]));
            ...
            for (unsigned int i_comp = 0; i_comp < 3; ++i_comp) {
                origin_list_variables.push_back(&KratosComponents<Variable<double>>::Get(r_origin_var_name + "_" + direction_string[i_comp]));
                destination_list_variables.push_back(&KratosComponents<Variable<double>>::Get(r_destination_var_name + "_" + direction_string[i_comp]));
            }
        } else {
    ...
    block_for_each(rModelPart3D.Elements(), [&](Element& rElement) {
        ...
        while (number_points_found == 0) {
            ...
            const PointGeometry point(center.X(), center.Y(), center.Z());
        ...
        for (IndexType i = 0; i < number_points_found; ++i) {
            ...
            if (r_geometry.IsInside(rNode.Coordinates(), aux_coordinates)) {
                ...
                for (std::size_t i_var = 0; i_var < origin_list_variables.size(); ++i_var) {
                    ...
                    const double origin_value = inner_prod(N, r_geometry.FastGetSolutionStepValue(*origin_list_variables[i_var]));
