        ...
        Node<3>::Pointer p_node)
{
    ...
    const double& radius         = p_node->FastGetSolutionStepValue(RADIUS);
    const double particle_volume = 4.0 * Globals::Pi_3 * mParticlesPerDepthDistance * radius * radius * radius;
    ...
    const double averaging_volume_inv)
{
    ...
    if (vector_size && p_node->Is(INSIDE)){
        const double& radius = p_node->FastGetSolutionStepValue(RADIUS);
        double solid_volume = 4.0 * Globals::Pi_3 * radius * radius * radius;
