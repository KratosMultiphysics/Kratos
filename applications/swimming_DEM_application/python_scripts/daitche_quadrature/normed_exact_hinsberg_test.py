...
def CalculateErrors(points_set, pp):
    ExactIntegral = quad.ExactIntegrationOfSinus(pp.final_time, pp.initial_time)
    ...
    for m in points_set.exponential_indices:
        ...
        F_tail = quad.ExactIntegrationOfTail(pp.final_time, pp.final_time_minus_tw, pp.initial_time, points_set.AsTs[m][0], tis)
...
def GetMaxTime(max_a, min_b, m, pp):
        from scipy.special import lambertw
        t_intersection = (2 * min_b * pp.final_time - lambertw(2 * min_b / ((m + 1) * min_a) ** 2)) / (2 * min_b)
...
for phase in phases:
    ...
    print('----------------------------------------------')
    pp.final_time = pp.initial_number_of_periods * 2 * math.pi + phase
