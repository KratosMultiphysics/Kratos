...
def CalculateErrors(points_set, pp):
    ...
    for m in points_set.exponential_indices:
        ...
        error_bound = ObjectiveFunction(points_set, m, pp) / ObjectiveFunctionInfinity(points_set, m, pp)
...
def ObjectiveFunction(points_set, m_index, pp):
    return 1.0 / math.sqrt(pp.t_w) * (abs(1 - K(points_set, m_index, 1, pp)) + PerformQuadratureOfObjectiveFunctionSecondTerm(points_set, m_index, 1, pp.ND_final_time)[0])   
...
for phase in phases:
    ...
    pp.initial_time = pp.final_time - 0.1
    pp.Delta_t = pp.final_time - pp.initial_time
    k_calc = 1    
    k_max = len(t_ws)
    ...
    for t_w in t_ws:
        ...
        print('t_w = ', t_w)
        pp.t_w = t_w
        pp.final_time_minus_tw = pp.final_time - t_w
        pp.ND_final_time = (pp.final_time -  pp.initial_time) / t_w
