...
for i_plot, ref_error in enumerate(ref_errors):
    ...
    plt.plot(list(range(11)), max_t_w_t_norm, '-o', color = 'b', ms = 20, linewidth = 3, label = '$I_{2t}$')
    plt.plot(list(range(11)), max_t_w_abs_norm, '-*', color = 'k', ms = 20, linewidth = 3, label = '$I_{1}$')
    plt.plot([10], max_t_w_hinsberg, 'x', color = 'brown', mew = 5, ms = 20, linewidth = 3, label = '$I_{2tH}$')
    plt.semilogy()
    # plt.legend(loc = 'upper right', prop={'size':22.5},frameon=False)
    plt.xlabel('$m$', fontsize = '30')
    percentage = '%g' % (100 * ref_error)
    plt.ylabel('$\\tilde{{t}}_{{w,{p}\%}}$'.format(p = percentage), fontsize = '30')
    ax = plt.gca()
    ...
    # plt.close()
file_name = 'non_dimensional_tw_for_x_percent_error.pdf'
