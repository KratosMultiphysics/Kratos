...
# import buildings
obj_file_in = "data/buildings/buildings_100.obj"
...
### COPY BUILDINGS ###
num_building = 2
ID_vertex = building_model_part.NumberOfNodes() + 1
ID_elem = building_model_part.NumberOfElements() + 1
for copy_y in range(2, 10):
	inc_y = 4 * (copy_y-1)
	# copy along x axis
	for copy_x in range(2, 10):
		inc_x = 4 * (copy_x-1)
		# inc_y = 0.0
		name_sub_model_building = "Building_{}".format(num_building)
		num_building += 1	# we update the number of building
		current_sub_model_building = building_model_part.CreateSubModelPart(name_sub_model_building)
		for node in building_model_part.GetSubModelPart("Building_1").Nodes:
			# ID_vertex = node.Id + (8 * (copy_x-1))
			building_model_part.CreateNewNode(ID_vertex, node.X+inc_x, node.Y+inc_y, node.Z)
			ID_vertex += 1
		for face in building_model_part.GetSubModelPart("Building_1").Elements:
			# ID_elem = face.Id + (10 * (copy_x-1))
			nodes_elem = face.GetNodes()
			n1 = nodes_elem[0].Id + (8 * num_building) + 1
			n2 = nodes_elem[1].Id + (8 * num_building) + 1
			n3 = nodes_elem[2].Id + (8 * num_building) + 1
			current_sub_model_building.AddNodes([n1, n2, n3])
			current_sub_model_building.CreateNewElement("Element2D3N", ID_elem, [n1, n2, n3], building_model_part.GetProperties()[2])
			ID_elem += 1

print(building_model_part)

importer.CreateGidControlOutput("cfd_data/test_{}/gid_file/04_buildings".format(num_test))
