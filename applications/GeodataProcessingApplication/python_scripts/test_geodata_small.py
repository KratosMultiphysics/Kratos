...
from geo_debug import GeoDebug
...
if import_terrain_mdpa:
	...
	model_part_folder = "data/mdpa_file/test_geodata_small/"
...
debug = GeoDebug()
debug.SetGeoModelPart(building.GetGeoModelPart())
...
model.SetName(num_test)

stop_03 = time.time()
# model.FillNoslip("SKIN_ISOSURFACE")
model.NoSlip("SKIN_ISOSURFACE")
stop_03_1 = time.time()
print("[DEBUG PY]\t-> SKIN_ISOSURFACE filled in ", (stop_03_1-stop_03))

# model.FillSlip("TopModelPart")
model.Slip("TopModelPart")
stop_03_2 = time.time()
print("[DEBUG PY]\t-> TopModelPart filled in ", (stop_03_2-stop_03_1))

# model.FillSlip("BottomModelPart")
model.Slip("BottomModelPart")
stop_03_3 = time.time()
print("[DEBUG PY]\t-> BottomModelPart filled in ", (stop_03_3-stop_03_2))

# model.FillPartsFluid("Parts_Fluid")
model.PartsFluid("Parts_Fluid")
stop_03_4 = time.time()
print("[DEBUG PY]\t-> Parts_Fluid filled in ", (stop_03_4-stop_03_3))


# NOW IT IS WITH OLY ONE DIRECTION.
# TODO: IN THE FUTURE IT WILL ALSO BE POSSIBLE TO CHOOSE ALL DIRECTIONS
direction = 10	# from South
n_sectors = 12
model.Inlet_Outlet(n_sectors, direction)
stop_03_5 = time.time()
print("[DEBUG PY]\t-> Inlet/Outlet filled in ", (stop_03_5-stop_03_4))


# model.FillInlet("Inlet")
# model.Inlet("Inlet")
# stop_03_4 = time.time()
# print("[DEBUG PY]\t-> Inlet filled in ", (stop_03_4-stop_03_3))

# model.FillOutlet("Outlet")
# model.Outlet("Outlet")
# stop_03_5 = time.time()
# print("[DEBUG PY]\t-> Outlet filled in ", (stop_03_5-stop_03_4))

# print("\n[DEBUG PY] Filled time: {}\n".format(stop_03_5-stop_03))

print("\n[DEBUG PY] GeoCfdModelPart AFTER\n", model.GetGeoCfdModelPart())
# print("\n[DEBUG PY] GeoCfdModelPart AFTER\n", model.GetGeoModelPart())

# we write the GiD file
model.CreateGidControlOutput("cfd_data/test_{}/gid_file/Ascii/12_CFD_model".format(num_test), "GiD_PostAscii")
model.CreateGidControlOutput("cfd_data/test_{}/gid_file/Binary/12_CFD_model".format(num_test), "GiD_PostBinary")
# writing file mdpa
...
model.cfd_param["solver_settings"]["model_import_settings"]["input_filename"].SetString("12_CFD_model")

# store json in a file
json_name = "cfd_data/test_{}/analysis_file/ProjectParameters.json".format(num_test)
with open(json_name, "w") as outfile:
	outfile.write(model.cfd_param.PrettyPrintJsonString())
print("[DEBUG PY]\t-> JSON stored!")
