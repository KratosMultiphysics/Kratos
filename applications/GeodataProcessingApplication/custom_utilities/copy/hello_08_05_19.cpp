    ...
    for (auto& node : VolumeModelPart.Nodes()){
        ...
        for (auto& elem : GeometryModelPart.Elements()){
            ...
            */
            ...
            {
				// for (int i = 0; i < 3; i++) {
                //     Node<3>& i_node = elem.GetGeometry()[i];
                //     double dist = (i_node[2] - pnt_inter[2]);
                //     KRATOS_INFO("i_node[2]") << i_node[2] << std::endl;
                //     KRATOS_INFO("pnt_inter[2]") << pnt_inter[2] << std::endl;
                //     i_node.GetSolutionStepValue(DISTANCE) = dist;
                // }
