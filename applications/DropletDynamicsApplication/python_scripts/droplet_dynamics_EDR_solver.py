...
class DropletDynamicsSolver(PythonSolver):  # Before, it was derived from NavierStokesTwoFluidsSolver
    ...
    def Initialize(self):
        ...
        self.EDRProcess = self._CreateEllipticDistanceReinitialization()
...
# 
        ...
        if (TimeStep % 1 == 0):
            ...
            distanceAux_max=-1.0e+12
            distanceAux_min=1.0e+12
            for node in self.main_model_part.Nodes:
                distance = node.GetSolutionStepValue(KratosDroplet.DISTANCE_AUX2)            
                if (distance<distanceAux_min):
                    distanceAux_min = distance
                elif (distance>distanceAux_max):
                    distanceAux_max = distance
            ...
            print("DISTANCE_min=",distance_min, ", DISTANCE_max=",distance_max,", DISTANCE_AUX2_min=",distanceAux_min, ", DISTANCE_AUX2_max=",distanceAux_max)
            (self.EDRProcess).Execute()        
            for node in self.main_model_part.Nodes:
        ...
        KratosMultiphysics.Logger.PrintInfo(self.__class__.__name__, "ConsistentNodalPressureGradientProcess is finished.")
        ...
        self._GetDistanceModificationProcess().ExecuteInitializeSolutionStep()
        KratosMultiphysics.Logger.PrintInfo(self.__class__.__name__, "DistanceModificationProcess is finished.")
        # Update the DENSITY and DYNAMIC_VISCOSITY values according to the new level-set
        self._SetNodalProperties()
        KratosMultiphysics.Logger.PrintInfo(self.__class__.__name__, "SetNodalProperties is finished.")
        # Initialize the solver current step
        self._GetSolutionStrategy().InitializeSolutionStep()
        KratosMultiphysics.Logger.PrintInfo(self.__class__.__name__, "GetSolutionStrategy is finished.")
    ...
    def SolveSolutionStep(self):
        KratosMultiphysics.Logger.PrintInfo(self.__class__.__name__, "SolveSolutionStep")
        is_converged = self._GetSolutionStrategy().SolveSolutionStep()
        KratosMultiphysics.Logger.PrintInfo(self.__class__.__name__, "SolveSolutionStep finished")
        if not is_converged:
