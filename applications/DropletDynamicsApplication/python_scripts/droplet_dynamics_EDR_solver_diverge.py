...
def CreateSolver(model, custom_settings):
    return DropletDynamicsEDRSolver(model, custom_settings)

class DropletDynamicsEDRSolver(PythonSolver):  # Before, it was derived from NavierStokesTwoFluidsSolver
    ...
    def GetDefaultParameters(cls):
        ...
        default_settings.AddMissingParameters(super(DropletDynamicsEDRSolver, cls).GetDefaultParameters())
    ...
    def __init__(self, model, custom_settings):
        ...
        super(DropletDynamicsEDRSolver,self).__init__(model, custom_settings)
    ...
    def InitializeSolutionStep(self):
        ...
        #     self._GetDistanceSmoothingProcess().Execute()
        #     for node in self.main_model_part.Nodes:
        #         smooth_distance = node.GetSolutionStepValue(KratosDroplet.DISTANCE_AUX)
        ...
        # for node in self.main_model_part.Nodes:
        #     old_distance = node.GetSolutionStepValue(KratosMultiphysics.DISTANCE)
        #     node.SetValue(KratosMultiphysics.AUX_DISTANCE, old_distance)
        ...
        if (TimeStep % 4 == 0):
            self._GetEllipticDistanceReinitialization().Execute()
            for node in self.main_model_part.Nodes:
                smooth_distance = node.GetSolutionStepValue(KratosDroplet.DISTANCE_AUX2)
                node.SetSolutionStepValue(KratosMultiphysics.DISTANCE, smooth_distance)
    ...
    def _GetEllipticDistanceReinitialization(self):
        if not hasattr(self, '_elliptic_distance_reinitialization'):
            self._elliptic_distance_reinitialization = self._CreateEllipticDistanceReinitialization()
        return self._elliptic_distance_reinitialization
    ...
    def _CreateEllipticDistanceReinitialization(self):
        ...
        elliptic_distance_reinitialization = KratosDroplet.EllipticDistanceReinitialization(
        ...
        return elliptic_distance_reinitialization
        ...
        if (self._reinitialization_type == "variational"):
            ...
            if self.main_model_part.ProcessInfo[KratosMultiphysics.DOMAIN_SIZE] == 2:
                ...
                distance_reinitialization_process = KratosMultiphysics.ParallelDistanceCalculationProcess3D(
                    self.main_model_part,
                    parallel_distance_settings)
        elif (self._reinitialization_type == "elliptic"):
