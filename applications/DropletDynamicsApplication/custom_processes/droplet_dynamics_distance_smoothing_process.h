...
class KRATOS_API(FLUID_DYNAMICS_APPLICATION) DropletDynamicsDistanceSmoothingProcess : public Process
{
public:
    ///@name Type Definitions
    ///@{
    ...
    KRATOS_CLASS_POINTER_DEFINITION(DropletDynamicsDistanceSmoothingProcess);
    ...
    DropletDynamicsDistanceSmoothingProcess(
    ...
    /// Destructor.
    ~DropletDynamicsDistanceSmoothingProcess() override
    {
        Clear();
    }
    ...
    {
        KRATOS_TRY;
        KRATOS_INFO("SurfaceSmoothingProcess") << "Execute: Start." << std::endl;
        if(mAuxModelPartIsInitialized == false){
            CreateAuxModelPart();
        }
        KRATOS_INFO("SurfaceSmoothingProcess") << "Execute: GetModelPart." << std::endl;
        auto& r_smoothing_model_part = mrModel.GetModelPart( mAuxModelPartName );

        const double scale = 1.0e1;
        KRATOS_INFO("SurfaceSmoothingProcess") << "Execute: Set Nodal_Area." << std::endl;
        ...
        block_for_each(r_smoothing_model_part.Nodes(), [&](Node& rNode){
                ...
                rNode.FastGetSolutionStepValue(DISTANCE, 1) = scale*distance;
                rNode.Set(BOUNDARY,false);
            });
        KRATOS_INFO("SurfaceSmoothingProcess") << "Execute: Set Geometry BOUNDARY." << std::endl;
...
private:
    ...
    void CreateAuxModelPart()
    {
        ...
        KRATOS_INFO("SurfaceSmoothingProcess") << "Create smoothing modelpart: Start." << std::endl;
        if(mrModel.HasModelPart( mAuxModelPartName ))
            mrModel.DeleteModelPart( mAuxModelPartName );
        ...
        KRATOS_INFO("SurfaceSmoothingProcess") << "Create smoothing modelpart: End." << std::endl;
    ...
    /// Default constructor.
    DropletDynamicsDistanceSmoothingProcess() = delete;

    /// Assignment operator.
    DropletDynamicsDistanceSmoothingProcess& operator=(DropletDynamicsDistanceSmoothingProcess const& rOther) = delete;

    /// Copy constructor.
    DropletDynamicsDistanceSmoothingProcess(DropletDynamicsDistanceSmoothingProcess const& rOther) = delete;
