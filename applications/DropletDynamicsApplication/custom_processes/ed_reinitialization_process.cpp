...
namespace Kratos
{
    ...
    void EDReinitializationProcess::CheckDefaultsAndProcessSettings(Parameters &rParameters)
    {
        Parameters default_parameters(R"(
    {
        ...
        mReconsctruction = rParameters["recunstruction"].GetBool();
        mOptimization = rParameters["optimization"].GetBool();
        mOptimizationIterations = rParameters["maximum_iterations"].GetInt();
        mOptimizationTolerance = rParameters["max_grad_norm_deviation"].GetDouble();
    }
...
#pragma omp parallel for
        ...
        if (mOptimization)
        {
            ...
            while (max_grad_norm_deviation > mOptimizationTolerance && iteration < mOptimizationIterations)
            {
                mpGradientCalculator->Execute();
...
#pragma omp critical
                ...
                KRATOS_INFO("Deviation in the norm of distance gradient") << norm_grad_norm_deviation << std::endl;
