...
#if !defined(KRATOS_TOPOLOGY_MESH_EXTRACTOR_UTILITIES_H_INCLUDED)
#define  KRATOS_TOPOLOGY_MESH_EXTRACTOR_UTILITIES_H_INCLUDED
...
class TopologyMeshExtractorUtilities
{
public:
	...
	/// Pointer definition of TopologyMeshExtractorUtilities
	KRATOS_CLASS_POINTER_DEFINITION(TopologyMeshExtractorUtilities);
	...
	/// Default constructor.
	TopologyMeshExtractorUtilities( ModelPart& rStructureModelPart )
	: mr_structure_model_part(rStructureModelPart)
	{
	}

	/// Destructor.
	virtual ~TopologyMeshExtractorUtilities()
	{
	}
	...
	void ExtractVolumeMesh(double threshold, ModelPart& rExtractedModelPart)
	{
		...
		rExtractedModelPart.Nodes() = mr_structure_model_part.Nodes();

		// Extracting mesh elements which are only above the threshold value
		std::cout<<"  Extracting elements > " << threshold <<std::endl;
		Element::Pointer pElem;
		for(ModelPart::ElementsContainerType::iterator it = mr_structure_model_part.ElementsBegin(); it != mr_structure_model_part.ElementsEnd(); ++it)
		{
	...
	/// For Topology Optimization purposes: Generates a .STL file from an extracted surface mesh
	void WriteSTLFile(const char* file_name, ModelPart& rExtractedSurfaceModelPart)
	{
		KRATOS_TRY;

		std::cout<<"\n::[Generating STL file]::"<<std::endl;

		// Write stl of skin model part
		std::ofstream myfile;
	...
	{
		return "TopologyMeshExtractorUtilities";
	}

	/// Print information about this object.
	virtual void PrintInfo(std::ostream& rOStream) const
	{
		rOStream << "TopologyMeshExtractorUtilities";
	}
...
private:
	...
	/// Assignment operator.
	//TopologyMeshExtractorUtilities& operator=(TopologyMeshExtractorUtilities const& rOther);

	/// Copy constructor.
	//TopologyMeshExtractorUtilities(TopologyMeshExtractorUtilities const& rOther);
...
#endif	/* KRATOS_TOPOLOGY_MESH_EXTRACTOR_UTILITIES_H_INCLUDED */
