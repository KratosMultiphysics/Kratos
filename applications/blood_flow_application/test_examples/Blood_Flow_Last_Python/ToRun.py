...
initial_pressure = config.systolic_pressure  # Pa

Coupled_Simulation = False  # True-->Coupled_3d_1d False-->Only 1D
...
step_size = 0.0001  # config.step_size
...
if (artery_type == 5):
    ...
    if (alas == 1):
        input_file_name = relative_path_1D + "Alastruey/alastruey_new_coronaries"
        print("1D Arterial model:::>>> ", input_file_name)
...
# Initial Values. Set the intial pressure as reference for the 1D model (all nodes take the systolic pressure and the initial flow)
for node in model_part1D.Nodes:
    if(node.IsFixed(FLOW) == False):
        ...
        node.SetSolutionStepValue(SYSTOLIC_PRESSURE, 0, initial_pressure)
    else:
        node.SetSolutionStepValue(FLOW, 0, q_inicial)
        node.SetSolutionStepValue(PRESSURE, 0, initial_pressure)
        node.SetSolutionStepValue(SYSTOLIC_PRESSURE, 0, initial_pressure)
...
if (Coupled_Simulation):
    ...
    transfer_obj = CouplingTools1D_3Dv3.TransferTools(
...
while(total_time < final_time):
    ...
    if ((Coupled_Simulation) and (control_sub_step == sub_step) and (nro_cardiac_cycle == cardiac_cycle_to_3D)):
        if(step >= 3):
            print("...............................Solve 3D..................................")
            ...
            integrator.SolveStep(model_part1D)
            integrator.ComputePressure(model_part1D, initial_pressure)
    ...
    if(math.fmod(step, output_step) == 0):
        if(plot3d == False):
        # removal_tool.ComputePressure(model_part1D)
            integrator.ComputePressure(model_part1D, initial_pressure)  # Only when I saved the results.
