...
def Factory(settings, model):
    ...
    return SWOutputProcess(model, settings["Parameters"])

class SWOutputProcess(KM.OutputProcess):
    """SWOutputProcess
    ...
    def __init__(self, model, settings):
        """The constructor of the SWOutputProcess"""
        ...
        if self.volume_model_part.ProcessInfo[KM.DOMAIN_SIZE] == 2:
            self.sw_output_process = SW.SWProcess2D(model, self._CreateIntegrationParameters())
        else:
            self.sw_output_process = SW.SWProcess3D(model, self._CreateIntegrationParameters())
        self.hdf5_process = single_mesh_temporal_output_process.Factory(self._CreateHDF5Parameters(), model)


    def Check(self):
        '''Check the processes.'''
        self.sw_output_process.Check()
    ...
    def ExecuteBeforeSolutionLoop(self):
        ...
        self.sw_output_process.Execute()
