...
const Parameters SWProcess<TDim>::GetDefaultParameters() const
{
...
template<std::size_t TDim>
void SWProcess<TDim>::GetBoundingVolumeLimits(double& rMin, double& rMax)
{
...
template<std::size_t TDim>
void SWProcess<TDim>::ReadAndSetValues(
    NodeType& rNode,
...
template<std::size_t TDim>
array_1d<double,3> SWProcess<TDim>::InterpolateVelocity(
    const Element::Pointer pElement,
    const Vector& rShapeFunctionValues) const
{
    KRATOS_DEBUG_ERROR_IF(pElement->GetGeometry().size() != rShapeFunctionValues.size()) << "DSWProcess: check the found element!" << std::endl;
    array_1d<double,3> velocity = ZeroVector(3);
    int n = 0;
...
template<std::size_t TDim>
void SWProcess<TDim>::FindBoundaryNeighbors()
{
...
template<std::size_t TDim>
void SWProcess<TDim>::CopyValues(const NodeType& rOriginNode, NodeType& rDestinationNode)
{
...
template class SWProcess<2>;
template class SWProcess<3>;
