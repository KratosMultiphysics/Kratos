...
public:
    ...
    {
        KRATOS_TRY
        return boost::make_shared< EulerPrimVarElement < TNumNodes > >(NewId, this->GetGeometry().Create(rThisNodes), pProperties);
        KRATOS_CATCH("")
    }
    ...
    {
        KRATOS_TRY
        return boost::make_shared< EulerPrimVarElement < TNumNodes > >(NewId, pGeom, pProperties);
        KRATOS_CATCH("")
    }
