...
//
#if !defined(KRATOS_NM_POINTS_MAPPER_H_INCLUDED )
#define  KRATOS_NM_POINTS_MAPPER_H_INCLUDED
...
//template<class TTree, class TVar>
    ...
    public:
		...
		//cosntructor
		NMPointsMapper(ModelPart& dest_model_part, ModelPart& origin_model_part);
		//!!!!!!!!!!!!!!!!!!!!!!
		~NMPointsMapper();
		template<class TDataType>
		void ScalarMap(const Variable<TDataType>& rOriginVariable, const Variable<TDataType>& rDestinationVariable, int max_iter, double tol_iter)
		{
				...
				for(int i=0;i<n;i++)
				{
					...
					//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					//we set destinationVarible to zero, remove this later, it is the start value of iteration
					//better could be the already existing value
				...
				for(int i=0;i<n;i++)
				{
					...
					//std::cout<<std::endl<<"GP index: "<<i<<std::endl;
					...
					//std::cout<<"local coordinates PP:  "<<OriginProjectionPoints[i][3]<<"  " <<OriginProjectionPoints[i][4]<<"  "<<std::endl;
					//std::cout<<"OriginScalarValue: "<<OriginScalarValue[i]<<std::endl<<std::endl;
				}
				...
				for(int k=0;k<max_iter;k++) //Iteration
				{
					...
					for(int i=0;i<n;i++)	
					{
							...
							p_orig[0]=DestArea[i]*(0.33333333333)*OriginScalarValue[i]; p_orig[1]=p_orig[0]; p_orig[2]=p_orig[0];
				...
				if((norm_dx/inodes)>pow(tol_iter,2))
					std::cout<<" Convergence criteria not achieved wihin "<<iter<<" Iterations."<<std::endl;
		...
		void VectorMap(const Variable<TDataType>& rOriginVariable, const Variable<TDataType>& rDestinationVariable, int max_iter, double tol_iter)
		{
				...
				//later, if we have a template class, make this outside of function, because we don´t want to 
				//create arrays in each timestep and take TVector
					...
					for(int i=0;i<n;i++)	
						{
							...
							p_orig=DestArea[i]*(0.3333333)*OriginVectorValue[i];
							
							//M_id*=DestArea[i]/(12.0);

							noalias( DestinationConds[i]->GetGeometry()[0].GetValue(VAUX) ) += //loc_RHS_0;
								p_orig - DestArea[i]/(12.0)*(prod(2.0*M_id, x_vect_0) + prod(M_id, x_vect_1) + prod(M_id, x_vect_2) );
							noalias( DestinationConds[i]->GetGeometry()[1].GetValue(VAUX) ) += //loc_RHS_1;
								p_orig - DestArea[i]/(12.0)*(prod(M_id, x_vect_0) + prod(2.0*M_id, x_vect_1) + prod(M_id, x_vect_2) ); 
							noalias( DestinationConds[i]->GetGeometry()[2].GetValue(VAUX) ) += //loc_RHS_2;
								 p_orig - DestArea[i]/(12.0)*(prod(M_id, x_vect_0) + prod(M_id, x_vect_1) + prod(2.0*M_id, x_vect_2) );
						}
					//////////////////////////////////////////////////////////////
					norm_dx=0.0;
					inodes=0;
