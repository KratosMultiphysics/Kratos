...
def print_results(dt):
    ...
    gnuplot_outfile1.write("set grid; set key center right; set xlabel 'Time (us)'; set ylabel 'Normal contact force (KN)'; plot [0:60][0:12] 'variables_for_node_1.dat' every 20 u (1e6*$1):(1e-3*$9) w lp ls 1 ps 1.5 pt 5 t 'DEM',\\\n")
    ...
    gnuplot_outfile2.write("set grid; set key center right; set xlabel 'Normal contact displacement (um)'; set ylabel 'Normal contact force (KN)'; plot [0:400][0:12] 'variables_for_node_1.dat' every 20 u (-2e6*$3):(1e-3*$9) w lp ls 1 ps 1.5 pt 5 t 'DEM',\\\n")
...
DEMAnalysisStageForChungOoiTest1(Model(), parameters).Run()
