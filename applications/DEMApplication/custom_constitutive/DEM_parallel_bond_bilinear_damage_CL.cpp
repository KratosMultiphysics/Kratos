...
void DEM_parallel_bond_bilinear_damage::Check(Properties::Pointer pProp) const {
    ...
    if (!pProp->Has(FRACTURE_ENERGY)) {
        KRATOS_WARNING("DEM")<<"\nWARNING: Variable FRACTURE_ENERGY was not found in cthe Properties when using DEM_parallel_bond_bilinear_damage. A default value of 0.0 was assigned.\n\n";
  ...
  //**********************************************
    ...
    if (!failure_type){ //if the bond is not broken
        ...
        if (bonded_indentation <= 0.0){ // in tension-shear mode
            ...
            const double delta_at_failure_point_tangential = (2.0 * fracture_energy) / bond_tau_zero;
            ...
            if (!fracture_energy && !(*mpProperties)[IS_UNBREAKABLE]){ // Fully fragile behaviour
