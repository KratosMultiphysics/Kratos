...
class DEMPropertiesMeasureUtility:
    ...
    def MeasureTotalBondBrokenRatio(self):
        return self.SphericElementGlobalPhysicsCalculator.CalculateTotalBondBrokenRatio(self.spheres_model_part)
    ...
    def MeasureGlobalConductivityTensor(self, Lx, Ly, Lz):
        if self.DEM_parameters["ContactMeshOption"].GetBool():
            ...
            return measured_non_homogenized_conductivity_tensor, conductivity_tensor_trace
        else:
            raise Exception('The \"PostStressStrainOption\" and \"ContactMeshOption\" in the [ProjectParametersDEM.json] should be [True].')
