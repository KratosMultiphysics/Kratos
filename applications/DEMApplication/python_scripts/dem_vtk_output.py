...
class VtkOutput():
    def __init__(self, main_path, problem_name, spheres_model_part, contact_model_part, rigid_face_model_part, DEM_parameters):
        ...
        if self.PostLocalContactForce:
            self.local_contact_force_X = np.empty(0)
            self.local_contact_force_Y = np.empty(0)
            self.local_contact_force_Z = np.empty(0)

        if self.PostFailureCriterionState:
            self.failure_criterion_state = np.empty(0)

        if self.PostContactFailureId:
            self.contact_failure_id = np.empty(0)

        if self.PostContactTau:
            self.contact_tau = np.empty(0)

        if self.PostContactSigma:
            self.contact_sigma = np.empty(0)

        if self.PostMeanContactArea:
            self.mean_contact_area = np.empty(0)
