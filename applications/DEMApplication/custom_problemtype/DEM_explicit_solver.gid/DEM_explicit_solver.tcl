...
proc DEM::SetSphereAndCircleNodes { } {
...
proc DEM::WriteSphereAndCircleNodes { fd } {
    variable sphere_and_circle_nodes
...
proc DEM::WriteNonSphereAndCircleNodes { fd } {
    variable sphere_and_circle_nodes
...
proc DEM::ReleaseSphereAndCircleNodes { } {
    variable sphere_and_circle_nodes
...
proc DEM::Get_Mean_Radius {} {
    ...
    set  radius_list [lindex [lindex [GiD_Info Mesh Elements $elemtype -array] 0] 4]    
     if { $radius_list == 0 } {
	return "Error: Elements should be Spheres or  Circles"
    }
    set r_average 0
    set r2_average2 0
    set r3_average2 0
    set r_count 0
    foreach r $radius_list {
	set r_average  [expr $r_average + $r]
	set r2 [expr $r *   $r]
	set r2_average2 [expr  $r2_average2 + $r2]
	set r2 [expr $r2 * $r]
	set r3_average2 [expr  $r3_average2 + $r2]
	incr r_count
    }
    set  r_average [expr $r_average/$r_count]
    set  r2_average [expr sqrt($r2_average2/$r_count)]
    set  r3_average [expr ($r3_average2/$r_count)**(1.0/3.0)]
    if { $num_spheres == 0 } {
	set vol [expr $r2_average2*3.1416]
    } else {
	set vol [expr $r3_average2*(4/3)*3.1416]
    }
    return $r_average
    #return $r_average,$r2_average,$r3_average,$vol
}
