...
class MultiCriterion():
    ...
    def __init__(self,**keywordArgs):
        ...
        # Methods
        self._flagFunction =  dynamicImport(keywordArgs.get('flag', 'xmc.methodDefs_multiCriterion.flag.plainFlag'))
...
#        self.__dict__.update(newState)
    ...
    def splitTolerance(self,splittingParameter):
        if self.toleranceToSplit is not None:
            split_tolerances = self.toleranceToSplit*np.array([splittingParameter, 1-splittingParameter])
    ...
    def updateTolerance(self,criteriaToUpdate=None):
        ...
        for coord in criteriaToUpdate:
            if(len(self.criteria[coord].tolerances)==0):
                ValueError('stoppingCriterion.criteria[',coord,'] has no tolerance to update')
