...
def centralMomentErrorTaskWrapper(dimension,order,*args):
    if dimension==0:
        if order==1:
            return computeCentralMomentsErrorEstimationOrderOneDimensionZero_Task(*args)
        elif order==2:
            return computeCentralMomentsErrorEstimationOrderTwoDimensionZero_Task(*args)
        else:
            raise ValueError('Moments of order > 2 are not supported yet for dimension 0.')
    elif dimension==1:
        if order==1:
            return computeCentralMomentsErrorEstimationOrderOneDimensionOne_Task(*args)
        else:
...
def centralCombinedMomentErrorTaskWrapper(dimension,order,*args):
    if dimension==0:
        if order==1:
            return computeCentralMomentsErrorEstimationOrderOneDimensionZero_Task(*args)
        else:
...
def computeCentralMomentsErrorEstimationOrderOneDimensionZero_Task(power_sum_1,power_sum_2,number_samples):
...
@ExaquteTask(returns=1)
def computeCentralMomentsErrorEstimationOrderTwoDimensionZero_Task(power_sum_1,power_sum_2,power_sum_3,power_sum_4,number_samples):
    central_moment_2 = (number_samples*power_sum_2-power_sum_1**2) / ((number_samples-1)*number_samples)
...
@ExaquteTask(returns=1)
def computeCentralMomentsErrorEstimationOrderOneDimensionOne_Task(power_sum_10,power_sum_01,power_sum_20,power_sum_11,power_sum_02,number_samples):
