...
output:
        QoI                        : Quantity of Interest
        serialized_model_to_refine : model serialized
'''
@ExaquteTask(returns=2)
def execution_task_for_refinement(model, parameters):
    sample = GenerateBetaSample()
    simulation = MonteCarloAnalysis(model,parameters,sample)
    simulation.Run()
    QoI =  EvaluateQuantityOfInterest(simulation)
    '''serialize model to export from the task
    observe only pickleable objects can be passed in compss'''
    serialized_model_to_refine = KratosMultiphysics.StreamSerializer()
    serialized_model_to_refine.Save("ModelSerialization",simulation.model)
    return QoI,serialized_model_to_refine
...
if __name__ == '__main__':
    ...
    ExactExpectedValueQoI,serialized_model_to_refine = execution_task_for_refinement(serialized_model_0,serialized_parameters_0)

    '''compute the refinement of the mesh and build the correspondent serialized parameters and model'''
    serialized_model_1,serialized_parameters_1 = refinement.compute_refinement(serialized_model_to_refine,0.2,0.3,serialized_parameters_0,"MLMCLaplacianModelPart")

    stop
    
    '''evaluate the exact expected value of Q (sample = 1.0)'''
    simulation_1 = exact_execution_task(serialized_model_1,serialized_parameters_1)
    ...
    for instance in range (0,number_samples):
        simulation_list.append(execution_task(serialized_model_1,serialized_parameters_1))
    ...
    for i in range (0,number_samples):
        nsam = i+1
        MC_mean, MC_second_moment, MC_variance = mc.update_onepass_M_VAR(simulation_list[i].QoI, MC_mean, MC_second_moment, nsam)
    '''Evaluation of the relative error between the computed mean value and the expected value of the QoI'''
    relative_error = compare_mean(MC_mean,simulation_1.QoI)
    # print("Values QoI:",simulation_list.QoI)
    MC_mean = compss_wait_on(MC_mean)
    ExactExpectedValueQoI = compss_wait_on(simulation_1.QoI)
    relative_error = compss_wait_on(relative_error)
    print("\nMC mean = ",MC_mean,"exact mean = ",simulation_1.QoI)
