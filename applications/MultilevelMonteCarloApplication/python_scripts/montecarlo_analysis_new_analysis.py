...
'''
@ExaquteTask(returns=1)
def ExecuteExactMonteCarloAnalysis_Task(pickled_model,pickled_parameters,current_analysis_stage):
    '''overwrite the old model serializer with the unpickled one'''
    model_serializer = pickle.loads(pickled_model)
    current_model = KratosMultiphysics.Model()
    model_serializer.Load("ModelSerialization",current_model)
...
if __name__ == '__main__':
    ...
    ExactExpectedValueQoI = ExecuteExactMonteCarloAnalysis_Task(pickled_model,pickled_parameters,mc_class.GetAnalysis())
    '''start MC algorithm'''
