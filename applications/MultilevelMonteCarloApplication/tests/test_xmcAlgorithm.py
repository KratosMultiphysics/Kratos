    ...
    def test_mlmc_Kratos(self):
        ...
        for parametersPath in parametersList:
            ...
            if "asynchronous_adaptivefixednumberlevels" in parametersPath:
                self.assertAlmostEqual(sum(estimations), 1.5256974986866585, delta=parameters["monoCriteriaInputDictionary"]["statisticalError"]["tolerance"][0])
                ...
                self.assertAlmostEqual(sum(estimations), 1.5292773966722353, delta=parameters["monoCriteriaInputDictionary"]["statisticalError"]["tolerance"][0])
