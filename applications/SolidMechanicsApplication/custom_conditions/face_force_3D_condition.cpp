...
FaceForce3DCondition::FaceForce3DCondition()
{
}

// Constructor

FaceForce3DCondition::FaceForce3DCondition(IndexType NewId, GeometryType::Pointer pGeometry)
...
FaceForce3DCondition::FaceForce3DCondition(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties)
...
Condition::Pointer FaceForce3DCondition::Create(
    IndexType NewId,
    NodesArrayType const& ThisNodes,
    PropertiesType::Pointer pProperties) const
{
    return Condition::Pointer(new FaceForce3DCondition(NewId, GetGeometry().Create(ThisNodes), pProperties));
}
...
void FaceForce3DCondition::EquationIdVector(
...
void FaceForce3DCondition::GetDofList(
...
void FaceForce3DCondition::CalculateRightHandSide(
...
void FaceForce3DCondition::CalculateLocalSystem(
...
void FaceForce3DCondition::MassMatrix(
...
void FaceForce3DCondition::DampMatrix(
...
void FaceForce3DCondition::GetValuesVector(
    Vector& values,
    int Step)
{
    unsigned int number_of_nodes = GetGeometry().size();
...
void FaceForce3DCondition::GetFirstDerivativesVector(
    Vector& values,
    int Step)
{
    unsigned int number_of_nodes = GetGeometry().size();
...
void FaceForce3DCondition::GetSecondDerivativesVector(
    Vector& values,
    int Step)
{
    unsigned int number_of_nodes = GetGeometry().size();
...
void FaceForce3DCondition::CalculateAndSubKp(
    Matrix& K,
...
void FaceForce3DCondition::MakeCrossMatrix(
...
void FaceForce3DCondition::CrossProduct(
...
void FaceForce3DCondition::ExpandReducedMatrix(
    Matrix& Destination,
...
void FaceForce3DCondition::SubtractMatrix(
...
void FaceForce3DCondition::CalculateAndAddFacePressure(
    VectorType& rF,
    const Vector& rN,
...
void FaceForce3DCondition::CalculateAndAddFaceForce(Vector& rF,
						    const Vector& rN,
						    Vector& rForce,
						    double  rIntegrationWeight )
{
...
void FaceForce3DCondition::CalculateConditionalSystem(MatrixType& rLeftHandSideMatrix,
						      VectorType& rRightHandSideVector,
	...
	Vector ForceLoad = ZeroVector(dimension);
        ...
        for (unsigned int ii = 0; ii < number_of_nodes; ii++)
	  {
	    ...
	    if (norm_2(ForceLoad) != 0.00)
	      CalculateAndAddFaceForce(rRightHandSideVector, row(Ncontainer, PointNumber),
...
int FaceForce3DCondition::Check( const ProcessInfo& rCurrentProcessInfo )
{
    return 0;
}
