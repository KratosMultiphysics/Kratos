...
namespace Kratos
{
  ...
  Matrix& SurfaceLoadCondition::CalculateDeltaPosition(Matrix & rDeltaPosition)
  {
    KRATOS_TRY
  ...
  Matrix& SurfaceLoadCondition::CalculateTotalDeltaPosition(Matrix & rDeltaPosition)
  {
    KRATOS_TRY
...

  {
      ...
      if( rVariables.ExternalScalarValue == 0 )
	{
	  ...
	  // no need to do anything. LHS is already set to zero from load_condition.InitializeSystemMatrices
	}
      else
	{
