...
namespace Kratos
{
	...
	*/
		...
		ProcessInfo& rCurrentProcessInfo)
	{
		...
		//line load with rotational dofs -> nodal forces + moments
		if (this->GetGeometry()[0].HasDofFor(ROTATION_X) &&
			this->GetGeometry()[0].HasDofFor(ROTATION_Y) &&
			this->GetGeometry()[0].HasDofFor(ROTATION_Z) &&
			number_of_nodes == 2)
		{
			this->mIsLineLoadWithRotDof = true; // true
		}
		...
		ProcessInfo& rCurrentProcessInfo)
	{
		...
		if (this->mIsLineLoadWithRotDof == true) {
			rResult.resize(condition_size * 2, false);
		}

		for (unsigned int i = 0; i < number_of_nodes; i++)
		{
			int index = i * dimension;
			if (this->mIsLineLoadWithRotDof == true) index *= 2;
	...
	void ForceLoadCondition::GetValuesVector(Vector& rValues, int Step)
	{
		...
		if (this->mIsLineLoadWithRotDof == true) {
			rValues.resize(condition_size * 2, false);
		}
...

	{
		...
		if (this->mIsLineLoadWithRotDof == true) MatSize *= 2;
	...
	void ForceLoadCondition::InitializeGeneralVariables(GeneralVariables& rVariables, const ProcessInfo& rCurrentProcessInfo)
	{
		...
		if (this->mIsLineLoadWithRotDof == true) dimension *= 2;
...

	{
		...
		double local_size = dimension;
		if (this->mIsLineLoadWithRotDof == true) local_size *= 2;


		Vector ForceVector(local_size);
		noalias(ForceVector) = ZeroVector(local_size);
