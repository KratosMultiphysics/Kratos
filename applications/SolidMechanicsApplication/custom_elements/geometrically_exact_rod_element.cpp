...
void GeometricallyExactRodElement::Initialize()
{
    ...
    Matrix MatrixZero = ZeroMatrix(dimension,dimension);
...
void GeometricallyExactRodElement::InitializeGeneralVariables(GeneralVariables& rVariables, const ProcessInfo& rCurrentProcessInfo)
{

    KRATOS_TRY

    LargeDisplacementBeamEMCElement::InitializeGeneralVariables(rVariables,rCurrentProcessInfo);
    ...
    Vector VectorZero = ZeroVector(3);
...
void GeometricallyExactRodElement::CalculateKinematics(GeneralVariables& rVariables, const unsigned int& rPointNumber)
{
    KRATOS_TRY
    ...
    rVariables.InitialAxisPositionDerivatives = ZeroVector(dimension);
    ...
    Matrix CurrentValueDirectors  = ZeroMatrix(3,3);

    Vector InitialValueVector    = ZeroVector(3);
...
void GeometricallyExactRodElement::CalculateFrameMapping(GeneralVariables& rVariables,const unsigned int& rPointNumber)
{
    KRATOS_TRY
    ...
    //directors previous
    rVariables.PreviousRotationMatrix = ZeroMatrix(3,3);
    for ( unsigned int i = 0; i < number_of_nodes; i++ )
      {
    ...
    //directors current
    rVariables.CurrentRotationMatrix = ZeroMatrix(3,3);
    for ( unsigned int i = 0; i < number_of_nodes; i++ )
      {
...
void GeometricallyExactRodElement::UpdateRotationVariables(GeneralVariables& rVariables, const unsigned int& rPointNumber)
{
    KRATOS_TRY
...
void GeometricallyExactRodElement::CalculateDirectorsMappingTensor(Matrix& rMappingTensor, GeneralVariables& rVariables, const int& rNode, double alpha)
{

    Matrix SkewSymDirector = ZeroMatrix(3,3);
    std::vector<Vector> DirectorVectorsAlpha;
    DirectorVectorsAlpha.resize(3);

    for( unsigned int i = 0; i < 3; i++ )
      {
	DirectorVectorsAlpha[i] = ZeroVector(3);
      }
    ...
    rMappingTensor = ZeroMatrix(12,6);
...
void GeometricallyExactRodElement::CalculateStrainResultants(Vector& rStrainResultants, GeneralVariables& rVariables, double alpha)
{
    KRATOS_TRY
...
void GeometricallyExactRodElement::CalculateStrainCouples(Vector& rStrainCouples, GeneralVariables& rVariables, double alpha)
{
    KRATOS_TRY
...
void GeometricallyExactRodElement::CalculateCurrentStrainResultantsVector(GeneralVariables& rVariables, 
									  ...
									  double Alpha)
{
    ...
    rCurrentStrainResultantsVector = ZeroVector(3); 
    ...
    // E3[2] = 1.0;
    // rCurrentStrainResultantsVector -= E3;  //next is equivalent
...
void GeometricallyExactRodElement::CalculateCurrentCurvatureVector(GeneralVariables& rVariables, 
								   ...
								   double Alpha)
{
    ...
    rCurrentCurvatureVector = ZeroVector(3); 
...
void GeometricallyExactRodElement::CalculateConstitutiveMatrix(GeneralVariables& rVariables)
{
    KRATOS_TRY
...
void GeometricallyExactRodElement::CalculateStressResultants(GeneralVariables& rVariables, const unsigned int& rPointNumber, double alpha)
{
    KRATOS_TRY
    ...
    this->CalculateStrainResultants(StrainResultants, rVariables, alpha);
    this->CalculateStrainCouples(StrainCouples, rVariables, alpha);
...
void GeometricallyExactRodElement::CalculateAndAddLHS(LocalSystemComponents& rLocalSystem, GeneralVariables& rVariables, double& rIntegrationWeight)
{
    KRATOS_TRY
    ...
    // std::cout<<"["<<this->Id()<<"] rLeftHandSideMatrix "<<rLeftHandSideMatrix<<std::endl;
...
void GeometricallyExactRodElement::CalculateAndAddRHS(LocalSystemComponents& rLocalSystem, GeneralVariables& rVariables, Vector& rVolumeForce, double& rIntegrationWeight)
{
    KRATOS_TRY
    ...
    // std::cout<<"["<<this->Id()<<"] rRightHandSideVector "<<rRightHandSideVector<<std::endl;
								 ...
								 double& rIntegrationWeight)
{
    ...
    VectorType Fe            = ZeroVector(12);
    VectorType Fext          = ZeroVector(6);
    MatrixType MappingTensor = ZeroMatrix(12,6);
								 ...
								 double& rIntegrationWeight)
{
    ...
    VectorType Fi   = ZeroVector(12);
    VectorType Fint = ZeroVector(6);

    //Initialize Local Matrices
    MatrixType DifferentialOperatorI = ZeroMatrix(12,6);
								 ...
								 double alpha)
{
    ...
    rDifferentialOperator = ZeroMatrix(12,6);

    Matrix OperatorTau     = ZeroMatrix(6,12);
    Matrix OperatorOmegaI  = ZeroMatrix(6,12);
    Matrix OperatorOmegaII = ZeroMatrix(6,12);
    ...
    for ( unsigned int j = 0; j < 3; j++ )
      {
    	PreviousDirectors[j] = ZeroVector(3);
    	CurrentDirectors[j]  = ZeroVector(3);
    ...
    Matrix AlphaNodeRotationMatrix = ZeroMatrix(3,3);
    AlphaNodeRotationMatrix = (1-alpha) * PreviousNodeDirectors + alpha * CurrentNodeDirectors;

    Matrix AuxiliarRotationMatrix = ZeroMatrix(6,6);
							      ...
							      const int& rComponent)
{
    ...
    rDiscreteOperator = ZeroMatrix(3,3);
							     ...
							     const int& rComponent)
{
    ...
    for ( unsigned int j = 0; j < 3; j++ )
        ...
        for ( unsigned int k = 0; k < 3; k++ )
	  {
	    ...
	    Matrix SkewSymDirectorKNodeI = ZeroMatrix(3,3);
	    ...
	    Matrix SkewSymDirectorK = ZeroMatrix(3,3);
	    ...
	    Matrix SkewSymDirectorKDerivatives = ZeroMatrix(3,3);
...
void GeometricallyExactRodElement::CalculateAlphaDirectors(Matrix& rDirectors, GeneralVariables& rVariables, const int& rNode, double alpha)
{
    KRATOS_TRY
...
void GeometricallyExactRodElement::CalculateAlphaDirectorVector(Vector& rDirectorVector, GeneralVariables& rVariables, const int& rNode, const int& rDirection, double alpha)
{
    KRATOS_TRY

    // nodal directors
    Matrix Directors = ZeroMatrix(3,3);
...
void GeometricallyExactRodElement::CalculateAlphaDirectorSkewSymTensor(Matrix& rDirectorSkewSymTensor, GeneralVariables& rVariables, const int& rNode, const int& rDirection, double alpha)
{

   KRATOS_TRY

   Vector DirectorVector = ZeroVector(3);
...
void GeometricallyExactRodElement::CalculateAlphaDirectorVector(Vector& rDirectorVector, GeneralVariables& rVariables, const int& rDirection, double alpha)
{
...
void GeometricallyExactRodElement::CalculateAlphaDirectorSkewSymTensor(Matrix& rDirectorSkewSymTensor, GeneralVariables& rVariables, const int& rDirection, double alpha)
{
...
void GeometricallyExactRodElement::CalculateDirectorDerivativesVector(Vector& rDirectorDerivativesVector, GeneralVariables& rVariables, const int& rDirection, double alpha)
{
   ...
   rDirectorDerivativesVector = ZeroVector(3);
...
void GeometricallyExactRodElement::CalculateDirectorDerivativesSkewSymTensor(Matrix& rDirectorDerivativesSkewSymTensor, GeneralVariables& rVariables, const int& rDirection, double alpha)
{

   KRATOS_TRY

   Vector DirectorDerivativesVector = ZeroVector(3);
						       ...
						       double& rIntegrationWeight)
{
    ...
    //Initialize Local Matrices
    MatrixType DifferentialOperatorI = ZeroMatrix(12,6);
    MatrixType DifferentialOperatorJ = ZeroMatrix(12,6);

    MatrixType Kij  = ZeroMatrix(12,12);
    MatrixType Kmij = ZeroMatrix(6,6);

    unsigned int RowIndex = 0;
    unsigned int ColIndex = 0;

    MatrixType MappingTensorI = ZeroMatrix(12,6);
    MatrixType MappingTensorJ = ZeroMatrix(12,6);
						       ...
						       double& rIntegrationWeight)
{
    ...
    Matrix SkewSymDirectorKNodeI = ZeroMatrix(3,3);
    Matrix SkewSymDirectorKNodeJ = ZeroMatrix(3,3);
	...
	for ( unsigned int j = 0; j < number_of_nodes; j++ )
	  {
		...
		//term 12
		GabK = ZeroMatrix(3,3);
		SkewSymDirectorKNodeJ = ZeroMatrix(3,3);
		...
		//term 21
		GabK = ZeroMatrix(3,3);
		SkewSymDirectorKNodeI = ZeroMatrix(3,3);
...
void GeometricallyExactRodElement::CalculateAndAddInertiaLHS(MatrixType& rLeftHandSideMatrix, GeneralVariables& rVariables, ProcessInfo& rCurrentProcessInfo, double& rIntegrationWeight )
{
    ...
    Matrix AlgorithmicInertia = ZeroMatrix(3,3);
	...
	for ( unsigned int j = 0; j < number_of_nodes; j++ )
	  {
	    ...
	    AlgorithmicInertia = ZeroMatrix(3,3);
...
void GeometricallyExactRodElement::CalculateAndAddInertiaRHS(VectorType& rRightHandSideVector, GeneralVariables& rVariables, ProcessInfo& rCurrentProcessInfo, double& rIntegrationWeight)
{
    KRATOS_TRY
   ...
   DirectorsVariables& Directors = rVariables.GetDirectors();
    ...
    for ( unsigned int i = 0; i < 3; i++ )
      {
	CurrentDirectorsVelocity[i] = ZeroVector(3);
	PreviousDirectorsVelocity[i] = ZeroVector(3);
      }   
    ...
    Vector TotalInertialForceVector = ZeroVector(12);
...
void GeometricallyExactRodElement::CalculateAlgorithmicInertia(Matrix & rAlgorithmicInertia, const Matrix& rInertiaDyadic, GeneralVariables& rVariables, const int& rNodeJ, const int& rNodeI, double alpha)
{
    KRATOS_TRY
    ...
    rAlgorithmicInertia = ZeroMatrix(3,3);

    Matrix DiagonalMatrix = IdentityMatrix(3);

    Vector DirectorKNodeI = ZeroVector(3);
    Vector DirectorKNodeJ = ZeroVector(3);
...
void GeometricallyExactRodElement::CalculateOnIntegrationPoints(  const Variable<array_1d<double, 3 > >& rVariable,
