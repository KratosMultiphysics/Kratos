...
namespace Kratos
{
    ...
    void ShellThinElement3D3N::Initialize()
    {
        ...
        if(geom.PointsNumber() != OPT_NUM_NODES)
            KRATOS_ERROR(std::logic_error, "ShellThinElement3D3N Element - Wrong number of nodes", geom.PointsNumber());
        ...
        if(integrationPoints.size() != OPT_NUM_GP)
            KRATOS_ERROR(std::logic_error, "ShellThinElement3D3N Element - Wrong integration scheme", integrationPoints.size());
