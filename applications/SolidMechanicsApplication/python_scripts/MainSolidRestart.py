...
class Solution(object):
    ...
    def Restart(self):
        ...
        for f in os.listdir(problem_path):
            if(f.endswith(".rest")):
                #if f.name = problem_tested__145.rest
                file_parts = f.split('__')  # you get ["problem_tested","145.rest"]
                num_parts  = len(file_parts)

                end_parts  = file_parts[num_parts-1].split(".") # you get ["145","rest"]
                rest_label = end_parts[0] # you get "145"
        ...
        if label is not None:
            if self.ProjectParameters["model_settings"].Has("input_file_settings") is False:
                ...
                input_parameters.AddEmptyValue("type").SetString("rest")
                input_parameters.AddEmptyValue("label").SetString(rest_label)
                self.ProjectParameters.AddValue("input_file_settings", input_parameters)
            else:
                if self.ProjectParameters["model_settings"]["input_file_settings"].Has("type") is False:
