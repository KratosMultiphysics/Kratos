    ...
    : public SolutionStrategy<TSparseSpace, TDenseSpace, TLinearSolver>
{
 public:
  ...
  void InitializeSolutionStep() override
  {
    ...
    mpBuilderAndSolver->InitializeSolutionStep(BaseType::GetModelPart(),(*mpStiffnessMatrix),(*pDx),(*pb));

    //initial operations ... things that are constant over the Solution Step
    mpScheme->InitializeSolutionStep(BaseType::GetModelPart(),(*mpStiffnessMatrix),(*pDx),(*pb));
 ...
 private:
  ...
  void SetSystemMatrices(SparseVectorPointerType& pDx, SparseVectorPointerType& pb)
  {
    ...
    // Mass matrix
    mpBuilderAndSolver->ResizeAndInitializeVectors(mpScheme, mpMassMatrix, pDx, pb,
                                                   this->GetModelPart().Elements(),
                                                   this->GetModelPart().Conditions(),
                                                   this->GetModelPart().GetProcessInfo());

    // Stiffness matrix
    mpBuilderAndSolver->ResizeAndInitializeVectors(mpScheme, mpStiffnessMatrix, pDx, pb,
                                                   this->GetModelPart().Elements(),
                                                   this->GetModelPart().Conditions(),
                                                   this->GetModelPart().GetProcessInfo());
    KRATOS_CATCH("")
