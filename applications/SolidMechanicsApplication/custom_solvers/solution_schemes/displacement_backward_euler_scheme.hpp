...
namespace Kratos
{
  ...
  class DisplacementBackwardEulerScheme: public DisplacementBdfScheme<TSparseSpace,TDenseSpace>
  {
  public:
    ...
    typedef DisplacementBdfScheme<TSparseSpace,TDenseSpace>                   DerivedType;
  ...
  protected:
    ...
    void SetIntegrationMethod(ProcessInfo& rCurrentProcessInfo) override
    {
      if ( this->mTimeIntegrationMethods.size() == 0 ) {
        this->mTimeIntegrationMethods.push_back(Kratos::make_shared< BackwardEulerMethod<Variable<array_1d<double, 3> >, array_1d<double,3> > >(DISPLACEMENT,VELOCITY,ACCELERATION));
