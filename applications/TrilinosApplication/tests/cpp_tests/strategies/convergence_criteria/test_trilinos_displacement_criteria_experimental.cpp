...
void GenerateTestTrilinosExperimentalDisplacementCriteriaModelPart(
 ...
 */
KRATOS_TEST_CASE_IN_SUITE(TrilinosExperimentalDisplacementCriteria, KratosTrilinosApplicationMPITestSuite)
{
    Model current_model;
    ...
    GenerateTestTrilinosExperimentalDisplacementCriteriaModelPart(r_model_part, n_nodes); // Create the geometry
    ...
    auto p_Dx = TrilinosSparseSpaceType::CreateVector(p_map);
    auto& Dx = *p_Dx;
    auto p_rb = TrilinosSparseSpaceType::CreateVector(p_map_aux); // Not used in this criteria but needed for API
    auto& rb = *p_rb;
