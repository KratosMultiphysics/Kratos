## This script collects the available convergence criteria to be used in the SolversApplication
...
class ConvergenceCriterion:
    ...
    def GetConvergenceCriterion(self):
        ...
        if( len(self.dofs) > 0 and self.settings["separate_dofs"].GetBool() ):
            ...
            convergence_criterion = KratosSolver.CompositeCriterion(convergence_criteria)
            convergence_criterion.Set(KratosSolver.CriterionLocalFlags.AND)
        else:
    ...
    def _get_dof_criterion(self, kratos_variable):
        ...
        if(self.settings["convergence_criterion"].GetString() == "Variable_criterion"):
            convergence_criterion = KratosSolver.DofsCriterion(kratos_variable,V_RT,V_AT)
            convergence_criterion.SetEchoLevel(echo_level)
            #convergence_criterion.Set(KratosSolver.CriterionLocalFlags.INCREMENTAL) //smaller reference value
        elif(self.settings["convergence_criterion"].GetString() == "Residual_criterion"):
            convergence_criterion = KratosSolver.ResidualCriterion(kratos_variable,R_RT,R_AT)
            convergence_criterion.SetEchoLevel(echo_level)
        elif(self.settings["convergence_criterion"].GetString() == "And_criterion"):
            Variable = KratosSolver.DofsCriterion(kratos_variable,V_RT,V_AT)
            Variable.SetEchoLevel(echo_level)
            #Variable.Set(KratosSolver.CriterionLocalFlags.INCREMENTAL) //smaller reference value
            Residual = KratosSolver.ResidualCriterion(kratos_variable,R_RT,R_AT)
            Residual.SetEchoLevel(echo_level)
            convergence_criterion = KratosSolver.CompositeCriterion(Residual,Variable)
            convergence_criterion.Set(KratosSolver.CriterionLocalFlags.AND)
            ...
            Residual.SetEchoLevel(echo_level)
            convergence_criterion = KratosSolver.CompositeCriterion(Residual,Variable)
            convergence_criterion.Set(KratosSolver.CriterionLocalFlags.OR)
        else:
    ...
    def _get_criterion(self):
        ...
        convergence_criterion = None
        if(self.settings["convergence_criterion"].GetString() == "Variable_criterion"):
            convergence_criterion = KratosSolver.DofsCriterion(V_RT,V_AT)
            convergence_criterion.SetEchoLevel(echo_level)
            convergence_criterion.Set(KratosSolver.CriterionLocalFlags.INCREMENTAL)
        elif(self.settings["convergence_criterion"].GetString() == "Residual_criterion"):
            convergence_criterion = KratosSolver.ResidualCriterion(R_RT,R_AT)
            convergence_criterion.SetEchoLevel(echo_level)
        elif(self.settings["convergence_criterion"].GetString() == "And_criterion"):
            Variable = KratosSolver.DofsCriterion(V_RT,V_AT)
            Variable.SetEchoLevel(echo_level)
            Variable.Set(KratosSolver.CriterionLocalFlags.INCREMENTAL)
            Residual = KratosSolver.ResidualCriterion(R_RT,R_AT)
            Residual.SetEchoLevel(echo_level)
            convergence_criterion = KratosSolver.CompositeCriterion(Residual,Variable)
            convergence_criterion.Set(KratosSolver.CriterionLocalFlags.AND)
        elif(self.settings["convergence_criterion"].GetString() == "Or_criterion"):
