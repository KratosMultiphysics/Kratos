...
class SegregatedSolver(BaseSolver.MonolithicSolver):
        ...
        if( not self._is_not_restarted() ):
            for solver in self.solvers:
                if hasattr(mechanical_solver, 'SetInitializePerformedFlag'):
                    ...
                    solver._get_mechanical_solver().Set(KratosSolver.SolverLocalFlags.INITIALIZED, True)

            if hasattr(mechanical_solver, 'SetInitializePerformedFlag'):
                self._get_mechanical_solver().SetInitializePerformedFlag(True)
            else:
                self._get_mechanical_solver().Set(KratosSolver.SolverLocalFlags.INITIALIZED, True)
    ...
    def _create_mechanical_solver(self):
        ...
        mechanical_solver =  KratosSolver.SegregatedStrategy(self.model_part, options, strategies)
        mechanical_solver.Set(KratosSolver.SolverLocalFlags.ADAPTIVE_SOLUTION,self._check_adaptive_solution())
