...
class MonolithicSolver(object):
    ...
    def __init__(self, Model, custom_settings):
        ...
        if not hasattr(KratosMultiphysics.ExternalSolversApplication, "super_lu"):
    ...
    def _check_reform_dofs(self):
        if self._domain_parts_updated():
            if not self._get_mechanical_solver().GetOptions().Is(KratosSolver.SolverLocalFlags.REFORM_DOFS):
                self._get_mechanical_solver().GetOptions().Set(KratosSolver.SolverLocalFlags.REFORM_DOFS, True)
            ...
            if self._get_mechanical_solver().GetOptions().Is(KratosSolver.SolverLocalFlags.REFORM_DOFS):
                self._get_mechanical_solver().GetOptions().Set(KratosSolver.SolverLocalFlags.REFORM_DOFS, False)
    ...
    def _domain_parts_updated(self):
        ...
        if not update_time and self.process_info.Has(KratosSolver.CONTACT_STEP_TIME):
            update_time = self._check_previous_time_step(self.process_info[KratosSolver.CONTACT_STEP_TIME])
    ...
    def _create_builder_and_solver(self):
        ...
        if(self.settings["solving_strategy_settings"]["builder_type"].GetString() == "block_builder"):
            ...
            builder_and_solver = KratosSolver.BlockBuilderAndSolver(linear_solver)
        else:
            builder_and_solver = KratosSolver.ReductionBuilderAndSolver(linear_solver)
    ...
    def _add_dofs(self):
        ...
        AddDofsProcess = KratosSolver.AddDofsProcess(self.main_model_part, dof_variables, dof_reactions)
    ...
    def _set_time_integration_methods(self):
        ...
        component_integration_methods_container = KratosSolver.ComponentTimeIntegrationMethods()
        ...
        if( len(scalar_integration_methods) ):
            scalar_integration_methods_container.AddToProcessInfo(KratosSolver.SCALAR_TIME_INTEGRATION_METHODS, scalar_integration_methods_container, self.process_info)

        component_integration_methods_container.AddToProcessInfo(KratosSolver.COMPONENT_TIME_INTEGRATION_METHODS, component_integration_methods_container, self.process_info)
