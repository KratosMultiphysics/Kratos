...
static int_t sDumpLine(FILE *fp)
{
    register int_t c;
    while ((c = fgetc(fp)) != '\n') ;
    return 0;
...
static int_t sParseIntFormat(char *buf, int_t *num, int_t *size)
{
    char *tmp;
...
static int_t sParseFloatFormat(char *buf, int_t *num, int_t *size)
{
    char *tmp, *period;
...
static int_t sReadValues(FILE *fp, int_t n, float *destination, int_t perline,
        int_t persize)
{
...
void
sreadrb(int_t *nrow, int_t *ncol, int_t *nonz,
