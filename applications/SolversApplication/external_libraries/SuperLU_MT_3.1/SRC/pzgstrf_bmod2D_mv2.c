...
void zmatvec2(int_t, int_t, int_t, doublecomplex*, doublecomplex*, doublecomplex*, doublecomplex*, doublecomplex*);
 ...
 */
	...
	if ( segsze == 1 ) {
	    ...
	    CTRSV( ftcs1, ftcs2, ftcs3, &segsze, &lusup[luptr], 
		   &nsupr32, TriTmp, &incx );
#else
	    ztrsv_( "L", "N", "U", &segsze, &lusup[luptr], &nsupr32, TriTmp, &incx );
#endif
#else		
	    zlsolve ( (int_t) nsupr, (int_t) segsze, &lusup[luptr], TriTmp );
#endif		
#ifdef TIMING	    
	    utime[FLOAT] += SuperLU_timer_() - f_time;
#endif	    
...
#endif
#else
		    zmatvec ((int_t) nsupr, (int_t) block_nrow, (int_t) segsze, &lusup[luptr],
			     tri[0], matvec[0]);
#endif
