...
def CreateVtkFiles(step_in,step_fin,SnapshotMatrix_stresses,folder_name):
    for i in range(step_in,step_fin):# Loop in time results
        # Read vtk output file
        fileName = 'vtk_output_HROM/Structure_VISUALIZE_HROM_0_'+str(i+1)+'.vtk' # Disp/Rot are obtained with respect to Interface Frame 1, first 6 DoF will not be necessary
        ...
        pk2_stress = meshNew.PointData['PK2_STRESS_VECTOR'] # Get the stresses pointer
        for j in range(int(pk2_stress.shape[0])):
            pk2_stress[j,:] = SnapshotMatrix_stresses[i,j,:]
...
def RunPostProcess(time_steps,Snapshot_stresses):
    folder_name = 'vtk_HROM'
    ...
    #     Snapshot_stresses = np.load(f)
    for k in range(n):
        step_in = time_steps_in_open*k
        step_fin = time_steps_in_open*(k+1)
        if k==n-1:
            step_fin += time_res
        p = Process(target=CreateVtkFiles, args=(step_in,step_fin,Snapshot_stresses,folder_name))
        processes.append(p)
        p.start()
