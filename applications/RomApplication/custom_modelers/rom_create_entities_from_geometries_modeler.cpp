...
#include "rom_create_entities_from_geometries_modeler.h"
...
void CreateEntitiesFromGeometries(
    ...
    const std::unordered_map<IndexType, IndexType>& rGeomToIdMap,
    const bool AssignIdsFromFile)
{
        ...
        it != rModelPart.GeometriesEnd(); ++it)
    {
        ...
        if (rEntityIdentifier.HasPrototypeEntity(*p_geom)) {
            ...
            if (AssignIdsFromFile) {
                auto it = rGeomToIdMap.find(p_geom->Id());
                if (it != rGeomToIdMap.end()) {
                    new_id = it->second;
                } else
                {
                    new_id = ++max_id;
            ...
            auto p_entity = r_ref_entity.Create(new_id, p_geom, nullptr);
...
void RomCreateEntitiesFromGeometriesModeler::RemoveModelPartEntities(ModelPart &rModelPart)
{
    ...
    KRATOS_WARNING_IF("RomCreateEntitiesFromGeometriesModeler", n_entities != 0)
...
template <class TEntitiesContainerType>
void RomCreateEntitiesFromGeometriesModeler::LoopEntitiesList(Parameters EntitiesList)
{
    ...
    for (auto& r_data : EntitiesList) {
        ...
        CreateEntitiesFromGeometries<TEntitiesContainerType, EntitiesUtilities::EntitityIdentifier<EntityType>>(
            entity_identifier, r_model_part, geom_to_entity_id, mParameters["assign_ids_from_file"].GetBool());
    }
}

void RomCreateEntitiesFromGeometriesModeler::BuldMapToAssignEntityIdsFromFile()
{
    KRATOS_TRY

    const std::string ids_filename =
        mParameters["geometry_ids_file"].GetString();

    KRATOS_ERROR_IF(ids_filename == "")
        << "geometry_ids_file is empty but assign_ids_from_file is true."
        << std::endl;

    std::ifstream infile(ids_filename);
    KRATOS_ERROR_IF_NOT(infile.is_open())
        << "Could not open file: " << ids_filename << std::endl;

    geom_to_entity_id.clear();
    ...
    while (std::getline(infile, line)) {
        ...
        std::string geom_id_str, entity_id_str;

        std::getline(ss, geom_id_str, ',');
        std::getline(ss, entity_id_str, ',');

        const IndexType geom_id   = std::stoul(geom_id_str);
        const IndexType entity_id = std::stoul(entity_id_str);

        KRATOS_ERROR_IF(geom_to_entity_id.find(geom_id) != geom_to_entity_id.end())
            << "Duplicate geometry id " << geom_id
            << " in geometry_ids_file." << std::endl;

        geom_to_entity_id[geom_id] = entity_id;
    }
...
void RomCreateEntitiesFromGeometriesModeler::SetupModelPart()
{
    if (mParameters["assign_ids_from_file"].GetBool()) {
        BuldMapToAssignEntityIdsFromFile();
    }
    ...
    KRATOS_ERROR_IF(ParallelEnvironment::GetDefaultDataCommunicator().IsDistributed())
        << "RomCreateEntitiesFromGeometriesModeler is not MPI compatible yet." << std::endl;
    ...
    KRATOS_INFO_IF("RomCreateEntitiesFromGeometriesModeler", mEchoLevel != 0 && n_element_pairs == 0)
    ...
    KRATOS_INFO_IF("RomCreateEntitiesFromGeometriesModeler", mEchoLevel != 0 && n_condition_pairs == 0)
