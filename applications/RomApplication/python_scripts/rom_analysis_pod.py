...
def CreatePODRomAnalysisInstance(cls, global_model, parameters):
    class PODRomAnalysis(cls):

        def __init__(self,global_model, parameters):
            super().__init__(global_model, parameters)
            print('STARTING CUSTOM POD SIMULATION')

        def _GetSVDPhiMatrix(self):
            """Define method to obtain the Phi matrix"""
        ...
        def Initialize(self):
            ...
            SVDPhiMatrix=self._GetSVDPhiMatrix()
            numberOfROMModes = SVDPhiMatrix.shape[1]
            ...
            aux = KratosMultiphysics.Matrix(nodal_dofs, numberOfROMModes)
            for node in computing_model_part.Nodes:
                node_id = node.Id-1
                for i in range(numberOfROMModes):
                    aux[0,i] = SVDPhiMatrix[node_id*nodal_dofs][i].copy()
                    aux[1,i] = SVDPhiMatrix[node_id*nodal_dofs+1][i].copy()
                node.SetValue(KratosROM.ROM_BASIS, aux)

                # print(np.array(node.GetValue(KratosROM.ROM_BASIS),copy=True)[:,:10])
                # print(nodal_unknown_names)


            # Initialize nodal ROM_BASIS to zeros
            # for node in computing_model_part.Nodes:
            #     node.SetValue(KratosROM.ROM_BASIS, SVDPhiMatrix[[(node.Id()-1)*nodal_dofs,(node.Id()-1)*nodal_dofs+1]])
    ...
    return PODRomAnalysis(global_model, parameters)
...
if __name__ == "__main__":
    ...
    simulation = CreatePODRomAnalysisInstance(analysis_stage_class, global_model, parameters)
    simulation.Run()