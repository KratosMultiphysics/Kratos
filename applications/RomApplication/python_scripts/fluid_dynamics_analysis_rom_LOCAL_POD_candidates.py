...
class FluidDynamicsAnalysisROM(FluidDynamicsAnalysis):
    ...
    def ModifyAfterSolverInitialize(self):
        ...
        computing_model_part = self._GetSolver().fluid_solver.GetComputingModelPart()
        ...
        self._GetSolver().fluid_solver._GetBuilderAndSolver().SetUpBases(Bases)
        #self._GetSolver().fluid_solver._GetBuilderAndSolver().SetUpDistances(distance_to_clusters)
        ...
        if self.hyper_reduction_element_selector != None:
            if self.hyper_reduction_element_selector.Name == "EmpiricalCubature":
                ...
                self.ResidualUtilityObject = romapp.RomResidualsUtility(self._GetSolver().GetComputingModelPart(), self.project_parameters["solver_settings"]["rom_settings"], self._GetSolver().fluid_solver._GetScheme(), Bases)
    ...
    def InitializeSolutionStep(self):
        ...
        #self._GetSolver().fluid_solver._GetBuilderAndSolver().UpdateCurrentCluster()


    def FinalizeSolutionStep(self):
        #self._GetSolver().fluid_solver._GetBuilderAndSolver().UpdateZMatrix()
        if self.hyper_reduction_element_selector != None:
            if self.time>0.2: #TODO why starting from 0 causes a segmentation fault? clearly, not everything is correctly set at the beggining
                if self.hyper_reduction_element_selector.Name == "EmpiricalCubature":
                    print('\n\n\n\nGenerating matrix of residuals')
                    ResMat = self.ResidualUtilityObject.GetResiduals(self._GetSolver().fluid_solver._GetBuilderAndSolver().GetCurrentCluster())
                    #print(np.shape(ResMat))
                    NP_ResMat = np.array(ResMat, copy=False)
                    self.time_step_residual_matrix_container[self._GetSolver().fluid_solver._GetBuilderAndSolver().GetCurrentCluster()].append(NP_ResMat)
        ...
        if self.hyper_reduction_element_selector != None:
            if self.hyper_reduction_element_selector.Name == "EmpiricalCubature":
                ...
                for i in range(number_bases):
                    ...
                    np.save(f'HROM_Local/Sr_{i}.npy', basis_i)
                ...
                # OriginalNumberOfElements = self._GetSolver().fluid_solver.GetComputingModelPart().NumberOfElements()
                # ModelPartName = self._GetSolver().fluid_solver.settings["model_import_settings"]["input_filename"].GetString()
