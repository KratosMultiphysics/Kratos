...
class StructuralMechanicsAnalysisROM(StructuralMechanicsAnalysis):
    ...
    def ModifyAfterSolverInitialize(self):
        ...
        with open('RomParameters.json') as f:
            ...
            self.multiple_bases_manager = romapp.MultipleBasesManager(number_of_bases)
        Bases = romapp.RomBases()
        for i in range(number_of_bases):
            Bases.AddBasis(i, POD_BASES[i])
        self.Number_Of_Clusters = number_of_bases
        self._GetSolver()._GetBuilderAndSolver().SetUpBases(Bases)
        self._GetSolver()._GetBuilderAndSolver().SetUpMultipleBasesManager(self.multiple_bases_manager)
    ...
    def FinalizeSolutionStep(self):
        if self.hyper_reduction_element_selector != None:
            if self.hyper_reduction_element_selector.Name == "EmpiricalCubature":
                ...
                ResMat = self.ResidualUtilityObject.GetResiduals(self._GetSolver()._GetBuilderAndSolver().GetCurrentCluster())
                #print(np.shape(ResMat))
                NP_ResMat = np.array(ResMat, copy=False)
                self.time_step_residual_matrix_container[self._GetSolver()._GetBuilderAndSolver().GetCurrentCluster()].append(NP_ResMat)
