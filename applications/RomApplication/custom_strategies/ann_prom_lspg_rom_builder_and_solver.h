...
/**
 * @class AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver
 * @ingroup RomApplication
 * @brief This class provides an implementation for the AnnPromLeastSquaresPetrovGalerkinROM builder and solver operations.
...
class AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver : public AnnPromGlobalROMBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>
{
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver);
    ...
    using ClassType = AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>;

    /// Definition of the classes from the base class
    using BaseBuilderAndSolverType = BuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>;
    using BaseType = AnnPromGlobalROMBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>;
    ...
    explicit AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver(
    ...
    ~AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver() = default;
        ...
        ModelPart &rModelPart) override
    {
        ...
        KRATOS_INFO_IF("AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 1)) << "Setting up the dofs" << std::endl;
        KRATOS_INFO_IF("AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 2)) << "Number of threads" << ParallelUtilities::GetNumThreads() << "\n" << std::endl;
        KRATOS_INFO_IF("AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 2)) << "Initializing element loop" << std::endl;
        ...
        KRATOS_INFO_IF("AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 2)) << "Initializing ordered array filling\n" << std::endl;
        ...
        KRATOS_INFO_IF("AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 2)) << "Number of degrees of freedom:" << BaseType::GetDofSet().size() << std::endl;
        KRATOS_INFO_IF("AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 2)) << "Finished setting up the dofs" << std::endl;
        ...
        TSystemVectorType &rDx) override
    {
        ...
        KRATOS_INFO_IF("AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 0)) << "Build and project time: " << time << std::endl;
    ...
    {
        BaseType::BuildRightROMBasis(rPhiGlobal);
    }
        ...
        )
    {
        ...
        if (mRightRomBasisInitialized==false){
            ...
            // mInit_A = mPhiGlobal;
            // mInit_A = Matrix(200,10);
            // Eigen::Map<EigenDynamicMatrix> eigen_init_A(mInit_A.data().begin(), mInit_A.size1(), mInit_A.size2());
        ...
        // if (mPeriodicCounter == 0) {
        //     mStaticRomA = eigen_rA;
        // }
        // else {
        //     eigen_my_A = mStaticRomA;
        // }

        // if (mPeriodicCounter == 3) {
        //     mPeriodicCounter = 0;
        // }
        // else {
        //     mPeriodicCounter++;
        ...
        // EigenDynamicMatrix eigen_rA_times_mPhiGlobal = eigen_my_A * eigen_mPhiGlobal; //TODO: Make it in parallel.
        ...
        if (BaseType::mHromSimulation) {
            if (mSolvingTechnique != "normal_equations") {
                KRATOS_ERROR << "AnnPromLeastSquaresPetrovGalerkinHROM is only valid for 'normal_equations'. The provided solving technique is '" << mSolvingTechnique << "'." << std::endl;
            }
    ...
    Parameters GetDefaultParameters() const override
    {
        ...
        {
            "name" : "ann_prom_lspg_rom_builder_and_solver",
            "nodal_unknowns" : [],
    ...
    {
        return "ann_prom_lspg_rom_builder_and_solver";
    }
    ...
    {
        return "AnnPromLeastSquaresPetrovGalerkinROMBuilderAndSolver";
    }
...
protected:
        ...
        TSystemVectorType& b)
    {
        ...
        KRATOS_INFO_IF("AnnPromLeastSquaresPetrovGalerkinROMResidualBasedBlockBuilderAndSolver", BaseType::GetEchoLevel() >= 1) << "Build time: " << timer.ElapsedSeconds() << std::endl;

        KRATOS_INFO_IF("AnnPromLeastSquaresPetrovGalerkinROMResidualBasedBlockBuilderAndSolver", (BaseType::GetEchoLevel() > 2 && rModelPart.GetCommunicator().MyPID() == 0)) << "Finished parallel building" << std::endl;
...
private:
    ...
    EigenDynamicMatrix mStaticRomA;
    // Vector mInit_x;
    // Matrix mInit_A;
    int mPeriodicCounter = 0;
