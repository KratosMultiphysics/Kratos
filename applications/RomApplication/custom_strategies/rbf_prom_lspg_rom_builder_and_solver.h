...
/**
 * @class RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver
 * @ingroup RomApplication
 * @brief This class provides an implementation for the RbfPromLeastSquaresPetrovGalerkinROM builder and solver operations.
...
class RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver : public RbfPromGlobalROMBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>
{
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver);
    ...
    using ClassType = RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>;

    /// Definition of the classes from the base class
    using BaseBuilderAndSolverType = BuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>;
    using BaseType = RbfPromGlobalROMBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>;
    ...
    explicit RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver(
    ...
    ~RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver() = default;
        ...
        ModelPart &rModelPart) override
    {
        ...
        KRATOS_INFO_IF("RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 1)) << "Setting up the dofs" << std::endl;
        KRATOS_INFO_IF("RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 2)) << "Number of threads" << ParallelUtilities::GetNumThreads() << "\n" << std::endl;
        KRATOS_INFO_IF("RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 2)) << "Initializing element loop" << std::endl;
        ...
        KRATOS_INFO_IF("RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 2)) << "Initializing ordered array filling\n" << std::endl;
        ...
        KRATOS_INFO_IF("RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 2)) << "Number of degrees of freedom:" << BaseType::GetDofSet().size() << std::endl;
        KRATOS_INFO_IF("RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 2)) << "Finished setting up the dofs" << std::endl;
        ...
        TSystemVectorType &rDx) override
    {
        ...
        KRATOS_INFO_IF("RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver", (BaseType::GetEchoLevel() > 0)) << "Build and project time: " << time << std::endl;
        ...
        )
    {
        ...
        if (mRightRomBasisInitialized==false){
            ...
            RomRBFUtility<TSparseSpace,TDenseSpace>::GetXAndDecoderGradient(r_xRom, r_xBase, BaseType::mPhiGlobal, r_svdPhiMatrices, r_W_mat, r_centers_mat, r_kernel_type, r_kernel_eps, r_refSnapshot);
            ...
            IndexType& r_kernel_type =  r_root_mp.GetValue(KERNEL_TYPE);
            double& r_kernel_eps = r_root_mp.GetValue(KERNEL_EPS);
            RomRBFUtility<TSparseSpace,TDenseSpace>::GetDecoderGradient(xromTotal, BaseType::mPhiGlobal, r_svdPhiMatrices, r_W_mat, r_centers_mat, r_kernel_type, r_kernel_eps);
        }
        ...
        if (BaseType::mHromSimulation) {
            if (mSolvingTechnique != "normal_equations") {
                KRATOS_ERROR << "RbfPromLeastSquaresPetrovGalerkinHROM is only valid for 'normal_equations'. The provided solving technique is '" << mSolvingTechnique << "'." << std::endl;
            }
    ...
    Parameters GetDefaultParameters() const override
    {
        ...
        {
            "name" : "rbf_prom_lspg_rom_builder_and_solver",
            "nodal_unknowns" : [],
    ...
    {
        return "rbf_prom_lspg_rom_builder_and_solver";
    }
    ...
    {
        return "RbfPromLeastSquaresPetrovGalerkinROMBuilderAndSolver";
    }
...
protected:
        ...
        TSystemVectorType& b)
    {
        ...
        KRATOS_INFO_IF("RbfPromLeastSquaresPetrovGalerkinROMResidualBasedBlockBuilderAndSolver", BaseType::GetEchoLevel() >= 1) << "Build time: " << timer.ElapsedSeconds() << std::endl;

        KRATOS_INFO_IF("RbfPromLeastSquaresPetrovGalerkinROMResidualBasedBlockBuilderAndSolver", (BaseType::GetEchoLevel() > 2 && rModelPart.GetCommunicator().MyPID() == 0)) << "Finished parallel building" << std::endl;
