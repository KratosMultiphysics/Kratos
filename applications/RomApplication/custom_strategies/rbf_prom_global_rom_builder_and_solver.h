...
/**
 * @class RbfPromGlobalROMBuilderAndSolver
 */
template <class TSparseSpace, class TDenseSpace, class TLinearSolver>
class RbfPromGlobalROMBuilderAndSolver : public ResidualBasedBlockBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>
{
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(RbfPromGlobalROMBuilderAndSolver);
    ...
    using ClassType = RbfPromGlobalROMBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>;
    ...
    explicit RbfPromGlobalROMBuilderAndSolver(
    ...
    ~RbfPromGlobalROMBuilderAndSolver() = default;
        ...
        ModelPart &rModelPart) override
    {
        ...
        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", (this->GetEchoLevel() > 1)) << "Setting up the dofs" << std::endl;
        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Number of threads" << ParallelUtilities::GetNumThreads() << "\n" << std::endl;
        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Initializing element loop" << std::endl;
        ...
        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Initializing ordered array filling\n" << std::endl;
        ...
        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Number of degrees of freedom:" << BaseBuilderAndSolverType::GetDofSet().size() << std::endl;
        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Finished setting up the dofs" << std::endl;
    ...
    )
    {
        ...
        r_root_mp.SetValue(W_MAT, W_mat);
        r_root_mp.SetValue(CENTERS_MAT, centers_mat);
        r_root_mp.SetValue(KERNEL_TYPE, kernel_type);
        r_root_mp.SetValue(KERNEL_EPS, kernel_eps);
    ...
    {
        auto& r_root_mp = rModelPart.GetRootModelPart();
        Matrix& r_W_mat = r_root_mp.GetValue(W_MAT);
        Matrix& r_centers_mat = r_root_mp.GetValue(CENTERS_MAT);
        IndexType& r_kernel_type =  r_root_mp.GetValue(KERNEL_TYPE);
        double& r_kernel_eps = r_root_mp.GetValue(KERNEL_EPS);
        vector<Matrix>& r_svdPhiMatrices = r_root_mp.GetValue(SVD_PHI_MATRICES);
        Vector& r_refSnapshot = r_root_mp.GetValue(SOLUTION_REFERENCE);

        TSystemVectorType xFom = ZeroVector(r_refSnapshot.size());

        RomRBFUtility<TSparseSpace,TDenseSpace>::GetXFromDecoder(xRom, xFom, r_svdPhiMatrices, r_W_mat, r_centers_mat, r_kernel_type, r_kernel_eps, r_refSnapshot);
    ...
    Parameters GetDefaultParameters() const override
    {
        ...
        {
            "name" : "rbf_prom_global_rom_builder_and_solver",
            "nodal_unknowns" : [],
    ...
    {
        return "rbf_prom_global_rom_builder_and_solver";
    }
    ...
    {
        return "RbfPromGlobalROMBuilderAndSolver";
    }
...
protected:
    ...
    void AssignSettings(const Parameters ThisParameters) override
    {
        ...
        mActiveHromSet = ThisParameters["weight_vector_index"].GetInt();
        mNumberOfHromSets = ThisParameters["number_of_hrom_sets"].GetInt();
        ...
        TSystemVectorType &rDx)
    {
        ...
        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", (this->GetEchoLevel() > 0)) << "Build and project time: " << time << std::endl;
        ...
        TSystemVectorType& b) override
    {
        ...
        #pragma omp parallel firstprivate(nelements,nconditions, LHS_Contribution, RHS_Contribution, EquationId )
        {
            ...
            for (int k = 0; k < nelements; k++) {
                ...
                if (it_elem->IsActive()) {
                    ...
                    const double h_rom_weight = mHromSimulation ? it_elem->GetValue(HROM_WEIGHT)[mActiveHromSet] : 1.0;
            ...
            for (int k = 0; k < nconditions; k++) {
                ...
                if (it_cond->IsActive()) {
                    ...
                    //Get HROM weight and multiply it by its contribution
                    const double h_rom_weight = mHromSimulation ? it_cond->GetValue(HROM_WEIGHT)[mActiveHromSet] : 1.0;
                    LHS_Contribution *= h_rom_weight;
                    RHS_Contribution *= h_rom_weight;
        ...
        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", this->GetEchoLevel() >= 1) << "Build time: " << timer.ElapsedSeconds() << std::endl;

        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", (this->GetEchoLevel() > 2 && rModelPart.GetCommunicator().MyPID() == 0)) << "Finished parallel building" << std::endl;
        ...
        TSystemVectorType &rb)
    {
        ...
        if (mRightRomBasisInitialized==false){
            ...
            RomRBFUtility<TSparseSpace,TDenseSpace>::GetXAndDecoderGradient(r_xRom, r_xBase, mPhiGlobal, r_svdPhiMatrices, r_W_mat, r_centers_mat, r_kernel_type, r_kernel_eps, r_refSnapshot);
            ...
            IndexType& r_kernel_type =  r_root_mp.GetValue(KERNEL_TYPE);
            double& r_kernel_eps = r_root_mp.GetValue(KERNEL_EPS);
            RomRBFUtility<TSparseSpace,TDenseSpace>::GetDecoderGradient(r_xRomTotal, mPhiGlobal, r_svdPhiMatrices, r_W_mat, r_centers_mat, r_kernel_type, r_kernel_eps);
        }
        ...
        TSystemVectorType &rDx)
    {
        ...
        if(rEigenRomA.colPivHouseholderQr().isInvertible() == false){
            KRATOS_INFO("RbfPromGlobalROMBuilderAndSolver") << "Linear system to be solved by QR is not invertible" << std::endl;
        }
        dxrom_eigen = rEigenRomA.colPivHouseholderQr().solve(rEigenRomB);
        
        double time = solving_timer.ElapsedSeconds();
        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", (this->GetEchoLevel() > 0)) << "Solve reduced system time: " << time << std::endl;

        // project reduced solution back to full order modela
        const auto backward_projection_timer = BuiltinTimer();

        RomRBFUtility<TSparseSpace,TDenseSpace>::GetXAndDecoderGradient(r_xRom+r_dxRom,xNew, mPhiGlobal, svdPhiMatrices, r_W_mat, r_centers_mat, r_kernel_type, r_kernel_eps, refSnapshot);
        
        GetDxAndUpdateXBase(xNew, r_xBase, rDx);
        KRATOS_INFO_IF("RbfPromGlobalROMBuilderAndSolver", (this->GetEchoLevel() > 0)) << "Project to fine basis time: " << backward_projection_timer.ElapsedSeconds() << std::endl;
