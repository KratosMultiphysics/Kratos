...
#if !defined(KRATOS_CUSTOM_ROM_BUILDER_AND_SOLVER)
#define KRATOS_CUSTOM_ROM_BUILDER_AND_SOLVER
...
class CustomROMBuilderAndSolver : public BuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>
{
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(CustomROMBuilderAndSolver);
    ...
    typedef CustomROMBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver> ClassType;
    ...
    explicit CustomROMBuilderAndSolver(
    ...
    ~CustomROMBuilderAndSolver() = default;
        ...
        ModelPart &rModelPart) override
    {
        ...
        KRATOS_INFO_IF("CustomROMBuilderAndSolver", (this->GetEchoLevel() > 1)) << "Setting up the dofs" << std::endl;
        KRATOS_INFO_IF("CustomROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Number of threads" << ParallelUtilities::GetNumThreads() << "\n" << std::endl;
        KRATOS_INFO_IF("CustomROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Initializing element loop" << std::endl;
        ...
        KRATOS_INFO_IF("CustomROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Initializing ordered array filling\n" << std::endl;
        ...
        KRATOS_INFO_IF("CustomROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Number of degrees of freedom:" << BaseType::GetDofSet().size() << std::endl;
        KRATOS_INFO_IF("CustomROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Finished setting up the dofs" << std::endl;
    ...
    //     // const string export_dir = "~/Documentos/ML_ROM/KratosAI/saved_models_newexample/Dense_extended_SOnly_w0_lay40_LRe4_svd_white_nostand_1000ep/decoder_model_bestx/saved_model.pb" // Hard-code export-dir to be the saved_model's path

    //     // Status status = LoadSavedModel(session_options, run_options, export_dir, {kSavedModelTagServe}, &model_bundle);

    //     // // Inference
    //     // outputLayer = {"StatefulPartitionedCall:0", 
    //     //                 "StatefulPartitionedCall:4", 
    //     //                 "StatefulPartitionedCall:1", 
    //     //                 "StatefulPartitionedCall:5",
    //     //                 "StatefulPartitionedCall:3"};
    //     // Status runStatus = model_bundle.GetSession()->Run({{"serving_default_input_tensor:0", input_tensor}}, outputLayer, {}, &outputs);
    ...
    Parameters GetDefaultParameters() const override
    {
        ...
        {
            "name" : "custom_rom_builder_and_solver",
            "nodal_unknowns" : [],
    ...
    {
        return "custom_rom_builder_and_solver";
    }
    ...
    {
        return "CustomROMBuilderAndSolver";
    }
...
protected:
    ...
    */
        ...
        RomSystemVectorType &rb)
    {
        ...
        KRATOS_INFO_IF("CustomROMBuilderAndSolver", (this->GetEchoLevel() > 0)) << "Build time: " << assembling_timer.ElapsedSeconds() << std::endl;
        KRATOS_INFO_IF("CustomROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Finished parallel building" << std::endl;
        ...
        TSystemVectorType &rDx)
    {
        ...
        // KRATOS_WATCH(dxrom)
        KRATOS_INFO_IF("CustomROMBuilderAndSolver", (this->GetEchoLevel() > 0)) << "Solve reduced system time: " << solving_timer.ElapsedSeconds() << std::endl;
        ...
        KRATOS_INFO_IF("CustomROMBuilderAndSolver", (this->GetEchoLevel() > 0)) << "Project to fine basis time: " << backward_projection_timer.ElapsedSeconds() << std::endl;
...
private:
    ...
    ///@}
}; /* Class CustomROMBuilderAndSolver */
...
#endif /* KRATOS_CUSTOM_ROM_BUILDER_AND_SOLVER  defined */