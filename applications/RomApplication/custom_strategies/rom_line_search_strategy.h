...
protected:
    ...
    ) override
    {
        ...
        TSystemVectorType rDq = BaseType::GetModelPart().GetRootModelPart().GetValue(ROM_CURRENT_NON_LINEAR_INCREMENT);
        ...
        BaseType::BuildRomRHS(pScheme, BaseType::GetModelPart(), b, b_rom);
        ...
        while(!converged && it < mMaxLineSearchIterations) {
            ...
            if(rmin < mLineSearchTolerance*rmax) {
                ...
                noalias(BaseType::GetModelPart().GetRootModelPart().GetValue(ROM_CURRENT_NON_LINEAR_INCREMENT)) = auxDq;
                break;
            }
