...
public:
        ...
        mpBuilderAndSolver(pBuilderAndSolver)
    {
        ...
        // Set derivative parameter type flag
        const auto& derivative_parameter = InputParameters["derivative_parameter"].GetString();
        ...
        // Get the sub model part names for material parameter derivatives
        mDerivativeSubModelPartNames = InputParameters["sub_model_parts_list"].GetStringArray();
        const std::size_t number_of_sub_model_parts = mDerivativeSubModelPartNames.size();

        // Resize the vector of eigenvalues and derivatives
        if (mDerivativeType == DerivativeType::Static && mDerivativeParameterType == DerivativeParameterType::ModalCoordinate)   // Static derivatives wrt modal parameters are symmetric
            rModelPart.GetProcessInfo()[EIGENVALUE_VECTOR].resize(mNumberInitialBasis + mNumberInitialBasis * ( mNumberInitialBasis + 1 ) / 2, true);
        else if ( mDerivativeType == DerivativeType::Dynamic && mDerivativeParameterType == DerivativeParameterType::ModalCoordinate)   // Dynamic derivatives wrt modal parameters are unsymmetric
            rModelPart.GetProcessInfo()[EIGENVALUE_VECTOR].resize(mNumberInitialBasis*( mNumberInitialBasis + 1 ), true);
        else if (mDerivativeType == DerivativeType::Dynamic && mDerivativeParameterType != DerivativeParameterType::ModalCoordinate)  // Dynamic derivatives wrt material parameters
            rModelPart.GetProcessInfo()[EIGENVALUE_VECTOR].resize(mNumberInitialBasis*(1+number_of_sub_model_parts), true);
        
        // Set the ordering of DOFs in the input
        const auto nodal_variable_names = InputParameters["rom_settings"]["nodal_unknowns"].GetStringArray();
    ...
    void Initialize() override
    {
        ...
        KRATOS_INFO_IF("ModalDerivativeStrategy", this->GetEchoLevel() > 2) <<  "Entering Initialize" << std::endl;
        ...
        KRATOS_INFO_IF("ModalDerivativeStrategy", this->GetEchoLevel() > 2) <<  "Exiting Initialize" << std::endl;
    ...
    void InitializeSolutionStep() override
    {
        ...
        if (mSolutionStepIsInitialized == false)
        {
                ...
                p_builder_and_solver->GetReshapeMatrixFlag() == true)
            {
                ...
                KRATOS_INFO_IF("Setup System Time", BaseType::GetEchoLevel() > 0) << setup_system_time.ElapsedSeconds() << std::endl;
    ...
    void FinalizeSolutionStep() override
    {
        ...
        KRATOS_INFO_IF("ModalDerivativeStrategy", BaseType::GetEchoLevel() > 2 ) <<  "Entering FinalizeSolutionStep" << std::endl;
     ...
     * 
     */
    bool SolveSolutionStepStaticModalCoordinateDerivatives()
    {
        KRATOS_TRY
    ...
    /**
     * @brief Solves the current step for dynamic modal coordinate derivative case
     * 
     */
    bool SolveSolutionStepDynamicModalCoordinateDerivatives()
    {
        KRATOS_TRY
    ...
    /**
     * @brief Solves the current step for dynamic material parameter derivative case
     * 
     */
    bool SolveSolutionStepDynamicMaterialParameterDerivatives()
    {
        KRATOS_TRY
        ...
        */
        ...
        for (std::size_t basis_i = 0; basis_i < mNumberInitialBasis; basis_i++)
        {
            ...
            for (std::string sub_model_part_name : mDerivativeSubModelPartNames)
            {
                ...
                // Get sub model part
                auto& r_sub_model_part = r_model_part.GetSubModelPart(sub_model_part_name);
                ...
                KRATOS_INFO_IF("ModalDerivativeStrategy", this->GetEchoLevel() >= 1) << "Compute dBasis_" << basis_i+1 << "_dparameter_" << ++sub_model_part_ctr << " time: " << time_derivative.ElapsedSeconds() << std::endl;
    ...
    bool SolveSolutionStep() override
    {
        ...
        else if ( mDerivativeType == DerivativeType::Dynamic && mDerivativeParameterType != DerivativeParameterType::ModalCoordinate )
    ...
    void MassOrthonormalize(TSystemVectorType& rDx)
    {
        ...
        const std::size_t current_basis_index = r_model_part.GetProcessInfo()[DERIVATIVE_INDEX];
    ...
    void ComputeAndAddNullSpaceSolution(TSystemVectorType& rDx, TSystemVectorType& rBasis)
    {
        ...
        if (mDerivativeParameterType == DerivativeParameterType::Mass)
        {
            auto& r_model_part = BaseType::GetModelPart();
    ...
    void AssignVariables(TSystemVectorType& rDx)
    {
        ...
        block_for_each(
            r_model_part.Nodes(), [&](ModelPart::NodeType& r_node) 
            {
                ...
                const std::size_t derivative_index = r_model_part.GetProcessInfo()[DERIVATIVE_INDEX];
    ...
    int Check() override
    {
        ...
        KRATOS_INFO_IF("ModalDerivativeStrategy", BaseType::GetEchoLevel() > 2) <<  "Entering Check" << std::endl;
        ...
        KRATOS_INFO_IF("ModalDerivativeStrategy", BaseType::GetEchoLevel() > 2) <<  "Exiting Check" << std::endl;
...
private:
    ...
    enum DerivativeParameterType {ModalCoordinate, Stiffness, Mass};
