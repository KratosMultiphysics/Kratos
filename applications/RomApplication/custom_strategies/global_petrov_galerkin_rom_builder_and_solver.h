...
class GlobalPetrovGalerkinROMBuilderAndSolver : public ResidualBasedBlockBuilderAndSolver<TSparseSpace, TDenseSpace, TLinearSolver>
{
public:
    ...
    //Distributed, dense
    using PetrovGalerkinSystemMatrixType = RomSystemMatrixType;
    //      ^ Change this to a distributed dense type
    // Here you have not defined a new type for PetrovGalerkinSystemVectorType. You can add it in the same manner
        ...
        ModelPart &rModelPart) override
    {
        ...
        KRATOS_INFO_IF("GlobalPetrovGalerkinROMBuilderAndSolver", (this->GetEchoLevel() > 2)) << "Number of degrees of freedom:" << BaseType::GetDofSet().size() << std::endl;
        ...
        const ModelPart& rModelPart) 
    {
        ...
        block_for_each(r_dof_set, [&](const DofType& r_dof)
        {
            ...
            if (r_dof.IsFixed())
            {
                noalias(row(mPsiGlobal, r_dof.EquationId())) = ZeroVector(r_rom_nodal_basis.size2());
            }
            else{
                noalias(row(mPsiGlobal, r_dof.EquationId())) = row(r_rom_nodal_basis, row_id);
            }
        });
    }
    ...
    SizeType GetNumberOfLeftROMModes() const noexcept
    {
        return mNumberOfPetrovGalerkinRomModes;
    }
...
protected:
        ...
        TSystemVectorType &rb)
    {
        ...
        KRATOS_INFO_IF("GlobalPetrovGalerkinROMBuilderAndSolver", (this->GetEchoLevel() > 0)) << "Build and project time: " << time << std::endl;
        ...
        TSystemVectorType &rb)
    {
        ...
        if (mLeftRomBasisInitialized==false){
            mPsiGlobal = ZeroMatrix(BaseBuilderAndSolverType::GetEquationSystemSize(), GetNumberOfLeftROMModes());
            mLeftRomBasisInitialized = true;
        }

        BuildRightROMBasis(rModelPart);
        BuildLeftROMBasis(rModelPart);
        ...
        TSystemVectorType &rDx)
    {
        ...
        KRATOS_INFO_IF("GlobalPetrovGalerkinROMBuilderAndSolver", (this->GetEchoLevel() > 0)) << "Solve reduced system time: " << time << std::endl;
        ...
        KRATOS_INFO_IF("GlobalPetrovGalerkinROMBuilderAndSolver", (this->GetEchoLevel() > 0)) << "Project to fine basis time: " << backward_projection_timer.ElapsedSeconds() << std::endl;
