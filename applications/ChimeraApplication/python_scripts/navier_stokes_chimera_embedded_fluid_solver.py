...
from KratosMultiphysics.FluidDynamicsApplication.navier_stokes_embedded_solver import NavierStokesEmbeddedMonolithicSolver

def CreateSolver(main_model_part, custom_settings):
    return NavierStokesSolverChimeraEmbedded(main_model_part, custom_settings)

class NavierStokesSolverChimeraEmbedded(NavierStokesEmbeddedMonolithicSolver):
    def __init__(self, model, custom_settings):
        self.chimera_settings = custom_settings["chimera_settings"].Clone()
        custom_settings.RemoveValue("chimera_settings")
        super(NavierStokesSolverChimeraEmbedded,self).__init__(model,custom_settings)

        KratosMultiphysics.Logger.PrintInfo("NavierStokesSolverChimeraEmbedded", "Construction of NavierStokesSolverChimeraEmbedded finished.")

    def AddVariables(self):
        super(NavierStokesSolverChimeraEmbedded,self).AddVariables()
        ...
        KratosMultiphysics.Logger.PrintInfo("NavierStokesSolverChimeraEmbedded", "Fluid solver variables added correctly.")
    ...
    def ImportModelPart(self):
        if(self.settings["model_import_settings"]["input_type"].GetString() == "chimera"):
            ...
            KratosMultiphysics.Logger.PrintInfo("NavierStokesSolverChimeraEmbedded", " Import of all chimera modelparts completed.")
        else:# we can use the default implementation in the base class
            super(NavierStokesSolverChimeraEmbedded,self).ImportModelPart()
    ...
    def Initialize(self):
        ...
        KratosMultiphysics.Logger.PrintInfo("NavierStokesSolverChimeraEmbedded", "Solver initialization finished.")
