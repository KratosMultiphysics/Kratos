...
class ApplyChimeraProcessFractionalStep : public Process
{
  public:
	...
	ApplyChimeraProcessFractionalStep(ModelPart &MainModelPart, Parameters rParameters) : Process(Flags()), mrMainModelPart(MainModelPart), m_parameters(rParameters)
	{
		...
		this->pMpcVelocity = MpcDataPointerType(new MpcData("nearest_element"));
		this->pMpcPressure = MpcDataPointerType(new MpcData("nearest_element"));
	...
	void ApplyMpcConstraintConservative(ModelPart &rBoundaryModelPart, BinBasedPointLocatorPointerType &pBinLocator, MpcDataPointerType pMpcV,MpcDataPointerType pMpcP)
	{
		double rtMinvR = 0;
		DofVectorType slaveDofVector;
		double R = 0;
		ApplyMpcConstraintForFractionalStep(rBoundaryModelPart, pBinLocator, pMpcV,pMpcP);
