...
namespace Kratos
{
    ...
    void GatherModelPartOnAllRanksUtility::GatherModelPartOnAllRanks(ModelPart & rModelPartToGather, ModelPart & rGatheredModelPart)
    {
...
#pragma omp critical
        ...
        all_node_ids.erase( remove_if( begin(all_node_ids),end(all_node_ids),
            [&](IndexType x){return binary_search(begin(node_ids_to_keep),end(node_ids_to_keep),x);}), end(all_node_ids) );
