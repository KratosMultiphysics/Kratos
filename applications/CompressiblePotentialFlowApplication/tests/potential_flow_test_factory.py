...
class PotentialFlowTests(UnitTest.TestCase):
    ...
    def test_Naca0012SmallTransonic(self):
        ...
        with WorkFolderScope(work_folder):
            self._runTest(settings_file_name)
            self._check_results(self.main_model_part.ProcessInfo[CPFApp.LIFT_COEFFICIENT], 1.5009599984331534, 0.0, 1e-8)
            self._check_results(self.main_model_part.ProcessInfo[CPFApp.MOMENT_COEFFICIENT], -0.766873468342707, 0.0, 1e-8)
            self._check_results(self.main_model_part.ProcessInfo[CPFApp.LIFT_COEFFICIENT_JUMP], 1.48574768289569, 0.0, 1e-8)
            self._check_results(self.main_model_part.ProcessInfo[CPFApp.LIFT_COEFFICIENT_FAR_FIELD], 1.508363541202099, 0.0, 1e-8)
