...
KRATOS_TEST_CASE_IN_SUITE(ComputeUpwindFactorDerivativeWRTVelocitySquared0, CompressiblePotentialApplicationFastSuite) {
    Model this_model;
    ModelPart& model_part = this_model.CreateModelPart("Main", 3);

    AssignFreeStreamValues(model_part);

    const double local_mach_number_squared = 0.35;

    // velocity corresponding to mach number sqrt(0.35)
    const double local_velocity_squared = PotentialFlowUtilities::ComputeVelocityMagnitude<2, 3>(local_mach_number_squared, model_part.GetProcessInfo());

    array_1d<double, 2> current_velocity(2, 0.0);
    current_velocity[0] = std::sqrt(local_velocity_squared);

    const double upwind_mach_number_squared = 0.49;
    ...
    const size_t upwind_factor_case = 0.0;

    const double upwind_factor_derivative = PotentialFlowUtilities::ComputeUpwindFactorDerivativeWRTVelocitySquared<2,3>(upwind_factor_case, current_velocity, upwind_velocity,model_part.GetProcessInfo());
    
    KRATOS_CHECK_NEAR(upwind_factor_derivative, 0.0, 1e-15);
}

// tests the function ComputeUpwindFactorDerivativeWRTVelocitySquared from utilities
KRATOS_TEST_CASE_IN_SUITE(ComputeUpwindFactorDerivativeWRTVelocitySquared1, CompressiblePotentialApplicationFastSuite) {
    Model this_model;
    ModelPart& model_part = this_model.CreateModelPart("Main", 3);
    ...
    const size_t upwind_factor_case = 1.0;
...
// tests the function ComputeUpwindFactorDerivativeWRTVelocitySquared from utilities
KRATOS_TEST_CASE_IN_SUITE(ComputeUpwindFactorDerivativeWRTVelocitySquared2, CompressiblePotentialApplicationFastSuite) {
    Model this_model;
    ModelPart& model_part = this_model.CreateModelPart("Main", 3);
    ...
    const size_t upwind_factor_case = 2.0;
