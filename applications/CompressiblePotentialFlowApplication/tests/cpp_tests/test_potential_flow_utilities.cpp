...
KRATOS_TEST_CASE_IN_SUITE(ComputeUpwindFactorDerivativeWRTVelocitySquared, CompressiblePotentialApplicationFastSuite) {
    ...
    double upwind_factor_derivative = PotentialFlowUtilities::ComputeUpwindFactorDerivativeWRTVelocitySquared<2,3>(upwindFactorCase, rCurrentVelocity, rUpwindVelocity,model_part.GetProcessInfo());
