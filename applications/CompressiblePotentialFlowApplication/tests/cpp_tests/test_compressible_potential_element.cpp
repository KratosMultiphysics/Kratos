...
namespace Kratos {
  namespace Testing {
    ...
    void CreateNodes(ModelPart& rModelPart){         
		rModelPart.CreateNewNode(1, 0.0, 1.0, 0.0);
		rModelPart.CreateNewNode(2, 1.0, 1.0, 0.0);
		rModelPart.CreateNewNode(3, 1.0, 2.0, 0.0);
    }
	...
	void GenerateElementStressesMix(ModelPart& rModelPart)
    {
		...
		std::vector<ModelPart::IndexType> condNodes2{ 2, 3};
		std::vector<ModelPart::IndexType> condNodes3{ 3, 1};
		rModelPart.CreateNewElement("IncompressibleStressesMixPotentialFlowElement2D3N", 1, elemNodes1, pProp);
		rModelPart.CreateNewCondition("IncompressibleStressesPotentialWallCondition2D2N", 1, condNodes1, pProp);
		rModelPart.CreateNewCondition("IncompressibleStressesPotentialWallCondition2D2N", 2, condNodes2, pProp);
		rModelPart.CreateNewCondition("IncompressibleStressesPotentialWallCondition2D2N", 3, condNodes3, pProp);
    }
...
std::cout<<"hey2 	"<<std::endl;
	...
	KRATOS_TEST_CASE_IN_SUITE(CompressiblePotentialFlowElement_CalculateLocalSystemStressesMix, CompressiblePotentialApplicationFastSuite)
    {
      	std::cout<<std::endl;
		Model this_model;
		ModelPart& model_part = this_model.CreateModelPart("Main", 3);
		GenerateElementStressesMix(model_part);
		model_part.GetProcessInfo().SetValue(WATER_PRESSURE,1000.0);
		model_part.GetProcessInfo().SetValue(TEMPERATURE,1000.0);
		SolveCutElement(model_part);
    }
	...
	KRATOS_TEST_CASE_IN_SUITE(CompressiblePotentialFlowElement_CalculateLocalSystemTangent, CompressiblePotentialApplicationFastSuite)
    {
      	std::cout<<std::endl;
		Model this_model;
		ModelPart& model_part = this_model.CreateModelPart("Main", 3);
    	...
    	int NumNodes = 3;
		for (unsigned int i = 0; i < 3; i++){
			potential(i) =0.316228*pElement -> GetGeometry()[i].X()+0.948683*pElement->GetGeometry()[i].Y();//
		}
		KRATOS_WATCH(potential)
		distances(0) = 3.0;
		distances(1) = -3.0;
		distances(2) = -1.0;
		...
		std::cout<< "Incompressible_stresses_tangent" << std::endl;
		KRATOS_WATCH(LHS);
