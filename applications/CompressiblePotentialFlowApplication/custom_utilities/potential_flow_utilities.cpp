        ...
        if (rElement.GetGeometry()[i].GetValue(KUTTA))
        {
            ...
            if (wake)
            {
                const array_1d<double, Dim> lower_velocity =
                    ComputePerturbedVelocityLowerElement<Dim, NumNodes>(
...
template void AddKuttaConditionPenaltyTermPerturbation<2, 3>(const Element& rElement, Matrix& rLeftHandSideMatrix, const ProcessInfo& rCurrentProcessInfo);
template void AddKuttaConditionPenaltyTermPerturbation<3, 4>(const Element& rElement,  Matrix& rLeftHandSideMatrix, const ProcessInfo& rCurrentProcessInfo);
template void AddKuttaConditionPenaltyTermPerturbation<2, 3>(const Element& rElement, Vector& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo);
template void AddKuttaConditionPenaltyTermPerturbation<3, 4>(const Element& rElement, Vector& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo);
