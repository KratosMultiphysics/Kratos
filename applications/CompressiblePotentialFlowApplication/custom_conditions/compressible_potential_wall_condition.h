...
#ifndef KRATOS_COMPRESSIBLE_POTENTIAL_WALL_CONDITION_H
#define KRATOS_COMPRESSIBLE_POTENTIAL_WALL_CONDITION_H
...
class CompressiblePotentialWallCondition : public Condition
{
public:
    ///@name Type Definitions
    ///@{
    ...
    /** Admits an Id as a parameter.
      ...
      */
    CompressiblePotentialWallCondition(IndexType NewId = 0):
        Condition(NewId)
    {
    }
    ...
    /// Copy constructor.
    CompressiblePotentialWallCondition(CompressiblePotentialWallCondition const& rOther):
        Condition(rOther)
    {
    }
    ...
    /// Copy constructor
    CompressiblePotentialWallCondition & operator=(CompressiblePotentialWallCondition const& rOther)
    {
    ...
    /// Create a new CompressiblePotentialWallCondition object.
    /**
      @param NewId Index of the new condition
      @param ThisNodes An array containing the nodes of the new condition
      @param pProperties Pointer to the element's properties
    ...
    {
        return Condition::Pointer(new CompressiblePotentialWallCondition(NewId, GetGeometry().Create(ThisNodes), pProperties));
    }
    ...
    {
        return Condition::Pointer(new CompressiblePotentialWallCondition(NewId, pGeom, pProperties));
    }
...
private:
    ...
    inline std::istream& operator >> (std::istream& rIStream,
                                      CompressiblePotentialWallCondition<TDim,TNumNodes>& rThis)
    {
        return rIStream;
    }
    ...
    inline std::ostream& operator << (std::ostream& rOStream,
                                      const CompressiblePotentialWallCondition<TDim,TNumNodes>& rThis)
    {
