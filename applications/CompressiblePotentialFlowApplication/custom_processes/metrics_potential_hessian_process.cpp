...
void ComputePotentialHessianSolMetricProcess::Execute()
{
...
array_1d<double, 3 * (TDim - 1)> ComputePotentialHessianSolMetricProcess::ComputeHessianMetricTensor(
    ...
    )
{
    ...
    if (interpolation_error < std::numeric_limits<double>::epsilon()) { // In practice, the Hessian of function u can be 0, e.g. if u is linear, then |Hu| is not definite. In this particular case, the interpolation error is 0 and we want to prescribe a mesh size which is infinite. To solve this issue, this infinite size prescription is truncated by imposing maximal size hmax . This is equivalent to truncate tiny eigenvalues by lambda  = 1/hmax^2 . See [1] pag. 34
        KRATOS_WARNING("ComputePotentialHessianSolMetricProcess") << "WARNING: Your interpolation error is near zero: " << interpolation_error  <<  ". Computing a local L(inf) upper bound of the interpolation error"<< std::endl;
...
void ComputePotentialHessianSolMetricProcess::CalculateAuxiliarHessian()
{
...
double ComputePotentialHessianSolMetricProcess::CalculateAnisotropicRatio(
...
template<SizeType TDim>
void ComputePotentialHessianSolMetricProcess::CalculateMetric()
{
    /// The type of array considered for the tensor
    typedef typename std::conditional<TDim == 2, array_1d<double, 3>, array_1d<double, 6>>::type TensorArrayType;
...
Parameters ComputePotentialHessianSolMetricProcess::GetDefaultParameters() const
{
...
void ComputePotentialHessianSolMetricProcess::InitializeVariables(Parameters ThisParameters)
{
