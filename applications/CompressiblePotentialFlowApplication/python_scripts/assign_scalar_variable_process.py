        ...
        if self.interval.IsInInterval(current_time):
            ...
            if self.value_is_numeric:
                ...
                if self.aux_function.DependsOnSpace() == False: #depends on time only
                    ...
                    self.model_part.ProcessInfo.SetValue( self.variable, self.value )
                    print(self.variable_name, "  ",self.value,"\n")
                ...
                self.model_part.ProcessInfo.SetValue( self.variable, self.value )
                print(self.variable_name, "  ",self.value,"\n")
                self.u_inf = self.model_part.ProcessInfo.GetValue(KratosMultiphysics.KratosGlobals.GetVariable("FREE_STREAM_MACH")) * self.model_part.ProcessInfo.GetValue(KratosMultiphysics.KratosGlobals.GetVariable("SOUND_VELOCITY"))
                self.free_stream_velocity = KratosMultiphysics.Vector(3)
                self.free_stream_velocity[0] = round(self.u_inf*math.cos(0.0534070751),8)
                self.free_stream_velocity[1] = 0.0
                self.free_stream_velocity[2] = round(self.u_inf*math.sin(0.0534070751),8)

                self.free_stream_velocity_direction = self.free_stream_velocity / self.u_inf

                self.model_part.ProcessInfo.SetValue(KratosMultiphysics.KratosGlobals.GetVariable("FREE_STREAM_VELOCITY"),self.free_stream_velocity)
                self.model_part.ProcessInfo.SetValue(KratosMultiphysics.KratosGlobals.GetVariable("FREE_STREAM_VELOCITY_DIRECTION"),self.free_stream_velocity_direction)
