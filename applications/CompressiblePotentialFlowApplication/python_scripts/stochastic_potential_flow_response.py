    ...
    def saveXMC(self, xmc_analysis):
        ...
        for i, index in enumerate(xmc_analysis.monteCarloSampler.indices):
            ...
            for j in range(len(this_index_random)):
                for level in range(len(this_index_random[j])):
                    for k in range(len(this_index_random[j][level])):
                        if not type(this_index_random[j][level][k]) == int:
                            this_index_random[j][level][k] = float(this_index_random[j][level][k])
