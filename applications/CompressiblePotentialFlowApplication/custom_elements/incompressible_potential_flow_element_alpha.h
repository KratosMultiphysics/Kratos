...
#if !defined(KRATOS_INCOMPRESSIBLE_ALPHA_POTENTIAL_FLOW_ELEMENT_H_INCLUDED )
#define KRATOS_INCOMPRESSIBLE_ALPHA_POTENTIAL_FLOW_ELEMENT_H_INCLUDED
...
class IncompressibleAlphaPotentialFlowElement : public Element
{
public:
    ...
    /// Pointer definition of IncompressibleAlphaPotentialFlowElement
    KRATOS_CLASS_POINTER_DEFINITION(IncompressibleAlphaPotentialFlowElement);
     ...
     * Constructor.
     */
    IncompressibleAlphaPotentialFlowElement(IndexType NewId = 0) {};

    /**
     * Constructor using an array of nodes
     */
    IncompressibleAlphaPotentialFlowElement(IndexType NewId, const NodesArrayType& ThisNodes):Element(NewId, ThisNodes) {};

    /**
     * Constructor using Geometry
     */
    IncompressibleAlphaPotentialFlowElement(IndexType NewId, GeometryType::Pointer pGeometry):Element(NewId, pGeometry) {};

    /**
     * Constructor using Properties
     */
    IncompressibleAlphaPotentialFlowElement(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties):Element(NewId, pGeometry, pProperties) {};

    /**
     * Copy Constructor
     */
    IncompressibleAlphaPotentialFlowElement(IncompressibleAlphaPotentialFlowElement const& rOther) {};

    /**
     * Destructor
     */
    ~IncompressibleAlphaPotentialFlowElement() override {};
    ...
    /// Assignment operator.
    IncompressibleAlphaPotentialFlowElement & operator=(IncompressibleAlphaPotentialFlowElement const& rOther)
    {
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new IncompressibleAlphaPotentialFlowElement(NewId, GetGeometry().Create(ThisNodes), pProperties));
        KRATOS_CATCH("");
    }
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new IncompressibleAlphaPotentialFlowElement(NewId, pGeom, pProperties));
        KRATOS_CATCH("");
    }
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new IncompressibleAlphaPotentialFlowElement(NewId, GetGeometry().Create(ThisNodes), pGetProperties()));
        KRATOS_CATCH("");
    }
        ...
        ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        {   
            ...
            Matrix lhs_sigma_plus_alpha = alpha*data.vol*prod(sigma_shape_func,trans(data.DN_DX));
            Matrix lhs_sigma_minus_alpha = -alpha*data.vol*prod(sigma_shape_func,trans(data.DN_DX));
    ...
    int Check(const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        if (this->Id() < 1)
        {
            KRATOS_THROW_ERROR(std::logic_error, "IncompressibleAlphaPotentialFlowElement found with Id 0 or negative", "")
        }

        if (this->GetGeometry().Area() <= 0)
        {
            std::cout << "error on IncompressibleAlphaPotentialFlowElement -> " << this->Id() << std::endl;
    ...
    std::string Info() const override
    {
        ...
        buffer << "IncompressibleAlphaPotentialFlowElement #" << Id();
        return buffer.str();
    }
...
/// Print information about this object.
    ...
    {
        rOStream << "IncompressibleAlphaPotentialFlowElement #" << Id();
    }
...
}; // Class IncompressibleAlphaPotentialFlowElement
