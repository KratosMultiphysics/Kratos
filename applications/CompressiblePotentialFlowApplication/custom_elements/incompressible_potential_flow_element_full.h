...
#if !defined(KRATOS_INCOMPRESSIBLE_FULL_POTENTIAL_FLOW_ELEMENT_H_INCLUDED )
#define KRATOS_INCOMPRESSIBLE_FULL_POTENTIAL_FLOW_ELEMENT_H_INCLUDED
...
class IncompressibleFullPotentialFlowElement : public Element
{
public:
    ...
    /// Pointer definition of IncompressibleFullPotentialFlowElement
    KRATOS_CLASS_POINTER_DEFINITION(IncompressibleFullPotentialFlowElement);
     ...
     * Constructor.
     */
    IncompressibleFullPotentialFlowElement(IndexType NewId = 0) {};

    /**
     * Constructor using an array of nodes
     */
    IncompressibleFullPotentialFlowElement(IndexType NewId, const NodesArrayType& ThisNodes):Element(NewId, ThisNodes) {};

    /**
     * Constructor using Geometry
     */
    IncompressibleFullPotentialFlowElement(IndexType NewId, GeometryType::Pointer pGeometry):Element(NewId, pGeometry) {};

    /**
     * Constructor using Properties
     */
    IncompressibleFullPotentialFlowElement(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties):Element(NewId, pGeometry, pProperties) {};

    /**
     * Copy Constructor
     */
    IncompressibleFullPotentialFlowElement(IncompressibleFullPotentialFlowElement const& rOther) {};

    /**
     * Destructor
     */
    ~IncompressibleFullPotentialFlowElement() override {};
    ...
    /// Assignment operator.
    IncompressibleFullPotentialFlowElement & operator=(IncompressibleFullPotentialFlowElement const& rOther)
    {
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new IncompressibleFullPotentialFlowElement(NewId, GetGeometry().Create(ThisNodes), pProperties));
        KRATOS_CATCH("");
    }
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new IncompressibleFullPotentialFlowElement(NewId, pGeom, pProperties));
        KRATOS_CATCH("");
    }
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new IncompressibleFullPotentialFlowElement(NewId, GetGeometry().Create(ThisNodes), pGetProperties()));
        KRATOS_CATCH("");
    }
            ...
            if(kutta_element)//false
            {
                ...
                for(unsigned int i=0; i<NumNodes; ++i)
                {
                    if (GetGeometry()[i].Is(STRUCTURE)){
                        ...
                        for(unsigned int j=0; j<NumNodes; ++j)
                        {
                            rLeftHandSideMatrix(i,j)                   =  lhs_positive(i,j)+lhs_negative(i,j);
                            rLeftHandSideMatrix(i+NumNodes,j+NumNodes) =  lhs_negative(i,j)+lhs_positive(i,j);                            
                            if(data.distances[i]<0)
                                rLeftHandSideMatrix(i,j+NumNodes)          =  -lhs_positive(i,j)-lhs_negative(i,j);   
                            if(data.distances[i]>0)                           
                                rLeftHandSideMatrix(i+NumNodes,j)          =  -lhs_positive(i,j)-lhs_negative(i,j);
                        }
    ...
    int Check(const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        if (this->Id() < 1)
        {
            KRATOS_THROW_ERROR(std::logic_error, "IncompressibleFullPotentialFlowElement found with Id 0 or negative", "")
        }

        if (this->GetGeometry().Area() <= 0)
        {
            std::cout << "error on IncompressibleFullPotentialFlowElement -> " << this->Id() << std::endl;
    ...
    std::string Info() const override
    {
        ...
        buffer << "IncompressibleFullPotentialFlowElement #" << Id();
        return buffer.str();
    }
...
/// Print information about this object.
    ...
    {
        rOStream << "IncompressibleFullPotentialFlowElement #" << Id();
    }
...
}; // Class IncompressibleFullPotentialFlowElement
