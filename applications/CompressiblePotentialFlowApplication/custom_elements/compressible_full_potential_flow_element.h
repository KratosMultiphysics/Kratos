...
#if !defined(KRATOS_COMPRESSIBLE_FULL_POTENTIAL_FLOW_ELEMENT_H_INCLUDED )
#define KRATOS_COMPRESSIBLE_FULL_POTENTIAL_FLOW_ELEMENT_H_INCLUDED
...
class CompressibleFullPotentialFlowElement : public Element
{
public:
    ...
    /// Pointer definition of CompressibleFullPotentialFlowElement
    KRATOS_CLASS_POINTER_DEFINITION(CompressibleFullPotentialFlowElement);
     ...
     * Constructor.
     */
    CompressibleFullPotentialFlowElement(IndexType NewId = 0) {};

    /**
     * Constructor using an array of nodes
     */
    CompressibleFullPotentialFlowElement(IndexType NewId, const NodesArrayType& ThisNodes):Element(NewId, ThisNodes) {};

    /**
     * Constructor using Geometry
     */
    CompressibleFullPotentialFlowElement(IndexType NewId, GeometryType::Pointer pGeometry):Element(NewId, pGeometry) {};

    /**
     * Constructor using Properties
     */
    CompressibleFullPotentialFlowElement(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties):Element(NewId, pGeometry, pProperties) {};

    /**
     * Copy Constructor
     */
    CompressibleFullPotentialFlowElement(CompressibleFullPotentialFlowElement const& rOther) {};

    /**
     * Destructor
     */
    ~CompressibleFullPotentialFlowElement() override {};
    ...
    /// Assignment operator.
    CompressibleFullPotentialFlowElement & operator=(CompressibleFullPotentialFlowElement const& rOther)
    {
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new CompressibleFullPotentialFlowElement(NewId, GetGeometry().Create(ThisNodes), pProperties));
        KRATOS_CATCH("");
    }
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new CompressibleFullPotentialFlowElement(NewId, pGeom, pProperties));
        KRATOS_CATCH("");
    }
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new CompressibleFullPotentialFlowElement(NewId, GetGeometry().Create(ThisNodes), pGetProperties()));
        KRATOS_CATCH("");
    }
        ...
        ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        else //it is a wake element
        {
            ...
            Matrix laplacian_total = ZeroMatrix(NumNodes,NumNodes);
                    ...
                    if (GetGeometry()[i].GetValue(TRAILING_EDGE)){
                        ...
                        // {
                            for(unsigned int j=0; j<NumNodes; ++j)
                            {
                                ...
                                rLaplacianMatrix(i,j)            =  laplacian_total(i,j);
                                rLaplacianMatrix(i,j+NumNodes)   =  0.0;
    ...
    int Check(const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        if (this->Id() < 1)
        {
            KRATOS_THROW_ERROR(std::logic_error, "CompressibleFullPotentialFlowElement found with Id 0 or negative", "")
        }

        if (this->GetGeometry().Area() <= 0)
        {
            std::cout << "error on CompressibleFullPotentialFlowElement -> " << this->Id() << std::endl;
    ...
    void GetValueOnIntegrationPoints(const Variable<int>& rVariable, std::vector<int>& rValues, const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        if (rValues.size() != 1)
            ...
            rValues[0] = r_this.GetValue(KUTTA);
    ...
    std::string Info() const override
    {
        ...
        buffer << "CompressibleFullPotentialFlowElement #" << Id();
        return buffer.str();
    }
...
/// Print information about this object.
    ...
    {
        rOStream << "CompressibleFullPotentialFlowElement #" << Id();
    }
...
}; // Class CompressibleFullPotentialFlowElement
