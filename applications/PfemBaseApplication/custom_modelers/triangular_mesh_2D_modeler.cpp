...
namespace Kratos
{
  ...
  void TriangularMesh2DModeler::DeleteContainer(ModelerUtilities::MeshContainer& rMesh, struct triangulateio& tr)
  {
    KRATOS_TRY
					   ...
					   ModelPart::IndexType MeshId)
  {
    ...
    if(rMeshingVariables.Options.Is(ModelerUtilities::REFINE)){
      ...
      rMeshingVariables.InputInitializedFlag=true; //second set must be true	  
      ////////////////////////////////////////////////////////////
      ...
      for(int el=0; el<out.numberoftriangles; el++)
	{
	  if( rMeshingVariables.PreservedElements[el]){
	    for(int pn=0; pn<3; pn++)
	      {
		//set vertices
		rNodes[out.trianglelist[el*3+pn]].Set(BLOCKED);
	      }
	  }
      ...
      for(int el=0; el<rNumberOfElements; el++)
	{
	  if( rMeshingVariables.PreservedElements[el]){
	    for(int pn=0; pn<3; pn++)
	      {
		//set vertices
		rNodes[pElementList[el*3+pn]].Set(BLOCKED);
	      }
	  }
						     ...
						     ModelPart::IndexType MeshId)
  {
    ...
    // j=0;
    // if(rMeshingVariables.InputInitiliazedFlag){
	...
	for(int el = 0; el< in.numberoftriangles; el++)
	  {
	    ...
	    PointsArray.push_back( *( (nodes_begin +  in.trianglelist[base]-1).base() ) ); 
	    PointsArray.push_back( *( (nodes_begin +  in.trianglelist[base+1]-1).base() ) ); 
	    PointsArray.push_back( *( (nodes_begin +  in.trianglelist[base+2]-1).base() ) ); 
