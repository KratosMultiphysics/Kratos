...
namespace Kratos
{
  ...
  public:
    ...
    * @date   April, 2017
    * @brief  calculates the binomial coefficients using boost. 
    *           B(n)
    *            (k)
    */
	...
	/*
	* counts the number of knot spans in a given knot vector.
	*/
	static int get_number_of_knot_spans(const Vector &knot_vector, const double &accuracy)
	{
		int number_of_elements = 0;
		for (int i = 1; i < knot_vector.size(); i++)
		{
			if (abs(knot_vector(i) - knot_vector(i - 1)) > accuracy)
				number_of_elements++;
		}
	...
	/**
	* evaluates all non-zero B-Spline basis functions (recursive formula)
	...
	*/
	...
	/*
	* returns the corresponding knot span based on a specified u
	* Piegl,L. and Tiller,W. "The NURBS Book - 2nd Edition", Springer Verlag
	* Algorithm A2.1
	* @date       December, 2016
