...
KRATOS_TEST_CASE_IN_SUITE(HDF5_Internals_ConnectivitiesData1, KratosHDF5TestSuite)
{
    ...
    HDF5::WriteInfo info;
    data.WriteData(*p_file, "/Elements", info);
    data.Clear();
    KRATOS_CHECK(data.size() == 0);
    data.ReadData(*p_file, "/Elements", info.StartIndex, info.BlockSize);
...
KRATOS_TEST_CASE_IN_SUITE(HDF5_Internals_ConnectivitiesData2, KratosHDF5TestSuite)
{
    ...
    HDF5::File::Pointer p_file = pGetFile();
    HDF5::WriteInfo info;
    data.WriteData(*p_file, "/Conditions", info);
    data.Clear();
    KRATOS_CHECK(data.size() == 0);
    data.ReadData(*p_file, "/Conditions", info.StartIndex, info.BlockSize);
