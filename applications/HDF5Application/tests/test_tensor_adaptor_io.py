...
class TestTensorAdaptorIO(UnitTest.TestCase):
    ...
    def __TestTensorAdaptor(self, variable, container):
        ...
        ta_out, ta_attributes_out = tensor_adaptor_io.Read(f"{container.__class__.__name__}")
        self.assertTrue(ta_attributes_out.IsEquivalentTo(attributes_in))
        self.assertEqual(np.linalg.norm(ta_out.data - ta.data), 0)

    def __TestWriteVariableAndReadExpression(self, variable, hdf5_io_type, container, prefix: str):
        ...
        ta_read, _ = KratosHDF5.TensorAdaptorIO(tensor_io_params, self.h5_file).Read(variable.Name())
    ...
    def test_WriteVariableReadExpressionScalar(self):
        self.__TestWriteVariableAndReadExpression(Kratos.PRESSURE, KratosHDF5.HDF5NodalDataValueIO, self.model_part.GetCommunicator().LocalMesh().Nodes, "/ResultsData/NodalDataValues/")
        self.__TestWriteVariableAndReadExpression(Kratos.PRESSURE, KratosHDF5.HDF5ConditionDataValueIO, self.model_part.GetCommunicator().LocalMesh().Conditions, "/ResultsData/ConditionDataValues/")
        self.__TestWriteVariableAndReadExpression(Kratos.PRESSURE, KratosHDF5.HDF5ElementDataValueIO, self.model_part.GetCommunicator().LocalMesh().Elements, "/ResultsData/ElementDataValues/")

    def test_WriteVariableReadExpressionArray3(self):
        self.__TestWriteVariableAndReadExpression(Kratos.VELOCITY, KratosHDF5.HDF5NodalDataValueIO, self.model_part.GetCommunicator().LocalMesh().Nodes, "/ResultsData/NodalDataValues/")
        self.__TestWriteVariableAndReadExpression(Kratos.VELOCITY, KratosHDF5.HDF5ConditionDataValueIO, self.model_part.GetCommunicator().LocalMesh().Conditions, "/ResultsData/ConditionDataValues/")
        self.__TestWriteVariableAndReadExpression(Kratos.VELOCITY, KratosHDF5.HDF5ElementDataValueIO, self.model_part.GetCommunicator().LocalMesh().Elements, "/ResultsData/ElementDataValues/")

    def test_WriteVariableReadExpressionVector(self):
        self.__TestWriteVariableAndReadExpression(Kratos.GREEN_LAGRANGE_STRAIN_VECTOR, KratosHDF5.HDF5NodalDataValueIO, self.model_part.GetCommunicator().LocalMesh().Nodes, "/ResultsData/NodalDataValues/")
        self.__TestWriteVariableAndReadExpression(Kratos.GREEN_LAGRANGE_STRAIN_VECTOR, KratosHDF5.HDF5ConditionDataValueIO, self.model_part.GetCommunicator().LocalMesh().Conditions, "/ResultsData/ConditionDataValues/")
        self.__TestWriteVariableAndReadExpression(Kratos.GREEN_LAGRANGE_STRAIN_VECTOR, KratosHDF5.HDF5ElementDataValueIO, self.model_part.GetCommunicator().LocalMesh().Elements, "/ResultsData/ElementDataValues/")

    def test_WriteVariableReadExpressionMatrix(self):
        self.__TestWriteVariableAndReadExpression(Kratos.GREEN_LAGRANGE_STRAIN_TENSOR, KratosHDF5.HDF5NodalDataValueIO, self.model_part.GetCommunicator().LocalMesh().Nodes, "/ResultsData/NodalDataValues/")
        self.__TestWriteVariableAndReadExpression(Kratos.GREEN_LAGRANGE_STRAIN_TENSOR, KratosHDF5.HDF5ConditionDataValueIO, self.model_part.GetCommunicator().LocalMesh().Conditions, "/ResultsData/ConditionDataValues/")
        self.__TestWriteVariableAndReadExpression(Kratos.GREEN_LAGRANGE_STRAIN_TENSOR, KratosHDF5.HDF5ElementDataValueIO, self.model_part.GetCommunicator().LocalMesh().Elements, "/ResultsData/ElementDataValues/")
