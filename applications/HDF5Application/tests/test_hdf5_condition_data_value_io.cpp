...
KRATOS_TEST_CASE_IN_SUITE(HDF5PointsData_ReadConditionResults, KratosHDF5TestSuite)
{
    ...
    TestModelPartFactory::CreateModelPart(r_write_model_part, {{"Element2D3N"}}, {{"Condition2D2N"}});
    ...
    {
        TestModelPartFactory::AssignDataValueContainer(r_condition.Data(), r_condition, variables_list);
    }
    ...
    {
        HDF5::ConditionType& r_read_condition = r_read_model_part.Conditions()[r_write_condition.Id()];
        CompareDataValueContainers(r_read_condition.Data(), r_read_condition, r_write_condition.Data(), r_write_condition);
    }
}
