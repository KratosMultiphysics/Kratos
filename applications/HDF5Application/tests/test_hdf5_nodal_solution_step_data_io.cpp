...
KRATOS_TEST_CASE_IN_SUITE(HDF5PointsData_ReadNodalResults, KratosHDF5TestSuite)
{
    ...
    read_model_part.AddNodalSolutionStepVariable(DISPLACEMENT); // array_1d
    read_model_part.AddNodalSolutionStepVariable(PRESSURE); // double
    read_model_part.AddNodalSolutionStepVariable(REFINEMENT_LEVEL); // int
    write_model_part.AddNodalSolutionStepVariable(DISPLACEMENT); // array_1d
    write_model_part.AddNodalSolutionStepVariable(PRESSURE); // double
    write_model_part.AddNodalSolutionStepVariable(REFINEMENT_LEVEL); // int
    for (unsigned i = 0; i < 15; ++i)
    {
        read_model_part.CreateNewNode(i + 1, 0.0, 0.0, 0.0);
        write_model_part.CreateNewNode(i + 1, 0.0, 0.0, 0.0);
    }
    read_model_part.SetBufferSize(2);
    write_model_part.SetBufferSize(2);

    for (auto& r_node : write_model_part.Nodes())
    {
    ...
    data_io.WriteNodalResults(write_model_part.Nodes());
    data_io.ReadNodalResults(read_model_part.Nodes(), read_model_part.GetCommunicator());
    for (unsigned i = 0; i < 15; ++i)
    {
        HDF5::NodeType& r_read_node = read_model_part.Nodes()[i + 1];
        HDF5::NodeType& r_write_node = write_model_part.Nodes()[i + 1];
