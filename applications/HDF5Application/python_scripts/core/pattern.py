...
def IdentifyPattern(entity_name: str) -> 'tuple[str, dict[str, typing.Any]]':
    """
    @brief Identifies and replaces all floating-point number patterns in the given entity name string.

    @detail This function scans the input string for substrings that match the floating-point number pattern
    ...
    @param entity_name The input string potentially containing floating-point numbers.

    @return tuple
        pattern: The input string with all float values replaced by their corresponding tags.
        tags_dict: A dictionary mapping each tag to the `float` type.
    """
