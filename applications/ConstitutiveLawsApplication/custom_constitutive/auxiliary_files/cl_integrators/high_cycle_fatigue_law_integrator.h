...
public:
                                            ...
                                            const double RefTemp = 0.0)
	{
        ...
        if (std::abs(ReversionFactor) < 1.0) {
            // KRATOS_WATCH("sii")
            rSth = Se + delta_t + (UltimateStress - Se - delta_t) * std::pow((0.5 + 0.5 * ReversionFactor), STHR1);
			rAlphat = ALFAF + (0.5 + 0.5 * ReversionFactor) * AUXR1;
        } else {
            // KRATOS_WATCH("NOO")
            rSth = Se + delta_t + (UltimateStress - Se - delta_t) * std::pow((0.5 + 0.5 / ReversionFactor), STHR2);
                                                                ...
                                                                const double FatigueReductionFactorSmoothness)
	{
        ...
        if (MaxStress > Sth) { //In those cases with no fatigue in course (MaxStress < Sth), tbe fatigue reduction factor does not evolve.
            ...
            const double min_fatigue_reduction_factor = 0.1;
     ...
     * @param MaterialParameters Material properties.
     */
    static double UltimateStressDamage(ConstitutiveLaw::Parameters& rValues, const double RefTemp = 0.0)
	{
