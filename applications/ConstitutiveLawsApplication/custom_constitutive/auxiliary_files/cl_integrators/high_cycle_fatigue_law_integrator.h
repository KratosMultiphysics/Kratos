...
public:
                                                                ...
                                                                double& rWohlerStress)
	{
        ...
        if (MaxStress > Sth) {
            const double aux_fred = std::exp(-B0 * std::pow(std::log10(static_cast<double>(LocalNumberOfCycles)), (BETAF * BETAF)));
            rFatigueReductionFactor = (std::isnan(aux_fred)) ? 1.0 : aux_fred;
