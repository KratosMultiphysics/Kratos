    ...
    double& rhmin)    
{
    ...
    for (int i = 0; i < N.size(); ++i) {
        damage_value_at_gauss_point += N[i] * rParametersValues.GetElementGeometry()[i].FastGetSolutionStepValue(DAMAGE_VARIABLE, 0);
    }
    const auto& dNdx = rParametersValues.GetShapeFunctionsDerivatives();    
    array_1d<double, 3> damage_derivative_at_gauss_point(0.0);    
    for (int i = 0; i < N.size(); ++i) {
        for (int j = 0; j < 3; ++j) {
            damage_derivative_at_gauss_point[j] += dNdx(i, j) * rParametersValues.GetElementGeometry()[i].FastGetSolutionStepValue(DAMAGE_VARIABLE, 0);
        }
    }
