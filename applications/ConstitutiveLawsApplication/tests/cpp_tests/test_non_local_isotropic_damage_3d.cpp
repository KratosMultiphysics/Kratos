...
KRATOS_TEST_CASE_IN_SUITE(_ConstitutiveLaw_NonLocalElasticIsotropicDamage, KratosConstitutiveLawsFastSuite)
{
    ...
    // Create the CL
    NonLocalElasticIsotropicDamage cl = NonLocalElasticIsotropicDamage();
    ...
    KRATOS_CHECK(cl.Has(DAMAGE_VARIABLE));  // = True
    double damage_variable;
    damage_variable = 0.106572;
    cl.SetValue(DAMAGE_VARIABLE, damage_variable, test_model_part.GetProcessInfo());
    double damage_variable_r; 
    cl.GetValue(DAMAGE_VARIABLE, damage_variable_r);
    KRATOS_CHECK_NEAR(damage_variable_r, 0.106572, 1.e-5);  // = True
