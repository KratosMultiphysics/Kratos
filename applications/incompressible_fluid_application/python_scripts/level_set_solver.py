...
class LevelSetSolver:
    ...
    def __init__(self, base_model_part, fluid_model_part,
                 extrapolation_model_part, domain_size, extrapolation_distance, body_force):
        ...
        self.extrapolation_mesh_neighbour_search = FindNodalNeighboursProcess(
            extrapolation_model_part, number_of_avg_elems, number_of_avg_nodes)
...
# self.pressure_linear_solver =  BICGSTABSolver(1e-4, 5000,pILUPrecond)
    ...
    def Initialize(self):
        ...
        if(self.reform_convection_matrix == False):
            ...
            # construct system -- could be done once if the mesh does not
            # change
        ...
        if(self.extrapolation_type == 2):
            ...
            self.extrapolation_solver = ResidualBasedFluidStrategy(
                ...
                press_toll,
                max_vel_its,
                self.extrapolation_max_press_its,
                time_order,
                self.domain_size,
                laplacian_form,
                predictor_corrector)
    ...
    def Convect(self):
        ...
        if(self.reform_convection_matrix):
            ...
            self.level_set_tools.GenerateModelPart(
                self.base_model_part,
                self.extrapolation_model_part,
                DISTANCE,
                IS_VISITED,
...
# self.level_set_tools.ExtrapolateVelocitiesByLayer(self.base_model_part,DISTANCE,CONVECTION_VELOCITY,VELOCITY,self.number_of_extrapolation_layers);
            ...
            self.level_set_tools.ImplicitExtrapolation_PostProcess(
                self.base_model_part,
                DISTANCE,
                CONVECTION_VELOCITY,
                VELOCITY,
