proc InitGIDProject {dir} {
	GiDMenu::Create "Edgebased levelset" PRE
	GiDMenu::InsertOption "Edgebased levelset" [list "Nodal Values"] 0 PRE "GidOpenConditions \"Nodal Values\"" "" ""
	GiDMenu::InsertOption "Edgebased levelset" [list "Elements"] 1 PRE "GidOpenConditions \"Elements\"" "" ""
	GiDMenu::InsertOption "Edgebased levelset" [list "Conditions"] 2 PRE "GidOpenConditions \"Conditions\"" "" ""
	GiDMenu::InsertOption "Edgebased levelset" [list "Conditional Data"] 3 PRE "GidOpenConditions \"Conditional Data\"" "" ""
	GiDMenu::InsertOption "Edgebased levelset" [list "Model Parts"] 4 PRE "GidOpenConditions \"Model Parts\"" "" ""
	GiDMenu::InsertOption "Edgebased levelset" [list "Default Elements"] 5 PRE "GidOpenProblemData \"Default Elements\"" "" ""
	GiDMenu::InsertOption "Edgebased levelset" [list "Problem Parameters"] 6 PRE "GidOpenProblemData \"Problem Parameters\"" "" ""
	GiDMenu::InsertOption "Edgebased levelset" [list "---"] 7 PRE "" "" ""
	GiDMenu::InsertOption "Edgebased levelset" [list "Model Status"] 8 PRE "cond_report" "" ""
	GiDMenu::UpdateMenus
	# Custom Menu
}
...
# Look for Elements with custom ElemTypes
	...
	cleanautomatic FixedPressure point line surface
	cleanautomatic SlipCondition point line surface
	cleanautomatic Boundary point line surface
	cleanautomatic FixedVelocity point line surface
	...
	cleanautomatic IS_STRUCTURE line surface
	cleanautomatic PRESSURE line surface point
	...
	if {[GiD_Info Geometry NumVolumes]>0} {
		...
		GiD_AssignData Condition surface_Boundary surfaces "1 BEGIN2D2D_Boundary_Condition Use_Default END2D2D_Boundary_Condition BEGIN3D3D_Boundary_Condition Use_Default END3D3D_Boundary_Condition" $boundary
		# 3D Boundary Section
	}
	...
	# Generate lists and assign conditions
	set surfaceFixedVelocitylist [createlist surface FixedVelocity]
	set surfaceSlipConditionlist [createlist surface SlipCondition]
	set surfaceFixedPressurelist [createlist surface FixedPressure]
	...
	foreach surface $surfacelist {
		if {
			[lsearch ${surfaceFixedVelocitylist} $surface] == -1 && \
			...
			set FixedVelocitypos [lsearch ${surfaceFixedVelocitylist} $surface]
			lreplace ${surfaceFixedVelocitylist} ${FixedVelocitypos} ${FixedVelocitypos}
			condfrompart FixedVelocity VELOCITY always surface $surface
		} elseif {
			[lsearch ${surfaceFixedVelocitylist} $surface] == -1 && \
			[lsearch ${surfaceSlipConditionlist} $surface] != -1
		} then {
			...
			if {[GiD_Info Geometry NumVolumes]>0} {
				condfrompart SlipCondition IS_STRUCTURE only3D surface $surface
			}
		} elseif {
			[lsearch ${surfaceFixedVelocitylist} $surface] != -1 && \
			[lsearch ${surfaceSlipConditionlist} $surface] != -1
	...
	foreach surface $surfacelist {
		if {
			[lsearch ${surfaceFixedVelocitylist} $surface] == -1 && \
			[lsearch ${surfaceFixedPressurelist} $surface] == -1
		} then {
		} elseif {
			[lsearch ${surfaceFixedVelocitylist} $surface] != -1 && \
			[lsearch ${surfaceFixedPressurelist} $surface] == -1
			...
			set FixedPressurepos [lsearch ${surfaceFixedPressurelist} $surface]
			lreplace ${surfaceFixedPressurelist} ${FixedPressurepos} ${FixedPressurepos}
			condfrompart FixedPressure PRESSURE always surface $surface
		} elseif {
			[lsearch ${surfaceFixedVelocitylist} $surface] != -1 && \
			[lsearch ${surfaceFixedPressurelist} $surface] != -1
		} then {
	...
	foreach surface $surfacelist {
		if {
			...
			set SlipConditionpos [lsearch ${surfaceSlipConditionlist} $surface]
			lreplace ${surfaceSlipConditionlist} ${SlipConditionpos} ${SlipConditionpos}
			GiD_AssignData Condition surface_PRESSURE surfaces "1 1 0.0" $surface
		} else {
			puts "Unexpected combination of Model Parts in surface $surface"
		}
	}
	...
	if {[GiD_Info Geometry NumVolumes]==0} {
		...
		GiD_AssignData Condition line_Boundary lines "1 BEGIN2D2D_Boundary_Condition Use_Default END2D2D_Boundary_Condition BEGIN3D3D_Boundary_Condition Use_Default END3D3D_Boundary_Condition" $boundary
		# 2D Boundary Section
	}

	cond_surfacetoline FixedVelocity
	cond_surfacetoline SlipCondition
	cond_surfacetoline FixedPressure
	...
	# Generate lists and assign conditions
	set lineFixedVelocitylist [createlist line FixedVelocity]
	set lineSlipConditionlist [createlist line SlipCondition]
	set lineFixedPressurelist [createlist line FixedPressure]
	set lineBoundarylist [createlist line Boundary]
	foreach line $linelist {
		if {
			[lsearch ${lineFixedVelocitylist} $line] == -1 && \
			...
			set FixedVelocitypos [lsearch ${lineFixedVelocitylist} $line]
			lreplace ${lineFixedVelocitylist} ${FixedVelocitypos} ${FixedVelocitypos}
			condfrompart FixedVelocity VELOCITY always line $line
		} elseif {
			[lsearch ${lineFixedVelocitylist} $line] == -1 && \
			[lsearch ${lineSlipConditionlist} $line] != -1
		} then {
			...
			if {[GiD_Info Geometry NumVolumes]==0} {
				condfrompart SlipCondition IS_STRUCTURE only2D line $line
			}
		} elseif {
			[lsearch ${lineFixedVelocitylist} $line] != -1 && \
			[lsearch ${lineSlipConditionlist} $line] != -1
	...
	foreach line $linelist {
		if {
			[lsearch ${lineFixedVelocitylist} $line] == -1 && \
			[lsearch ${lineFixedPressurelist} $line] == -1
		} then {
		} elseif {
			[lsearch ${lineFixedVelocitylist} $line] != -1 && \
			[lsearch ${lineFixedPressurelist} $line] == -1
			...
			set FixedPressurepos [lsearch ${lineFixedPressurelist} $line]
			lreplace ${lineFixedPressurelist} ${FixedPressurepos} ${FixedPressurepos}
			condfrompart FixedPressure PRESSURE always line $line
		} elseif {
			[lsearch ${lineFixedVelocitylist} $line] != -1 && \
			[lsearch ${lineFixedPressurelist} $line] != -1
		} then {
	...
	foreach line $linelist {
		if {
			...
			set SlipConditionpos [lsearch ${lineSlipConditionlist} $line]
			lreplace ${lineSlipConditionlist} ${SlipConditionpos} ${SlipConditionpos}
			GiD_AssignData Condition line_PRESSURE lines "1 1 0.0" $line
		} else {
			puts "Unexpected combination of Model Parts in line $line"
		}
	}
	...
	cond_linetopoint FixedVelocity
	cond_linetopoint SlipCondition
	cond_linetopoint FixedPressure
	...
	# Generate lists and assign conditions
	set pointFixedVelocitylist [createlist point FixedVelocity]
	set pointSlipConditionlist [createlist point SlipCondition]
	set pointFixedPressurelist [createlist point FixedPressure]
	set pointBoundarylist [createlist point Boundary]
	foreach point $pointlist {
		if {
			[lsearch ${pointFixedVelocitylist} $point] == -1 && \
			...
			set FixedVelocitypos [lsearch ${pointFixedVelocitylist} $point]
			lreplace ${pointFixedVelocitylist} ${FixedVelocitypos} ${FixedVelocitypos}
			condfrompart FixedVelocity VELOCITY always point $point
		} elseif {
			[lsearch ${pointFixedVelocitylist} $point] == -1 && \
			[lsearch ${pointSlipConditionlist} $point] != -1
		} then {
	...
	foreach point $pointlist {
		if {
			[lsearch ${pointFixedVelocitylist} $point] == -1 && \
			[lsearch ${pointFixedPressurelist} $point] == -1
		} then {
		} elseif {
			[lsearch ${pointFixedVelocitylist} $point] != -1 && \
			[lsearch ${pointFixedPressurelist} $point] == -1
			...
			set FixedPressurepos [lsearch ${pointFixedPressurelist} $point]
			lreplace ${pointFixedPressurelist} ${FixedPressurepos} ${FixedPressurepos}
			condfrompart FixedPressure PRESSURE always point $point
		} elseif {
			[lsearch ${pointFixedVelocitylist} $point] != -1 && \
			[lsearch ${pointFixedPressurelist} $point] != -1
		} then {
	...
	foreach point $pointlist {
		if {
			...
			set SlipConditionpos [lsearch ${pointSlipConditionlist} $point]
			lreplace ${pointSlipConditionlist} ${SlipConditionpos} ${SlipConditionpos}
			GiD_AssignData Condition point_PRESSURE points "1 1 0.0" $point
		} else {
			puts "Unexpected combination of Model Parts in point $point"
		}
	}
	...
	assign_element_choice 2D_Boundary_Condition line Condition2D
	assign_element_choice 3D_Boundary_Condition surface Condition3D
	assign_element_choice 2D_Body_Element surface Fluid2D
	assign_element_choice 3D_Body_Element volume Fluid3D
