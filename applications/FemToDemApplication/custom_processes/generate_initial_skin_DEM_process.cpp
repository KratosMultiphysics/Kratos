    ...
    for (int i = 0; i < static_cast<int>(r_submodel_part.Nodes().size()); i++) {
        ...
        if ((!it_node->GetValue(IS_DEM) && body.HasNode(it_node->Id()))
            ...
            if (mrDEMModelPart.Elements().size() == 0)
                this->CreateDEMParticle(id + max_id_FEM_nodes, r_coordinates, p_DEM_properties, 0.0015, it_node);
            else
                this->CreateDEMParticle(id, r_coordinates, p_DEM_properties, 0.0015, it_node);
            num_DEM++;
        }
    }
}
