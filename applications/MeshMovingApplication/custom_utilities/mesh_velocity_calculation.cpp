                                             ...
                                             const double Gamma)
{
    ...
    using Node = typename Kratos::Communicator::MeshType::NodeType;     // <-- TODO: replace with auto when compiling with C++14

    block_for_each( rModelPart.GetCommunicator().LocalMesh().Nodes(),
        [&]( Node& node )
        {
            const auto& r_mesh_u0 = node.FastGetSolutionStepValue(MESH_DISPLACEMENT);
            auto&       r_mesh_v0 = node.FastGetSolutionStepValue(MESH_VELOCITY);
            auto&       r_mesh_a0 = node.FastGetSolutionStepValue(MESH_ACCELERATION);

            const auto& r_mesh_u1 = node.FastGetSolutionStepValue(MESH_DISPLACEMENT, 1);
            const auto& r_mesh_v1 = node.FastGetSolutionStepValue(MESH_VELOCITY,     1);
            const auto& r_mesh_a1 = node.FastGetSolutionStepValue(MESH_ACCELERATION, 1);
                             ...
                             const TimeDiscretization::BDF1& rBDF)
{
    ...
    block_for_each( rModelPart.GetCommunicator().LocalMesh().Nodes(),
        ...
        {
            auto& r_mesh_v0       = node.FastGetSolutionStepValue(MESH_VELOCITY);
            noalias(r_mesh_v0) = coeffs[0] * node.FastGetSolutionStepValue(MESH_DISPLACEMENT);
            noalias(r_mesh_v0) += coeffs[1] * node.FastGetSolutionStepValue(MESH_DISPLACEMENT, 1);
        }
    );
                             ...
                             const TimeDiscretization::BDF2& rBDF)
{
    ...
    block_for_each( rModelPart.GetCommunicator().LocalMesh().Nodes(),
        [&]( Node& node )
        {
            ...
            noalias(r_mesh_v0) = coeffs[0] * node.FastGetSolutionStepValue(MESH_DISPLACEMENT);
            noalias(r_mesh_v0) += coeffs[1] * node.FastGetSolutionStepValue(MESH_DISPLACEMENT, 1);
            noalias(r_mesh_v0) += coeffs[2] * node.FastGetSolutionStepValue(MESH_DISPLACEMENT, 2);
        }
    );
