...
public:
  ...
  void BuildAndSolveSystem(ModelPart &model_part,
    ...
    for (ModelPart::ElementsContainerType::iterator i =
      ...
      disps[7] = ccc[1];
      disps[8] = ccc[2];
      const array_1d<double, 3> &aaa =
          geom[3].FastGetSolutionStepValue(DISPLACEMENT);
      disps[9] = aaa[0];
      disps[10] = aaa[1];
      disps[11] = aaa[2];
    ...
    //                 for(ModelPart::NodesContainerType::iterator i =
    //                 model_part.NodesBegin();
...
private:
  ...
  void triangle_area(double xa, double xb, double xc, double ya, double yb,
                     double yc, double za, double zb, double zc, double &area) {

    area = .5 *
           sqrt(pow((xa * yc - xa * yb + xb * ya - xb * yc + xc * yb - xc * ya),
                    2) +
                pow((ya * zb + yb * zc + yc * za - yc * zb - ya * zc - yb * za),
                    2) +
                pow((za * xb + zb * xc + zc * xa - zc * xb - za * xc - zb * xa),
                    2));
  }
};
