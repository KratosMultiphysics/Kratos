...
#G
n_nodes = 110000 + spheres_model_part.NumberOfNodes(0)
zeros   = [0. for i in range(n_nodes)]
entered = [0  for i in range(n_nodes)]
escaped = [0  for i in range(n_nodes)]
ids = []
x0_values = zeros[:]
theta0_values = zeros[:]
t_values  = zeros[:]
...
class Particle:
    ...
    def __init__(self, xv_0, diameter, print_trajectory = False, trajectory_dt = 0.01):
        self.xv_0 = xv_0
        self.diameter = diameter
        Particle.number_of_particles += 1
        self.particle_id = Particle.number_of_particles
...
rot_vel = 10.1936799184505607
n_vanes = 3
cone_angle = math.pi / 12.
cos_cone_angle_inv = 1. / math.cos(cone_angle)
inter_vane_angle = 2. * math.pi / n_vanes
        ...
        for node in spheres_model_part.Nodes:
            ...
            if node.IsNot(BLOCKED):
                ...
                if entered[i] == 0:
                    ...
                    x0_values[i] = r * cos_cone_angle_inv
                    theta0_values[i] = theta
                    D_values[i]  = 2. * node.GetSolutionStepValue(RADIUS, 0) 
                    
                r2 = GetPolarRCoordinate2(node)            

                if r2 > 1 and escaped[i] == 0:
                    ids.append(i)
                    escaped[i] = 1            
                    t_values[i]  = time
                    x_values[i]  = node.X
                    y_values[i]  = node.Y
                    vz_values[i] = node.Z
                    vx_values[i] = node.GetSolutionStepValue(VELOCITY_X)
                    vy_values[i] = node.GetSolutionStepValue(VELOCITY_Y)
                    vz_values[i] = node.GetSolutionStepValue(VELOCITY_Z)
                    wx_values[i] = node.GetSolutionStepValue(ANGULAR_VELOCITY_X)
                    wy_values[i] = node.GetSolutionStepValue(ANGULAR_VELOCITY_Y)
                    wz_values[i] = node.GetSolutionStepValue(ANGULAR_VELOCITY_Z)            
#Z
...
for node_id in ids:
    out_file.write(str( t_values[node_id]) + ' ' + 
                   str( x_values[node_id]) + ' ' +  
                   str( y_values[node_id]) + ' ' +
                   str( z_values[node_id]) + ' ' +
                   str(vx_values[node_id]) + ' ' +
                   str(vy_values[node_id]) + ' ' +
                   str(vz_values[node_id]) + ' ' +
                   str(wx_values[node_id]) + ' ' +
                   str(wy_values[node_id]) + ' ' +
                   str(wz_values[node_id]) + ' ' +
                   str( D_values[node_id]) + '\n')
out_file.close()

inputs_file = open('in_data.txt', 'w')

for node_id in ids:
    inputs_file.write(str( x0_values[node_id]) + ' ' + 
                      str( theta0_values[node_id]) + ' ' + 
                      str( D_values[node_id]) + '\n')
