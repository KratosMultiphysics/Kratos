...
namespace Kratos {
    ...
    void DEMIntegrationScheme::RotateClusterNode(Node<3> & i, const double delta_t, const double moment_reduction_factor, const int StepFlag) {
        ...
        if (StepFlag == 1 || StepFlag == 2) {
            array_1d<double, 3 > local_angular_acceleration, aux_torque, local_torque, quarter_local_angular_velocity, quarter_angular_velocity, AuxAngularVelocity;
            ...
            for (int j = 0; j < 3; j++) {
                if (Fix_Ang_vel[j] == true) aux_torque[j] = 0.0;
                else aux_torque[j] = torque[j];
            }

            if (StepFlag == 1) { //PREDICT
                //Angular velocity and torques are saved in the local framework:
                GeometryFunctions::QuaternionVectorGlobal2Local(Orientation, aux_torque, local_torque);
                ...
                Quaternion<double  > Temp = Orientation;
                Orientation = AuxOrientation;
                AuxOrientation = Temp;

                GeometryFunctions::QuaternionVectorLocal2Global(Orientation, LocalAuxAngularVelocity, angular_velocity);
            }//if StepFlag == 1
            ...
            if (StepFlag == 2) { //CORRECT
                ...
                UpdateRotationalVariables(i, rotated_angle, delta_rotation, angular_velocity, delta_t, Fix_Ang_vel);
                GeometryFunctions::UpdateOrientation(AuxOrientation, Orientation, delta_rotation);
