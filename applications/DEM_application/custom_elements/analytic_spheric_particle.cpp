...
void AnalyticSphericParticle::ClearImpactMemberVariables()
{
    ...
    for (unsigned int i = 0; i < MaxCollidingSpheres; ++i){
        mCollidingIds[i] = 0;
                                                       ...
                                                       array_1d<double, 3>& neighbour_elastic_contact_force)
{
    ...
    if (IsNewNeighbour(id) && mNumberOfCollidingSpheres < MaxCollidingSpheres){
...

{
    ...
    for (unsigned int i = 0; i < rNeighbours.size(); i++) {
        ...
        int p_wall_id;
        p_wall_id = p_wall->Id();    
        if (IsNewFaceNeighbour(p_wall_id) && mNumberOfCollidingSpheresWithFaces < MaxCollidingFaceSpheres){
