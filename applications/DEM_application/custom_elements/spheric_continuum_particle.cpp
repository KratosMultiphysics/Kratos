...
namespace Kratos
{
     // using namespace GeometryFunctions;
      ...
      SphericContinuumParticle::SphericContinuumParticle() : DiscreteElement(){}

      SphericContinuumParticle::SphericContinuumParticle( IndexType NewId, GeometryType::Pointer pGeometry) : DiscreteElement(NewId, pGeometry){}
      ...
      {
         return DiscreteElement::Pointer(new SphericContinuumParticle(NewId, GetGeometry().Create( ThisNodes ), pProperties) );
      }
    ...
    //**************************************************************************************************************************************************
      ...
      void SphericContinuumParticle::CalculateRightHandSide(VectorType& rRightHandSideVector,ProcessInfo& rCurrentProcessInfo){

        ComputeParticleContactForce(rCurrentProcessInfo);

        if( (rCurrentProcessInfo[LIMIT_SURFACE_OPTION] == 1) )
        {
            ComputeParticleSurfaceContactForce(rCurrentProcessInfo);
        }
    ...
    //**************************************************************************************************************************************************
...

      {
        ...
        void SphericContinuumParticle::SetInitialContacts(int case_opt, ProcessInfo& rCurrentProcessInfo  ) //vull ficar que sigui zero si no son veins cohesius.
        {
            /*
             *
      ...
      void SphericContinuumParticle::ComputeParticleContactForce(ProcessInfo& rCurrentProcessInfo )
      {
          KRATOS_TRY
          ...
          double momentofinertia = GetGeometry()(0)->FastGetSolutionStepValue(PARTICLE_MOMENT_OF_INERTIA);
          array_1d<double, 3>& mMax_Rota_Moment     = this->GetGeometry()(0)->FastGetSolutionStepValue(MAX_ROTA_MOMENT);
          ...
          if ( rotation_OPTION == 1 )
          {
              ...
              mInitial_Rota_Moment[0] = RotaAcc[0] * momentofinertia;
              mInitial_Rota_Moment[1] = RotaAcc[1] * momentofinertia;
              mInitial_Rota_Moment[2] = RotaAcc[2] * momentofinertia;

              mMax_Rota_Moment[0] = mInitial_Rota_Moment[0];
              mMax_Rota_Moment[1] = mInitial_Rota_Moment[1];
              mMax_Rota_Moment[2] = mInitial_Rota_Moment[2];
            }
...
//                                 }
          ...
          // VISCODAMPING (applyied locally)
              ...
              if ( rotation_OPTION == 1 )
              {
                    ...
                    Max_Rota_Moment[0] = mMax_Rota_Moment[0];
                    Max_Rota_Moment[1] = mMax_Rota_Moment[1];
                    Max_Rota_Moment[2] = mMax_Rota_Moment[2];
                    ...
                    if(rCurrentProcessInfo[ROTA_DAMP_TYPE]==2)  //Rolling friccion type
                    {
                       ...
                       if (equiv_RollingFrictionCoeff != 0.0)
                       {
                          ...
                          {
                              Rota_Moment[0] = -mInitial_Rota_Moment[0];
                              Rota_Moment[1] = -mInitial_Rota_Moment[1];
                              Rota_Moment[2] = -mInitial_Rota_Moment[2];
                          }
                       } // if (equiv_RollingFrictionCoeff != 0.0)
                    ...
                    mMax_Rota_Moment[0] = Max_Rota_Moment[0];
                    mMax_Rota_Moment[1] = Max_Rota_Moment[1];
                    mMax_Rota_Moment[2] = Max_Rota_Moment[2];
      ...
      void SphericContinuumParticle::ComputeParticleRotationSpring(const ProcessInfo& rCurrentProcessInfo) // shan de corregir areees etc etc etc...
      {
