   ...
   struct matrix_prod_row_row_row<double, 1>
   {
    ...
    static void init()
    {
      ...
      if (!init_done[context_.handle()])
      {
        ...
        source.append(viennacl::tools::make_double_kernel(matrix_prod_row_row_row_align1_prod_TA, pf_info));
        source.append(viennacl::tools::make_double_kernel(matrix_prod_row_row_row_align1_prod_TT, pf_info));
        source.append(viennacl::tools::make_double_kernel(matrix_prod_row_row_row_align1_prod_AT, pf_info));
        source.append(viennacl::tools::make_double_kernel(matrix_prod_row_row_row_align1_prod_AA, pf_info));
        std::string prog_name = program_name();
        #ifdef VIENNACL_BUILD_INFO
        std::cout << "Creating program " << prog_name << std::endl;
        #endif
