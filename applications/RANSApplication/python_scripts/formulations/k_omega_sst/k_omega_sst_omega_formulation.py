...
class KOmegaSSTOmegaFormulation(Formulation):
    def __init__(self, model_part, settings):
        super(KOmegaSSTOmegaFormulation,
    ...
    def SetStabilizationMethod(self, stabilization_method):
        if (stabilization_method == "algebraic_flux_corrected"):
            self.element_name = "RansEvmKOmegaSSTOmegaAFC"
    ...
    def SetWallFunctionSettings(self, settings):
        ...
        if (wall_function_region_type == "logarithmic_region_only"):
            if (wall_friction_velocity_calculation_method == "velocity_based"):
                self.condition_name = "RansEvmKOmegaOmegaUBasedWall"
            elif (wall_friction_velocity_calculation_method ==
                  "turbulent_kinetic_energy_based"):
                self.condition_name = "RansEvmKOmegaOmegaKBasedWall"
            else:
            ...
            # make sure to fix omega and k at walls with respective values
            self.condition_name = GetDefaultConditionName(self.GetBaseModelPart())
        else:
