...
from KratosMultiphysics.HDF5Application.core import CreateController
from KratosMultiphysics.HDF5Application.core.operations import AggregateOperation
...
def Factory(parameters, model):
    ...
    for variable_name in model_part.GetHistoricalVariablesNames():
        list_of_solution_step_variables["list_of_variables"].Append(variable_name)
    ...
    list_of_element_variables = ParametersWrapper("""{
        "list_of_variables" : ["ELEMENT_H", "ELEMENT_ERROR"]
    }""")
    ...
    for i in core_settings:
        operation = AggregateOperation(model, core_settings[i].Get())
        controller = CreateController(core_settings[i].Get(), model, operation)
