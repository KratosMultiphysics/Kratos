#include "rans_fractional_step.h"
...
{
/*
 * public RansFractionalStep<TDim> functions
 */

template <unsigned int TDim>
void RansFractionalStep<TDim>::Initialize()
{
}

template <unsigned int TDim>
void RansFractionalStep<TDim>::InitializeSolutionStep(ProcessInfo& rCurrentProcessInfo)
{
}

template <unsigned int TDim>
void RansFractionalStep<TDim>::InitializeNonLinearIteration(ProcessInfo& rCurrentProcessInfo)
{
}

template <unsigned int TDim>
void RansFractionalStep<TDim>::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix,
                                                    ...
                                                    ProcessInfo& rCurrentProcessInfo)
{
    ...
    const ProcessInfo& r_process_info =
        rCurrentProcessInfo; // Using a const reference for thread safety

    switch (r_process_info[FRACTIONAL_STEP])
    {
    case 1:
    {
        this->CalculateLocalFractionalVelocitySystem(
            rLeftHandSideMatrix, rRightHandSideVector, r_process_info);
        break;
    }
    case 5:
    {
        this->CalculateLocalPressureSystem(
            rLeftHandSideMatrix, rRightHandSideVector, r_process_info);
        break;
    }
    case 6:
    ...
    default:
    {
        KRATOS_THROW_ERROR(std::logic_error,
                           "Unexpected value for FRACTIONAL_STEP index: ",
                           r_process_info[FRACTIONAL_STEP]);
    }
    }
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::Calculate(const Variable<double>& rVariable,
                                         double& rOutput,
                                         const ProcessInfo& rCurrentProcessInfo)
{
    if (rVariable == DIVPROJ)
    {
        ...
        for (SizeType i = 0; i < NumNodes; ++i)
        {
            ...
            array_1d<double, 3>& rMomValue =
                this->GetGeometry()[i].FastGetSolutionStepValue(ADVPROJ);
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::Calculate(const Variable<array_1d<double, 3>>& rVariable,
                                         ...
                                         const ProcessInfo& rCurrentProcessInfo)
{
    ...
    else if (rVariable == CONV_PROJ)
    {
        ...
        {
            this->GetGeometry()[i].SetLock(); // So it is safe to write in the node in OpenMP
            array_1d<double, 3>& rConvVal =
                this->GetGeometry()[i].FastGetSolutionStepValue(CONV_PROJ);
            array_1d<double, 3>& rPresVal =
                this->GetGeometry()[i].FastGetSolutionStepValue(PRESS_PROJ);
            for (unsigned int d = 0; d < TDim; ++d)
            {
    ...
    else if (rVariable == VELOCITY)
    {
        ...
        for (unsigned int g = 0; g < NumGauss; ++g)
        {
            ...
            const double Coeff =
                GaussWeights[g] / (Density * rCurrentProcessInfo[BDF_COEFFICIENTS][0]);
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::EquationIdVector(EquationIdVectorType& rResult,
                                                ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::GetDofList(DofsVectorType& rElementalDofList,
                                          ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
...
template <unsigned int TDim>
GeometryData::IntegrationMethod RansFractionalStep<TDim>::GetIntegrationMethod() const
{
    return GeometryData::GI_GAUSS_2;
}

template <unsigned int TDim>
void RansFractionalStep<TDim>::GetValueOnIntegrationPoints(
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    if (rVariable == CONV_PROJ)
    {
        ...
        for (unsigned int g = 0; g < NumGauss; g++)
        {
            ...
            for (SizeType i = 0; i < NumNodes; ++i)
                for (SizeType d = 0; d < TDim; ++d)
                    rOut[d] += ConvOp[i] * this->GetGeometry()[i].FastGetSolutionStepValue(
                                               VELOCITY)[d];
    ...
    else if (rVariable == PRESS_PROJ)
    {
        ...
        for (unsigned int g = 0; g < NumGauss; g++)
        {
            ...
            for (SizeType i = 0; i < NumNodes; ++i)
                for (SizeType d = 0; d < TDim; ++d)
                    rOut[d] -= rDN_DX(i, d) *
                               this->GetGeometry()[i].FastGetSolutionStepValue(PRESSURE);
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::GetValueOnIntegrationPoints(const Variable<double>& rVariable,
                                                           ...
                                                           const ProcessInfo& rCurrentProcessInfo)
{
    ...
    else if (rVariable == MU)
    {
        ...
        for (unsigned int g = 0; g < NumGauss; g++)
        {
            ...
            rValues[g] = this->EffectiveViscosity(Density, N, rDN_DX, ElemSize,
                                                  rCurrentProcessInfo);
        }
    }
    else if (rVariable == TAU)
    {
        ...
        for (unsigned int g = 0; g < NumGauss; g++)
        {
            ...
            double mu = this->EffectiveViscosity(Density, N, rDN_DX, ElemSize,
                                                 rCurrentProcessInfo);
    ...
    else if (rVariable == VORTICITY_MAGNITUDE)
    {
        ...
        VorticityUtilities<TDim>::CalculateVorticityMagnitude(
            this->GetGeometry(), DN_DX, rValues);
    }
    else
    {
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::CalculateLocalFractionalVelocitySystem(
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::CalculateLocalPressureSystem(MatrixType& rLeftHandSideMatrix,
                                                            VectorType& rRightHandSideVector,
                                                            const ProcessInfo& rCurrentProcessInfo)
{
    GeometryType& rGeom = this->GetGeometry();
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::CalculateEndOfStepSystem(MatrixType& rLeftHandSideMatrix,
                                                        VectorType& rRightHandSideVector,
                                                        const ProcessInfo& rCurrentProcessInfo)
{
    const GeometryType& rGeom = this->GetGeometry();
...
template <unsigned int TDim>
int RansFractionalStep<TDim>::Check(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY;
    ...
    if (this->GetGeometry().WorkingSpaceDimension() == 2)
    {
        for (unsigned int i = 0; i < this->GetGeometry().size(); ++i)
        {
            if (this->GetGeometry()[i].Z() != 0.0)
                KRATOS_ERROR << "Node " << this->GetGeometry()[i].Id()
                             << "has non-zero Z coordinate." << std::endl;
        }
    }
...
template <>
void RansFractionalStep<2>::VelocityEquationIdVector(EquationIdVectorType& rResult,
                                                     ProcessInfo& rCurrentProcessInfo)
{
    GeometryType& rGeom = this->GetGeometry();
    const SizeType NumNodes = rGeom.PointsNumber();
...
template <>
void RansFractionalStep<3>::VelocityEquationIdVector(EquationIdVectorType& rResult,
                                                     ProcessInfo& rCurrentProcessInfo)
{
    GeometryType& rGeom = this->GetGeometry();
    const SizeType NumNodes = rGeom.PointsNumber();
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::PressureEquationIdVector(EquationIdVectorType& rResult,
                                                        ProcessInfo& rCurrentProcessInfo)
{
    GeometryType& rGeom = this->GetGeometry();
    const SizeType NumNodes = rGeom.PointsNumber();
...
template <>
void RansFractionalStep<2>::GetVelocityDofList(DofsVectorType& rElementalDofList,
                                               ProcessInfo& rCurrentProcessInfo)
{
    GeometryType& rGeom = this->GetGeometry();
    const SizeType NumNodes = rGeom.PointsNumber();
...
template <>
void RansFractionalStep<3>::GetVelocityDofList(DofsVectorType& rElementalDofList,
                                               ProcessInfo& rCurrentProcessInfo)
{
    GeometryType& rGeom = this->GetGeometry();
    const SizeType NumNodes = rGeom.PointsNumber();
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::GetPressureDofList(DofsVectorType& rElementalDofList,
                                                  ProcessInfo& rCurrentProcessInfo)
{
    GeometryType& rGeom = this->GetGeometry();
    const SizeType NumNodes = rGeom.PointsNumber();
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::GetPressureValues(Vector& rValues, const int Step)
{
    GeometryType& rGeom = this->GetGeometry();
    const SizeType NumNodes = rGeom.PointsNumber();
...
template <>
void RansFractionalStep<2>::GetVelocityValues(Vector& rValues, const int Step)
{
    GeometryType& rGeom = this->GetGeometry();
    const SizeType NumNodes = rGeom.PointsNumber();
    const SizeType LocalSize = 2 * NumNodes;
...
template <>
void RansFractionalStep<3>::GetVelocityValues(Vector& rValues, const int Step)
{
    GeometryType& rGeom = this->GetGeometry();
    const SizeType NumNodes = rGeom.PointsNumber();
    const SizeType LocalSize = 3 * NumNodes;
...
/*
 * protected RansFractionalStep<TDim> functions
 */

template <unsigned int TDim>
void RansFractionalStep<TDim>::CalculateGeometryData(ShapeFunctionDerivativesArrayType& rDN_DX,
                                                     Matrix& NContainer,
                                                     Vector& rGaussWeights)
{
    ...
    for (unsigned int g = 0;
         g < rGeom.IntegrationPointsNumber(GeometryData::GI_GAUSS_2); g++)
...
template <unsigned int TDim>
double RansFractionalStep<TDim>::ElementSize(/*ShapeFunctionDerivativesType &rDN_DX*/)
{
    const GeometryType& rGeom = this->GetGeometry();
    const SizeType NumNodes = rGeom.PointsNumber();
    ...
    //    // calculate minimum element height (for stabilization and
    //    Smagorinsky) mElemSize = 0.0; for ( unsigned int d = 0; d < TDim; ++d)
    //    {
...
template <unsigned int TDim>
double RansFractionalStep<TDim>::EffectiveViscosity(double Density,
                                                    const ShapeFunctionsType& rN,
                                                    const ShapeFunctionDerivativesType& rDN_DX,
                                                    double ElemSize,
                                                    const ProcessInfo& rProcessInfo)
{
    const RansFractionalStep<TDim>* const_this =
        static_cast<const RansFractionalStep<TDim>*>(this);
...
template <unsigned int TDim>
double RansFractionalStep<TDim>::EquivalentStrainRate(const ShapeFunctionDerivativesType& rDN_DX) const
{
    const GeometryType& rGeom = this->GetGeometry();
    const unsigned int NumNodes = rGeom.PointsNumber();
    ...
    {
        const array_1d<double, 3>& rVel = rGeom[n].FastGetSolutionStepValue(
            VELOCITY, 1); // OLD VELOCITY (which is incompressible, unlike the fractional step one)
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::AddMomentumMassTerm(Matrix& rMassMatrix,
                                                   const ShapeFunctionsType& rN,
                                                   const double Weight)
{
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::AddMomentumSystemTerms(Matrix& rLHSMatrix,
                                                      Vector& rRHSVector,
                                                      ...
                                                      const double Weight)
{
    ...
    for (SizeType i = 0; i < NumNodes; ++i)
    {
        ...
        for (SizeType d = 0; d < TDim; ++d)
        {
            ...
            // Momentum Stabilization
            RHSi += Density * rConvOperator[i] * TauOne *
                    (/*rBodyForce[d] + rOldPressureGradient[d]*/ -rMomentumProjection[d]);
            // Mass Stabilization
        ...
        for (SizeType j = 0; j < NumNodes; ++j)
        {
            ...
            for (SizeType m = 0; m < TDim; ++m)
                for (SizeType n = 0; n < TDim; ++n)
                    rLHSMatrix(FirstRow + m, FirstCol + n) +=
                        Weight * rDN_DX(i, m) * TauTwo * rDN_DX(j, n);
...
template <>
void RansFractionalStep<2>::AddViscousTerm(MatrixType& rDampingMatrix,
                                           ...
                                           const double Weight)
{
    ...
    for (SizeType j = 0; j < NumNodes; ++j)
    {
        ...
        {
            // First Row
            rDampingMatrix(FirstRow, FirstCol) +=
                Weight * (FourThirds * rShapeDeriv(i, 0) * rShapeDeriv(j, 0) +
                          rShapeDeriv(i, 1) * rShapeDeriv(j, 1));
            rDampingMatrix(FirstRow, FirstCol + 1) +=
                Weight * (nTwoThirds * rShapeDeriv(i, 0) * rShapeDeriv(j, 1) +
                          rShapeDeriv(i, 1) * rShapeDeriv(j, 0));

            // Second Row
            rDampingMatrix(FirstRow + 1, FirstCol) +=
                Weight * (nTwoThirds * rShapeDeriv(i, 1) * rShapeDeriv(j, 0) +
                          rShapeDeriv(i, 0) * rShapeDeriv(j, 1));
            rDampingMatrix(FirstRow + 1, FirstCol + 1) +=
                Weight * (FourThirds * rShapeDeriv(i, 1) * rShapeDeriv(j, 1) +
                          rShapeDeriv(i, 0) * rShapeDeriv(j, 0));
...
template <>
void RansFractionalStep<3>::AddViscousTerm(MatrixType& rDampingMatrix,
                                           const ShapeFunctionDerivativesType& rShapeDeriv,
                                           const double Weight)
{
    const SizeType NumNodes = this->GetGeometry().PointsNumber();
    ...
    for (SizeType j = 0; j < NumNodes; ++j)
    {
        ...
        {
            // (dN_i/dx_k dN_j/dx_k)
            const double Diag = rShapeDeriv(i, 0) * rShapeDeriv(j, 0) +
                                rShapeDeriv(i, 1) * rShapeDeriv(j, 1) +
                                rShapeDeriv(i, 2) * rShapeDeriv(j, 2);

            // First Row
            rDampingMatrix(FirstRow, FirstCol) +=
                Weight * (OneThird * rShapeDeriv(i, 0) * rShapeDeriv(j, 0) + Diag);
            rDampingMatrix(FirstRow, FirstCol + 1) +=
                Weight * (nTwoThirds * rShapeDeriv(i, 0) * rShapeDeriv(j, 1) +
                          rShapeDeriv(i, 1) * rShapeDeriv(j, 0));
            rDampingMatrix(FirstRow, FirstCol + 2) +=
                Weight * (nTwoThirds * rShapeDeriv(i, 0) * rShapeDeriv(j, 2) +
                          rShapeDeriv(i, 2) * rShapeDeriv(j, 0));
            ...
            rDampingMatrix(FirstRow + 1, FirstCol) +=
                Weight * (nTwoThirds * rShapeDeriv(i, 1) * rShapeDeriv(j, 0) +
                          rShapeDeriv(i, 0) * rShapeDeriv(j, 1));
            rDampingMatrix(FirstRow + 1, FirstCol + 1) +=
                Weight * (OneThird * rShapeDeriv(i, 1) * rShapeDeriv(j, 1) + Diag);
            rDampingMatrix(FirstRow + 1, FirstCol + 2) +=
                Weight * (nTwoThirds * rShapeDeriv(i, 1) * rShapeDeriv(j, 2) +
                          rShapeDeriv(i, 2) * rShapeDeriv(j, 1));

            // Third Row
            rDampingMatrix(FirstRow + 2, FirstCol) +=
                Weight * (nTwoThirds * rShapeDeriv(i, 2) * rShapeDeriv(j, 0) +
                          rShapeDeriv(i, 0) * rShapeDeriv(j, 2));
            rDampingMatrix(FirstRow + 2, FirstCol + 1) +=
                Weight * (nTwoThirds * rShapeDeriv(i, 2) * rShapeDeriv(j, 1) +
                          rShapeDeriv(i, 1) * rShapeDeriv(j, 2));
            rDampingMatrix(FirstRow + 2, FirstCol + 2) +=
                Weight * (OneThird * rShapeDeriv(i, 2) * rShapeDeriv(j, 2) + Diag);
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::CalculateTau(double& TauOne,
                                            double& TauTwo,
                                            double ElemSize,
                                            ...
                                            const ProcessInfo& rCurrentProcessInfo)
{
    ...
    TauOne = 1.0 / (Density * (TimeFactor / DeltaTime + 2.0 * AdvVelNorm / ElemSize) +
                    4.0 * Viscosity / (ElemSize * ElemSize));
    TauTwo = Viscosity + 0.5 * Density * ElemSize * AdvVelNorm;
}

template <unsigned int TDim>
void RansFractionalStep<TDim>::CalculateProjectionRHS(VectorType& rMomentumRHS,
                                                      VectorType& rMassRHS,
                                                      const ShapeFunctionsType& rN,
                                                      const ShapeFunctionDerivativesType& rDN_DX,
                                                      const double Weight)
{
    ...
    for (unsigned int j = 0; j < NumNodes; ++j)
    {
        for (unsigned int d = 0; d < TDim; ++d)
        {
            rMomentumRHS[RowIndex++] +=
                Weight * rN[j] *
                (Density * (BodyForce[d] - Convection[d]) - PressureGradient[d]);
        }
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::CalculateProjectionRHS(VectorType& rConvTerm,
                                                      ...
                                                      const double Weight)
{
    ...
    for (unsigned int j = 0; j < NumNodes; ++j)
    {
        ...
        {
            rPresTerm[RowIndex] +=
                Weight * rN[j] * (Density * BodyForce[d] - PressureGradient[d]);
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::ModulatedGradientDiffusion(MatrixType& rDampMatrix,
                                                          const ShapeFunctionDerivativesType& rDN_DX,
                                                          const double Weight)
{
    const GeometryType& rGeom = this->GetGeometry();
    ...
    for (unsigned int n = 0; n < NumNodes; n++)
    {
        const array_1d<double, 3>& rVel =
            this->GetGeometry()[n].FastGetSolutionStepValue(VELOCITY);
        for (unsigned int i = 0; i < TDim; i++)
            for (unsigned int j = 0; j < TDim; j++)
        ...
        for (unsigned int i = 0; i < NumNodes; i++)
        {
            for (unsigned int j = 0; j < NumNodes; j++)
            {
                for (unsigned int d = 0; d < TDim; d++)
                {
                    ...
                    rDampMatrix(RowIndex + d, ColIndex + d) +=
                        Weight * 2.0 * ksgs * rDN_DX(i, d) * Aux;
                }
...
/*
 * private RansFractionalStep<TDim> functions
 */

template <unsigned int TDim>
void RansFractionalStep<TDim>::ConvectionOperator(Vector& rResult,
...
template <unsigned int TDim>
void RansFractionalStep<TDim>::EvaluateConvVelocity(array_1d<double, 3>& rConvVel,
...
// template<>
// RansFractionalStep<2>::ShapeFunctionsContainerType RansFractionalStep<2>::InitializeShapeFunctions()
//{
...
///*
// * RansFractionalStep<TDim> static members
// */

// template< unsigned int TDim >
// static const RansFractionalStep<TDim>::ShapeFunctionsType RansFractionalStep<TDim>::msNg = RansFractionalStep<TDim>::InitializeShapeFunctions();

/*
 * Template class definition (this should allow us to compile the desired template instantiations)
 */

template class RansFractionalStep<2>;
template class RansFractionalStep<3>;
