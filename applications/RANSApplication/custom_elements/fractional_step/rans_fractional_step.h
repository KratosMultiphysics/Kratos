...
#if !defined(KRATOS_RANS_FRACTIONAL_STEP_H_INCLUDED)
#define KRATOS_RANS_FRACTIONAL_STEP_H_INCLUDED
...
class RansFractionalStep : public Element
{
public:
    ///@name Type Definitions
    ///@{

    /// Pointer definition of RansFractionalStep
    KRATOS_CLASS_INTRUSIVE_POINTER_DEFINITION(RansFractionalStep);
     ...
     */
    RansFractionalStep(IndexType NewId = 0) : Element(NewId)
    {
    }
     ...
     */
    RansFractionalStep(IndexType NewId, const NodesArrayType& ThisNodes)
     ...
     */
    RansFractionalStep(IndexType NewId, GeometryType::Pointer pGeometry)
     ...
     */
    RansFractionalStep(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties)
        : Element(NewId, pGeometry, pProperties)
    {
    }

    /// Destructor.
    ~RansFractionalStep() override
    {
    }
    ...
    /**
     * Returns a pointer to a new RansFractionalStep element, created using given input
    ...
    {
        return Kratos::make_intrusive<RansFractionalStep<TDim>>(
    ...
    Element::Pointer Create(IndexType NewId,
                            ...
                            Element::PropertiesType::Pointer pProperties) const override
    {
        return Kratos::make_intrusive<RansFractionalStep<TDim>>(NewId, pGeom, pProperties);
    }
                               ...
                               ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        KRATOS_THROW_ERROR(
            std::logic_error,
            "RansFractionalStep::CalculateLeftHandSide not implemented", "");
        KRATOS_CATCH("");
    }
                                ...
                                ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        KRATOS_THROW_ERROR(
            std::logic_error,
            "RansFractionalStep::CalculateRightHandSide not implemented", "");
        KRATOS_CATCH("");
    }
    ...
    {
        rOStream << "RansFractionalStep" << TDim << "D";
    }
...
protected:
    ...
    void VelocityEquationIdVector(EquationIdVectorType& rResult, ProcessInfo& rCurrentProcessInfo);

    void PressureEquationIdVector(EquationIdVectorType& rResult, ProcessInfo& rCurrentProcessInfo);

    void GetVelocityDofList(DofsVectorType& rElementalDofList, ProcessInfo& rCurrentProcessInfo);

    void GetPressureDofList(DofsVectorType& rElementalDofList, ProcessInfo& rCurrentProcessInfo);
                                    ...
                                    std::vector<TValueType>& rOutput)
    {
        unsigned int NumValues =
            this->GetGeometry().IntegrationPointsNumber(GeometryData::GI_GAUSS_2);
...
private:
    ...
    /// Assignment operator.
    RansFractionalStep& operator=(RansFractionalStep const& rOther);

    /// Copy constructor.
    RansFractionalStep(RansFractionalStep const& rOther);

    ///@}

}; // Class RansFractionalStep
...
template <unsigned int TDim>
inline std::istream& operator>>(std::istream& rIStream, RansFractionalStep<TDim>& rThis)
{
    return rIStream;
}
...
template <unsigned int TDim>
inline std::ostream& operator<<(std::ostream& rOStream, const RansFractionalStep<TDim>& rThis)
{
...
#endif // KRATOS_RANS_FRACTIONAL_STEP_H_INCLUDED  defined
