...
// Project includes
#include "evm_k_epsilon_adjoint_element_data_utilities.h"
                                       ...
                                       const Vector& rShapeFunctions)
{
    if (Theta > 0.0)
    {
        ...
        noalias(rOutput) -=
            rGaussNutTKESensitivities * (Theta / TurbulentKinematicViscosity);
    }
    else
    {
        rOutput.clear();
...
{
    noalias(rOutput) =
        rGaussNutEpsilonSensitivities * (-1.0 * Theta / TurbulentKinematicViscosity);
}
...
template void CalculateGaussThetaTkeSensitivity<3>(BoundedVector<double, 3>&,
                                                   const double,
                                                   const double,
                                                   const double,
                                                   const BoundedVector<double, 3>&,
...
template void CalculateGaussThetaTkeSensitivity<4>(BoundedVector<double, 4>&,
                                                   const double,
                                                   const double,
                                                   const double,
                                                   const BoundedVector<double, 4>&,
...
template void CalculateGaussThetaEpsilonSensitivity<3>(BoundedVector<double, 3>&,
                                                       const double,
                                                       const double,
                                                       const BoundedVector<double, 3>&);

template void CalculateGaussThetaEpsilonSensitivity<4>(BoundedVector<double, 4>&,
                                                       const double,
                                                       const double,
                                                       const BoundedVector<double, 4>&);
