    ...
    mAbsoluteTolerance(AbsoluteTolerance)
{
    ...
    block_for_each(mrModelPart.Elements(),[&](const ElementType& rElement) {
        KRATOS_ERROR_IF_NOT(rElement.Has(CONSTITUTIVE_LAW)) << "CONSTITUTIVE_LAW is not found in element"
            <<" data value container. [ Element.Id() = " << rElement.Id() << " ].\n";

        const auto p_constitutive = rElement.GetValue(CONSTITUTIVE_LAW);
