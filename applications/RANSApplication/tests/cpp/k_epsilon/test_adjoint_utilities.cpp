...
void InitializeProcessInfo(ModelPart& rModelPart)
{
    ...
    rModelPart.GetProcessInfo().SetValue(RANS_STABILIZATION_DISCRETE_UPWIND_OPERATOR_COEFFICIENT, 1.5);
    rModelPart.GetProcessInfo().SetValue(RANS_STABILIZATION_DIAGONAL_POSITIVITY_PRESERVING_COEFFICIENT, 3.0);

    // Set the element properties
    Properties::Pointer p_elem_prop = rModelPart.CreateNewProperties(0);
    p_elem_prop->SetValue(KINEMATIC_VISCOSITY, 3.0e-03);
}
...
void CreateModelPartNodes(ModelPart& rModelPart)
{
    ...
    VariableUtils().AddDof<Variable<double>>(VELOCITY_X, rModelPart);
    VariableUtils().AddDof<Variable<double>>(VELOCITY_Y, rModelPart);
    VariableUtils().AddDof<Variable<double>>(VELOCITY_Z, rModelPart);

    VariableUtils().AddDof<Variable<double>>(ADJOINT_FLUID_SCALAR_1, rModelPart);
    VariableUtils().AddDof<Variable<double>>(ADJOINT_FLUID_VECTOR_1_X, rModelPart);
    VariableUtils().AddDof<Variable<double>>(ADJOINT_FLUID_VECTOR_1_Y, rModelPart);
    VariableUtils().AddDof<Variable<double>>(ADJOINT_FLUID_VECTOR_1_Z, rModelPart);
}
...
void InitializeNodalVariables(ModelPart& rModelPart)
{
    ...
    RandomFillNodalHistoricalVariable(rModelPart, TURBULENT_KINETIC_ENERGY_RATE,
                                      5.0, 10.0, 0);
    RandomFillNodalHistoricalVariable(
        rModelPart, TURBULENT_ENERGY_DISSIPATION_RATE, 10.0, 20.0, 0);
    RandomFillNodalHistoricalVariable(
        rModelPart, TURBULENT_ENERGY_DISSIPATION_RATE_2, 5.0, 10.0, 0);
    ...
    RandomFillNodalHistoricalVariable(rModelPart, AUX_ADJOINT_FLUID_VECTOR_1,
                                      45.0, 100.0, 0);
    ...
    RandomFillNodalHistoricalVariable(rModelPart, TURBULENT_KINETIC_ENERGY_RATE,
                                      5.0, 10.0, 1);
    RandomFillNodalHistoricalVariable(
        rModelPart, TURBULENT_ENERGY_DISSIPATION_RATE, 10.0, 20.0, 1);
    RandomFillNodalHistoricalVariable(
        rModelPart, TURBULENT_ENERGY_DISSIPATION_RATE_2, 5.0, 10.0, 1);
...
void RunRansEvmKEpsilonTest(
    const std::string& PrimalName,
    const std::string& AdjointName,
    const Variable<TDataType>& rPerturbVariable,
    std::function<void(Matrix&, typename TContainer::data_type&, const ProcessInfo&)> CalculateElementResidualScalarSensitivity,
    const double Delta,
    ...
    const int EquationOffset)
{
    ...
    RansNutKEpsilonUpdateProcess adjoint_nut_process(adjoint_model, empty_nut_parameters);
    RansNutKEpsilonUpdateProcess primal_nut_process(primal_model, empty_nut_parameters);
    ...
    auto perturbation_variable =
        RansApplicationTestUtilities::GetPerturbationMethod(rPerturbVariable);

    RansApplicationTestUtilities::RunResidualSensitivityTest<TContainer>(
...
template void RunRansEvmKEpsilonTest<double, ModelPart::ElementsContainerType>(
    ...
    std::function<void(Matrix&, ElementType&, const ProcessInfo&)>,
    const double,
    const double,
    const int,
    const int);
