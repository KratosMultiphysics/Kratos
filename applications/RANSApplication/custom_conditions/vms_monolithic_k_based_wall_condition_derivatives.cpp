    ...
    const ProcessInfo& rProcessInfo)
{
    ...
    const auto& r_element_properties = rCondition.GetValue(NEIGHBOUR_ELEMENTS)[0];
      ...
      mrParentElement(rParentElement)
{
    ...
    mWallHeight = inner_prod(rCondition.GetGeometry().Center() - rParentElement.GetGeometry().Center(), mUnitNormal);

    mConstitutiveLawParameters = ConstitutiveLaw::Parameters(rParentElement.GetGeometry(), element_properties, rProcessInfo);
    ...
    const int Step)
{
    ...
    auto p_constitutive_law = mrParentElement.GetValue(CONSTITUTIVE_LAW);
    mConstitutiveLawParameters.SetShapeFunctionsValues(rN);
    p_constitutive_law->CalculateValue(mConstitutiveLawParameters, EFFECTIVE_VISCOSITY, mKinematicViscosity);
