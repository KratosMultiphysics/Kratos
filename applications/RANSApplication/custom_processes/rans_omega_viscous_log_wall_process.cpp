    ...
    block_for_each(r_conditions, tls_type(), [&](ModelPart::ConditionType& rCondition, tls_type& rTLS) {
        ...
        double u_tau, y_plus;
            RansCalculationUtilities::CalculateYPlusAndUtau(
                ...
                if (rCondition.Id()==109 || rCondition.Id()==107){  
                    KRATOS_WATCH(fluid_velocity);
                    KRATOS_WATCH(mesh_velocity);
                    KRATOS_WATCH(wall_velocity);
                    KRATOS_WATCH(tke);
                    KRATOS_WATCH(u_tau);
                    KRATOS_WATCH(normal); 
                    KRATOS_WATCH(wall_velocity_magnitude);     
                    KRATOS_WATCH(y);
                    KRATOS_WATCH(nu);
                    KRATOS_WATCH(kappa);
                    KRATOS_WATCH(beta);
        ...
        if (y_plus > y_plus_limit) {
            ...
            if (rCondition.Id()==109 || rCondition.Id()==107){
                KRATOS_WATCH(rCondition.Id());
                KRATOS_WATCH(y_plus);
                KRATOS_WATCH(y_plus_limit);
                KRATOS_WATCH(tke_u_tau);
                KRATOS_WATCH(tke_u_tau);
                KRATOS_WATCH(c_mu_25);
                KRATOS_WATCH(kappa);
                KRATOS_WATCH(y);
                KRATOS_WATCH(omega);
                std::cout << std::endl;
            ...
            if (rCondition.Id()==109 || rCondition.Id()==107){
                ...
                KRATOS_WATCH(y_plus);
                KRATOS_WATCH(y_plus_limit);
                KRATOS_WATCH(k_s_plus);
                KRATOS_WATCH(k_s);
                KRATOS_WATCH(u_tau);
                KRATOS_WATCH(nu);
                KRATOS_WATCH(kappa);
                KRATOS_WATCH(y);
                KRATOS_WATCH(omega_wall_plus);
                KRATOS_WATCH(omega);  
                std::cout << std::endl;  
            }
        ...
        for (auto& r_node : r_condition_geometry) {
            if (rCondition.Id()==109 || rCondition.Id()==107){
                KRATOS_WATCH(r_node.FastGetSolutionStepValue(TURBULENT_SPECIFIC_ENERGY_DISSIPATION_RATE));
                KRATOS_WATCH(omega);
            }
            r_node.SetLock();
