...
// Include base h
#include "rans_vector_cell_center_averaging_process.h"

namespace Kratos
{
RansVectorCellCenterAveragingProcess::RansVectorCellCenterAveragingProcess(Model& rModel,
...
RansVectorCellCenterAveragingProcess::~RansVectorCellCenterAveragingProcess()
{
}

int RansVectorCellCenterAveragingProcess::Check()
{
    KRATOS_TRY
    ...
    const Variable<array_1d<double, 3>>& input_variable =
        KratosComponents<Variable<array_1d<double, 3>>>::Get(mAveragingInputVariableName);
...
void RansVectorCellCenterAveragingProcess::Execute()
{
    FindNodalConditionNeighbourCount();
    CalculateCellCenterAverage();
}

std::string RansVectorCellCenterAveragingProcess::Info() const
{
    return std::string("RansVectorCellCenterAveragingProcess");
}

void RansVectorCellCenterAveragingProcess::PrintInfo(std::ostream& rOStream) const
{
    rOStream << this->Info();
}

void RansVectorCellCenterAveragingProcess::PrintData(std::ostream& rOStream) const
{
}

void RansVectorCellCenterAveragingProcess::FindNodalConditionNeighbourCount()
{
    KRATOS_TRY
...
void RansVectorCellCenterAveragingProcess::CalculateCellCenterAverage()
{
    KRATOS_TRY
...
#pragma omp parallel for
    for (int i_condition = 0; i_condition < number_of_conditions; ++i_condition)
    {
        ...
        const Element& r_parent_element = *r_condition.GetValue(NEIGHBOUR_ELEMENTS)(0);
        ...
        for (int i_node = 0; i_node < number_of_nodes; ++i_node)
        {
            ...
            const array_1d<double, 3>& current_output_value =
                r_node.GetValue(r_output_variable);
            r_node.SetLock();
