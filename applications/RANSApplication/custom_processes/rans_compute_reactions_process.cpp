...
int RansComputeReactionsProcess::Check()
{
    ...
    block_for_each(r_model_part.Conditions(), [](ConditionType& rCondition){
        ...
        KRATOS_ERROR_IF_NOT(rCondition.GetValue(NEIGHBOUR_ELEMENTS)[0].Has(CONSTITUTIVE_LAW))
            << "CONSTITUTIVE_LAW is not found in the parent element data container. [ Condition.Id() = "
            << rCondition.Id() << " ].\n";
    });
