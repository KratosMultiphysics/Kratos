...
def CreateSolver( model_part, config ):
    ...
    ##default settings 
    if( hasattr(config,"time_order") ): conv_diff_solver.time_order = config.time_order
    if( hasattr(config,"domain_size") ): conv_diff_solver.domain_size = config.domain_size
    if( hasattr(config,"prediction_order") ): conv_diff_solver.prediction_order = config.prediction_order
    if( hasattr(config,"ReformDofAtEachIteration") ): conv_diff_solver.ReformDofAtEachIteration = config.ReformDofAtEachIteration
    if( hasattr(config,"echo_level") ): conv_diff_solver.echo_level = config.echo_level
    if( hasattr(config,"max_ite") ): conv_diff_solver.max_iter = config.max_iter
    if( hasattr(config,"toll") ): conv_diff_solver.toll = config.toll


    #linear solver settings
    import linear_solver_factory
    if( hasattr(config,"linear_solver_config") ): conv_diff_solver.linear_solver =  linear_solver_factory.ConstructSolver(config.linear_solver_config)
