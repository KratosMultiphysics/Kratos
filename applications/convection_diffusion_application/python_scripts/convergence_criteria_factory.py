...
class convergence_criterion:
    def __init__(self, convergence_criterion_parameters):
        ...
        if(echo_level >= 1):
            KratosMultiphysics.Logger.PrintInfo("::[Convection-Diffusion Solver]:: ", "CONVERGENCE CRITERION : " + 
                    convergence_criterion_parameters["convergence_criterion"].GetString())

        if(convergence_crit == "solution_criterion"):
            self.convection_diffusion_convergence_criterion = KratosMultiphysics.DisplacementCriteria(D_RT, D_AT)
            self.convection_diffusion_convergence_criterion.SetEchoLevel(echo_level)
            
        elif(convergence_crit == "residual_criterion"):
            self.convection_diffusion_convergence_criterion = KratosMultiphysics.ResidualCriteria(R_RT, R_AT)
            self.convection_diffusion_convergence_criterion.SetEchoLevel(echo_level)
            ...
            self.convection_diffusion_convergence_criterion = KratosMultiphysics.AndCriteria(Residual, Displacement)
            ...
            Residual = KratosMultiphysics.ResidualCriteria(R_RT, R_AT)
            Residual.SetEchoLevel(echo_level)
            self.convection_diffusion_convergence_criterion = KratosMultiphysics.OrCriteria(Residual, Displacement)
        else:
