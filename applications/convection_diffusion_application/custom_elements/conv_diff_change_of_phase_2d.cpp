...
void ConvDiffChangeOfPhase2D::CC(double int_weight,double rho, double N1, double N2, double N3, double Tn2, double Tn1, MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    ...
    {
        rLeftHandSideMatrix(0,0) += (N1 * N1)*(temp / (Tn2 - Tn1));
        rLeftHandSideMatrix(0,1) += (N1 * N2)*(temp / (Tn2 - Tn1));
        rLeftHandSideMatrix(0,2) += (N1 * N3)*(temp / (Tn2 - Tn1));
        rLeftHandSideMatrix(1,0) += (N2 * N1)*(temp / (Tn2 - Tn1));
        rLeftHandSideMatrix(1,1) += (N2 * N2)*(temp / (Tn2 - Tn1));
        rLeftHandSideMatrix(1,2) += (N2 * N3)*(temp / (Tn2 - Tn1));
        rLeftHandSideMatrix(2,0) += (N3 * N1)*(temp / (Tn2 - Tn1));
        rLeftHandSideMatrix(2,1) += (N3 * N2)*(temp / (Tn2 - Tn1));
        rLeftHandSideMatrix(2,2) += (N3 * N3)*(temp / (Tn2 - Tn1));
