...
class ListNeuralNetworkData:
    ...
    def ExportAsArray(self):
        ...
        if not self.data_array[0].GetDataShape() == ():
            try:
                array = np.reshape(array, (len(self.data_array),self.data_array[0].GetDataShape()[0], self.data_array[0].GetDataShape()[1]))
            except IndexError:
                pass
        return array
...
class ListDataWithLookback(ListNeuralNetworkData):
    ...
    def __Reshape(self, array, data_only = False):
        ...
        if self.timesteps_as_features or self.features_as_timesteps:
            lookback_length = self.data_array[0].GetLookbackShape()[1] * self.lookback_index
            # Calculate the lenght of the data (with record of the lookback or not)
            try:
                if self.record_data:
                    data_length = self.data_array[0].GetDataShape()[1] * self.lookback_index
                else:
                    data_length = self.data_array[0].GetDataShape()[1]
            except IndexError:
                if self.record_data:
                    data_length = self.data_array[0].GetDataShape()[0] * self.lookback_index
                else:
        ...
        if not self.data_array[0].GetDataShape() == ():
            if self.timesteps_as_features:
                ...
                out_array = np.reshape(array, (len(self.data_array),self.lookback_index, feature_length))
        return out_array
