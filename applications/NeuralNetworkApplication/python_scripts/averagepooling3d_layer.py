...
def Factory(settings):
    ...
    return AveragePooling3DLayer(settings)

class AveragePooling3DLayer(NeuralNetworkLayerClass):
    ...
    def __init__(self, settings):
        ...
        if not settings["strides_2"].GetInt() == 0:
            strides_2 = settings["strides_2"].GetInt()
        elif not self.tunable:
            strides_2 = None
        
        if not settings["strides_3"].GetInt() == 0:
            strides_3 = settings["strides_3"].GetInt()
        elif not self.tunable:
            strides_3 = None

        if strides_1 == None or strides_2 == None or strides_3 == None:
            self.strides = None
        else:
            self.strides = tuple((strides_1, strides_2, strides_3))
        self.pool_size = tuple((settings["pool_size_1"].GetInt(),settings["pool_size_2"].GetInt(),settings["pool_size_3"].GetInt()))
    ...
    def Build(self, hp = None):
        ...
        self.layer = layers.AveragePooling3D(pool_size=self.pool_size, strides=self.strides, padding=self.padding, 
