...
class TestDataGenerationFactory(KratosUnittest.TestCase):
    ...
    def _check_data_dimensions(self):
        ...
        input_data = DataLoadingUtils.ImportDataFromFile(self.output_file_in, 'InputData')
        self.assertEqual(input_data[0].size, self.dimension_in)
        # os.remove(self.output_file_in)
        output_data = DataLoadingUtils.ImportDataFromFile(self.output_file_out, 'OutputData')
        self.assertEqual(output_data[0].size, self.dimension_out)
