...
/*
         ...
         for (unsigned int i = 0; i < number_of_nodes; i++) {
            for (unsigned int j = 0; j < number_of_nodes; j++) {
               Mass(i,j) += rVariables.N[i]*rVariables.N[j] * rIntegrationWeight;
            }
         }

         Mass /= ConstrainedModulus;

         for (unsigned int i = 0; i < number_of_nodes; i++) {
            for (unsigned int j = 0; j < number_of_nodes; j++) {
               rMassMatrix( (i+1)*dofs_per_node-1, (j+1)*dofs_per_node-1) += alpha_factor*tau_factor/rPermeability*Mass(i,j);
            }
         }
         */
      }
