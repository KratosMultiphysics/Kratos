...
static void set_comment(const char* c)
{
    ...
    int len=strlen(c);
    if (len>=_maxlinea) return;
    else comment_len=len;
    strcpy(comment,c);
}
...
bool cascara::agrega_con(const char *stipo, const char *arch)
{
    ...
    else strcpy(filename,parent->nombre);
    char *extaddr=ext_begin(filename);
    if (extaddr) *(extaddr-1)=0;
    if (!abre(filename,".con",true)) return false;
    _initime;
    elemento e1(elemento::etipo(stipo));
    int nv=e1.nv();
    ...
    _gettoken;
    int count=1;
    while(_nexttoken) count++;
    if (count==nv) elemento::flag_mask(0);
    if (count==nv+1)
    {
        elemento::flag_mask(fmask);
        hayfe=true;
    }
...
//double factorp=.0001/RAND_MAX;
    for (i=0; i<qn; i++)
    {
        ...
        index.resize(in+1);
        if (index.len<in+1) index.len=in+1;/*for (k=index.len;k<in+1;k++) index+=-1;*/
        index[in]=nl+=pt;
    }
    _savetime(lee_nodos);
    ...
    char stipo[20];
    int old_base=elemento::io_base();
    elemento::io_base(0); // hay index (va en 0)
    elemento ei;
    e_tipo tipo;
    _initime;
    while (!f.eof())
    {
        ...
        _gettoken;
        sscanf(token,"%d",&qe); // cantidad de elementos
        _nexttoken;
        sscanf(token,"%s",stipo); // tipo
        tipo=ei.tipo(stipo);
        ...
        for (i=0; i<qe; i++)
        {
            ...
            {
                in=ei[j];
                if (in<0||in>index.len||index[in]==-1)
                {
                    malformed();
        ...
        if (token&&!strcmp("coordinates",token)) while (getlinea()&&linea[0])
            {
                ...
                i=atoi(token);
                index.resize(i+1);
                if (index.len<i+1) index.len=i+1;
                j=0;
                while (j<3&&(token=strtok(NULL,seps)))
                {
                    coord=atof(token);
                    pt[j]=coord;
                    j++;
        ...
        if (token&&!strcmp("elements",token))while (getlinea()&&linea[0])
            {
                ...
                if (!strcmp("end",token)) break;
                j=0;
                while (j<nv&&(token=strtok(NULL,seps)))
                {
                    i=atoi(token);
                    e1[j]=index[i];
                    j++;
...
bool malla::lee_mai(_NE)
{
    ...
    */
    ...
    {
        _getlinea;
        sscanf(linea," i %d x %s %s %s",&ix, x, y, z);
        pt[0]=strtod(x,&stop);
        pt[1]=strtod(y,&stop);
        pt[2]=strtod(z,&stop);
        index.resize(ix+1);
        if (index.len<ix+1) index.len=ix+1;
        index[ix]=nl+=pt;
    }
...
bool malla::lee_e(_NE)
{
    ...
    for (i=0; i<qp; i++)
    {
        ...
        _chktoken;
        pr[i]=token;
        if (pr[i].tipo==e_indefinido) return malformed();
    }
    ...
    elemento ei;
    int nv,dim0=-1,ie,iec;  // -1 para evitar warning de iniciaclizacion
    while (getlinea()&&linea[0]&&linea[0]!='}')
    {
        token=strtok(linea,seps);
        _chktoken; // numero
        ...
        _nexttoken;
        i=0;
        while(i<qp&&pr[i]!=token) i++;
        if (i==qp) return malformed();
        emap+=i;
        ei.tipo(pr[i].tipo);
        nv=ei.nv();
        // nodos
        ...
        if (ie)
        {
            if (ei.dim()<dim0)
            {
                ...
                {
                    iec=comun[0];
                    const elemento &ec=el[iec];
                    i=ec.icara(ei);
                    if (i==-1) return malformed();
                    emap+=i;
                    emap+=iec;
...
bool malla::lee_nas(_NE)
{
    ...
    pline index(int(1e6));
    nodo pt;
    pt.f.set(n_borrado); // no guardar nodos intermedios en cuadraticos
    ...
    do
    {
        ...
        _nexttoken;
        _nexttoken;
        e3[0]=i=index[atoi(token)];
        nl[i].f.reset(n_borrado);
        _nexttoken;
        e3[1]=i=index[atoi(token)];
        nl[i].f.reset(n_borrado);
        _nexttoken;
        e3[2]=i=index[atoi(token)];
        nl[i].f.reset(n_borrado);
        el+=e3;
        _gettoken;
    }
...
bool malla::lee_key(_NE)
{
    ...
    el.resize(1000);
    int maxindex=-1;
    pline index(1000); // indices de los nodos
    if (!abre(arch,ext,true)) return false;
    _initime;
    ...
    while (linea[0]=='*')
    {
        ...
        else if (_es("*element_solid"))
        {
            ...
            do
            {
                ...
                el+=elemento(tipo,ixe);
                if (pid<=_FREE_FLAGS) el.last().f=1<<pid;
                _gettoken;
            }
            while (token[0]!='*');
        }
        ...
        else if (_es("*node"))
        {
            ...
            int i,llen;
            nodo pt;
            if (_es("*node_rigid_surface")) pt.f.set(n_permanente);
            char rd[17];
            _getlinea;
            do
            {
                if (strchr(linea,','))  // formato libre separado por comas
                {
                    ...
                    i=atoi(token);
                    _nexttoken;
                    if (token&&token[0]) pt[0]=atof(token);
                    else pt[0]=0;
                    _nexttoken;
                    if (token&&token[0]) pt[1]=atof(token);
                    else pt[1]=0;
                    _nexttoken;
                    if (token&&token[0]) pt[2]=atof(token);
                    else pt[2]=0;
                    _nexttoken;
                    if (token&&token[0]) pt.h=atof(token);
                    else pt.h=0;
                }
...
//		  if (llen && linea[llen-1]=='\r') linea[--llen]='\0';
            ...
            while (linea[0]!='*');
            llen=strlen(linea);
            if (linea[llen-1]=='\r') linea[llen-1]='\0';
        }
        else search("*",0,false);
    }
    ...
    if (el)
    {
        pline anti_index(maxindex+1);
        anti_index.set(-1); //set asigna len
        int j,k;
            ...
            else if (flag&64)  // lee una polyface
            {
                ...
                else  // lee los elms triangulos o cuadrilateros (no mas)
                {
                    ...
                    for (i=0; i<elms; i++)
                    {
                        ...
                        elemento elm((v4==-999999)? e_triangulo : e_cuadrilatero);
                        elm.f=lf;
                        elm[0]=mapnod[abs(v1)-1];
                        elm[1]=mapnod[abs(v2)-1];
                        elm[2]=mapnod[abs(v3)-1];
