...
double& MohrCoulombYieldCriterion::CalculateYieldCondition(double& rStateFunction, const Vector& rStressVector, const double& rAlpha)
{
   ...
   rStateFunction = pow ( StressInvariants.J2InvSQ * K, 2) + pow( HiperbolicA * std::sin( FrictionAngle), 2);
...
void MohrCoulombYieldCriterion::CalculateYieldFunctionDerivative(const Vector& rStressVector, Vector& rYieldFunctionD, const double& rAlpha)
{
   ...
   if ( fabs(StressInvariants.LodeAngle) < LodeCut ) {
        ...
        C3 = - sqrt(3.0) / ( 2.0 * pow( StressInvariants.J2InvSQ, 2) * std::cos( 3.0*StressInvariants.LodeAngle) ) * Kderivative;
        ...
        C3 =   - 3.0 * sqrt(3.0) * SmoothingConstants.B / ( 2.0* pow(StressInvariants.J2InvSQ, 2) ); // to avoid dividing by cos(3teta)
   }

   Alpha = pow ( StressInvariants.J2InvSQ * K, 2) + pow( HiperbolicA * std::sin( FrictionAngle), 2);
