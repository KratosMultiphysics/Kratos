...
class MPMPointOutputProcess(KratosMultiphysics.OutputProcess):
    ...
    def __init__(self, model, params):
        ...
        elif model_part_name.startswith('Background_Grid'):
            model_part_name = model_part_name.replace('Background_Grid','MPM_Material')
        ...
        for var in self.output_variables:
            if _IsDoubleVariable(var) or _IsVectorVariable(var) or _IsArrayVariable(var):
                continue
            else:
    ...
    def ExecuteBeforeSolutionLoop(self):
        ...
        if entity_type == "element":
            ...
            if entity_id > -1:
                ...
                self.coordinates = self.material_point.CalculateOnIntegrationPoints(KratosMPM.MP_COORD, self.model_part.ProcessInfo)[0]
        ...
        # Search material point condition
            ...
            err_msg += f'"element" or "condition". Found: {entity_type}.'
            raise Exception(err_msg)
    ...
    def PrintOutput(self):
        ...
        if self.output_file:
            ...
            for var in self.output_variables:
                ...
                if _IsArrayVariable(var) or _IsVectorVariable(var):
                    out += " " + " ".join( format(v,self.format) for v in value )
                elif _IsDoubleVariable(var):
    ...
    def _GetVariablesNameHeader(self):
        header = "# time"
        for var in self.output_variables:
            if _IsArrayVariable(var):
                header += " {0}_X {0}_Y {0}_Z".format(var.Name())
            elif _IsDoubleVariable(var):
                header += " " + var.Name()
            elif _IsVectorVariable(var):
