...
class KRATOS_API(PFEM_MELTING_APPLICATION) HYPOELASTICSOLID : public Element
...
public:
    ...
    KRATOS_CLASS_INTRUSIVE_POINTER_DEFINITION(HYPOELASTICSOLID);
    ///base type: an IndexedObject that automatically has a unique number
    typedef IndexedObject BaseType;
    ///Element from which it is derived
    typedef HYPOELASTICSOLID<TDim, TNumNodes> ElementBaseType;
     ...
     */
    HYPOELASTICSOLID(IndexType NewId = 0) :
     ...
     */
    HYPOELASTICSOLID(IndexType NewId, const NodesArrayType& ThisNodes) :
     ...
     */
    HYPOELASTICSOLID(IndexType NewId, GeometryType::Pointer pGeometry) :
     ...
     */
    HYPOELASTICSOLID(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties) :
	...
	//     int pepe=0;
     //KRATOS_THROW_ERROR(std::logic_error, "!!!!!!!!!!!  FIRST TIME < FIRST !!!!!!!!", pepe);
        return Kratos::make_intrusive< HYPOELASTICSOLID >(NewId, (this->GetGeometry()).Create(ThisNodes), pProperties);
    }
                           ...
                           PropertiesType::Pointer pProperties) const override
    {
     //int pepe=0;
     //KRATOS_THROW_ERROR(std::logic_error, "!!!!!!!!!!!  FIRST TIME < FIRST !!!!!!!!", pepe);
        return Kratos::make_intrusive< HYPOELASTICSOLID >(NewId, pGeom, pProperties);
    }
	...
	noalias(rLeftHandSideMatrix) = ZeroMatrix(LocalSize, LocalSize);//ZeroMatrix(6, 6);
        // Calculate RHS
    ...
    void Calculate(const Variable<Matrix >& rVariable, Matrix& Output, const ProcessInfo& rCurrentProcessInfo)
{
	...
	CauchyStress(0,3)=msDN_Dx(0,2)*v0[0]+msDN_Dx(1,2)*v1[0]+msDN_Dx(2,2)*v2[0]+msDN_Dx(3,2)*v3[0]+msDN_Dx(0,0)*v0[2]+msDN_Dx(1,0)*v1[2]+msDN_Dx(2,0)*v2[2]+msDN_Dx(3,0)*v3[2];
	...
	CauchyStress(1,2)=msDN_Dx(0,2)*v0[1]+msDN_Dx(1,2)*v1[1]+msDN_Dx(2,2)*v2[1] +msDN_Dx(3,2)*v3[1] + msDN_Dx(0,1)*v0[2]+msDN_Dx(1,1)*v1[2]+msDN_Dx(2,1)*v2[2]+msDN_Dx(2,1)*v3[2];


	CauchyStress(1,0)=msDN_Dx(0,2)*v0[0]+msDN_Dx(1,2)*v1[0]+msDN_Dx(2,2)*v2[0]+msDN_Dx(3,2)*v3[0] + msDN_Dx(0,0)*v0[2]+msDN_Dx(1,0)*v1[2]+msDN_Dx(2,0)*v2[2]+msDN_Dx(3,0)*v3[2];
	CauchyStress(1,2)=msDN_Dx(0,2)*v0[1]+msDN_Dx(1,2)*v1[1]+msDN_Dx(2,2)*v2[1] +msDN_Dx(3,2)*v3[1] + msDN_Dx(0,1)*v0[2]+msDN_Dx(1,1)*v1[2]+msDN_Dx(2,1)*v2[2]+msDN_Dx(2,1)*v3[2];
    ...
    //void HYPOELASTICSOLID<2>::EquationIdVector(EquationIdVectorType& rResult, const ProcessInfo& rCurrentProcessInfo) 
{
    ...
    if(rResult.size() != number_of_nodes*TDim)
        rResult.resize(number_of_nodes*TDim,false);

    for (unsigned int i=0; i<number_of_nodes; i++)
    {
        rResult[i*TDim] = this->GetGeometry()[i].GetDof(VELOCITY_X).EquationId();
        rResult[i*TDim+1] = this->GetGeometry()[i].GetDof(VELOCITY_Y).EquationId();
        if(TDim==3)
        	rResult[i*TDim+2] = this->GetGeometry()[i].GetDof(VELOCITY_Z).EquationId();
    }
    ...
    if(rElementalDofList.size() != number_of_nodes*TDim)
        rElementalDofList.resize(number_of_nodes*TDim);

    for (unsigned int i=0; i<number_of_nodes; i++)
    {
        rElementalDofList[i*TDim] = this->GetGeometry()[i].pGetDof(VELOCITY_X);
        rElementalDofList[i*TDim+1] = this->GetGeometry()[i].pGetDof(VELOCITY_Y);
        if(TDim==3) rElementalDofList[i*TDim+2] = this->GetGeometry()[i].pGetDof(VELOCITY_Z);
    ...
    const unsigned int dim = this->GetGeometry().WorkingSpaceDimension();
    ...
    for (unsigned int i=0; i<number_of_nodes; i++)
    {
        ...
        const array_1d<double,3>& vel = this->GetGeometry()[i].FastGetSolutionStepValue(VELOCITY,Step);
        Values[index] = vel[0];
        Values[index + 1] = vel[1];
        if(TDim==3) Values[index + 2] = vel[2];
    }
}
    ...
    {
        unsigned int index = i*dim;
        const array_1d<double,3>& acc = this->GetGeometry()[i].FastGetSolutionStepValue(ACCELERATION,Step);
        Values[index] = acc[0];
        Values[index + 1] = acc[1];
        if(TDim==3) Values[index + 2] = acc[2];
    }
    }
...
*/
    ...
    std::string Info() const override
    {
        ...
        buffer << "HYPOELASTICSOLID #" << this->Id();
        return buffer.str();
    }
    ...
    {
        rOStream << "HYPOELASTICSOLID" << TDim << "D";
    }
...
private:
    ...
    /// Assignment operator.
    HYPOELASTICSOLID & operator=(HYPOELASTICSOLID const& rOther);

    /// Copy constructor.
    HYPOELASTICSOLID(HYPOELASTICSOLID const& rOther);

    ///@}

}; // Class HYPOELASTICSOLID
...
inline std::istream& operator >>(std::istream& rIStream,
                                 HYPOELASTICSOLID<TDim, TNumNodes>& rThis)
{
    return rIStream;
}
...
inline std::ostream& operator <<(std::ostream& rOStream,
                                 const HYPOELASTICSOLID<TDim, TNumNodes>& rThis)
{
