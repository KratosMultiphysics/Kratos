...
#if !defined(KRATOS_LAGRANGIAN_FLUID_VMS_H_INCLUDED )
#define  KRATOS_LAGRANGIAN_FLUID_VMS_H_INCLUDED
// System includes
#include <string>
#include <iostream>
// External includes
...
/**The "LagrangianFluidVMS" element is an element based on the Variation Multiscale Stabilization technique (VMS)
...
class LagrangianFluidVMS : public VMS<TDim, TNumNodes>
{
public:
    ///@name Type Definitions
    ///@{
    /// Pointer definition of TwoFluidVMS
    KRATOS_CLASS_INTRUSIVE_POINTER_DEFINITION(LagrangianFluidVMS);
     ...
     */
    LagrangianFluidVMS(IndexType NewId = 0) :
     ...
     */
    LagrangianFluidVMS(IndexType NewId, const NodesArrayType& ThisNodes) :
     ...
     */
    LagrangianFluidVMS(IndexType NewId, GeometryType::Pointer pGeometry) :
     ...
     */
    LagrangianFluidVMS(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties) :
        ElementBaseType(NewId, pGeometry, pProperties)
    {
    }
    /// Destructor.
    ~LagrangianFluidVMS() override
    {
    }
    ///@}
    ///@name Operators
                           ...
                           PropertiesType::Pointer pProperties) const override
    {
     //int pepe=0;
     //KRATOS_THROW_ERROR(std::logic_error, "!!!!!!!!!!!  FIRST TIME < FIRST !!!!!!!!", pepe);
        return Kratos::make_intrusive< LagrangianFluidVMS >(NewId, pGeom, pProperties);
    }
                                      ...
                                      const ProcessInfo &rProcessInfo)
    {
        ...
        //KRATOS_THROW_ERROR(std::logic_error, "!!!!!!!!!!!  SECOND TIME < FIRST !!!!!!!!", Viscosity);
...
//IF WE WANT USE A DIFFERENT MODEL THAT THE NEWTONIAN ONE THIS FUNCTION MUST BE DISCONECTED
...
//             KRATOS_THROW_ERROR(std::invalid_argument, "DIVPROJ Key is 0. Check if the application was correctly registered.", "");
    ...
    std::string Info() const override
    {
        ...
        buffer << "LAGRANGIAN_FLUID_ #" << this->Id();
        return buffer.str();
    }
    /// Print information about this object.
    void PrintInfo(std::ostream& rOStream) const override
    {
        rOStream << "LAGRANGIAN_FLUID_" << TDim << "D";
    }
    //        /// Print object's data.
...
void CalculateLocalVelocityContribution(MatrixType& rDampingMatrix,
    ...
    virtual void AddIntegrationPointVelocityContribution(MatrixType& rDampingMatrix,
            VectorType& rDampRHS,
...
private:
    ...
    ///@{
    /// Assignment operator.
    LagrangianFluidVMS & operator=(LagrangianFluidVMS const& rOther);
    /// Copy constructor.
    LagrangianFluidVMS(LagrangianFluidVMS const& rOther);
    ///@}
}; // Class TwoFluidVMS
///@}
///@name Type Definitions
...
inline std::istream & operator >>(std::istream& rIStream,
                                  LagrangianFluidVMS<TDim, TNumNodes>& rThis)
{
    return rIStream;
}
...
inline std::ostream & operator <<(std::ostream& rOStream,
                                  const LagrangianFluidVMS<TDim, TNumNodes>& rThis)
{
