...
/*    KRATOS_WATCH("SOLIDOOOOOOOOOOOOOOOOOOOOOOOOOO")
    ...
    KRATOS_WATCH("SOLIDOOOOOOOOOOOOOOOOOOOOOOOOOO")*/
        ...
        bool hypo=false;
        if(this->GetGeometry()[0].FastGetSolutionStepValue(IS_SOLID) == 1.0 && this->GetGeometry()[1].FastGetSolutionStepValue(IS_SOLID) == 1.0 && this->GetGeometry()[2].FastGetSolutionStepValue(IS_SOLID) == 1.0 ) hypo=true;
	//hypo=false;
	if(hypo==true)             Density=this->GetProperties()[DENSITY_WATER];//Density=2500.0; //Density=1100.0;	
	else Density=1.0; 
	    ...
	    //get the value of Cauchy stress at the Gauss point. It is given by:
		    ...
		    rRightHandSideVector[5] -= DN_DX(2,0) * SYX + DN_DX(2,1) * SYY;
                   ...
                   /*
                   rRightHandSideVector[0] -= DN_DX(0,0)*this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_XX) + DN_DX(0,1)*this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY);
		    rRightHandSideVector[1] -= DN_DX(0,0)*this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY) + DN_DX(0,1)*this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY);

                   rRightHandSideVector[2] -= DN_DX(1,0)*this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_XX) + DN_DX(1,1)*this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY);
		    rRightHandSideVector[3] -= DN_DX(1,0)*this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY) + DN_DX(1,1)*this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY);

                   rRightHandSideVector[4] -= DN_DX(2,0)*this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_XX) + DN_DX(2,1)*this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY);
		    rRightHandSideVector[5] -= DN_DX(2,0)*this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY) + DN_DX(2,1)*this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY);
*/
		...
		//get the old pressure
		    ...
		    rRightHandSideVector[0] += DN_DX(0,0)*pavg;
		    rRightHandSideVector[1] += DN_DX(0,1)*pavg;
		    rRightHandSideVector[2] += DN_DX(1,0)*pavg;
		    rRightHandSideVector[3] += DN_DX(1,1)*pavg;
		    rRightHandSideVector[4] += DN_DX(2,0)*pavg;
		    rRightHandSideVector[5] += DN_DX(2,1)*pavg;
    		    }
...
/*    if (rDampingMatrix.size1() != 6)
    ...
    if(TDim==2)
    {
	    ...
	    //getting properties
            double nu = 0.333333333*(GetGeometry()[0].FastGetSolutionStepValue(VISCOSITY)+
           ...
           double density = 0.333333333*(GetGeometry()[0].FastGetSolutionStepValue(DENSITY)+
                                         GetGeometry()[1].FastGetSolutionStepValue(DENSITY) +
                                         GetGeometry()[2].FastGetSolutionStepValue(DENSITY));
            //KRATOS_ERROR << "AddTimeIntegratedLHS is not implemented." << std::endl;

            bool hypo=false;
            if(GetGeometry()[0].FastGetSolutionStepValue(IS_SOLID) == 1.0 && GetGeometry()[1].FastGetSolutionStepValue(IS_SOLID) == 1.0 && GetGeometry()[2].FastGetSolutionStepValue(IS_SOLID) == 1.0 ) hypo=true;
            //hypo=false;
	    if(hypo==true)             {density=this->GetProperties()[DENSITY_WATER];  }//density= 2500.0; //density=1100.0;	
	    else density=1.0;	
...
*/
  ...
  if(rVariable == CAUCHY_STRESS_TENSOR)
    ...
    void Calculate(const Variable<double>& rVariable, double& rOutput, const ProcessInfo& rCurrentProcessInfo) override
    {
        if (rVariable == NODAL_VOLUME)
        {
            ...
            for (unsigned int i = 0; i < TNumNodes; ++i)
            {
                ...
                this->GetGeometry()[i].FastGetSolutionStepValue(NODAL_VOLUME) += Area * N[i];
		//KRATOS_WATCH(Area)
		//KRATOS_WATCH(N)
	...
	//KRATOS_WATCH(E)	
	    ...
	    CauchyStress(1,0)=CauchyStress(0,1);//msDN_Dx(0,1)*v0[0]+msDN_Dx(1,1)*v1[0]+msDN_Dx(2,1)*v2[0] + msDN_Dx(0,0)*v0[1]+msDN_Dx(1,0)*v1[1]+msDN_Dx(2,0)*v2[1];
	    CauchyStress(1,1)=2.0*(msDN_Dx(0,1)*v0[1]+msDN_Dx(1,1)*v1[1]+msDN_Dx(2,1)*v2[1]);
...
protected:
                                ...
                                const double Weight)
    {
        ...
        if(this->GetGeometry()[0].FastGetSolutionStepValue(IS_SOLID) == 1.0 && this->GetGeometry()[1].FastGetSolutionStepValue(IS_SOLID) == 1.0 && this->GetGeometry()[2].FastGetSolutionStepValue(IS_SOLID) == 1.0 ) hypo=true;
        //hypo=false;
	if(hypo==true)             density=this->GetProperties()[DENSITY_WATER]; //density=2500.0; //this->GetProperties()[DENSITY_WATER]; //density=1100.0;	
	else density=1.0; 
