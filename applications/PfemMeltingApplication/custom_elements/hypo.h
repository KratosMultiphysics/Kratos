...
public:
	...
	//KRATOS_WATCH("SHAPE FUNCTIONS")
	//KRATOS_WATCH("SHAPE FUNCTIONS")
	//KRATOS_WATCH("SHAPE FUNCTIONS")	
	//KRATOS_WATCH(DN_DX)
	
	//calculo del jacobian
	//----------------------
	//----------------------
	
            MatrixType Fgrad = ZeroMatrix(2, 2);

            MatrixType invFgrad = ZeroMatrix(2, 2);
		...
		for (unsigned int j = 0; j < TDim; ++j)
		    ...
		    for (unsigned int k=0; k<TNumNodes; k++)
		    	{
		    		dispn=this->GetGeometry()[k].FastGetSolutionStepValue(DISPLACEMENT)[i];
		    		dispn0=this->GetGeometry()[k].FastGetSolutionStepValue(DISPLACEMENT,1)[i];
		    		sj+= DN_DX(k,j)*(dispn-dispn0);
		    	}
		    Fgrad(i,j)=sj;
		    }
		 }   
	    ...
	    MatrixType invFgradTransp = ZeroMatrix(2, 2);

          
             double lagDNX0 = DN_DX(0, 0) * invFgrad(0, 0) + DN_DX(0, 1) * invFgrad(1, 0);
             double lagDNY0 = DN_DX(0, 0) * invFgrad(0, 1) + DN_DX(0, 1) * invFgrad(1, 1);

    	     double lagDNX1 = DN_DX(1, 0) * invFgrad(0, 0) + DN_DX(1, 1) * invFgrad(1, 0);
             double lagDNY1 = DN_DX(1, 0) * invFgrad(0, 1) + DN_DX(1, 1) * invFgrad(1, 1);

             double lagDNX2 = DN_DX(2, 0) * invFgrad(0, 0) + DN_DX(2, 1) * invFgrad(1, 0);
             double lagDNY2 = DN_DX(2, 0) * invFgrad(0, 1) + DN_DX(2, 1) * invFgrad(1, 1);
    
             BoundedMatrix<double,3,2> lagDN_Dx;
    	     lagDN_Dx(0,0)=lagDNX0;
    	     lagDN_Dx(0,1)=lagDNY0;
    	     
    	     lagDN_Dx(1,0)=lagDNX1;
    	     lagDN_Dx(1,1)=lagDNY1;

    	     lagDN_Dx(2,0)=lagDNX2;
    	     lagDN_Dx(2,1)=lagDNY2;



             //Compute cartesian derivatives [dN/dx_n]
             BoundedMatrix<double, TNumNodes, TDim> DN_DX_e;
  	     noalias(DN_DX_e) = prod(DN_DX, invFgrad);

	     //KRATOS_WATCH("DN_DX_e")
	     ...
	     DN_DX(0,0)=lagDN_Dx(0,0);
	     DN_DX(0,1)=lagDN_Dx(0,1);
	     DN_DX(1,0)=lagDN_Dx(1,0);
	     DN_DX(1,1)=lagDN_Dx(1,1);
	     DN_DX(2,0)=lagDN_Dx(2,0);
	     DN_DX(2,1)=lagDN_Dx(2,1);

	     //KRATOS_WATCH("DN_DX")	
	     //KRATOS_WATCH(DN_DX)
	     
	     //KRATOS_WATCH("TENSOR GRADIENTE DEFORMACION")
	     //KRATOS_WATCH("Fgrad")
	     //KRATOS_WATCH(Fgrad)
	     //KRATOS_WATCH("invFgrad")
	     //KRATOS_WATCH(invFgrad)
	     //KRATOS_WATCH("Jacobiano")
	     //KRATOS_WATCH(FJacobian)
	...
	//
        ...
        Density=this->GetProperties()[DENSITY_WATER];
		...
		//KRATOS_WATCH(body_force[0])
		//KRATOS_WATCH(body_force[1])
...
/*		    double SXX= 0.333333333 * this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_XX,1) + 0.333333333 * this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_XX,1)+0.333333333 * this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_XX,1);
		    double SXY= 0.333333333 * this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY,1) + 0.333333333 * this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY,1)+0.333333333 * this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY,1);

		    double SYX= 0.333333333 * this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_YX,1) + 0.333333333 * this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_YX,1)+0.333333333 * this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_YX,1);
		    double SYY= 0.333333333 * this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY,1) + 0.333333333 * this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY,1)+0.333333333 * this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY,1);
*/



		    double SXXinc= 0.333333333 * this->GetGeometry()[0].FastGetSolutionStepValue(DELTA_SIGMA_XX) + 0.333333333 * this->GetGeometry()[1].FastGetSolutionStepValue(DELTA_SIGMA_XX)+0.333333333 * this->GetGeometry()[2].FastGetSolutionStepValue(DELTA_SIGMA_XX);
		    double SXYinc= 0.333333333 * this->GetGeometry()[0].FastGetSolutionStepValue(DELTA_SIGMA_XY) + 0.333333333 * this->GetGeometry()[1].FastGetSolutionStepValue(DELTA_SIGMA_XY)+0.333333333 * this->GetGeometry()[2].FastGetSolutionStepValue(DELTA_SIGMA_XY);

		    double SYXinc= 0.333333333 * this->GetGeometry()[0].FastGetSolutionStepValue(DELTA_SIGMA_YX) + 0.333333333 * this->GetGeometry()[1].FastGetSolutionStepValue(DELTA_SIGMA_YX)+0.333333333 * this->GetGeometry()[2].FastGetSolutionStepValue(DELTA_SIGMA_YX);
		    double SYYinc= 0.333333333 * this->GetGeometry()[0].FastGetSolutionStepValue(DELTA_SIGMA_YY) + 0.333333333 * this->GetGeometry()[1].FastGetSolutionStepValue(DELTA_SIGMA_YY)+0.333333333 * this->GetGeometry()[2].FastGetSolutionStepValue(DELTA_SIGMA_YY);
		    ...
		    double SYY= 0.333333333 * this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY) + 0.333333333 * this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY)+0.333333333 * this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY);
                   ...
                   double SXX_old= SXX-SXXinc;
                   double SXY_old= SXY-SXYinc;
                   double SYX_old= SYX-SYXinc;
                   double SYY_old= SYY-SYYinc;
                   
		    /*KRATOS_WATCH("INCREMENTO    RECONSTRUCCIONNNNNN")                   
		    KRATOS_WATCH(SXXinc)
		    KRATOS_WATCH(SXYinc)
		    KRATOS_WATCH(SYXinc)
		    KRATOS_WATCH(SYYinc)
		    ...
		    KRATOS_WATCH("RECONSTRUCCIONNNNNN")                   
		    KRATOS_WATCH(SXX)
		    KRATOS_WATCH(SXY)
		    KRATOS_WATCH(SYX)
		    KRATOS_WATCH(SYY)
		    
		    KRATOS_WATCH("PASO_ANTERIOR")                   
		    KRATOS_WATCH(SXX_old)
		    KRATOS_WATCH(SXY_old)
		    KRATOS_WATCH(SYX_old)
		    KRATOS_WATCH(SYY_old)*/
		    

		    //KRATOS_WATCH(SXX_new)
		    //KRATOS_WATCH(SXY_new)
		    //KRATOS_WATCH(SYX_new)
		    //KRATOS_WATCH(SYY_new)


                   //KRATOS_ERROR << "AddTimeIntegratedLHS is not implemented." << std::endl; 
/*		    rRightHandSideVector[0] -= DN_DX(0,0)*this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_XX,1) + DN_DX(0,1)*this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY,1);

		    //0  +   dN1/dy*SigmaY + 0 + dN1/dx*TauXY
		    rRightHandSideVector[1] -= DN_DX(0,0)*this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_YX,1) + DN_DX(0,1)*this->GetGeometry()[0].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY,1);

		    //dN2/dx*SigmaX + 0 + dN2/dy*TauXY
		    rRightHandSideVector[2] -= DN_DX(1,0)*this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_XX,1) + DN_DX(1,1)*this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY,1);

		    //0  +   dN2/dy*SigmaY + 0 + dN2/dx*TauXY
		    rRightHandSideVector[3] -= DN_DX(1,0)*this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_YX,1) + DN_DX(1,1)*this->GetGeometry()[1].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY,1);

		    //dN3/dx*SigmaX + 0 + dN3/dy*TauXY
		    rRightHandSideVector[4] -= DN_DX(2,0)*this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_XX,1) + DN_DX(2,1)*this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_XY,1);

		    //0  +   dN3/dy*SigmaY + 0 + dN3/dx*TauXY
		    rRightHandSideVector[5] -= DN_DX(2,0)*this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_YX,1) + DN_DX(2,1)*this->GetGeometry()[2].FastGetSolutionStepValue(HISTORICAL_SIGMA_YY,1);
*/
		  ...
		  //Adding the traction force
		//if (rCurrentProcessInfo[STEP]==1){
		
		  //KRATOS_ERROR << "AddTimeIntegratedLHS is not implemented." << std::endl; 
		  if(this->GetGeometry()[1].FastGetSolutionStepValue(FLAG_VARIABLE)==1 and  this->GetGeometry()[0].FastGetSolutionStepValue(FLAG_VARIABLE)==1)
		  {
		    double x21 = this->GetGeometry()[0].X() - this->GetGeometry()[1].X();
		    double y21 = this->GetGeometry()[0].Y() - this->GetGeometry()[1].Y();
		    double lenght = sqrt(x21*x21 + y21*y21);
		    //lenght = sqrt(lenght);
                   //KRATOS_ERROR << "AddTimeIntegratedLHS is not implemented." << std::endl;  
		    //double area1=norm;
		    normaledge1(0)= -y21/lenght;
		    normaledge1(1)= x21/lenght;

		    rRightHandSideVector[0] += this->GetGeometry()[0].FastGetSolutionStepValue(EXTERNAL_PRESSURE) * 0.5 * lenght * normaledge1(0);
		    rRightHandSideVector[1] += this->GetGeometry()[0].FastGetSolutionStepValue(EXTERNAL_PRESSURE) * 0.5 * lenght * normaledge1(1);
		    
		    //rRightHandSideVector[1] += this->GetGeometry()[0].FastGetSolutionStepValue(EXTERNAL_PRESSURE) * 0.5 * lenght;

		    rRightHandSideVector[2] += this->GetGeometry()[1].FastGetSolutionStepValue(EXTERNAL_PRESSURE) * 0.5 * lenght * normaledge1(0);
   		    rRightHandSideVector[3] += this->GetGeometry()[1].FastGetSolutionStepValue(EXTERNAL_PRESSURE) * 0.5 * lenght * normaledge1(1);


		    //rRightHandSideVector[3] += this->GetGeometry()[1].FastGetSolutionStepValue(EXTERNAL_PRESSURE) * 0.5 * lenght;
		  }


		  if(this->GetGeometry()[2].FastGetSolutionStepValue(FLAG_VARIABLE)==1 and  this->GetGeometry()[1].FastGetSolutionStepValue(FLAG_VARIABLE)==1)
		  {
		    double x21 = this->GetGeometry()[1].X() - this->GetGeometry()[2].X();
		    double y21 = this->GetGeometry()[1].Y() - this->GetGeometry()[2].Y();
		    double lenght = sqrt(x21*x21 + y21*y21);

		    normaledge1(0)= -y21/lenght;
		    normaledge1(1)= x21/lenght;

		    rRightHandSideVector[4] += this->GetGeometry()[2].FastGetSolutionStepValue(EXTERNAL_PRESSURE) * 0.5 * lenght * normaledge1(0);
		    rRightHandSideVector[5] += this->GetGeometry()[2].FastGetSolutionStepValue(EXTERNAL_PRESSURE) * 0.5 * lenght * normaledge1(1);
		    
		    //rRightHandSideVector[5] += this->GetGeometry()[2].FastGetSolutionStepValue(EXTERNAL_PRESSURE) * 0.5 * lenght;
		  ...
		  if(this->GetGeometry()[2].FastGetSolutionStepValue(FLAG_VARIABLE)==1 and  this->GetGeometry()[0].FastGetSolutionStepValue(FLAG_VARIABLE)==1)
		  {
		    double x21 = this->GetGeometry()[2].X() - this->GetGeometry()[0].X();
		    double y21 = this->GetGeometry()[2].Y() - this->GetGeometry()[0].Y();
		    double lenght = sqrt(x21*x21 + y21*y21);
    ...
    void CalculateMassMatrix(MatrixType& rMassMatrix, const ProcessInfo& rCurrentProcessInfo) override
{
	...
	//----------------------
        ...
        double dispn=0.0;
        double dispn0=0.0;
        //array_1d< double, 3 > dispn = ZeroVector(3);
        //array_1d< double, 3 > dispn0 = ZeroVector(3);
...
/*    if (rDampingMatrix.size1() != 6)
    ...
    if(TDim==2)
    {
	    ...
	    double KAPPA=LAMBDA+0.6666666*MU;
            ...
            //BoundedMatrix<double,2,2> Fgrad;
            MatrixType Fgrad = ZeroMatrix(2, 2);
            //noalias(Fgrad) = ZeroMatrix(TDim, TDim);
            //BoundedMatrix<double,2,2> invFgrad;
            MatrixType invFgrad = ZeroMatrix(2, 2);
		...
		for (unsigned int j = 0; j < TDim; ++j)
            ...
            //BoundedMatrix<double,2,2> invFgradTransp=ZeroMatrix(TDim, TDim);
	    ...
	    MatrixType invFgradTransp = ZeroMatrix(2, 2);
            ...
            //KRATOS_WATCH(invFgrad)
            //KRATOS_WATCH(FJacobian)
            //KRATOS_WATCH(invFgradTransp)
            
            //MatrixType SpatialVelocityGrad = ZeroMatrix(2, 3);
            //SpatialVelocityGrad = prod(VelDefgrad, invFgrad);
            
            
             double lagDNX0 = msDN_Dx(0, 0) * invFgrad(0, 0) + msDN_Dx(0, 1) * invFgrad(1, 0);
             double lagDNY0 = msDN_Dx(0, 0) * invFgrad(0, 1) + msDN_Dx(0, 1) * invFgrad(1, 1);

    	     double lagDNX1 = msDN_Dx(1, 0) * invFgrad(0, 0) + msDN_Dx(1, 1) * invFgrad(1, 0);
             double lagDNY1 = msDN_Dx(1, 0) * invFgrad(0, 1) + msDN_Dx(1, 1) * invFgrad(1, 1);

             double lagDNX2 = msDN_Dx(2, 0) * invFgrad(0, 0) + msDN_Dx(2, 1) * invFgrad(1, 0);
             double lagDNY2 = msDN_Dx(2, 0) * invFgrad(0, 1) + msDN_Dx(2, 1) * invFgrad(1, 1);
	     ...
	     msDN_Dx(0,0)=lagDN_Dx(0,0);
	     msDN_Dx(0,1)=lagDN_Dx(0,1);
	     msDN_Dx(1,0)=lagDN_Dx(1,0);
	     msDN_Dx(1,1)=lagDN_Dx(1,1);
	     msDN_Dx(2,0)=lagDN_Dx(2,0);
	     msDN_Dx(2,1)=lagDN_Dx(2,1);
		...
		//msB(0,index+0)=lagDN_Dx(i,0);//msDN_Dx(i,0);
		//msB(0,index+1)= 0.0;
		//msB(1,index+0)=0.0;
		//msB(1,index+1)= lagDN_Dx(i,1);//msDN_Dx(i,1);
		//msB(2,index+0)= lagDN_Dx(i,1);//msDN_Dx(i,1);
		//msB(2,index+1)= lagDN_Dx(i,0);//msDN_Dx(i,0);
...
/*                  
        // Check sizes and initialize matrix
    	  BoundedMatrix<double,LocalSize,LocalSize> K;

	  K(0,0) = 2.0 * pow(msDN_Dx(0,0), 2) * viscosity + viscosity * pow(msDN_Dx(0,1), 2)           + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,0) *  msDN_Dx(0,0);
	  K(0,1) = msDN_Dx(0,1) * viscosity * msDN_Dx(0,0)                                    + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,0) *  msDN_Dx(0,1);
	  K(0,2) = 2.0 * msDN_Dx(0,0) * viscosity * msDN_Dx(1,0) + msDN_Dx(0,1) * viscosity * msDN_Dx(1,1) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,0) *  msDN_Dx(1,0);
	  K(0,3) = msDN_Dx(0,1) * viscosity * msDN_Dx(1,0)                                     + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,0) *  msDN_Dx(1,1);
	  K(0,4) = 2.0 * msDN_Dx(0,0) * viscosity * msDN_Dx(2,0) + msDN_Dx(0,1) * viscosity * msDN_Dx(2,1)+ (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,0) *  msDN_Dx(2,0);
	  K(0,5) = msDN_Dx(0,1) * viscosity * msDN_Dx(2,0)                                     + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,0) *  msDN_Dx(2,1);
	  K(1,0) = msDN_Dx(0,1) * viscosity * msDN_Dx(0,0)                                      + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,1) *  msDN_Dx(0,0);
	  K(1,1) = 2.0 * viscosity * pow(msDN_Dx(0,1), 2) + pow(msDN_Dx(0,0), 2) * viscosity         + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,1) *  msDN_Dx(0,1);
	  K(1,2) = msDN_Dx(0,0) * viscosity * msDN_Dx(1,1)                                     + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,1) *  msDN_Dx(1,0);
	  K(1,3) = 2.0 * msDN_Dx(0,1) * viscosity * msDN_Dx(1,1) + msDN_Dx(0,0) * viscosity * msDN_Dx(1,0) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,1) *  msDN_Dx(1,1);
	  K(1,4) = msDN_Dx(0,0) * viscosity * msDN_Dx(2,1)                                      + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,1) *  msDN_Dx(2,0);
	  K(1,5) = 2.0 * msDN_Dx(0,1) * viscosity * msDN_Dx(2,1) + msDN_Dx(0,0) * viscosity * msDN_Dx(2,0) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(0,1) *  msDN_Dx(2,1);
	  K(2,0) = 2.0 * msDN_Dx(0,0) * viscosity * msDN_Dx(1,0) + msDN_Dx(0,1) * viscosity * msDN_Dx(1,1) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,0) *  msDN_Dx(0,0);
	  K(2,1) = msDN_Dx(0,0) * viscosity * msDN_Dx(1,1)                                     + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,0) *  msDN_Dx(0,1);
	  K(2,2) = 2.0 * pow(msDN_Dx(1,0), 2) * viscosity + viscosity * pow(msDN_Dx(1,1), 2)           + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,0) *  msDN_Dx(1,0);
	  K(2,3) = msDN_Dx(1,1) * viscosity * msDN_Dx(1,0)                                    + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,0) *  msDN_Dx(1,1);
	  K(2,4) = 2.0 * msDN_Dx(1,0) * viscosity * msDN_Dx(2,0) + msDN_Dx(1,1) * viscosity * msDN_Dx(2,1) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,0) *  msDN_Dx(2,0);
	  K(2,5) = msDN_Dx(1,1) * viscosity * msDN_Dx(2,0)                                     + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,0) *  msDN_Dx(2,1);
	  K(3,0) = msDN_Dx(0,1) * viscosity * msDN_Dx(1,0)                                      + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,1) *  msDN_Dx(0,0);
	  K(3,1) = 2.0 * msDN_Dx(0,1) * viscosity * msDN_Dx(1,1) + msDN_Dx(0,0) * viscosity * msDN_Dx(1,0) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,1) *  msDN_Dx(0,1);
	  K(3,2) = msDN_Dx(1,1) * viscosity * msDN_Dx(1,0)                                     + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,1) *  msDN_Dx(1,0);
	  K(3,3) = 2.0 * viscosity * pow(msDN_Dx(1,1), 2) + pow(msDN_Dx(1,0), 2) * viscosity          + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,1) *  msDN_Dx(1,1);
	  K(3,4) = msDN_Dx(1,0) * viscosity * msDN_Dx(2,1)                                   + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,1) *  msDN_Dx(2,0);
	  K(3,5) = 2.0 * msDN_Dx(1,1) * viscosity * msDN_Dx(2,1) + msDN_Dx(1,0) * viscosity * msDN_Dx(2,0) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(1,1) *  msDN_Dx(2,1);
	  K(4,0) = 2.0 * msDN_Dx(0,0) * viscosity * msDN_Dx(2,0) + msDN_Dx(0,1) * viscosity * msDN_Dx(2,1) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,0) *  msDN_Dx(0,0);
	  K(4,1) = msDN_Dx(0,0) * viscosity * msDN_Dx(2,1)                                      + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,0) *  msDN_Dx(0,1);
	  K(4,2) = 2.0 * msDN_Dx(1,0) * viscosity * msDN_Dx(2,0) + msDN_Dx(1,1) * viscosity * msDN_Dx(2,1) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,0) *  msDN_Dx(1,0);
	  K(4,3) = msDN_Dx(1,0) * viscosity * msDN_Dx(2,1)                                      + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,0) *  msDN_Dx(1,1);
	  K(4,4) = 2.0 * pow(msDN_Dx(2,0), 2) * viscosity + viscosity * pow(msDN_Dx(2,1), 2)           + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,0) *  msDN_Dx(2,0);
	  K(4,5) = msDN_Dx(2,1) * viscosity * msDN_Dx(2,0)                                      + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,0) *  msDN_Dx(2,1);
	  K(5,0) = msDN_Dx(0,1) * viscosity * msDN_Dx(2,0)                                      + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,1) *  msDN_Dx(0,0);
	  K(5,1) = 2.0 * msDN_Dx(0,1) * viscosity * msDN_Dx(2,1) + msDN_Dx(0,0) * viscosity * msDN_Dx(2,0) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,1) *  msDN_Dx(0,1);
	  K(5,2) = msDN_Dx(1,1) * viscosity * msDN_Dx(2,0)                                    + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,1) *  msDN_Dx(1,0);
	  K(5,3) = 2.0 * msDN_Dx(1,1) * viscosity * msDN_Dx(2,1) + msDN_Dx(1,0) * viscosity * msDN_Dx(2,0) + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,1) *  msDN_Dx(1,1);
	  K(5,4) = msDN_Dx(2,1) * viscosity * msDN_Dx(2,0)                                     + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,1) *  msDN_Dx(2,0);
	  K(5,5) = 2.0 * viscosity * pow(msDN_Dx(2,1), 2) + pow(msDN_Dx(2,0), 2) * viscosity         + (k-2.0 / 3.0 * viscosity) * msDN_Dx(2,1) *  msDN_Dx(2,1);
          K *= Area; 
...
//calculo del jacobian
	...
	//----------------------
	    ...
	    MatrixType invFgradTransp = ZeroMatrix(2, 2);
	     ...
	     /*KRATOS_WATCH("TENSOR GRADIENTE DEFORMACION _______CALCULO SIGMA SOMBRERO")
	     KRATOS_WATCH("Fgrad")
	     KRATOS_WATCH(Fgrad)
	     KRATOS_WATCH("invFgrad")
	     KRATOS_WATCH(invFgrad)
	...
	//KRATOS_WATCH(E)	
	    ...
	    /*CauchyStress(0,0)+=(KAPPA - 2.0 / 3.0 * MU)*div_v;
	    CauchyStress(1,1)+=(KAPPA - 2.0 / 3.0 * MU)*div_v;*/


            //KRATOS_THROW_ERROR(std::logic_error,"pressure calculation mjhkjhkhkjjh not implemented","");
...
protected:
                                ...
                                const double Weight)
    {
        ...
        double density=this->GetProperties()[DENSITY_WATER];
//KRATOS_THROW_ERROR(std::logic_error,"not dereeeeeeeeeeeeeeeeeeeeeeecha","");
