...
class PfemCoupledFluidThermalSolver(PythonSolver):
    ...
    def new(self):
        ...
        if(self.step==self.fluid_solver.main_model_part.ProcessInfo[KratosMultiphysics.STEP]):
            ...
            for node in self.fluid_solver.main_model_part.Nodes:    
                fluid_model_part.AddNode(node,0)     
    ...
    def ReMesh_aux(self):
        ...
        (self.Mesher).ReGenerateMesh("QFLUID2D","ThermalFace2D2N", fluid_model_part, self.node_erase_process, True, False, 1.2, 0.05)  #1.8         
        ...
        neighbor_search = KratosMultiphysics.FindGlobalNodalElementalNeighboursProcess(fluid_model_part)
        neighbor_search.Execute()

        if(self.domain_size ==3):
            neighbor_condition_search = KratosMultiphysics.FindConditionsNeighboursProcess(fluid_model_part,3, 20)
