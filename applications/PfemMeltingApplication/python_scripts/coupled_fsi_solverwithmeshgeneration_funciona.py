...
class PfemCoupledFluidThermalSolver(PythonSolver):
    ...
    def new(self):
        ...
        if(self.step==self.fluid_solver.main_model_part.ProcessInfo[KratosMultiphysics.STEP]):
            ...
            for node in self.fluid_solver.main_model_part.Nodes:
                #node.SetSolutionStepValue(KratosMultiphysics.IS_INTERFACE,0, 1.0) #SOLID
                node.SetSolutionStepValue(KratosMultiphysics.IS_INTERFACE,0, 0.0) #FLUID                
            for elem in self.fluid_solver.main_model_part.Elements:
                #solid_model_part.AddElement(elem,0) #SOLID
                fluid_model_part.AddElement(elem,0) #FLUID
        ...
        if(self.domain_size==3):
            ...
            new_elem_name= "QFLUID2D" #"LagrangianFluidVMS3D"#"HYPO3D"
    ...
    def ReMesh(self):
        ...
        if(self.domain_size ==3):
            (self.Mesher).ReGenerateMesh("QFLUID3D","ThermalFace3D3N", self.fluid_solver.main_model_part, self.node_erase_process, True, False, 1.2, 0.01)  #1.8
        else:
            (self.Mesher).ReGenerateMesh("QFLUID2D","ThermalFace2D2N", self.fluid_solver.main_model_part, self.node_erase_process, True, False, 1.2, 0.01)  #1.8
