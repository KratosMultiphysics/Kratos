...
class PfemCoupledFluidThermalSolver(PythonSolver):
    ...
    def PrepareModelPart(self):
        ...
        if(Viga==True):
            for node in self.fluid_solver.main_model_part.Nodes:   
                ...
                node.SetSolutionStepValue(KratosMultiphysics.TEMPERATURE,0, 298.0)
                if(node.X<0.000001):
    ...
    def SolveSolutionStep(self):
        ...
        print("INICIO PASO DE TIEMPO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("________________________________________________________")
        print("VELOCIDADESSSSSSSSSSSSSSSSSSS")
        
        #for node in self.fluid_solver.main_model_part.Nodes:
        #    print(node.GetSolutionStepValue(KratosMultiphysics.VELOCITY_X))
        #    print(node.GetSolutionStepValue(KratosMultiphysics.VELOCITY_Y))
        ...
        if(self.fluid_solver.main_model_part.ProcessInfo[KratosMultiphysics.STEP]<6):
            #print("aquiiiiiiiiii")
            #print(self.fluid_solver.main_model_part.ProcessInfo[KratosMultiphysics.STEP])
        ...
        for node in self.fluid_solver.main_model_part.Nodes:
            ...
            if(T>0.0001):      
                ...
                self.outputfile4.write(str(self.fluid_solver.main_model_part.ProcessInfo[KratosMultiphysics.TIME])+"; "+ str(node.X)+"; "+ str(node.Y)+ "; "+ str(vel_x)+"; "+ str(vel_y)+ "\n")
