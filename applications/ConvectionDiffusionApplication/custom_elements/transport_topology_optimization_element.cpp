...
const double crRHS4 = N[0]*t_adj[0] + N[1]*t_adj[1] + N[2]*t_adj[2];
const double crRHS5 = crRHS3*crRHS4;
const double crRHS6 = N[0]*t[0] + N[1]*t[1] + N[2]*t[2];
const double crRHS7 = 2.0*crRHS3*crRHS6*functional_weights[7];
const double crRHS8 = D[0]*N[0] + D[1]*N[1] + D[2]*N[2];
const double crRHS9 = DN(0,0)*t_adj[0] + DN(1,0)*t_adj[1] + DN(2,0)*t_adj[2];
const double crRHS10 = DN(0,1)*t_adj[0] + DN(1,1)*t_adj[1] + DN(2,1)*t_adj[2];
const double crRHS11 = DN(0,0)*t[0] + DN(1,0)*t[1] + DN(2,0)*t[2];
const double crRHS12 = DN(0,1)*t[0] + DN(1,1)*t[1] + DN(2,1)*t[2];
const double crRHS13 = 2.0*crRHS8*functional_weights[5];
const double crRHS14 = N[0]*vconv_adj(0,0) + N[1]*vconv_adj(1,0) + N[2]*vconv_adj(2,0);
const double crRHS15 = DN(0,0)*crRHS14;
const double crRHS16 = N[0]*vconv_adj(0,1) + N[1]*vconv_adj(1,1) + N[2]*vconv_adj(2,1);
const double crRHS17 = DN(0,1)*crRHS16;
const double crRHS18 = N[0]*c[0] + N[1]*c[1] + N[2]*c[2];
const double crRHS19 = crRHS18*crRHS4;
const double crRHS20 = N[0]*vconv(0,0) + N[1]*vconv(1,0) + N[2]*vconv(2,0);
const double crRHS21 = N[0]*vconv(0,1) + N[1]*vconv(1,1) + N[2]*vconv(2,1);
const double crRHS22 = crRHS11*crRHS20 + crRHS12*crRHS21;
const double crRHS23 = crRHS18*functional_weights[6];
const double crRHS24 = 1.0*(crRHS0 - crRHS1 + crRHS10*crRHS16*crRHS18 + crRHS14*crRHS18*crRHS9 + crRHS2 - crRHS5 + crRHS6*functional_weights[6]*(crRHS20*(DN(0,0)*c[0] + DN(1,0)*c[1] + DN(2,0)*c[2]) + crRHS21*(DN(0,1)*c[0] + DN(1,1)*c[1] + DN(2,1)*c[2])) - crRHS7)/(crRHS3*stab_c3 + crRHS8*stab_c1/pow(h, 2) + stab_c2*sqrt(pow(crRHS14, 2) + pow(crRHS16, 2))*fabs(crRHS18)/h);
const double crRHS25 = crRHS24*crRHS3;
const double crRHS26 = crRHS18*crRHS24;
const double crRHS27 = DN(1,0)*crRHS14;
const double crRHS28 = DN(1,1)*crRHS16;
const double crRHS29 = DN(2,0)*crRHS14;
const double crRHS30 = DN(2,1)*crRHS16;
rRHS[0]+=-gauss_weight*(-N[0]*crRHS0 + N[0]*crRHS1 - N[0]*crRHS2 + N[0]*crRHS25 + N[0]*crRHS5 + N[0]*crRHS7 + crRHS13*(DN(0,0)*crRHS11 + DN(0,1)*crRHS12) + crRHS15*crRHS26 + crRHS17*crRHS26 + crRHS19*(crRHS15 + crRHS17) + crRHS23*(N[0]*crRHS22 + crRHS6*(DN(0,0)*crRHS20 + DN(0,1)*crRHS21)) + crRHS8*(DN(0,0)*crRHS9 + DN(0,1)*crRHS10));
rRHS[1]+=-gauss_weight*(-N[1]*crRHS0 + N[1]*crRHS1 - N[1]*crRHS2 + N[1]*crRHS25 + N[1]*crRHS5 + N[1]*crRHS7 + crRHS13*(DN(1,0)*crRHS11 + DN(1,1)*crRHS12) + crRHS19*(crRHS27 + crRHS28) + crRHS23*(N[1]*crRHS22 + crRHS6*(DN(1,0)*crRHS20 + DN(1,1)*crRHS21)) + crRHS26*crRHS27 + crRHS26*crRHS28 + crRHS8*(DN(1,0)*crRHS9 + DN(1,1)*crRHS10));
rRHS[2]+=-gauss_weight*(-N[2]*crRHS0 + N[2]*crRHS1 - N[2]*crRHS2 + N[2]*crRHS25 + N[2]*crRHS5 + N[2]*crRHS7 + crRHS13*(DN(2,0)*crRHS11 + DN(2,1)*crRHS12) + crRHS19*(crRHS29 + crRHS30) + crRHS23*(N[2]*crRHS22 + crRHS6*(DN(2,0)*crRHS20 + DN(2,1)*crRHS21)) + crRHS26*crRHS29 + crRHS26*crRHS30 + crRHS8*(DN(2,0)*crRHS9 + DN(2,1)*crRHS10));
...
const double crRHS4 = N[0]*t_adj[0] + N[1]*t_adj[1] + N[2]*t_adj[2] + N[3]*t_adj[3];
const double crRHS5 = crRHS3*crRHS4;
const double crRHS6 = N[0]*t[0] + N[1]*t[1] + N[2]*t[2] + N[3]*t[3];
const double crRHS7 = 2.0*crRHS3*crRHS6*functional_weights[7];
const double crRHS8 = D[0]*N[0] + D[1]*N[1] + D[2]*N[2] + D[3]*N[3];
const double crRHS9 = DN(0,0)*t_adj[0] + DN(1,0)*t_adj[1] + DN(2,0)*t_adj[2] + DN(3,0)*t_adj[3];
const double crRHS10 = DN(0,1)*t_adj[0] + DN(1,1)*t_adj[1] + DN(2,1)*t_adj[2] + DN(3,1)*t_adj[3];
...
const double crRHS17 = DN(0,1)*crRHS16;
const double crRHS18 = N[0]*c[0] + N[1]*c[1] + N[2]*c[2] + N[3]*c[3];
const double crRHS19 = crRHS18*crRHS4;
const double crRHS20 = N[0]*vconv(0,0) + N[1]*vconv(1,0) + N[2]*vconv(2,0) + N[3]*vconv(3,0);
const double crRHS21 = N[0]*vconv(0,1) + N[1]*vconv(1,1) + N[2]*vconv(2,1) + N[3]*vconv(3,1);
const double crRHS22 = crRHS11*crRHS20 + crRHS12*crRHS21;
const double crRHS23 = crRHS18*functional_weights[6];
const double crRHS24 = 1.0*(crRHS0 - crRHS1 + crRHS10*crRHS16*crRHS18 + crRHS14*crRHS18*crRHS9 + crRHS2 - crRHS5 + crRHS6*functional_weights[6]*(crRHS20*(DN(0,0)*c[0] + DN(1,0)*c[1] + DN(2,0)*c[2] + DN(3,0)*c[3]) + crRHS21*(DN(0,1)*c[0] + DN(1,1)*c[1] + DN(2,1)*c[2] + DN(3,1)*c[3])) - crRHS7)/(crRHS3*stab_c3 + crRHS8*stab_c1/pow(h, 2) + stab_c2*sqrt(pow(crRHS14, 2) + pow(crRHS16, 2))*fabs(crRHS18)/h);
const double crRHS25 = crRHS24*crRHS3;
const double crRHS26 = crRHS18*crRHS24;
const double crRHS27 = DN(1,0)*crRHS14;
const double crRHS28 = DN(1,1)*crRHS16;
const double crRHS29 = DN(2,0)*crRHS14;
const double crRHS30 = DN(2,1)*crRHS16;
const double crRHS31 = DN(3,0)*crRHS14;
const double crRHS32 = DN(3,1)*crRHS16;
rRHS[0]+=-gauss_weight*(-N[0]*crRHS0 + N[0]*crRHS1 - N[0]*crRHS2 + N[0]*crRHS25 + N[0]*crRHS5 + N[0]*crRHS7 + crRHS13*(DN(0,0)*crRHS11 + DN(0,1)*crRHS12) + crRHS15*crRHS26 + crRHS17*crRHS26 + crRHS19*(crRHS15 + crRHS17) + crRHS23*(N[0]*crRHS22 + crRHS6*(DN(0,0)*crRHS20 + DN(0,1)*crRHS21)) + crRHS8*(DN(0,0)*crRHS9 + DN(0,1)*crRHS10));
rRHS[1]+=-gauss_weight*(-N[1]*crRHS0 + N[1]*crRHS1 - N[1]*crRHS2 + N[1]*crRHS25 + N[1]*crRHS5 + N[1]*crRHS7 + crRHS13*(DN(1,0)*crRHS11 + DN(1,1)*crRHS12) + crRHS19*(crRHS27 + crRHS28) + crRHS23*(N[1]*crRHS22 + crRHS6*(DN(1,0)*crRHS20 + DN(1,1)*crRHS21)) + crRHS26*crRHS27 + crRHS26*crRHS28 + crRHS8*(DN(1,0)*crRHS9 + DN(1,1)*crRHS10));
rRHS[2]+=-gauss_weight*(-N[2]*crRHS0 + N[2]*crRHS1 - N[2]*crRHS2 + N[2]*crRHS25 + N[2]*crRHS5 + N[2]*crRHS7 + crRHS13*(DN(2,0)*crRHS11 + DN(2,1)*crRHS12) + crRHS19*(crRHS29 + crRHS30) + crRHS23*(N[2]*crRHS22 + crRHS6*(DN(2,0)*crRHS20 + DN(2,1)*crRHS21)) + crRHS26*crRHS29 + crRHS26*crRHS30 + crRHS8*(DN(2,0)*crRHS9 + DN(2,1)*crRHS10));
rRHS[3]+=-gauss_weight*(-N[3]*crRHS0 + N[3]*crRHS1 - N[3]*crRHS2 + N[3]*crRHS25 + N[3]*crRHS5 + N[3]*crRHS7 + crRHS13*(DN(3,0)*crRHS11 + DN(3,1)*crRHS12) + crRHS19*(crRHS31 + crRHS32) + crRHS23*(N[3]*crRHS22 + crRHS6*(DN(3,0)*crRHS20 + DN(3,1)*crRHS21)) + crRHS26*crRHS31 + crRHS26*crRHS32 + crRHS8*(DN(3,0)*crRHS9 + DN(3,1)*crRHS10));
...
const double crRHS9 = DN(0,0)*t_adj[0] + DN(1,0)*t_adj[1] + DN(2,0)*t_adj[2] + DN(3,0)*t_adj[3];
const double crRHS10 = DN(0,1)*t_adj[0] + DN(1,1)*t_adj[1] + DN(2,1)*t_adj[2] + DN(3,1)*t_adj[3];
const double crRHS11 = DN(0,2)*t_adj[0] + DN(1,2)*t_adj[1] + DN(2,2)*t_adj[2] + DN(3,2)*t_adj[3];
...
const double crRHS14 = DN(0,2)*t[0] + DN(1,2)*t[1] + DN(2,2)*t[2] + DN(3,2)*t[3];
const double crRHS15 = 2.0*crRHS8*functional_weights[5];
const double crRHS16 = N[0]*vconv_adj(0,0) + N[1]*vconv_adj(1,0) + N[2]*vconv_adj(2,0) + N[3]*vconv_adj(3,0);
const double crRHS17 = DN(0,0)*crRHS16;
const double crRHS18 = N[0]*vconv_adj(0,1) + N[1]*vconv_adj(1,1) + N[2]*vconv_adj(2,1) + N[3]*vconv_adj(3,1);
const double crRHS19 = DN(0,1)*crRHS18;
const double crRHS20 = N[0]*vconv_adj(0,2) + N[1]*vconv_adj(1,2) + N[2]*vconv_adj(2,2) + N[3]*vconv_adj(3,2);
const double crRHS21 = DN(0,2)*crRHS20;
const double crRHS22 = N[0]*c[0] + N[1]*c[1] + N[2]*c[2] + N[3]*c[3];
const double crRHS23 = crRHS22*crRHS4;
const double crRHS24 = N[0]*vconv(0,0) + N[1]*vconv(1,0) + N[2]*vconv(2,0) + N[3]*vconv(3,0);
const double crRHS25 = N[0]*vconv(0,1) + N[1]*vconv(1,1) + N[2]*vconv(2,1) + N[3]*vconv(3,1);
const double crRHS26 = N[0]*vconv(0,2) + N[1]*vconv(1,2) + N[2]*vconv(2,2) + N[3]*vconv(3,2);
const double crRHS27 = crRHS12*crRHS24 + crRHS13*crRHS25 + crRHS14*crRHS26;
const double crRHS28 = crRHS22*functional_weights[6];
const double crRHS29 = 1.0*(crRHS0 - crRHS1 + crRHS10*crRHS18*crRHS22 + crRHS11*crRHS20*crRHS22 + crRHS16*crRHS22*crRHS9 + crRHS2 - crRHS5 + crRHS6*functional_weights[6]*(crRHS24*(DN(0,0)*c[0] + DN(1,0)*c[1] + DN(2,0)*c[2] + DN(3,0)*c[3]) + crRHS25*(DN(0,1)*c[0] + DN(1,1)*c[1] + DN(2,1)*c[2] + DN(3,1)*c[3]) + crRHS26*(DN(0,2)*c[0] + DN(1,2)*c[1] + DN(2,2)*c[2] + DN(3,2)*c[3])) - crRHS7)/(crRHS3*stab_c3 + crRHS8*stab_c1/pow(h, 2) + stab_c2*sqrt(pow(crRHS16, 2) + pow(crRHS18, 2) + pow(crRHS20, 2))*fabs(crRHS22)/h);
const double crRHS30 = crRHS29*crRHS3;
const double crRHS31 = crRHS22*crRHS29;
const double crRHS32 = DN(1,0)*crRHS16;
const double crRHS33 = DN(1,1)*crRHS18;
const double crRHS34 = DN(1,2)*crRHS20;
const double crRHS35 = DN(2,0)*crRHS16;
const double crRHS36 = DN(2,1)*crRHS18;
const double crRHS37 = DN(2,2)*crRHS20;
const double crRHS38 = DN(3,0)*crRHS16;
const double crRHS39 = DN(3,1)*crRHS18;
const double crRHS40 = DN(3,2)*crRHS20;
rRHS[0]+=-gauss_weight*(-N[0]*crRHS0 + N[0]*crRHS1 - N[0]*crRHS2 + N[0]*crRHS30 + N[0]*crRHS5 + N[0]*crRHS7 + crRHS15*(DN(0,0)*crRHS12 + DN(0,1)*crRHS13 + DN(0,2)*crRHS14) + crRHS17*crRHS31 + crRHS19*crRHS31 + crRHS21*crRHS31 + crRHS23*(crRHS17 + crRHS19 + crRHS21) + crRHS28*(N[0]*crRHS27 + crRHS6*(DN(0,0)*crRHS24 + DN(0,1)*crRHS25 + DN(0,2)*crRHS26)) + crRHS8*(DN(0,0)*crRHS9 + DN(0,1)*crRHS10 + DN(0,2)*crRHS11));
rRHS[1]+=-gauss_weight*(-N[1]*crRHS0 + N[1]*crRHS1 - N[1]*crRHS2 + N[1]*crRHS30 + N[1]*crRHS5 + N[1]*crRHS7 + crRHS15*(DN(1,0)*crRHS12 + DN(1,1)*crRHS13 + DN(1,2)*crRHS14) + crRHS23*(crRHS32 + crRHS33 + crRHS34) + crRHS28*(N[1]*crRHS27 + crRHS6*(DN(1,0)*crRHS24 + DN(1,1)*crRHS25 + DN(1,2)*crRHS26)) + crRHS31*crRHS32 + crRHS31*crRHS33 + crRHS31*crRHS34 + crRHS8*(DN(1,0)*crRHS9 + DN(1,1)*crRHS10 + DN(1,2)*crRHS11));
rRHS[2]+=-gauss_weight*(-N[2]*crRHS0 + N[2]*crRHS1 - N[2]*crRHS2 + N[2]*crRHS30 + N[2]*crRHS5 + N[2]*crRHS7 + crRHS15*(DN(2,0)*crRHS12 + DN(2,1)*crRHS13 + DN(2,2)*crRHS14) + crRHS23*(crRHS35 + crRHS36 + crRHS37) + crRHS28*(N[2]*crRHS27 + crRHS6*(DN(2,0)*crRHS24 + DN(2,1)*crRHS25 + DN(2,2)*crRHS26)) + crRHS31*crRHS35 + crRHS31*crRHS36 + crRHS31*crRHS37 + crRHS8*(DN(2,0)*crRHS9 + DN(2,1)*crRHS10 + DN(2,2)*crRHS11));
rRHS[3]+=-gauss_weight*(-N[3]*crRHS0 + N[3]*crRHS1 - N[3]*crRHS2 + N[3]*crRHS30 + N[3]*crRHS5 + N[3]*crRHS7 + crRHS15*(DN(3,0)*crRHS12 + DN(3,1)*crRHS13 + DN(3,2)*crRHS14) + crRHS23*(crRHS38 + crRHS39 + crRHS40) + crRHS28*(N[3]*crRHS27 + crRHS6*(DN(3,0)*crRHS24 + DN(3,1)*crRHS25 + DN(3,2)*crRHS26)) + crRHS31*crRHS38 + crRHS31*crRHS39 + crRHS31*crRHS40 + crRHS8*(DN(3,0)*crRHS9 + DN(3,1)*crRHS10 + DN(3,2)*crRHS11));
