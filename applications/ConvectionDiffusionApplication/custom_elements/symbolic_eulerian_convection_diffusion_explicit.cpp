...
const double crhs9 =             N[0]*prj[0] + N[1]*prj[1] + N[2]*prj[2];
const double crhs10 =             crhs1*crhs6 + crhs3*crhs7;
const double crhs11 =             (N[0]*(phi[0] - phi_old[0]) + N[1]*(phi[1] - phi_old[1]) + N[2]*(phi[2] - phi_old[2]))/(RK_time_coefficient*delta_time);
const double crhs12 =             tau*(DN(1,0)*crhs6 + DN(1,1)*crhs7);
const double crhs13 =             tau*(DN(2,0)*crhs6 + DN(2,1)*crhs7);
...
const double crhs12 =             N[0]*prj[0] + N[1]*prj[1] + N[2]*prj[2] + N[3]*prj[3];
const double crhs13 =             (N[0]*(phi[0] - phi_old[0]) + N[1]*(phi[1] - phi_old[1]) + N[2]*(phi[2] - phi_old[2]) + N[3]*(phi[3] - phi_old[3]))/(RK_time_coefficient*delta_time);
const double crhs14 =             crhs1*crhs8 + crhs10*crhs5 + crhs3*crhs9;
const double crhs15 =             tau*(DN(1,0)*crhs8 + DN(1,1)*crhs9 + DN(1,2)*crhs10);
const double crhs16 =             tau*(DN(2,0)*crhs8 + DN(2,1)*crhs9 + DN(2,2)*crhs10);
const double crhs17 =             tau*(DN(3,0)*crhs8 + DN(3,1)*crhs9 + DN(3,2)*crhs10);
            rhs[0]=-DN(0,0)*crhs2 - DN(0,1)*crhs4 - DN(0,2)*crhs6 + N[0]*crhs0 - N[0]*crhs14 - N[0]*crhs7 + crhs0*crhs11 + crhs11*crhs12 - crhs11*crhs13 - crhs11*crhs14 - crhs11*crhs7;
            rhs[1]=-DN(1,0)*crhs2 - DN(1,1)*crhs4 - DN(1,2)*crhs6 + N[1]*crhs0 - N[1]*crhs14 - N[1]*crhs7 + crhs0*crhs15 + crhs12*crhs15 - crhs13*crhs15 - crhs14*crhs15 - crhs15*crhs7;
            rhs[2]=-DN(2,0)*crhs2 - DN(2,1)*crhs4 - DN(2,2)*crhs6 + N[2]*crhs0 - N[2]*crhs14 - N[2]*crhs7 + crhs0*crhs16 + crhs12*crhs16 - crhs13*crhs16 - crhs14*crhs16 - crhs16*crhs7;
            rhs[3]=-DN(3,0)*crhs2 - DN(3,1)*crhs4 - DN(3,2)*crhs6 + N[3]*crhs0 - N[3]*crhs14 - N[3]*crhs7 + crhs0*crhs17 + crhs12*crhs17 - crhs13*crhs17 - crhs14*crhs17 - crhs17*crhs7;
