...
namespace Kratos
{
    ...
    )
    {
        ...
        KRATOS_WATCH('support penalty condition')
        exit(0) ;
        // Useless for convection-diffusion?
        ...
        for (IndexType point_number = 0; point_number < integration_points.size(); ++point_number)
        {
            ...
            //FOR DISPLACEMENTS
            Matrix H = ZeroMatrix(1, mat_size);
            for (IndexType i = 0; i < number_of_nodes; ++i)
            {
                IndexType index = 1 * i;  // 3 * i;
            ...
            if (CalculateResidualVectorFlag) {
                ...
                const array_1d<double, 1>& temperature = Has(TEMPERATURE)
                ...
                for (IndexType i = 0; i < number_of_nodes; ++i)
                {
                    const array_1d<double, 1> disp = r_geometry[i].FastGetSolutionStepValue(TEMPERATURE);
                    IndexType index = 1 * i;
                    u[index] = (disp[0] - temperature[0]);
                    // u[index + 1] = (disp[1] - temperature[1]);
                    // u[index + 2] = (disp[2] - temperature[2]);
                }
    ...
    ) const
    {
        ...
        if (rResult.size() != 1 * number_of_nodes)
            rResult.resize(1 * number_of_nodes, false);

        for (IndexType i = 0; i < number_of_nodes; ++i) {
            const IndexType index = i * 1;
            const auto& r_node = r_geometry[i];
            rResult[index] = r_node.GetDof(TEMPERATURE).EquationId();
    ...
    ) const
    {
        ...
        rElementalDofList.reserve(1 * number_of_nodes);
