...
//                    const double new_step_factor = (1.0 - old_step_factor);
        ...
        void CalculateAccelerationOnTheMeshFirstOrder(ModelPart& rModelPart)
        {
         KRATOS_TRY
        ...
        //   R -= prod(Mc,a);    
           ...
           for (unsigned int i = 0; i < (TDim + 1); i++)
             {
              geom[i].SetLock();   
              geom[i].FastGetSolutionStepValue(ACCELERATION_AUX_X) += R[i*TDim] / geom[i].FastGetSolutionStepValue(LUMPED_MASS_VALUE);
              geom[i].FastGetSolutionStepValue(ACCELERATION_AUX_Y) += R[i*TDim+1] / geom[i].FastGetSolutionStepValue(LUMPED_MASS_VALUE);
              geom[i].FastGetSolutionStepValue(ACCELERATION_AUX_Z) += 0.0;
              geom[i].UnSetLock();
             }
         }
        }
