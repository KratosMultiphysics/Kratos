...
#if !defined(KRATOS_BFECC_CONVECTION_RK_INCLUDED )
#define  KRATOS_BFECC_CONVECTION_RK_INCLUDED
...
class BFECCConvectionRK
{
public:
    KRATOS_CLASS_POINTER_DEFINITION(BFECCConvectionRK<TDim>);
        ...
        const double substeps)
    {
        ...
        for (int i = 0; i < nparticles; i++)
        {
            ...
            if (is_found!=true)
            {
             bckPos = it_particle->Coordinates();
             has_valid_elem_pointer = false;    
...
//             is_found2 = mpSearchStructure->FindPointOnMesh(k2_aux, N, pelement_aux, result_begin, max_results);
            ...
            if (is_found4 == true)
            {
             const double new_step_factor = 1.0;
             const double old_step_factor = (1.0 - new_step_factor);   
             Geometry< Node < 3 > >& geom = pelement_aux->GetGeometry(); 
             noalias(veulerian) = N[0] * ( new_step_factor*geom[0].FastGetSolutionStepValue(conv_var) + old_step_factor*geom[0].FastGetSolutionStepValue(conv_var,1));
             for (unsigned int k = 1; k < geom.size(); k++)
        ...
        else //going to the future
        {
            ...
            if (is_found4 == true)
            {
             const double old_step_factor = 1.0;
             const double new_step_factor = (1.0 - old_step_factor);      
             Geometry< Node < 3 > >& geom = pelement_aux->GetGeometry(); 
             noalias(veulerian) = N[0] * ( new_step_factor*geom[0].FastGetSolutionStepValue(conv_var) + old_step_factor*geom[0].FastGetSolutionStepValue(conv_var,1));
             for (unsigned int k = 1; k < geom.size(); k++)
...
#endif // KRATOS_BFECC_CONVECTION_RK_INCLUDED  defined