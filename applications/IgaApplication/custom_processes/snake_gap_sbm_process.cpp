    ...
    const ModelPart& rSurrogateSubModelPart)
{
    ...
    for (std::size_t j = 0; j < size_surrogate_loop; ++j) {
        ...
        auto select_surrogate_by_normal = [&](const Node::Pointer& rBaseNode,
                                              const array_1d<double,3>& rNormal) {
            if (!rBaseNode) {
                return Node::Pointer();
            }

            array_1d<int, 3> expected;
            expected[0] = (std::abs(rNormal[0]) < 1.0e-14) ? -1 : (rNormal[0] >= 0.0 ? 1 : 0);
            expected[1] = (std::abs(rNormal[1]) < 1.0e-14) ? -1 : (rNormal[1] >= 0.0 ? 1 : 0);
            expected[2] = -1;

            auto matches_expected = [&](const array_1d<int, 3>& rEntering) {
                for (std::size_t idx = 0; idx < 3; ++idx) {
                    if (rEntering[idx] == 2 || rEntering[idx] == -1 || expected[idx] == -1) {
                        continue;
                    }
                    if (rEntering[idx] != expected[idx]) {
                        return false;
                    }
                }
                return true;
            ...
            if (rBaseNode->Has(SURROGATE_ENTERING_XYZ)) {
                if (matches_expected(rBaseNode->GetValue(SURROGATE_ENTERING_XYZ))) {
                    return rBaseNode;
                }
            }

            if (rBaseNode->Has(SURROGATE_REPEATED_VERTICES_ID)) {
                const auto& repeated_ids = rBaseNode->GetValue(SURROGATE_REPEATED_VERTICES_ID);
                for (const auto repeated_id : repeated_ids) {
                    if (!rSurrogateSubModelPart.HasNode(repeated_id)) {
                        continue;
                    }
                    auto p_repeated = rSurrogateSubModelPart.pGetNode(repeated_id);
                    if (!p_repeated->Has(SURROGATE_ENTERING_XYZ)) {
                        continue;
                    }
                    if (matches_expected(p_repeated->GetValue(SURROGATE_ENTERING_XYZ))) {
                        return p_repeated;
                    }
                }
            }
        ...
        if (it_surrogate_node_1 != surrogate_node_lookup.end()) {
            p_surrogate_node_1 = select_surrogate_by_normal(it_surrogate_node_1->second, normal_cond);
        ...
        if (it_surrogate_node_2 != surrogate_node_lookup.end()) {
            p_surrogate_node_2 = select_surrogate_by_normal(it_surrogate_node_2->second, normal_cond);
        ...
        if (!p_surrogate_node_1 || !p_surrogate_node_2) {
            for (auto& r_surrogate_node : rSurrogateSubModelPart.Nodes())
            {
                ...
                {
                    p_surrogate_node_1 = select_surrogate_by_normal(&r_surrogate_node, normal_cond);
                ...
                {
                    p_surrogate_node_2 = select_surrogate_by_normal(&r_surrogate_node, normal_cond);
                    auto& r_neighbour_geometries =
        ...
        //         create_projected_clone(p_surrogate_node_1, projection_chain[0]),
        //         create_projected_clone(p_surrogate_node_2, projection_chain[1])});
        //     for (std::size_t idx = 1; idx < pair_count; ++idx) {
        //         projection_pairs.push_back({
        //             create_projected_clone(p_surrogate_node_2, projection_chain[idx]),
        //             create_projected_clone(p_surrogate_node_2, projection_chain[idx + 1])});
        //     }
        // }
        ...
        auto process_pair = [&](const Node::Pointer& rp_surrogate_node_1, const Node::Pointer& rp_surrogate_node_2) {
            ...
            if (dot1_base < -1.0e-12) {
                ...
                //     p_nurbs_surface,
                //     p_first_for_fallback,
                //     p_fake_surrogate_node,
                //     surrogate_brep_middle_geometry,
                //     *mpIgaModelPart,
                //     rSkinSubModelPart);

                // p_first_for_fallback = p_fake_surrogate_node;
            }
            if (dot2_base < -1.0e-12) {
                ...
                //     p_nurbs_surface,
                //     p_fake_surrogate_node,
                //     p_second_for_fallback,
                //     surrogate_brep_middle_geometry,
                //     *mpIgaModelPart,
                //     rSkinSubModelPart);

                // p_second_for_fallback = p_fake_surrogate_node;
            } 

            surrogate_segment_nodes.push_back(p_second_for_fallback);

            // std::vector<Node::Pointer> surrogate_segment_nodes{p_first_for_fallback, p_second_for_fallback};
    ...
    const ModelPart& rSkinSubModelPart)
{
    ...
    const double subdivision_threshold = rIntegrationParameters.rKnotSpanSizes[0] / 1000.0; //FIXME:
    ...
    auto build_skin_curve = [&](IndexType left_id,
        ...
        {
            std::size_t number_interpolation_cuts = 1;
        ...
        AppendPolynomialCurveInfo<TIsInnerLoop>(p_nurbs_curve_skin, left_id, right_id);
    ...
    auto create_skin_conditions = [&](const BrepCurveType::Pointer& p_brep_curve_skin,
        ...
        rIntegrationParameters.CurveIntegrationInfo.SetNumberOfIntegrationPointsPerSpan(0, ((mGapInterpolationOrder+1)*(mGapInterpolationOrder+1)));
        ...
        for (auto& quad_geom: brep_quadrature_point_list_skin)
        {
            ...
            // const IndexType closest_node_id = FindClosestNodeInLayer(
            //     quadrature_point_coords,
            //     layer_name,
            //     rSkinSubModelPart,
            //     rIntegrationParameters.rKnotSpanSizes,
            //     r_skin_conditions_per_span);
            // auto p_closest_skin_node = rSkinSubModelPart.pGetNode(closest_node_id);
            // array_1d<double,3> diff_closest = p_closest_skin_node->Coordinates();
            // diff_closest -= quadrature_point_coords;
            // const double distance_closest_node = norm_2(diff_closest);
            ...
            //FIXME: it was p_skin_node
            empty_vector.push_back(p_skin_node); // Just it_node-plane neighbours
            const double distance_created_node = norm_2(p_skin_node->Coordinates() - quadrature_point_coords);
            NodePointerVector neighbour_nodes;
            // if (distance_closest_node <= distance_created_node) {
            //     neighbour_nodes.push_back(p_closest_skin_node);
            // } else {
                neighbour_nodes.push_back(p_skin_node);
            // }
            quad_geom.SetValue(NEIGHBOUR_NODES, neighbour_nodes);

            // if (distance_created_node > 1e-7 && distance_closest_node > 1e-7)
            // {
            //     KRATOS_WATCH(distance_created_node)
            //     KRATOS_WATCH(distance_closest_node)
            //     KRATOS_WATCH(quadrature_point_coords)
            //     KRATOS_WATCH("---------------")
            // }
        }
    ...
    auto create_area_elements = [&](const BrepCurveType& rB0,
        ...
        IntegrationPointsArrayType surface_integration_points = CreateCoonsPatchGaussPoints(
            2*(mGapInterpolationOrder+1), /*Order*/
            rB0,
            rL0,
            rL1,
            rB1,
                             ...
                             std::size_t negative_count) -> void
    {
        ...
        // if (reached_max_divisions) {
            ...
            double new_piece_contribution_sign = 1.0;
            // if (positive_count + negative_count > 0) {
            //     new_piece_contribution_sign = (positive_count >= negative_count) ? 1.0 : -1.0;
            // } else {
                const CoordinatesArrayType mid_b0 = GlobalPoint(*p_brep_curve_linear, 0.5);
                const CoordinatesArrayType mid_b1 = GlobalPoint(*p_brep_curve_skin, 0.5);
                const array_1d<double,3> delta_mid = mid_b1 - mid_b0;
                new_piece_contribution_sign = (inner_prod(delta_mid, normal_skin_segment) >= 0.0) ? 1.0 : -1.0;
            // }

            if constexpr(!TIsInnerLoop)
            {
                new_piece_contribution_sign *=-1;
            } 

            auto p_nurbs_curve_l0 = this->CreateBrepCurve(p_left_skin, p_left_skin, active_range_knot_vector);
            auto p_nurbs_curve_l1 = this->CreateBrepCurve(p_right_skin, p_right_skin, active_range_knot_vector);
            ...
            // if (new_piece_contribution_sign < 0)
            // {
            //     KRATOS_WATCH(skin_left_coords)
            //     KRATOS_WATCH(skin_right_coords)
            //     KRATOS_WATCH("EDDAIE")
            //     KRATOS_WATCH("-------------")
            // }
            // else{
    ...
    const KnotSpanIdsCSR& rSkinConditionsPerSpan)
{
    ...
    auto gather_candidate_node_ids = [&](const Node& rSurrogateNode, const array_1d<double,3>& rNormal) {
        ...
        const auto x_indices = gather_indices_with_tolerance(rSurrogateNode.X()+ 0.1* span_size_x * rNormal[0], tol_x, min_u, max_u, span_size_x, span_count_x);
        const auto y_indices = gather_indices_with_tolerance(rSurrogateNode.Y()+ 0.1* span_size_y * rNormal[1], tol_y, min_v, max_v, span_size_y, span_count_y);
    ...
    for (auto& r_surrogate_condition : rSurrogateSubModelPart.Conditions())
    {
        ...
        auto compute_normal_from_node = [&](const Node::Pointer& rp_node) {
            array_1d<double,3> normal = ZeroVector(3);
            if (!rp_node || !rp_node->Has(SURROGATE_ENTERING_XYZ) || !rp_node->Has(SURROGATE_REPEATED_VERTICES_ID)) {
                return normal;
            }
            const auto entering_xyz = rp_node->GetValue(SURROGATE_ENTERING_XYZ);
            for (std::size_t idx = 0; idx < 2; ++idx) {
                if (entering_xyz[idx] == 0) {
                    normal[idx] -= 1.0;
                } else if (entering_xyz[idx] == 1) {
                    normal[idx] += 1.0;
                }
            }
        ...
        array_1d<double,3> normal_at_vertex_1 = compute_normal_from_node(p_surrogate_node_1);
        array_1d<double,3> normal_at_vertex_2 = compute_normal_from_node(p_surrogate_node_2);
        ...
        if (!has_proj_1) {
            const auto selection = select_candidate(p_surrogate_node_1, forced_layers, normal_at_vertex_1);
        ...
        if (!has_proj_2) {
            const auto selection = select_candidate(p_surrogate_node_2, forced_layers, normal_at_vertex_2);
            KRATOS_ERROR_IF(selection.Id == std::numeric_limits<IndexType>::max())
                << "::[SnakeGapSbmProcess]::SetSurrogateToSkinProjections: no skin node candidate found for surrogate node "
...
IndexType SnakeGapSbmProcess::FindClosestNodeInLayer(
    ...
    const KnotSpanIdsCSR& rSkinConditionsPerSpan) const
{
    KRATOS_ERROR_IF(rSkinConditionsPerSpan.NumberOfSpansX == 0 ||
                    rSkinConditionsPerSpan.NumberOfSpansY == 0)
        << "::[SnakeGapSbmProcess]::FindClosestNodeInLayer: condition span matrix is empty." << std::endl;
    ...
    const std::size_t number_spans_x = rSkinConditionsPerSpan.NumberOfSpansX;
    const std::size_t number_spans_y = rSkinConditionsPerSpan.NumberOfSpansY;
    const std::size_t base_ix = compute_index(rPoint[0], min_u, max_u, span_size_x, number_spans_x);
    const std::size_t base_iy = compute_index(rPoint[1], min_v, max_v, span_size_y, number_spans_y);

    const auto& S = rSkinConditionsPerSpan;
    constexpr std::size_t max_extension = 5;

    for (std::size_t extension = 1; extension <= max_extension; ++extension) {
        const std::size_t min_ix = (base_ix > extension) ? base_ix - extension : 0;
        const std::size_t max_ix = std::min<std::size_t>(base_ix + extension, number_spans_x > 0 ? number_spans_x - 1 : 0);
        const std::size_t min_iy = (base_iy > extension) ? base_iy - extension : 0;
        const std::size_t max_iy = std::min<std::size_t>(base_iy + extension, number_spans_y > 0 ? number_spans_y - 1 : 0);
        ...
        std::vector<IndexType> candidate_conditions;
        candidate_conditions.reserve(64);
        std::unordered_set<IndexType> seen_conditions;
        seen_conditions.reserve(128);
        ...
        for (std::size_t ix = min_ix; ix <= max_ix; ++ix) {
            ...
            for (std::size_t iy = min_iy; iy <= max_iy; ++iy) {
                ...
                for (std::size_t t = 0; t < ids.size; ++t) {
                    const IndexType cid = ids.data[t];
                    if (seen_conditions.insert(cid).second) {
                        candidate_conditions.push_back(cid);
                    }
                }
            }
        ...
        IndexType best_node_id = std::numeric_limits<IndexType>::max();
        double best_distance = std::numeric_limits<double>::max();
        std::unordered_set<IndexType> seen_nodes;
        seen_nodes.reserve(128);

        for (const IndexType condition_id : candidate_conditions) {
            const auto r_condition = rSkinSubModelPart.pGetCondition(condition_id);
            const auto p_geometry = r_condition->pGetGeometry();
            const std::size_t geometry_size = p_geometry->size();
            if (geometry_size == 0) {
                continue;
            }

            for (std::size_t i = 0; i < geometry_size; ++i) {
                const auto p_node = p_geometry->pGetPoint(i);
                if (!seen_nodes.insert(p_node->Id()).second) {
                    continue;
                }
                const auto& candidate_layers = p_node->GetValue(CONNECTED_LAYERS);
                if (std::find(candidate_layers.begin(), candidate_layers.end(), rLayer) == candidate_layers.end()) {
                    continue;
                }

                array_1d<double,3> diff = p_node->Coordinates();
                diff -= rPoint;
                const double distance = norm_2(diff);
                if (distance < best_distance) {
                    best_distance = distance;
                    best_node_id = p_node->Id();
                }
            }
        }
    ...
    KRATOS_ERROR << "::[SnakeGapSbmProcess]::FindClosestNodeInLayer: no node found for layer "
                 << rLayer << " starting from point " << rPoint
                 << " within search extension=" << max_extension
                 << " (base span (" << base_ix << "," << base_iy << "))." << std::endl;
    ...
    const Vector& rDirection) const
{
    ...
    auto update_candidate = [&](Node::Pointer p_node_a, Node::Pointer p_node_b, const CoordinatesArrayType& rIntersectionPoint){
        ...
        //     return;
        // } //FIXME: probably it's not necessary
        array_1d<double,3> diff_intersection = rIntersectionPoint;
        diff_intersection -= rStartPoint;
        const double distance_intersection = norm_2(diff_intersection);
        if (distance_intersection < best_intersection_distance) {
