    ...
    SnakeSbmProcess(rModel, ThisParameters)
{
    ...
    // "default" : consider BC as if they were exactly on the true boundary
    // "interpolation": interpolate BC values from the projection nodes on the skin
    // "sbm": use the SBM approach to impose BCs on the new approximated boundary //TODO:
    ...
    const ModelPart& rSurrogateSubModelPart) const
{
    ...
    auto compute_span_count = [](double spans_real) -> std::size_t {
    ...
    const std::size_t Nx = compute_span_count(spans_x_real);
    const std::size_t Ny = compute_span_count(spans_y_real);
    ...
    auto compute_index = [tol](double coordinate, double min_value, double max_value, double span_size, std::size_t span_count) {
        ...
        std::size_t idx = static_cast<std::size_t>(std::floor(relative + tol));
        if (idx >= span_count) idx = span_count - 1;
        return idx;
    };

    // --- Pass 1: gather unique columns per row for sparsity pattern ---
    std::vector<std::vector<std::size_t>> cols_per_row(Nx);
    ...
    for (const auto& r_node : rSkinSubModelPart.Nodes()) {
        ...
        const std::size_t ix = compute_index(u, min_u, max_u, span_size_x, Nx);
        const std::size_t iy = compute_index(v, min_v, max_v, span_size_y, Ny);
    ...
    for (auto& cols : cols_per_row) {
        ...
        nnz += static_cast<std::size_t>(cols.size());
    }
    ...
    for (std::size_t i = 0; i < Nx; ++i) {
        for (const std::size_t j : cols_per_row[i]) {
    ...
    for (const auto& r_node : rSkinSubModelPart.Nodes()) {
        const std::size_t ix = compute_index(r_node.X(), min_u, max_u, span_size_x, Nx);
        const std::size_t iy = compute_index(r_node.Y(), min_v, max_v, span_size_y, Ny);

        const std::size_t k = FindNnzIndex(A, ix, iy);
        KRATOS_DEBUG_ERROR_IF(k == static_cast<std::size_t>(-1))
    ...
    const SnakeGapSbmProcess::KnotSpanIdsCSR& rReferenceMatrix) const
{
    ...
    const std::size_t Nx = out.NumberOfSpansX, Ny = out.NumberOfSpansY;
    ...
    auto idx = [tol](double c, double mn, double mx, double h, std::size_t n) {
        double cl = c;
        if (c < mn) { KRATOS_ERROR_IF(c < mn - tol) << "coord < min\n"; cl = mn; }
        else if (c > mx) { KRATOS_ERROR_IF(c > mx + tol) << "coord > max\n"; cl = mx; }
        std::size_t k = static_cast<std::size_t>(std::floor((cl - mn) / h + tol));
        if (k >= n) k = n - 1;
        return k;
    };

    // Sparsity pattern
    std::vector<std::vector<std::size_t>> cols(Nx);
    for (const auto& r_cond : rSkinSubModelPart.Conditions()) {
        ...
        const std::size_t ix = idx(c[0], u0, u1, hx, Nx);
        const std::size_t iy = idx(c[1], v0, v1, hy, Ny);
    ...
    A.resize(Nx, Ny, false);
    A.reserve(nnz);
    for (std::size_t i = 0; i < Nx; ++i) for (std::size_t j : cols[i]) A.push_back(i, j, 0.0);
    ...
    for (const auto& r_cond : rSkinSubModelPart.Conditions()) {
        ...
        const std::size_t iy = idx(c[1], v0, v1, hy, Ny);
        const std::size_t k = FindNnzIndex(A, ix, iy);
        KRATOS_DEBUG_ERROR_IF(k == static_cast<std::size_t>(-1)) << "pattern miss\n";
    ...
    const ModelPart& rSurrogateSubModelPart)
{
    ...
    double searchRadius;
    if (domain_size == 2) {
        searchRadius = 2*std::sqrt(2.0) * knot_span_reference_size;
    } else {
    ...
    std::vector<double> listOfDistances(numberOfResults);

    BinSearchParameters binSearchParameters(
        testBins, 
        numberOfResults, 
        results, 
        listOfDistances, 
    ...
    for (std::size_t j = 0; j < size_surrogate_loop; ++j) {
        ...
        std::size_t n_skin_nodes = rSkinSubModelPart.NumberOfNodes();
        ...
        std::size_t current_internal_divisions = mInternalDivision;
        ...
        if (abs(projection_id_2 - projection_id_1) < mInternalDivision*10 || 
            ...
            const std::size_t subdivision_depth = current_internal_divisions;
    ...
    //     for (std::size_t d = 0; d < current_internal_divisions; ++d)
    //     {
    ...
    //         CreateGapAndSkinQuadraturePoints<TIsInnerLoop>(integration_parameters, binSearchParameters,
    ...
    for (IndexType i_cond_id = first_condition_id; i_cond_id <= last_condition_id; ++i_cond_id)
    {
        ...
        if (!is_first_surrogate_already_computed) {
            ...
            brep_integration_info_surrogate1_skin1.SetNumberOfIntegrationPointsPerSpan(0,2*mGapApproximationOrder+1);
            ...
            this->CreateConditions(
                ...
                *mpGapInterfaceSubModelPart, mGapInterfaceConditionName, id, PropertiesPointerType(), knot_span_sizes, neighbour_geometries);
        ...
        if (!is_second_surrogate_already_computed) {
            ...
            brep_integration_info_surrogate2_skin2.SetNumberOfIntegrationPointsPerSpan(0,2*mGapApproximationOrder+1);
    ...
    const ModelPart& rSkinSubModelPart)
{
    ...
    //     const std::size_t segment_count = number_interpolation_cuts == 0 ? 1 : static_cast<std::size_t>(1) << number_interpolation_cuts;
    ...
    rIntegrationParameters.CurveIntegrationInfo.SetNumberOfIntegrationPointsPerSpan(0, ((mGapApproximationOrder+1)*(mGapApproximationOrder+1)));
    ...
    for (auto& quad_geom: brep_quadrature_point_list_skin1_skin2)
    {
        ...
        IndexType id_skin_node = FindClosestNodeInLayerWithDirection(quadrature_point_coords, layer_name, rSkinSubModelPart, rIntegrationParameters.rKnotSpanSizes,
    ...
    if constexpr (TIsInnerLoop)  
    {
        ...
        IntegrationPointsArrayType surface_integration_points = CreateCoonsPatchGaussPoints(
            (mGapApproximationOrder+1)*2, /*Order*/
    ...
    this->CreateElements(
        ...
        *mpGapElementsSubModelPart, mGapElementName, id_element, PropertiesPointerType(), neighbour_geometries_skin1_skin2);
}


// bool SnakeGapSbmProcess::ProjectToSkinBoundary(
    ...
    const array_1d<double,3>&     rP11)
{
    ...
    // KRATOS_WATCH(brep_B0_at_xi)
    // KRATOS_WATCH(brep_B1_at_xi)
    // KRATOS_WATCH(brep_L0_at_eta)
    // KRATOS_WATCH(brep_L1_at_eta)
    

    // KRATOS_WATCH(dB0_at_xi)
    
    // KRATOS_WATCH(dB1_at_xi)
        
    // KRATOS_WATCH(dL0_at_eta)
        
    // KRATOS_WATCH(dL1_at_eta)
    ...
    auto gather_candidate_node_ids = [&](const Node& rSurrogateNode) {
        ...
        for (const std::size_t ix : x_indices) {
            ...
            for (const std::size_t iy : y_indices) {
                ...
                const std::size_t k = FindNnzIndex(A, ix, iy);
                if (k != static_cast<std::size_t>(-1)) append_ids_by_k(S, k);
            }
        }
...
// IndexType SnakeGapSbmProcess::FindClosestNodeInLayer( //FIXME: remove
...
//     // Perform the search
//     std::size_t obtained_results = rSearchParameters.rTestBins.SearchInRadius(
//         *rStartPoint,
...
//     KRATOS_ERROR_IF(rSkinConditionsPerSpan.NumberOfSpansX == 0 ||
//                     rSkinConditionsPerSpan.NumberOfSpansY == 0)
//         << "::[SnakeGapSbmProcess]::FindClosestNodeInLayerWithDirection: empty span matrix." << std::endl;
...
//     if (rKnotSpanSizes.size() > 0) {
//         ref_span = 0.0;
//         for (std::size_t i = 0; i < rKnotSpanSizes.size(); ++i) ref_span = std::max(ref_span, rKnotSpanSizes[i]);
...
//     const std::size_t NX = rSkinConditionsPerSpan.NumberOfSpansX;
//     const std::size_t NY = rSkinConditionsPerSpan.NumberOfSpansY;

//     // --- Helpers ---
//     auto clamp_val = [](double x, double a, double b){ return x < a ? a : (x > b ? b : x); };
//     auto compute_index = [&](double c, double cmin, double cmax, double span, std::size_t count){
//         if (count == 0) return std::size_t(0);
//         double clamped = clamp_val(c, cmin, cmax);
//         std::size_t idx = static_cast<std::size_t>(std::floor((clamped - cmin) / span + 1e-12));
//         if (idx >= count) idx = count - 1;
//         return idx;
//     };
//     auto in_bounds = [&](std::size_t i, std::size_t j){ return i < NX && j < NY; };

//     // --- Start cell and point ---
//     std::size_t ix = compute_index(rStartPoint[0], min_u, max_u, span_x, NX);
//     std::size_t iy = compute_index(rStartPoint[1], min_v, max_v, span_y, NY);
...
//     double traveled = 0.0;
//     std::size_t safety = (NX + NY) * 4 + 16; // conservative cap
...
//             std::size_t jx = compute_index(p_next[0], min_u, max_u, span_x, NX);
//             std::size_t jy = compute_index(p_next[1], min_v, max_v, span_y, NY);
...
//             std::size_t jx = compute_index(p[0], min_u, max_u, span_x, NX);
//             std::size_t jy = compute_index(p[1], min_v, max_v, span_y, NY);
//             ix = jx; iy = jy;
//         }
//     }

//     KRATOS_ERROR << "::[SnakeGapSbmProcess]::FindClosestNodeInLayerWithDirection: no intersection for layer "
//                  << rLayer << " from point " << rStartPoint << std::endl;
//     return 0;
// }


void SnakeGapSbmProcess::FindClosestTruePointToSurrogateVertexByNurbs()
{
    // // get the data
    // mEchoLevel = mThisParameters["echo_level"].GetInt();
    ...
    // ModelPart& rGapSbmSubModelPart = mpIgaModelPart->CreateSubModelPart("extended_sbm");
    ...
    // // Loop over the nodes of the surrogate sub model part
    // IndexType iel = 1;
    // std::size_t number_of_shape_functions_derivatives = 5;
    ...
    // std::size_t size_surrogate_loop = last_condition_id - first_condition_id + 1;
