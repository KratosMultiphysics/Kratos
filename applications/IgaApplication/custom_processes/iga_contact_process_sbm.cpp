...
namespace Kratos
{
    ...
    for (const auto& r_segment : master_segments) {
        ...
        for (IndexType i = 0; i < quadrature_geometries.size(); ++i) {
            ...
            // This geometric projection node is not inserted in a ModelPart,
            // yet it is one endpoint of the coupling condition.
            p_slave_skin_node->SetSolutionStepVariablesList(
                mrSlaveSkinModelPart->pGetNodalSolutionStepVariablesList());
            p_slave_skin_node->AddDof(DISPLACEMENT_X);
            p_slave_skin_node->AddDof(DISPLACEMENT_Y);
            ...
            // The quadrature geometries may reference skin-only control
            // points, which are not traversed by MechanicalSolver::AddDofs.
            // They enter the SBM coupling condition directly, therefore add
            // the displacement DOFs here as part of the condition setup.
            auto add_displacement_dofs = [](Geometry<Node>& rGeometry) {
                for (IndexType i = 0; i < rGeometry.PointsNumber(); ++i) {
                    auto& r_node = rGeometry[i];
                    if (!r_node.HasDofFor(DISPLACEMENT_X)) {
                        r_node.AddDof(DISPLACEMENT_X);
                    }
                    if (!r_node.HasDofFor(DISPLACEMENT_Y)) {
                        r_node.AddDof(DISPLACEMENT_Y);
                    }
                }
            };
            add_displacement_dofs(*p_gp);
            add_displacement_dofs(*surrogate_quadrature_list(0));
        ...
        double tolerance)
    {
        ...
        // Contact quadrature nodes are created after the mechanical solver has
        // added its DOFs.  They participate in the coupling geometry, hence
        // must receive the displacement DOFs explicitly.
        p_new_node->AddDof(DISPLACEMENT_X);
        p_new_node->AddDof(DISPLACEMENT_Y);
        ...
        const CoordinatesArrayType& rCoordinates)
    {
        ...
        if (mrMasterContactSkinModelPart != nullptr) {
            auto p_node = mrMasterContactSkinModelPart->CreateNewNode(
                new_node_id,
                rCoordinates[0],
                rCoordinates[1],
                rCoordinates[2]);
        ...
        auto p_node = NodeType::Pointer(new NodeType(
            ...
            rCoordinates[1],
            rCoordinates[2]));
        // This node is kept outside a ModelPart, so it does not inherit the
        // nodal variables list through CreateNewNode.  The contact condition
        // nevertheless contributes its displacement DOFs to the system.
        p_node->SetSolutionStepVariablesList(
            mrMasterSkinModelPart->pGetNodalSolutionStepVariablesList());
        p_node->AddDof(DISPLACEMENT_X);
        p_node->AddDof(DISPLACEMENT_Y);
