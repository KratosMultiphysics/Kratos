...
//         IndexType knot_span_surrounded_u_id = floor((1.39176- slave_parameter_space_extremes[0]+epsilon)/slave_knot_step_uv[0]);
//         IndexType knot_span_surrounded_v_id = floor((7.87801- slave_parameter_space_extremes[1]+epsilon)/slave_knot_step_uv[1]);
...
//                 // KRATOS_WATCH("SECOND RUN---------------------------------------------------")
//                 std::vector closest_brep_id = FindClosestBrepId(slave_skin_node_coords, *mrSlaveModelPart, mSparseBrepMatrixSlave);
//                 IndexType best_brep_id = -1;
//                 CoordinatesArrayType best_surrogate_projection = ZeroVector(3);
//                 for (IndexType brep_id: closest_brep_id)
//                 {
//                     // KRATOS_WATCH("*******************************************************")
//                     auto p_brep = mrSlaveModelPart->pGetGeometry(brep_id);
...
//                     // KRATOS_WATCH(first_vertex)
//                     // KRATOS_WATCH(second_vertex)
//                     // KRATOS_WATCH(brep_geometry->DomainInterval().GetT0())
//                     // KRATOS_WATCH(brep_geometry->DomainInterval().GetT1())
//                     // KRATOS_WATCH(slave_skin_node_coords)

//                     if (std::abs(first_vertex[0] - second_vertex[0]) < 1e-10)
//                     {
//                         // y aligned (x constant)
//                         double t_ray_tracing_intersection = (first_vertex[0] - slave_skin_node_coords[0])/ normal_vector[0];

//                         //compute y of the intersection
//                         double y_intersection = slave_skin_node_coords[1] + t_ray_tracing_intersection * normal_vector[1];

//                         // check if the intersection is inside the brep
//                         if (y_intersection > std::max(first_vertex[1], second_vertex[1]) || y_intersection < std::min(first_vertex[1], second_vertex[1]))
//                         {
//                             // the intersection is outside the brep
//                             continue;
//                         }
//                         brep_intersection_local_variable = y_intersection;

//                     }
//                     else if (std::abs(first_vertex[1] - second_vertex[1]) < 1e-10)
//                     {
//                         // x aligned (y constant)
//                         double t_ray_tracing_intersection = (first_vertex[1] - slave_skin_node_coords[1])/ normal_vector[1];

//                         //compute x of the intersection
//                         double x_intersection = slave_skin_node_coords[0] + t_ray_tracing_intersection * normal_vector[0];
//                         // check if the intersection is inside the brep
                        
//                         if (x_intersection > std::max(first_vertex[0], second_vertex[0]) || x_intersection < std::min(first_vertex[0], second_vertex[0]))
//                         {
//                             // the intersection is outside the brep
//                             continue;
//                         }
...
//                             brep_intersection_local_variable = first_vertex[0] - (x_intersection - second_vertex[0]);
//                         else
//                             brep_intersection_local_variable = x_intersection;
                        

//                         // KRATOS_WATCH(x_intersection)
//                         // KRATOS_WATCH(brep_intersection_local_variable)
...
//                     best_brep_id = brep_id;
//                     brep_geometry->SetValue(IDENTIFIER, "active");
