...
//     const auto t_set_surrogate_to_skin_begin = Clock::now();
...
//     const double time_set_surrogate_to_skin =
//         std::chrono::duration<double>(Clock::now() - t_set_surrogate_to_skin_begin).count();
//     KRATOS_WATCH("Timing CreateSbmExtendedGeometries::SetSurrogateToSkinProjections [s]");
//     KRATOS_WATCH(time_set_surrogate_to_skin);
...
//     double time_create_gap_and_skin_quadrature_points = 0.0;
//     const auto t_surrogate_loop_begin = Clock::now();
...
//                 const auto t_gap_skin_begin = Clock::now();
...
//                     *mpIgaModelPart,
//                     rSkinSubModelPart);
//                 time_create_gap_and_skin_quadrature_points +=
//                     std::chrono::duration<double>(Clock::now() - t_gap_skin_begin).count();
//             }
...
//         }
//     }
//     const double time_surrogate_loop =
//         std::chrono::duration<double>(Clock::now() - t_surrogate_loop_begin).count();
//     const double time_surrogate_loop_without_gap_skin =
//         time_surrogate_loop - time_create_gap_and_skin_quadrature_points;
//     KRATOS_WATCH("Timing CreateSbmExtendedGeometries::SurrogateLoop j=0..size_surrogate_loop-1 [s]");
//     KRATOS_WATCH(time_surrogate_loop);
//     KRATOS_WATCH("Timing CreateSbmExtendedGeometries::CreateGapAndSkinQuadraturePoints total [s]");
//     KRATOS_WATCH(time_create_gap_and_skin_quadrature_points);
//     KRATOS_WATCH("Timing CreateSbmExtendedGeometries::SurrogateLoop WITHOUT CreateGapAndSkinQuadraturePoints [s]");
//     KRATOS_WATCH(time_surrogate_loop_without_gap_skin);
...
//         const auto x_indices = gather_indices_with_tolerance(rSurrogateNode.X(), tol_x, min_u, max_u, span_size_x, span_count_x);
//         const auto y_indices = gather_indices_with_tolerance(rSurrogateNode.Y(), tol_y, min_v, max_v, span_size_y, span_count_y);
