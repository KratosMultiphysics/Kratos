...
namespace Kratos
{
    ...
    )
    {
        ...
        KRATOS_ERROR_IF(number_of_nodes < 3 || number_of_nodes % 2 == 0)
            << "StabilizationPenaltyCondition requires node ordering "
        ...
        // Coefficients for Lukas / Leidinger averaged multi-direction constraints
        // node 0 = A
        // nodes 1,2 = R1_1, R2_1
        // nodes 3,4 = R1_2, R2_2
        // ...
        std::vector<double> c_t(number_of_nodes, 0.0);
        std::vector<double> c_r(number_of_nodes, 0.0);

        c_t[0] = 1.0;
        c_r[0] = 1.0;

        // Initial position of light control point A
        ...
        for (SizeType k = 0; k < number_of_directions; ++k) {
            ...
            KRATOS_ERROR_IF(L12 <= std::numeric_limits<double>::epsilon())
                ...
                << " in StabilizationPenaltyCondition" << std::endl;

            const double r = L1 / L12;

            // Lukas / Leidinger translational constraint:
            // g_t = u_A - (1/n_d) * sum_k [ (1+r_k) u_R1,k - r_k u_R2,k ]
            c_t[iR1] += -(1.0 + r) * inv_nd;
            c_t[iR2] +=  (r)       * inv_nd;

            // Lukas / Leidinger rotational constraint:
            // g_r = th_A - (1/n_d) * sum_k th_R1,k
            c_r[iR1] += -inv_nd;
            // c_r[iR2] stays zero
        }
