...
namespace Kratos
{
    void SupportPenaltyRotationCondition::CalculateAll(
        ...
        const bool CalculateResidualVectorFlag)
    {
        ...
        const GeometryType::ShapeFunctionsGradientsType& r_shape_function_gradients =
            r_geometry.ShapeFunctionsLocalGradients(r_geometry.GetDefaultIntegrationMethod());

        for (IndexType point_number = 0; point_number < integration_points.size(); ++point_number)
        {
            const double penalty_integration =
                penalty * integration_points[point_number].Weight() * determinant_jacobian_vector[point_number];

            const Matrix& shape_functions_gradients = r_shape_function_gradients(point_number);

            Vector phi_r = ZeroVector(mat_size);
            Matrix phi_rs = ZeroMatrix(mat_size, mat_size);
            double phi = 0.0;

            CalculateRotation(point_number, shape_functions_gradients,
                              phi_r, phi_rs, phi, local_tangent);

            if (CalculateStiffnessMatrixFlag) {
                noalias(rLeftHandSideMatrix) += (outer_prod(phi_r, phi_r)
                    + phi * phi_rs) * penalty_integration;
            }
            if (CalculateResidualVectorFlag) {
                noalias(rRightHandSideVector) -= (phi * penalty_integration) * phi_r;
            }
        }
        KRATOS_CATCH("")
    }

    void SupportPenaltyRotationCondition::DeterminantOfJacobianInitial(
    ...
    void SupportPenaltyRotationCondition::CalculateRotation(
        ...
        const Vector& local_tangent)
    {
        ...
        for (SizeType i = 0; i < number_of_points; ++i) {
            g10[0] += r_geometry.GetPoint(i).X0() * rShapeFunctionGradientValues(i, 0);
            g10[1] += r_geometry.GetPoint(i).Y0() * rShapeFunctionGradientValues(i, 0);
            g10[2] += r_geometry.GetPoint(i).Z0() * rShapeFunctionGradientValues(i, 0);

            g20[0] += r_geometry.GetPoint(i).X0() * rShapeFunctionGradientValues(i, 1);
            g20[1] += r_geometry.GetPoint(i).Y0() * rShapeFunctionGradientValues(i, 1);
            g20[2] += r_geometry.GetPoint(i).Z0() * rShapeFunctionGradientValues(i, 1);
        }
        ...
        Matrix J;
        r_geometry.Jacobian(J, IntegrationPointIndex);
        g1 = column(J, 0);
        g2 = column(J, 1);
        ...
        double sinus_omega = inner_prod(sinus_omega_vector, T2);
        if (sinus_omega > 1.0) sinus_omega = 0.999999;
        if (sinus_omega < -1.0) sinus_omega = -0.999999;

        phi = std::asin(sinus_omega);

        array_1d<double, 3> t3 = g3;
        array_1d<double, 3> tilde_t3;
        MathUtils<double>::CrossProduct(tilde_t3, g1, g2);
        const double length_t3 = norm_2(tilde_t3);
        ...
        const double one_minus_sin2 = 1.0 - sinus_omega * sinus_omega;
        const double inv_sqrt = 1.0 / std::sqrt(one_minus_sin2);

        for (IndexType n = 0; n < number_of_points; ++n) {
            for (IndexType i = 0; i < 3; ++i) {
                const int nb_dof = n * 3 + i;
                ...
                array_1d<double, 3> a1_r__g2, g1__a2_r;
                ...
                t3_r[nb_dof] = tilde_3_r[nb_dof] / length_t3
                    - line_t3_r[nb_dof] * t3 / length_t3;

                MathUtils<double>::CrossProduct(sinus_omega_r[nb_dof], g30, t3_r[nb_dof]);
                phi_r(nb_dof) = inv_sqrt * inner_prod(sinus_omega_r[nb_dof], T2);
            }
        }

        const double sin_over_pow15 = sinus_omega / std::pow(one_minus_sin2, 1.5);

        for (IndexType n = 0; n < number_of_points; ++n) {
            for (IndexType i = 0; i < 3; ++i) {
                const int nb_dof_n = n * 3 + i;
                ...
                for (IndexType m = 0; m < number_of_points; ++m) {
                    for (IndexType j = 0; j < 3; ++j) {
                        ...
                        array_1d<double, 3> a1_r_n__a2_r_m, a1_r_m__a2_r_n;
                        ...
                        const double line_t3_rs =
                            inner_prod(t3_r[nb_dof_m], tilde_3_r[nb_dof_n])
                            + inner_prod(t3, tilde_t3_rs);

                        array_1d<double, 3> t3_rs = (tilde_t3_rs * length_t3
                            - line_t3_r[nb_dof_m] * tilde_3_r[nb_dof_n])
                            / std::pow(length_t3, 2)
                            - line_t3_rs * t3 / length_t3
                            - line_t3_r[nb_dof_n] * (t3_r[nb_dof_m] * length_t3
                                - line_t3_r[nb_dof_m] * t3) / std::pow(length_t3, 2);

                        array_1d<double, 3> sinus_omega_rs;
                        MathUtils<double>::CrossProduct(sinus_omega_rs, g30, t3_rs);

                        phi_rs(nb_dof_n, nb_dof_m) =
                            inv_sqrt * inner_prod(sinus_omega_rs, T2)
                            + inner_prod(sinus_omega_r[nb_dof_m], T2)
                                * inner_prod(sinus_omega_r[nb_dof_n], T2)
                                * sin_over_pow15;
                    }
                }
            }
    ...
    int SupportPenaltyRotationCondition::Check(const ProcessInfo& rCurrentProcessInfo) const
    {
        KRATOS_ERROR_IF_NOT(GetProperties().Has(PENALTY_FACTOR))
            << "No penalty factor (PENALTY_FACTOR) defined in property of "
               "SupportPenaltyRotationCondition" << std::endl;
        return 0;
    }

    void SupportPenaltyRotationCondition::EquationIdVector(
    ...
    void SupportPenaltyRotationCondition::GetDofList(
