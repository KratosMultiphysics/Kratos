...
namespace Kratos
{
    ...
    )
    {
        ...
        for (IndexType point_number = 0; point_number < integration_points.size(); ++point_number)
        {
            ...
            {   
                // std::cout<<"displacement DBC"<<std::endl;
            ...
            {   
                ...
                if (CalculateStiffnessMatrixFlag) {
                    for (IndexType i = 0; i < mat_size; ++i)
                    {
                        ...
                        {
                            rLeftHandSideMatrix(i, j) += (phi_r(i) * phi_r(j) + diff_phi(0) * phi_rs(i, j)) * penalty_integration * determinant_jacobian_vector[point_number];
                        }
                    }
                }
                ...
                if (CalculateResidualVectorFlag) {
                    for (IndexType i = 0; i < mat_size; ++i)
                    {
                        rRightHandSideVector[i] -= (diff_phi(0) * phi_r(i)) * penalty_integration * determinant_jacobian_vector[point_number];
                    }
                }
            }
        }
