...
namespace Kratos
{
    ...
    )
    {
        ...
        for (IndexType point_number = 0; point_number < integration_points.size(); ++point_number)
        {
            ...
            if(r_geometry.GetGeometryParent(0).GetGeometryFamily() == GeometryData::KratosGeometryFamily::Kratos_Point)
            {
            ...
            else
            {
                ...
                // KRATOS_WATCH(integration_points[point_number].Weight())
                // KRATOS_WATCH(determinant_jacobian_vector[point_number])
            }   

            //Rotation coupling
            // KRATOS_WATCH(Is(IgaFlags::FIX_ROTATION_X))
            // exit(0);
            if (Is(IgaFlags::FIX_ROTATION_X))
            {   
                std::cout<<"rotation DBC"<<std::endl;
                KRATOS_WATCH(penalty_rotation)
                exit(0);
