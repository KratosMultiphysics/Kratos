...
#error "Unknown SBM contact formulation"
    ...
    ) const
    {
        ...
        for (IndexType i = 0; i < number_of_nodes_master; ++i) {
            const auto& r_node = r_geometry_master[i];
            KRATOS_ERROR_IF_NOT(r_node.HasDofFor(DISPLACEMENT_X))
                << "SBM contact master geometry has no DISPLACEMENT_X DOF at node #"
                << r_node.Id() << std::endl;
        ...
        for (IndexType i = 0; i < number_of_nodes_slave; ++i) {
            const auto& r_node = r_geometry_slave[i];
            KRATOS_ERROR_IF_NOT(r_node.HasDofFor(DISPLACEMENT_X))
                << "SBM contact slave geometry has no DISPLACEMENT_X DOF at node #"
                << r_node.Id() << std::endl;
            rElementalDofList.push_back(r_node.pGetDof(DISPLACEMENT_X));
            rElementalDofList.push_back(r_node.pGetDof(DISPLACEMENT_Y));
        }
