...
//     // ##FIXME: debug, impose analytical value for the stress

//     const double x_pos_true = GetGeometry().Center().X();
//     const double R1 = 13.91;
//     const double R2 = 20.96;
//     const double p_N = 15800/2/13.91/20;
//     const double E = 2.1e5;
//     const double nu = 0.29;
//     const double PI = 3.1416;

//     const double R_star = 1.0 / (1.0 / (2*R1) + 1.0 / (2*R2));
//     const double E_star = 1.0 / (((1.0 - nu*nu) / E) + ((1.0 - nu*nu) / E));
//     const double b = sqrt(2*p_N * R1 *2.0 * R_star  / (PI * E_star));

//     double true_stress_normal_dot_n = 0;

//     double p_max = 2*2.0*p_N*R1 /(PI * b);

//     if (std::abs(x_pos_true) < b) {
//         true_stress_normal_dot_n = -p_max * std::sqrt(std::max(0.0, 1.0 - (x_pos_true / b) * (x_pos_true / b)));
//     } else {
//         true_stress_normal_dot_n = 0;
//     }
...
//             // // FIXME: debug, impose analytical value for the stress
//             //  rRightHandSideVector(iglob) += (true_stress_normal_dot_n) * N_sum_vec_master(i) * mNormalPhysicalSpaceMaster[idim] * integration_weight;
...
//             // // FIXME: debug, impose analytical value for the stress
//             // rRightHandSideVector(iglob) += (true_stress_normal_dot_n) * N_sum_vec_slave(i) * (-mNormalPhysicalSpaceMaster[idim]) * integration_weight;
