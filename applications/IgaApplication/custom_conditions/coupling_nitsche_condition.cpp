...
namespace Kratos
{
    ...
    void CouplingNitscheCondition::CalculateKinematicsPoint(
        IndexType IntegrationPointIndex,
    ...
    )
    {
        ...
        rKinematicVariablesPoint.a1 = g1 * local_tangent[0] + g2 * local_tangent[1];

        array_1d<double, 3> n;
        noalias(n) = rKinematicVariablesPoint.a1 / norm_2(rKinematicVariablesPoint.a1);

        // array_1d<double, 3> n, a3;

        // //not-normalized base vector 3
        // MathUtils<double>::CrossProduct(a3, g1, g2);
        // noalias(a3) = a3 / norm_2(a3);

        // MathUtils<double>::CrossProduct(n, rKinematicVariablesPoint.a1/norm_2(rKinematicVariablesPoint.a1), a3);

        // // transform the normal into the contavariant basis
        // rKinematicVariablesPoint.n_contravariant = rKinematicVariablesPoint.a1[0]*n[0] + rKinematicVariablesPoint.a1[1]*n[1] + rKinematicVariablesPoint.a1[2]*n[2];
    ...
    */
        ...
        const bool CalculateResidualVectorFlag)
    {
        ...
        if (Is(IgaFlags::FIX_ROTATION_X))
        {
            // const double stabilization_rotation_parameter = stabilization_parameter;
            stabilization_rotation_parameter = GetProperties()[NITSCHE_STABILIZATION_ROTATION_FACTOR];
            // KRATOS_WATCH(stabilization_rotation_parameter)
        }
        ...
        if (m_T_vector_point_master.size() != r_number_of_integration_points_master)
            m_T_vector_point_master.resize(r_number_of_integration_points_master);
        if (m_T_vector_point_slave.size() != r_number_of_integration_points_slave)
            m_T_vector_point_slave.resize(r_number_of_integration_points_slave);
        if (m_n_contravariant_vector_point_master.size() != r_number_of_integration_points_master)
            m_n_contravariant_vector_point_master.resize(r_number_of_integration_points_master);
        if (m_n_contravariant_vector_point_slave.size() != r_number_of_integration_points_slave)
            m_n_contravariant_vector_point_slave.resize(r_number_of_integration_points_slave);
        ...
        for (IndexType point_number = 0; point_number < integration_points.size(); point_number++)
        {
            if (r_geometry_master.GetGeometryParent(0).GetGeometryFamily() == GeometryData::KratosGeometryFamily::Kratos_Point)
            {
                ...
                KinematicVariablesPoint kinematic_variables_reference_master(
                    r_geometry_master.WorkingSpaceDimension());
                
                KinematicVariablesPoint kinematic_variables_reference_slave(
                    r_geometry_slave.WorkingSpaceDimension());

                CalculateKinematicsPoint(
                    point_number,
                ...
                const double reference_a_master = norm_2(kinematic_variables_reference_master.a1);   
                const double reference_a_slave = norm_2(kinematic_variables_reference_slave.a1);

                m_T_vector_point_master[point_number] = 1 / (reference_a_master * reference_a_master);
                m_T_vector_point_slave[point_number] = 1 / (reference_a_slave * reference_a_slave);      

                // Compute Kinematics 
                KinematicVariablesPoint kinematic_variables_master(
                    r_geometry_master.WorkingSpaceDimension());
                KinematicVariablesPoint kinematic_variables_slave(
                    r_geometry_slave.WorkingSpaceDimension());
                CalculateKinematicsPoint(
                    point_number,
                ...
                m_n_contravariant_vector_point_master[point_number] = kinematic_variables_reference_master.n_contravariant;
                m_n_contravariant_vector_point_slave[point_number] = kinematic_variables_reference_slave.n_contravariant;

                // green-lagrange strain
                const double e11_membrane_master = 0.5 * (inner_prod(kinematic_variables_master.a1, kinematic_variables_master.a1) - inner_prod(kinematic_variables_reference_master.a1, kinematic_variables_reference_master.a1));
                const double e11_membrane_slave = 0.5 * (inner_prod(kinematic_variables_slave.a1, kinematic_variables_slave.a1) - inner_prod(kinematic_variables_reference_slave.a1, kinematic_variables_reference_slave.a1));

                //Prestress component
                const double prestress_master = GetProperties().GetSubProperties().front()[PRESTRESS_CAUCHY];
                const double prestress_slave = GetProperties().GetSubProperties().back()[PRESTRESS_CAUCHY];

                const double E_master = GetProperties().GetSubProperties().front()[YOUNG_MODULUS];
                const double E_slave = GetProperties().GetSubProperties().back()[YOUNG_MODULUS];

                const double A_master = GetProperties().GetSubProperties().front()[CROSS_AREA];
                const double A_slave = GetProperties().GetSubProperties().back()[CROSS_AREA];

                // normal force reference_aa
                const double s11_membrane_master = prestress_master * A_master + e11_membrane_master * A_master * E_master * m_T_vector_point_master[point_number];
                const double s11_membrane_slave = prestress_slave * A_slave + e11_membrane_slave * A_slave * E_slave * m_T_vector_point_slave[point_number];

                const double s11_membrane_covariant_master = s11_membrane_master * m_T_vector_point_master[point_number];
                const double s11_membrane_covariant_slave = s11_membrane_slave * m_T_vector_point_slave[point_number];

                // calculate traction vectors
                array_1d<double, 3> traction_vector_master = s11_membrane_covariant_master * m_n_contravariant_vector_point_master[point_number] * kinematic_variables_master.a1;
                array_1d<double, 3> traction_vector_slave = s11_membrane_covariant_slave * m_n_contravariant_vector_point_slave[point_number] * kinematic_variables_slave.a1;

                // calculate the first variations of the 2nd Piola-Kichhoff stresses at the covariant bases
                Vector first_variations_stress_covariant_master = ZeroVector(3 * number_of_nodes_master);
                Vector first_variations_stress_covariant_slave = ZeroVector(3 * number_of_nodes_slave);

                CalculateFirstVariationStressCovariantPoint(point_number, first_variations_stress_covariant_master, kinematic_variables_master, PatchType::Master);
                CalculateFirstVariationStressCovariantPoint(point_number, first_variations_stress_covariant_slave, kinematic_variables_slave, PatchType::Slave);
                ...
                CalculateFirstVariationTractionPoint(point_number, first_variations_traction_master, first_variations_stress_covariant_master, kinematic_variables_master, s11_membrane_covariant_master, PatchType::Master);
                CalculateFirstVariationTractionPoint(point_number, first_variations_traction_slave, first_variations_stress_covariant_slave, kinematic_variables_slave, s11_membrane_covariant_slave, PatchType::Slave);
                ...
                Vector Pi_master = ZeroVector(3);
                Vector Pi_slave = ZeroVector(3);

                Pi_master = m_T_vector_point_master[point_number] * s11_membrane_covariant_master * m_n_contravariant_vector_point_master[point_number] * kinematic_variables_master.a1;
                Pi_slave = m_T_vector_point_slave[point_number] * s11_membrane_covariant_slave * m_n_contravariant_vector_point_slave[point_number] * kinematic_variables_slave.a1;

                double second_variations_traction_product_vector_master = inner_prod(Pi_master, displacement_vector_master);
                double second_variations_traction_product_vector_slave = inner_prod(Pi_slave, displacement_vector_slave);
                double second_variations_traction_product_vector_master_slave = inner_prod(Pi_slave, displacement_vector_master);
                double second_variations_traction_product_vector_slave_master = inner_prod(Pi_master, displacement_vector_slave);
                ...
                CalculateSecondVariationTractionPoint(point_number, second_variations_traction_master, kinematic_variables_master, first_variations_stress_covariant_master, displacement_vector_master, displacement_vector_slave, 
                                                second_variations_traction_product_vector_master, second_variations_traction_product_vector_slave_master, PatchType::Master);
                CalculateSecondVariationTractionPoint(point_number, second_variations_traction_slave, kinematic_variables_slave, first_variations_stress_covariant_slave, displacement_vector_master, displacement_vector_slave, 
                ...
                double determinant_jacobian = 1.0;
            ...
            else
            {
                ...
                {   
                    ////0.
                    //a. Get the rotation vectors of the previous iteration step
                    ...
                    if (inner_prod(T2_master, T2_slave) > 0) // tangents have the same direction (assumption for coordinates system changes)
                    {
                    ...
                    ////2.
                    //a. calculate the first variations of the 2nd Piola-Kichhoff moments at the covariant bases
                    ...
                    // KRATOS_WATCH(first_variations_moment_covariant_master)
                    // KRATOS_WATCH(first_variations_moment_covariant_slave)

                    //b. calculate first variation of moment vectors
                    ...
                    // KRATOS_WATCH(first_variations_moment_master)
                    // KRATOS_WATCH(first_variations_moment_slave)

                    // KRATOS_WATCH(T2_master)
                    // KRATOS_WATCH(T2_slave)

                    // KRATOS_WATCH(T1_master)
                    // KRATOS_WATCH(T1_slave)
                    ...
                    //c. calculate first variation of moment vectors T2
                    ...
                    // KRATOS_WATCH(opposite_direction_of_trims)
                    // KRATOS_WATCH(T2_slave)
                    // KRATOS_WATCH(first_variations_moment_T2)
                    // KRATOS_WATCH(phi_r)

                    ////3.
                    //a. Compute the necessary products needed for the second variations of the traction vectors
                    Matrix Pi_master = ZeroMatrix(3, 3);
                    Matrix Pi_slave = ZeroMatrix(3, 3);
                    ...
                    //b. calculate second variation of moment vectors
                    ...
                    if (CalculateStiffnessMatrixFlag) {
                        // first variation moment * first vatiation rotation
                        // moment * second variation rotation
                        // penalty term
                        ...
                        // second variation moment * rotation
                        for (IndexType i = 0; i < 3 * number_of_nodes_master; i++)
                        {
                            for (IndexType j = 0; j < 3 * number_of_nodes_master; j++)
                            {
                    ...
                    if (CalculateResidualVectorFlag) {
                        // first variation moment * rotation
                        // moment * first variation rotation
                        // penalty term
                        for (IndexType i = 0; i < mat_size; ++i)
                        {
        ...
        const ProcessInfo& rCurrentProcessInfo)
    {
        ...
        r_geometry_slave.Calculate(LOCAL_TANGENT, local_tangent_slave);
        ...
        if (r_geometry_master.GetGeometryParent(0).GetGeometryFamily() == GeometryData::KratosGeometryFamily::Kratos_Point)
        {
            r_geometry_master.GetGeometryParent(0).GetGeometryPart(GeometryType::BACKGROUND_GEOMETRY_INDEX).pGetGeometryPart(GeometryType::BACKGROUND_GEOMETRY_INDEX)->Calculate(CHARACTERISTIC_GEOMETRY_LENGTH, characteristic_length_master);
            r_geometry_slave.GetGeometryParent(0).GetGeometryPart(GeometryType::BACKGROUND_GEOMETRY_INDEX).pGetGeometryPart(GeometryType::BACKGROUND_GEOMETRY_INDEX)->Calculate(CHARACTERISTIC_GEOMETRY_LENGTH, characteristic_length_slave);
        }
        else
        {
        ...
        double tol_surface_normal = tol_basic*characteristic_area; //*0.0

        for (IndexType point_number = 0; point_number < integration_points.size(); point_number++)
        {
            KRATOS_WATCH("I AM HERE 3")
            ...
            if (r_geometry_master.GetGeometryParent(0).GetGeometryFamily() == GeometryData::KratosGeometryFamily::Kratos_Point)
            {
                ...
                Pi_master = m_T_vector_point_master[point_number] * E_master * m_T_vector_point_master[point_number] * m_n_contravariant_vector_point_master[point_number] * kinematic_variables_master.a1;
                Pi_slave = m_T_vector_point_slave[point_number] * E_slave * m_T_vector_point_master[point_number] * m_n_contravariant_vector_point_slave[point_number] * kinematic_variables_slave.a1;

                double second_variations_traction_product_vector_master = inner_prod(Pi_master, traction_vector_master);
                double second_variations_traction_product_vector_slave = inner_prod(Pi_slave, traction_vector_slave);
                double second_variations_traction_product_vector_master_slave = inner_prod(Pi_slave, traction_vector_master);
                double second_variations_traction_product_vector_slave_master = inner_prod(Pi_master, traction_vector_slave);
                ...
                // {
                CalculateSecondVariationTractionPoint(point_number, second_variations_traction_master, kinematic_variables_master, first_variations_stress_covariant_master, traction_vector_master, traction_vector_slave, 
                                                second_variations_traction_product_vector_master, second_variations_traction_product_vector_slave_master, PatchType::Master);
                CalculateSecondVariationTractionPoint(point_number, second_variations_traction_slave, kinematic_variables_slave, first_variations_stress_covariant_slave, traction_vector_master, traction_vector_slave, 
                                                second_variations_traction_product_vector_slave, second_variations_traction_product_vector_master_slave, PatchType::Slave);
                // }
                // else
                // {
                //     std::cout<<"testing"<<std::endl;
                //     KRATOS_WATCH(norm_2(kinematic_variables_reference_master.a3_tilde))
                //     KRATOS_WATCH(norm_2(kinematic_variables_reference_slave.a3_tilde))
                //     KRATOS_WATCH(tol_surface_normal)
                // }
        ...
        const PatchType& rPatch)
    {
      ...
      void CouplingNitscheCondition::CalculateFirstVariationStressCovariantPoint(
        IndexType IntegrationPointIndex,
        ...
        const PatchType& rPatch)
    {
        ...
        r_geometry.Calculate(LOCAL_TANGENT, tangents);
        ...
        //Compute the first variation of the Green-Lagrange strains
        Vector dE_cartesian = ZeroVector(mat_size);
        double T_patch;
        double E;

        if (rPatch==PatchType::Master)
        {
            T_patch = m_T_vector_point_master[IntegrationPointIndex];
            E = GetProperties().GetSubProperties().front()[YOUNG_MODULUS];
        }
        else
        {
            T_patch = m_T_vector_point_slave[IntegrationPointIndex];
            E = GetProperties().GetSubProperties().back()[YOUNG_MODULUS];
        }
        ...
        for (IndexType r = 0; r < mat_size; r++)
        {
            ...
            // strain
            double dE_curvilinear = (r_DN_De(kr, 0) * tangents[0] + r_DN_De(kr, 1) * tangents[1]) * rActualKinematic.a1(dirr);

            dE_cartesian[r] = T_patch * dE_curvilinear;
        }

        //Compute the first variations of the 2nd Piola-Kichhoff stresses in the local Cartesian bases
        Vector first_variations_stress_cartesian = E * dE_cartesian;

        //Transform the first variations of the 2nd Piola-Kichhoff stresses at the covariant bases
        rFirstVariationStressCovariant = T_patch * first_variations_stress_cartesian;
    }
    ...
    void CouplingNitscheCondition::CalculateFirstVariationTractionPoint(
        IndexType IntegrationPointIndex,
        Matrix& rFirstVariationTraction,
        Vector& rFirstVariationStressCovariant,
        const KinematicVariablesPoint& rActualKinematic,
        ...
        const PatchType& rPatch)
    {
        ...
        //get the normal vector
        double n_contravariant_vector; 
        if (rPatch==PatchType::Master)
        {
            n_contravariant_vector = m_n_contravariant_vector_point_master[IntegrationPointIndex];
        }
        else
        {
            n_contravariant_vector = m_n_contravariant_vector_point_slave[IntegrationPointIndex];
        }
        ...
        for(IndexType r = 0; r < mat_size; r++)
        {
            rFirstVariationTraction(0, r) = rActualKinematic.a1[0] * rFirstVariationStressCovariant[r] * n_contravariant_vector;
            rFirstVariationTraction(1, r) = rActualKinematic.a1[1] * rFirstVariationStressCovariant[r] * n_contravariant_vector;
            rFirstVariationTraction(2, r) = rActualKinematic.a1[2] * rFirstVariationStressCovariant[r] * n_contravariant_vector;
        }
        ...
        rFirstVariationTraction += (r_DN_Dxi * tangents[0] + r_DN_Deta * tangents[1]) * n_contravariant_vector * rStressMembraneCovariant;
    }
    ...
    void CouplingNitscheCondition::CalculateSecondVariationTractionPoint(
        IndexType IntegrationPointIndex,
        Matrix& rSecondVariationTraction,
        const KinematicVariablesPoint& rActualKinematic,
        Vector& rFirstVariationStressCovariant, 
        ...
        const PatchType& rPatch)
    {
        ...
        // displacement * first variation stress covariant
        Matrix displacement_dNCovariant_master = ZeroMatrix(3, mat_size); 
        Matrix displacement_dNCovariant_slave = ZeroMatrix(3, mat_size); ; 

        for (IndexType r = 0; r < mat_size; r++)
        {
            displacement_dNCovariant_master(0, r) = rDisplacementMaster(0)*rFirstVariationStressCovariant(r);
            displacement_dNCovariant_master(1, r) = rDisplacementMaster(1)*rFirstVariationStressCovariant(r);
            displacement_dNCovariant_master(2, r) = rDisplacementMaster(2)*rFirstVariationStressCovariant(r);

            displacement_dNCovariant_slave(0, r) = rDisplacementSlave(0)*rFirstVariationStressCovariant(r);
            displacement_dNCovariant_slave(1, r) = rDisplacementSlave(1)*rFirstVariationStressCovariant(r);
            displacement_dNCovariant_slave(2, r) = rDisplacementSlave(2)*rFirstVariationStressCovariant(r);
        }

        if (rPatch==PatchType::Slave)
        {
            displacement_dNCovariant_master *= -1.0;
            displacement_dNCovariant_slave *= -1.0;
        }

        rSecondVariationTraction += (prod(trans(r_DN_Dxi), r_DN_Dxi)*tangents[0] + prod(trans(r_DN_Deta), r_DN_Deta)*tangents[1])*rSecondVariationTractionProduct;
        rSecondVariationTraction += (prod(trans(r_DN_Dxi), displacement_dNCovariant_master)*tangents[0] + prod(trans(r_DN_Deta), displacement_dNCovariant_master)*tangents[1])*n_contravariant_vector;
        rSecondVariationTraction += (prod(trans(displacement_dNCovariant_master), r_DN_Dxi)*tangents[0] + prod(trans(displacement_dNCovariant_master), r_DN_Deta)*tangents[1])*n_contravariant_vector;
        
        rSecondVariationTraction -= (prod(trans(r_DN_Dxi), r_DN_Dxi)*tangents[0] + prod(trans(r_DN_Deta), r_DN_Deta)*tangents[1])*rSecondVariationTractionProductMasterSlave;
        rSecondVariationTraction -= (prod(trans(r_DN_Dxi), displacement_dNCovariant_slave)*tangents[0] + prod(trans(r_DN_Deta), displacement_dNCovariant_slave)*tangents[0])*n_contravariant_vector;
        rSecondVariationTraction -= (prod(trans(displacement_dNCovariant_slave), r_DN_Dxi)*tangents[0] + prod(trans(displacement_dNCovariant_slave), r_DN_Deta)*tangents[0])*n_contravariant_vector;
    }
        ...
        array_1d<double, 2> &diff_phi)
    {
        ...
        {
            diff_phi = -(phi_slave - phi_master); //should be negative
        }
