...
void GapSbmContactCondition::ComputeTaylorExpansionContribution(Vector& H_sum_vec)
{
    const auto& r_geometry = GetSurrogateGeometry();
...
void GapSbmContactCondition::ComputeGradientTaylorExpansionContribution(Matrix& grad_H_sum)
{
    const auto& r_geometry = GetSurrogateGeometry();
    const SizeType number_of_control_points = r_geometry.PointsNumber();
...
const GapSbmContactCondition::NodeType::Pointer GapSbmContactCondition::pGetProjectionNode() const
{
    KRATOS_ERROR_IF_NOT(GetGeometry().Has(PROJECTION_NODE))
        << "\"GapSbmContactCondition\" #" << Id() << " has no PROJECTION_NODE set." << std::endl;
