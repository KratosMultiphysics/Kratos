...
)
{
    ...
    // ASSEMBLE
    //-----------------------------------------------------
    // Boundary data are assigned to the condition through its physical layer.
    // The projection node only carries the original-skin position (and normal
    // for the enhanced load condition); it is deliberately not part of the
    // computational model part and therefore receives no external process data.
    const Vector u_D = this->GetValue(DISPLACEMENT);
