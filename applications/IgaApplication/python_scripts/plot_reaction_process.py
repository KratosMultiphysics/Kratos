...
class OuputReactionProcess(KM.Process):
    def __init__(self, model, settings ):
        ...
        print(self.output_damage_file_name)
    ...
    def ExecuteFinalizeSolutionStep(self):
        ...
        if self.is_output_step(time_step):
            ...
            print(self.output_damage_file_name_compression)
            if not self.output_damage_file_name_compression.endswith(".dat"):
                self.output_damage_file_name_compression += ".dat"

            with open(self.output_damage_file_name_compression, 'w') as output_file:
                for element in self.model_part_root.Elements:
                    coords = element.CalculateOnIntegrationPoints(KM.INITIAL_COORDINATES, self.model_part_root.ProcessInfo)[0]
                    output_file.write(str(coords[0])  + " " + str(coords[1])  + " " + str(coords[2])  + " " + str(element.CalculateOnIntegrationPoints(CLA.DAMAGE_COMPRESSION, self.model_part_root.ProcessInfo)[0]) + "\n")

            self.output_damage_file_name_tension = self.output_file_name_start + "_DAMAGE_TENSION_" + str(int(time_step*1000)) + "_" + str(self.model_part_root.NumberOfNodes())
            print(self.output_damage_file_name_tension)
            if not self.output_damage_file_name_tension.endswith(".dat"):
                self.output_damage_file_name_tension += ".dat"

            with open(self.output_damage_file_name_tension, 'w') as output_file:
                for element in self.model_part_root.Elements:
                    coords = element.CalculateOnIntegrationPoints(KM.INITIAL_COORDINATES, self.model_part_root.ProcessInfo)[0]
                    output_file.write(str(coords[0])  + " " + str(coords[1])  + " " + str(coords[2])  + " " + str(element.CalculateOnIntegrationPoints(CLA.DAMAGE_TENSION, self.model_part_root.ProcessInfo)[0]) + "\n")
