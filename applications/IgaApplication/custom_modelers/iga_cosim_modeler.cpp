...
namespace Kratos
{
    ...
    void IgaCosimModeler::SetupGeometryModel()
    {
        ...
        // if (dim == 2)
        // {
            CreateInterfaceLineCouplingConditions(mpModels[0]->GetModelPart(origin_interface_sub_model_part_name), true);
            CreateInterfaceLineCouplingConditions(mpModels.back()->GetModelPart(destination_interface_sub_model_part_name), false);
        // }
        // else
        // {
        //     KRATOS_ERROR << "Not implemented yet" << std::endl;
        ...
        // {
            ...
            MappingIntersectionIgaUtilities::FindIntersection1DGeometries2D(
            ...
            MappingIntersectionIgaUtilities::CreateQuadraturePointsCoupling1DGeometries2D(
                coupling_model_part, 1e-6);
    ...
    void IgaCosimModeler::CreateInterfaceLineCouplingConditions(ModelPart& rInterfaceModelPart, bool MasterToSlave)
    {
        ...
        for (auto& r_cond : rInterfaceModelPart.Conditions()) {
            ...
            for (IndexType i = 0; i<r_N.size2();++i)
            {
                if(r_N(0,i) > 1e-6)
                {
                    // coupling_conditions.AssignNode(r_geom.pGetPoint(i));
                    // coupling_conditions.AddNode(r_geom.pGetPoint(i));
        ...
        {
            points.push_back(rInterfaceModelPart.GetRootModelPart().pGetNode(nodes_id[i]));
        }

        KRATOS_WATCH(rInterfaceModelPart.GetRootModelPart())
        KRATOS_WATCH(points)

        // exit(0);
        // KRATOS_WATCH(coupling_conditions)
        // KRATOS_WATCH(*(coupling_conditions.NodesBegin()))
        
        // for (auto& r_node : coupling_conditions.Nodes()) {

        //     KRATOS_WATCH(r_node.Id())
        //     KRATOS_WATCH(r_node)
        //     // //create a new node
        //     // Node::Pointer p_new_node = Node::Pointer(new Node(r_node.Id(), r_node.X(), r_node.Y(), r_node.Z()));
        //     // // Node::Pointer p_new_node = Node::Pointer(new Node(r_node.Id()+100, r_node.X(), r_node.Y(), r_node.Z()));
        //     // p_new_node->SetSolutionStepVariablesList(&(rInterfaceModelPart.GetNodalSolutionStepVariablesList()));
        //     // p_new_node->SetBufferSize(rInterfaceModelPart.NodesBegin()->GetBufferSize());
        //     // for (auto it_dof = r_node.GetDofs().begin(); it_dof != r_node.GetDofs().end(); ++it_dof)
        //     //     p_new_node->pAddDof(**it_dof);
        //     Node::Pointer p_new_node = r_node.Clone();
        //     points.push_back(p_new_node);
        // }
