...
namespace Kratos
{
        ...
        const Parameters rParameters) const
    {
        ...
        if (rParameters["geometry_type"].GetString() == "NurbsSurface") {
            ...
            for (IndexType n = 0; n < geometry_list.size(); ++n) {
                // Get the THBSurfaceGeometry from the LocalRefinedBrepSurface's background.
                ...
                KRATOS_ERROR_IF(p_thb == nullptr)
                    << "LocalRefinedSurface p-refinement: background geometry is not a THBSurfaceGeometry." << std::endl;

                // Build a temporary NurbsSurfaceGeometry from level-0 so we can reuse
                // the existing DegreeElevation utilities unchanged.
                const auto& level0 = p_thb->Levels()[0];
                Vector weights = level0.Weights;
                if (weights.size() == 0) {
                    weights.resize(p_thb->size(), 1.0);
                }
                auto p_temp = Kratos::make_shared<NurbsSurfaceGeometry<3, PointerVector<NodeType>>>(
                    p_thb->Points(), level0.DegreeU, level0.DegreeV,
                    level0.KnotsU, level0.KnotsV, weights);
                ...
                if (rParameters["parameters"].Has("increase_degree_u")) {
                    ...
                    if (increase_degree_u > 0) {
                        ...
                        Vector weights_refined;

                        NurbsSurfaceRefinementUtilities::DegreeElevationU(*p_temp, increase_degree_u,
                            points_refined, knots_u_refined, weights_refined);

                        for (auto& r_point : p_temp->Points())
                            r_point.Set(TO_ERASE, true);
                        r_model_part.RemoveNodesFromAllLevels(TO_ERASE);
                        IndexType node_id = r_model_part.Nodes().empty() ? 1 : (r_model_part.NodesEnd() - 1)->Id() + 1;
                        for (IndexType i = 0; i < points_refined.size(); ++i) {
                            if (points_refined(i)->Id() == 0) {
                                points_refined(i) = r_model_part.CreateNewNode(node_id++,
                                    points_refined[i][0], points_refined[i][1], points_refined[i][2]);
                            }
                        }

                        p_temp->SetInternals(points_refined,
                            level0.DegreeU + increase_degree_u, p_temp->PolynomialDegreeV(),
                            knots_u_refined, p_temp->KnotsV(), weights_refined);
                    }
                }
                if (rParameters["parameters"].Has("increase_degree_v")) {
                    SizeType increase_degree_v = rParameters["parameters"]["increase_degree_v"].GetInt();
                    if (increase_degree_v > 0) {
                        ...
                        NurbsSurfaceRefinementUtilities::DegreeElevationV(*p_temp, increase_degree_v,
                            points_refined, knots_v_refined, weights_refined);
                        ...
                        p_temp->SetInternals(points_refined,
                            p_temp->PolynomialDegreeU(), level0.DegreeV + increase_degree_v,
                            p_temp->KnotsU(), knots_v_refined, weights_refined);
                    }
                }

                // Push the elevated level-0 data back into the THBSurfaceGeometry.
                p_thb->SetLevel0(p_temp->Points(),
                    p_temp->PolynomialDegreeU(), p_temp->PolynomialDegreeV(),
                    p_temp->KnotsU(), p_temp->KnotsV(), p_temp->Weights());
            }
