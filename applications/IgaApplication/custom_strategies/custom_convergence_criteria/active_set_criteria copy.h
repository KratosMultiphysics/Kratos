...
//             // Use the historical geometric tolerance to seed the active set.
//             // Once contact is active, use the pressure-like augmented criterion.
//             constexpr double initial_gap_tolerance = -1.0e-2;
//             // constexpr double initial_gap_tolerance = 0.0;
...
//                 // Penetration has positive gap and compression has negative normal traction.
//                 // Hence the biased Nitsche flux P_gamma = t_N^m - gamma*g activates for P_gamma < 0.
//                 const double gamma = i_cond->GetValue(NITSCHE_STABILIZATION_FACTOR);
//                 const double p_gamma = sigma_nn_master - gamma * normal_gap_master;
...
//             // Seed contact from the undeformed geometric gap, as in the
//             // pre-c15b54c implementation. Afterwards retain the augmented
//             // pressure criterion introduced by that commit.
// #if SBM_CONTACT_FORMULATION == SBM_CONTACT_PENALTY_FORMULATION
//                 const bool activate_master = has_active_condition
//                     ? check_value_master >= 0.0
//                     : normal_gap_master >= initial_gap_tolerance;
// #elif SBM_CONTACT_FORMULATION == SBM_CONTACT_NITSCHE_FORMULATION
//                 const bool activate_master = p_gamma < 0.0;
// #else
// #error "Unknown SBM contact formulation"
// #endif
//                 if (activate_master)
