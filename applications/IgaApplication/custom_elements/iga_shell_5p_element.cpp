...
namespace Kratos
{
    void IgaShell5pElement::Initialize()
    {
        ...
        InitializeMetricVariables(m_initial_metric, 3);      // the parameter 3 defines the dimension
    ...
    void IgaShell5pElement::InitializeMetricVariables(
        MetricVariables& rMetric,
        const unsigned int& Dimension
    )
    {
            rMetric.gab = ZeroVector(Dimension);
            rMetric.gab_con = ZeroVector(Dimension);

            rMetric.curvature = ZeroVector(Dimension);

            rMetric.J = ZeroMatrix(Dimension, Dimension);
            rMetric.detJ = 1.0;

            rMetric.g1 = ZeroVector(Dimension);
            rMetric.g2 = ZeroVector(Dimension);
            rMetric.g3 = ZeroVector(Dimension);

            rMetric.dA = 1.0;
            
            rMetric.H = ZeroMatrix(Dimension, Dimension);
            rMetric.Q = ZeroMatrix(Dimension, Dimension);
            rMetric.T = ZeroMatrix(Dimension, Dimension);
    }
    ...
    {
        // m_phi1 = DotProduct((rActualMetric.g2 - m_initial_metric.g2),m_initial_metric.g3)
        // / norm_2(CrossProduct(m_initial_metric.g1,m_initial_metric.g2));
        // m_phi2 = DotProduct((rActualMetric.g1 - m_initial_metric.g1),m_initial_metric.g3)
        // / norm_2(CrossProduct(m_initial_metric.g1,m_initial_metric.g2));
