...
namespace Kratos
{
    ...
    * New T 
    * ε_curvilinear is defined: [ε_11, ε_22,ε_33, ε_12, ε_13, ε_23]
    *  The transformation matrix T transforms to voigt notation:
    *  ε_local_cartesian = [ε_11, ε_22,ε_33, 2*ε_12,2* ε_13, 2*ε_23]
    */
    ...
    *  the local cartesian basis.
    */
    void ShellRMElement::CalculateTransformationFromCovariantToCartesian(
        const KinematicVariables& rKinematicVariables,
        Matrix& rTCovToCar
    ) const
    {
        ...
        array_1d<double, 3> a_contravariant_3 = rKinematicVariables.a3;
        ...
        // e * a_covariant
        Matrix GT = ZeroMatrix(3, 3);
        GT(0, 0) = inner_prod(e1, rKinematicVariables.a1);
        GT(0, 1) = inner_prod(e1, rKinematicVariables.a2);
        GT(0, 2) = inner_prod(e1, rKinematicVariables.a3);
        GT(1, 0) = inner_prod(e2, rKinematicVariables.a1);
        GT(1, 1) = inner_prod(e2, rKinematicVariables.a2);
        GT(1, 2) = inner_prod(e2, rKinematicVariables.a3);
        GT(2, 0) = inner_prod(e3, rKinematicVariables.a1);
        GT(2, 1) = inner_prod(e3, rKinematicVariables.a2);
        GT(2, 2) = inner_prod(e3, rKinematicVariables.a3);

        //Transformation matrix T from covariant to local cartesian coordinate system
        if (rTCovToCar.size1() != 6 && rTCovToCar.size2() != 6)  
            rTCovToCar.resize(6, 6);
         noalias(rTCovToCar) = ZeroMatrix(6, 6);

        rTCovToCar(0, 0) = pow(GT(0, 0), 2);
        rTCovToCar(0, 1) = pow(GT(0, 1), 2);
        rTCovToCar(0, 2) = pow(GT(0, 2), 2);
        rTCovToCar(0, 3) = 2 * GT(0, 0) * GT(0, 1);
        rTCovToCar(0, 4) = 2 * GT(0, 1) * GT(0, 2);
        rTCovToCar(0, 5) = 2 * GT(0, 0) * GT(0, 2);

        rTCovToCar(1, 0) = pow(GT(1, 0), 2);
        rTCovToCar(1, 1) = pow(GT(1, 1), 2);
        rTCovToCar(1, 2) = pow(GT(1, 2), 2);
        rTCovToCar(1, 3) = 2 * GT(1, 0) * GT(1, 1);
        rTCovToCar(1, 4) = 2 * GT(1, 1) * GT(1, 2);
        rTCovToCar(1, 5) = 2 * GT(1, 0) * GT(1, 2);

        rTCovToCar(2, 0) = pow(GT(2, 0), 2);
        rTCovToCar(2, 1) = pow(GT(2, 1), 2);
        rTCovToCar(2, 2) = pow(GT(2, 2), 2);
        rTCovToCar(2, 3) = 2 * GT(2, 0) * GT(2, 1);
        rTCovToCar(2, 4) = 2 * GT(2, 1) * GT(2, 2);
        rTCovToCar(2, 5) = 2 * GT(2, 0) * GT(2, 2);

        rTCovToCar(3, 0) = GT(0, 0) * GT(1, 0);
        rTCovToCar(3, 1) = GT(0, 1) * GT(1, 1);
        rTCovToCar(3, 2) = GT(0, 2) * GT(1, 2);
        rTCovToCar(3, 3) = GT(0, 0) * GT(1, 1) + GT(0, 1) * GT(1, 0);
        rTCovToCar(3, 4) = GT(0, 1) * GT(1, 2) + GT(0, 2) * GT(1, 1);
        rTCovToCar(3, 5) = GT(0, 0) * GT(1, 2) + GT(0, 2) * GT(1, 0);

        rTCovToCar(4, 0) = GT(1, 0) * GT(2, 0);
        rTCovToCar(4, 1) = GT(1, 1) * GT(2, 1);
        rTCovToCar(4, 2) = GT(1, 2) * GT(2, 2);
        rTCovToCar(4, 3) = GT(1, 0) * GT(2, 1) + GT(1, 1) * GT(2, 0);
        rTCovToCar(4, 4) = GT(1, 1) * GT(2, 2) + GT(1, 2) * GT(2, 1);
        rTCovToCar(4, 5) = GT(1, 0) * GT(2, 2) + GT(1, 2) * GT(2, 0);

        rTCovToCar(5, 0) = GT(0, 0) * GT(2, 0);
        rTCovToCar(5, 1) = GT(0, 1) * GT(2, 1);
        rTCovToCar(5, 2) = GT(0, 2) * GT(2, 2);
        rTCovToCar(5, 3) = GT(0, 0) * GT(2, 1) + GT(0, 1) * GT(2, 0);
        rTCovToCar(5, 4) = GT(0, 1) * GT(2, 2) + GT(0, 2) * GT(2, 1);
        rTCovToCar(5, 5) = GT(0, 0) * GT(2, 2) + GT(0, 2) * GT(2, 0);
    }
        ...
        const KinematicVariables& rActualKinematic) const
    {
        ...
        //noalias(rBd) = -prod(m_T_vector[IntegrationPointIndex],rBd);
        }
    }

   

    void ShellRMElement::CalculateSecondVariationStrainCurvature(
        const IndexType IntegrationPointIndex,
 ...
 inline void ShellRMElement::CalculateAndAddNonlinearKm(
    ...
    void ShellRMElement::CalculateShearForce(
        const IndexType IntegrationPointIndex,
    ...
    void ShellRMElement::CalculateDerivativeOfCurvatureInitial(
        const IndexType IntegrationPointIndex,
    ...
    void ShellRMElement::CalculateDerivativeOfCurvatureActual(
        const IndexType IntegrationPointIndex,
        array_1d<double, 3>& rDCurvature_D1,
        array_1d<double, 3>& rDCurvature_D2,
    ...
    void ShellRMElement::CalculateDerivativeTransformationMatrices(
        const IndexType IntegrationPointIndex,
