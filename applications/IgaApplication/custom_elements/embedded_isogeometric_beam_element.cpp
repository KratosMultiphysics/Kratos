...
namespace Kratos {
    ...
    {
        //KRATOS_WATCH("EmbeddedIsogeometricBeamElement::EquationIdVector");
        KRATOS_TRY;
    ...
    {
        //KRATOS_WATCH("EmbeddedIsogeometricBeamElement::GetDofList");
    ...
    void EmbeddedIsogeometricBeamElement::GetElementOrientation(const Matrix& r_DN_De, const ConfigurationType& rConfiguration , Vector3d& B1, Vector3d& B2, Vector3d& B3, float& A, float& B)
    {
        ...
        B1 = A1 * tangents[0] + A2 * tangents[1]; //calculates the tangential vector B1 or b1 according to eq.(6)
        cross_prod(B2, A1, A2); //calculates the normal vector B2 or b2 according to eq. (7)
        cross_prod(B3, B1, B2); //calculates the binormalvector B3 or b3 according to eq. (8)
    ...
    void EmbeddedIsogeometricBeamElement::comp_mat_rodrigues_var_var(Matrix& _mat_rod_var_var, Vector3d _vec, Vector _vec_var, Matrix _vec_var_var, Vector _func, float _phi)
    {
        //const unsigned int N_Dof = 4;//this->GetGeometry().PointsNumber()* (this->GetGeometry().WorkingSpaceDimension() + 1);
    ...
    void EmbeddedIsogeometricBeamElement::comp_mat_rodrigues_deriv_var(Matrix& _mat_rod_der_var, Vector3d _vec, Vector _vec_var, Vector3d _vec_der, Vector _vec_der_var, Vector _func, Vector _deriv, float _phi, float _phi_der)
    {
        //const unsigned int N_Dof = 4;//this->GetGeometry().PointsNumber()* (this->GetGeometry().WorkingSpaceDimension() + 1);
    ...
    void EmbeddedIsogeometricBeamElement::comp_mat_rodrigues_deriv_var_var(Matrix& _mat_rod_der_var_var, Vector3d _vec, Vector _vec_var, Vector3d _vec_der, Vector _vec_der_var, Matrix& _vec_var_var, Matrix& _vec_der_var_var, Vector _func, Vector _deriv, float _phi, float _phi_der)
    {
        //const unsigned int N_Dof = 4;//this->GetGeometry().PointsNumber()* (this->GetGeometry().WorkingSpaceDimension() + 1);
    ...
    void EmbeddedIsogeometricBeamElement::comp_mat_rodrigues_deriv2(Matrix3d& _mat_rod_derder, Vector3d _vec, Vector3d _vec_deriv, Vector3d _vec_deriv2, float _phi, float _phi_deriv, float _phi_deriv2)
    {
    ...
    void EmbeddedIsogeometricBeamElement::comp_mat_rodrigues_deriv2_var(Matrix& _mat_rod_derder_var, Vector3d _vec, Vector _vec_var, Vector3d _vec_der, Vector _vec_der_var, Vector3d _vec_derder, Vector _vec_derder_var, Vector _func, Vector _deriv, Vector _deriv2, float _phi, float _phi_der, float _phi_der2)
    {
        //const unsigned int N_Dof = 4;//this->GetGeometry().PointsNumber()* (this->GetGeometry().WorkingSpaceDimension() + 1);
    ...
    void EmbeddedIsogeometricBeamElement::comp_Geometry_reference(Vector _deriv, Vector _deriv2, Vector _deriv3, Vector3d& _R1, Vector3d& _R2, Vector3d& _R3, float& _A_ref, float& _B_ref)
    {
        const auto& r_geometry = GetGeometry();
    ...
    void EmbeddedIsogeometricBeamElement::comp_Geometry_actual(const ProcessInfo& rCurrentProcessInfo, Vector& _deriv, Vector& _deriv2, Vector& _deriv3, Vector3d& _r1, Vector3d& _r2, Vector3d& _r3, float& _a, float& _b)
    {
        const auto& r_geometry = GetGeometry();
    ...
    void EmbeddedIsogeometricBeamElement::comp_Geometry_actual(const ProcessInfo& rCurrentProcessInfo,Vector& _deriv, Vector& _deriv2, Vector3d& _r1, Vector3d& _r2, float& _a, float& _b)
    {
        const auto& r_geometry = GetGeometry();
        Vector3d displacement;
    ...
    void EmbeddedIsogeometricBeamElement::comp_Geometry_initial(Vector& _deriv, Vector& _deriv2, Vector& _deriv3, Vector3d& _r1, Vector3d& _r2, Vector3d& _r3, float& _a, float& _b)
    {
        const auto& r_geometry = GetGeometry();
        Vector3d displacement;
    ...
    void EmbeddedIsogeometricBeamElement::comp_Geometry_initial(Vector& _deriv, Vector& _deriv2, Vector3d& _r1, Vector3d& _r2, float& _a, float& _b)
    {
        const auto& r_geometry = GetGeometry();
        Vector3d displacement;
    ...
    void EmbeddedIsogeometricBeamElement::comp_dof_lin(Vector& _cur_var_n, Vector& _cur_var_v, Vector& _tor_var_n, Vector& _tor_var_v, Vector3d& _r1, Vector3d& _R1, Vector3d& _r2, Vector3d& _R2, Vector3d& _N0, Vector3d& _V0, Vector& _func, Vector& _deriv, Vector& _deriv2, float _phi, float _phi_der, float _Phi, float _Phi_der)
    {
    ...
    void EmbeddedIsogeometricBeamElement::comp_dof_lin(Vector& _cur_var_n, Vector& _cur_var_v, Vector& _tor_var_n, Vector& _tor_var_v, Vector& _shear_var_n, Vector& _shear_var_v, Vector3d& _r1, Vector3d& _R1, Vector3d& _r2, Vector3d& _R2, Vector3d& _r3, Vector3d& _R3, Vector3d& _N0, Vector3d& _V0, Vector& _func, Vector& _deriv, Vector& _deriv2, Vector& _deriv3, float _phi, float _phi_der, float _phi_der2, float _Phi, float _Phi_der, float _Phi_der2)
    {
        //KRATOS_WATCH("EmbeddedIsogeometricBeamElement::comp_dof_lin");
        KRATOS_TRY
    ...
    void EmbeddedIsogeometricBeamElement::comp_dof_nln(Vector& _cur_var_n, Vector& _cur_var_v, Vector& _tor_var_n, Vector& _tor_var_v, Matrix& _cur_var_n_2, Matrix& _cur_var_v_2, Matrix& _tor_var_n_2, Matrix& _tor_var_v_2,  Vector3d& _r1, Vector3d& _R1, Vector3d& _r2, Vector3d& _R2, Vector3d& _N0, Vector3d& _V0, Vector& _func, Vector& _deriv, Vector& _deriv2, float _phi, float _phi_der, float _Phi, float _Phi_der)
    {
        KRATOS_TRY
        //const unsigned int N_Dof = this->GetGeometry().PointsNumber() * (this->GetGeometry().WorkingSpaceDimension() + 1);
        Vector3d t0_0 = this->GetProperties()[T_0];
    ...
    void EmbeddedIsogeometricBeamElement::stiff_mat_el_lin(const ProcessInfo& rCurrentProcessInfo, IndexType integration_point_index, Matrix& _gke, Vector& _gfie, float& _dL)
    {
    ...
    void EmbeddedIsogeometricBeamElement::mass_mat_el_lin(const ProcessInfo& rCurrentProcessInfo, Matrix& _me)   ///!!!! nodal thickness and relative denstiy is missing
    {
        const auto& r_geometry = GetGeometry();
