...
class TestTruss(KratosUnittest.TestCase):
    def setUp(self):
        pass
    ...
    def _apply_material_properties(self,mp,dim):
        ...
        mp.GetProperties()[0].SetValue(IGA.CROSS_AREA,0.01)
        mp.GetProperties()[0].SetValue(IGA.TRUSS_PRESTRESS_PK2,0)
    ...
    def test_truss_linear(self):
        ...
        #create Element
        mp.CreateNewElement("TrussElement", 1, [1,2], mp.GetProperties()[0])
        mp.CreateNewElement("TrussElement", 2, [2,3], mp.GetProperties()[0])
    ...
    def test_truss_nonlinear_material(self):
    ...
    def test_truss_prestress_nonlinear_fix(self):
        dim = 3
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        mp.ProcessInfo.SetValue(KratosMultiphysics.DOMAIN_SIZE, dim)
    ...
    def test_truss_prestress_nonlinear_free(self):
        dim = 3
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        mp.ProcessInfo.SetValue(KratosMultiphysics.DOMAIN_SIZE, dim)
    ...
    def test_truss_prestress_linear_fix(self):
        dim = 3
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        mp.ProcessInfo.SetValue(KratosMultiphysics.DOMAIN_SIZE, dim)
    ...
    def test_truss_prestress_linear_free(self):
        dim = 3
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        mp.ProcessInfo.SetValue(KratosMultiphysics.DOMAIN_SIZE, dim)
    ...
    def test_truss_dynamic_explicit_nonlinear(self):
        dim = 3
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        mp.ProcessInfo.SetValue(KratosMultiphysics.DOMAIN_SIZE, dim)
    ...
    def test_truss_dynamic_explicit_multi_stage_nonlinear(self):
        dim = 3
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        mp.ProcessInfo.SetValue(KratosMultiphysics.DOMAIN_SIZE, dim)
    ...
    def test_truss_dynamic_explicit_linear(self):
        dim = 3
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        mp.ProcessInfo.SetValue(KratosMultiphysics.DOMAIN_SIZE, dim)
    ...
    def test_truss_linear_plasticity(self):
        dim = 3
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        mp.ProcessInfo.SetValue(KratosMultiphysics.DOMAIN_SIZE, dim)
    ...
    def test_truss_nonlinear_plasticity(self):
        dim = 3
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        mp.ProcessInfo.SetValue(KratosMultiphysics.DOMAIN_SIZE, dim)
    ...
    def test_truss_nonlinear_plasticity_prestress(self):
        dim = 3
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        mp.ProcessInfo.SetValue(KratosMultiphysics.DOMAIN_SIZE, dim)
    ...
    def test_truss_multi_linear_elasticity(self):
        current_model = KratosMultiphysics.Model()
        mp = current_model.CreateModelPart("solid_part")
        self._add_variables(mp)
        ...
        strain_test = [0.0, 0.5394377058439297, 0.9223043541525537, 1.6897858441878664,
            2.5535355965600406, 3.378162557729854, 4.082858306729372, 4.457942841548205,
            4.816385796035347, 5.160781025490863]
        stress_test = [0.0, 1.0788754116878594, 1.8446087083051075, 2.229928614729289,
            2.5178451988533466, 2.792720852576618, 3.0828583067293724, 3.4579428415482054,
            3.8163857960353473, 4.160781025490863]



        while (time_i < time_end):
            #apply non-constant boundary conditions
            Force_i = force*time_i
            self._apply_Neumann_BCs(bcs_neumann,'x',Force_i)
            #solve + compare
            self._solve_nonlinear(mp)

            stress = mp.Elements[1].CalculateOnIntegrationPoints(KratosMultiphysics.PK2_STRESS_VECTOR,mp.ProcessInfo)
            strain = mp.Elements[1].CalculateOnIntegrationPoints(KratosMultiphysics.GREEN_LAGRANGE_STRAIN_VECTOR,mp.ProcessInfo)

            self.assertAlmostEqual(stress[0][0], stress_test[time_step],2)
            self.assertAlmostEqual(strain[0][0], strain_test[time_step],2)
