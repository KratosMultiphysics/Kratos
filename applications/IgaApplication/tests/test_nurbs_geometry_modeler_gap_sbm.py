...
class TestNurbsGeometryModelerGapSbm(KratosUnittest.TestCase):
    def test_quadrature_points_gap_sbm_on_circle(self):
        ...
        comp_mp = current_model.GetModelPart("IgaModelPart.GapElements")
        fig, ax = plt.subplots()
        gap_elem_centers = [elem.GetGeometry().Center() for elem in comp_mp.Elements]
        if gap_elem_centers:
            elem_x = [c[0] for c in gap_elem_centers]
            elem_y = [c[1] for c in gap_elem_centers]
        ax.scatter(elem_x, elem_y, s=3, c="tab:orange", marker="^", label="GapElements")
        gap_points = [(node.X, node.Y) for condition in support_mp.Conditions for node in condition.GetGeometry()]
        if gap_points:
            gap_x, gap_y = zip(*gap_points)
        ax.scatter(gap_x, gap_y, s=3, c="tab:red", label="GapInterfaces")
        print(f"Gap surrogate boundary sample: {gap_points[:5]}")
        skin_node_coords = [(node.X, node.Y) for node in skin_model_part.Nodes]
        if skin_node_coords:
            node_x, node_y = zip(*skin_node_coords)
        ax.scatter(node_x, node_y, s=3, c="tab:blue", marker="x", label="SkinNodes")
        print(f"Skin nodes sample: {skin_node_coords[:5]}")
        skin_condition_coords = [
        (node.X, node.Y) for condition in skin_model_part.Conditions for node in condition.GetGeometry()
        ]
        if skin_condition_coords:
            cond_x, cond_y = zip(*skin_condition_coords)
        ax.scatter(cond_x, cond_y, s=3, c="tab:green", marker="s", label="SkinConditions")
        print(f"Skin condition nodes sample: {skin_condition_coords[:5]}")
        ax.set_aspect("equal")
        ax.legend(loc="best")
        print(f"Gap interface conditions: {support_mp.NumberOfConditions()}")
        print(f"Skin model part: {skin_model_part.NumberOfNodes()} nodes, {skin_model_part.NumberOfConditions()} conditions")
