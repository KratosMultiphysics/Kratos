...
KRATOS_TEST_CASE_IN_SUITE(SnakeSbmUtilityInner, KratosIgaFastSuite)
{
    ...
    skin_model_part_inner_initial.CreateNewCondition("LineCondition2D2N", 7, {{7, 5}}, p_prop);
...
KRATOS_TEST_CASE_IN_SUITE(SnakeSbmUtilityInnerOuter, KratosIgaFastSuite)
{
    ...
    // Ensure the number of nodes matches expectation
    KRATOS_EXPECT_NEAR(surrogate_sub_model_part_inner.NumberOfNodes(), expected_coordinates_inner.size(), tolerance);
    KRATOS_EXPECT_NEAR(surrogate_sub_model_part_outer.NumberOfNodes(), expected_coordinates_outer.size(), tolerance);
