...
namespace Kratos
{
        ...
        const bool CalculateResidualVectorFlag)
    {
        ...
        const GeometryType::IntegrationPointsArrayType& integration_points
            = r_geometry_master.IntegrationPoints(integration_method);
        for (IndexType point_number = 0; point_number < integration_points.size(); point_number++)
        {
            ...
            for (unsigned int i = 0; i < number_of_nodes_master; i++) //loop over Lagrange Multipliers
            {
                ...
                {
                    double NN = N_master(point_number, j) * N_master(point_number, i);
                    ...
                    if (augmention_factor > 0.0)
                    {
                        ...
                        if (rLeftHandSideMatrix(jbase, ibase) <= 0.000001)
                            rLeftHandSideMatrix(i * 3, ibase) = augmention_factor;
                        if (rLeftHandSideMatrix(jbase + 1, ibase + 1) <= 0.000001)
                            rLeftHandSideMatrix(i * 3 + 1, ibase + 1) = augmention_factor;
                        if (rLeftHandSideMatrix(jbase + 2, ibase + 2) <= 0.000001)
                ...
                {
                    double NN = N_slave(point_number, j) * N_master(point_number, i);
            ...
            const double determinat_jacobian = r_geometry_master.DeterminatJacobian(point_number);
            ...
            if (CalculateResidualVectorFlag) {
                ...
                for (unsigned int i = 0; i < number_of_nodes_master; i++)
                {
                    const array_1d<double, 3> lagrange_multiplier = r_geometry_master[i].FastGetSolutionStepValue(VECTOR_LAGRANGE_MULTIPLIER);
...

    {
        ...
        if (rResult.size() != 3 * (2 * number_of_nodes_master + number_of_nodes_slave))
            rResult.resize(3 * (2 * number_of_nodes_master + number_of_nodes_slave), false);
        ...
        for (unsigned int i = 0; i < number_of_nodes_slave; ++i) {
            ...
            rResult[index]     = r_geometry_slave.GetDof(DISPLACEMENT_X).EquationId();
            rResult[index + 1] = r_geometry_slave.GetDof(DISPLACEMENT_Y).EquationId();
            rResult[index + 2] = r_geometry_slave.GetDof(DISPLACEMENT_Z).EquationId();
        }
        ...
        ProcessInfo& rCurrentProcessInfo)
    {
        ...
        rElementalDofList.reserve(3 * (2 * number_of_nodes_master + number_of_nodes_slave));
