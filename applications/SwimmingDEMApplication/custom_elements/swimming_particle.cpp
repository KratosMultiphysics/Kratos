...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ApplyNumericalAveragingWithOldForces(NodeType& node, array_1d<double, 3>& non_contact_nor_basset_force, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeBuoyancy(NodeType& node, array_1d<double, 3>& buoyancy, const array_1d<double, 3>& gravity, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeDragForce(NodeType& node, array_1d<double, 3>& drag_force, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeVirtualMassPlusUndisturbedFlowForce(NodeType& node, array_1d<double, 3>& virtual_mass_plus_undisturbed_flow_force, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::GetDaitcheCoefficient(int order, unsigned int n, unsigned int j, const double last_h_over_h, const int n_steps_per_quad_step)
{
    const int l = (int)(last_h_over_h * n_steps_per_quad_step + 0.5) - 1;

    if (order == 1){
        if (j < n){
            return SwimmingParticle<TBaseElement>::mAjs[n_steps_per_quad_step * j + l];
        }
        else {
            return SwimmingParticle<TBaseElement>::mBns[n_steps_per_quad_step * j + l];
        }
    }
        ...
        if (n > 3){
            if (j < n - 1){
                ...
                return SwimmingParticle<TBaseElement>::mCns[n_steps_per_quad_step * j + l];
            }
        }
        else {
            if (n == 1){ // use formula for the phis of first order
                SwimmingParticle<TBaseElement>::GetDaitcheCoefficient(1, n, j, last_h_over_h, n_steps_per_quad_step);
            }
        ...
        if (n > 6){
            if (j < n - 3){
                return SwimmingParticle<TBaseElement>::mAjs[j];
            }
            else if (j == n - 3){
                return SwimmingParticle<TBaseElement>::mBns[n];
            }
            else if (j == n - 2){
                return SwimmingParticle<TBaseElement>::mCns[n];
            }
            else if (j == n - 1){
                return SwimmingParticle<TBaseElement>::mDns[n];
            }
            else {
                return SwimmingParticle<TBaseElement>::mEns[n];
            }
        }

        else if (n == 2){ // use formula for the betas of second order
            SwimmingParticle<TBaseElement>::GetDaitcheCoefficient(2, n, j, last_h_over_h, n_steps_per_quad_step);
        }
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::CalculateExplicitFractionalDerivative(NodeType& node, array_1d<double, 3>& fractional_derivative,
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::Phi(const double x)
{
    if (std::abs(x) < 1e-10){
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::Ki(const double alpha, const double beta, const double time)
{
...
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::AddFdi(const int order, array_1d<double, 3>& F, const double t_win, const double alpha, const double beta, const double phi, const double dt, const DenseVector<double>& historic_integrands, const array_1d<double, 3>& oldest_integrand)
{
    if (order == 1){
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::AddFre(array_1d<double, 3>& old_Fi, const double beta, const double dt)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::AddHinsbergTailContribution(NodeType& node, array_1d<double, 3>& fractional_derivative_of_slip_vel, const int order, const int n_steps_per_quad_step, const double time, const double quadrature_delta_time, const double last_h_over_h, DenseVector<double>& historic_integrands)
{
    ...
    const double t_win = SwimmingParticle<TBaseElement>::mTimeWindow;
    ...
    if (n_steps_per_quad_step * last_h_over_h < 1.5 && 2 * n_steps_per_quad_step * (time - t_win) > quadrature_delta_time){ // first calculation after appending and already later than t_win
        ...
        const std::vector<double>& Ts = SwimmingParticle<SphericParticle>::mTs;
    ...
    for (int i = 0; i < m; i++){
        double ai = SwimmingParticle<TBaseElement>::mAs[i];
...
void SwimmingParticle<TBaseElement>::AddHinsbergTailContributionStrict(NodeType& node, array_1d<double, 3>& fractional_derivative_of_slip_vel, const int order, const int n_steps_per_quad_step, const double time, const double quadrature_delta_time, const double last_h_over_h, DenseVector<double>& historic_integrands)
{
    ...
    array_1d<double, 3> F_tail = ZeroVector(3);
    const std::vector<double>& As = SwimmingParticle<TBaseElement>::mAs;
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeBassetForce(NodeType& node, array_1d<double, 3>& basset_force, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeSaffmanLiftForce(NodeType& node, array_1d<double, 3>& lift_force, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeMagnusLiftForce(NodeType& node, array_1d<double, 3>& lift_force, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeHydrodynamicTorque(NodeType& node, array_1d<double, 3>& hydro_torque, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeBrownianMotionForce(NodeType& node, array_1d<double, 3>& brownian_motion_force, const ProcessInfo& r_current_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeParticleReynoldsNumber(double& reynolds)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputePowerLawParticleReynoldsNumber(double& reynolds,
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeNondimensionalRotVelocity(const array_1d<double, 3>& slip_rot_velocity)
{
    if (mNormOfSlipVel > 0){
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeParticleRotationReynoldsNumber(double norm_of_slip_rot, double& reynolds)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeParticleRotationReynoldsNumberOverNormOfSlipRot(double& reynolds)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeParticleAccelerationNumber(const array_1d<double, 3>& slip_acc, double& acc_number)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::Calculate(const Variable<array_1d<double, 3 > >& rVariable, array_1d<double, 3 > & Output, const ProcessInfo& r_process_info)
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeDragCoefficient(const ProcessInfo& r_current_process_info)
{
    double drag_coeff;
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeStokesDragCoefficient()
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeWeatherfordDragCoefficient(const ProcessInfo& r_current_process_info)
{
    KRATOS_TRY
...
//**************************************************************************************************************************************************
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::CalculateNewtonianDragCoefficient(int non_newtonian_option,
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::CalculateShahsTerm(double power_law_N,
                                                   double power_law_K,
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeGanserDragCoefficient(const ProcessInfo& r_current_process_info)
{
    KRATOS_TRY
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeIshiiDragCoefficient(const ProcessInfo& r_current_process_info)
{
    double coeff = 0.45;
    double reynolds;
    ComputeParticleReynoldsNumber(reynolds);
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeNewtonRegimeDragCoefficient()
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeIntermediateRegimeDragCoefficient()
{
    double reynolds;
    double drag_coeff  = 0.5 * Globals::Pi * SWIMMING_POW_2(mRadius) * mFluidDensity * mNormOfSlipVel;
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeHaiderDragCoefficient()
{
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeBeetstraDragCoefficient()
{
    double drag_coeff;
    double particle_reynolds;
    ComputeParticleReynoldsNumber(particle_reynolds);
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeShahDragCoefficient(const ProcessInfo& r_current_process_info, const bool use_shahi_correction)
{
...
template < class TBaseElement >
void SwimmingParticle<TBaseElement>::ComputeGanserParameters(const int isometric_shape, const double dn, double& k_1, double& k_2)
{
     if (isometric_shape){
...
//**************************************************************************************************************************************************
template < class TBaseElement >
double SwimmingParticle<TBaseElement>::ComputeElSamniLiftCoefficient(const double norm_of_shear_rate,
...
//**************************************************************************************************************************************************
template < class TBaseElement >
 double SwimmingParticle<TBaseElement>::ComputeMeiLiftCoefficient(const double reynolds, const double reynolds_shear)
 {
...
template <typename T> std::vector<double> SwimmingParticle<T>::mAjs;
template <typename T> std::vector<double> SwimmingParticle<T>::mBns;
template <typename T> std::vector<double> SwimmingParticle<T>::mCns;
template <typename T> std::vector<double> SwimmingParticle<T>::mDns;
template <typename T> std::vector<double> SwimmingParticle<T>::mEns;
template <typename T> std::vector<double> SwimmingParticle<T>::mAs;
template <typename T> std::vector<double> SwimmingParticle<T>::mTs;
template <typename T> std::vector<double> SwimmingParticle<T>::mAlphas;
template <typename T> std::vector<double> SwimmingParticle<T>::mBetas;
template <typename T> double SwimmingParticle<T>::mTimeWindow;
template <typename T> bool SwimmingParticle<T>::mDaitcheVectorsAreFull;

// Instantiation
#define INSTANTIATE_SPHERIC_SWIMMING(_T)                            \
template std::vector<double> SwimmingParticle<_T>::mAjs;     \
template std::vector<double> SwimmingParticle<_T>::mBns;     \
template std::vector<double> SwimmingParticle<_T>::mCns;     \
template std::vector<double> SwimmingParticle<_T>::mDns;     \
template std::vector<double> SwimmingParticle<_T>::mEns;     \
template std::vector<double> SwimmingParticle<_T>::mAs;      \
template std::vector<double> SwimmingParticle<_T>::mTs;      \
template std::vector<double> SwimmingParticle<_T>::mAlphas;  \
template std::vector<double> SwimmingParticle<_T>::mBetas;   \
template double SwimmingParticle<_T>::mTimeWindow;           \
template bool SwimmingParticle<_T>::mDaitcheVectorsAreFull;
