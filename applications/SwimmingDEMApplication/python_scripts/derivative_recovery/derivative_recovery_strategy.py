...
class DerivativeRecoveryStrategy:
    def __init__(self, parameters, fluid_model_part, custom_functions_tool = None):
        ...
        self.mat_deriv_type = self.project_parameters["material_acceleration_calculation_type"].GetInt()
        self.laplacian_type = self.project_parameters["laplacian_calculation_type"].GetInt()
        self.vorticity_type = self.project_parameters["vorticity_calculation_type"].GetInt()
        self.pressure_grad_type = self.project_parameters["pressure_grad_recovery_type"].GetInt()
        self.fluid_fraction_grad_type = self.project_parameters["fluid_fraction_grad_type"].GetInt()

        self.do_pre_recovery = False
        self.must_reconstruct_gradient = self.laplacian_type in {0, 3, 4, 5, 6} and self.mat_deriv_type in {3, 4}

        if self.project_parameters["fluid_already_calculated"].GetBool(): # the fluid has been calculated before, and the derivatives fed to the fluid_model_part
            self.pre_computed_derivatives = self.project_parameters["load_derivatives"].GetBool()
        else:
    ...
    def GetMatDerivTool(self):
        if self.pre_computed_derivatives:
            return recoverer.MaterialAccelerationRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.mat_deriv_type == 0:
            return recoverer.EmptyGradientRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.mat_deriv_type == 1:
            return standard_recoverer.StandardMaterialAccelerationRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.mat_deriv_type == 2:
            if self.laplacian_type == 2:
                return zhang_guo_recoverer.ZhangGuoMaterialAccelerationAndLaplacianRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
            else:
                return zhang_guo_recoverer.ZhangGuoMaterialAccelerationRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.mat_deriv_type == 3:
            return L2_projection_recoverer.L2ProjectionDirectMaterialAccelerationRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.mat_deriv_type == 4:
            return L2_projection_recoverer.L2ProjectionMaterialAccelerationRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.mat_deriv_type == 5:
            return L2_projection_recoverer.L2ProjectionMaterialAccelerationRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.mat_deriv_type == 6:
            return pouliot_2012_edge_recoverer.Pouliot2012EdgeMaterialAccelerationRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
            #return pouliot_2012_recoverer.Pouliot2012MaterialAccelerationRecoverer(self.project_parameters, self.pp, self.fluid_model_part, self.do_pre_recovery)
            ...
            return lagrangian_recoverer.LagrangianMaterialAccelerationRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        else:
    ...
    def GetLaplacianTool(self):
        if self.laplacian_type == 0 or self.pre_computed_derivatives:
            return recoverer.EmptyLaplacianRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.laplacian_type == 1:
            return standard_recoverer.StandardLaplacianRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.laplacian_type == 2:
            if self.mat_deriv_type == 2:
                return zhang_guo_recoverer.ZhangGuoMaterialAccelerationAndLaplacianRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
            else:
                return zhang_guo_recoverer.ZhangGuoDirectLaplacianRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.laplacian_type in {3, 4, 5, 6}:
            if self.store_full_gradient:
                return L2_projection_recoverer.L2ProjectionLaplacianRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
            else:
    ...
    def GetVorticityTool(self):
        if self.pre_computed_derivatives:
            return recoverer.VorticityRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        if self.vorticity_type == 0:
            return recoverer.EmptyVorticityRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
    ...
    def GetVelocityGradTool(self):
        ...
        if self.must_reconstruct_gradient:
            if self.mat_deriv_tool == 6:
                return pouliot_2012_edge_recoverer.Pouliot2012EdgeGradientRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
                # return pouliot_2012_recoverer.Pouliot2012GradientRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
            else:
                return L2_projection_recoverer.L2ProjectionGradientRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        else:
            return recoverer.EmptyGradientRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
    ...
    def GetPressureGradTool(self):
        if self.pressure_grad_type == 0:
            ...
            return standard_recoverer.StandardGradientRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        elif self.pressure_grad_type == 2:
            return zhang_guo_recoverer.ZhangGuoGradientRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
        else:
    ...
    def Recover(self):
        ...
        # standard = standard_recoverer.StandardMaterialAccelerationRecoverer(self.project_parameters, self.pp, self.fluid_model_part)
