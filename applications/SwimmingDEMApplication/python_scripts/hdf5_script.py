...
class ErrorProjectionPostProcessTool(object):
    ...
    def WriteData(self, model_part, velocity_L2_error_norm, pressure_L2_error_norm, velocity_H1_error_seminorm, pressure_H1_error_seminorm, projection_type, model_type, subscale_type, porosity_mean, n_iterations, max_iteration, relax_alpha, lowest_alpha, damkohler, omega,reynolds_number, viscous_projector):
        ...
        #element_size = [Element.GetGeometry().Length() for Element in self.model_part.Elements]
        element_size = []
        for Element in self.model_part.Elements:
            element_size.append(Element.GetGeometry().Length())
            break

        # for elem in self.model_part.Elements:

        #     #Quadratic
        #     x10 = elem.GetGeometry()[1].X - elem.GetGeometry()[0].X
        #     y10 = elem.GetGeometry()[1].Y - elem.GetGeometry()[0].Y
        #     z10 = elem.GetGeometry()[1].Z - elem.GetGeometry()[0].Z

        #     x20 = elem.GetGeometry()[2].X - elem.GetGeometry()[0].X
        #     y20 = elem.GetGeometry()[2].Y - elem.GetGeometry()[0].Y
        #     z20 = elem.GetGeometry()[2].Z - elem.GetGeometry()[0].Z

        #     x30 = elem.GetGeometry()[3].X - elem.GetGeometry()[0].X
        #     y30 = elem.GetGeometry()[3].Y - elem.GetGeometry()[0].Y
        #     z30 = elem.GetGeometry()[3].Z - elem.GetGeometry()[0].Z
        #     # Average Element size
        #     detJ = x10 * y20 * z30 - x10 * y30 * z20 + y10 * z20 * x30 - y10 * x20 * z30 + z10 * x20 * y30 - z10 * y20 * x30
        #     element_size.append((detJ/6)**(1/3))
        #self.element_size = np.mean(elem_size)
        self.element_size = np.max(element_size)

        self.reynolds_number = reynolds_number

        self.n_iterations.append(n_iterations)
        self.time.append(self.model_part.ProcessInfo[Kratos.TIME])
    ...
    def WriteDataToFile(self, file_or_group, names, data):
        ...
        self.sub_group.attrs['n_elements'] = str(len(self.model_part.Elements))
        self.sub_group.attrs['delta_time'] = str(self.model_part.ProcessInfo[Kratos.DELTA_TIME])
...
class KineticEnergy(object):
    ...
    def WriteData(self, fluid_model_part, model_type, amean, time):
        ...
        vel = [elem.CalculateOnIntegrationPoints(KratosMultiphysics.VELOCITY, process_info) for elem in self.fluid_model_part.Elements]
        ff = [elem.CalculateOnIntegrationPoints(KratosMultiphysics.FLUID_FRACTION, process_info) for elem in self.fluid_model_part.Elements]
        vort = [elem.CalculateOnIntegrationPoints(KratosMultiphysics.VORTICITY, process_info) for elem in self.fluid_model_part.Elements]

        ngauss = len(ff[0])
        Agauss = A/ngauss

        total_volume = sum([Agauss*ff[elem][g] for g in range(0, ngauss) for elem in range(0,len(vel))])
        fke = sum([0.5*Agauss*density*ff[elem][g]*(vel[elem][g][0]**2 + vel[elem][g][1]**2 + vel[elem][g][2]**2) for g in range(0, ngauss) for elem in range(0,len(vel))])
        dis_rate = sum([density*ff[elem][g]*viscosity*Agauss*(vort[elem][g][0]**2 + vort[elem][g][1]**2 + vort[elem][g][2]**2) for g in range(0, ngauss) for elem in range(0,len(vel))])

        # for elem in self.fluid_model_part.Elements:
        #     v_tmp = elem.CalculateOnIntegrationPoints(Kratos.VELOCITY, process_info)
        #     ff_tmp = elem.CalculateOnIntegrationPoints(Kratos.FLUID_FRACTION, process_info)
        #     w_tmp = elem.CalculateOnIntegrationPoints(Kratos.VORTICITY, process_info)
        #     ngauss = len(v_tmp)
        #     A = elem.GetGeometry().Area()
        #     Agauss = A/ngauss
        #     for i in range(0, ngauss):
        #         mod_sqr_velocity = v_tmp[i][0]**2 + v_tmp[i][1]**2 + v_tmp[i][2]**2
        #         mod_sqr_vorticity = w_tmp[i][0]**2 + w_tmp[i][1]**2 + w_tmp[i][2]**2
        #         fke += 0.5 * density * ff_tmp[i] * mod_sqr_velocity * Agauss
        #         dis_rate += Agauss * density * mod_sqr_vorticity * viscosity
        #         total_volume += Agauss*ff_tmp[i]
        ...
        self.kinetic_energy_dissipation_rate.append(dis_rate/total_volume)
