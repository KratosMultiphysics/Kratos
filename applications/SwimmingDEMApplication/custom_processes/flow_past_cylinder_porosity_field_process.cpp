...
void FlowPastCylinderPorosityFieldProcess::SetPorosityField()
{
    ...
    for (auto it_node = mrModelPart.NodesBegin(); it_node != mrModelPart.NodesEnd(); it_node++){
        ...
        const double velocity_norm = std::sqrt(std::pow(u1,2) + std::pow(u2,2));
        ...
        const double p_diameter = 0.01;
        const double drag_coef = 0.44;
        if (mAlternativeFormulation)
            r_sigma = (3.0/4.0*drag_coef/p_diameter*(1.0-r_alpha)*velocity_norm)*I;
        else
            r_sigma = ((3.0/4.0*drag_coef/p_diameter*(1.0-r_alpha)*velocity_norm)/r_alpha)*I;
...
void FlowPastCylinderPorosityFieldProcess::SetValuesOnIntegrationPoints()
{
    ...
    for (int i_elem = 0; i_elem < static_cast<int>(mrModelPart.NumberOfElements()); ++i_elem){
        ...
        for (unsigned int g = 0; g < r_number_integration_points; g++){
            ...
            reaction_on_gauss_points[g] = ZeroMatrix(3, 3);
            ...
            if (mAlternativeFormulation)
                reaction = 3.0/4.0*drag_coef/p_diameter*(1.0-alpha)*velocity_norm;
            else
                reaction = 3.0/4.0*drag_coef/p_diameter*(1.0-alpha)*velocity_norm/alpha;

            reaction_on_gauss_points[g](0,0) = reaction;
            reaction_on_gauss_points[g](0,1) = 0.0;
            reaction_on_gauss_points[g](1,0) = 0.0;
            reaction_on_gauss_points[g](1,1) = reaction;
        ...
        it_elem->SetValuesOnIntegrationPoints(PERMEABILITY, reaction_on_gauss_points, mrModelPart.GetProcessInfo());
