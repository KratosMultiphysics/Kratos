...
namespace Kratos {
                                       ...
                                       const ProcessInfo& r_current_process_info)
    {
        ...
        const array_1d<double, 3> minus_slip_mean_velocity = fluid_vel - mean_particle_vel;
        const double norm_minus_slip_velocity = SWIMMING_MODULUS_3(minus_slip_velocity);
        const double norm_minus_slip_mean_velocity = SWIMMING_MODULUS_3(minus_slip_mean_velocity);
        ...
        double drag_force_magnitude = 1.0 / 8.0 * drag_coeff * fluid_density  * Globals::Pi * std::pow(2.0*particle_radius, 2) * norm_minus_slip_velocity *  std::pow(eps, 1.0 - chi);
        noalias(drag_force) = drag_force_magnitude * (minus_slip_mean_velocity) / norm_minus_slip_mean_velocity;
    }
