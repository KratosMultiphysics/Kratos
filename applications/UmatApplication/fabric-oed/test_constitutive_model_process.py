...
class TestConstitutiveModelProcess(KratosMultiphysics.Process):
    ...
    def CalculateMaterialResponse(self):
        ...
        self.strain_vector[0] = 0.0;
        self.strain_vector[1] = 0.0;
        self.strain_vector[2] = 0.0;

        for t in range(0, nSize):
            sign = 1.0;
            if ( float(t) / float(nSize) > 0.95):
                sign = -1.0
            self.strain_vector[0] = self.strain_vector[0] -  final/ float(nSize) * sign
            #self.strain_vector[1] = self.strain_vector[1] -  final / float(nSize) * sign
            #self.strain_vector[2] = self.strain_vector[2] -  final / float(nSize) * sign
            self.parameters.SetStrainVector( self.strain_vector )
            ...
            pStrain[t] = self.strain_vector[0];
            pStress[t] = stress[0]


        plt.plot( -pStress, pStrain, 'ro-')
        plt.show()
