...
for normalvar in range(normal_combs):
    ...
    for dim, nnodes, nnodes_master in zip(dim_combinations, nnodes_combinations, nnodes_master_combinations):
        ...
        rhs_string = ""
        output_count += 1
        number_dof = dim * (nnodes_master + nnodes) + nnodes
        ...
        custom_sympy_fe_utilities.CreateVariableVectorList(lm_var, LMNormal)
        custom_sympy_fe_utilities.CreateVariableVectorList(u1_lm_var, LMNormal)
        all_var=u12_var.copy()
        custom_sympy_fe_utilities.CreateVariableVectorList(all_var, LMNormal)
        ...
        if output_count == 1:
            first_input = open("ALM_frictionless_mortar_contact_condition_template.cpp", 'r').read()
