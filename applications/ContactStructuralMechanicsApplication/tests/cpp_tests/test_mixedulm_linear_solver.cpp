...
namespace Kratos
{
    namespace Testing
    {
        ...
        void CreateAuxiliarFiles()
        {
            ...
            buffer_lhs.open(FileSystem::JoinPaths({FileSystem::CurrentPath(), "A_testing_condensation.mm"}),std::ios::out);
            ...
            buffer_rhs.open(FileSystem::JoinPaths({FileSystem::CurrentPath(), "b_testing_condensation.rhs"}),std::ios::out);
...
//             LinearSolverType::Pointer psolver = LinearSolverType::Pointer( new AMGCLSolverType(empty_parameters) );
            ...
            CreateAuxiliarFiles();
            const bool read_a = Kratos::ReadMatrixMarketMatrix(FileSystem::JoinPaths({FileSystem::CurrentPath(), "A_testing_condensation.mm"}).c_str(), A);
            const bool read_b = Kratos::ReadMatrixMarketVector(FileSystem::JoinPaths({FileSystem::CurrentPath(), "b_testing_condensation.rhs"}).c_str(), b);

            // Removing files
            FileSystem::Remove(FileSystem::JoinPaths({FileSystem::CurrentPath(), "A_testing_condensation.mm"}));
            FileSystem::Remove(FileSystem::JoinPaths({FileSystem::CurrentPath(), "b_testing_condensation.rhs"}));
