...
namespace Kratos
{
    namespace Testing
    {
        ...
        void CreateAuxiliarFiles()
        {
            ...
            std::error_code ec;
            buffer_lhs.open((Kratos::filesystem::current_path(ec)).string() + "/A_testing_condensation.mm",std::ios::out);
            ...
            buffer_rhs.open((Kratos::filesystem::current_path(ec)).string() + "/b_testing_condensation.rhs",std::ios::out);
...
//             LinearSolverType::Pointer psolver = LinearSolverType::Pointer( new AMGCLSolverType(empty_parameters) );
            ...
            CreateAuxiliarFiles();
            std::error_code ec;
            const bool read_a = Kratos::ReadMatrixMarketMatrix(((Kratos::filesystem::current_path(ec)).string() + "/A_testing_condensation.mm").c_str(), A);
            const bool read_b = Kratos::ReadMatrixMarketVector(((Kratos::filesystem::current_path(ec)).string() + "/b_testing_condensation.rhs").c_str(), b);

            // Removing files
            remove(((Kratos::filesystem::current_path(ec)).string() + "/A_testing_condensation.mm").c_str());
            remove(((Kratos::filesystem::current_path(ec)).string() + "/b_testing_condensation.rhs").c_str());
