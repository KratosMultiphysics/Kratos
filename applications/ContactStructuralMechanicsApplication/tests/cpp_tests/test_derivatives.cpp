...
namespace Kratos 
{
    namespace Testing 
    {
            ...
            )
        {
            ...
            {
                for (unsigned int i_per = 0; i_per < NodesPerturbation.size(); ++i_per)
                {
        ...
        KRATOS_TEST_CASE_IN_SUITE(TestShapeFunctionDerivativesTriangle4, ContactStructuralApplicationFastSuite)
        {
            ModelPart model_part("Main");
            model_part.SetBufferSize(2);
        ...
        KRATOS_TEST_CASE_IN_SUITE(TestNormalDerivativesTriangle2, ContactStructuralApplicationFastSuite)
        {
            ModelPart model_part("Main");
            model_part.SetBufferSize(2);
            model_part.GetProcessInfo()[CONSIDER_NORMAL_VARIATION] = static_cast<int>(NODAL_ELEMENTAL_DERIVATIVES);
