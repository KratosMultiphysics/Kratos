    ...
    for (IndexType iter = 0; iter < NumberIterations; ++iter) {
        ...
        if (is_inside && is_inside0) {
            ...
            if (conditions_points_slave.size() == conditions_points_slave0.size()) {// Just in case we have the "same configuration"
                DerivativesUtilitiesType::CalculateAeAndDeltaAe(r_slave_geometry_1, r_normal_slave_1, MasterCondition1->GetGeometry(), r_normal_master_1, rDerivativeData, rVariables, consider_normal_variation, conditions_points_slave, this_integration_method, 1.0);
                DerivativesUtilitiesType::CalculateAeAndDeltaAe(r_slave_geometry_0, r_normal_slave_0, MasterCondition0->GetGeometry(), r_normal_master_0, rDerivativeData0, rVariables0, consider_normal_variation, conditions_points_slave0, this_integration_method, 1.0);
