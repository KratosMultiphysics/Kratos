...
class TestMortarMapping(KratosUnittest.TestCase):
    ...
    def __base_test_mapping(self, input_filename, num_nodes):
        ...
        for node in self.model_part_slave.Nodes:
            ...
            y = node.Y 
            z = node.Z
            node.SetSolutionStepValue(KratosMultiphysics.PRESSURE, z)
            node.SetSolutionStepValue(KratosMultiphysics.VELOCITY_X, x)
            node.SetSolutionStepValue(KratosMultiphysics.VELOCITY_Y, y)
