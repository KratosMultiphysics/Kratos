        ...
        ) override
    {
        if (SparseSpaceType::Size(rb) != 0) { //if we are solving for something
            ...
            if (!r_process_info[ACTIVE_SET_COMPUTED]) {
                const array_1d<std::size_t, 2> is_converged = ActiveSetUtilities::ComputeALMFrictionalActiveSet(rModelPart, mOptions.Is(DisplacementLagrangeMultiplierMixedFrictionalContactCriteria::PURE_SLIP));
