...
for dim, nnodes in zip(dim_combinations, nnodes_combinations):
    ...
    for active_inactive in active_inactive_combinations: # Change output in combination of this
        ...
        DOperator = DefineDofDependencyMatrix(DOperator, u1_var)
        ...
        for node in range(nnodes):
            ...
            if (active == 1):
                rv_galerkin += ((scale_factor * lmnormal[node] + penalty_parameter * gap[node]) * wgap[node] + scale_factor * gap[node] * wlmnormal[node])
            else:
        ...
        var_strings, var_strings_subs, var_strings_aux_subs, der_var_strings, der_var_list = DefineVariableLists(DOperator, "DOperator", "DOperator", u1_var, var_strings, var_strings_subs, var_strings_aux_subs, der_var_strings, der_var_list, "matrix")
