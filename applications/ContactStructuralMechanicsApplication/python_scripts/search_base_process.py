...
class SearchBaseProcess(KM.Process):
    ...
    def ExecuteInitialize(self):
        ...
        for key in self.settings["search_model_part"].keys():
            if self.settings["search_model_part"][key].size() > 0:
                ...
                self.search_utility[key].CheckContactModelParts()
                self.search_utility[key].CreatePointListMortar()
                self.search_utility[key].InitializeMortarConditions()
    ...
    def ExecuteInitializeSolutionStep(self):
        ...
        if self._compute_search():
            ...
            for key in self.settings["search_model_part"].keys():
                if self.settings["search_model_part"][key].size() > 0:
                    # Clear current pairs
                    self.search_utility[key].ClearMortarConditions()
                    # Update database
                    self.search_utility[key].UpdateMortarConditions()
                    #self.search_utility[key].CheckMortarConditions()
    ...
    def _create_main_search(self, key = "0"):
        ...
        if self.dimension == 2:
            if simple_search:
                self.search_utility[key] = CSMA.SimpleContactSearch2D2N(self.computing_model_part, search_parameters)
            else:
                self.search_utility[key] = CSMA.AdvancedContactSearch2D2N(self.computing_model_part, search_parameters)
        else:
            if number_nodes == 3:
                if number_nodes_master == 3:
                    if simple_search:
                        self.search_utility[key] = CSMA.SimpleContactSearch3D3N(self.computing_model_part, search_parameters)
                    else:
                        self.search_utility[key] = CSMA.AdvancedContactSearch3D3N(self.computing_model_part, search_parameters)
                else:
                    if simple_search:
                        self.search_utility[key] = CSMA.SimpleContactSearch3D3N4N(self.computing_model_part, search_parameters)
                    else:
                        self.search_utility[key] = CSMA.AdvancedContactSearch3D3N4N(self.computing_model_part, search_parameters)

            elif number_nodes == 4:
                if number_nodes_master == 3:
                    if simple_search:
                        self.search_utility[key] = CSMA.SimpleContactSearch3D4N3N(self.computing_model_part, search_parameters)
                    else:
                        self.search_utility[key] = CSMA.AdvancedContactSearch3D4N3N(self.computing_model_part, search_parameters)
                else:
                    if simple_search:
                        self.search_utility[key] = CSMA.SimpleContactSearch3D4N(self.computing_model_part, search_parameters)
                    else:
                        self.search_utility[key] = CSMA.AdvancedContactSearch3D4N(self.computing_model_part, search_parameters)
            else:
