    ...
    )
{
    ...
    if (aux_index > 0) {
        ...
        const auto& DeltaMOperator = rMortarConditionMatrices.DeltaMOperator[IndexDerivative];
        const auto& DeltaDOperator = rMortarConditionMatrices.DeltaDOperator[IndexDerivative];
        ...
        const BoundedMatrix<double, TNumNodes, TDim> D_Deltax1_M_Deltax2 = prod(DOperator, Deltax1) - prod(MOperator, Deltax2);
        ...
        // Adding delta contribution
        const BoundedMatrix<double, TNumNodes, TDim> DeltaD_x1_DeltaM_x2 = prod(DeltaDOperator, x1) - prod(DeltaMOperator, x2);
