...
BOOST_PYTHON_MODULE(KratosContactStructuralMechanicsApplication)
{
    ...
    KRATOS_REGISTER_IN_PYTHON_VARIABLE( PENALTY_FACTOR )               // The constant that is considered for the check if the node is slip/stick
    KRATOS_REGISTER_IN_PYTHON_VARIABLE( SCALE_FACTOR )                 // The constant that is considered for the check if the node is slip/stick
