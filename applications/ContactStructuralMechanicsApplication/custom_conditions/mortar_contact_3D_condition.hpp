...
/** \brief MortarContact3DCondition
 * This is a 3D contact condition which employes the mortar method with double lagrange multiplier 
...
protected:
    ...
    */
    ...
    public:
        ...
        // Integration point gap and normal (aka. discrete gap in Popp's thesis)
        double IntegrationPointNormalGap;
        Vector IntegrationPointNormalVector;
        
        // Slave jacobian and its determinant
        Matrix j_Slave;
        double DetJSlave;
        ...
        void Initialize( 
            const unsigned int& rNumberOfMasterNodes     = 3,  // 3-node triangle
            const unsigned int& rNumberOfSlaveNodes      = 3,  // 3-node triangle
            ...
            )
        {
            ...
            j_Slave = ZeroMatrix( rNumberOfSlaveNodes, local_dimension_slave );
            DetJSlave = 0.0;
            
            // Master jacobian
            j_Master = ZeroMatrix( rNumberOfMasterNodes, local_dimension_master );
        }
    ...
    */
    ...
    public:
        ...
        */
        ...
        Matrix DeltaXiMasterIntegrationPoint; // DeltaXig_master
        Matrix DeltaEtaMasterIntegrationPoint; // DeltaEtag_master
        ...
        */
        // Initializer method
        void Initialize( 
            const unsigned int& rNumberOfMasterNodes,
            const unsigned int& rNumberOfSlaveNodes, 
            const unsigned int& rDimension = 3  // 3D physical space
            )
        {
            ...
            // Integration Point Directional Derivative in xi and eta
            if ( DeltaXiMasterIntegrationPoint.size1() != rDimension || DeltaXiMasterIntegrationPoint.size2() != ( size1 + size2 ) )
            {
                DeltaXiMasterIntegrationPoint.resize( rDimension, size1 + size2 , false );
            }
            noalias(DeltaXiMasterIntegrationPoint) = ZeroMatrix( rDimension, size1 + size2 );
            
            if ( DeltaEtaMasterIntegrationPoint.size1() != rDimension || DeltaEtaMasterIntegrationPoint.size2() != ( size1 + size2 ) )
            {
                DeltaEtaMasterIntegrationPoint.resize( rDimension, size1 + size2 , false );
            }
            noalias(DeltaEtaMasterIntegrationPoint) = ZeroMatrix( rDimension, size1 + size2 );
        ...
        void print()
        {
            ...
            KRATOS_WATCH(DeltaXiMasterIntegrationPoint);
            KRATOS_WATCH(DeltaEtaMasterIntegrationPoint);
            KRATOS_WATCH(N);
        }
    };
