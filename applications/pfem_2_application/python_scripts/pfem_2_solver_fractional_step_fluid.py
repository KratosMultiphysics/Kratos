...
class PFEM2Solver:
	...
	self.viscosity_solver = strategy_python.SolvingStrategyPython(self.model_part,self.time_scheme,self.viscosity_linear_solver,self.conv_criteria,CalculateReactionFlag,ReformDofSetAtEachStep,MoveMeshFlag)
    ...
    #######################################################################   
    def Solve(self):
		add_gravity=True #in the full xivas solver we DO add the gravity, and it is not added later in the solving of implicit systems.
		...
		(self.VariableUtils).CopyVectorVar(MESH_VELOCITY,FRACT_VEL,self.model_part.Nodes)
		#self.CalculateExplicitPressureViscousCorrection()
		#pressure iterations
		if (calculate_viscous_contribution_after_everything==True):
		...
		if (calculate_viscous_contribution_after_everything==False):
			t8=timer.time()
			self.model_part.ProcessInfo.SetValue(FRACTIONAL_STEP, 4)
			...
			#self.CalculateExplicitPressureViscousCorrection()
			t9=timer.time()
			self.implicitviscosity = self.implicitviscosity + t9-t8
			number_of_pressure_iterations=3
			for i in range(0,number_of_pressure_iterations):
			#for i in range(1,number_of_pressure_iterations+1):
				print "pressure iteration number after viscous step: " , i+1
				self.CalculatePressureIteration(i+1)
				#(self.moveparticles).ResetBoundaryConditions() 
