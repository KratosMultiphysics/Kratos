...
# print "Initialization monolithic solver finished"
    ...
    def Remesh(self):
        ...
        for node in (self.model_part).Nodes: 
            ...
            node.SetSolutionStepValue(NODAL_H,0,0.006) #fin
            node.SetSolutionStepValue(NODAL_H,0,0.060) #fin
            node.SetSolutionStepValue(NODAL_H,0,0.0011) #fin
...
#        (self.Pfem2_apply_bc_process).Execute();
        if (self.domain_size == 2):
            ...
            (self.Mesher).ReGenerateMesh("LagrangianFluidVMS3D","Condition3D", self.model_part, self.node_erase_process, False, False, self.alpha_shape, h_factor)    
