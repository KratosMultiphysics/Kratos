...
#include "custom_elements/2fluid_discontinuousvelocity_2d.h"
...
namespace Kratos
{
	...
	//************************************************************************************
	//************************************************************************************
	PFEM2DiscontinuousVelocity2D::PFEM2DiscontinuousVelocity2D(IndexType NewId, GeometryType::Pointer pGeometry)
	...
	//************************************************************************************
	//************************************************************************************
	PFEM2DiscontinuousVelocity2D::PFEM2DiscontinuousVelocity2D(IndexType NewId, GeometryType::Pointer pGeometry,  PropertiesType::Pointer pProperties)
		: Element(NewId, pGeometry, pProperties)
	{

	}

	Element::Pointer PFEM2DiscontinuousVelocity2D::Create(IndexType NewId, NodesArrayType const& ThisNodes,  PropertiesType::Pointer pProperties) const
	{
		return Element::Pointer(new PFEM2DiscontinuousVelocity2D(NewId, GetGeometry().Create(ThisNodes), pProperties));
	}

	PFEM2DiscontinuousVelocity2D::~PFEM2DiscontinuousVelocity2D()
	{
	}
	...
	void PFEM2DiscontinuousVelocity2D::AddExplicitContribution(ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY;
	...
	void PFEM2DiscontinuousVelocity2D::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix,
	...
	void PFEM2DiscontinuousVelocity2D::EquationIdVector(EquationIdVectorType& rResult,
												ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY;
	...
	void PFEM2DiscontinuousVelocity2D::GetDofList(DofsVectorType& rElementalDofList,
										  ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY;
	...
	void PFEM2DiscontinuousVelocity2D::CalculateLocalFractionalVelocitySystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY
		...
		else //split element:
		{
			...
			boost::numeric::ublas::bounded_matrix<double,3, 3> Ndiscontinuous;
			...
			std::vector< Matrix > gauss_gradients_discontinuous(3);
			//fill coordinates
			...
			//now the discontinous shape functions. WE REPLACE SOME OF THE VALUES CALCULATED PREVIOUSLY(ENRICHED FUNCTIONS) SINCE THE SPLITTING ALGORITHM MATCHES; SO NO NEED TO HAVE 2 COPIES
			for (unsigned int i = 0; i < 3; i++)
				gauss_gradients_discontinuous[i].resize(3, 3, false);  //3 values of the 3 shape functions, and derivates in (xy) direction).	
			unsigned int ndivisions = DiscontinuousShapeFunctionsUtilities::CalculateDiscontinuousShapeFunctions(coords, DN_DX, distances, volumes, Ngauss, signs, gauss_gradients_discontinuous, Ndiscontinuous);	
			...
			//this->AddViscousTerm(rLeftHandSideMatrix , gauss_gradients_discontinuous , viscosities, volumes, ndivisions );
			//this->AddViscousTerm(rLeftHandSideMatrix,DN_DX, viscosity_air, viscosity_water, volumes, signs, Ngauss, Area);
			KRATOS_ERROR(std::logic_error,"not implemented aas first step. copy from last step!!! ",rCurrentProcessInfo[FRACTIONAL_STEP]);
			
			
			double highest_cinematic_viscosity= viscosity_air/density_air;
			if ((viscosity_water/density_water)>highest_cinematic_viscosity) highest_cinematic_viscosity=(viscosity_water/density_water);
			
			const double fourier= highest_cinematic_viscosity *delta_t/pow((0.6*this->GetValue(MEAN_SIZE)),2);
			theta = 0.2*fourier - 0.1;
			if (theta<0.0) theta=0.0;
			else if (theta>1.0) theta=1.0;
			...
			for (unsigned int i = 0; i < 3; i++)  // local node (or shape function)
			{
				...
				{
					Mass_matrix(2*i,2*i) +=  volumes(j)*Ndiscontinuous(j,i)*densities(j);	 
				}
	...
	//************************************************************************************
	//************************************************************************************
		void PFEM2DiscontinuousVelocity2D::CalculateLocalPressureSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY
...
//        const unsigned int LocalSize = (2 + 1) * TNumNodes;
        ...
        boost::numeric::ublas::bounded_matrix<double, 3, 2> DN_DX;
        boost::numeric::ublas::bounded_matrix<double, 3, 6 > D_matrix = ZeroMatrix(3,6); //(gradient)
        boost::numeric::ublas::bounded_matrix<double, 3, 3 > Laplacian_matrix;
        //boost::numeric::ublas::bounded_matrix<double, TNumNodes*2, 2> N_vel_matrix; //useless to calculate it, it's simply the DN_DX matrix multiplied by 1/3, arranged in a different shape.
        ...
        if (split_element)
        {
			...
			array_1d<double,3>  volumes(3);
			boost::numeric::ublas::bounded_matrix<double,3, 2 > coords;
			boost::numeric::ublas::bounded_matrix<double,3, 2 > local_coords;
			boost::numeric::ublas::bounded_matrix<double,3, 3 > Ngauss;
			boost::numeric::ublas::bounded_matrix<double,3, 3 > Ndiscontinuous;
			...
			boost::numeric::ublas::bounded_matrix<double, 3, 2> DN_DX_in_local_axis;
			//fill coordinates
			...
			for (unsigned int i = 0; i < 3; i++)
				gauss_gradients_discontinuous[i].resize(3, 2, false);  //3 values of the 3 shape functions, and derivates in (xy) direction).	
			DiscontinuousShapeFunctionsUtilities::CalculateDiscontinuousShapeFunctionsInLocalAxis(coords,local_coords, DN_DX, distances, volumes, Ngauss, signs, gauss_gradients_discontinuous, Ndiscontinuous, RotationMatrix, DN_DX_in_local_axis);	

			for (unsigned int i = 0; i < 3; i++)
				gauss_gradients_in_local_axis[i].resize(2, 2, false);  //2 values of the 2 shape functions, and derivates in (xy) direction).
			EnrichmentUtilities::CalculateEnrichedShapeFuncionsInLocalAxis(coords, DN_DX, distances, volumes, Ngauss, signs, gauss_gradients_in_local_axis, Nenriched);
			
			//we must replace the D matrix by using the new gradients:.
			boost::numeric::ublas::bounded_matrix<double, 3, 6 > D_matrix_in_local_axis = ZeroMatrix(3,6); //(gradient)
			...
			for (unsigned int i = 0; i < 3; i++)
			{
				for (unsigned int j = 0; j < 3; j++)
				{
					for (unsigned int division = 0; division < ndivisions; division++)
						D_matrix_in_local_axis(i, (j*2) ) -=  gauss_gradients_discontinuous[division](i,0)*volumes[division]*Ndiscontinuous(division,j);  
					   
					D_matrix_in_local_axis(i, (j*2+1) ) =  -DN_DX_in_local_axis(i,1)*Area*one_third; //in local axis in the normal direction to the cut, we use the standard shape functions ( but in the local reference, then we rotate)
				}
			}
			
			//now we rotate it and save it in the correct place:
			for (unsigned int i = 0; i < 3; i++)
			{
				for (unsigned int j = 0; j < 3; j++)
				{
					D_matrix(i, (j*2) ) =  D_matrix_in_local_axis(i, (j*2) )*RotationMatrix(0,0) + D_matrix_in_local_axis(i, (j*2)+1 )*RotationMatrix(0,1);     
					D_matrix(i, (j*2+1) ) =  D_matrix_in_local_axis(i, (j*2) )*RotationMatrix(1,0) + D_matrix_in_local_axis(i, (j*2)+1 )*RotationMatrix(1,1);     
				}
			}
			...
			boost::numeric::ublas::bounded_matrix<double, 1, 6 > D_matrix_mixed_in_local_axis;
			noalias(D_matrix_mixed_in_local_axis) = ZeroMatrix(1,6);	
			//lower part of D, the one corresponding to the second enrichmend function (jump)
			boost::numeric::ublas::bounded_matrix<double, 1, 6 > D_matrix_mixed_jump_in_local_axis;
			noalias(D_matrix_mixed_jump_in_local_axis) = ZeroMatrix(1,6);	
			...
			for (unsigned int i = 0; i < ndivisions; i++)  //we go through the 3 partitions of the domain
			{
				...
				for (unsigned int j = 0; j < 3; j++) //we go through the 3 standard shape functions
				{
					...
					D_matrix_mixed_in_local_axis(0,j*2) -= gauss_gradients_in_local_axis[i](0,0) *volumes(i)*Ndiscontinuous(i,j);
					D_matrix_mixed_in_local_axis(0,j*2+1) -= gauss_gradients_in_local_axis[i](0,1) *volumes(i)*Ngauss(i,j);
					D_matrix_mixed_jump_in_local_axis(0,j*2) -= gauss_gradients_in_local_axis[i](1,0) *volumes(i)*Ndiscontinuous(i,j);
					D_matrix_mixed_jump_in_local_axis(0,j*2+1) -= gauss_gradients_in_local_axis[i](1,1) *volumes(i)*Ngauss(i,j);
			...
			{
				//and also the D matrixes
				D_matrix_mixed(0,j*2) = D_matrix_mixed_in_local_axis(0, (j*2) )*RotationMatrix(0,0) + D_matrix_mixed_in_local_axis(0, (j*2)+1 )*RotationMatrix(0,1);     
				D_matrix_mixed(0,j*2+1) = D_matrix_mixed_in_local_axis(0, (j*2) )*RotationMatrix(1,0) + D_matrix_mixed_in_local_axis(0, (j*2)+1 )*RotationMatrix(1,1);     
				D_matrix_mixed_jump(0,j*2) = D_matrix_mixed_jump_in_local_axis(0, (j*2) )*RotationMatrix(0,0) + D_matrix_mixed_jump_in_local_axis(0, (j*2)+1 )*RotationMatrix(0,1);     
				D_matrix_mixed_jump(0,j*2+1) = D_matrix_mixed_jump_in_local_axis(0, (j*2) )*RotationMatrix(1,0) + D_matrix_mixed_jump_in_local_axis(0, (j*2)+1 )*RotationMatrix(1,1);     
					
			}
			
			//KRATOS_WATCH(D_matrix_mixed)
	...
	*/
	...
	void PFEM2DiscontinuousVelocity2D::CalculateLocalFinalVelocitySystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY
		...
		else //split element:
		{
			...
			double Weight=0.0;
			double mass=0.0;
			double enriched_dof_mass=0.0;
			
			for (unsigned int i=0;i<ndivisions;i++) //we go over the three partitions;
			{
				if (signs(i)>0.0)
				{
					enriched_dof_mass += volumes(i)*Nenriched(i,0)*density_air;
					Weight += volumes(i)*viscosity_air;
					mass += volumes(i)*density_air;
				}
				else
				{
					enriched_dof_mass += volumes(i)*Nenriched(i,0)*density_water;
	...
	//***************************************************************************
		...
		void PFEM2DiscontinuousVelocity2D::CalculateViscousRHS(ProcessInfo& CurrentProcessInfo)
	{
		KRATOS_TRY
	...
	void PFEM2DiscontinuousVelocity2D::CalculatePressureProjection(ProcessInfo& CurrentProcessInfo)
	{
		KRATOS_TRY
		...
		if ((this->GetValue(IS_INACTIVE))==false) //elements can be inactive to add temporary walls. fractional velocity is integrated by parts so walls are seen as having zero velocity without doing anything 
				{
					...
					array_1d<double,(2+1)*2> nodal_masses = ZeroVector((2+1)*2); //2 components of the nodal mass
					...
					boost::numeric::ublas::bounded_matrix<double, (2+1), (2-1)*6 > G_matrix_no_ro_in_local_axis; //(gradient)
					noalias(G_matrix_no_ro_in_local_axis) = ZeroMatrix((2+1),(2-1)*6);	
					//new (upper) part of D, corresponding the first enrichment function (gradient discontinuity
					boost::numeric::ublas::bounded_matrix<double, 1, (2-1)*6 > G_matrix_mixed_no_ro_in_local_axis;
					noalias(G_matrix_mixed_no_ro_in_local_axis) = ZeroMatrix(1,(2-1)*6);	
					//lower part of D, the one corresponding to the second enrichmend function (jump)
					boost::numeric::ublas::bounded_matrix<double, 1, (2-1)*6> G_matrix_mixed_jump_no_ro_in_local_axis;
					noalias(G_matrix_mixed_jump_no_ro_in_local_axis) = ZeroMatrix(1,(2-1)*6);	
					...
					array_1d<double,(3*(2-1))> inv_densities;
					boost::numeric::ublas::bounded_matrix<double,(2+1), 2 > coords;
					boost::numeric::ublas::bounded_matrix<double,(2+1), 2 > local_coords;
					boost::numeric::ublas::bounded_matrix<double, 3*(2-1), (2+1) > Ngauss;
					boost::numeric::ublas::bounded_matrix<double,3, 3 > Ndiscontinuous;
					boost::numeric::ublas::bounded_matrix<double,2, 2 > RotationMatrix;
					array_1d<double,3>  signs(3);
					...
					if ((this->GetValue(SPLIT_ELEMENT))==true)
					{
						...
						noalias(G_matrix_no_ro_in_local_axis) = ZeroMatrix((2+1),6*(2-1)*6);	
						noalias(G_matrix_mixed_no_ro_in_local_axis) = ZeroMatrix(1,(2-1)*6);	
						noalias(G_matrix_mixed_jump_no_ro_in_local_axis) = ZeroMatrix(1,(2-1)*6);	
						...
						unsigned int ndivisions;
						single_triangle_node=1;
						ndivisions = EnrichmentUtilities::CalculateEnrichedShapeFuncions(coords, DN_DX, distances, volumes, Ngauss, signs, gauss_gradients, Nenriched);
						...
						{ 
							for (unsigned int j = 0; j < (2+1); j++) //shape function (Nj)
							{
								...
								{
									//G_matrix_mixed(0,j*(2)+k) += gauss_gradients[i](0,k) *volumes(i)*Ngauss(i,j)*inv_densities(i);
									G_matrix_mixed_no_ro_in_local_axis(0,j*(2)+k) += gauss_gradients_in_local_axis[i](0,k) *volumes(i)*Ndiscontinuous(i,j);
									

									//G_matrix_mixed_jump(0,j*(2)+k) += gauss_gradients[i](1,k) *volumes(i)*Ngauss(i,j)*inv_densities(i);
									G_matrix_mixed_jump_no_ro_in_local_axis(0,j*(2)+k) += gauss_gradients_in_local_axis[i](1,k) *volumes(i)*Ngauss(i,j);
								}
									
								for (unsigned int l = 0; l < (2+1); l++) //shape function derivative (dNl/dk)
								{
									//G_matrix(l, (j*(2)+k) ) += DN_DX(l,k)*volumes(i)*Ngauss(i,j)*inv_densities(i);
									G_matrix_no_ro_in_local_axis(l, (j*(2)+0) ) += gauss_gradients_discontinuous[i](l,0)*volumes(i)*Ndiscontinuous(i,j);
								}
									
								
								nodal_masses(j*2+0) += volumes(i)*Ndiscontinuous(i,j)*densities(i);
								nodal_masses(j*2+1) += volumes(i)*Ngauss(i,j)*densities(i);
							}
						}
						//for the massless g matrix (in the normal direction to the cut
						for (unsigned int j = 0; j < (2+1); j++) //shape function (Nj)
						{
		
								for (unsigned int l = 0; l < (2+1); l++) //shape function derivative (dNl/dk)
								{
									G_matrix_no_ro_in_local_axis(l, (j*(2)+1) ) = DN_DX_in_local_axis(l,1)*Area*mass_factor;
								}
						}
						...
						for (unsigned int i=0; i!=(2+1); i++) //loop around the nodes of the element to add contribution to node i
						{
							...
							//geom[i].FastGetSolutionStepValue(NODAL_AREA) += Area*mass_factor;
							//geom[i].FastGetSolutionStepValue(NODAL_MASS) += nodal_masses(i);
							geom[i].FastGetSolutionStepValue(MASS_X) += nodal_masses(i*2+0);
							geom[i].FastGetSolutionStepValue(MASS_Y) += nodal_masses(i*2+1);
					...
					else //normal (uncut) element
					{
						...
						for (unsigned int i=0; i!=(2+1); i++) //loop around the nodes of the element to add contribution to node i
						{
							...
							geom[i].FastGetSolutionStepValue(NODAL_AREA) += Area*mass_factor;
							//geom[i].FastGetSolutionStepValue(NODAL_MASS) += Area*mass_factor*density;
							geom[i].FastGetSolutionStepValue(MASS_X) += Area*mass_factor*density;
							geom[i].FastGetSolutionStepValue(MASS_Y) += Area*mass_factor*density;
	...
	*/
	...
	void PFEM2DiscontinuousVelocity2D::InitializeSolutionStep(ProcessInfo& CurrentProcessInfo)
	{
		KRATOS_TRY
	...
	//************************************************************************************
	//************************************************************************************
	void PFEM2DiscontinuousVelocity2D::PressureEquationIdVector(EquationIdVectorType& rResult, ProcessInfo& CurrentProcessInfo)
	{
	...
	//************************************************************************************
	void PFEM2DiscontinuousVelocity2D::VelocityEquationIdVector(EquationIdVectorType& rResult, ProcessInfo& CurrentProcessInfo)
	{
		...
		//************************************************************************************
	void PFEM2DiscontinuousVelocity2D::FractionalVelocityEquationIdVector(EquationIdVectorType& rResult, ProcessInfo& CurrentProcessInfo)
	{
		const SizeType NumNodes = 3;
		const SizeType LocalSize = 6;
		GeometryType& rGeom = this->GetGeometry();
	...
	//************************************************************************************
	//************************************************************************************
	void PFEM2DiscontinuousVelocity2D::GetPressureDofList(DofsVectorType& ElementalDofList,ProcessInfo& CurrentProcessInfo)
	{
		unsigned int number_of_nodes = GetGeometry().PointsNumber();
	...
	//************************************************************************************
	void PFEM2DiscontinuousVelocity2D::GetVelocityDofList(DofsVectorType& ElementalDofList,ProcessInfo& CurrentProcessInfo)
	{
		const SizeType NumNodes = 3;
		const SizeType LocalSize = 6;
		GeometryType& rGeom = this->GetGeometry();
		...
		//************************************************************************************
	void PFEM2DiscontinuousVelocity2D::GetFractionalVelocityDofList(DofsVectorType& ElementalDofList,ProcessInfo& CurrentProcessInfo)
	{
		const SizeType NumNodes = 3;
		const SizeType LocalSize = 6;
		GeometryType& rGeom = this->GetGeometry();
						  ...
						  boost::numeric::ublas::bounded_matrix<double, (2+1), 2 >& rRotationMatrix)
	{
		...
		for (unsigned int i=0; i!=(2+1); i++) //i node
		{
			//ONLY ALONG THE LOCAL Y axis. on X we use the discontinuous shape functions
			//in 3D, again, only one DOF is continuous and the other 2 will be discontinuous
			B_matrix(i*(2)+1,1)=rShapeDeriv(i,1);
			
			//useful for both 2d and 3d:	
			//again, we modify only the second row.
			B_matrix(i*(2)+1,2)=rShapeDeriv(i,0);
		}
		...
		for (unsigned int division=0; division!=ndivisions; division++)
		{
			for (unsigned int i=0; i!=(2+1); i++) //i node
			{
				//now the discontinuous interpolations. in 2d only along the local x axis
				B_matrix(i*(2)+0,0) = gauss_gradients_discontinuous[division](i,0);
				//crossed term
				B_matrix(i*(2)+1,2) = gauss_gradients_discontinuous[division](i,1);
			}
		...
		for (unsigned int i=0; i!=(2+1); i++) //i node
		{
			for (unsigned int j=0; j!=(2+1); j++) //j neighbour
			{
				//we create the Kij submatrix.
				boost::numeric::ublas::bounded_matrix<double, 2 , 2  > LocalAxisKij;
						  ...
						  const unsigned int ndivisions)
	{
		...
		for (unsigned int division=0; division!=ndivisions; division++)
		{
			...
			for (unsigned int j=0; j!=(2); j++) //x,y,z
				B_matrix(3*(2)+j,j)= gauss_gradients_discontinuous[division](0,j);
			
			//useful for both 2d and 3d:	
			//relating 12 and 21 stresses
			B_matrix(3*(2)+0,2)=gauss_gradients_discontinuous[division](0,1);
			B_matrix(3*(2)+1,2)=gauss_gradients_discontinuous[division](0,0);
