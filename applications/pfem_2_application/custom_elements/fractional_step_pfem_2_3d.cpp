...
namespace Kratos
{
	...
	void FractionalStepPFEM23D::CalculateLocalPressureSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	{
		...
		|| (geom[0].FastGetSolutionStepValue(DISTANCE)*geom[3].FastGetSolutionStepValue(DISTANCE))<0.0)
        {
			...
			BoundedMatrix<double, 1, TDim+1 > mixed_Laplacian;
	...
	void FractionalStepPFEM23D::AddViscousTerm(BoundedMatrix<double, (3-1)*6, (3-1)*6 >& rDampMatrix,
                         ...
                         const double Weight)
	{
		...
		BoundedMatrix<double, (TDim-1)*3 , (TDim-1)*6  > temp_matrix = prod(C_matrix,trans(B_matrix));
