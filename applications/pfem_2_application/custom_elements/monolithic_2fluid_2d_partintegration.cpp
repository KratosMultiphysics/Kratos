...
/*	{
		...
		//if(true==true)
		{
			...
			bounded_matrix<double, 3, 6 > D_matrix; //(gradient)
			noalias(D_matrix) = ZeroMatrix(3,6);	
			bounded_matrix<double, 3, 3 > Laplacian_matrix;
			noalias(Laplacian_matrix) = ZeroMatrix(3,3);	
			bounded_matrix<double, 9, 9 > Mass_matrix;
		...
		else //split element:
		{
			...
			bounded_matrix<double,3, 2> Nenriched;
			array_1d<double,3>  volumes(0);
			bounded_matrix<double,3, 2 > coords;
			bounded_matrix<double,3, 3 > Ngauss;
			...
			bounded_matrix<double, 3, 3> Laplacian_matrix =ZeroMatrix(3,3); //standard pressures. we will keep these dof so the size remains			
			bounded_matrix<double, 3, 6+enrich_velocity_dofs > D_matrix ; //(divergence) //this matrix will increase its size since we need more  
			noalias(D_matrix) = ZeroMatrix(3,6+enrich_velocity_dofs);	
			bounded_matrix<double, 3, 6+enrich_velocity_dofs > G_matrix; //(gradient)
			noalias(G_matrix) = ZeroMatrix(3,6+enrich_velocity_dofs);

			bounded_matrix<double, 9+enrich_velocity_dofs+enrich_pressure_dofs, 9+enrich_velocity_dofs+enrich_pressure_dofs > Momentum_matrix; //2 vel + 1 pressure per node   plus 2 enriched pressures and 2 enriched velocities
			...
			bounded_matrix<double, enrich_pressure_dofs, 3 > mixed_Laplacian;
			...
			bounded_matrix<double, enrich_velocity_dofs+enrich_pressure_dofs, 9 > condensed_rows; //Vx1,Vy1,p1,Vx2,...
			bounded_matrix<double, enrich_velocity_dofs+enrich_pressure_dofs, 9 > condensed_columns; //Vx1,Vy1,p1,Vx2,...
			bounded_matrix<double, enrich_velocity_dofs+enrich_pressure_dofs, enrich_velocity_dofs+enrich_pressure_dofs > condensed_block; //Vx1,Vy1,p1,Vx2,...
			...
			//condensing
			bounded_matrix<double, 9 , enrich_pressure_dofs+enrich_velocity_dofs  > temp_matrix;
	...
	*/
		...
		else //split element:
		{
			...
			bounded_matrix<double, TDim+1, (TDim+1)*TDim+enrich_velocity_dofs > D_matrix ; //(divergence) //this matrix will increase its size since we need more  
			noalias(D_matrix) = ZeroMatrix(TDim+1,(TDim+1)*TDim+enrich_velocity_dofs);	
			bounded_matrix<double,TDim+1,(TDim+1)*TDim+enrich_velocity_dofs > G_matrix; //(gradient)
			...
			*/
			...
			bounded_matrix<double, enrich_velocity_dofs+enrich_pressure_dofs, (TDim+1)*(TDim+1) > condensed_rows; //Vx1,Vy1,p1,Vx2,...
			bounded_matrix<double, enrich_velocity_dofs+enrich_pressure_dofs, (TDim+1)*(TDim+1) > condensed_columns; //Vx1,Vy1,p1,Vx2,...
			bounded_matrix<double, enrich_velocity_dofs+enrich_pressure_dofs, enrich_velocity_dofs+enrich_pressure_dofs > condensed_block; //Vx1,Vy1,p1,Vx2,...
	...
	void MonolithicAutoSlipPFEM22D::AddViscousTerm(bounded_matrix<double, 13, 13 > & output,
