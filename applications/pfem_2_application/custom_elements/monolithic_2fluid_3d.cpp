...
namespace Kratos
{
	...
	void MonolithicPFEM23D::AddViscousTerm(bounded_matrix<double, 21, 21 > & output,
										  bounded_matrix<double, (4), 3 >& rShapeDeriv,
	...
	{		
		bounded_matrix<double, 15, 15 > ExtendedDampMatrix=ZeroMatrix(15,15);
		//bounded_matrix<double, 8, 8 > rExtendedDampMatrix= ZeroMatrix(8,8);

		bounded_matrix<double, 15,6 > B_matrix = ZeroMatrix(15,6);

		
		int counter=0;
		bounded_matrix<double, 6, 6 > C_matrix = ZeroMatrix(6,6);
		...
		for (unsigned int division=0; division!=ndivisions; division++)
		{
			...
			bounded_matrix<double, 6 , 15  > temp_matrix = prod(C_matrix,trans(B_matrix));
                         ...
                         double& Viscosity,const double Area)
	{
		...
		bounded_matrix<double, 12,6 > B_matrix = ZeroMatrix(12,6);
		for (unsigned int i=0; i!=(4); i++) //i node
		{
			for (unsigned int j=0; j!=(3); j++) //x,y,z
				B_matrix(i*(3)+j,j)=rShapeDeriv(i,j);
		...
		bounded_matrix<double, 6 , 12  > temp_matrix = prod(C_matrix,trans(B_matrix));
		bounded_matrix<double, 12 , 12  > viscosity_matrix = prod(B_matrix, temp_matrix );
		for (unsigned int i=0; i!=4; i++) //i node
		{
