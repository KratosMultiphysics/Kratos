...
//This process permits one to "REMOVE" the elements that have all nodes "IS_VISITEDD" (flag) from the model part
...
//public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(SubdomainProcess);
...
//		}
	...
	{	  	
	in->FastGetSolutionStepValue(IS_VISITED)=false;
	}
	...
	int n_nodes=0;
	    ...
	    //KRATOS_WATCH(n_int);
	    //if the element has at least one non-interface node, save the element and mark all its node with a IS_VISITEDIS_VISITED flag (change flag name.. coz in reality its "enabled")
            if (n_int<n_nodes)
            {
                reduced_model_part.AddElement(*(im.base())); 
		for (int i=0;i<n_nodes;i++)
			{
			im->GetGeometry()[i].FastGetSolutionStepValue(IS_VISITED)=true;
			}               
            }
	    //KRATOS_WATCH(n_int)
	    //KRATOS_WATCH(n_nodes)
            //if (n_int>n_nodes)
                //KRATOS_THROW_ERROR(std::logic_error,  "Number of IS_VISITED flags cant exceed number of the element nodes.... " , "");
	...
	for(ModelPart::NodesContainerType::iterator in = full_model_part.NodesBegin() ; in != full_model_part.NodesEnd() ; ++in)
        {
            ...
            if (in->FastGetSolutionStepValue(IS_VISITED)==true)
            {
    ...
    {
        return "SubdomainProcess";
    }

    /// Print information about this object.
    virtual void PrintInfo(std::ostream& rOStream) const
    {
        rOStream << "SubdomainProcess";
    }
...
private:
    ...
    /// Assignment operator.
//		SubdomainIS_VISITEDProcess& operator=(SubdomainIS_VISITEDProcessconst& rOther);

    /// Copy constructor.
//		SubdomainIS_VISITEDProcess(SubdomainIS_VISITEDProcessconst& rOther);


    ///@}

}; // Class SubdomainIS_VISITEDProcess
...
inline std::istream& operator >> (std::istream& rIStream,
                                  SubdomainProcess& rThis);

/// output stream function
inline std::ostream& operator << (std::ostream& rOStream,
                                  const SubdomainProcess& rThis)
{
...
#endif // KRATOS_SUBDOMAIN_IS_VISITED_PROCESS_INCLUDED  defined 
