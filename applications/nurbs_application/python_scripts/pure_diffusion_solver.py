...
class StaticPoissonSolver:
    ...
    def __init__(self, model_part, domain_size):  # constructor of the class
        ...
        )  # we set the type of solver that we want

        # definition of the convergence criteria
        self.conv_criteria = DisplacementCriteria(
            1e-6, 1e-9)  # tolerance for the solver
    ...
    def Initialize(self):
        ...
        self.solver = strategy_python.SolvingStrategyPython(
            self.model_part, self.time_scheme, self.poisson_linear_solver, self.conv_criteria, CalculateReactionFlag, ReformDofSetAtEachStep, MoveMeshFlag)
