...
IF(TPETRA_INCLUDE_PATH AND TPETRA_LIBRARY)
    ...
    # Belos (Tpetra-era iterative solvers) - optional
    FIND_PATH(BELOS_INCLUDE_PATH BelosSolverFactory.hpp ${TRILINOS_INCLUDE_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_LIBRARY(BELOS_LIBRARY "${TRILINOS_LIBRARY_PREFIX}belos" ${TRILINOS_LIB_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_LIBRARY(BELOSTPETRA_LIBRARY "${TRILINOS_LIBRARY_PREFIX}belostpetra" ${TRILINOS_LIB_SEARCH_PATH} NO_DEFAULT_PATH)
    IF(BELOS_INCLUDE_PATH AND BELOS_LIBRARY AND BELOSTPETRA_LIBRARY)
        SET(TRILINOS_INCLUDE_DIR ${TRILINOS_INCLUDE_DIR} ${BELOS_INCLUDE_PATH})
        SET(TRILINOS_LIBRARIES ${TRILINOS_LIBRARIES} ${BELOS_LIBRARY} ${BELOSTPETRA_LIBRARY})
        SET(HAVE_BELOS YES)
        find_package_handle_standard_args(BELOS DEFAULT_MSG BELOS_LIBRARY)
        MESSAGE(STATUS "BELOS_INCLUDE_PATH: " ${BELOS_INCLUDE_PATH})
        MESSAGE(STATUS "BELOS_LIBRARY: " ${BELOS_LIBRARY})
        MESSAGE(STATUS "BELOSTPETRA_LIBRARY: " ${BELOSTPETRA_LIBRARY})
    ENDIF(BELOS_INCLUDE_PATH AND BELOS_LIBRARY AND BELOSTPETRA_LIBRARY)

    # Ifpack2 (Tpetra-era preconditioners) - optional
    FIND_PATH(IFPACK2_INCLUDE_PATH Ifpack2_Factory.hpp ${TRILINOS_INCLUDE_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_LIBRARY(IFPACK2_LIBRARY "${TRILINOS_LIBRARY_PREFIX}ifpack2" ${TRILINOS_LIB_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_LIBRARY(IFPACK2_ADAPTERS_LIBRARY "${TRILINOS_LIBRARY_PREFIX}ifpack2-adapters" ${TRILINOS_LIB_SEARCH_PATH} NO_DEFAULT_PATH)
    IF(IFPACK2_INCLUDE_PATH AND IFPACK2_LIBRARY)
        SET(TRILINOS_INCLUDE_DIR ${TRILINOS_INCLUDE_DIR} ${IFPACK2_INCLUDE_PATH})
        SET(TRILINOS_LIBRARIES ${TRILINOS_LIBRARIES} ${IFPACK2_LIBRARY})
        IF(IFPACK2_ADAPTERS_LIBRARY)
            SET(TRILINOS_LIBRARIES ${TRILINOS_LIBRARIES} ${IFPACK2_ADAPTERS_LIBRARY})
        ENDIF(IFPACK2_ADAPTERS_LIBRARY)
        SET(HAVE_IFPACK2 YES)
        find_package_handle_standard_args(IFPACK2 DEFAULT_MSG IFPACK2_LIBRARY)
        MESSAGE(STATUS "IFPACK2_INCLUDE_PATH: " ${IFPACK2_INCLUDE_PATH})
        MESSAGE(STATUS "IFPACK2_LIBRARY: " ${IFPACK2_LIBRARY})
    ENDIF(IFPACK2_INCLUDE_PATH AND IFPACK2_LIBRARY)

    # MueLu (Tpetra-era algebraic multigrid) - optional
    FIND_PATH(MUELU_INCLUDE_PATH MueLu_CreateTpetraPreconditioner.hpp ${TRILINOS_INCLUDE_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_LIBRARY(MUELU_LIBRARY "${TRILINOS_LIBRARY_PREFIX}muelu" ${TRILINOS_LIB_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_LIBRARY(MUELU_ADAPTERS_LIBRARY "${TRILINOS_LIBRARY_PREFIX}muelu-adapters" ${TRILINOS_LIB_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_LIBRARY(MUELU_INTERFACE_LIBRARY "${TRILINOS_LIBRARY_PREFIX}muelu-interface" ${TRILINOS_LIB_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_LIBRARY(XPETRA_LIBRARY "${TRILINOS_LIBRARY_PREFIX}xpetra" ${TRILINOS_LIB_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_LIBRARY(XPETRA_SUP_LIBRARY "${TRILINOS_LIBRARY_PREFIX}xpetra-sup" ${TRILINOS_LIB_SEARCH_PATH} NO_DEFAULT_PATH)
    IF(MUELU_INCLUDE_PATH AND MUELU_LIBRARY)
        SET(TRILINOS_INCLUDE_DIR ${TRILINOS_INCLUDE_DIR} ${MUELU_INCLUDE_PATH})
        SET(TRILINOS_LIBRARIES ${TRILINOS_LIBRARIES} ${MUELU_LIBRARY})
        IF(MUELU_ADAPTERS_LIBRARY)
            SET(TRILINOS_LIBRARIES ${TRILINOS_LIBRARIES} ${MUELU_ADAPTERS_LIBRARY})
        ENDIF(MUELU_ADAPTERS_LIBRARY)
        IF(MUELU_INTERFACE_LIBRARY)
            SET(TRILINOS_LIBRARIES ${TRILINOS_LIBRARIES} ${MUELU_INTERFACE_LIBRARY})
        ENDIF(MUELU_INTERFACE_LIBRARY)
        IF(XPETRA_LIBRARY)
            SET(TRILINOS_LIBRARIES ${TRILINOS_LIBRARIES} ${XPETRA_LIBRARY})
        ENDIF(XPETRA_LIBRARY)
        IF(XPETRA_SUP_LIBRARY)
            SET(TRILINOS_LIBRARIES ${TRILINOS_LIBRARIES} ${XPETRA_SUP_LIBRARY})
        ENDIF(XPETRA_SUP_LIBRARY)
        SET(HAVE_MUELU YES)
        find_package_handle_standard_args(MUELU DEFAULT_MSG MUELU_LIBRARY)
        MESSAGE(STATUS "MUELU_INCLUDE_PATH: " ${MUELU_INCLUDE_PATH})
        MESSAGE(STATUS "MUELU_LIBRARY: " ${MUELU_LIBRARY})
    ENDIF(MUELU_INCLUDE_PATH AND MUELU_LIBRARY)
