...
class LevelSetControl(Control):
    def __init__(self, name: str, model: Kratos.Model, parameters: Kratos.Parameters, optimization_problem: OptimizationProblem) -> None:
        ...
        default_settings = Kratos.Parameters("""{
            ...
            "minimum_modulus"                   : 1e-6,
            "reinit_interval"                   : 50,
            "heaviside_precision"               : 100,
            "CFL_factor"                        : 0.1
        }""")
        ...
        self.epsilon = parameters["minimum_modulus"].GetDouble()
        self.reinit_interval = parameters["reinit_interval"].GetInt()
        if self.reinit_interval == 0:
            self.reinit = False
        else:
            self.reinit = True
        self.heaviside_precision = parameters["heaviside_precision"].GetInt()
        self.CFL_factor = parameters["CFL_factor"].GetDouble()
        ...
        self.filtering_on = parameters["filtering"].GetBool()
    ...
    def Initialize(self) -> None:
        ...
        if self.init_mode == "const":
            ...
            if self.filtering_on:
                ...
                self.control_phi = self.physical_phi
    ...
    # Reinitializes φ via the Sussman PDE.
    # ∂φ/∂t + S(φ₀)(|∇φ| - 1) = 0
    ...
    def _ReinitializePhiSussman(self, num_iterations=10):
        ...
        for it in range(num_iterations):
            ...
            if self.filtering_on:
                ...
                self.physical_phi = self.control_phi

            if self.echo_level > 1 and it % 5 == 0:
                print(f"[Reinit] Iter {it}/{num_iterations} | mean(|∇φ|-1) = {np.mean(np.abs(grad_norm - 1)):.3e}")

    # Get an approximated characteristic length of elements
    ...
    # Get empty elemental field for physical variables and nodal for control field
    ...
    # Transform physical gradients to control space gradients
    ...
    # Update fields using the Hamilton-Jacobi equation
    def Update(self, control_field: ContainerExpressionTypes) -> bool:
        ...
        if Kratos.Expression.Utils.NormL2(update) > 1e-15:
            # Calculate nodal phi gradient
            gradient_norm = self.GetEmptyField()
            gradient_norm = Kratos.Expression.Utils.Collapse(self._ComputePhiGradientNorm(self.control_phi+update))
            ...
            if self.reinit == True and self.optimization_problem.GetStep() % self.reinit_interval == 0:
    ...
    def _UpdateAndOutputFields(self, update: ContainerExpressionTypes) -> None:
        ...
        KratosOA.ExpressionUtils.Heaviside(heaviside, elemental_phi, self.heaviside_precision, self.k)
        ...
        KratosOA.ExpressionUtils.DiracDelta(dirac_delta, elemental_phi, self.heaviside_precision, self.k)
    ...
    # Compute control field gradient norm for HJ equation
    def _ComputePhiGradientNorm(self, delta_phi: ContainerExpressionTypes) -> ContainerExpressionTypes:
    ...
    def _ComputeCFLCondition(self, design_velocity: ContainerExpressionTypes) -> float:
        ...
        return min(time.Evaluate()) * self.CFL_factor    
