...
template<class TBDF>
void TestBDFTimeIntegration(const TBDF& rBDF,
                            const ResultsMapType& rResultsX,
                            const ResultsMapType& rResultsY,
                            const bool SetupTest)
{
    Model current_model;
    const std::size_t buffer_size = TimeDiscretization::GetMinimumBufferSize(rBDF);
    ...
    for (std::size_t i_step = 0; i_step < num_steps; ++i_step){
        ...
        MeshVelocityCalculation::CalculateMeshVelocities(r_model_part, rBDF);
...
KRATOS_TEST_CASE_IN_SUITE(CalculateMeshVelocitiesBDF1, MeshMovingApplicationFastSuite)
{
    ...
    TimeDiscretization::BDF1 bdf;

    TestBDFTimeIntegration(bdf, results_x, results_y, false);
}
...
KRATOS_TEST_CASE_IN_SUITE(CalculateMeshVelocitiesGeneralizedAlpha, MeshMovingApplicationFastSuite)
{
    ...
    for (std::size_t i_step = 0; i_step < num_steps; ++i_step){
        ...
        MeshVelocityCalculation::CalculateMeshVelocities(r_model_part, gen_alpha);
