...
void  AddGeometriesToPython(pybind11::module& m)
{
    ...
    .def("Normal", [](GeometryType& self)
        { const auto& r_type = self.GetGeometryType(); KRATOS_WARNING_IF("Geometry", !(r_type == GeometryData::KratosGeometryType::Kratos_Line2D2 || r_type == GeometryData::KratosGeometryType::Kratos_Triangle3D3)) << "WARNING:: Your geometry is not linear, please provide local coordinates or a GP index. Normal will be computed in zero local coordinates" << std::endl; CoordinatesArrayType LocalCoords; LocalCoords.clear(); return(self.Normal(LocalCoords)); })
    ...
    .def("UnitNormal", [](GeometryType& self)
        { const auto& r_type = self.GetGeometryType(); KRATOS_WARNING_IF("Geometry", !(r_type == GeometryData::KratosGeometryType::Kratos_Line2D2 || r_type == GeometryData::KratosGeometryType::Kratos_Triangle3D3)) << "WARNING:: Your geometry is not linear, please provide local coordinates or a GP index. Normal will be computed in zero local coordinates" << std::endl; CoordinatesArrayType LocalCoords; LocalCoords.clear(); return(self.UnitNormal(LocalCoords)); })
