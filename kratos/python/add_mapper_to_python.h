...
void ExposeMapperToPython(pybind11::module& m)
{
    ...
    const auto mapper
        = py::class_< MapperType, typename MapperType::Pointer >(m, mapper_name.c_str())
            ...
            .def("Map", py::overload_cast<typename MapperType::ExpressionType&,const Variable<double>&,Flags>(&MapperType::Map))
            .def("Map", [](MapperType& rSelf, typename MapperType::ExpressionType& rSourceExpression, const Variable<double>& rTargetVariable)
                        {rSelf.Map(rSourceExpression, rTargetVariable, Flags());})
