...
def generate(input_file, base_folder='.'):
        ...
        if len(line) > 0:
            ...
            if line[0] == 'PROBLEMTYPE':
                ...
                if current_material is not None:
                    line.pop(0)
                    if current_material in list(material_dictionary.keys()):
                ...
                print('Reading Model Part group:', end=' ')
                for part in current_group:
                    print(part, end=' ')
                print('')
                ...
                while len(actions) > 0:  # for i in range(len(actions)):
                    word = actions.pop(0)
                    if word in list(condition_dictionary.keys()):
    ...
    if stop == False:
        ...
        for part, part_obj in part_dictionary.items():
        ...
        clean_tcl = ''
        for name, part_obj in part_dictionary.items():
        ...
        for condition, entities in auto_conditions.items():
...
def store_auto_data(part_obj, auto_dict):
    for entity, i in zip(('point', 'line', 'surface', 'volume'), list(range(4))):
