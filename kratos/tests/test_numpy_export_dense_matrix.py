...
class TestNumpyExportDenseMatrix(KratosUnittest.TestCase):
    ...
    def test_numpy_export_dense_matrix(self):
        ...
        for i in range(3):
            for j in range(3):
                self.assertAlmostEqual(KratosMatrix[i,j], NumpyMatrix[i,j], 1e-12)
        
        #Change an entry in Kratos Matrix
        KratosMatrix[0,0] = 123

        #Test change in Numpy matrix  
        self.assertAlmostEqual(KratosMatrix[0,0], NumpyMatrix[0,0], 1e-12)

        #Change an entry in Numpy matrix
        NumpyMatrix[2,2] = 555
        
        #Test change in Kratos matrix    
        self.assertAlmostEqual(KratosMatrix[2,2], NumpyMatrix[2,2], 1e-12)
