...
class TestMaterialsInput(KratosUnittest.TestCase):
    def _prepare_test(self, input_file = "materials.json"):
        ...
        self.model_part_io = KratosMultiphysics.ModelPartIO(GetFilePath("auxiliary_files_for_python_unittest/mdpa_files/test_model_part_io_read")) #reusing the file that is already in the directory
        ...
        self.test_settings["Parameters"]["materials_filename"].SetString(GetFilePath("auxiliary_files_for_python_unittest/materials_files/" + input_file))
    ...
    def test_overdefined_materials(self):
        ...
        test_settings["Parameters"]["materials_filename"].SetString(
            GetFilePath(os.path.join("auxiliary_files_for_python_unittest","materials_files","wrong_materials_input","wrong_materials_1.json")))
        ...
        test_settings["Parameters"]["materials_filename"].SetString(
            GetFilePath(os.path.join("auxiliary_files_for_python_unittest","materials_files","wrong_materials_input","wrong_materials_2.json")))
        expected_error_msg = "Error: Materials for ModelPart \"Main.sub\" are specified multiple times!"
        with self.assertRaisesRegex(RuntimeError, expected_error_msg):
            KratosMultiphysics.ReadMaterialsUtility(test_settings, current_model)

        test_settings["Parameters"]["materials_filename"].SetString(
            GetFilePath(os.path.join("auxiliary_files_for_python_unittest","materials_files","wrong_materials_input","wrong_materials_3.json")))
        expected_error_msg =  "Error: Materials for SubModelPart \"Main.sub\" is being overrided by Parent Model Part \"Main\"!\n"
        with self.assertRaisesRegex(RuntimeError, expected_error_msg):
            KratosMultiphysics.ReadMaterialsUtility(test_settings, current_model)

        test_settings["Parameters"]["materials_filename"].SetString(
            GetFilePath(os.path.join("auxiliary_files_for_python_unittest","materials_files","wrong_materials_input","wrong_materials_4.json")))
        expected_error_msg =  "Error: Materials for SubModelPart \"Main.sub1.subsub\" is being overrided by Parent Model Part \"Main.sub1\"!\n"
        with self.assertRaisesRegex(RuntimeError, expected_error_msg):
            KratosMultiphysics.ReadMaterialsUtility(test_settings, current_model)

        test_settings["Parameters"]["materials_filename"].SetString(
            GetFilePath(os.path.join("auxiliary_files_for_python_unittest","materials_files","wrong_materials_input","wrong_materials_5.json")))
    ...
    def test_input_without_tables_and_variables(self):
        self._prepare_test()
        self.test_settings["Parameters"]["materials_filename"].SetString(GetFilePath("auxiliary_files_for_python_unittest/materials_files/material_without_tables_and_variables.json"))
