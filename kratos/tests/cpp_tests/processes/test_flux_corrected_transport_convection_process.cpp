...
KRATOS_TEST_CASE_IN_SUITE(FluxCorrectedTransportConvectionProcess2D, KratosCoreFastSuite)
{
    ...
    // auto dist_func = [&](Node& rNode){return a*std::exp(-0.5*std::pow(rNode.X()-b,2)/std::pow(c,2));};
    // auto vel_func = [&](Node& rNode, array_1d<double,3>& rVel){rVel[0] = 0.1;rVel[1] = 0.0;rVel[2] = 0.0;};
