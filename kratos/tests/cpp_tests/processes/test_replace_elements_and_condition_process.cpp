...
namespace Kratos
{
    namespace Testing
    {
        ...
        */
        ...
        /**
        * Checks the replacement works in submodelparts
        * Test triangle
        */
        KRATOS_TEST_CASE_IN_SUITE(ReplaceElementsAndConditionsProcess3, KratosCoreFastSuite)
        {
            Model current_model;
            ModelPart& this_model_part = current_model.CreateModelPart("Main");
            ModelPart& this_sub_model_part = this_model_part.CreateSubModelPart("SubMain");
            ...
            this_model_part.AddElement(p_elem_0);
            this_model_part.AddElement(p_elem_1);
            this_sub_model_part.AddElement(p_elem_2);
            this_sub_model_part.AddElement(p_elem_3);
            ...
            ReplaceElementsAndConditionsProcess process = ReplaceElementsAndConditionsProcess(this_sub_model_part, settings);
            process.Execute();

            // Same element type, same geometry type
            std::string component_name;
            for (auto& r_element : this_sub_model_part.Elements()) {
                CompareElementsAndConditionsUtility::GetRegisteredName(r_element, component_name);
                KRATOS_CHECK_EQUAL(component_name, "Element2D3N");
            }
        }
