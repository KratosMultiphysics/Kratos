...
namespace Kratos {
    namespace Testing {
        ...
        /// More tests Compute Axis Intersection
        /// Test Compute Axis Intersection 2
        KRATOS_TEST_CASE_IN_SUITE(CurveAxisIntersectionComputeAxisIntersection_2, KratosCoreFastSuite) {
            ...
            // KRATOS_CHECK_EQUAL(spans.size(), 7);
            KRATOS_EXPECT_NEAR(spans[0], 0.0, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[1], 0.166667, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[2], 0.333333, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[3], 0.5, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[4], 0.666667, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[5], 0.833333, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[6], 1.0, TOLERANCE);
        }

        /// Test Compute Axis Intersection 3
        KRATOS_TEST_CASE_IN_SUITE(CurveAxisIntersectionComputeAxisIntersection_3, KratosCoreFastSuite) {
            ...
            knot_vector_curve[0] = 0.0;
            knot_vector_curve[1] = 0.0;
            knot_vector_curve[2] = 12.0;
            knot_vector_curve[3] = 12.0;
            Vector weights_curve = ZeroVector(2);
            weights_curve[0] = 1.0;
            weights_curve[1] = 1.0;
            ...
            surface_spans_u[2] = 12.0;
            std::vector<double> surface_spans_v;
            surface_spans_v.resize(7);
            surface_spans_v[0] = 0.0;
            surface_spans_v[1] = 2.0;
            surface_spans_v[2] = 4.0;
            surface_spans_v[3] = 6.0;
            surface_spans_v[4] = 8.0;
            surface_spans_v[5] = 10.0;
            surface_spans_v[6] = 12.0;
            ...
            KRATOS_EXPECT_NEAR(spans[0], 0.0, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[1], 2.0, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[2], 4.0, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[3], 6.0, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[4], 8.0, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[5], 10.0, TOLERANCE);
            KRATOS_EXPECT_NEAR(spans[6], 12.0, TOLERANCE);
        }
