...
KRATOS_TEST_CASE_IN_SUITE(PolynomialUtilitiesDegree, KratosCoreFastSuite) {
    ...
    KRATOS_EXPECT_EQ(PolynomialUtilities::Degree(p), 3);
}

KRATOS_TEST_CASE_IN_SUITE(PolynomialUtilitiesEvaluate, KratosCoreFastSuite) {
    Polynomial p{1.0, 2.0, 3.0, 4.0}; // x^3 + 2x^2 + 3x + 4

    KRATOS_EXPECT_NEAR(PolynomialUtilities::Evaluate(p, 0.0), 4.0, POLYNOMIAL_TOLERANCE);
    KRATOS_EXPECT_NEAR(PolynomialUtilities::Evaluate(p, 0.5), 6.125, POLYNOMIAL_TOLERANCE);
}
...
KRATOS_TEST_CASE_IN_SUITE(PolynomialUtilitiesDifferentiate, KratosCoreFastSuite) {
    ...
    KRATOS_EXPECT_VECTOR_NEAR(PolynomialUtilities::Differentiate(p), d, POLYNOMIAL_TOLERANCE);
}
...
KRATOS_TEST_CASE_IN_SUITE(PolynomialUtilitiesMultiply, KratosCoreFastSuite) {
    ...
    KRATOS_EXPECT_VECTOR_NEAR(PolynomialUtilities::Multiply(a, b), c, POLYNOMIAL_TOLERANCE);
}
...
KRATOS_TEST_CASE_IN_SUITE(PolynomialUtilitiesDivide, KratosCoreFastSuite) {
    ...
    KRATOS_EXPECT_VECTOR_NEAR(q, expected_quotient, POLYNOMIAL_TOLERANCE);
    KRATOS_EXPECT_VECTOR_NEAR(r, expected_remainder, POLYNOMIAL_TOLERANCE);

    Polynomial c{4.0, 13.0, 22.0, 15.0}; // a*b
    PolynomialUtilities::Divide(q, r, c, a);
    KRATOS_EXPECT_VECTOR_NEAR(q, b, POLYNOMIAL_TOLERANCE);
    KRATOS_EXPECT_VECTOR_NEAR(r, Polynomial{0.0}, POLYNOMIAL_TOLERANCE);
}
...
KRATOS_TEST_CASE_IN_SUITE(PolynomialUtilitiesIsolateRoots, KratosCoreFastSuite) {
    ...
    KRATOS_EXPECT_EQ(intervals.size(), 3);
    KRATOS_EXPECT_EQ(CountIntervalsContaining(intervals, -0.7360625845831237), 1);
    KRATOS_EXPECT_EQ(CountIntervalsContaining(intervals, -0.45955361708183773), 1);
    KRATOS_EXPECT_EQ(CountIntervalsContaining(intervals,  0.4880690318514098), 1);

    PolynomialUtilities::IsolateRoots(intervals, a, Interval{0, 1});
    KRATOS_EXPECT_EQ(intervals.size(), 1);
    KRATOS_EXPECT_EQ(CountIntervalsContaining(intervals, 0.4880690318514098), 1);

    PolynomialUtilities::IsolateRoots(intervals, a, Interval{-2, -1});
    KRATOS_EXPECT_EQ(intervals.size(), 0);
}

KRATOS_TEST_CASE_IN_SUITE(PolynomialUtilitiesFindRoot, KratosCoreFastSuite) {
    Polynomial a{1.06, 0.75, -0.26, -0.175}; // Has three roots in [-1, 1]

    KRATOS_EXPECT_NEAR(PolynomialUtilities::FindRoot(a, Interval{-1, -0.5}), -0.7360625845831237, POLYNOMIAL_TOLERANCE);
    KRATOS_EXPECT_NEAR(PolynomialUtilities::FindRoot(a, Interval{-0.5, 0}), -0.45955361708183773, POLYNOMIAL_TOLERANCE);
    KRATOS_EXPECT_NEAR(PolynomialUtilities::FindRoot(a, Interval{0, 1}), 0.4880690318514098, POLYNOMIAL_TOLERANCE);
}
