	...
	namespace Testing {
		...
		KRATOS_TEST_CASE_IN_SUITE(VariableChecks, KratosCoreFastSuite)
        {
            ...
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_VARIABLE_IN_NODAL_DATA(BODY_FORCE, r_node),
                "Missing BODY_FORCE variable in solution step data for node "
                "1.");
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_VARIABLE_IN_NODAL_DATA(BODY_FORCE_X, r_node),
            ...
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_DOF_IN_NODE(VELOCITY_X, r_node),
        ...
        KRATOS_TEST_CASE_IN_SUITE(VectorChecks, KratosCoreFastSuite)
        {
            ...
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_VECTOR_EQ(v1, v3),
                "Check failed because vector arguments do not have the same size"
            );
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_VECTOR_NEAR(v1, v3, 0.1),
                "Check failed because vector arguments do not have the same size"
            );
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_VECTOR_EQ(v1,v2),
                "Mismatch found in component 1:"
            );
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_VECTOR_NEAR(v1,v2, 0.001),
                "Mismatch found in component 1:"
            );
        }
        ...
        KRATOS_TEST_CASE_IN_SUITE(MatrixChecks, KratosCoreFastSuite)
        {
            ...
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_MATRIX_EQUAL(m1, m3),
                "Check failed because matrix arguments do not have the same dimensions"
            );
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_MATRIX_NEAR(m1, m3, 0.1),
                "Check failed because matrix arguments do not have the same dimensions"
            );
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_MATRIX_EQUAL(m1,m2),
                "Mismatch found in component (0,1):"
            );
            KRATOS_EXPECT_EXCEPTION_IS_THROWN(
                KRATOS_EXPECT_MATRIX_NEAR(m1,m2, 0.001),
                "Mismatch found in component (0,1):"
            );
        }
