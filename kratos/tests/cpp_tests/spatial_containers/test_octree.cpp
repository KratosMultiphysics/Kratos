...
KRATOS_TEST_CASE_IN_SUITE(OctreeExistPoint, KratosCoreFastSuite)
{
    ...
    KRATOS_EXPECT_EQ(testOctree.ExistPoint(OTPointPointerType(new OTPointType(10.0, 10.0, 10.0))), nullptr);
    KRATOS_EXPECT_EQ(testOctree.ExistPoint(OTPointPointerType(new OTPointType(9.0, 9.0, 9.0))), points[9]);
}
...
KRATOS_TEST_CASE_IN_SUITE(OctreeSearchNearestPoint, KratosCoreFastSuite)
{
    ...
    KRATOS_EXPECT_EQ(testOctree.SearchNearestPoint(point_10), points[9]);
    double distance;
    KRATOS_EXPECT_EQ(testOctree.SearchNearestPoint(point_10, distance), points[9]);
    KRATOS_EXPECT_DOUBLE_EQ(distance, 3.0); // NOTE: Should be sqrt of 3, may require to check that
}
...
KRATOS_TEST_CASE_IN_SUITE(OctreeSearchInRadius, KratosCoreFastSuite)
{
    ...
    auto point_10 = OTPointType(10.0, 10.0, 10.0);
    KRATOS_EXPECT_EQ(testOctree.SearchInRadius(point_10, 1.0, result_points.begin(), distances.begin(), max_number_results), 0);

    KRATOS_EXPECT_EQ(testOctree.SearchInRadius(point_10, 3.0, result_points.begin(), distances.begin(), max_number_results), 1);
    KRATOS_EXPECT_DOUBLE_EQ(distances[0], 3.0); // NOTE: Should be sqrt of 3, it is always the quare for performance reasons

    KRATOS_EXPECT_EQ(testOctree.SearchInRadius(point_10, 4.0, result_points.begin(), max_number_results), 2);
    KRATOS_EXPECT_EQ(testOctree.SearchInRadius(point_10, 4.0, result_points.begin(), distances.begin(), max_number_results), 2);
    KRATOS_EXPECT_DOUBLE_EQ(distances[0] + distances[1], 15.0); // NOTE: Should be sqrt of 3 + sqrt of 12, it is always the quare for performance reasons
}
...
KRATOS_TEST_CASE_IN_SUITE(OctreeSearchInBox, KratosCoreFastSuite)
{
    ...
    KRATOS_EXPECT_EQ(testOctree.SearchInBox(point_11, point_10, result_points.begin(), max_number_results), 0);

    auto point_12 = OTPointType(9.0, 9.0, 9.0);
    KRATOS_EXPECT_EQ(testOctree.SearchInBox(point_12, point_10, result_points.begin(), max_number_results), 1);
}
...
KRATOS_TEST_CASE_IN_SUITE(OctreeBB, KratosCoreFastSuite)
{
    ...
    KRATOS_EXPECT_VECTOR_EQ(testOctree.BoundingBoxLowPoint().Coordinates(), points[0]->Coordinates());
    KRATOS_EXPECT_VECTOR_EQ(testOctree.BoundingBoxHighPoint().Coordinates(), points[9]->Coordinates());
}
