...
namespace Testing {
    ...
    KRATOS_TEST_CASE_IN_SUITE(QuadraturePointSurfaceInVolumeGeometryTest, KratosCoreNurbsGeometriesFastSuite)
    {
        ...
        auto p_node_1 = Kratos::make_intrusive<NodeType>(0, 0.0, 1.0, 0.0);
        auto p_node_2 = Kratos::make_intrusive<NodeType>(1, 1.0, 1.0, 0.0);
        auto p_node_3 = Kratos::make_intrusive<NodeType>(2, 1.0, 1.0, 0.0);
        auto p_node_4 = Kratos::make_intrusive<NodeType>(3, 0.0, 1.0, 1.0);

        auto triangle = Kratos::make_shared<Triangle3D3<NodeType>>( p_node_1, p_node_3, p_node_4);
        ...
        KRATOS_WATCH( surface_in_volume.LocalSpaceDimension() );
        KRATOS_WATCH( quad_geometries[0].LocalSpaceDimension() );
        KRATOS_WATCH( quad_geometries[0].IntegrationPoints() );
        KRATOS_WATCH( surface_in_volume.DeterminantOfJacobian(test_point) );
        KRATOS_WATCH( quad_geometries[0].DeterminantOfJacobian(0, quad_geometries[0].GetDefaultIntegrationMethod() ) );
        KRATOS_WATCH( quad_geometries[0].Normal(0,quad_geometries[0].GetDefaultIntegrationMethod()) );
