...
namespace Testing {
    ...
    Kratos::shared_ptr<NurbsVolumeGeometry<PointerVector<NodeType>>> GenerateIrregularCubeNurbsVolume() {
    ...
    {
        auto p_cube = GenerateIrregularCubeNurbsVolume();
        ...
        // Check integration
        //double global_area_triangle = local_area_triangle * p_nurbs_volume->DeterminantOfJacobian(test_point);
        double global_area_triangle = 0.5;
        KRATOS_CHECK_NEAR( surface_in_volume.DeterminantOfJacobian(test_point), 2.0*global_area_triangle, 1e-10);
...
KRATOS_TEST_CASE_IN_SUITE(SurfaceInVolumeGeometrySquareInCubeTest, KratosCoreNurbsGeometriesFastSuite)
    {
        ...
        std::vector<IntegrationPoint<3>> dummy_integration_points(1);
        surface_in_volume.CreateQuadraturePointGeometries(quad_geometries, 2, dummy_integration_points);
        ...
        // Check integration
        auto integration_points = quad_geometries[0].IntegrationPoints();
        double global_area_triangle;
        for( SizeType i = 0; i < quad_geometries.size(); ++i ){
            global_area_triangle += quad_geometries[i].DeterminantOfJacobian(i, integration_method ) *
                quad_geometries[i].IntegrationPoints()[0].Weight();
        }

        double global_area_triangle_ref = local_area_triangle*surface_in_volume.DeterminantOfJacobian(test_point);
        KRATOS_CHECK_NEAR( global_area_triangle_ref, std::sqrt(2)*2, 1e-10);

        KRATOS_CHECK_EQUAL(integration_points.size(), 4);
        double weight = integration_points[0].Weight();
        KRATOS_CHECK_VECTOR_NEAR(integration_points[0].Coordinates(), local_center_ref, 1e-10);
        KRATOS_CHECK_NEAR(weight, 0.5, 1e-10);
        KRATOS_CHECK_NEAR( quad_geometries[0].DeterminantOfJacobian(0, integration_method )*weight, global_area_triangle, 1e-10);
    }
