...
namespace {
  ...
  Geometry<NodeType>::Pointer GenerateCurvedTriangle2D6() {
    ...
    TEST(Triangle2D6EdgesNumber, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D6();
        KRATOS_EXPECT_EQ(geom->EdgesNumber(), 3);
    }

    TEST(Triangle2D6FacesNumber, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D6();
    ...
    TEST(Triangle2D6MaxEdgeLength, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D6();
    ...
    TEST(Triangle2D6AverageEdgeLength, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D6();
    ...
    TEST(Triangle2D6DeterminantOfJacobianArray1, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D6();
    ...
    TEST(Triangle2D6IntegrationPointsNumber, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D6();
        KRATOS_EXPECT_EQ(geom->IntegrationPointsNumber(), 3);
    }

    TEST(Triangle2D6DeterminantOfJacobianIndex1, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D6();
        for (unsigned g = 0; g < geom->IntegrationPointsNumber(); ++g)
        KRATOS_EXPECT_NEAR(geom->DeterminantOfJacobian(g), 1.0, TOLERANCE);
    }

    TEST(Triangle2D6ShapeFunctionsValues, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D6();
    ...
    TEST(Triangle2D6ShapeFunctionsLocalGradients, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D6();
        TestAllShapeFunctionsLocalGradients(*geom);
    }

    TEST(Triangle2D6LumpingFactorsRegularShape, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D6();
