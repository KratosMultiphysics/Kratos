...
namespace {
  ...
  Geometry<NodeType>::Pointer GenerateCurvedTriangle2D10() {
    ...
    TEST(Triangle2D10EdgesNumber, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
        KRATOS_EXPECT_EQ(geom->EdgesNumber(), 3);
    }

    TEST(Triangle2D10FacesNumber, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
        KRATOS_EXPECT_EQ(geom->FacesNumber(), 3);
    }

    TEST(Triangle2D10Area, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
        KRATOS_EXPECT_NEAR(geom->Area(), 0.5, TOLERANCE);
    }

    TEST(Triangle2D10Volume, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
        KRATOS_EXPECT_EXCEPTION_IS_THROWN(geom->Volume(), "Calling base class 'Volume' method instead of derived class one.");
    }

    TEST(Triangle2D10MinEdgeLength, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
    ...
    TEST(Triangle2D10MaxEdgeLength, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
    ...
    TEST(Triangle2D10AverageEdgeLength, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
    ...
    TEST(Triangle2D10DeterminantOfJacobianArray1, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
    ...
    TEST(Triangle2D10IntegrationPointsNumber, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
        KRATOS_EXPECT_EQ(geom->IntegrationPointsNumber(), 6);
    }

    TEST(Triangle2D10DeterminantOfJacobianIndex1, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
        for (unsigned g = 0; g < geom->IntegrationPointsNumber(); ++g)
        KRATOS_EXPECT_NEAR(geom->DeterminantOfJacobian(g), 1.0, TOLERANCE);
    }

    TEST(Triangle2D10ShapeFunctionsValues, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
    ...
    TEST(Triangle2D10ShapeFunctionsLocalGradients, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
        TestAllShapeFunctionsLocalGradients(*geom);
    }

    TEST(Triangle2D10LumpingFactorsRegularShape, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle2D10();
