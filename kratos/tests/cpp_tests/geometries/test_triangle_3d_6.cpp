...
namespace {
  ...
  Geometry<NodeType>::Pointer GenerateReferenceTriangle3D6() {
    ...
    TEST(Triangle3D6EdgesNumber, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle3D6();
        KRATOS_EXPECT_EQ(geom->EdgesNumber(), 3);
    }

    TEST(Triangle3D6Area, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle3D6();
    ...
    TEST(Triangle3D6MaxEdgeLength, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle3D6();
    ...
    TEST(Triangle3D6AverageEdgeLength, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle3D6();
    ...
    TEST(Triangle3D6IntegrationPointsNumber, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle3D6();
        KRATOS_EXPECT_EQ(geom->IntegrationPointsNumber(), 3);
    }

    TEST(Triangle3D6DeterminantOfJacobianIndex1, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle3D6();
        for (unsigned g = 0; g < geom->IntegrationPointsNumber(); ++g)
        KRATOS_EXPECT_NEAR(geom->DeterminantOfJacobian(g), 1.0, TOLERANCE);
    }

    TEST(Triangle3D6ShapeFunctionsLocalGradients, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle3D6();
        TestAllShapeFunctionsLocalGradients(*geom);
    }

    TEST(Triangle3D6LumpingFactorsRegularShape, KratosCoreGeometriesFastSuite) {
        auto geom = GenerateReferenceTriangle3D6();
