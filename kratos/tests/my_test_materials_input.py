...
class TestMaterialsInput(KratosUnittest.TestCase):
    ...
    def test_input(self):
        ...
        """)
        #assign the real path
        test_settings["materials_filename"].SetString(GetFilePath("materials.json"))

        #import read_materials_process
        #read_materials_process.Factory(test_settings,Model)
        KratosMultiphysics.ReadMaterialsUtility(test_settings,Model)
        ...
        self.assertEqual(model_part.Properties[1].Has(KratosMultiphysics.YOUNG_MODULUS), True)
        self.assertEqual(model_part.Properties[1].GetValue(KratosMultiphysics.YOUNG_MODULUS), 200.0)
        self.assertEqual(model_part.Properties[1].GetValue(KratosMultiphysics.POISSON_RATIO), 0.3)
        self.assertEqual(model_part.Properties[1].GetValue(KratosMultiphysics.YIELD_STRESS), 400.0)
        self.assertEqual(model_part.Properties[1].GetValue(KratosFluid.SUBSCALE_PRESSURE), 0.1)
        self.assertEqual(model_part.Properties[1].GetValue(KratosFluid.VORTICITY_MAGNITUDE), -5.888)

        #self.assertEqual(model_part.Properties[2].GetValue(KratosMultiphysics.YOUNG_MODULUS), 100.0)
        #self.assertEqual(model_part.Properties[2].GetValue(KratosMultiphysics.POISSON_RATIO), 0.1)
        #self.assertEqual(model_part.Properties[2].GetValue(KratosMultiphysics.YIELD_STRESS), 800.0)
        #self.assertEqual(model_part.Properties[2].GetValue(KratosMultiphysics.HTC), 0.3)
        #self.assertEqual(model_part.Properties[2].GetValue(KratosMultiphysics.TIME_STEPS), 159) # int
        #self.assertEqual(model_part.Properties[2].GetValue(KratosMultiphysics.UPDATE_SENSITIVITIES), True) # bool
        #self.assertEqual(model_part.Properties[2].GetValue(KratosMultiphysics.IDENTIFIER), "MyTestString") # std::string
