...
class PrintGraphPrinter:
    def __init__(self, output_nodes_list, model_part, variables_dictionary, domain_size):
	...
	for item in output_nodes_list:
		...
		if( (type(elem.GetNodes()[0].GetSolutionStepValue(var,0)) is int) or (type(elem.GetNodes()[0].GetSolutionStepValue(var,0)) is float)):
