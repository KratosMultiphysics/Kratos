        ...
        mOutputSettings(ThisParameters)
{
    ...
    if (file_format == "ascii") {
        mFileFormat = VtuOutput::FileFormat::VTK_ASCII;
    } else if (file_format == "binary") {
        mFileFormat = VtuOutput::FileFormat::VTK_BINARY;
...
void VtuOutput::WriteModelPartToFile(const ModelPart& rModelPart, const bool IsSubModelPart)
{
...
std::string VtuOutput::GetOutputFileName(const ModelPart& rModelPart, const bool IsSubModelPart)
{
...
void VtuOutput::WriteHeaderToFile(const ModelPart& rModelPart, std::ofstream& rFileStream) const
{
...
void VtuOutput::WriteMeshToFile(const ModelPart& rModelPart, std::ofstream& rFileStream) const
{
...
void VtuOutput::WriteNodesToFile(const ModelPart& rModelPart, std::ofstream& rFileStream) const
{
    ...
    if (mOutputSettings["write_deformed_configuration"].GetBool()) {
        for (const auto& r_node : rModelPart.Nodes()) {
            ...
            if (mFileFormat == VtuOutput::FileFormat::VTK_ASCII) rFileStream << "\n";
        }
    } else {
        for (const auto& r_node : rModelPart.Nodes()) {
...
void VtuOutput::WriteConditionsAndElementsToFile(const ModelPart& rModelPart, std::ofstream& rFileStream) const
{
    const auto& r_local_mesh = rModelPart.GetCommunicator().LocalMesh();
...
unsigned int VtuOutput::DetermineVtkCellListSize(const TContainerType& rContainer) const
{
...
template <typename TContainerType>
void VtuOutput::WriteConnectivity(const TContainerType& rContainer, std::ofstream& rFileStream) const
{
    // NOTE: also in MPI all nodes (local and ghost) have to be written, because
    // they might be needed by the elements/conditions due to the connectivity
...
template <typename TContainerType>
void VtuOutput::WriteCellType(const TContainerType& rContainer, std::ofstream& rFileStream) const
{
    ...
    std::ofstream& rFileStream) const
{
    if (rContainer.size() == 0) {
        KRATOS_WARNING("VtuOutput") << mrModelPart.GetCommunicator().GetDataCommunicator() << "Empty container!" << std::endl;
        return;
    }
...
void VtuOutput::WriteScalarDataToFile(const TData& rData, std::ofstream& rFileStream) const
{
    if (mFileFormat == VtuOutput::FileFormat::VTK_ASCII) {
        rFileStream << rData;
    } else if (mFileFormat == VtuOutput::FileFormat::VTK_BINARY) {
...
template <class TData>
void VtuOutput::WriteVectorDataToFile(const TData& rData, std::ofstream& rFileStream) const
{
    if (mFileFormat == VtuOutput::FileFormat::VTK_ASCII) {
...
void VtuOutput::WriteModelPartWithoutNodesToFile(ModelPart& rModelPart)
{
