...
public:
    ...
    enum class FileFormat {
        VTK_ASCII,  //legacy, to be removed
        VTK_BINARY  //legacy, to be removed
    };
...
protected:
    ...
    VtuOutput::FileFormat mFileFormatLegacy;             /// The file format considered
    std::string mFileFormat;             /// The file format considered
     ...
     */
    void WriteModelPartToFile(ModelPart& rModelPart, const bool IsSubModelPart, const std::string& rOutputFilename);

    /**
     * @brief Get the output file name based on the provided settings and the MPI rank
     * @param rModelPart modelpart which is beging output
     */
    std::tuple<std::string, std::string> GetOutputFileName(const ModelPart& rModelPart, const bool IsSubModelPart, const std::string& rOutputFilename) const;
    ...
    {
        if (mFileFormatLegacy == VtuOutput::FileFormat::VTK_ASCII) {
            rFileStream << rData;
        } else if (mFileFormatLegacy == VtuOutput::FileFormat::VTK_BINARY) {
            TData data = rData;
