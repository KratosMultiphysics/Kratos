      ...
      mEchoLevel(EchoLevel)
{
    ...
    KRATOS_INFO_IF(this->Info(), mEchoLevel > 0) <<"Reading CSV data from \"" << mFileName << "\"...\n";
    ...
    while (std::getline(input_file, line)) {
        if (line[0] == '#') {
            ...
            if (line.find("End of column information") != std::string::npos) {
                ...
                KRATOS_ERROR_IF(mReadData.front().first != rRowIdName)
                    << "The first column should be \"" << rRowIdName << "\" column with data type \"integer\" [ first column name = " << mReadData.front().first << " ].\n" << *this << std::endl;
            ...
            if (CSVDatabaseIOUtils::Trim(line.substr(0, line.find(','))) == rRowIdName) {
            ...
            if (found_header_line) {
                ...
                for (auto part : line | std::views::split(',')) {
                    if (column_index < mReadData.size()) {
                        ...
                        switch (mReadData[column_index].second.index()) {
                            case 0: {
                                if (value != "n/a")
                                    std::get<std::vector<bool>>(mReadData[column_index].second).push_back(value == rBooleanTrueValue);
                                else
                ...
                KRATOS_INFO_IF("CSVDatabaseIO", mEchoLevel > 1) << "Read data for " << rRowIdName << " = " << std::get<std::vector<int>>(mReadData[0].second).back() << "." << std::endl;
            }
        }
    }
      ...
      mWriteTimeStamp(WriteTimeStamp)
{
    ...
    mpFormatSettings = std::make_shared<FormatSettings>(
                            ...
                            FloatPrecision,
                            StringLength,
                            std::vector<std::string>{rBooleanFalseValue, rBooleanTrueValue});

    // Adding the step column
    mWritingData.push_back(std::make_pair(Column(rRowIdName, 0, mpFormatSettings), 0));

    KRATOS_INFO_IF(this->Info(), mEchoLevel > 0) <<"Writing CSV header information to \"" << mFileName << "\"...\n";

    std::ofstream output_file(mFileName, std::ios::out | std::ios::trunc | std::ios::binary);

    output_file << "# ===========================================================" << std::endl;
    output_file << "# ";
    const int left_gap = (59 - rTitle.size()) / 2 - 1;
    if (left_gap > 0) {
        int i;
        ...
        output_file << " ";
        for (; i - left_gap < static_cast<int>(rTitle.size()); ++i) {
            output_file << rTitle[i - left_gap];
        }
        output_file << " ";
        for (; i < 59 - 2 ; ++i) {
            output_file << "=";
    ...
    output_file << rHeaderInformation;

    output_file.close();

    KRATOS_INFO_IF(this->Info(), mEchoLevel > 0) << "Header block is written to file \"" << mFileName << "\".\n";
