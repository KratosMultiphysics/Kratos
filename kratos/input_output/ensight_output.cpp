...
Parameters EnSightOutput::GetDefaultParameters()
{
    ...
    return Parameters(R"(
    {
        ...
        "ensight_file_format"                         : "gold", // Options: "5", "6", "gold"
        ...
        "use_local_ids"                               : false, // When true, use per-part sequential (1-based) IDs instead of global Kratos IDs
      ...
      mOutputSettings(ThisParameters)
{
    ...
    if (ensight_file_format == "5") {
        mEnSightFileFormat = EnSightFileFormat::EnSight5;
    } else if (ensight_file_format == "6") {
        ...
        KRATOS_ERROR << "Unknown ensight_file_format \"" << ensight_file_format << "\". Available options are \"5\", \"6\", \"gold\"" << std::endl;
    }
...
void EnSightOutput::WriteCaseFile()
{
    ...
    // } else if (mEnSightFileFormat == EnSightFileFormat::EnSight6) {
    //     case_file << "type: ensight 6\n\n";
    // } else if (mEnSightFileFormat == EnSightFileFormat::EnSight5) {
    //     case_file << "type: ensight 5\n\n";
    } else {
...
void EnSightOutput::WriteGeometryFile(const std::string& rFileName)
{
    ...
    // Write Header
    const bool is_ensight_5 = mEnSightFileFormat == EnSightFileFormat::EnSight5;
    const bool is_ensight_6 = mEnSightFileFormat == EnSightFileFormat::EnSight6;
    const bool is_ensight_5_or_6 = is_ensight_5 || is_ensight_6;
    if (is_ensight_6) {
    ...
    // EnSight 5 requires a second description line (no format-identifier first line)
    if (is_ensight_5) {
        WriteString(geo_file, "Generated by Kratos Multi-Physics Framework");
    }
    // Options:
    // node id <off/given/assign/ignore>
    // element id <off/given/assign/ignore>
    const bool use_local_ids = mOutputSettings["use_local_ids"].GetBool();
    WriteString(geo_file, use_local_ids ? "node id assign"    : "node id given");
    WriteString(geo_file, use_local_ids ? "element id assign" : "element id given");
    ...
    // Build a global node position map (Kratos ID -> 1-based sequential position) for EnSight 6
    std::unordered_map<std::size_t, std::size_t> global_node_position;
    if (is_ensight_6) {
        ...
        std::size_t pos = 1;
        if (mFileFormat == FileFormat::ASCII) {
            // ASCII: interleaved (id x y z) per node on one line
            ...
            // Binary: blocked layout — all IDs first (if given), then all X, then Y, then Z
            for (const auto& r_node : mrModelPart.Nodes()) {
                global_node_position[r_node.Id()] = pos++;
            }
            if (!use_local_ids) {
    ...
    for (const auto& r_part_data : mPartDatas) {
        ...
        // Write coordinates for EnSight 5 (per-part, id x y z on same line) and EnSight Gold (per-part, separate arrays)
        if (is_ensight_5) {
            // EnSight 5: per-part coordinates
            WriteString(geo_file, "coordinates");
            WriteScalarData(geo_file, r_part_data.PartNodes.size());
            if (mFileFormat == FileFormat::ASCII) {
                // ASCII: interleaved (id x y z) per node
                for (const auto* p_node : r_part_data.PartNodes) {
                    if (!use_local_ids) {
                        WriteScalarData(geo_file, p_node->Id(), false);
                        WriteString(geo_file, " ", false);
                    }
                    WriteScalarData(geo_file, p_node->X(), false);
                    WriteString(geo_file, " ", false);
                    WriteScalarData(geo_file, p_node->Y(), false);
                    WriteString(geo_file, " ", false);
                    WriteScalarData(geo_file, p_node->Z());
                }
            } else {
                // Binary: blocked layout — all IDs (if given), all X, all Y, all Z
                if (!use_local_ids) {
                    for (const auto* p_node : r_part_data.PartNodes) {
                        WriteScalarData(geo_file, p_node->Id(), false);
                    }
                ...
                for (const auto* p_node : r_part_data.PartNodes) {
                    WriteScalarData(geo_file, p_node->Z(), false);
                }
            }
        } else if (mEnSightFileFormat == EnSightFileFormat::EnSightGold) {
            // EnSight Gold: coordinates block — IDs (if given), then X, Y, Z arrays
            WriteString(geo_file, "coordinates");
        ...
        for (const auto& it : r_part_data.PartGeometricalObjects) {
            ...
            if (mFileFormat == FileFormat::BINARY) {
                // Binary (all formats): element IDs as a contiguous block (if given), then connectivity flat
                if (!use_local_ids) {
                ...
                for (const auto* p_geometrical_object : r_geometrical_objects) {
                    ...
                    for (const auto index : connectivity) {
                        WriteScalarData(geo_file, resolve_node_id(index), false);
                    }
                }
            } else {
                // ASCII only: per-element ((id +) connectivity inline)
                for (const auto* p_geometrical_object : r_geometrical_objects) {
                    if (!use_local_ids) {
                        WriteScalarData(geo_file, p_geometrical_object->Id(), !is_ensight_5_or_6, is_ensight_5_or_6);
                    } else if (mFileFormat == FileFormat::ASCII) {
                        WriteString(geo_file, "", false); // no element ID written, no leading tab
                    }
                    GetGeometryConnectivity(*p_geometrical_object, connectivity);
                    for (const auto index : connectivity) {
                        if (mFileFormat == FileFormat::ASCII) WriteString(geo_file, "\t\t", false);
    ...
    for (const auto& r_part_data : mPartDatas) {
        ...
        for (const auto& it : r_part_data.PartGeometricalObjects) {
            ...
            for (const auto* p_geometrical_object : r_geometrical_objects) {
                ...
                WriteScalarData(var_file, flag, new_line, !is_ensight_gold && counter == 1, !new_line);
            }
        ...
        for (const auto& it : r_part_data.PartGeometricalObjects) {
            ...
            if (KratosComponents<Variable<bool>>::Has(rVariableName)) {
                ...
                for (const auto* p_geometrical_object : r_geometrical_objects) {
                    ...
                    WriteScalarData(var_file, value, new_line, !is_ensight_gold && counter == 1, !new_line);
                }
            } else if (KratosComponents<Variable<int>>::Has(rVariableName)) {
                const auto& r_variable = KratosComponents<Variable<int>>::Get(rVariableName);
                for (const auto* p_geometrical_object : r_geometrical_objects) {
