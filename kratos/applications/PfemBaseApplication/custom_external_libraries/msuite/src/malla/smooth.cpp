...
bool malla::smooth_surface()
{
    ...
    while (unohecho&&(++count)<5)
    {
        ...
        for (i=0; i<n.len-1; i++) // compara con uno mayor
        {
            nodo &ni=n[i];
            h21=pown(n[i].h,2);
            if (h21<ERRADM) continue; // si h~0 refina ad-infinitum
            // nodos vecinos
            cpline &nni=nn[i];
            ...
            for (cmin=MAXREAL,nc=-1,j=0; j<nni.len; j++)
            {
                ...
                h22=pown(n[nni[j]].h,2);
                if (h22<ERRADM) continue; // si h~0 refina ad-infinitum
            ...
            while(1)
            {
                ...
                elemento &evuela=e[ie];
                cpline &vie=vecino[ie];
                ...
                else if (t==e_cuadrilatero)
                {
                    ...
                    else
                    {
                        ...
                        if (((ixq+1)%4)==ixv) l=ixv;
                        else l=ixq; // punto de partida
...
//count=1000; continue;
        ...
        for (i=0; i<e.len-1; i++)
        {
            ...
            for (j=0; j<nci; j++)
            {
                ...
                in2=ei[(j+1)%nci];
                h21=pown(n[in1].h,2);
                if (h21<ERRADM) continue;
                h22=pown(n[in2].h,2);
                if (h22<ERRADM) continue;
...
//count=1000;continue;
        ...
        for (i=0; i<e.len; i++)
        {
            ...
            cpline &vi=vecino[i];
            for (hm=0,j=0; j<3; j++) hm+=n[ei[j]].h;
            hm/=3;
...
bool malla::diagonal_swap(int ie1, int ie2, int metodo)
{
    ...
    i21=vc1.index(ie2);
    if (i21==3) return false;
    i12=vc2.index(ie1);
    if (i12==3) return false;
    ...
    {
        punto dir1,dir2,cen1,cen2;
        double rad1,rad2,vol1,vol2;
...
bool malla::diagonal_swap(int metodo)
{
    ...
    while (rehacer)
    {
        ...
        for (j=0; j<3; j++)
        {
            ...
            retval=true;
            if (e[ie2].f.noes(flag1))
            {
                rehacer+=ie2;
                e[ie2].f.set(flag1);
            }
            if (e[ie1].f.noes(flag1))
            {
                rehacer+=ie1;
                e[ie1].f.set(flag1);
            }
...
bool malla::laplace_smooth(int i, double factor)
{
    ...
    cpline &nni=nn[i];
    punto g(0,0,0);
    for (int j=0; j<nni.len; j++) g+=n[nni[j]];
    g/=nni.len;
    nodo &ni=n[i];
...
//????  if (!dir) {dir.resize(e.len); edir=true;}
    ...
    for (i=0; i<e.len-1; i++)
    {
        ...
        for (j=0; j<nv; j++)
        {
            ...
            s[1]=ei[(j+1)%nv];
            a.e+=s;
            if (n[s[0]].f.noes(flag1))
            {
                n[s[0]].f.set(flag1|n_edge);
                a.n+=s[0];
            }
            if (n[s[1]].f.noes(flag1))
            {
                n[s[1]].f.set(flag1|n_edge);
                a.n+=s[1];
            }
        }
    }
...
bool malla::colapsa_aristas_chicas()
{
    ...
    {
        nodo &n0=n[in0];
        if (n0.f.es(n_frontera)) continue;
        pline &nn0=nn[in0];
        for(i=0; i<nn0.len; i++)
        {
            in1=nn0[i];
            if (in1<in0) continue;
            nodo &n1=n[in1];
            if (n1.f.es(n_frontera)) continue;
...
//      if (n0.distancia2(n1)>pown((n0.h+n1.h)/4,2)) continue; //h/2
            ...
            for (j=elms.len-1; j>=0; j--)
            {
                ...
                if (iej!=ilast) // si es el ultimo, simplemente lo elimina
                {
                    ...
                    {
                        v=vl[k];
                        if(v>=0) vecino[v].replace1(ilast,iej);
                    }
            ...
            // recalcula nn
            nn1(in0,&nn0,true);
            for (j=0; j<nn0.len; j++) nn1(nn0[j],&nn[nn0[j]],true);
