...
class ElemBasedLevelSetSolver:
    ...
    def __init__(self, model_part, domain_size, body_force):
            ...
            self.mesh_neighbour_search = FindNodalNeighboursProcess(
            ...
            # extrapolation tools
            self.extrapolation_tools = ElemBasedExtrapolationUtilities(
                model_part)
            ...
            self.convection_linear_solver = BICGSTABSolver(
...
##            self.distance_calculator.CalculateDistances3D(self.model_part.Elements,DISTANCE, self.reorder);
    ...
    def RecalculateDistanceFunction(self):
        self.distance_utils.CalculateDistances(
            self.model_part, DISTANCE, 10000000.0)
...
# self.convection_solver.CalculateProjection(self.model_part,DISTANCE,NODAL_AREA,VELOCITY,MESH_VELOCITY,TEMP_CONV_PROJ);
            ...
            # calculate projections version#2 convecting with darcy_velocity /
            # epsilon
...
# self.convection_solver.CalculateProjection(self.model_part,DISTANCE,NODAL_AREA,VELOCITY,MESH_VELOCITY,TEMP_CONV_PROJ);
    ...
    def Extrapolate(self):
        ...
        self.extrapolation_tools.ExtrapolateVelocities(
            self.number_of_extrapolation_layers)
...
# print "MODEL PART FREE *****************************************************************"
    ...
    def CalculateDelta_t(self, delta_t_in):
        ...
        for node in self.model_part.Nodes:
            ...
            kinv = 150.0 * \
                (1.0 - eps) * (1.0 - eps) / (eps * eps * eps * dp * dp)
    # print kinv
    # print max_vel
