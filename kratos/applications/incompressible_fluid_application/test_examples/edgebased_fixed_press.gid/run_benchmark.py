    ...
    for i in range (0,(number_points_x)):
      if(i==j):
        if (i+j<(number_points_x + number_points_y-2)):
          f.write( 'X = ' + str(X_position[i]) + ', Y = ' + str(Y_position[j]) + '    ')
        if (i+j==(number_points_x + number_points_y-2)):
          f.write( 'X = ' + str(X_position[i]) + ', Y = ' + str(Y_position[j]) + '\n')    
      i=i+1
    j=j+1  
...
while(time < final_time):
    ...
    if(time >= next_output_time):
      ...
      Cut_App.GenerateScalarVarCut(fluid_model_part,cut_model_part,variable,isovalue,1,tolerance)
      ...
      gid_io.WriteNodalResults(NONLIN_DARCY_COEF,cut_model_part.Nodes,time,0)
      gid_io.Flush()
