...
//TODO:!!!!reset mFixedVelocityDofset y mFixedVelocityDofValues
		...
		//this variable Frac_Step_Switch is an array_1d just because of the structure of the function calculate....[1] and [2] aren't important
		Frac_Step_Switch[0]=2.0; //this is to exectute the final step of Frac Step
		Frac_Step_Switch[1]=2.0;
		Frac_Step_Switch[2]=2.0;
		ProcessInfo& proc_info = model_part.GetProcessInfo();
		for(ModelPart::ElementIterator im = model_part.ElementsBegin() ; im != model_part.ElementsEnd() ; ++im)
			{
			im->Calculate(VELOCITY, Frac_Step_Switch, proc_info);
