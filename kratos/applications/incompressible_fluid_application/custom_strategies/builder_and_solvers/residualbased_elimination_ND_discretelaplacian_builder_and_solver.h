...
#if !defined(KRATOS_RESIDUAL_BASED_ELIMINATION_ND_DISCRETE_LAPLACIAN_BUILDER_AND_SOLVER )
#define  KRATOS_RESIDUAL_BASED_ELIMINATION_ND_DISCRETE_LAPLACIAN_BUILDER_AND_SOLVER
...
namespace Kratos
{
	...
	*/
	...
	int TDim
	>
	class ResidualBasedEliminationNDDiscreteLaplacianBuilderAndSolver 
	...
	public:
		...
		*/    
		...
		/** Constructor.
		*/
		ResidualBasedEliminationNDDiscreteLaplacianBuilderAndSolver(
		...
		/** Destructor.
		*/
		virtual ~ResidualBasedEliminationNDDiscreteLaplacianBuilderAndSolver(){}
		...
		*/  
			...
			) 
		{
			...
			WeakPointerVector<Node<3> > aux; aux.resize(mActiveNodes.size());
			...
			TSystemVectorType& b)
		{
			...
			{	
				...
				double laplacian_coeff = (tau*eps)/density;
...
// 				KRATOS_WATCH(prod(DN_DX , proj_aux));
/*
			...
			if(TDim == 2)
			{
								...
								in!=mActiveNodes.end(); in++)
				{
				  ...
				  //double nu = in->FastGetSolutionStepValue(VISCOSITY);
					double eps = in->FastGetSolutionStepValue(POROSITY);
					//int index_i = in->GetValue(AUX_INDEX);
					int index_i = in->GetDof(PRESSURE,dof_position).EquationId();
					int base_index = index_i*TDim;
					double nodal_mass = in->FastGetSolutionStepValue(NODAL_MASS);
					//introducing the influence o POROSITY for each node;
					nodal_mass /=eps;
...
#endif /* KRATOS_RESIDUAL_BASED_ELIMINATION_ND_DISCRETE_LAPLACIAN_BUILDER_AND_SOLVER  defined */
