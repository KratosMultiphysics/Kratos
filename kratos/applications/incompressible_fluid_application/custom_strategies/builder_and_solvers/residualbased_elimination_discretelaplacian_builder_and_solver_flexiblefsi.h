...
#if !defined(KRATOS_RESIDUAL_BASED_ELIMINATION_DISCRETE_LAPLACIAN_BUILDER_AND_SOLVER_FLEXIBLEFSI )
#define  KRATOS_RESIDUAL_BASED_ELIMINATION_DISCRETE_LAPLACIAN_BUILDER_AND_SOLVER_FLEXIBLEFSI
...
template<class TSparseSpace,
         ...
         int TDim
         >
class ResidualBasedEliminationDiscreteLaplacianBuilderAndSolverFlexibleFSI
...
public:
    ...
    */
    ...
    /** Constructor.
    */
    ResidualBasedEliminationDiscreteLaplacianBuilderAndSolverFlexibleFSI(
    ...
    /** Destructor.
    */
    virtual ~ResidualBasedEliminationDiscreteLaplacianBuilderAndSolverFlexibleFSI() {}
...
// 						const array_1d<double,3>& v_old = geom[kk].FastGetSolutionStepValue(VELOCITY,1);
                    Gaux += DN_DX(kk,tt)*(fv[tt]/*-v_old[tt]*/);
                }
            }
...
// 			double c = 1e10; //compressibility

// 			////adding the compressibility effect
        for(WeakPointerVector< Node<3> >::iterator in = mActiveNodes.begin();
                in!=mActiveNodes.end(); in++)
        {
            if(in->FastGetSolutionStepValue(IS_INTERFACE) > 0)
            {
                ...
                /*					double delta_p = in->FastGetSolutionStepValue(PRESSURE)- 	in->FastGetSolutionStepValue(PRESSURE_OLD_IT);*/
//
                //b -= Minv/(rho c^2)*delta_p
                double mass = in->FastGetSolutionStepValue(NODAL_MASS);
                double density = in->FastGetSolutionStepValue(DENSITY);
                double bulk = in->FastGetSolutionStepValue(BULK_MODULUS);
                double aaa= (mass/density)*BDFcoeffs[0]/(bulk);
                /*					KRATOS_WATCH(aaa);
                					KRATOS_WATCH(delta_p);*/
// 					b[index_i] -= aaa * delta_p;

                A(index_i,index_i) += aaa;
            }
    ...
    void ComputeMinvMatrices(const double& BDF0, TSystemVectorType& Minv_dt, TSystemVectorType& Minv_stab)
    {
        ...
        if(TDim == 2)
        {
                    ...
                    in!=mActiveNodes.end(); in++)
            {
                ...
                if(in->FastGetSolutionStepValue(IS_INTERFACE) > 0)
                {
                    double nodal_tau_fsi = tmp*in->FastGetSolutionStepValue(TAU);
                    Minv_stab[base_index] = nodal_tau_fsi;
                    Minv_stab[base_index+1] = nodal_tau_fsi;
                }
            }
        }
        else if (TDim == 3)
...
// 					double stab = tau / ( density*area );
                ...
                if(in->FastGetSolutionStepValue(IS_INTERFACE) > 0)
                {
                    ...
                    Minv_stab[base_index] = nodal_tau_fsi;
                    Minv_stab[base_index+1] = nodal_tau_fsi;
                    Minv_stab[base_index+2] = nodal_tau_fsi;
                }
            }
        }
...
#endif /* KRATOS_RESIDUAL_BASED_ELIMINATION_DISCRETE_LAPLACIAN_BUILDER_AND_SOLVER_FLEXIBLEFSI  defined */
