...
class ExplicitStrategy:
    ...
    def __init__(self, model_part, domain_size):
        ...
        if (self.homogeneous_material_OPTION == "ON"):
            self.general_density            = GeneralDensity
            self.general_young_mod          = GeneralYoungModulus
            self.general_poisson_ratio      = GeneralPoissonRatio
            self.general_cohesion           = GeneralCohesion
            self.general_rota_damp_ratio    = GeneralRollingFriction
            self.general_rolling_friction   = GeneralRotaDampRatio
            self.general_stat_friction_coef = GeneralStaticFrictionCoef
            self.general_dyn_friction_coef  = GeneralDynamicFrictionCoef
            self.general_restitution_coef   = GeneralRestitutionCoef
    ...
    def Initialize(self):
        ...
        if (self.homogeneous_material_OPTION == "ON"):
            ...
            #self.model_part.ProcessInfo.SetValue(GENERAL_DENSITY, self.general_young_mod)
            #self.model_part.ProcessInfo.SetValue(GENERAL_YOUNG_MODULUS, self.general_poisson_ratio)
            #self.model_part.ProcessInfo.SetValue(GENERAL_POISSON_RATIO, self.general_cohesion)
            #self.model_part.ProcessInfo.SetValue(GENERAL_COHESION, self.general_rota_damp_ratio)
            #self.model_part.ProcessInfo.SetValue(GENERAL_ROLLING_FRICTION, self.general_rolling_friction)
            #self.model_part.ProcessInfo.SetValue(GENERAL_STATIC_FRICTION, self.general_stat_friction_coef)
            #self.model_part.ProcessInfo.SetValue(GENERAL_DYNAMIC_FRICTION, self.general_dyn_friction_coef)
            #self.model_part.ProcessInfo.SetValue(GENERAL_RESTITUTION_COEF, self.general_restitution_coef)
