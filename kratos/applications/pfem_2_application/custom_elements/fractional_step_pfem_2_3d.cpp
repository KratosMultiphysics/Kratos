...
#include "custom_elements/fractional_step_pfem_2_3d.h"
...
namespace Kratos
{
	...
	//************************************************************************************
	//************************************************************************************
	FractionalStepPFEM23D::FractionalStepPFEM23D(IndexType NewId, GeometryType::Pointer pGeometry)
	...
	//************************************************************************************
	//************************************************************************************
	FractionalStepPFEM23D::FractionalStepPFEM23D(IndexType NewId, GeometryType::Pointer pGeometry,  PropertiesType::Pointer pProperties)
		: Element(NewId, pGeometry, pProperties)
	{

	}

	Element::Pointer FractionalStepPFEM23D::Create(IndexType NewId, NodesArrayType const& ThisNodes,  PropertiesType::Pointer pProperties) const
	{
		return Element::Pointer(new FractionalStepPFEM23D(NewId, GetGeometry().Create(ThisNodes), pProperties));
	}

	FractionalStepPFEM23D::~FractionalStepPFEM23D()
	{
	}
	...
	void FractionalStepPFEM23D::CalculateLocalPressureSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	{
        ...
        boost::numeric::ublas::bounded_matrix<double, (TDim+1)*TDim , 1 > fract_vel;
        ...
        //if (split_element)
		if((geom[0].FastGetSolutionStepValue(DISTANCE)*geom[1].FastGetSolutionStepValue(DISTANCE))<0.0 || (geom[0].FastGetSolutionStepValue(DISTANCE)*geom[2].FastGetSolutionStepValue(DISTANCE))<0.0
		|| (geom[0].FastGetSolutionStepValue(DISTANCE)*geom[3].FastGetSolutionStepValue(DISTANCE))<0.0)
        {
			...
			for (unsigned int i = 0; i!=ndivisions; i++) //the 3 partitions of the triangle
			{
				...
				laplacian_matrix_factor +=volumes(i)*laplacian_coeffs(i);
			}
			...
			boost::numeric::ublas::bounded_matrix<double, 1, TDim+1 > mixed_Laplacian;
			...
			boost::numeric::ublas::bounded_matrix<double, 1, (TDim+1)*TDim > D_matrix_mixed;
			...
			{
				Laplacian_enrich(0,0) -= (pow(gauss_gradients[i](0,0),2)+pow(gauss_gradients[i](0,1),2)+pow(gauss_gradients[i](0,2),2))*volumes(i)*laplacian_coeffs(i); //add z direction in 3d
				...
				{
					mixed_Laplacian(0,j) -= (DN_DX(j,0)*(gauss_gradients[i](0,0))+ DN_DX(j,1)*(gauss_gradients[i](0,1)) + DN_DX(j,2)*(gauss_gradients[i](0,2)))*volumes(i)*laplacian_coeffs(i);
					...
					D_matrix_mixed(0,j*TDim+2) -= gauss_gradients[i](0,2) *volumes(i)*Ngauss(i,j); //3D
					 
					//D_matrix_mixed(0,j*TDim) += DN_DX(j,0) * volumes(i) * Nenriched(i,0); //D matrix
					//D_matrix_mixed(0,j*TDim+1) += DN_DX(j,1) * volumes(i) * Nenriched(i,0); //D matrix
					//D_matrix_mixed(0,j*TDim+2) += DN_DX(j,2) * volumes(i) * Nenriched(i,0); //D matrix
        ...
        boost::numeric::ublas::bounded_matrix<double, TDim+1,1 > temp_rhs;
		...
		void FractionalStepPFEM23D::CalculateViscousRHS(ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY
					...
					boost::numeric::ublas::bounded_matrix<double, (TDim+1), 3 > fluid_velocities = ZeroMatrix((TDim+1), 3);	
					boost::numeric::ublas::bounded_matrix<double, (TDim+1), TDim > DN_DX;
					array_1d<double, (TDim+1) > N;
					double Area;
					Geometry<Node<3> >& geom = this->GetGeometry();
					...
					|| (geom[0].FastGetSolutionStepValue(DISTANCE)*geom[3].FastGetSolutionStepValue(DISTANCE))<0.0) 
					{
					    ...
					    boost::numeric::ublas::bounded_matrix<double, TDim*(TDim+1), TDim*(TDim+1) > Viscosity_matrix = ZeroMatrix(TDim*(TDim+1), TDim*(TDim+1)); 
	...
	void FractionalStepPFEM23D::CalculatePressureProjection(ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY
					...
					boost::numeric::ublas::bounded_matrix<double, (TDim+1), TDim*(TDim+1) > G_matrix_no_ro; //(gradient)
					...
					boost::numeric::ublas::bounded_matrix<double, 1, TDim*(TDim+1) > G_matrix_mixed_no_ro;
					...
					boost::numeric::ublas::bounded_matrix<double, 1, TDim*(TDim+1)> G_matrix_mixed_jump_no_ro;
					...
					if((geom[0].FastGetSolutionStepValue(DISTANCE)*geom[1].FastGetSolutionStepValue(DISTANCE))<0.0 || (geom[0].FastGetSolutionStepValue(DISTANCE)*geom[2].FastGetSolutionStepValue(DISTANCE))<0.0
					|| (geom[0].FastGetSolutionStepValue(DISTANCE)*geom[3].FastGetSolutionStepValue(DISTANCE))<0.0) //do not forget the 3d
					{
	...
	void FractionalStepPFEM23D::AddViscousTerm(boost::numeric::ublas::bounded_matrix<double, (3-1)*6, (3-1)*6 >& rDampMatrix,
                         boost::numeric::ublas::bounded_matrix<double, (3+1), 3 >& rShapeDeriv,
                         const double Weight)
	{

        const unsigned int TDim=3;
		
		boost::numeric::ublas::bounded_matrix<double, (TDim+1)*TDim,(TDim-1)*3 > B_matrix = ZeroMatrix((TDim+1)*TDim,(TDim-1)*3);
		for (unsigned int i=0; i!=(TDim+1); i++) //i node
		{
			...
			B_matrix(i*(TDim)+0,3)=rShapeDeriv(i,1);
			B_matrix(i*(TDim)+1,3)=rShapeDeriv(i,0);
			
			B_matrix(i*(TDim)+0,4)=rShapeDeriv(i,2);
			B_matrix(i*(TDim)+2,4)=rShapeDeriv(i,0);
			
			B_matrix(i*(TDim)+2,5)=rShapeDeriv(i,1);
			B_matrix(i*(TDim)+1,5)=rShapeDeriv(i,2);
...
template<class T>
bool FractionalStepPFEM23D::InvertMatrix(const T& input, T& inverse)
{
