...
//        const unsigned int LocalSize = (2 + 1) * TNumNodes;
		...
		//if( ((split_element==false) ) )
		{
			...
			TauOne =  10.0 / ( ( 1.0/ delta_t + 4.0 * Viscosity / (mElemSize * mElemSize) + 2.0 * AdvVelNorm / mElemSize) );
		}
        ...
        if (split_element)
        {
			...
			for (unsigned int i = 0; i!=ndivisions; i++) //the 3 partitions of the triangle
			{
				...
				inv_densities(i)=delta_t/partition_densities(i);	
		...
		else //uncut(normal, not interfase) element
        {
			...
			if (geom[0].FastGetSolutionStepValue(DISTANCE)<0.0)
				inv_density = delta_t/rCurrentProcessInfo[DENSITY_WATER];
			else
				inv_density = delta_t/rCurrentProcessInfo[DENSITY_AIR];
	...
	void PFEM22D::AddViscousTerm(MatrixType& rDampMatrix,
	...
	void PFEM22D::AddViscousTerm(boost::numeric::ublas::bounded_matrix<double, (2-1)*6, (2-1)*6 >& rDampMatrix,
