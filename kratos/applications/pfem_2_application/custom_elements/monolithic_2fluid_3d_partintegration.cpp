...
namespace Kratos
{
	...
	void MonolithicAutoSlipPFEM23D::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	{
			...
			*/
			...
			for (unsigned int i = 0; i < 4; i++)
			{
				...
				if (use_press_proj)
					...
					rRightHandSideVector(i*4+3) -= TauOne*(DN_DX(i,0)*(gravity(0))+DN_DX(i,1)*(gravity(1))+DN_DX(i,2)*(gravity(2)))*Area*1.0;
		...
		else //split element:
		{
			...
			array_1d<double,6>  viscosities(0);
			...
			for (unsigned int i = 0; i < 4; i++)
			{
				for (unsigned int j = 0; j < 4; j++)
				{
					...
					{
						D_matrix(i, (j*3+k) ) =  DN_DX(j,k)*Area*one_quarter; //(i,k) -> standard
						G_matrix(i, (j*3+k) ) =  -DN_DX(i,k)*Area*one_quarter; //(i,k) -> integrated by parts ( or G)
					}
				}
			}
			/*
			...
			*/
			...
			for (unsigned int i = 0; i < 4; i++)
			{
				...
				{	
					for (unsigned int k = 0; k < 3; k++)
					{
						rLeftHandSideMatrix(i*4+3, j*4+k ) -= G_matrix(i,j*3+k);//  + DN_DX(i,0)*TauOne*one_third*Area/delta_t;     
						rLeftHandSideMatrix(j*4+k, i*4+3 ) -=  G_matrix(i,j*3+k);
					}
					rLeftHandSideMatrix(i*4+3, j*4+3 ) += Laplacian_matrix(i,j);
				}
			}
	...
	void MonolithicAutoSlipPFEM23D::CalculateLocalThermalSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
	{
		KRATOS_TRY
...
//        const unsigned int LocalSize = (2 + 1) * TNumNodes;
	...
	void MonolithicAutoSlipPFEM23D::ThermalEquationIdVector(EquationIdVectorType& rResult, ProcessInfo& CurrentProcessInfo)
	{
	...
	void MonolithicAutoSlipPFEM23D::GetThermalDofList(DofsVectorType& ElementalDofList,ProcessInfo& CurrentProcessInfo)
	{
		unsigned int number_of_nodes = GetGeometry().PointsNumber();
	...
	void MonolithicAutoSlipPFEM23D::AddViscousTerm(boost::numeric::ublas::bounded_matrix<double, 12, 12 >& rDampMatrix,
	...
	void MonolithicAutoSlipPFEM23D::AddViscousTerm(boost::numeric::ublas::bounded_matrix<double, 21, 21 > & output,
										  boost::numeric::ublas::bounded_matrix<double, (4), 3 >& rShapeDeriv,
										  array_1d<double,4>&  distances,
