...
class Model:
        def __init__( self, problem_name, path ):
                ...
                ## DEFINE MODELPART ##############################################
                ##################################################################
                self.model_part = ModelPart("beam_3d_umat_example")
                self.path = path
...
#                        #item.Free(AIR_PRESSURE)
        ...
        def InitializeModel( self, linear_elastic ):
                ...
                ##################################################################
                #set material parameters for UMAT material
                params1 = Vector(3)
                params1[0] =      0 # material ID: von mises
                params1[1] = 1.0e11 # young's modulus
                params1[2] =    0.3 # Poisson Ratio
                #params1[3] = 5.0e10 # sigma_yield
                #params1[4] = 1.0e11 # E_yield
                self.model_part.Properties[1].SetValue(MATERIAL_PARAMETERS, params1 )
                self.model_part.Properties[1].SetValue(CONSTITUTIVE_LAW, Umat() )
                print "UMAT material selected"        
