    ...
    */
    ...
    )
    {
        ...
        for (ModelPart::NodeIterator i = r_model_part.NodesBegin();
                i != r_model_part.NodesEnd(); ++i)
        {
            //predicting displacement = PreviousRotation + PreviousVelocity * DeltaTime;
            //ATTENTION::: the prediction is performed only on free nodes
            ...
            if ((i->pGetDof(DISPLACEMENT_X))->IsFixed() == false)
            {
                CurrentRotation[0] = PreviousRotation[0];// + DeltaTime * PreviousVelocity[0];
            }
            else
            {
                CurrentRotation[0]  = PreviousRotation[0] + ImposedRotation[0];//to impose fixed displacements;
                //PreviousRotation[0] = 0;
            }

            if (i->pGetDof(DISPLACEMENT_Y)->IsFixed() == false)
            {
                CurrentRotation[1] = PreviousRotation[1]; //+ DeltaTime * PreviousVelocity[1];
            }
            else
            {
                CurrentRotation[1]  = PreviousRotation[1] + ImposedRotation[1];//to impose fixed displacements;
            ...
            if (i->HasDofFor(DISPLACEMENT_Z))
            {
                if (i->pGetDof(DISPLACEMENT_Z)->IsFixed() == false)
                {
                    CurrentRotation[2] = PreviousRotation[2]; // + DeltaTime * PreviousVelocity[2];
                }
                else
                {
                    CurrentRotation[2]  = PreviousRotation[2] + ImposedRotation[2];//to impose fixed displacements;
