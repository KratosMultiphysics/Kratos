...
public:
     ...
     * Specialisations of element.h (e.g. the UpdatedLagrangian) must specify the actual
...
private:
    ...
    /**
     * Calculation of the Strain. E = 0.5 * (FT * F - 1)
     */
    void CalculateGreenLagrangeStrain( const Matrix & rF,
                                       Vector& rStrainVector ); //total strain

    /**
     * Calculation of the StrainIncrement. DE = 0.5 * (FT * Grad (u) + GradT (u) * F) = BL * u
     */
    void CalculateGreenLagrangeStrainIncrement( const Matrix & rF,
            const Matrix& rDN_DX,
            Vector& rStrainVector ); //current increment


    /**
     * Calculation of the StrainDerivative. dE/dt = 0.5 * (FT * Grad (v) + GradT (v) * F)
     */
    void CalculateGreenLagrangeStrainDerivative( const Matrix & rF,
            const Matrix& rDN_DX,
            Vector& rStrainVector ); //current increment
    ...
    UpdatedLagrangianElement() : Element()
    {
    }
...
/*  inline std::istream& operator >> (std::istream& rIStream,
    UpdatedLagrangianElement& rThis);
*/
/// output stream function
/*  inline std::ostream& operator << (std::ostream& rOStream,
    const UpdatedLagrangianElement& rThis)
    {
...
#endif // KRATOS_UPDATED_LAGRANGIAN_UPDATED_ELEMENT_H_INCLUDED  defined 
