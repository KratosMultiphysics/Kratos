...
namespace Kratos
{
							   ...
							   bool CalculateResidualVectorFlag )
  {
	...
	// std::cout<<" F0: "<<F0<<" detF0: "<<Variables.detF0<<std::endl;
        // std::cout<<" Strain: "<<Variables.StrainVector<<std::endl;
        // std::cout<<" Stress Cauchy: "<<Variables.StressVector<<std::endl;
	// Vector StressPK2=Variables.StressVector;
	// mConstitutiveLawVector[PointNumber]->TransformStresses(StressPK2,Variables.F,Variables.detF,ConstitutiveLaw::StressMeasure_Cauchy,ConstitutiveLaw::StressMeasure_PK2);
	// std::cout<<" Stress PK2: "<<StressPK2<<std::endl;
  ...
  void SpatialLagrangianElement::CalculateOnIntegrationPoints( const Variable<Matrix >& rVariable, std::vector< Matrix >& rOutput, const ProcessInfo& rCurrentProcessInfo )
  {
    ...
    for ( unsigned int PointNumber = 0; PointNumber < integration_points.size(); PointNumber++ )
	...
	else if ( rVariable == CONSTITUTIVE_MATRIX )
	  {
	    ...
	    //Constitutive Tensor for contact in the last configuration:
