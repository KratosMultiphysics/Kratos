...
#if !defined(KRATOS_RECONSTRUCT_MESH_BOUNDARY_PROCESS_FOR_FLUIDS_H_INCLUDED )
#define  KRATOS_RECONSTRUCT_MESH_BOUNDARY_PROCESS_FOR_FLUIDS_H_INCLUDED
...
namespace Kratos
{
   ...
   */
  class ReconstructMeshBoundaryProcessForFluids
    : public Process
  {
  public:
    ///@name Type Definitions
    ...
    /// Pointer definition of ReconstructMeshBoundaryProcessForFluids
    KRATOS_CLASS_POINTER_DEFINITION( ReconstructMeshBoundaryProcessForFluids );
    ...
    /// Default constructor.
    ReconstructMeshBoundaryProcessForFluids(ModelPart& rModelPart,
    ...
    /// Destructor.
    virtual ~ReconstructMeshBoundaryProcessForFluids()
    {
    }
	...
	std::cout<<" [ Skin Search on Mesh["<<mMeshId<<"] ]"<<std::endl;
      ...
      std::cout<<"--> reconstruct mesh boundary...Execute() "<<std::endl;
    ...
    {
      return "ReconstructMeshBoundaryProcessForFluids";
    }

    /// Print information about this object.
    virtual void PrintInfo(std::ostream& rOStream) const
    {
      rOStream << "ReconstructMeshBoundaryProcessForFluids";
    }
  ...
  private:
      ...
      for(ModelPart::NodesContainerType::const_iterator in = mrModelPart.NodesBegin(MeshId); in!=mrModelPart.NodesEnd(MeshId); in++)
	{
      ...
      std::cout<<"BOUNDARY NODES: "<<countWall<<"   RIGID NODES: "<<countWall<<"    FREE-SURFACE NODES: "<<countFreeSurf<<std::endl;
      ...
      for(ModelPart::ConditionsContainerType::iterator ic = TemporaryConditions.begin(); ic!= TemporaryConditions.end(); ic++)
	{
      ...
      std::cout<<"TemporaryConditions.size() "<<TemporaryConditions.size()<<std::endl;
      ConditionId=0;
		...
		if( element_neighbour == elements_end )
		  {
			...
			if( ic->IsNot(TO_ERASE) ){
			  ...
			  if( ic->IsNot(CONTACT) ){
				...
				// else{
				//   std::cout<<"___ condition not found"<<std::endl;
				// }
			      }
			...
			if(condition_found==true){
			   ...
			   // std::cout<<" Condition Found:  "<<ic->Id()<<" ("<<ic->GetGeometry()[0].Id()<<", "<<ic->GetGeometry()[1].Id()<<") ==  ->  Used : "<<PreservedConditions[ic->Id()-1]<<" times "<<std::endl;
			  break;
			}
		      }
		      ...
		      if(condition_found)
			{
			  ...
			  // std::cout<<"   FOUND CONDITION :: CREATED-> ["<<ConditionId<<"] ("<<FaceNodes[0].Id()<<","<<FaceNodes[1].Id()<<")"<<std::endl;
    ...
    /// Assignment operator.
    ReconstructMeshBoundaryProcessForFluids& operator=(ReconstructMeshBoundaryProcessForFluids const& rOther);

    /// Copy constructor.
    //ReconstructMeshBoundaryProcessForFluids(ReconstructMeshBoundaryProcessForFluids const& rOther);
  ...
  inline std::istream& operator >> (std::istream& rIStream,
				    ReconstructMeshBoundaryProcessForFluids& rThis);

  /// output stream function
  inline std::ostream& operator << (std::ostream& rOStream,
				    const ReconstructMeshBoundaryProcessForFluids& rThis)
  {
...
#endif // KRATOS_RECONSTRUCT_MESH_BOUNDARY_PROCESS_FOR_FLUIDS_H_INCLUDED  defined 
