...
def AddDofs(model_part, config=None):
    ...
    print("::[KPFEM-Mechanical Solver]:: DOF's ADDED")


class PfemMechanicalSolver:
    #
    ...
    def __init__(self, model_part, domain_size):
        ...
        print("::[KPFEM-Mechanical Solver]:: -START-")
    ...
    def Initialize(self):
        ...
        print("::[KPFEM-Mechanical Solver]:: -END- ")
    ...
    def SetSolutionScheme(self):
        ...
        if(self.time_integration_method == "Explicit"):
          ...
          if(self.scheme_type == "StaticSolver"):
              if (self.water_pressure_dofs):
                  print(" KPFEM_thisAndThat, UwPStaticScheme ")
                  self.mechanical_scheme = ResidualBasedUwPStaticScheme()
              else:
    ...
    def SetConvergenceCriterion(self):
        ...
        if(self.rotation_dofs):
            ...
            if(self.echo_level > 1):
                print("::[KPFEM-Mechanical Solver]:: CONVERGENCE CRITERION : ", self.convergence_criterion_type)
...
def CreateSolver(model_part, config):
    ...
    print( " ITS YOU; ITS ME; ITS DANCING ")

    structural_solver = PfemMechanicalSolver(model_part, config.domain_size)
    ...
    if(hasattr(config, "WaterPressureDofs")):
        structural_solver.water_pressure_dofs = config.WaterPressureDofs  # bool
    if(hasattr(config, "PressureDofs")):
    ...
    if(hasattr(config, "linear_solver_config")):
        if(config.echo_level > 1):
            print("::[KPFEM-Mechanical Solver]:: LINEAR SOLVER : ", config.linear_solver_config.solver_type)
