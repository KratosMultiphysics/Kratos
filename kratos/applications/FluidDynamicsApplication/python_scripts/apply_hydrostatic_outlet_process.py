...
class ApplyHydrostaticOutletProcess(python_process.PythonProcess):
    ...
    def ExecuteBeforeSolutionLoop(self):
        ...
        if self.outlet_type == "hydrostatic":
            ...
            body_force_norm = (self.body_force[0]*self.body_force[0]+self.body_force[1]*self.body_force[1]+self.body_force[2]*self.body_force[2])**0.5
            
            h_top = self.parameters["h_top"].GetDouble()
            rho = self.parameters["hydrostatic_density"].GetDouble()
        
            ##### Body force in x-direction #####
            if abs(self.body_force[0]) > tol:
                x_coords_list = []
                gravity = abs(self.body_force[0])
                
                for node in self.model_part.Nodes:
                    x_coords_list.append(node.X)
            
                # Body force in negative direction
                if self.body_force[0]<0.0:
                    self.ref_coord = max(x_coords_list)
                    
                    for node in self.model_part.Nodes:
                        hyd_pres = rho*gravity*(h_top+(self.ref_coord-node.X))
                        cur_pres = node.GetSolutionStepValue(KratosMultiphysics.PRESSURE,0)
                
                        node.SetSolutionStepValue(KratosMultiphysics.PRESSURE,0,hyd_pres+cur_pres)
                        node.Fix(KratosMultiphysics.PRESSURE)
            
                # Body force in positive directon
                elif self.body_force[0]>0.0:
            ...
            if abs(self.body_force[1])>1e-5:
                y_coords_list = []
                gravity = abs(self.body_force[1])
            
                for node in self.model_part.Nodes:
                    y_coords_list.append(node.Y)
            
                # Body force in negative direction
                if self.body_force[1]<0.0:
                    self.ref_coord = max(y_coords_list)
                    
                    for node in self.model_part.Nodes:
                        hyd_pres = rho*self.gravity*(h_top+(self.ref_coord-node.Y))
                        cur_pres = node.GetSolutionStepValue(KratosMultiphysics.PRESSURE,0)
                ...
                elif self.body_force[1]>0.0:
                    self.ref_coord = min(y_coords_list)
                    
                    for node in self.model_part.Nodes:
                        hyd_pres = rho*self.gravity*(h_top+(node.Y-self.ref_coord))
                        cur_pres = node.GetSolutionStepValue(KratosMultiphysics.PRESSURE,0)
            ...
            if abs(self.body_force[2])>1e-5:
                z_coords_list = []
                self.gravity = abs(self.body_force[2])
            
                for node in self.model_part.Nodes:
                    z_coords_list.append(node.Z)
                            
                # Body force in negative direction
                if self.body_force[2]<0.0:
                    self.ref_coord = max(z_coords_list)
                    
                    for node in self.model_part.Nodes:
                        hyd_pres = rho*self.gravity*(h_top+(self.ref_coord-node.Z))
                        cur_pres = node.GetSolutionStepValue(KratosMultiphysics.PRESSURE,0)
                ...
                # Body force in positive direction
                elif self.body_force[2]>0.0:
                    self.ref_coord = min(z_coords_list)
                    
                    for node in self.model_part.Nodes:
                        hyd_pres = rho*self.gravity*(h_top+(node.Z-self.ref_coord))
                        cur_pres = node.GetSolutionStepValue(KratosMultiphysics.PRESSURE,0)
