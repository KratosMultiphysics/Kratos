  ...
  */
  class ParallelFillCommunicator
	...
	return buffer.str();
      ...
      virtual void PrintInfo(std::ostream& rOStream) const {rOStream << "ParallelFillCommunicator" << std::endl;}
...
//       {
// 	int my_rank = mrComm.MyPID();
// 	int* non_overlapping_indices = new int[mrModelPart.Nodes().size()];
// 	int* overlapping_indices = new int[mrModelPart.Nodes().size()];
// 	
// 	int all_nodes = 0;
// 	for(ModelPart::NodesContainerType::iterator it = mrModelPart.NodesBegin(); it!=mrModelPart.NodesEnd(); it++)
// 	     overlapping_indices[ all_nodes++ ] = it->Id() - 1; //note the - 1!
// 	
// 	int own_nodes = 0;
// 	for(ModelPart::NodesContainerType::iterator it = mrModelPart.NodesBegin(); it!=mrModelPart.NodesEnd(); it++)
// 	   if(it->FastGetSolutionStepValue(PARTITION_INDEX) == my_rank)
// 	     non_overlapping_indices[ own_nodes++ ] = it->Id() - 1;  //note the - 1!
// 	
// 	//defining the maps
// 	Epetra_Map non_overlapping_map(-1,own_nodes,non_overlapping_indices,0,mrComm );
// 	Epetra_Map overlapping_map(-1,own_nodes,overlapping_indices,0,mrComm );
// 
// 	//defining an Export Class
// 	Epetra_Export exporter( overlapping_map, non_overlapping_map );
// 	
// 	//loop on the nodes we need to send
// 	int NumExportIDs = non_overlapping_map.NumExportIds;
// 	int* ExportTo = non_overlapping_map.ExportPIDs();
// 	int* node_ids = non_overlapping_map.ExportLIDs(); //this are the local Ids. We need to convert to global ids
// 	for(unsigned int i=0; i<NumExportIDs; i++)
// 	  node_ids[i] = non_overlapping_map.GID(node_ids[i])+1; //note the +1 to have the real global ids here!
// 	
// 	//loop on the nodes we need to receive - easy as we have them with a different partition index
// 	
// 	delete [] non_overlapping_indices;
// 	delete [] overlapping_indices;
//       }
...
// 	    ModelPart::NodesContainerType kratos_nodes_to_receive;
	    ...
	    std::cout << "my_rank = " << my_rank << " " <<  nnodes_to_send << std::endl;
...
// 	      r_interface_nodes.push_back(*it);
    ...
    private:
      ...
      /// Assignment operator.
      ParallelFillCommunicator& operator=(ParallelFillCommunicator const& rOther){return *this;}
