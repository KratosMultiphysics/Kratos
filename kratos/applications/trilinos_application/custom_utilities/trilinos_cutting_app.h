...
#define  KRATOS_TRILINOS_LOCAL_REFINE_CUTTING_APP
...
namespace Kratos {
    ...
    public:
	    ...
	    ElementsArrayType& rElements = this_model_part.Elements();
            ...
            double local_smallest_edge = 1000000000000000000000000000.0; //maybe the mesh is huge, setting a big number just in case    
            ...
            for (ElementsArrayType::iterator it = it_begin; it != it_end; ++it) //looping all the elements
            {
                    ...
                    for(unsigned int i = 0; i < it->GetGeometry().size() ; i++) {//edge i
                            ...
                            for(unsigned int j = 0; j < it->GetGeometry().size() ; j++) {//edge i
                                    ...
                                    if ((dist_node_neigh<local_smallest_edge) && (i!=j)) local_smallest_edge=dist_node_neigh; //saving the smallest found up to now
                            }//closing j loop
                    } //closing i loop
            } //closing element loop
            KRATOS_WATCH(local_smallest_edge);
            mrComm.MinAll(&local_smallest_edge, &smallest_edge, 1); //using the global one.
        ...
        void AddSkinConditions(ModelPart& mr_model_part, ModelPart& mr_new_model_part, int plane_number )
        {
            ...
            boost::shared_ptr<Epetra_FEVector > aux_non_overlapping_graph(new Epetra_FEVector(*pmy_map,1,false)); //, guess_row_size
            ...
            //Node < 3 > ::Pointer reference_node = Kratos::Node< TDimension, TDofType >::Node 	( 1, 0.0, 0.0, 0.0);
            //Node < 3 > ::Pointer reference_node = this_model_part.Nodes()(1) ;
            //Node < 3 > ::Pointer reference_node = Kratos::Node< 3 >( 1, 0.0, 0.0, 0.0);
            //reference_node ->SetBufferSize(this_model_part.NodesBegin()->GetBufferSize());
            //reference_node ->GetValue(FATHER_NODES).resize(0);        
                ...
                boost::shared_ptr<Epetra_FECrsMatrix>& used_nodes_matrix)
        {
            ...
            boost::shared_ptr<Epetra_FECrsMatrix> pB = boost::shared_ptr<Epetra_FECrsMatrix > (new Epetra_FECrsMatrix(Copy, *mp_overlapping_map, guess_row_size));
            pB->PutScalar(0.0);
        ...
        void FirstLoop(ModelPart& this_model_part, boost::shared_ptr<Epetra_FECrsMatrix>& p_edge_ids,
                boost::shared_ptr<Epetra_FECrsMatrix>& p_partition_ids, array_1d<double, 3 > versor,
                array_1d<double, 3 > Xp, int& number_of_triangles, vector<int>& Elems_In_Plane, double tolerance,
                boost::shared_ptr<Epetra_FECrsMatrix>& used_nodes_matrix)//
...
*/           
        ...
        void GenerateElements(
                ...
                array_1d<double, 3 > versor, int plane_number, int& number_of_triangles
                ) {
            KRATOS_TRY
            ...
            for (unsigned int k = 0; k != 4; ++k) {
                TriangleNodesArray[k] = -1;
            } //initializing in -1, meaning we have no nodes yet
...
#endif // KRATOS_TRILINOS_LOCAL_CUTTING_APP  defined 
