...
#if !defined(KRATOS_MPIRefineSimplexMesh)
#define  KRATOS_MPIRefineSimplexMesh
    ...
    class MPIRefineSimplexMesh
      {
	      ...
	      typedef PointVector::iterator PointIterator;
              ...
              MPIRefineSimplexMesh(ModelPart& model_part) : mr_model_part(model_part) {}
...
///************************************************************************************************          
///************************************************************************************************  
///this function fills a sparse matrix by looping over the elements and conditions
///the matrix is ordered following the local ids (variable AUX_INDEX);
void CSR_Row_Matrix(ModelPart& this_model_part, mapped_matrix<int>& Coord )
{  
    ...
    //first of all loop on nodes and assign a continuous local id (use AUX_INDEX for this)
    unsigned int k=0;
    for(ModelPart::NodesContainerType::iterator it=this_model_part.NodesBegin(); it!=this_model_part.NodesEnd(); it++)
      it->GetValue(AUX_INDEX) = k++;
          
    //loop on elements and fill the matrix -- note that only the top triangular part is filled
    for(ModelPart::ElementsContainerType::iterator it=this_model_part.ElementsBegin(); it!=this_model_part.ElementsEnd(); it++)
    {
      ...
      {
	unsigned int index_I = geom[k].GetValue(AUX_INDEX);
	for(unsigned int l=0; l<geom.size(); l++)
	{
	  unsigned int index_J = geom[l].GetValue(AUX_INDEX);
	  if(index_J > index_I)
	    Coord(index_I, index_J) =  -1;
	  else
	    Coord(index_J, index_I) =  -1;	  
	}
      }
    }
    
    //loop on conditions and fill the matrix -- note that only the top triangular part is filled
...
///************************************************************************************************          
///************************************************************************************************  
///mark the edges to be refined with a -2. This is first done locally, then communication is performed 
///to syncronize the results
void Search_Edge_To_Be_Refined(ModelPart& this_model_part, mapped_matrix<int>& Coord)
{
  ...
  {     	
    ...
    { 
      ...
      {  
	  int index_i = geom[i].GetValue(AUX_INDEX);
// 	  int partition_index_I = geom[i].FastGetSolutionStepValue(AUX_INDEX);
	  for (unsigned int j = 0; j<geom.size(); j++)
	  {  
// 	      int partition_index_J = geom[i].FastGetSolutionStepValue(AUX_INDEX);
	      int index_j = geom[j].GetValue(AUX_INDEX);
	      if(index_j > index_i)
	      {                     
  ...
  //now gather on each of the processors the rows they own
  
  //perform the sync so that the computation of each row of the matrix is finalized
  
  //now spread the results
}

///************************************************************************************************          
///************************************************************************************************ 

void Create_List_Of_New_Nodes(ModelPart& this_model_part, mapped_matrix<int>& Coord, boost::numeric::ublas::vector<int> &List_New_Nodes,
...
typedef mapped_matrix<int>::iterator1 i1_t;
typedef mapped_matrix<int>::iterator2 i2_t;
