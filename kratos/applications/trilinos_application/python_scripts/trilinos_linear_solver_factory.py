...
def ConstructSolver( configuration ):
    ...
    overlap_level = 0
    if hasattr(configuration, 'overlap_level'): overlap_level=configuration.overlap_level
    ...
    verbosity = 0
    if hasattr(configuration, 'verbosity'): verbosity = configuration.verbosity
	...
	aztec_parameters = ParameterList()
        ...
        if(verbosity == 0): aztec_parameters.set("AZ_output","AZ_none");
        else: aztec_parameters.set("AZ_output",verbosity);
	...
	symmetric = False
	if hasattr(configuration, 'symmetric'): symmetric = configuration.symmetric
	    
        max_it = configuration.max_iteration
	tol    = configuration.tolerance
	
	max_levels = 3
	if hasattr(configuration, 'amg_max_levels'): amg_max_levels = configuration.amg_max_levels
