...
for (i=0  ; i<vertexes->Count() ; i++)
{
 ...
 {
   if (showMessages)
	   printMessage( "Triangulos con vecinos por arista ");
 }

 if (nv > 0) 
 {
   if (showMessages)
	    printMessage("Vertices sueltos ");
 }
 ne = 0;
 for (i = 0 ; i< elements->Count() ; i++)
 {
 ...
 {
    if (showMessages) 
		printMessage("Elementos con mas de 4 vecinos por cara ");
 }

 if (result == 0)
 {
   if (showMessages) 
	   printMessage("No se detectaron errores");
 }
