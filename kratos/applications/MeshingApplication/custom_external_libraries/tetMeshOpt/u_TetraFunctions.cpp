...
void splitMeshin4(TMesh* m)
{
	...
	for (int i=0; i<m->elements->Count() ; i++)
	{
		...
		splitTetrain4(t , vs,ts);
	}

	m->elements->Clear();
	for (int i=0; i<ts->Count() ; i++)
	{
		TTetra *t = (TTetra*)(ts->elementAt(i));
		m->elements->Add(t);
	}
}
...
bool innerGetElementsSurface(TList<TObject*>* elements,TList<TVertex*>* surfaceT,TList<TVertex*>* vertexes)
{
	...
	for (i = 0; i<elements->Count() ; i++)
	{
		...
		for (j = 0 ;j<4 ;j++)
		{
			...
			{
				if( ((surfaceT->structure[3*k] == v0)   ||  (surfaceT->structure[3*k] == v1)   || (surfaceT->structure[3*k] == v2)) &&
					((surfaceT->structure[3*k+1] == v0) ||  (surfaceT->structure[3*k+1] == v1) || (surfaceT->structure[3*k+1] == v2)) &&
					((surfaceT->structure[3*k+2] == v0) ||  (surfaceT->structure[3*k+2] == v1) || (surfaceT->structure[3*k+2] == v2)) )
				{
					faceVisited = true;
					break;
				}
	...
	for (i = 0; i<elements->Count() ; i++)
	{
		...
		for (j = 0 ;j<4 ;j++)
		{
			...
			for (k = 0 ; k<surfaceT->Count()/3 ; k = k+1)
			{
				if( ((surf[3*k] == v0)   ||  (surf[3*k] == v1)   || (surf[3*k] == v2)) &&
					((surf[3*k+1] == v0) ||  (surf[3*k+1] == v1) || (surf[3*k+1] == v2)) &&
					((surf[3*k+2] == v0) ||  (surf[3*k+2] == v1) || (surf[3*k+2] == v2)) )
				{
					faceVisited = true;
					break;
				}
