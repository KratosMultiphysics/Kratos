...
bool malla::mk_vecino(bool remake)
{
    ...
    for (i=0; i<nlen; i++) // cada nodo
    {
        ...
        if (!enlen)
        {
            n[i].f.set(n_frontera);    // nodo aislado
            continue;
        }
        cn.clean();
        elc.clean();
        ...
        for(j=0; j<enlen; j++) // cada elemento del nodo
        {
            ...
            for(k=0; k<nc; k++) // cada cara
            {
                ...
                if (l==cn.len)   // agrega la cara
                {
                    ...
                    cn+=c;
                    elc+=ie;
                    ic+=k; // el flag es para contar
                }
                else  // la cara ya fue puesta
                {
                    qv[l]++;
                    iv=elc[l];
                    ie_v=ic[l]; // indice del vecino y de la cara en iv
...
bool malla::piezas_conexas()
{
    ...
    while (qetotal<qe)
    {
        ...
        for (in=lastin; in<qn; in++)
        {
            if (n[in].f.noes(flag1)&&n[in].e) break;
        }
