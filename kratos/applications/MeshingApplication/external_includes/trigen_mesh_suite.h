...
//			for(ModelPart::NodesContainerType::iterator in = ModelNodes.begin(); in!=ModelNodes.end(); in++)
			for(ModelPart::NodesContainerType::iterator in = ThisModelPart.NodesBegin(); in!=ThisModelPart.NodesEnd(); in++)
			{
				pippo+=in->FastGetSolutionStepValue(IS_FLUID);
				//a node without any element is always boundary
//				if(in->FastGetSolutionStepValue(IS_FLUID) == 1)
//					{
//					aaa++;					
//					}
			}
...
*/
//			triangulate("NPB", &in, &out, &vorout); //version with no elemental neighbours
			...
			for(int el = 0; el< el_number; el++)
			{
				...
				int nboundary = int( temp[0].FastGetSolutionStepValue(IS_FREE_SURFACE) );
				nboundary += int( temp[1].FastGetSolutionStepValue(IS_FREE_SURFACE) );
				nboundary += int( temp[2].FastGetSolutionStepValue(IS_FREE_SURFACE) );
				...
				{
					if (nboundary != 0 || nfluid != 3)  //in this case it is close to the surface so i should use alpha shape 
					{
