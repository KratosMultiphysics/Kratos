...
class FiniteElementMapper:
    def __init__(self, name, dimension,  modelPart1, modelPart2, dual=False, oppositeSurfaceNormal=False, enforceConsistency=True):
        ...
        libMapper.init_FE_MortarMapper(b"defaultMapper", c_numNodes1[0], c_numElems1[0], c_numNodesPerElem1, c_nodes1, c_nodeIDs1, c_elems1, c_numNodes2[0], c_numElems2[0], c_numNodesPerElem2, c_nodes2, c_nodeIDs2, c_elems2, c_oppositeSurfaceNormal[0], c_dual[0], c_enforceConsistency[0])
...
# No model parts are stored in this class. only names are stored. Then comparison to the names is done. 
    def doMapping(self, modelPart1, dataField1, modelPart2, dataField2):
        ...
        if(modelPart1.Name == self.modelPart1 and modelPart2.Name == self.modelPart2):
            libMapper.doConsistentMapping(b"defaultMapper", self.dimension, c_lenDf1[0], c_df1, c_lenDf2[0], c_df2)
            i=0
            val = Vector(3)
            for node in modelPart2.Nodes:
                val[0] = c_df2[3*i + 0]
        ...
        if(modelPart1.Name == self.modelPart2 and modelPart2.Name == self.modelPart1):
            libMapper.doConservativeMapping(b"defaultMapper", self.dimension, c_lenDf1[0], c_df1, c_lenDf2[0], c_df2)
            i=0
            val = Vector(3)
            for node in modelPart2.Nodes:
                val[0] = c_df1[3*i + 0]
