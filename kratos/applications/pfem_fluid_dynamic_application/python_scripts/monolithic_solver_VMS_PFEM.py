...
def AddVariables(model_part):
 ...
 #  model_part.AddNodalSolutionStepVariable(SOUND_VELOCITY)
  ...
  #  model_part.AddNodalSolutionStepVariable(NEIGHBOUR_NODES)
  #  model_part.AddNodalSolutionStepVariable(NEIGHBOUR_ELEMENTS)
...
class MonolithicSolver:
    ...
    def __init__(self, model_part, domain_size, box_corner1, box_corner2):
        ...
        #self.linear_solver = AMGCLSolver(AMGCLSmoother.DAMPED_JACOBI, AMGCLIterativeSolverType.BICGSTAB, self.tol, self.max_it, self.verbosity, self.m)
...
# self.remeshing_flag = True
      ...
      #  self.mark_close_nodes_process = MarkCloseNodesProcess(model_part);
        ...
        if(domain_size == 2):
            self.Mesher = TriGenPFEMModelerVMS()
            self.neigh_finder = FindNodalNeighboursProcess(model_part, 9, 18)
        elif(domain_size == 3):
            self.Mesher = TetGenPfemModelerVms()
