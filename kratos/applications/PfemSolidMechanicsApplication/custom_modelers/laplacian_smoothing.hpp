 ...
 */
	...
	static const Flags::FlagType SMOOTH_ALL          = Flags::Flag1;
	...
	LaplacianSmoothing(ModelPart& rModelPart)
	  ...
	  for (unsigned int i = 0; i < rModelPart.NumberOfMeshes(); i++)
            mPreviousMeshes.push_back((rModelPart.GetMesh(i)).Clone()); //in fact do not clones
				  ...
				  ModelPart::IndexType MeshId=0)
	{
	  ...
	  if(iters==smoothing_iters && !converged)
	    std::cout<<" Laplacian smoothing convergence NOT achieved "<<std::endl;
