...
namespace Kratos
{
  ...
  KRATOS_CREATE_LOCAL_FLAG( AxisymContactDomain2DCondition, ACTIVE, 4 );

//******************************CONSTRUCTOR*******************************************
//************************************************************************************

AxisymContactDomain2DCondition::AxisymContactDomain2DCondition( IndexType NewId, GeometryType::Pointer pGeometry )
...
AxisymContactDomain2DCondition::AxisymContactDomain2DCondition( IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties )
...
AxisymContactDomain2DCondition::AxisymContactDomain2DCondition( AxisymContactDomain2DCondition const& rOther)
    :Condition(rOther)
...
AxisymContactDomain2DCondition&  AxisymContactDomain2DCondition::operator=(AxisymContactDomain2DCondition const& rOther)
{
...
Condition::Pointer AxisymContactDomain2DCondition::Create( IndexType NewId, NodesArrayType const& ThisNodes, PropertiesType::Pointer pProperties ) const
{
    return Condition::Pointer(new AxisymContactDomain2DCondition( NewId, GetGeometry().Create( ThisNodes ), pProperties ) );
}
...
AxisymContactDomain2DCondition::~AxisymContactDomain2DCondition()
{
}
...
AxisymContactDomain2DCondition::IntegrationMethod AxisymContactDomain2DCondition::GetIntegrationMethod() const
{
...
void AxisymContactDomain2DCondition::GetDofList( DofsVectorType& rConditionalDofList, ProcessInfo& rCurrentProcessInfo )
{
...
void AxisymContactDomain2DCondition::EquationIdVector( EquationIdVectorType& rResult, ProcessInfo& rCurrentProcessInfo )
{
...
void AxisymContactDomain2DCondition::GetValuesVector( Vector& rValues, int Step )
{
...
void AxisymContactDomain2DCondition::GetFirstDerivativesVector( Vector& rValues, int Step )
{
    const unsigned int number_of_nodes = GetGeometry().size();
    const unsigned int dimension = GetGeometry().WorkingSpaceDimension();
    unsigned int MatSize = (number_of_nodes + 1) * dimension;
...
void AxisymContactDomain2DCondition::GetSecondDerivativesVector( Vector& rValues, int Step )
{
    const unsigned int number_of_nodes = GetGeometry().size();
    const unsigned int dimension = GetGeometry().WorkingSpaceDimension();
    unsigned int MatSize = (number_of_nodes + 1) * dimension;
...
void AxisymContactDomain2DCondition::SetValueOnIntegrationPoints( const Variable<Vector>& rVariable,
...
void AxisymContactDomain2DCondition::SetValueOnIntegrationPoints( const Variable<Matrix>& rVariable,
...
void AxisymContactDomain2DCondition::GetValueOnIntegrationPoints( const Variable<double>& rVariable,
        ...
        const ProcessInfo& rCurrentProcessInfo )
{
    ...
    for ( unsigned int ii = 0; ii < rValues.size(); ii++ )
...
void AxisymContactDomain2DCondition::GetValueOnIntegrationPoints( const Variable<Vector>& rVariable,
        std::vector<Vector>& rValues,
        const ProcessInfo& rCurrentProcessInfo )
{
...
void AxisymContactDomain2DCondition::GetValueOnIntegrationPoints( const Variable<Matrix>& rVariable,
...
void AxisymContactDomain2DCondition::Initialize()
{
    KRATOS_TRY
...
void AxisymContactDomain2DCondition::InitializeSolutionStep( ProcessInfo& CurrentProcessInfo )
{
...
void AxisymContactDomain2DCondition::InitializeNonLinearIteration( ProcessInfo& CurrentProcessInfo )
{
  ClearNodalForces();
...
void AxisymContactDomain2DCondition::FinalizeSolutionStep( ProcessInfo& CurrentProcessInfo )
{
  KRATOS_TRY
...
void AxisymContactDomain2DCondition::ClearNodalForces()
{
    KRATOS_TRY
...
void AxisymContactDomain2DCondition::CalculateTauStab( ProcessInfo& rCurrentProcessInfo )
{
...
void AxisymContactDomain2DCondition::CalculateKinematics(const double& rPointNumber,ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    unsigned int integration_points_number = mpMasterGeometry->IntegrationPointsNumber( MasterElement.GetIntegrationMethod() );
    std::vector<Matrix> DeformationGradientVector( integration_points_number );
    DeformationGradientVector[rPointNumber]=identity_matrix<double>( 3 );
    ...
    if(mpMasterGeometry->IntegrationPointsNumber( MasterElement.GetIntegrationMethod() ) == GetGeometry().IntegrationPointsNumber( mThisIntegrationMethod )){
      mVariables.StressVector=StressVector[rPointNumber];
    }
    else{     //option if master element has several integration points:
      mVariables.StressVector=StressVector[0];
      for(unsigned int i=1; i<integration_points_number; i++)
	{
	  mVariables.StressVector += StressVector[i];
	}
      mVariables.StressVector *= (1.0/double(integration_points_number));
    }
    ...
    //option if master element has one integration point:
    if(mpMasterGeometry->IntegrationPointsNumber( MasterElement.GetIntegrationMethod() ) == GetGeometry().IntegrationPointsNumber( mThisIntegrationMethod )){
      mVariables.StrainVector=MathUtils<double>::StrainTensorToVector( StrainTensor[rPointNumber], mVariables.StrainVector.size() );
    }
    else{     //option if master element has several integration points:
      mVariables.StrainVector=MathUtils<double>::StrainTensorToVector( StrainTensor[0], mVariables.StrainVector.size() );
      for(unsigned int i=1; i<integration_points_number; i++)
	{
	  mVariables.StrainVector += MathUtils<double>::StrainTensorToVector( StrainTensor[i], mVariables.StrainVector.size() );
	}
      mVariables.StrainVector *= (1.0/double(integration_points_number));
    }
...
void AxisymContactDomain2DCondition::CalculateDeformationGradient(const Matrix& rDN_DX,
        Matrix& rF )
{
    KRATOS_TRY
...
void AxisymContactDomain2DCondition::CalculateLocalSystem( MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo )
{
    //calculation flags
...
void AxisymContactDomain2DCondition::CalculateRightHandSide( VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo )
{
    //calculation flags
...
void AxisymContactDomain2DCondition::CalculateLeftHandSide( MatrixType& rLeftHandSideMatrix, ProcessInfo& rCurrentProcessInfo )
{
...
void AxisymContactDomain2DCondition::SetMasterGeometry()
...
void AxisymContactDomain2DCondition::CalcPreviousGap() //prediction of the lagrange multiplier
{
    ...
    const unsigned int dimension = GetGeometry().WorkingSpaceDimension();
    Matrix StressMatrix =zero_matrix<double> (dimension+1);
    Matrix F            =zero_matrix<double> (dimension+1);
    ...
    mConstitutiveLawVector[0]->TransformStresses(StressVector,F,detF,ConstitutiveLaw::StressMeasure_Cauchy,ConstitutiveLaw::StressMeasure_PK1);
    ...
    for(unsigned int i=0; i<StressMatrix.size1(); i++)
    {
      for(unsigned int j=0; j<StressMatrix.size2(); j++)
	{
...
inline void AxisymContactDomain2DCondition:: CalcBaseDistances (BaseLengths& Base,array_1d<double, 3>& P1,array_1d<double, 3>& P2,array_1d<double, 3>& PS,array_1d<double, 3>& Normal)
{
...
//************************************************************************************
//************************************************************************************
inline array_1d<double, 3> & AxisymContactDomain2DCondition::ComputeFaceNormal(array_1d<double, 3> &Normal, array_1d<double, 3>& P1, array_1d<double, 3> &P2)
{
...
inline array_1d<double, 3> & AxisymContactDomain2DCondition::ComputeFaceTangent(array_1d<double, 3> &Tangent ,array_1d<double, 3>& P1, array_1d<double, 3> &P2)
{
...
inline array_1d<double, 3> & AxisymContactDomain2DCondition::CalcCurrentTangent ( array_1d<double, 3> &Tangent )
{
...
inline array_1d<double, 3> & AxisymContactDomain2DCondition::ComputeFaceTangent(array_1d<double, 3> &Tangent ,array_1d<double, 3>& Normal)
{
...
inline double AxisymContactDomain2DCondition::CalculateVol(const double x0, const double y0,
...
inline bool AxisymContactDomain2DCondition::CalculatePosition(const double x0, const double y0,
							const double x1, const double y1,
							const double x2, const double y2,
...
inline bool AxisymContactDomain2DCondition::CalculateObtuseAngle(const double x0, const double y0,
							   const double x1, const double y1,
							   const double xc, const double yc)
{
...
inline bool AxisymContactDomain2DCondition::CheckFictiousContacts()
{
  ...
  if(real_contact==false && fabs(projection)>0.707){
    real_contact = true;
    std::cout<<" NORMALS say that this is a REAL CONTACT "<<projection<<std::endl;
  }
...
void AxisymContactDomain2DCondition:: CalcMultipliers(ProcessInfo& rCurrentProcessInfo)
{
    ...
    Matrix StressMatrix ( dimension+1, dimension+1 );
	...
	this->Set(AxisymContactDomain2DCondition::ACTIVE);
    }
    else
    {
        mVariables.Contact.active=0; //normal contact not active
	this->Reset(AxisymContactDomain2DCondition::ACTIVE);
    }
...
void AxisymContactDomain2DCondition:: CalcPenaltyParameters(ProcessInfo& rCurrentProcessInfo)
{
...
void AxisymContactDomain2DCondition::CalcDomainShapeN()
{
...
void AxisymContactDomain2DCondition::FSigmaP(std::vector<Vector > &SigmaP, array_1d<double, 3>& DirVector,unsigned int &ndi,unsigned int &ndj,unsigned int &ndk,unsigned int &ndr)
{
...
void AxisymContactDomain2DCondition::FSigmaPnd(std::vector<Vector > &SigmaP, array_1d<double, 3>& DirVector,unsigned int &ndi,unsigned int &ndj)
{
    //Computation with the ndi and storage to ndj
...
//***********************************************************************
//***********************************************************************
void AxisymContactDomain2DCondition::CalculateRadius(double & rCurrentRadius,
...
void AxisymContactDomain2DCondition::InitializeSystemMatrices(MatrixType& rLeftHandSideMatrix,
...
void AxisymContactDomain2DCondition::CalculateConditionalSystem( MatrixType& rLeftHandSideMatrix,
        VectorType& rRightHandSideVector,
        ...
        bool CalculateResidualVectorFlag )
{
    ...
    mVariables.F.resize( dimension+1, dimension+1 );
	...
	if(mpMasterGeometry->IntegrationPointsNumber( MasterElement.GetIntegrationMethod() ) == GetGeometry().IntegrationPointsNumber( mThisIntegrationMethod )){
	  ...
	  mVariables.ConstitutiveMatrix = ConstitutiveMatrix[0];
	}
	else{     //option if master element has several integration points:
	  mVariables.ConstitutiveMatrix = ConstitutiveMatrix[0];
	  for(unsigned int i=1; i<integration_points_number; i++)
	    {
	      mVariables.ConstitutiveMatrix += ConstitutiveMatrix[i];
	    }
	  mVariables.ConstitutiveMatrix *= (1.0/double(integration_points_number));

	  //std::cout<<" Several constitutive "<<mVariables.ConstitutiveMatrix<<std::endl;
	}
	...
	CalculateRadius (CurrentRadius, ReferenceRadius, mVariables.N);
...
inline void AxisymContactDomain2DCondition::CalcNormalForce (double &F,unsigned int& ndi,unsigned int& idir)
{    
...
inline void AxisymContactDomain2DCondition::CalcTangentStickForce (double &F,unsigned int& ndi,unsigned int& idir)
{
...
inline void AxisymContactDomain2DCondition::CalcTangentSlipForce (double &F,unsigned int& ndi,unsigned int& idir)
{
...
inline void AxisymContactDomain2DCondition::CalculateAndAddContactForces(VectorType& rRightHandSideVector,
...
inline void AxisymContactDomain2DCondition::CalcNormalPenaltyForce (double &F,unsigned int& ndi,unsigned int& idir)
{    
...
inline void AxisymContactDomain2DCondition::CalcTangentStickPenaltyForce (double &F,unsigned int& ndi,unsigned int& idir)
{
    double constant=mVariables.Contact.friction_active;
...
inline void AxisymContactDomain2DCondition::CalcTangentSlipPenaltyForce (double &F,unsigned int& ndi,unsigned int& idir)
{
...
inline void AxisymContactDomain2DCondition::CalculateAndAddContactPenaltyForces(VectorType& rRightHandSideVector,
									  double& rIntegrationWeight)
{
    KRATOS_TRY
...
void AxisymContactDomain2DCondition::CalcContactStiffness (double &Kcont,unsigned int& ndi,unsigned int& ndj,unsigned int& idir,unsigned int& jdir)
{
    Kcont=0;
...
void AxisymContactDomain2DCondition::CalcContactPenaltyStiffness (double &Kcont,unsigned int& ndi,unsigned int& ndj,unsigned int& idir,unsigned int& jdir)
{
    Kcont=0;
...
void AxisymContactDomain2DCondition::CalculateAndAddKm(MatrixType& rK,
						 double& rIntegrationWeight)
...
void AxisymContactDomain2DCondition::CalculateAndAddPenaltyKm(MatrixType& rK,
							double& rIntegrationWeight)
...
void AxisymContactDomain2DCondition::CalculateOnIntegrationPoints( const Variable<double>& rVariable, Vector& rOutput, const ProcessInfo& rCurrentProcessInfo )
{
    ...
    for ( unsigned int ii = 0; ii < rOutput.size(); ii++ )
...
void AxisymContactDomain2DCondition::CalculateOnIntegrationPoints( const Variable<Vector>& rVariable, std::vector<Vector>& rOutput, const ProcessInfo& rCurrentProcessInfo )
{
    unsigned int StrainSize;
...
void AxisymContactDomain2DCondition::CalculateOnIntegrationPoints( const Variable<Matrix >& rVariable, std::vector< Matrix >& rOutput, const ProcessInfo& rCurrentProcessInfo )
{
...
inline void AxisymContactDomain2DCondition::CalcRelativeVelocity (array_1d<double, 3 > & TangentVelocity)
{
...
inline void AxisymContactDomain2DCondition::CalcRelativeDisplacement (array_1d<double, 3 > & TangentDisplacement)
{
...
void AxisymContactDomain2DCondition::CalcFrictionCoefficient (const array_1d<double, 3 > & TangentVelocity)
{
...
void AxisymContactDomain2DCondition::Calculate( const Variable<double>& rVariable, double& rOutput, const ProcessInfo& rCurrentProcessInfo )
{
 ...
 */
int  AxisymContactDomain2DCondition::Check( const ProcessInfo& rCurrentProcessInfo )
{
    KRATOS_TRY
...
void AxisymContactDomain2DCondition::save( Serializer& rSerializer ) const
{
...
void AxisymContactDomain2DCondition::load( Serializer& rSerializer )
{
