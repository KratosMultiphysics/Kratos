...
void ContactDomainLM2DCondition::FSigmaP(GeneralVariables& rVariables, std::vector<Vector > &SigmaP, VectorType& DirVector,unsigned int &ndi,unsigned int &ndj,unsigned int &ndk,unsigned int &ndr)
{
...
void ContactDomainLM2DCondition::FSigmaPnd(GeneralVariables& rVariables, std::vector<Vector > &SigmaP, VectorType& DirVector,unsigned int &ndi,unsigned int &ndj)
{
    //Computation with the ndi and storage to ndj
...
void ContactDomainLM2DCondition::CalcContactStiffness (double &Kcont,GeneralVariables& rVariables,unsigned int& ndi,unsigned int& ndj,unsigned int& idir,unsigned int& jdir)
{
    ...
    //KI:
    Kcont= (0.5/(mContactVariables.StabilizationFactor*rVariables.Contact.ReferenceBase[0].L))*rVariables.Contact.dN_dn[ndi]*rVariables.Contact.CurrentSurface.Normal[idir]*rVariables.Contact.dN_dn[ndj]*rVariables.Contact.CurrentSurface.Normal[jdir]-
