...
print("ToRun.PY:VERSION 11_March_2014")
...
print("XXXXXXXXXXXXXX")
print("XXXXXXXXXXXXXX")
print("CAMBIAAAAAAAAAAAAAAAARRRRRRRRRRRR")
...
if (diastolic_pressure < diastolic_hypermia_pressure):
    ...
    print("diastolic pressure -> ", systolic_pressure,
          "must be higher than diastolic hypermia_pressure-> ", diastolic_pressure)
    print("Please check config file")
    input()
    error
...
if ((artery_type < 5)):
    for i in range(0, len(config.deactivate_list)):
        ...
        if (artery_number_id < 15):
            print("Please check your config file. Artery number = ",
                  artery_number_id, ", dont belong to the ", input_file_name1)
...
if(only1Dtest == False):
    while(total_time < final_time):
		...
		if((math.fmod(step, output_step) == 0) and (step >= 3)):
			...
			# removal_tool.ComputePressure(model_part1D)
			# if ((FitValues)):  # and (step>=sub_step)):
...
if (Coupled_Simulation == True):
    ...
    Only1D.Only1D(model_part1D, config, simulation_config,
                  input_file_name, FFR_Inlet_NODES_Values, FFR_Outlet_NODES_Values)
