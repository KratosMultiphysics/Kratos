...
print("ToRun.PY:VERSION 10_March_2014")
...
# Write Conditions Used
file_test = str("Summary_Case(Pressure).txt")
summary_file_pressure= open(file_test, 'w')
...
if (Coupled_Simulation == False):
    ...
    if (plot3d):
        if(ascii == False):
            ...
            gid_io_centerline = GidIO(
                input_centerline_name, gid_mode, multifile, deformed_mesh_flag, write_conditions)
        else:
...
if(ascii == False):
    ...
    if(plot3d and only1Dtest == False):
        ...
        gid_io_centerline.InitializeResults(
            mesh_name, (model_part_centerline).GetMesh())
...
pressure_parameter_1 /= pressure_parameter_1
print (str(pressure_parameter_1))
...
if (Coupled_Simulation == True):
    ...
    #solver3D.predictor_corrector = True
    #solver3D.vel_toll = 1e-3
    #solver3D.press_toll = 1e-2
    #solver3D.dynamic_tau = 0.01
    #solver3D.compute_reactions = False
    #solver3D.activate_smagorinsky(0.1)
    ##solver3D.pressure_linear_solver = SuperLUSolver()
    #solver3D.Initialize()
    #import CouplingTools1D_3Dv5
    #transfer_obj = CouplingTools1D_3Dv5.TransferTools(model_part1D, model_part3D)
    #transfer_obj.Setting3d(P_initial, blood_density, blood_viscosity)
...
if (InletPressure):
    ...
    ToWriteIn_Summary += "Diastolic pressure: " + \
...
if (step_size_control == True):
    ...
    ToWriteIn_Summary += "Using Adaptative Time Step: " + \
        str(Dt) + " (aprox) Seconds" + "\n"
else:
    Dt = step_size
    ...
    while(total_time < final_time):
        ...
        if ((Coupled_Simulation) and (control_sub_step == sub_step) and (cycle_to_couple == True)):
            if(step >= 3):
                ...
                print(
                    "----------------------------Transfer 1d(tn) to 3d(tn+1)------------------")
                transfer_obj.Transfer1D_to_3D(diastolic_pressure, summary_file_pressure)
            ...
            if (plot1d):
                if(ascii):
                    ...
                    for node in model_part1D.Nodes:
                        ...
                        f1d.write(ToWrite)
                    print(
                        "Writing CVPR results 1D Results(time)------------------------>", total_time)
                else:
                    ...
                    print(
                        "Writing GiD results 1D Results(time)------------------------>", total_time)
            ...
            if((plot3d) and (cycle_to_couple == True)):
                if(ascii):
                    ...
                    for node in model_part3D.Nodes:
                        ...
                        f3d.write(ToWrite)
                    print(
                        "Writing CVPR results 3D Results(time)------------------------>", total_time)
                else:
                    ...
                    gid_io_3d.WriteNodalResults(
                        DISTANCE, model_part3D.Nodes, total_time, 0)
                    out = 0
                    print(
                        "Writing GiD results 3D results(step)------------------------>", total_time)
            ...
            if ((Coupled_Simulation) and (cycle_to_couple == False)):
                if (nro_cardiac_cycle == cardiac_cycle_to_3D):
                    ...
                    transfer_obj.Setting3d(
                        P_initial, blood_density, blood_viscosity)
            control_sub_step = 0
            out = 0
            print(
                "Cardiac_cycle: ------------------------>", nro_cardiac_cycle)
            ToWriteIn_Summary = "Cardiac_cycle: ------------------------> " + \
                str(nro_cardiac_cycle) + "\n"
            summary_file.write(ToWriteIn_Summary)
