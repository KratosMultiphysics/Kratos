...
class TransferTools:
	...
	for i in range(0,len(self.outlets_3d)):  
	  ...
	  beta = ((outlet_nodes_1d[0].GetValue(YOUNG_MODULUS) * outlet_nodes_1d[0].GetValue(THICKNESS) * math.sqrt(math.pi))/(1-(outlet_nodes_1d[0].GetValue(POISSON_RATIO)*outlet_nodes_1d[0].GetValue(POISSON_RATIO))))
	  ...
	  press = initial_pressure+beta*(math.sqrt(A)-math.sqrt(A0))/A0  #math.sqrt(A/A0)*beta - beta 
