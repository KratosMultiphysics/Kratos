...
public:
    ...
    Explicit_Solver(int solver_id, double radius, double tol, ModelPart& model_part)
    {
        ...
        for (PointPointerIterator inode = model_part.NodesBegin(); inode != model_part.NodesEnd(); inode++)
        {
            ...
            if (mMinRadius > particle_pointer->GetRadius())
            {
                mMinRadius = particle_pointer->GetRadius();
            }
            if (mMaxRadius < particle_pointer->GetRadius())
            {
                mMaxRadius = particle_pointer->GetRadius();
            }
            if (mMinMass > particle_pointer->GetMass())
            {
                mMinMass = particle_pointer->GetMass();
            }
