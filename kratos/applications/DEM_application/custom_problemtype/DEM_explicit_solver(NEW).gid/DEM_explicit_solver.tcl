...
proc DEM::GetDimensions { } {    
    ...
    if { [expr {abs($zmin)}] >$epsilon || [expr {abs($zmax)}] >$epsilon } {        
	set dimensions 3
    } else {
	set dimensions 2
    }    
...
proc DEM::GetElementCenter { element_id } {
...
proc  DEM::Elements_Substitution { } {    
    ...
    #SAVING ALL THE CONDITIONS APPLIED TO LINE, SURFACE AND VOLUME
    
    set radius_conditions {line_RADIUS surface_RADIUS volume_RADIUS}
    foreach over {ovline ovsurf ovvol} {
	foreach condition_it [GiD_Info conditions $over] {
	    if { [lsearch $radius_conditions $condition_it]==-1 } {
		set data [GiD_Info conditions $condition_it mesh]             
		foreach item $data {
		    if { [lindex $item 0]  == "E" } {
		        set element_id [lindex $item 1] 
		        set condition_values [lrange $item 3 end]
		        lappend applied_conditions_by_element($element_id) [list $condition_it $condition_values]
		    }
		}
	    }
	}
    ...
    #SAVING ALL THE CONDITIONS APPLIED TO POINTS
    foreach condition_it [GiD_Info conditions ovpnt] {
	...
	foreach item $data {
	    set element_id [lindex $item 1] 
	    set element_radius [lindex $item 4]
	    set element_type [lindex [GiD_Mesh get element $element_id] 1]
	    set element_material [lindex [GiD_Info Mesh elements $element_type $element_id] end] 
	    if { $element_material == 0} {
		if { !$fail } {
		    WarnWinText [= "Error: element %s without material" $element_id]
		}
		set fail 1                
	    } elseif  { [string is integer $element_material] &&  $element_material > 0 } {
		set element_material_name [lindex $materials $element_material-1]
	    } else { 
		WarnWinText [= "Wrong material number %s" $element_material]
	    }
	    set node_id [GiD_Mesh create node append [DEM::GetElementCenter $element_id]]
	    if  { $dimensions==3 } {
		if  {$element_material} {
		    set new_element_id [GiD_Mesh create element append sphere 1 $node_id $element_radius $element_material_name]
		} else {
		    set new_element_id [GiD_Mesh create element append sphere 1 $node_id $element_radius]
		}
		...
		if  {$element_material} {
		    set new_element_id [GiD_Mesh create element append circle 1 $node_id $element_radius 0 0 1 $element_material_name]
		} else  {
		...
		WarnWinText [= "Please, define 2D or 3D in Problem Parameters and mesh again"]
	    }                                
	}
    }
	...
	set point_radius [lindex $item 4]        
	set point_id [lindex $item end]
	set text [GiD_Info list_entities Point $point_id]
	set point_material 0
	regexp {material: ([0-9]+)} $text dummy point_material
	if { $point_material == 0} {
	    if { !$fail } {
		WarnWinText [= "Error: node %s without material" $node_id]
	    }
	    set fail 1                
	} elseif  { [string is integer $point_material] &&  $point_material > 0 } {
	    set point_material_name [lindex $materials $point_material-1]            
	} else { 
	    WarnWinText [= "Wrong material number %s" $point_material]
	}         
	if  { $dimensions==3 } {
	    if  {$point_material} {
		set new_element_id [GiD_Mesh create element append sphere 1 $node_id $point_radius $point_material_name]
	    } else {
		set new_element_id [GiD_Mesh create element append sphere 1 $node_id $point_radius]
	    }
		...
		foreach point_data $applied_conditions_by_point($node_id) {
		    ...
		    GiD_AssignData condition volume_VELOCITY nodes $values $new_element_id; #POSSIBLE ERROR nodes realment?
		}
	    }
	} elseif { $dimensions==2 } {
	    if  {$point_material} {
		set new_element_id [GiD_Mesh create element append circle 1 $node_id $point_radius 0 0 1 $point_material_name]
	    } else {
		set new_element_id [GiD_Mesh create element append circle 1 $node_id $point_radius 0 0 1]
	    }
	    if {  [info exists applied_conditions_by_point($node_id)] } {
		foreach point_data $applied_conditions_by_point($node_id) {
		    set values [lindex $point_data 1]
		    GiD_AssignData condition surface_VELOCITY nodes $values $new_element_id; #POSSIBLE ERROR nodes realment?
		}
	    }        
	} else { 
	    WarnWinText [= "Please, define 2D or 3D in Problem Parameters"]
	}
    }
    return $fail
}  
...
proc InitGIDProject {dir} {
	...
	GiDMenu::InsertOption "DEM explicit solver" [list "Model Parts"] 1 PRE "GidOpenConditions \"Model Parts\"" "" ""
	GiDMenu::InsertOption "DEM explicit solver" [list "Problem Parameters"] 2 PRE "GidOpenProblemData \"Problem Parameters\"" "" ""
...
proc BeforeMeshGeneration {elementsize} {
	...
	foreach surface $surfacelist {
		if {[lsearch ${surfaceFixedVelocitylist} $surface] != -1} then {
	...
	foreach line $linelist {
		if {[lsearch ${lineFixedVelocitylist} $line] != -1} then {
	...
	foreach point $pointlist {
		if {[lsearch ${pointFixedVelocitylist} $point] != -1} then {
...
proc DemElems_Addtolist { iterator elemsconec } { 
    global DemElems_list
    set DemElems_list($iterator) $elemsconec
    return
}

proc DemElems_Getoflist { iterator } {
    global DemElems_list
