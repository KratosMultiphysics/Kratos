...
void SphericRotatingParticle::ComputeForcesOnCenterNode (double dt, array_1d<double, 3 >& gravity){
    ...
    for (ParticleWeakIteratorType ineighbour = SphericRotatingParticle::mContactingNeighbours.begin();
            ineighbour != SphericRotatingParticle::mContactingNeighbours.end(); ineighbour++){
        array_1d<double, 3 > tang_displacement = *(idisplacement.base());
        array_1d<double, 3 > other_ang_displ = ineighbour->GetPosition() - ineighbour->GetOldPosition();
        double other_mass = ineighbour->GetMass();
        double other_radius = ineighbour->GetRadius();
        double other_stiffness = ineighbour->GetNormalStiffness() / other_radius;
        ...
        double normal_force_norm = equiv_stiffness * indentation;//Minus the viscous part (Elastic case)
...
//#endif
        ...
        max_tang_forces.push_back(max_tang_force_norm); //Needed for SphericRotatingParticle::ComputeNewTangentialDisplacements()
...
} // SphericRotatingParticle::ComputeForcesOnCenterNode

void SphericRotatingParticle::ComputeNewTangentialDisplacements(){
    int n_neigbours = SphericRotatingParticle::GetNumberOfNeighbours();
    int n_contacts = SphericRotatingParticle::GetNumberOfContactingNeighbours();
    if (n_neigbours > 0 || (n_neigbours < 1 && n_contacts > 0)){
        ...
        ForceNormsIteratorType inormalf = SphericRotatingParticle::mMaxTangForces.begin();
        BoolSlidingContactsIteratorType islide = SphericRotatingParticle::mSlidingFlagsOfNeighbours.begin();
        TangDisplacementsIteratorType idisplacement = SphericRotatingParticle::mTangentialDisplacementsOfNeighbours.begin();
        for (ParticleWeakIteratorType ineighbour = SphericRotatingParticle::mContactingNeighbours.begin();
        ineighbour != SphericRotatingParticle::mContactingNeighbours.end(); ineighbour++){
            //array_1d<double, 3 > tang_displacement = *(idisplacement.base());
            double other_radius = ineighbour->GetRadius();
...
} // SphericRotatingParticle::ComputeNewTangentialDisplacements

void SphericRotatingParticle::UpdateContactsList(){
    int n_neigbours = SphericRotatingParticle::GetNumberOfNeighbours();
    int n_contacts = SphericRotatingParticle::GetNumberOfContactingNeighbours();
    if (n_neigbours > 0 || (n_neigbours < 1 && n_contacts > 0)){
        array_1d<double, 3 > init_tang_displ = ZeroVector(3);
        double radius = mRadius;
        ParticleWeakVectorType& contact_neighbours = SphericRotatingParticle::mContactingNeighbours;
        ParticleWeakVectorType old_contact_neighbours;
        TangDisplacementsVectorType& tangential_displacements = SphericRotatingParticle::mTangentialDisplacementsOfNeighbours;
        TangDisplacementsVectorType old_tangential_displacements;
        old_contact_neighbours.reserve(SphericRotatingParticle::mNeighbours.size());
        old_tangential_displacements.reserve(SphericRotatingParticle::mNeighbours.size());
...
}//SphericRotatingParticle::UpdateContactsList
