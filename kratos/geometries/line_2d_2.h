    ...
    bool HasIntersection(const BaseType& rOtherGeometry) override
    {
        ...
        if (std::abs(denominator) < tolerance) 
        {
            ...
            if (std::abs(lhs - rhs) < tolerance)
            {
                ...
                {
                    if (std::abs(inner_prod(AB, AC)) <= tolerance)
                    {
                        // p3 coincides with p1. Check if the lines are in the same direction
                    ...
                    else if (std::abs(inner_prod(AB, AC) - inner_prod(AB, AB)) < tolerance)
                    {
                        // p3 coincides with p2. Check if the lines are in the same direction
                ...
                {
                    if (std::abs(inner_prod(AB, AD)) <= tolerance)
                    {
                        // p4 coincides with Point 1. Check if the lines are in the same direction
                    ...
                    else if (std::abs(inner_prod(AB, AD) - inner_prod(AB, AB)) < tolerance)
                    {
                        // p4 coincides with p2. Check if the lines are in the same direction
