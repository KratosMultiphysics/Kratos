...
//  }
    ...
    */
    ...
    /** This method gives second order derivatives of all shape
     * functions evaluated in integration points.
        ...
        IntegrationMethod ThisMethod ) const
    {
        ...
        if ( rResult.size() != integration_points_number ){
            DenseVector<DenseVector<Matrix>> temp(integration_points_number);
            rResult.swap(temp);
        }

        //calculating the local gradients
        ShapeFunctionsGradientsType DN_DX;
        this->ShapeFunctionsIntegrationPointsGradients( DN_DX, ThisMethod );
        ...
        for ( unsigned int pnt = 0; pnt < integration_points_number; ++pnt ) {
            ...
            if (aux.size() != this->PointsNumber())
            {
                DenseVector<Matrix> temp(this->PointsNumber());
                aux.swap( temp );
                rResult[pnt].swap(temp);
            }

            const GeometryType::CoordinatesArrayType& local_point_coordinates = this->IntegrationPoints( ThisMethod )[pnt];

            ShapeFunctionsSecondDerivativesType DDN_DDe;
            this->ShapeFunctionsSecondDerivatives(DDN_DDe, local_point_coordinates);
            ...
            else if (this->WorkingSpaceDimension() == 3){
                A.resize(6,6,false);
            ...
            DenseVector<Matrix> H(this->WorkingSpaceDimension());
            for (unsigned int d = 0; d < this->WorkingSpaceDimension(); ++d)
                H[d] = ZeroMatrix(this->LocalSpaceDimension(),this->LocalSpaceDimension());

            for (IndexType p = 0; p < this->PointsNumber(); ++p) {
                const array_1d<double, 3>& r_coordinates = (*this)[p].Coordinates();
                for (unsigned int d=0 ; d < this->WorkingSpaceDimension(); ++d){
                    for (unsigned int e=0 ; e < this->WorkingSpaceDimension(); ++e){
                        for (unsigned int f=0 ; f < this->WorkingSpaceDimension(); ++f){
            ...
            for (IndexType p = 0; p < this->PointsNumber(); ++p) {
                ...
                if (this->WorkingSpaceDimension() == 2){
                    rhs.resize(3);
                    result.resize(3);
                    rhs[0] = DDN_DDe[p](0,0) - DN_DX[pnt](p,0) * H[0](0,0) - DN_DX[pnt](p,1) * H[1](0,0);
                    rhs[1] = DDN_DDe[p](1,1) - DN_DX[pnt](p,0) * H[0](1,1) - DN_DX[pnt](p,1) * H[1](1,1);
                    rhs[2] = DDN_DDe[p](0,1) - DN_DX[pnt](p,0) * H[0](0,1) - DN_DX[pnt](p,1) * H[1](0,1);
                }
                else if (this->WorkingSpaceDimension() == 3){
                    rhs.resize(6);
                    result.resize(6);
                    rhs[0] = DDN_DDe[p](0,0) - DN_DX[pnt](p,0) * H[0](0,0) - DN_DX[pnt](p,1) * H[1](0,0) - DN_DX[pnt](p,2) * H[2](0,0);
                    rhs[1] = DDN_DDe[p](1,1) - DN_DX[pnt](p,0) * H[0](1,1) - DN_DX[pnt](p,1) * H[1](1,1) - DN_DX[pnt](p,2) * H[2](0,0);
                    rhs[2] = DDN_DDe[p](2,2) - DN_DX[pnt](p,0) * H[0](2,2) - DN_DX[pnt](p,1) * H[1](2,2) - DN_DX[pnt](p,2) * H[2](2,2);
                    rhs[3] = DDN_DDe[p](0,1) - DN_DX[pnt](p,0) * H[0](0,1) - DN_DX[pnt](p,1) * H[1](0,1) - DN_DX[pnt](p,2) * H[2](0,1);
                    rhs[4] = DDN_DDe[p](1,2) - DN_DX[pnt](p,0) * H[0](1,2) - DN_DX[pnt](p,1) * H[1](1,2) - DN_DX[pnt](p,2) * H[2](1,2);
                    rhs[5] = DDN_DDe[p](0,2) - DN_DX[pnt](p,0) * H[0](0,2) - DN_DX[pnt](p,1) * H[1](0,2) - DN_DX[pnt](p,2) * H[2](0,2);
                }

                aux[p].resize(this->WorkingSpaceDimension(), this->WorkingSpaceDimension(), false );
