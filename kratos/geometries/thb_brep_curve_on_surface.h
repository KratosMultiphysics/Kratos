...
#if !defined(KRATOS_THB_BREP_CURVE_ON_SURFACE_3D_H_INCLUDED )
#define  KRATOS_THB_BREP_CURVE_ON_SURFACE_3D_H_INCLUDED
...
/**
 * @class THBBrepCurveOnSurface
 * @ingroup KratosCore
 * @brief The THBBrepCurveOnSurface acts as topology for curves on surfaces.
 */
...
public:
    ...
    /** Pointer definition of THBBrepCurveOnSurface */
    KRATOS_CLASS_POINTER_DEFINITION( THBBrepCurveOnSurface );
    ...
    typedef THBCurveOnSurfaceGeometry<3, TContainerPointEmbeddedType, TContainerPointType> THBCurveOnSurfaceType;

    typedef typename THBCurveOnSurfaceType::Pointer THBCurveOnSurfacePointerType;
    ...
    THBBrepCurveOnSurface(
        THBCurveOnSurfacePointerType pNurbsCurveOnSurface,
        bool SameCurveDirection = true)
        : BaseType(PointsArrayType(), &msGeometryData)
    ...
    explicit THBBrepCurveOnSurface(const PointsArrayType& ThisPoints)
        : BaseType(ThisPoints, &msGeometryData)
    {
    }

    /// Copy constructor.
    THBBrepCurveOnSurface( THBBrepCurveOnSurface const& rOther )
        : BaseType( rOther )
    ...
    explicit THBBrepCurveOnSurface(
        THBBrepCurveOnSurface<TOtherContainerPointType, TOtherContainerPointEmbeddedType> const& rOther )
        : BaseType( rOther )
        , mpCurveOnSurface(rOther.mpCurveOnSurface)
        , mCurveNurbsInterval(rOther.mCurveNurbsInterval)
        , mSameCurveDirection(rOther.mSameCurveDirection)
    ...
    /// Destructor
    ~THBBrepCurveOnSurface() override = default;
    ...
    /// Assignment operator.
    THBBrepCurveOnSurface& operator=( const THBBrepCurveOnSurface& rOther )
    {
    ...
    template<class TOtherContainerPointType, class TOtherContainerPointEmbeddedType>
    THBBrepCurveOnSurface& operator=( THBBrepCurveOnSurface<TOtherContainerPointType, TOtherContainerPointEmbeddedType> const & rOther )
    {
        BaseType::operator=( rOther );
        mpCurveOnSurface = rOther.mpCurveOnSurface;
        mCurveNurbsInterval = rOther.mCurveNurbsInterval;
    ...
    {
        return typename BaseType::Pointer( new THBBrepCurveOnSurface( ThisPoints ) );
    }
    ...
    */
    const GeometryPointer pGetGeometryPart(const IndexType Index) const override
    {
        ...
        KRATOS_ERROR << "Index " << Index << " not existing in THBBrepCurveOnSurface: "
            << this->Id() << std::endl;
    }
    ...
    */
    ...
    THBCurveOnSurfacePointerType pGetCurveOnSurface()
    {
        return mpCurveOnSurface;
    }

    /// Returns the const NurbsCurveOnSurface::Pointer of this brep.
    const THBCurveOnSurfacePointerType pGetCurveOnSurface() const
    {
        return mpCurveOnSurface;
    }
    ...
    */
    ...
    {
        KRATOS_ERROR << "IsInside is not yet implemented within the THBBrepCurveOnSurface";
    }
...
private:
    ...
    THBCurveOnSurfacePointerType mpCurveOnSurface;
...
template<class TContainerPointType, class TContainerPointEmbeddedType = TContainerPointType> inline std::istream& operator >> (
    ...
    THBBrepCurveOnSurface<TContainerPointType, TContainerPointEmbeddedType>& rThis );
...
template<class TContainerPointType, class TContainerPointEmbeddedType = TContainerPointType> inline std::ostream& operator << (
    ...
    const THBBrepCurveOnSurface<TContainerPointType, TContainerPointEmbeddedType>& rThis )
{
...
GeometryData THBBrepCurveOnSurface<TContainerPointType, TContainerPointEmbeddedType>::msGeometryData(
    &msGeometryDimension,
    GeometryData::IntegrationMethod::GI_GAUSS_1,
    {}, {}, {});

template<class TContainerPointType, class TContainerPointEmbeddedType>
const GeometryDimension THBBrepCurveOnSurface<TContainerPointType, TContainerPointEmbeddedType>::msGeometryDimension(3, 1);

///@}
}// namespace Kratos.

#endif // KRATOS_THB_BREP_CURVE_ON_SURFACE_3D_H_INCLUDED  defined
