...
namespace Kratos
{
     ...
     * @class Line2D5
     * @ingroup KratosCore
     * @brief An five node 2D line geometry with quartic shape functions
    ...
    public:
        ...
        */
         ...
         */
        Line2D5(Line2D5 const& rOther) : BaseType(rOther)
        {
        }
         ...
         * source geometry's points too.
         */
        template<class TOtherPointType> explicit Line2D5(Line2D5<TOtherPointType> const& rOther)
            : BaseType(rOther)
        {
        }
        ...
        {
            auto p_geometry = typename BaseType::Pointer(new Line2D5(NewGeometryId, rGeometry.Points()));
            ...
            const CoordinatesArrayType& rPoint) const override
        {
            ...
            for (IndexType k = 0; k < MaxIteratioNumberPointLocalCoordinates; ++k) {
                ...
                if (delta_xi > MaxNormPointLocalCoordinates) {
                    KRATOS_WARNING_IF("Line2D5", k > 0) << "detJ =\t" << J(0, 0) << " DeltaX =\t" << delta_xi << " stopping calculation. Iteration:\t" << k << std::endl;
                    break;
                }
    ...
    const GeometryData Line2D5<TPointType>::msGeometryData(
        ...
        Line2D5<TPointType>::AllIntegrationPoints(),
