...
#if !defined(KRATOS_NURBS_3D_3_H_INCLUDED ) //modified by Matthias
#define  KRATOS_NURBS_3D_3_H_INCLUDED  //modified by Matthias
...
template<class TPointType> class NurbsPatchGeometry3D  //modified by Matthias
...
public:
     ...
     */
    KRATOS_CLASS_POINTER_DEFINITION( NurbsPatchGeometry3D ); //modified by Matthias
    ...
    */
    ...
    NurbsPatchGeometry3D( const PointerVector< Node<3> > ControlPoints,
     ...
     */
    NurbsPatchGeometry3D( NurbsPatchGeometry3D const& rOther )
        : BaseType( rOther ),
          ...
          mNumberOfCPsV(rOther.mNumberOfCPsV)
 {
     ...
     * source geometry's points too.
     */
    template<class TOtherPointType> NurbsPatchGeometry3D( NurbsPatchGeometry3D<TOtherPointType> const& rOther )
        : BaseType( rOther )
    {
    }
    ...
    NurbsPatchGeometry3D& operator=( const NurbsPatchGeometry3D& rOther )
    ...
    NurbsPatchGeometry3D& operator=( NurbsPatchGeometry3D<TOtherPointType> const & rOther )
    {
        BaseType::operator=( rOther );
        return *this;
    }
    ...
    /**
     * PointLocalCoordinates is not yet implemented for NURBS!
     * @param rResult
     * @param rPoint
     * @return
     */
    ...
    * @see InverseOfJacobian
    ...
    virtual Vector& DeterminantOfJacobian( Vector& rResult,
                                           IntegrationMethod ThisMethod ) const
    {
        KRATOS_ERROR<< "NurbsPatchGeometry3D::DeterminantOfJacobian"<< "Jacobian is not square" << std::endl;
        return rResult;
    }
    ...
    virtual JacobiansType& InverseOfJacobian( JacobiansType& rResult,
            IntegrationMethod ThisMethod ) const
    {
        KRATOS_ERROR << "NurbsPatchGeometry3D::InverseOfJacobian" << "Jacobian is not square" << std::endl;
        return rResult;
    }
    ...
    */
    ...
    {
        return "3 dimensional NURBS-surface";
    }
    ...
    {
        rOStream << "3 dimensional NURBS-surface";
    }
    ...
    Vector& ShapeFunctionsValues(Vector &rResult,const CoordinatesArrayType& rCoordinates) const
    {
    ...
    */
    ...
    {
            KRATOS_ERROR << "NurbsPatchGeometry3D::ShapeFunctionsIntegrationPointsGradients" << "Jacobian is not square"  << std::endl;
        return rResult;
    }
     ...
     */
        ...
        Matrix& ShapeFunctionsLocalGradients(   Matrix& rResult, const CoordinatesArrayType& rPoint) const
    {
        rResult.resize(mPolynomialDegreeP+1,mPolynomialDegreeQ+1);
        rResult = ElementGeometryDerivatives(rPoint[0],rPoint[1]);
        return rResult;
     ...
     * one new geometry of the type NurbsPatchGeometry3D. Each geometry will contain
    ...
    void DefineGeometries(std::vector< Geometry< Node<3> >::Pointer>& GeometryContainer)
    {
        ...
        for (unsigned int i = 1; i <= mKnotsXi.size() - 2*mPolynomialDegreeP; i++)
        {
        ...
        for (unsigned int i = 1; i <= mKnotsEta.size() - 2*mPolynomialDegreeQ; i++)
        {
            ...
            if (mKnotsXi[mPolynomialDegreeP+i] != mKnotsXi[mPolynomialDegreeP+i-1])
            {
                for (int j = 1; j <= mKnotsEta.size() - 2*mPolynomialDegreeQ; j++)
                {
                    if (mKnotsEta[mPolynomialDegreeQ+j] != mKnotsEta[mPolynomialDegreeQ+j-1])
                    {
                        ...
                        GeometryContainer[counter] = Geometry< Node<3> >::Pointer( new NurbsPatchGeometry3D(ControlPointsElement,
                                                                                                            WeightsElement,
                                                                                                            VectorKnotsXi,
                                                                                                            VectorKnotsEta,
                                                                                                            mPolynomialDegreeP,
                                                 ...
                                                 CoordinatesArrayType &GlobalCoordinates)
    {
        ...
        for (int i=0; i<mPolynomialDegreeP+1; i++)
        {
            for (int j=0; j<mPolynomialDegreeQ+1; j++)
            {
                ...
                z += NurbsBasisFunction(i,j)* this->Points()[i+mNumberOfCPsU*j].Z();
            }
        }
        GlobalCoordinates[0] = x;
        GlobalCoordinates[1] = y;
        GlobalCoordinates[2] = z;
    ...
    void BaseVectors(double Xi, double Eta, Vector &gXi, Vector &gEta) const
    {
             ...
             i++)
        {
            for(int j=0;
                ...
                j++)
            {
                gXi[0] += XiDerivatives(i,j)*this->Points()[i+j*(mPolynomialDegreeP+1)].X();
                gXi[1] += XiDerivatives(i,j)*this->Points()[i+j*(mPolynomialDegreeP+1)].Y();
                gXi[2] += XiDerivatives(i,j)*this->Points()[i+j*(mPolynomialDegreeP+1)].Z();
                ...
                gEta[2] += EtaDerivatives(i,j)*this->Points()[i+j*(mPolynomialDegreeP+1)].Z();
            ...
            const CoordinatesArrayType& rPoint )
    {
        ...
        //starting with xi and eta at the middle of the element
        rResult = ZeroVector( 3 );
        rResult[0] = (mUpperXi-mLowerXi)/2+mLowerXi;
        rResult[1] = (mUpperEta-mLowerEta)/2+mLowerEta;
        ...
        for ( int k = 0; k < maxiter; k++ )
        {
            ...
            rResult[0] += damper*DeltaXi[0];
            rResult[1] += damper*DeltaXi[1];
            rResult[2] = 0.0;
            ...
            if ( norm_2( DeltaXi ) > 300 )
            {
                ...
                std::cout << "Problem detected at Node:" <<std::endl;
                KRATOS_WATCH(rPoint);
                std::cout << std::endl;
                break;
...
//       KRATOS_WATCH(rResult);
        ...
        if(rResult[0] < mLowerXi || rResult[0] > mUpperXi ||rResult[1] < mLowerEta ||rResult[1] > mUpperEta )
        {
            rResult[2] = 1;
          if(rResult[0] < mLowerXi) rResult[0] = mLowerXi;
          if(rResult[0] > mUpperXi) rResult[0] = mUpperXi;
          if(rResult[1] < mLowerEta) rResult[1] = mLowerEta;
          if(rResult[1] > mUpperEta) rResult[1] = mUpperEta;
        }
...
protected:
    /**
     * There are no protected members in class NurbsPatchGeometry3D3
     */
...
private:
    ...
    /**
     * @brief PrintGeometryBoundaries: Function which prints the geometry boundaries in
     * global coordinates.
     */

    void PrintGeometryBoundaries()
    {
        ...
        GlobalCoordinates = ZeroVector(3);
            ...
            ShapeFunctionsValues(ShapeFunctions, mLowerXi, mLowerEta);
                 ...
                 j++)
            {
                ...
                GlobalCoordinates[0] += this->Points()[j].X() * ShapeFunctions[j];
                GlobalCoordinates[1] += this->Points()[j].Y() * ShapeFunctions[j];
                GlobalCoordinates[2] += this->Points()[j].Z() * ShapeFunctions[j];
            }

            std::cout<<"low, low :";
            KRATOS_WATCH(GlobalCoordinates);
            GlobalCoordinates = ZeroVector(3);


            ShapeFunctionsValues(ShapeFunctions, mLowerXi, mUpperEta);
            ...
            ShapeFunctionsValues(ShapeFunctions, mUpperXi, mLowerEta);
            ...
            ShapeFunctionsValues(ShapeFunctions, mUpperXi, mUpperEta);
    ...
    /**
     * ElementGeometryDerivatives calculates the local derivatives of x,y and z
     ...
     * The returned matrix F_i_j will have the dimension 3 x 2.
                                      ...
                                      Vector &NurbsFunctionsValues)const
    {
        ...
        Matrix Jacobian3D(2,3);
        Jacobian3D = ZeroMatrix(2,3);
        ...
        double invSumDenominatorXiEta(1/(SumDenominatorXiEta*SumDenominatorXiEta));
            ...
            for (int i=0; i<mPolynomialDegreeP+1; i++)
            {
                ...
                Jacobian3D(0,0) += NurbsBasisFunctionDerivativesXi(i,j)* this->Points()[i+(mPolynomialDegreeP+1)*j].X();
                Jacobian3D(0,1) += NurbsBasisFunctionDerivativesXi(i,j)* this->Points()[i+(mPolynomialDegreeP+1)*j].Y();
                Jacobian3D(0,2) += NurbsBasisFunctionDerivativesXi(i,j)* this->Points()[i+(mPolynomialDegreeP+1)*j].Z();
                Jacobian3D(1,0) += NurbsBasisFunctionDerivativesEta(i,j)* this->Points()[i+(mPolynomialDegreeP+1)*j].X();
                Jacobian3D(1,1) += NurbsBasisFunctionDerivativesEta(i,j)* this->Points()[i+(mPolynomialDegreeP+1)*j].Y();
                Jacobian3D(1,2) += NurbsBasisFunctionDerivativesEta(i,j)* this->Points()[i+(mPolynomialDegreeP+1)*j].Z();
            }
            }
                                      ...
                                      Matrix &NurbsBasisFunctionDerivativesEta)const
    {
        ...
        Jacobian3D = ElementGeometryDerivatives(Xi,Eta,NurbsBasisFunctionDerivativesXi,NurbsBasisFunctionDerivativesEta,NurbsFunctionsValues);
    ...
    Matrix ElementGeometryDerivatives(double Xi,
                                      double Eta)const
    {
        Matrix NurbsBasisFunctionDerivativesXi, NurbsBasisFunctionDerivativesEta, Jacobian3D;
        Vector NurbsFunctionsValues;
        Jacobian3D = ElementGeometryDerivatives(Xi,Eta,NurbsBasisFunctionDerivativesXi,NurbsBasisFunctionDerivativesEta,NurbsFunctionsValues);
        return Jacobian3D;
    }
    ...
    *****************************************************************************/
    ...
    template<class TOtherPointType> friend class NurbsPatchGeometry3D;
...
GeometryData NurbsPatchGeometry3D<TPointType>::msGeometryData(
    2, 3, 2,
    GeometryData::GI_GAUSS_1,
    NurbsPatchGeometry3D<TPointType>::AllIntegrationPoints(),
    NurbsPatchGeometry3D<TPointType>::AllShapeFunctionsValues(),
    AllShapeFunctionsLocalGradients()
);
