...
public:
    ...
    {
        return 0.0;//ShapeFunctionValue(DerivativeRow, ControlPointIndex);
    }
    ...
    {
        return 0.0;// ShapeFunctionValue(DerivativeRow, ControlPointIndexU, ControlPointIndexV);
    }
    ...
    const double ShapeFunctionDerivativeValue(
        ...
        const double ParameterV)
    {
        ...
        std::fill(mShapeFunctionValues.begin(), mShapeFunctionValues.begin() + number_of_values, 0);
        ...
        const double ParameterV)
    {
        ...
        for (int shape_row_index = 0; shape_row_index < NumberOfShapeFunctionRows(); shape_row_index++) {
            ...
            for (int i = 0; i < NumberOfNonzeroControlPointsU(); i++) {
                for (int j = 0; j < NumberOfNonzeroControlPointsV(); j++) {
                    const IndexType ControlPointIndexU = GetFirstNonzeroControlPointU() + i;
                    const IndexType ControlPointIndexV = GetFirstNonzeroControlPointV() + j;

                    const double weight = Weights(GetControlPointIndex(ControlPointIndexU, ControlPointIndexV));
                    ShapeFunctionValue(shape_row_index, i, j) *= weight;
                    GetWeightedSum(shape_row_index) += ShapeFunctionValue(shape_row_index, i, j);
                }
            }
        }
...
private:
    ...
    inline int GetIndex(
        ...
        const
    {
        const IndexType control_point_index = GetControlPointIndex(
            ControlPointIndexU, ControlPointIndexV);
        const IndexType index = NurbsUtilities::GetVectorIndexFromMatrixIndices(
            NumberOfShapeFunctionRows(), NumberOfNonzeroControlPoints(),
    ...
    double& ShapeFunctionValue(const IndexType DerivativeRow, const IndexType ControlPointIndex)
    {
        const IndexType index = NurbsUtilities::GetVectorIndexFromMatrixIndices(NumberOfShapeFunctionRows(),
            NumberOfNonzeroControlPoints(), DerivativeRow, ControlPointIndex);
