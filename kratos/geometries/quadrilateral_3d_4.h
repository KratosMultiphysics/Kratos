    ...
    * @see InverseOfJacobian
                                           ...
                                           IntegrationMethod ThisMethod ) const override
    {
        ...
        for ( unsigned int pnt = 0; pnt < integration_points_number; pnt++ )
        {
            ...
            const double detJ = jacobian(0,1)*jacobian(0,1)*(jacobian(1,0)*jacobian(1,0)+jacobian(2,0)*jacobian(2,0))+(jacobian(1,1)*jacobian(2,0)-jacobian(1,0)*jacobian(2,1))*(jacobian(1,1)*jacobian(2,0)-jacobian(1,0)*jacobian(2,1))-2*jacobian(0,0)*jacobian(0,1)*(jacobian(1,0)*jacobian(1,1)+jacobian(2,0)*jacobian(2,1))+jacobian(0,0)*jacobian(0,0)*(jacobian(1,1)*jacobian(1,1)+jacobian(2,1)*jacobian(2,1));
