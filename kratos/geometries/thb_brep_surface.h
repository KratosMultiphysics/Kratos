...
#if !defined(KRATOS_THB_BREP_FACE_3D_H_INCLUDED )
#define  KRATOS_THB_BREP_FACE_3D_H_INCLUDED
...
/**
 * @class THBBrepSurface
 * @ingroup KratosCore
 * @brief The THBBrepSurface acts as topology for faces. Those
...
public:
    ...
    /** Pointer definition of THBBrepSurface */
    KRATOS_CLASS_POINTER_DEFINITION( THBBrepSurface );
    ...
    explicit THBBrepSurface(const PointsArrayType& ThisPoints)
        : BaseType(ThisPoints, &msGeometryData)
    {
    }

    /// Copy constructor.
    THBBrepSurface( THBBrepSurface const& rOther )
        : BaseType( rOther )
    ...
    explicit THBBrepSurface(
        THBBrepSurface<TOtherContainerPointType, TOtherContainerPointEmbeddedType> const& rOther )
        : BaseType( rOther )
        , mpNurbsSurface(rOther.mpNurbsSurface)
        , mOuterLoopArray(rOther.mOuterLoopArray)
        , mInnerLoopArray(rOther.mInnerLoopArray)
    ...
    /// Destructor
    ~THBBrepSurface() override = default;
    ...
    /// Assignment operator.
    THBBrepSurface& operator=( const THBBrepSurface& rOther )
    {
    ...
    template<class TOtherContainerPointType, class TOtherContainerPointEmbeddedType>
    THBBrepSurface& operator=( THBBrepSurface<TOtherContainerPointType, TOtherContainerPointEmbeddedType> const & rOther )
    {
        BaseType::operator=( rOther );
        mpNurbsSurface = rOther.mpNurbsSurface;
        mOuterLoopArray = rOther.mOuterLoopArray;
    ...
    {
        return typename BaseType::Pointer( new THBBrepSurface( ThisPoints ) );
    }
    ...
    */
    const GeometryPointer pGetGeometryPart(const IndexType Index) const override
    {
        ...
        KRATOS_ERROR << "Index " << Index << " not existing in THBBrepSurface: "
            << this->Id() << std::endl;
    }

    /**
    * @brief This function is used to check if this THBBrepSurface
    ...
    */
    ...
    /*
    * @brief checks if the THBBrepSurface has any boundary trim information.
...
template<class TContainerPointType, class TContainerPointEmbeddedType = TContainerPointType> inline std::istream& operator >> (
    ...
    THBBrepSurface<TContainerPointType, TContainerPointEmbeddedType>& rThis );
...
template<class TContainerPointType, class TContainerPointEmbeddedType = TContainerPointType> inline std::ostream& operator << (
    ...
    const THBBrepSurface<TContainerPointType, TContainerPointEmbeddedType>& rThis )
{
...
GeometryData THBBrepSurface<TContainerPointType, TContainerPointEmbeddedType>::msGeometryData(
    &msGeometryDimension,
    GeometryData::IntegrationMethod::GI_GAUSS_1,
    {}, {}, {});

template<class TContainerPointType, class TContainerPointEmbeddedType>
const GeometryDimension THBBrepSurface<TContainerPointType, TContainerPointEmbeddedType>::msGeometryDimension(3, 2);
