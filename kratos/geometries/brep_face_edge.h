...
#if !defined(KRATOS_BREP_FACE_EDGE_3D_H_INCLUDED )
#define  KRATOS_BREP_FACE_EDGE_3D_H_INCLUDED
 ...
 * @class BrepFaceEdge
 * @ingroup KratosCore
 * @brief The BrepFaceEdge acts as topology for faces having a pointer
 *        to a surface geometry and a list of BrepEdge s. Those edges
 *        define the trimming of the respective surface.
 */
template<class TPointType> class BrepFaceEdge
...
public:
    ...
    /** Pointer definition of BrepFaceEdge */
    KRATOS_CLASS_POINTER_DEFINITION( BrepFaceEdge );
     ...
     */
    BrepFaceEdge( BrepFaceEdge const& rOther )
        : BaseType( rOther )
    {
    }
     ...
     * source geometry's points too.
     */
    template<class TOtherPointType> explicit BrepFaceEdge( BrepFaceEdge<TOtherPointType> const& rOther )
        : BaseType( rOther )
    {
    }
     ...
     * @see ClonePoints
     */
    BrepFaceEdge& operator=( const BrepFaceEdge& rOther )
    {
    ...
    BrepFaceEdge& operator=( BrepFaceEdge<TOtherPointType> const & rOther )
    {
        BaseType::operator=( rOther );
        return *this;
    }
    ...
    {
        return typename BaseType::Pointer( new BrepFaceEdge( ThisPoints ) );
    }
    ...
    */
    ...
    ) const
    {
        ...
        for (each point in mLinearizedPolygon) { //The first node is already assigned, so we can start from 1
...
protected:
    /**
     * There are no protected members in class BrepFaceEdge
     */
...
private:
    ...
    NurbsCurve<2>::Pointer mpNurbsEdge;

    bool mIsGeometryDataInitialized;
    GeometryData mGeometryData;

    bool mIsLinearizedPolygonInitialized;
    ...
    BrepFaceEdge(): BaseType( PointsArrayType(), &mGeometryData ) {}
    ...
    template<class TOtherPointType> friend class BrepFaceEdge;
...
template<class TPointType> inline std::istream& operator >> (
    ...
    BrepFaceEdge<TPointType>& rThis );
/**
 * output stream functions
 */
template<class TPointType> inline std::ostream& operator << (
    std::ostream& rOStream,
    const BrepFaceEdge<TPointType>& rThis )
{
...
#endif // KRATOS_BREP_FACE_EDGE_3D_H_INCLUDED  defined
