...
protected:
        ...
        )
    {
        ...
        if (BaseType::mCalculateReactionsFlag) {
            for (auto& r_dof : BaseType::mDofSet) {
                const bool is_master_fixed = mDoFMasterFixedSet.find(r_dof.Id()) == mDoFMasterFixedSet.end() ? false : true;
                const bool is_slave = mDoFSlaveSet.find(r_dof.Id()) == mDoFSlaveSet.end() ? false : true;
        ...
        if (norm_b > 0.0) {
             ...
             for (auto& r_dof : BaseType::mDofSet) {
                 if (r_dof.EquationId() < BaseType::mEquationSystemSize) {
                    auto it = mDoFSlaveSet.find(r_dof.Id());
    ...
    */
        ...
        )
    {
        ...
        for (auto& dof : BaseType::mDofSet) {
            if (dof.EquationId() < BaseType::mEquationSystemSize) {
                ...
                auto it = mDoFSlaveSet.find(dof.Id());
                if (it == mDoFSlaveSet.end()) {
        ...
        ) override
    {
        ...
        // Updating variables
        for (auto& r_dof : BaseType::mDofSet) {
            if ((r_dof.IsFixed()) || mDoFSlaveSet.find(r_dof.Id()) != mDoFSlaveSet.end()) {
        ...
        if (mDoFMasterFixedSet.size() > 0) {
            ...
            for (IndexType i = 0; i < BaseType::mDofSet.size(); ++i) {
                ...
                if (equation_id < BaseType::mEquationSystemSize ) {
                    auto it_first_check = mDoFSlaveSet.find(it_dof->Id());
                    if (it_first_check == mDoFSlaveSet.end()) {
                        auto it_second_check = mDoFSlaveSet.find(it_dof->Id());
                        if (it_second_check == mDoFSlaveSet.end()) {
                            if(mDoFMasterFixedSet.find(it_dof->Id()) == mDoFMasterFixedSet.end()) {
...
private:
    ...
    void SetUpSystemWithConstraints(ModelPart& rModelPart)
    {
        ...
        for (auto& dof : BaseType::mDofSet) {
            if (dof.IsFixed()) {
                auto it = mDoFMasterFixedSet.find(dof.Id());
        ...
        )
    {
        ...
        for (IndexType i = 0; i < BaseType::mDofSet.size(); ++i) {
            ...
            const IndexType equation_id = it_dof->EquationId();
            if (equation_id < BaseType::mEquationSystemSize ) {
                auto it = mDoFSlaveSet.find(it_dof->Id());
                if (it == mDoFSlaveSet.end()) {
        ...
        )
    {
        ...
        if (!BaseType::mCalculateReactionsFlag) {
            ...
            for (IndexType i_local = 0; i_local < local_size; ++i_local) {
                ...
                const bool is_master_fixed = mDoFMasterFixedSet.find(it_dof->Id()) == mDoFMasterFixedSet.end() ? false : true;
                const bool is_slave = mDoFSlaveSet.find(it_dof->Id()) == mDoFSlaveSet.end() ? false : true;
                if (is_master_fixed || is_slave) { // Fixed or MPC dof
