    ...
    int Verbosity) {
        ...
        KRATOS_TRY
            ...
            do {
                ...
                IndexPartition<std::size_t>(rRelationMatrix.size1()).for_each(
                    [
                        &rRelationMatrix,
                        &rConstraintGaps,
...
template <class TSparse, class TDense>
AugmentedLagrangeConstraintAssembler<TSparse,TDense>::AugmentedLagrangeConstraintAssembler(
    Parameters Settings,
    std::string&& rInstanceName)
        :   Base(ConstraintImposition::AugmentedLagrange, std::move(rInstanceName)),
            mpImpl(new Impl{
                /*mConstraintIdToIndexMap: */ CSRHashMap<std::size_t,std::pair<std::size_t,std::size_t>>(),
...
template <class TSparse, class TDense>
void AugmentedLagrangeConstraintAssembler<TSparse,TDense>::AllocateConstraints(
...
template <class TSparse, class TDense>
void AugmentedLagrangeConstraintAssembler<TSparse,TDense>::AllocateSystem(
...
template <class TSparse, class TDense>
void AugmentedLagrangeConstraintAssembler<TSparse,TDense>::Assemble(
...
template <class TSparse, class TDense>
void AugmentedLagrangeConstraintAssembler<TSparse,TDense>::Initialize(
    typename TSparse::MatrixType& rLhs,
    typename TSparse::VectorType& rSolution,
    typename TSparse::VectorType& rRhs,
    [[maybe_unused]] typename Base::DofSet& rDofs) {
        ...
        KRATOS_TRY
            ...
            // Apply initial penalty- and lagrange terms to the left- and right hand sides.
            {
                ...
                KRATOS_ERROR_IF(
                    r_transpose_relation_matrix.size1() != this->GetRelationMatrix().size2()
...
template <class TSparse, class TDense>
void AugmentedLagrangeConstraintAssembler<TSparse,TDense>::InitializeConstraintIteration(
    typename TSparse::MatrixType& rLhs,
    typename TSparse::VectorType& rSolution,
    typename TSparse::VectorType& rRhs,
    ...
    [[maybe_unused]] typename Base::DofSet::iterator itDofEnd) {
        ...
        KRATOS_TRY
            ...
            // Update the RHS.
            BalancedProduct<TSparse,TSparse,TSparse>(
                this->GetTransposeRelationMatrix(),
                constraint_residual,
                rRhs,
...
template <class TSparse, class TDense>
bool
AugmentedLagrangeConstraintAssembler<TSparse,TDense>::FinalizeConstraintIteration(
    typename TSparse::MatrixType& rLhs,
    typename TSparse::VectorType& rSolution,
...
template <class TSparse, class TDense>
void AugmentedLagrangeConstraintAssembler<TSparse,TDense>::Finalize(
    [[maybe_unused]] typename TSparse::MatrixType& rLhs,
...
AugmentedLagrangeConstraintAssembler<TSparse,TDense>::GetTransposeRelationMatrix() {
    KRATOS_TRY
        if (!mpImpl->mMaybeTransposeRelationMatrix.has_value()) {
            ...
            SparseMatrixMultiplicationUtility::TransposeMatrix(
                mpImpl->mMaybeTransposeRelationMatrix.value(),
                this->GetRelationMatrix());
        }
