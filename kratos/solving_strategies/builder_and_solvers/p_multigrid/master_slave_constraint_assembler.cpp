...
#include "solving_strategies/builder_and_solvers/p_multigrid/constraint_utilities.hpp" // detail::MakeRelationTopology
...
struct MasterSlaveConstraintAssembler<TSparse,TDense>::Impl {
    ...
    CSRHashMap<
        std::size_t,        //< identifier of the constraint equation (slave ID for MasterSlaveConstraint, CONSTRAINT_LABEL for MultifreedomConstraint)
        std::pair<
    ...
    typename TSparse::VectorType mMasterSlaveConstants;
    ...
    std::vector<Dof<double>::IndexType> mSlaves;

    int mVerbosity;

    std::pair<typename TSparse::MatrixType&, LinearSolver<TSparse,TDense>&> GetConstraintSystem() {
        if (!mConstraintSystem.has_value()) {
            KRATOS_TRY
            ...
            mConstraintSystem.emplace();

            typename TSparse::MatrixType& r_lhs = mConstraintSystem.value().first;
            SparseMatrixMultiplicationUtility::MatrixMultiplication(
                transpose_master_slave_relations,
                mMasterSlaveRelations,
                r_lhs);

            mConstraintSystem.value().second = LinearSolverFactory<TSparse,TDense>().Create(mConstraintSolverSettings);
            typename TSparse::VectorType dummy(r_lhs.size1());
            mConstraintSystem.value().second->Initialize(r_lhs, dummy, dummy);
            mConstraintSystem.value().second->InitializeSolutionStep(r_lhs, dummy, dummy);
            KRATOS_CATCH("")
        }

        return std::pair<typename TSparse::MatrixType&,LinearSolver<TSparse,TDense>&>(
            mConstraintSystem.value().first,
            *mConstraintSystem.value().second);
    }

    Parameters mConstraintSolverSettings;
...
template <class TSparse, class TDense>
MasterSlaveConstraintAssembler<TSparse,TDense>::MasterSlaveConstraintAssembler(
    Parameters Settings,
        ...
        :   Base(ConstraintImposition::MasterSlave, std::move(rInstanceName)),
            mpImpl(new Impl) {
                KRATOS_TRY
                    ...
                    mpImpl->mConstraintSolverSettings = Settings["constraint_solver_settings"];
...
template <class TSparse, class TDense>
void MasterSlaveConstraintAssembler<TSparse,TDense>::AllocateConstraints(
    ...
    typename Base::DofSet::const_iterator itDofEnd) {
        ...
        KRATOS_TRY
            ...
            if (itConstraintBegin == itConstraintEnd) {
                ...
                std::fill(
                    this->GetRelationMatrix().index1_data().begin(),
            ...
            // Compute the relation matrix (T) for master-slave imposition.
            // This involves a partitioning of the DoFs (u) into:
            // 0) DoFs that don't participate in any constraint     (w)
            // 1) master DoFs                                       (m)
            // 2) slave DoFs                                        (s)
            //
            ...
            mpImpl->mMasterSlaveRelations = typename TSparse::MatrixType(
                dependent_system_size,
                master_slave_system_size,
                master_slave_entry_count);
            TSparse::SetToZero(mpImpl->mMasterSlaveRelations);

            mpImpl->mMasterSlaveConstants = typename TSparse::VectorType(dependent_system_size);
            TSparse::SetToZero(mpImpl->mMasterSlaveConstants);
            ...
            mpImpl->mSlaves.clear();
            mpImpl->mSlaves.reserve(dependent_system_size);
            ...
            for (typename TSparse::IndexType i_row=0; i_row<dependent_system_size; ++i_row) {
                if (row_sizes[i_row].has_value()) {
                    mpImpl->mSlaves.push_back(i_row);
                    mpImpl->mMasterSlaveRelations.index1_data()[i_row + 1] = mpImpl->mMasterSlaveRelations.index1_data()[i_row] + row_sizes[i_row].value();
                } else {
                    mpImpl->mIndependentMap[i_row] = i_independent_dof++;
                    mpImpl->mMasterSlaveRelations.index1_data()[i_row + 1] = mpImpl->mMasterSlaveRelations.index1_data()[i_row] + 1;
                }
...
template <class TSparse, class TDense>
void MasterSlaveConstraintAssembler<TSparse,TDense>::Assemble(
    ...
    const bool AssembleRhs) {
        ...
        if (mpImpl->mConstraintIdMap.empty())
            return;
        ...
        KRATOS_TRY
            ...
            for (const auto& r_constraint_equation_info : mpImpl->mConstraintIdMap) {
                ...
                auto it_slave = std::lower_bound(
                    this->GetRelationMatrix().index2_data().begin() + i_relation_entry_begin,
                    this->GetRelationMatrix().index2_data().begin() + i_relation_entry_end,
                    i_slave);
                KRATOS_ERROR_IF(
                    it_slave == this->GetRelationMatrix().index2_data().begin() + i_relation_entry_end
                    || *it_slave != i_slave)
                ...
                const typename TSparse::IndexType i_slave_in_master_slave = std::distance(
                    this->GetRelationMatrix().index2_data().begin(),
                    it_slave);
                const typename TSparse::DataType slave_coefficient = this->GetRelationMatrix().value_data()[i_slave_in_master_slave];

                KRATOS_ERROR_IF_NOT(slave_coefficient)
                    << "coefficient of slave DoF " << i_slave << " in constraint equation " << i_constraint << " vanished";
                ...
                mpImpl->mMasterSlaveConstants[i_slave] += coefficient_scale * this->GetConstraintGapVector()[i_constraint];
                ...
                KRATOS_ERROR_IF_NOT(i_master_slave_entry == mpImpl->mMasterSlaveRelations.index1_data()[i_slave + 1])
                    << "incorrect mapping between constraint gradient and relation matrix "
                    << "associated with slave DoF " << (itDofBegin + i_slave)->GetVariable().Name() << " "
                    << "of node " << (itDofBegin + i_slave)->Id();
            }
        KRATOS_CATCH("")
}
...
template <class TSparse, class TDense>
void MasterSlaveConstraintAssembler<TSparse,TDense>::Initialize(
        ...
        KRATOS_TRY
            ...
            // Compute the transposed matrix of the global relation matrix
            {
                ...
                typename TSparse::MatrixType left_multiplied_lhs(mpImpl->mMasterSlaveRelations.size2(), rLhs.size2());
                {
                    ...
                    // Compute part of the modified LHS.
                    SparseMatrixMultiplicationUtility::MatrixMultiplication(
                        transposed_relation_matrix,
                        rLhs,
                        left_multiplied_lhs);
...
template <class TSparse, class TDense>
bool
MasterSlaveConstraintAssembler<TSparse,TDense>::FinalizeConstraintIteration(
    typename TSparse::MatrixType& rLhs,
    typename TSparse::VectorType& rSolution,
    typename TSparse::VectorType& rRhs,
    ...
    PMGStatusStream& rStream) {
        KRATOS_TRY
            ...
            mpImpl->mDependentSystem.value().mSolution = rSolution;
            this->ComputeDependentSolution(mpImpl->mDependentSystem.value().mSolution);
...
template <class TSparse, class TDense>
void MasterSlaveConstraintAssembler<TSparse,TDense>::Finalize(
...
void MasterSlaveConstraintAssembler<TSparse,TDense>::ComputeDependentResidual(typename TSparse::VectorType& rResidual) const {
    KRATOS_TRY
        ...
        typename TSparse::VectorType buffer(mpImpl->mMasterSlaveRelations.size2());
        TSparse::SetToZero(buffer);

        // Solve linear system.
        auto [r_lhs, r_linear_solver] = mpImpl->GetConstraintSystem();

        KRATOS_TRY
            r_linear_solver.PerformSolutionStep(r_lhs, buffer, rResidual);
        KRATOS_CATCH("")

        TSparse::Resize(rResidual, mpImpl->mMasterSlaveRelations.size1());
        TSparse::SetToZero(rResidual);
        BalancedProduct<TSparse,TSparse,TSparse>(
            mpImpl->mMasterSlaveRelations,
            buffer,
            rResidual);
...
void MasterSlaveConstraintAssembler<TSparse,TDense>::ComputeIndependentSolution(typename TSparse::VectorType& rSolution) const {
    KRATOS_TRY
        if (mpImpl->mConstraintIdMap.empty())
            return;
        typename TSparse::VectorType independent_solution(mpImpl->mMasterSlaveRelations.size2());

        Dof<double>::IndexType i_dependent_begin = 0;
        Dof<double>::IndexType i_independent_begin = 0;
        for (const auto i_dependent_end : mpImpl->mSlaves) {
            std::copy(
                rSolution.begin() + i_dependent_begin,
                rSolution.begin() + i_dependent_end,
                independent_solution.begin() + i_independent_begin);
            i_independent_begin += i_dependent_end - i_dependent_begin;
            i_dependent_begin = i_dependent_end + 1;
        }
        std::copy(
            rSolution.begin() + i_dependent_begin,
            rSolution.end(),
...
void MasterSlaveConstraintAssembler<TSparse,TDense>::ComputeDependentSolution(typename TSparse::VectorType& rSolution) const {
    KRATOS_TRY
        ...
        BalancedProduct<TSparse,TSparse,TSparse>(
            ...
            rSolution,
            dependent_solution);
        TSparse::UnaliasedAdd(dependent_solution, 1, mpImpl->mMasterSlaveConstants);
...
MasterSlaveConstraintAssembler<TSparse,TDense>::GetDependentDofs(const typename MasterSlaveConstraintAssembler<TSparse,TDense>::DofSetType& rIndependentDofSet) const noexcept {
    if (mpImpl->mConstraintIdMap.empty()) return rIndependentDofSet;
    else return mpImpl->mDependentSystem.value().mDofSet;
}
