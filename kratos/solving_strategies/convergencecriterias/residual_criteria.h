...
public:
        ...
        ) override
    {
        ...
        if (size_b != 0) { //if we are solving for something
            ...
            KRATOS_INFO_IF("RESIDUAL CRITERION", this->GetEchoLevel() > 0 && rModelPart.GetCommunicator().MyPID() == 0) << " :: [ Initial residual norm = " << mInitialResidualNorm << "; Current residual norm =  " << mCurrentResidualNorm << "]" << std::endl;
...
protected:
        ...
        )
    {
        ...
        KRATOS_WATCH(rDofNum)
        KRATOS_WATCH(rResidualSolutionNorm)
    }
