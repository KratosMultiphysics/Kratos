...
protected:
        ...
        ) override
    {
        ...
        if (rD.size1() != 0 && TDenseSpace::TwoNorm(rD) > ZeroTolerance) {
            rConstElemRef.GetFirstDerivativesVector(DerivedBaseType::mVector.v[this_thread], 0);
        ...
        ) override
    {
        ...
        if (rD.size1() != 0 && TDenseSpace::TwoNorm(rD) > ZeroTolerance) {
            rConstCondRef.GetFirstDerivativesVector(DerivedBaseType::mVector.v[this_thread], 0);
            noalias(rRHSContribution) -= prod(rD, DerivedBaseType::mVector.v[this_thread]);
        } else if (rM.size1() != 0) {
            const double beta = rCurrentProcessInfo[*mpRayleighBeta];
            rConstCondRef.GetFirstDerivativesVector(DerivedBaseType::mVector.v[this_thread], 0);
