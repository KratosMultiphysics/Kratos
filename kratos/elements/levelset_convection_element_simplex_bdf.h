...
class LevelSetConvectionElementSimplexBDF
    : public Element
{
public:
    ///@name Type Definitions
    ...
    /// Default constructor.
    LevelSetConvectionElementSimplexBDF() : Element()
    {}

    LevelSetConvectionElementSimplexBDF(IndexType NewId, GeometryType::Pointer pGeometry)
        : Element(NewId, pGeometry)
    {}

    LevelSetConvectionElementSimplexBDF(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties)
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new LevelSetConvectionElementSimplexBDF(NewId, GetGeometry().Create(ThisNodes), pProperties));
        KRATOS_CATCH("");
    }
    ...
    void CalculateLocalSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        for (unsigned int i = 0; i < TNumNodes; i++)
        {
            ...
            phi_old_1[i] = GetGeometry()[i].FastGetSolutionStepValue(rUnknownVar,2);
...
//         if(nneg > 0) beta = 1.0; //beta = 0.1;
        ...
        BoundedMatrix<double, TNumNodes, TNumNodes> aux_bdf = ZeroMatrix(TNumNodes, TNumNodes); // terms multiplying dphi/dt
        ...
        for(unsigned int igauss=0; igauss<TDim+1; igauss++)
        {
            ...
            //terms multiplying bdf coefficients

            noalias(aux_bdf) = (1 + tau * beta * div_v) * outer_prod(N, N);
            noalias(aux_bdf) += tau * outer_prod(a_dot_grad, N);

            noalias(aux1) =  tau * beta * div_v * outer_prod(N, N);
        ...
        //adding the second and third term in the LHS terms regarding to phi
        noalias(rLeftHandSideMatrix) = (BDFcoeffs[0]) * aux_bdf;
        noalias(rLeftHandSideMatrix) += aux1;

        noalias(rRightHandSideVector) = BDFcoeffs[1] * prod(aux_bdf, phi_old);
        noalias(rRightHandSideVector) += BDFcoeffs[2] * prod(aux_bdf, phi_old_1);
        noalias(rRightHandSideVector) -= prod(aux_bdf, a_n);
        // if (mFractionalStep){
        //     noalias(rRightHandSideVector) -= prod(aux_bdf, a_n);
        // }
...
private:
    ...
    // bool mFractionalStep; // Flag to indicate if the convection element has acceleration.
    ///@}
    ///@name Serialization
    ///@{
