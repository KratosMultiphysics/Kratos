...
#if !defined(KRATOS_DISTANCE_CALCULATION_MODIFIED_ELEMENT_H_INCLUDED )
#define  KRATOS_DISTANCE_CALCULATION_MODIFIED_ELEMENT_H_INCLUDED
...
class DistanceCalculationElementSimplexModified : public Element
{
public:
    ///@name Type Definitions
    ///@{

    /// Pointer definition of DistanceCalculationElementSimplexModified
    KRATOS_CLASS_INTRUSIVE_POINTER_DEFINITION(DistanceCalculationElementSimplexModified);
     ...
     */
    DistanceCalculationElementSimplexModified(IndexType NewId = 0) :
        Element(NewId)
    {}
     ...
     */
    DistanceCalculationElementSimplexModified(IndexType NewId, const NodesArrayType& ThisNodes) :
     ...
     */
    DistanceCalculationElementSimplexModified(IndexType NewId, GeometryType::Pointer pGeometry) :
     ...
     */
    DistanceCalculationElementSimplexModified(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties) :
        Element(NewId, pGeometry, pProperties)
    {}

    /// Destructor.
    ~DistanceCalculationElementSimplexModified() override
    {}
    ...
    /**
     * Returns a pointer to a new DistanceCalculationElementSimplexModified element, created using given input
    ...
    {
        return Element::Pointer(new DistanceCalculationElementSimplexModified(NewId, GetGeometry().Create(ThisNodes), pProperties));
    }
    ...
    Element::Pointer Create(IndexType NewId,
                            ...
                            PropertiesType::Pointer pProperties) const override
    {
        return Element::Pointer(new DistanceCalculationElementSimplexModified(NewId, pGeom, pProperties));
    }
                              ...
                              const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        {
            const double dist = GetGeometry()[i].FastGetSolutionStepValue(DISTANCE);
            distances[i] = dist;
//			if (distances[i] >= 0.0 && distances[i] < 1e-30) distances[i] = 1e-30;
       ...
       const double dgauss = inner_prod(N,distances);
        ...
        if(step == 1) //solve a poisson problem with a positive/negative heat source depending on the sign of the existing distance function
        {
            ...
            //compute RHS
            double source = 1.0;
            if(dgauss <= 0.0) source=-5.0;
            //else if (dgauss > 0.02) source=1.0;
            /* double source = 0.0;
            if (num_pos == number_of_points){
                source = 1;
            } else {
                source = -10;
            } */
        ...
        else //solve an optimization problem with the goal of achievieng a gradient of one for the distance function
        {
            ...
            const Vector temp = prod(DN_DX,grad);
            noalias(rRightHandSideVector) = Area*(1.0 - grad_norm)*temp;// prod(DN_DX,grad);
    ...
    int Check(const ProcessInfo& rCurrentProcessInfo) const override
{
    ...
    std::string Info() const override
    {
        ...
        buffer << "DistanceCalculationElementSimplexModified #" << Id();
        return buffer.str();
    }
    ...
    {
        rOStream << "DistanceCalculationElementSimplexModified" << TDim << "D";
    }
...
private:
    ...
    /// Assignment operator.
    DistanceCalculationElementSimplexModified & operator=(DistanceCalculationElementSimplexModified const& rOther);

    /// Copy constructor.
    DistanceCalculationElementSimplexModified(DistanceCalculationElementSimplexModified const& rOther);

    ///@}

}; // Class DistanceCalculationElementSimplexModified
...
inline std::istream& operator >>(std::istream& rIStream,
                                 DistanceCalculationElementSimplexModified<TDim>& rThis)
{
    return rIStream;
}
...
inline std::ostream& operator <<(std::ostream& rOStream,
                                 const DistanceCalculationElementSimplexModified<TDim>& rThis)
{
...
#endif // KRATOS_DISTANCE_CALCULATION_MODIFIED_ELEMENT_H_INCLUDED  defined
