...
#if !defined(KRATOS_LEVELSET_CONVECTION_ELEMENT_SIMPLEX_FLUX_INCLUDED )
#define  KRATOS_LEVELSET_CONVECTION_ELEMENT_SIMPLEX_FLUX_INCLUDED
...
class LevelSetConvectionElementSimplexFlux
    : public Element
{
public:
    ///@name Type Definitions
    ...
    /// Counted pointer of
    KRATOS_CLASS_INTRUSIVE_POINTER_DEFINITION(LevelSetConvectionElementSimplexFlux);
    ...
    /// Default constructor.
    LevelSetConvectionElementSimplexFlux() : Element()
    {}

    LevelSetConvectionElementSimplexFlux(IndexType NewId, GeometryType::Pointer pGeometry)
    : Element(NewId, pGeometry)
    {}

    LevelSetConvectionElementSimplexFlux(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties)
    : Element(NewId, pGeometry, pProperties)
    {}

    /// Destructor.
    ~LevelSetConvectionElementSimplexFlux() override {};
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new LevelSetConvectionElementSimplexFlux(NewId, GetGeometry().Create(ThisNodes), pProperties));
        KRATOS_CATCH("");
    }
    ...
    void CalculateLocalSystem(MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        for(unsigned int igauss=0; igauss<TDim+1; igauss++)
        {
            ...
            noalias(K_matrix) += outer_prod(v_dot_grad_N, N);
        ...
        for (unsigned int i = 0; i < TNumNodes; i++)
        {
            for (unsigned int j = 0; j < TNumNodes; j++)
            {
                ...
                //nu_e_vector = std::max( nu_e, std::max(0.0, -K_matrix(i, j))/std::abs(S_vector[i]) ); ? Kij needs to be multiplied by phi
            }
        }
    ...
    {
        return "LevelSetConvectionElementSimplexFlux #";
    }
...
#endif // KRATOS_LEVELSET_CONVECTION_ELEMENT_SIMPLEX_FLUX_INCLUDED  defined
