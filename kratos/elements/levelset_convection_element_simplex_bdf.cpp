...
namespace Kratos
{
   ...
   template< unsigned int TDim, unsigned int TNumNodes>
        ...
        const ProcessInfo &rCurrentProcessInfo)
    {
        ...
        array_1d<double, TNumNodes> phi, phi_old, phi_old_1, a_n;
        ...
        for (unsigned int i = 0; i < TNumNodes; i++)
        {
            ...
            // a_n[i] = GetGeometry()[i].FastGetSolutionStepValue(rVolumeSourceVar, 0);
        ...
        for(unsigned int igauss=0; igauss<TDim+1; igauss++)
        {
            ...
            if (this->IsElementTauNodal())
            {
                ...
                tau = 1.0 / (tau_denom);
                //  const double tau_denom = 2.0* norm_vel / h;
                // tau = 1.0 / (tau_denom);
            ...
            // assemble RHS and LHS Gaus point contribution
             ...
             for (unsigned int i = 0; i < TNumNodes; i++)
            {
                double phi_s_rhs = 0.0;
                double phi_s_lhs = 0.0;
                for (unsigned int j = 0; j < TNumNodes; j++) {
                    ...
                    // // Stabilization terms
                    // const double acc = BDFcoeffs[0] * phi[j] + BDFcoeffs[1] * phi_old[j] + BDFcoeffs[2] * phi_old_1[j];

                    phi_s_rhs += tau * (-a_dot_grad[j]*phi[j]+proj_oss);
                    phi_s_lhs += tau * (-a_dot_grad[j]);


                    for (unsigned int d = 0; d < TDim; ++d)
                    {

                        rRightHandSideVector[i] += DN_DX(i, d) * vel_gauss[d] * phi_s_rhs;

                        rLeftHandSideMatrix(i, j) -= DN_DX(i, d) * vel_gauss[d] * phi_s_lhs;
                    }
        ...
        const ProcessInfo &rCurrentProcessInfo)
    {
        ...
        for (unsigned int igauss = 0; igauss < TDim + 1; igauss++)
        {
            ...
            // convection operator
            double a_dot_grad_phi = inner_prod(vel_gauss, grad_phi);
            a_dot_grad_phi *= Volume / static_cast<double>(TNumNodes);
            ...
            for (IndexType i_node = 0; i_node < TNumNodes; ++i_node)
            {
                ...
                AtomicAdd(r_geometry[i_node].GetValue(VOLUMETRIC_STRAIN_PROJECTION),  N[i_node] * a_dot_grad_phi);
            }
        }
