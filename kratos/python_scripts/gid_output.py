...
class GiDOutput(object):
    ...
    def write_step_to_list(self,step_label=None):
        ...
        with open(self.listfilename,"a") as listfile:
            if step_label is not None:
                listfile.write("{0}_{1}{2}\n".format(self.filename,step_label,ext))
            else:
    ...
    def __init__(self,
        ...
        # Initialize GiD io
        self.io = GidIO(self.filename,self.post_mode,self.multi_file,self.write_deformed_mesh,self.write_conditions)
    ...
    def get_out_model_part(self,model_part):
        if self.volume_output:
            ...
            if self.cut_number == 0:
                raise Exception("GiD IO Error: writing mesh in cut mode, but no cuts were defined. Please call define_cuts first")
