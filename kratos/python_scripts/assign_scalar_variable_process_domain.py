...
class AssignScalarVariableProcess(KratosMultiphysics.Process):
    def __init__(self, Model, settings ):
        ...
        self.p_in   = settings["p_in"].GetDouble()
        self.rho_in   = settings["rho_in"].GetDouble()
        self.radius = settings["radius"].GetDouble()
            ...
            if self.value_is_numeric:
                for node in self.mesh.Nodes:
                    ...
                    if(x**2 + y**2 < self.radius**2):
                        self.variable_utils.SetVariable(self.variable, self.p_in, node)
                    else:
                        self.variable_utils.SetVariable(self.variable, self.value, node)
            else:
