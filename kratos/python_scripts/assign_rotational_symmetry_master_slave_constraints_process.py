...
class AssignRotationalSymmetryMasterSlaveConstraintsProcess(KM.Process):
    ...
    def __init__(self, model, settings):
        ...
        model.CreateModelPart("AuxiliarMasterModelPart")
        self.aux_master_model_part = model.GetModelPart("AuxiliarMasterModelPart")
    ...
    def ExecuteInitialize(self):
        ...
        # Create ghost nodes in the master model part
        self.__CreateGhostNodes(self.master_model_part.Nodes)

        # Initialize MasterSlaveConstraints to neighbours utility with the master model part nodes (which now includes ghost nodes)
        self.assign_mscs_utility = KM.AssignMasterSlaveConstraintsToNeighboursUtility(self.aux_master_model_part.Nodes)
    ...
    def ExecuteInitializeSolutionStep(self):
        ...
        self -- It signifies an instance of a class.
        """
        # Assign dynamically the master-slave constraints
        ...
        self.assign_mscs_utility.AssignRotationalMasterSlaveConstraintsToNodes(self.slave_model_part.Nodes, self.search_radius, self.computing_model_part, self.variables_list, self.minimum_number_of_neighbouring_nodes, self.ghost_to_original_mapping)
    ...
    def ExecuteFinalizeSolutionStep(self):
        ...
        self -- It signifies an instance of a class.
        """
        # Since MasterSlaveConstraints are updated every time step, they need to be removed before being re-assigned.
    ...
    def __CreateGhostNodes(self, nodes):
        """Create ghost nodes for each node by rotating them about the origin.

        Keyword arguments:
        nodes -- The nodes to be rotated to create the ghost nodes.
        """
        self.ghost_to_original_mapping = {}  # Initialize the mapping as an attribute of the class
        ...
        for angle in rotation_angles:
            ...
            for original_node in nodes:
                ...
                # Store the mapping using node IDs and the rotation angle
                self.ghost_to_original_mapping[ghost_node.Id] = (original_node.Id, np.pi*angle/180)
                if original_node.Id==922:
                    print(angle)
                    print(ghost_node.Id)
    ...
    def __RotateNode(self, original_node, angle_degrees):
        ...
        Keyword arguments:
        original_node -- The node to be rotated.
        angle_degrees -- The angle in degrees by which the node is to be rotated.
        """
