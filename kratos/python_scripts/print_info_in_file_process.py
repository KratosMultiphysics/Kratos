...
class PrintInfoInFileProcess(KratosMultiphysics.OutputProcess):
    ...
    def PrintInFile(self, values):
        self.ascii_writer.write("{0:.8e}".format(self.__GetTime()).rjust(20) + "\t")
        if not isinstance(values, float):
            for value in values:
                self.ascii_writer.write("{0:.8e}".format(value).rjust(20) + "\t")
        else:
            self.ascii_writer.write("{0:.8e}".format(values).rjust(20) + "\t")
