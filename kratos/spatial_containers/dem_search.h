...
class RadiusPoint
{
    public:
      ...
      void Initialize(ModelPart::ElementPointerType baseElem)
      {
...
//           mRadius = baseElem->GetGeometry()[0].FastGetSolutionStepValue(RADIUS);
      ...
      void Initialize(ModelPart::ElementPointerType baseElem, double Radius)
      {
          for(std::size_t i = 0; i < Dimension; i++)
              coord[i] = baseElem->GetGeometry()[0][i];
...
//           mRadius = Radius;
    ...
    public:
      ...
      ModelPart::ElementPointerType pNaseElem;
