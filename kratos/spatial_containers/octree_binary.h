...
namespace Kratos {
    ...
    public:
        ...
        key_type Key(coordinate_type coordinate) const {
        ...
        bool CheckConstrain2To1() const{
          //POOYAN. Esta funcion tiene que devolver true si el octree esta balanceado, y false si no.
          return true;
        }
                  ...
                  if (current_leaf->GetNeighbourKey(i_direction, neighbour_key)) {
                    ...
                    if (neighbour_cell->GetLevel() > i_level + 1) {
                      ...
                      for (char j_level = neighbour_cell->GetLevel(); j_level > i_level + 1; j_level--) {
                        ...
                        temp_neighbour_cell->TransferObjectsToSons();
                        //                                        for (std::size_t j = 0; j < CHILDREN_NUMBER; j++) {
        ...
        {        
            ...
            key_type min_x_key = Key(coord1[0]);
            key_type min_y_key = Key(coord1[1]);
            key_type min_z_key = Key(coord1[2]);

            key_type max_x_key = Key(coord2[0]);
            key_type max_y_key = Key(coord2[1]);
            key_type max_z_key = Key(coord2[2]);
        ...
        cell_type * pGetCell(const coordinate_type * point) const {
            key_type keys[3];
            keys[0] = Key(point[0]);
            keys[1] = Key(point[1]);
            keys[2] = Key(point[2]);
        ...
        int SubvidiveUntilSize(double* coord, const double desired_size){
            key_type x_key = Key(coord[0]);
            key_type y_key = Key(coord[1]);
            key_type z_key = Key(coord[2]);
        ...
        int RefineWithUniformSize(const double uniform_size){
