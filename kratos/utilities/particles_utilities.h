...
/**@brief: this function takes all the nodes in rParticlesModelPart and finds in which element they fall within
    ...
    */
    ...
    )
    {
        ...
        std::vector<char> is_found_aux_vector(rCoordinates.size1()); //aux vector using ints to allow running in omp
        ...
        IndexPartition(rCoordinates.size1()).for_each(TLS, [&rLocator, &rCoordinates, &interpolations, &is_found_aux_vector, &rInterpolationVariable, &zero, SearchTolerance](const auto& i, auto& rTLS)
        {
            ...
            is_found_aux_vector[i] = static_cast<char>(is_found);
            if(is_found)
            {
        ...
        std::copy(is_found_aux_vector.begin(), is_found_aux_vector.end(), interpolations.first.begin());
