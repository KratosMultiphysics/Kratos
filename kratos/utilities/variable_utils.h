...
#pragma omp parallel for
    for (int k = 0; k < static_cast<int>(rNodes.size()); ++k) {
      NodesIteratorType it_node = rNodes.begin() + k;
...
#pragma omp parallel for
  ...
  void SetFlag(const Flags &rFlag, const bool &rFlagValue,
...
#pragma omp parallel for
    for (int k = 0; k < static_cast<int>(rContainer.size()); ++k) {
      TIteratorType i_cont = rContainer.begin() + k;
      i_cont->Set(rFlag, rFlagValue);
    }
    KRATOS_CATCH("")
  }

  /**
   * Takes the value of a non-historical vector variable and sets it in other
   * variable
...
#pragma omp parallel for
    for (int k = 0; k < static_cast<int>(rNodes.size()); ++k) {
      NodesIteratorType it_node = rNodes.begin() + k;
      it_node->SetValue(SavedVariable,
                        it_node->FastGetSolutionStepValue(OriginVariable));
    }
    KRATOS_CATCH("")
  }

  /**
   * Takes the value of a non-historical scalar variable and sets it in other
   * variable
...
#pragma omp parallel for
  ...
  /**
   * Takes the value of an historical vector variable and sets it in other
   * variable
   * @param OriginVariable: reference to the origin vector variable
...
#pragma omp parallel for
    for (int k = 0; k < static_cast<int>(rNodes.size()); ++k) {
      NodesIteratorType it_node = rNodes.begin() + k;
      noalias(it_node->FastGetSolutionStepValue(DestinationVariable)) =
          it_node->FastGetSolutionStepValue(OriginVariable);
    }
    KRATOS_CATCH("")
  }
  ...
  /**
   * Takes the value of an historical double variable and sets it in other
   * variable
...
#pragma omp parallel for
    for (int k = 0; k < static_cast<int>(rNodes.size()); ++k) {
      NodesIteratorType it_node = rNodes.begin() + k;
      it_node->FastGetSolutionStepValue(DestinationVariable) =
          it_node->FastGetSolutionStepValue(OriginVariable);
    }
    KRATOS_CATCH("")
  }
...
#pragma omp parallel for
    for (int k = 0; k < static_cast<int>(rNodes.size()); ++k) {
      NodesIteratorType it_node = rNodes.begin() + k;
      noalias(it_node->FastGetSolutionStepValue(Variable)) = ZeroVector(3);
    }
    KRATOS_CATCH("")
  }
...
#pragma omp parallel for
    for (int k = 0; k < static_cast<int>(rNodes.size()); ++k) {
      NodesIteratorType it_node = rNodes.begin() + k;
      it_node->FastGetSolutionStepValue(Variable) = 0.0;
    }
    KRATOS_CATCH("")
  }

  /**
   * In a node set, sets a vector non-historical variable to zero
   * @param Variable: reference to the vector variable to be set to 0
   * @param rNodes: reference to the objective node set
   */
...
#pragma omp parallel for
    for (int k = 0; k < static_cast<int>(rNodes.size()); ++k) {
      NodesIteratorType it_node = rNodes.begin() + k;
      it_node->SetValue(Variable, ZeroVector(3));
    }
    KRATOS_CATCH("")
  }

  /**
   * In a node set, sets a double non-historical variable to zero
   * @param Variable: reference to the double variable to be set to 0
   * @param rNodes: reference to the objective node set
   */
...
#pragma omp parallel for
   ...
   */
  NodesContainerType SelectNodeList(const Variable<double> &Variable,
                                    const double &value,
                                    NodesContainerType &rOriginNodes) {
    KRATOS_TRY
    NodesContainerType selected_nodes;
    for (NodesIteratorType it_node = rOriginNodes.begin();
         it_node != rOriginNodes.end(); ++it_node) {
  ...
  int CheckVariableExists(const Variable<double> &rVariable,
    ...
    for (NodesIteratorType it_node = rNodes.begin(); it_node != rNodes.end();
         ++it_node) {
      if (it_node->SolutionStepsDataHas(rVariable) == false) {
        KRATOS_ERROR << "Problem on node with Id " << it_node->Id()
                     << "variable " << rVariable << " is not allocated!"
                     << std::endl;
      }
    }
  ...
  void ApplyFixity(const TVarType &rVar, const bool is_fixed,
    ...
    if (rNodes.size() != 0) {
      if (rNodes.begin()->SolutionStepsDataHas(rVar) == false)
        KRATOS_ERROR << "Trying to fix/free a variable that is not in the "
                        "model_part - variable is "
                     << rVar;
...
#pragma omp parallel for
  ...
  /**
   * Loops along a vector data to set its values to the nodes contained in a
   * node set.
   * Note that this function is suitable for scalar historical variables, since
   * each
   * one of the values in the data vector is set to its correspondent node.
   * Besides,
   * the values must be sorted as the nodes are (value i corresponds to node i).
   * @param rVar: reference to the variable to be fixed or freed
   * @param data: data vector. Note that its lenght must equal the number of
   * nodes
  ...
  void ApplyVector(const TVarType &rVar, const Vector &data,
    ...
    if (rNodes.size() != 0 && rNodes.size() == data.size()) {
      if (rNodes.begin()->SolutionStepsDataHas(rVar) == false)
        KRATOS_ERROR << "Trying to set a variable that is not in the "
                        "model_part - variable is "
                     << rVar;
...
#pragma omp parallel for
      for (int k = 0; k < static_cast<int>(rNodes.size()); ++k) {
        NodesIteratorType it_node = rNodes.begin() + k;
        it_node->FastGetSolutionStepValue(rVar) = data[k];
      }
    } else
      KRATOS_ERROR << "There is a mismatch between the size of data array and "
                      "the number of nodes ";
   ...
   * @param rModelPart: reference to the model part that contains the objective
   * node set
   * @return sum_value: summation vector result
   */
  array_1d<double, 3>
  SumNonHistoricalNodeVectorVariable(const Variable<array_1d<double, 3>> &rVar,
...
#pragma omp for
      for (int k = 0; k < static_cast<int>(rModelPart.NumberOfNodes()); ++k) {
        NodesIteratorType it_node = rModelPart.NodesBegin() + k;
...
#pragma omp parallel for reduction(+ : sum_value)
   ...
   */
  array_1d<double, 3>
  SumHistoricalNodeVectorVariable(const Variable<array_1d<double, 3>> &rVar,
                                  ModelPart &rModelPart,
...
#pragma omp parallel for reduction(+ : sum_value)
   ...
   */
  array_1d<double, 3>
  SumConditionVectorVariable(const Variable<array_1d<double, 3>> &rVar,
                             ModelPart &rModelPart) {
    KRATOS_TRY
...
#pragma omp for
      for (int k = 0; k < static_cast<int>(rModelPart.NumberOfConditions());
           ++k) {
        ConditionsIteratorType it_cond = rModelPart.ConditionsBegin() + k;
...
#pragma omp parallel for reduction(+ : sum_value)
   ...
   */
  array_1d<double, 3>
  SumElementVectorVariable(const Variable<array_1d<double, 3>> &rVar,
                           ModelPart &rModelPart) {
    KRATOS_TRY
...
#pragma omp for
      for (int k = 0; k < static_cast<int>(rModelPart.NumberOfElements());
           ++k) {
        ElementsIteratorType it_elem = rModelPart.ElementsBegin() + k;
...
#pragma omp atomic
   ...
   */
  template <class TVarType>
  double SumElementScalarVariable(const TVarType &rVar, ModelPart &rModelPart) {
    KRATOS_TRY
...
#pragma omp parallel for reduction(+ : sum_value)
  ...
  void AddDof(const TVarType &rVar, ModelPart &rModelPart) {
    ...
    if (rVar.Key() == 0)
      KRATOS_ERROR
          << " Variable : " << rVar
          << " has a 0 key. Check if the application was correctly registered.";
...
#pragma omp parallel for
  ...
  void AddDofWithReaction(const TVarType &rVar, const TVarType &rReactionVar,
    ...
    if (rModelPart.NumberOfNodes() != 0) {
      ...
      if (rModelPart.NodesBegin()->SolutionStepsDataHas(rReactionVar) ==
          false) {
        KRATOS_ERROR << " Variable : " << rReactionVar
                     << "not included in the Soluttion step data ";
      }
    }
...
#ifdef KRATOS_DEBUG
      if (it_node->SolutionStepsDataHas(rVar) == false) {
        KRATOS_ERROR << " Variable : " << rVar
                     << "not included in the Soluttion step data ";
      }
      if (it_node->SolutionStepsDataHas(rReactionVar) == false) {
        KRATOS_ERROR << " Variable : " << rReactionVar
                     << "not included in the Soluttion step data ";
      }
#endif
  ...
  bool CheckDofs(ModelPart &rModelPart) {
    ...
    for (auto &node : rModelPart.Nodes()) {
      for (auto &dof : node.GetDofs()) {
        ...
        //	KRATOS_ERROR << "node : " << node << " does not have allocated
        //space for the variable " << dof << std::endl;
        if (dof.GetVariable().Key() == 0) {
          KRATOS_ERROR << "found a zero key on a dof of node " << node
                       << std::endl;
        }
      }
    }
...
private:
  ...
  /**
   * This method is used in the check variable key method
   */
  template <class TVarType> bool CheckVariableKeysHelper() {
    KRATOS_TRY

    for (const auto &var : KratosComponents<TVarType>::GetComponents()) {
      if (var.first == "NONE" || var.first == "") {
        std::cout << " var first is NONE or empty " << var.first << var.second
                  << std::endl;
      }
      if (var.second->Name() == "NONE" || var.second->Name() == "") {
        std::cout << var.first << var.second << std::endl;
      }
      if (var.first != var.second->Name()) // name of registration does not
                                           // correspond to the var name
      {
        std::cout << "Registration Name = " << var.first
                  << " Variable Name = " << std::endl;
      }
      if (var.second->Key() == 0) {
        KRATOS_ERROR << "found a key of zero for the variable registered with "
                        "the name : "
                     << var.first << std::endl;
        return false;
      }
