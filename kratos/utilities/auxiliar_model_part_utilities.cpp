...
std::unordered_map<IndexType, std::vector<IndexType>> AuxiliarModelPartUtilities::RetrieveConditionsNeighbourConditionsIds()
{
    ...
    for (auto& r_cond : mrModelPart.Conditions()) {
        ...
        KRATOS_ERROR_IF_NOT(static_cast<int>(r_neighbours.size()) == dim) << "Condition " << r_cond.Id() << " has not correct size solution for NEIGHBOUR_CONDITIONS " << r_neighbours.size() << " vs " << dim << std::endl;
