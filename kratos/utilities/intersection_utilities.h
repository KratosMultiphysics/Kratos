...
public:
    ...
    static void ComputeTriangleLineIntersectionInTheSamePlane(
        ...
        IntersectionUtilitiesTetrahedraLineIntersectionStatus& rSolution,
        const double Epsilon = 1e-12
        ) 
    {
            ...
            if (check_1 == 2 || check_2 == 2) { // Aligned
                ...
                if ((diff1m < Epsilon || diff1p < Epsilon) && (diff2m < Epsilon || diff2p < Epsilon)) {
                    ...
                    if (is_inside_projected(r_edge, rLinePoint1)) { // Is inside the line
                        if (rSolution == IntersectionUtilitiesTetrahedraLineIntersectionStatus::NO_INTERSECTION) {
                            noalias(rIntersectionPoint1) = rLinePoint1;
                            rSolution = IntersectionUtilitiesTetrahedraLineIntersectionStatus::ONE_POINTS_INTERSECTION;
                        } else {
                            if (norm_2(rIntersectionPoint1 - rLinePoint1) > Epsilon) { // Must be different from the first one
                                ...
                                rSolution = IntersectionUtilitiesTetrahedraLineIntersectionStatus::TWO_POINTS_INTERSECTION;
                                break;
                            }
                        }
                    ...
                    // Second point
                    if (rSolution == IntersectionUtilitiesTetrahedraLineIntersectionStatus::ONE_POINTS_INTERSECTION) {
    ...
    static IntersectionUtilitiesTetrahedraLineIntersectionStatus ComputeTetrahedraLineIntersection(
    ...
    {
        IntersectionUtilitiesTetrahedraLineIntersectionStatus solution = IntersectionUtilitiesTetrahedraLineIntersectionStatus::NO_INTERSECTION;
        for (auto& r_face : rTetrahedraGeometry.GenerateFaces()) {
            ...
            if (face_solution == 1) { // The line intersects the face
                if (solution == IntersectionUtilitiesTetrahedraLineIntersectionStatus::NO_INTERSECTION) {
                    noalias(rIntersectionPoint1) = intersection_point;
                    solution = IntersectionUtilitiesTetrahedraLineIntersectionStatus::ONE_POINTS_INTERSECTION;
                } else {
                    if (norm_2(rIntersectionPoint1 - intersection_point) > Epsilon) { // Must be different from the first one
                        noalias(rIntersectionPoint2) = intersection_point;
                        solution = IntersectionUtilitiesTetrahedraLineIntersectionStatus::TWO_POINTS_INTERSECTION;
                        break;
                    }
                }
                ...
                if (solution == IntersectionUtilitiesTetrahedraLineIntersectionStatus::TWO_POINTS_INTERSECTION) break;
            }
        }
        ...
        if constexpr (TConsiderInsidePoints) {
            if (solution == IntersectionUtilitiesTetrahedraLineIntersectionStatus::NO_INTERSECTION) {
                ...
                if (rTetrahedraGeometry.IsInside(rLinePoint1, local_coordinates)) {
                    noalias(rIntersectionPoint1) = rLinePoint1;
                    solution = IntersectionUtilitiesTetrahedraLineIntersectionStatus::TWO_POINTS_INTERSECTION_ONE_INSIDE;
                }
                if (rTetrahedraGeometry.IsInside(rLinePoint2, local_coordinates)) {
                    if (solution == IntersectionUtilitiesTetrahedraLineIntersectionStatus::NO_INTERSECTION) {
                        ...
                        noalias(rIntersectionPoint2) = rLinePoint2;
                        solution = IntersectionUtilitiesTetrahedraLineIntersectionStatus::TWO_POINTS_INTERSECTION_BOTH_INSIDE;
                    }
                }
            } else if (solution == IntersectionUtilitiesTetrahedraLineIntersectionStatus::ONE_POINTS_INTERSECTION) {
                array_1d<double,3> local_coordinates;
                if (rTetrahedraGeometry.IsInside(rLinePoint1, local_coordinates)) {
                    if (norm_2(rIntersectionPoint1 - rLinePoint1) > Epsilon) { // Must be different from the first one
                        noalias(rIntersectionPoint2) = rLinePoint1;
                ...
                if (solution == IntersectionUtilitiesTetrahedraLineIntersectionStatus::ONE_POINTS_INTERSECTION) {
                    if (rTetrahedraGeometry.IsInside(rLinePoint2, local_coordinates)) {
                        if (norm_2(rIntersectionPoint1 - rLinePoint2) > Epsilon) {  // Must be different from the first one
                            noalias(rIntersectionPoint2) = rLinePoint2;
                            solution = IntersectionUtilitiesTetrahedraLineIntersectionStatus::TWO_POINTS_INTERSECTION_ONE_INSIDE;
