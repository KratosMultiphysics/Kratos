...
// header includes
#include "local_refined_brep_trimming_utilities.h"
...
namespace Kratos
{
    ...
    void LocalRefinedBrepTrimmingUtilities<TShiftedBoundary>::CreateBrepSurfaceTrimmingIntegrationPoints(
        ...
        for (IndexType i_outer_loops = 0; i_outer_loops < rOuterLoops.size(); ++i_outer_loops) {
            ...
            for (IndexType j = 0; j < rOuterLoops[i_outer_loops].size(); ++j) {
                ...
                for (IndexType u = 0; u < tesselation.size(); ++u) {
                    auto new_int_point = LocalRefinedBrepTrimmingUtilities::ToIntPoint(std::get<1>(tesselation[u])[0], std::get<1>(tesselation[u])[1], factor);
            ...
            for (IndexType i = 0; i < rSpansU.size() - 1; ++i) {
                for (IndexType j = 0; j < rSpansV.size() - 1; ++j) {
                    ...
                    if (solution_outer.size() == 0 || clip_area/span_area < 1e-6) {
                        ...
                        {
                            LocalRefinedBrepTrimmingUtilities::Triangulate_OPT(solution_inner[i], triangles, factor, span_area);
                        }
    ...
    template<bool TShiftedBoundary>
    void LocalRefinedBrepTrimmingUtilities<TShiftedBoundary>::ComputeSpanTriangulation(
        const DenseVector<DenseVector<BrepCurveOnLocalRefinedSurfacePointerType>>& rOuterLoops,
        const DenseVector<DenseVector<BrepCurveOnLocalRefinedSurfacePointerType>>& rInnerLoops,
        const double u0,
        const double u1,
        ...
        std::vector<Matrix>& rTriangles)
    {
        ...
        for (IndexType i_outer_loops = 0; i_outer_loops < rOuterLoops.size(); ++i_outer_loops)
        {
            ...
            for (IndexType j = 0; j < rOuterLoops[i_outer_loops].size(); ++j)
            {
                ...
                for (IndexType u = 0; u < tesselation.size(); ++u)
                {
                    auto new_int_point = LocalRefinedBrepTrimmingUtilities::ToIntPoint(
            ...
            else
            {
                ...
                for (IndexType i = 0; i < solution_inner.size(); ++i)
                {
                    LocalRefinedBrepTrimmingUtilities::Triangulate_OPT(
                        solution_inner[i],
                        rTriangles,
                        factor,
                        span_area);
    ...
    template class KRATOS_API(KRATOS_CORE) LocalRefinedBrepTrimmingUtilities<true>;
    template class KRATOS_API(KRATOS_CORE) LocalRefinedBrepTrimmingUtilities<false>;
} // namespace Kratos.
