...
//     }
//     bool is_outer_loop_defined = false;
//     if (rSurrogateOuterLoopGeometries.Conditions().size() > 0) {is_outer_loop_defined = true;}
    
//     for (auto i_cond : rSurrogateOuterLoopGeometries.Conditions()) {
//         // Check if the condition is perpendicular to x-direction
//         if (std::abs((*p_geometry)[0].X()-i_cond.GetGeometry()[2].X()) < 1e-13 ) {

//             // When local refining is performed, the surrogate steps might be larger that the refined knot spans
//             const double direction_v = (i_cond.GetGeometry()[2].Y()-(*p_geometry)[0].Y())/std::abs((i_cond.GetGeometry()[2].Y()-(*p_geometry)[0].Y()));
...
//             const double direction_w = (i_cond.GetGeometry()[2].Z()-(*p_geometry)[0].Z())/std::abs((i_cond.GetGeometry()[2].Z()-(*p_geometry)[0].Z()));
...
//     for (auto i_cond : rSurrogateInnerLoopGeometries.Conditions()) {
//         // Check if the condition is perpendicular to x-direction
//         if (std::abs((*p_geometry)[0].X()-i_cond.GetGeometry()[2].X()) < 1e-13 ) {
