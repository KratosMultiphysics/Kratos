...
public:
    ...
    void AddConstraint(Args&&... args)
    {
        if (mrModelPart.IsDistributed())
            AddDistributedConstraint(std::forward<Args>(args)...);
        else
            mrModelPart.CreateNewMasterSlaveConstraint(std::forward<Args>(args)...);
    }
...
private:
    ...
    public:
        BaseMasterSlaveDetails(std::string ConstraintName,
        ...
        virtual IndexType Id()
        {
            return mId;
        }
        virtual IndexType SlaveNodeId()
        {
            return mSlaveNodeId;
        }
        virtual IndexType MasterNodeId()
        {
            return mMasterNodeId;
        }
        virtual double Constant()
        {
            return mConstant;
        }
        ...
        virtual IndexType GetSlaveVariableKey() = 0;
        virtual IndexType GetMasterVariableKey() = 0;
    ...
    private:
        ...
        double mWeight;
        double mConstant;
        std::string mConstraintName;
    };

    template <class TSlaveVariableType, class TMasterVariableType>
    class MasterSlaveDetails : public BaseMasterSlaveDetails {
    public:
        typedef TSlaveVariableType SlaveVariableType;
        typedef TMasterVariableType MasterVariableType;

        MasterSlaveDetails(const std::string ConstraintName,
                           IndexType ConstraintId,
                           IndexType MasterNodeId,
        ...
        IndexType GetSlaveVariableKey() override
        {
            return mrSlaveVariable.Key();
        }

        IndexType GetMasterVariableKey() override
        {
            return mrMasterVariable.Key();
        }

    private:
        TSlaveVariableType& mrSlaveVariable;
        TMasterVariableType& mrMasterVariable;
    };
    ...
    void CreateConstraints()
    {
        ...
        GlobalPointerCommunicator<Node<3>> nodes_pointer_comm(
            r_data_communicator, vector_of_node_global_pointers);
        auto nodes_result_proxy = nodes_pointer_comm.Apply([](GlobalPointer<Node<3>>& gp) {
            ...
            for (auto& dof : gp->GetDofs()) {
                ...
                vec_of_dofs_global_ptr.push_back(std::make_pair(
                    dof.GetVariable().Key(),
                    GlobalPointer<DofType>(
                        dof_shr_ptr, gp->FastGetSolutionStepValue(PARTITION_INDEX))));
            }
        ...
        for (const auto& constraint_info : mMasterSlaveDetailsVector) {
            ...
            auto slave_node_var_dofs_gps_map =
                nodes_result_proxy.Get(remote_nodes_gps_map[r_slave_node_id]);
            auto master_node_var_dofs_gps_map =
            ...
            // mrModelPart.CreateNewMasterSlaveConstraint(
            //     "LinearMasterSlaveConstraint", constraint_info->Id(), master_dof_vector,
            //     slave_dof_vector, relation_matrix, constant_vector);
        }
