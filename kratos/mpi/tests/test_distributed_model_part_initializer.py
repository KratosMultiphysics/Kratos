...
class TestDistributedModelPartInitializer(KratosUnittest.TestCase):
    ...
    def test_with_real_model(self):
        ...
        if KM.DataCommunicator.GetDefault().Rank() == 0:
            KM.ModelPartIO(GetFilePath("auxiliary_files_for_python_unittest/mdpa_files/test_mpi_communicator")).ReadModelPart(model_part)

        DistributedModelPartInitializer(model_part, KM.Testing.GetDefaultDataCommunicator(), 0).Execute()
