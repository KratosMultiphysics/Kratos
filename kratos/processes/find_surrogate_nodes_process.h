...
#if !defined(KRATOS_FIND_SURROGATE_NODES_PROCESS_H_INCLUDED )
#define  KRATOS_FIND_SURROGATE_NODES_PROCESS_H_INCLUDED
...
class KRATOS_API(KRATOS_CORE) FindSurrogateNodesProcess : public CalculateDiscontinuousDistanceToSkinProcess<TDim>
{
...
public:
    ...
    /// Pointer definition of FindSurrogateNodesProcess
    KRATOS_CLASS_POINTER_DEFINITION(FindSurrogateNodesProcess);
    ...
    /// Destructor.
    ~FindSurrogateNodesProcess() override;
    ...
    /// Default constructor.
    FindSurrogateNodesProcess() = delete;;

    /// Copy constructor.
    FindSurrogateNodesProcess(FindSurrogateNodesProcess const& rOther) = delete;

    /// Assignment operator.
    FindSurrogateNodesProcess& operator=(FindSurrogateNodesProcess const& rOther) = delete;
    ...
    std::vector<int> FindClosestElement(
        ModelPart& rVolumePart,
        ModelPart& rSkinPart);
...
private:
    ...
    ///@}
}; // Class FindSurrogateNodesProcess
...
inline std::istream& operator >> (
    ...
    FindSurrogateNodesProcess<>& rThis);

/// output stream function
inline std::ostream& operator << (
    std::ostream& rOStream,
    const FindSurrogateNodesProcess<>& rThis)
{
