...
{
    // write cells header
    rFileStream << "CELLS " << rModelPart.NumberOfConditions() + rModelPart.NumberOfElements() << " " << mVtkCellListSize << "\n";
    ...
    for (const auto& elem : rModelPart.Elements())
    {
        ...
        WriteScalarDataToFile(number_of_nodes, rFileStream);
        for (const auto& node : elem_geometry){
            rFileStream << " ";
            auto id = mKratosIdToVtkId[node.Id()];
...
void VtkOutput::WriteConditionAndElementTypes(const ModelPart &rModelPart, std::ofstream& rFileStream)
{
    ...
    rFileStream << "CELL_TYPES " << rModelPart.NumberOfConditions() + rModelPart.NumberOfElements() << "\n";

    // write elements types
    for (const auto& elem : rModelPart.Elements())
    {
        const unsigned int number_of_nodes = elem.GetGeometry().size();
        ...
        WriteScalarDataToFile(element_type, rFileStream);
        rFileStream << "\n";
    }
    ...
    for (const auto& condition : rModelPart.Conditions())
    {
        const unsigned int number_of_nodes = condition.GetGeometry().size();
        unsigned int element_type;
...
void VtkOutput::WriteVectorDataToFile(const TData& rData, std::ofstream& rFileStream)
{
    ...
    else if (mFileFormat == VtkOutput::FileFormat::VTK_BINARY)
    {
        ...
        rFileStream.write((char *)(&num1), sizeof(num1));
        auto num2 = rData[1];
        ForceBigEndian((unsigned char *)&num2);
        rFileStream.write((char *)(&num2), sizeof(num2));
        auto num3 = rData[2];
        ForceBigEndian((unsigned char *)&num3);
        rFileStream.write((char *)(&num3), sizeof(num3));
    }
