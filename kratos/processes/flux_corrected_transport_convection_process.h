...
public:
    ...
    void Execute() override
    {
        ...
        KRATOS_INFO_IF("FluxCorrectedTransportConvectionProcess", mEchoLevel > 0) << "Solving FCT convection with Dt = " << dt << " (max allowed Dt = " <<  max_dt << ")" << std::endl;

        // Substepping time loop
        for (IndexType step = 1; step <= n_steps; ++step) {
            // Solve current substep
            KRATOS_INFO_IF("FluxCorrectedTransportConvectionProcess", mEchoLevel > 1) << "Substep = " << step << " - Time = " <<  prev_time << " - Dt = " << dt << std::endl;
