...
public:
    ...
    {
        return GetResultaData(GPIndex)[ComponentIndex]->GetNearestValue(Time);
    }
...
public:
        ...
        )
    {
        ...
        auto r_table_generator =[](const SizeType NumberOfEntites, const SizeType NumberOfComponents, const SizeType NumberOfGP){std::vector<Table<double, double>::Pointer> aux_1(NumberOfComponents, nullptr); EntityDatabase aux_2(NumberOfGP, aux_1); VariableDatabase data(NumberOfEntites, aux_2); for (IndexType k = 0; k < NumberOfComponents; ++k){for (IndexType j = 0; j < NumberOfGP; ++j){ for (IndexType i = 0; i < NumberOfComponents; ++i){ data[k][j][i] = Kratos::make_shared<Table<double, double>>();}}}; return data;};
        ...
        for (IndexType i = 0; i < rVariablesIndexes.size(); ++i) {
            ...
            this->insert(std::pair<IndexType, VariableDatabase>(index, r_table_generator(NumberOfEntites, size, NumberOfGP)));
        }
    }
