...
#else
    for (typename MatrixType::iterator2 row_iterator = begin(a_iterator,boost::numeric::ublas::iterator1_tag());
...
#pragma omp parallel for firstprivate(size)
  ...
  inline static void AssembleLHS(MatrixType& A,
...
#pragma omp parallel for
  ...
  {
    rOStream << "Ublas Space";
  }

  /// Print object's data.
  virtual void PrintData(std::ostream& rOStream) const
  {
    rOStream << "Ublas Space Data";
  }
...
inline std::istream& operator >> (std::istream& rIStream,
                                  UblasSpace<TDataType,TMatrixType,TVectorType>& rThis)
{
  return rIStream;
}
