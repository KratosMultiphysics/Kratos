...
for (std::size_t i = 0; i < a.size(); ++i) {                               \
    const double difference = std::abs(a[i] - b[i]);                       \
    if (difference > tolerance) {                                          \
        msg << "Check failed because vector component " << #a << "["       \
            << i << "] = " << a[i] << " is not near vector component "     \
            << #b << "[" << i << "] = " << b[i] << " with tolerance "      \
            << tolerance << " [ absolute difference = " << difference      \
            << " ]." << std::endl;                                         \
        msg << "Vectors are followings:" << std::endl;                     \
        found_mismatch = true;                                             \
        break;                                                             \
    }                                                                      \
}                                                                          \
if (found_mismatch) {                                                      \
    ...
    msg << std::scientific << std::setprecision(precision);                \
    for (std::size_t i = 0; i < a.size(); ++i) {                           \
    ...
    {                                                                                     \
        ...
        for (std::size_t i = 0; i < a.size1(); ++i) {                                     \
            for (std::size_t j = 0; j < a.size2(); ++j) {                                 \
                ...
                if (difference > tolerance) {                                             \
                    msg << "Check failed because matrix component " << #a                 \
                        << "(" << i << ", " << j << ") = " << a(i, j)                     \
                        << " is not near matrix component " << #b << "(" << i             \
                        << ", " << j << ") = " << b(i, j) << " with tolerance "           \
                        << tolerance << " [ absolute difference = " << difference         \
                        << " ]." << std::endl;                                            \
                    msg << "Matrices are followings: " << std::endl;                      \
                    found_mismatch = true;                                                \
                    break;                                                                \
                }                                                                         \
            }                                                                             \
